...
def _git_show(base_ref: str, rel_path: str) -> str | None:
    ...
    if the path did not exist there (a newly-added file). Never raises: a git
    failure other than 'path absent' returns None and the caller treats it as
    'no base to compare', failing open ONLY to the extent that CI still runs the
    whole-tree scan — the append-only check is CI-scoped, gated on the base ref
    env var, exactly like the sibling comment-history / subprocess-encoding
    gates."""
    try:
        proc = subprocess.run(
    ...
    if proc.returncode != 0:
        # Non-zero: the path did not exist at base (newly added) or git errored.
        return None
    return proc.stdout
...
def _git_ls_files(base_ref: str, dir_rel: str) -> set[str]:
    ...
    if the dir did not exist there or git errored). Used to enumerate the
    evidence files that EXISTED at base, so a whole-file deletion at head is
    detected (iterating only head's files would miss it). Never raises."""
    try:
        proc = subprocess.run(
