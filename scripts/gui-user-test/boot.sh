...
# kiro_crew.testing.fake_a2a_server). It is plain HTTP, which the provider
# refuses unless the gateway's environment names the origin -- see
# KIROCREW_A2A_PLAINTEXT_ORIGINS in the gateway env below. It must be listening
# before the gateway serves the first spawn, because A2AProvider fetches the
# Agent Card on start.
...
env \
  ...
  KIROCREW_A2A_PLAINTEXT_ORIGINS="http://127.0.0.1:${GUI_A2A_PORT}" \
