...
class ChannelHistory:
    ...
    def set_observe(self, channel_id: str) -> None:
        ...
        # cancelled above. Gated on a terminal op having been scheduled for
        # this channel in THIS process (_flush_epoch): only a real observe
        # off-then-on transition can have an unlink in flight, so gateway
        # boot — where _init_services calls set_observe for every persisted
        # observe channel before the dashboard binds — schedules no
        # data-scaled serialization work.
        buf = self._channels.get(channel_id)
        if buf and channel_id in self._flush_epoch:
    ...
    def _load_observe(self, channel_id: str) -> None:
        ...
        def _identity(e: HistoryEntry) -> object:
            # wall_ts keeps two distinct ts-less messages apart: they share
            # author/text/thread only when they also arrived at the same
            # wall-clock instant, while a reloaded copy of one persisted
            # entry carries the identical persisted ts and still dedupes.
