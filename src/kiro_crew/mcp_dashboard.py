...
def _chat_folder_name_key(folder: dict) -> bytes:
    ...
    No case folding. ``str.lower()`` and ``String.prototype.toLowerCase()`` each
    read their own runtime's Unicode tables, so a character whose case mapping was
    added or changed between the interpreter's version and the browser's folds
    differently on the two sides — a skew no code here can close, because neither
    side owns both tables. Comparing unfolded means the tables are never consulted.
    The cost is that this tie-break is case-sensitive, which is not a property the
    ordering needs: ``order`` is the sort key, the name only separates rows that
    share one, and any placement or drag renumbers siblings to distinct values.
...
def _chat_folder_siblings(folders: list[dict], parent_id: str) -> list[dict]:
    ...
    The name key is ``_chat_folder_name_key``, which compares the raw UTF-16
    encoding: no case fold, so the tie-break is CASE-SENSITIVE and ``Beta`` sorts
    ahead of ``alpha``. Folding would read each runtime's own Unicode tables, and
    the sidebar's runtime is not this one — see that function for why the two
    cannot be made to agree on a fold.
    """
