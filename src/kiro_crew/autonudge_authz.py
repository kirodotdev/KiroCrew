...
async def authorize_and_add_nudge(
    *,
    ...
    if self_armed:
        ...
        try:
            assert reserved_loop_id is not None
            # Scheduled composer text can wait long enough for its outbound
            # audience to change. Capture the same admission contract queued
            # user speech uses; fire time revalidates it before dispatch.
            from kiro_crew.dashboard.session_control import containment_meta

            current_slot = state._slots.get(slot_key)
            if current_slot is not authorized_slot:
                return _deny("session changed before nudge arm committed", 409)
            scheduled_containment = containment_meta(cast(Any, state), cast(Any, current_slot))
            await asyncio.to_thread(
                record_scheduled_message,
