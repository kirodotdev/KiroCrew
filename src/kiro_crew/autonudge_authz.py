...
def banner_is_echoed_projection(current: Any, banner: Any) -> bool:
    ...
    the operator's text, irreversibly.

    The stored-vs-projection comparison is the part the message guard does not need: a
    banner the scrub left ALONE projects to itself, so an equal incoming value is an
    idempotent set (``PATCH`` re-sending a row to quiet a loop), not data loss.
