...
Two transports, same copy loop:

* unix: ``client.py /tmp/kirocrew-mcp-bridge/<token>.<sub>.sock``
  (native Linux; bind-mounted AF_UNIX shares a kernel with the gateway)
* tcp: ``client.py tcp host.docker.internal <port>``
...
def _connect(args: list[str]) -> socket.socket:
    if len(args) == 1 and args[0] and args[0] != "tcp":
        sock = socket.socket(socket.AF_UNIX, socket.SOCK_STREAM)
        sock.connect(args[0])
        return sock
    if len(args) == 3 and args[0] == "tcp" and args[1] and args[2].isdigit():
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        sys.stderr.write("usage: client.py <unix-socket-path> | client.py tcp <host> <port>\n")
        return 2
