...
from kiro_crew.agent_sdk.backends import agent_process_markers
...
# its own change with its own review -- see the deferred follow-up, which carries the
# projection to reuse. Until then a codex-acp, opencode, pi-acp, goose or dsh tree in an
...
def _harness_naming_tokens(cmdline: bytes) -> list[bytes]:
    ...
    Two, at most, and never an arbitrary argument.

    - argv0, always: the file the kernel actually execed.
    - the first non-flag token AFTER argv0, and only when argv0's basename is an
      interpreter from :data:`_HARNESS_INTERPRETERS`. That is the script slot of the
      shebang shape above, which is the only way a bespoke adapter appears at all.

    What is excluded is the rest of argv. A process's arguments are chosen by whoever
    started it and say nothing about what it IS, so ``node build.js --agent goose`` or
    an editor opened on a file called ``dsh`` would otherwise be answered "this is a
    harness" -- and on the reclaim path that answer authorizes a SIGKILL of a PID this
    ...
    if not tokens:
        return []
    naming = [tokens[0]]
    if _basename_of(tokens[0]) in _HARNESS_INTERPRETERS:
...
# Entry-script filenames that name a PACKAGE rather than a program. A Node adapter can be
# launched either through its installed bin shim (``node /opt/n/bin/codex-acp``) or through
# the package entry the resolver builds -- ``_CODEX_ACP_PKG_ENTRY`` in ``acp/client.py`` is
# ``<pkg>/dist/index.js`` -- and in that second shape the basename is ``index.js``, which
# names nothing.
_PACKAGE_ENTRY_FILENAMES: frozenset[bytes] = frozenset({b"index.js", b"index.mjs", b"index.cjs"})
# Build directories to step over on the way from such an entry file to the package
# directory that carries the adapter's name.
_PACKAGE_BUILD_DIRS: frozenset[bytes] = frozenset({b"dist", b"lib", b"build", b"out", b"src"})


def _package_dir_name(token: bytes) -> bytes:
    """The package directory name for a ``<pkg>/dist/index.js``-shaped script path.

    ``b""`` for anything else. Walking UP from a generic entry filename, past the build
    directory, reaches the one segment that carries the adapter's name -- and stopping
    there is what keeps this from becoming "match any path segment", which would accept a
    harness name from any directory a script happens to live under.
    """
    segments = [seg for seg in token.replace(b"\\", b"/").split(b"/") if seg]
    if len(segments) < 2 or segments[-1] not in _PACKAGE_ENTRY_FILENAMES:
        return b""
    for segment in reversed(segments[:-1]):
        if segment in _PACKAGE_BUILD_DIRS:
            continue
        return segment
    return b""
...
def _cmdline_names_a_harness(cmdline: bytes) -> bool:
    ...
    slot of an interpreter-hosted adapter. For that script slot the basename is tried
    first and then, when the basename is a generic package entry file, the package
    directory that carries the adapter's name (:func:`_package_dir_name`): the resolver
    hands the adapter to Node either way, so recognising only the bin-shim spelling left
    the ``dist/index.js`` launch unreclaimable.
    """
    for token in _harness_naming_tokens(cmdline):
        if _basename_names_a_harness(_basename_of(token)):
            return True
        package = _package_dir_name(token)
        if package and _basename_names_a_harness(package):
            return True
    return False
...
def _is_managed_agent_process(pid: int) -> bool:
    ...
    identity independent of argv rather than a better name match. Tracked in the
    deferred follow-up.
    """
