...
def create_upload_file(name: str, data: bytes) -> Path:
    ...
    the uploads directory itself is pinned with
    :func:`kiro_crew.platform_compat.pin_directory` before the by-name create.
    Windows holds that directory handle until after the last byte is written, so
    neither the directory nor its ancestors can be swapped, and the pin refuses
    a link or junction already sitting at ``uploads/``.
    ...
    if pinned_fs.supports_pinned_walk():
        ...
        pin_fd = platform_compat.pin_directory(directory)
    try:
        if dir_fd >= 0:
