...
* ``verifies_agent_activation`` is True only for an ``agent_spec``-routed
  descriptor that actually puts an agent in its argv (``agent_args`` carrying
  ``{agent}`` -- which :func:`descriptor.validate_descriptor` requires of every
  ``agent_spec`` routing, so a registered descriptor cannot claim the routing
  without the block): that is the only configuration where a ``--agent``-style
  selection was made at spawn for a later check to confirm, mirroring
  ``kiro.py`` True / ``codex.py``/``kas.py`` False.

The remaining seams inherit :class:`MembershipHarness`'s membership lookups. A
registered descriptor id is in none of the frozen ``ACP_BACKENDS_*`` vocabulary
sets, so ``internal_sandbox`` / ``pod_home_remap`` / ``reads_markdown_agent_specs``
all answer False and ``reclaim_policy`` passes the operator's thresholds through --
which is exactly the fail-safe posture a host that has demonstrated nothing
should get. There is no config-side exception: a descriptor carries no
``capabilities`` block, so a registered host is a member of NO session-path
capability set (``ACP_BACKENDS_SESSION_MCP_ARRAY``, ``_HARNESS_OWNED_SESSIONS``,
``_LOAD_WITHOUT_MODES``, ``_MODEL_VIA_CONFIG_OPTION``,
``_ADVERTISED_MODEL_SELECTION`` included) and takes the default branch at every
gate that reads one. Admitting a harness to a capability is a code-reviewed edit
to the named frozen set (harness-parity H6/H7).
...
class DescriptorHarness(MembershipHarness):
    ...
    async def resolve_spawn(self, ctx: SpawnContext) -> SpawnPlan:
        ...
        # The routing attestation was bound to the BYTES that were verified, not
        # to the path -- and a check followed by an exec of the operator's path
        # would still leave a window between the two for the file to be swapped.
        # So the spawn reads the file once, judges those bytes, and execs a
        # gateway-private copy of exactly them (routing_verification.
        # pin_verified_executable): verification and execution share one file
        # identity. A mismatch refuses this spawn and withdraws selectability
        # until the operator verifies the replacement.
        from kiro_crew.acp.harness.operator_registry import revoke_routing_verification_async
        from kiro_crew.acp.harness.routing_verification import pin_verified_executable

        # The spawn's working directory identifies the probe's own spawn (the
        # probe's private scratch); every other spawn is held to the record.
        work_dir = str(ctx.work_dir) if ctx.work_dir is not None else None
        pinned, problem = await asyncio.to_thread(
            pin_verified_executable, self._descriptor, exe, work_dir=work_dir
        )
        if problem is not None or pinned is None:
            problem = problem or "the verified executable could not be pinned for execution"
            try:
                # The store rewrite runs in a worker thread; only the registry
                # withdrawal touches this loop.
                await revoke_routing_verification_async(self._descriptor.id, problem)
            except ValueError:
                # Not registered (a descriptor constructed directly, e.g. by the
                # probe itself): nothing to withdraw, the refusal below stands.
                pass
            raise AcpRuntimeError(f"harness {self._descriptor.id!r} refused to spawn: {problem}")
        # The attestation vouches for the agent the probe ran under, not for every
        # agent: the permission posture of an agent_spec host IS its agent
        # selection, and any descriptor may hand the name to the binary. A spawn
        # for an agent the record does not name is refused -- this spawn only; the
        # backend stays verified for the agents it was probed with.
        from kiro_crew.acp.harness.routing_verification import agent_attestation_problem

        agent_problem = await asyncio.to_thread(
            agent_attestation_problem, self._descriptor, ctx.agent, work_dir=work_dir
        )
        if agent_problem is not None:
            raise AcpRuntimeError(
                f"harness {self._descriptor.id!r} refused to spawn: {agent_problem}"
            )
        argv = render_argv(
            self._descriptor,
            executable=pinned,
    ...
    def verifies_agent_activation(self) -> bool:
        ...
        The descriptor must be ``agent_spec``-routed (its permission decision
        travels as an agent selection), and it must carry an ``agent_args``
        block -- the argv fragment that puts the agent name on the command line.
        :func:`descriptor.validate_descriptor` refuses an ``agent_spec`` routing
        whose ``agent_args`` cannot deliver ``{agent}``, so for a REGISTERED
        descriptor the two conditions coincide; the block is still read here so
        this answer never depends on how the descriptor reached this object.
        With the block, the spawn is the only thing that selected the agent, so
        the activation must be verified (the ``kiro.py`` answer); a
        ``session_config``-routed descriptor selects nothing at spawn and has no
        activation to confirm (the ``codex.py`` / ``kas.py`` answer).
