...
def harness_for(backend: str) -> HarnessAdapter:
    ...
    Consults the builtin literal ``_HARNESSES`` FIRST, by DIRECT subscript, and
    the operator register only after that lookup raises ``KeyError``. The shape
    is load-bearing: kiro's construction path is the literal's own subscript with
    no conditional in front of it (H13) -- the default path resolves through the
    literal and never touches the operator register at all -- while a
    config-authored backend is still resolvable through the same one function
