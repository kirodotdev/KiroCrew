...
class AcpRuntime:
    ...
    async def _signal_tree(
        ...
        *,
        ...
        if reached:
        ...
        elif not group_vouching_available():
            # Nothing ran that could reach this tree: the number is not provably
            # ours, and the vouched read that would reach the members by token is
            # Linux-only. The tree is leaked to the orphan sweep -- the deliberate
            # trade, a leak over a signal to a stranger -- but a silent return made
            # that trade invisible in the field, where it reads as a teardown that
            # worked. Reporting only, never routing: the attempt above is
            # unconditional on POSIX and _marked_group_members owns the platform
            # answer, so this decides what to SAY about {} and nothing else.
            #
            # And only when something is plausibly still there. A root that exited
            # cleanly before a routine kill() leaves an empty group, and a line on
            # that path would be noise on the ordinary teardown -- the same reason
            # a Linux host that reaches no member stays quiet.
            # _pgroup_has_member_besides answers on every platform and is
            # conservative on a failed read, so an unreadable group still speaks.
            # It scans /proc or sysctl, hence the executor.
            ...
            if leaked:
                logger.warning(
                    ...
                    "%s, and this host cannot vouch a process group by incarnation "
                    "token, so its tree is left to the orphan sweep",
                    pid,
                    sig,
