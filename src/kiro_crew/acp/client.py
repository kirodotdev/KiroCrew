...
class AcpModelUnavailable(AcpError):  # noqa: N818
    ...
    def __init__(self, model_id: str, advertised: Sequence[str] | None = None) -> None:
        ...
        if model_id in self.advertised:
            # The adapter ADVERTISED this id and then refused it: that is a
            # spelling gap between the adapter's list and its switch channel (a
            # codex ``<model>[<effort>]`` pair its ``model`` option does not take),
            # not an entitlement verdict. Pointing the user at their sign-in here
...
async def _push_model_via_effort_split(driver: Any, backend: str, model_id: str) -> str:
    ...
    if not driver.supports_config_option(REASONING_EFFORT_CONFIG_ID):
        logger.warning(
            "ACP model %s applied as %s; adapter exposes no %r option, effort %r not applied",
            _model_log,
            _base_log,
            REASONING_EFFORT_CONFIG_ID,
            effort,
        ...
        return applied_base
    try:
        await driver.set_config_option(REASONING_EFFORT_CONFIG_ID, effort)
    except AcpError as exc:
        if "unknown config option" not in str(exc).lower() and not _is_config_value_rejection(
            exc, REASONING_EFFORT_CONFIG_ID
        ):
            raise  # transport/protocol failure -- the model DID switch, but do not hide this
        logger.warning(
            "ACP model %s applied as %s; adapter refused effort %r, keeping its default",
            _model_log,
            _base_log,
            effort,
        )
