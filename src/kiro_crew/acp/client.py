...
class AcpClient:
    ...
    def _permission_surface_governed(self) -> bool:
        ...
        failed re-validation to keep the file pinned for a later re-earn.
        ...
        **Crew touches only the file it owns.** Ownership is not the path -- a
        ...
        if authored and not self._claude_settings_is_still_ours():
            ...
            if seed_provenance.release(local_settings, self._seed_owner):
                ...
                return
            logger.warning(
                "could not durably release Crew's claim on %s; retaining authorship so "
                "teardown retries",
                local_settings,
            )
            return  # keep _claude_settings_authored=True so _discard/_settle retries
