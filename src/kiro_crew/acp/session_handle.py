...
# Retained length of a backend-supplied session recap (EVENT_SESSION_RECAP).
# The text is LLM-influenced and persisted per transcript row, so it is bounded
# like every other retained backend field in this module; the recap is a short
# re-orientation line (goal / task / next step), which this holds with room.
...
class AcpSessionHandle:
        ...
        *,
        ...
        while True:
            ...
            if (
                ...
                # (An at-return push, before any turn, is the declared
                # follow-up: the slot-focus eager spawn (chat_runner
                # schedule_eager_spawn, allow_resume=True) already runs
                # session/load at the moment of return and can consume the
                # staged recap there; this turn-start path stays the fallback
                # for installs with eager spawn off. This closes the "or
                # never" case.)
    ...
    def _handle_kas_session_info(self, update: dict) -> list[AcpEvent]:
        ...
        if kind == kas_wire.KIND_RECAP:
            ...
            # echoed, LLM-influenced text: bound before redaction (the [:4096]
            # pre-redaction idiom) and cap what is retained, since this text is
            # persisted per transcript row and every arrival window routes here.
            recap_text = redact_text(str(kiro.get(kas_wire.FIELD_TEXT) or "")[:4096])[
                :_RECAP_MAX_CHARS
            ].strip()
            if not recap_text:
                return []
