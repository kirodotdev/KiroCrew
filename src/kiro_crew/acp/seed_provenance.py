...
def _persist(
    ...
    *pending* is the entry a :func:`record` is trying to make durable, supplied as an
    ...
    try:
        ...
        # lock-free read stops reporting a path whose file is gone. A key with a
        # LIVE SHARER is exempt even when its file is missing: the record is what
        # the sharer's repair validates its byte-identical re-creation against,
        # and pruning it would leave the sharer governed with no path back to a
        # restorable seed.
        for key in [
            k for k in list(_RECORDS) if k != keep and not _SHARERS.get(k) and not os.path.isfile(k)
        ]:
        ...
        with _cross_process_lock():
            ...
            # ``keep`` exempts this transaction's own just-written key, and a key a
            # live sharer pins is kept for the same repair reason as above.
            for key in [
                k
                for k in list(merged)
                if k != keep and not _SHARERS.get(k) and not os.path.isfile(k)
            ]:
                merged.pop(key, None)
...
def held_by_another(path: Path | str, owner: str) -> bool:
    """``True`` when a DIFFERENT session's live client currently holds *path*.

    The create path has no :func:`claim` of its own (``O_EXCL`` arbitrates the
    on-disk race), so this is its guard against authorship under a LIVE owner:
    a sibling recreating a vanished seed would otherwise have :func:`record`
    displace the owner's live slot, and the sibling's teardown would then
    remove a file the owner still governs.
    """
    holder = _LIVE.get(_key(path))
