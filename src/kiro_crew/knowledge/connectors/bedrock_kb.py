...
def _refuse_withdrawn(kb_id: str) -> NoReturn:
    """Refuse one paid Retrieve whose grant is gone, and audit that refusal.
    ...
    the ones the gate turned away up front.
    """
    aws_consent.audit_decision(
        aws_consent.SERVICE_BEDROCK_KB,
        outcome="denied",
        detail=f"retrieve refused mid-search: grant withdrawn or replaced (kb={kb_id})",
    )
...
def _retrieve_one(
    ...
    *,
    ...
    try:
        ...
        # the per-KB loop applies, and the same re-assertion of the grant.
