...
class HistoryConsolidator:
    ...
    def _renew_consolidation_lease(self, key: str, token: str) -> bool:
        ...
        Returns False when the on-disk lease token differs from *token*, which
        is not an error the caller can act on — the pass is already unguarded
        at that point and stopping it would waste a turn that is nearly always
        about to finish.
