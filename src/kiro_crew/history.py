        ...
        *mid* persists the row's delivery identity as ``meta.mid`` — the SAME
        ...
        with self._locked(key):
            ...
            msg: dict = {
                "role": role,
                "content": scrubbed,
                # No original is kept, so without this a false positive is indistinguishable
                # from the author's words. Set for a row scrubbed by ANY caller.
                **({"redacted": True} if carries_redaction_marker(scrubbed) else {}),
                **({"cls": cls} if cls else {}),
