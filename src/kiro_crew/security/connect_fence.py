...
# Datagram/stream send syscalls whose flags word can carry MSG_FASTOPEN.
# Keys mirror _ARCH_TABLE exactly: arch_entry() gates membership, so a
# machine that resolves there always resolves here.
_SEND_TABLE: dict[str, tuple[int, int, int]] = {
    # machine -> (__NR_sendto, __NR_sendmsg, __NR_sendmmsg)
    "x86_64": (44, 46, 307),
    "aarch64": (206, 211, 269),
}
...
_SECCOMP_DATA_ARG2_OFFSET = 32  # args[2]: sendmsg flags word
_SECCOMP_DATA_ARG3_OFFSET = 40  # args[3]: sendto/sendmmsg flags word

#: TCP Fast Open initiates the TCP handshake inside the send call itself
#: (``tcp_sendmsg_fastopen``), so a connection opened this way never invokes
#: ``connect(2)`` and the notify trap cannot adjudicate its target. The
#: program denies the flag outright on every send-family syscall: the fence
#: cannot read a sockaddr it is never notified about, and fail-closed beats
#: an unfenced path to the host's own ports.
...
def build_connect_notif_prog(machine: str) -> bytes:
    ...
    space with ENOSYS, deny MSG_FASTOPEN sends with EPERM, allow everything
    else.
    ...
    ``sendto``/``sendmsg``/``sendmmsg`` pass through UNLESS their flags word
    carries MSG_FASTOPEN, which returns EPERM: a Fast Open send performs the
    TCP handshake inside the send path without ever invoking ``connect``, so
    the notify trap never sees the target and the only fail-closed answer is
    to refuse the flag itself. The flags word sits in the low half of the
    syscall argument (args[2] for ``sendmsg``, args[3] for ``sendto`` and
    ``sendmmsg``), and both supported arches are little-endian, so one
    32-bit absolute load reads it.
    """
    audit_arch, connect_nr = arch_entry(machine)
    sendto_nr, sendmsg_nr, sendmmsg_nr = _SEND_TABLE[machine]
    return b"".join(
        [
            # 0-1: arch pin (foreign arch -> ALLOW at 12)
            _insn(_BPF_LD | _BPF_W | _BPF_ABS, 0, 0, _SECCOMP_DATA_ARCH_OFFSET),
            _insn(_BPF_JMP | _BPF_JEQ | _BPF_K, 0, 10, audit_arch),
            # 2-3: x32 alias space -> ENOSYS at 13
            _insn(_BPF_LD | _BPF_W | _BPF_ABS, 0, 0, _SECCOMP_DATA_NR_OFFSET),
            _insn(_BPF_JMP | _BPF_JSET | _BPF_K, 9, 0, _X32_SYSCALL_BIT),
            # 4: connect -> NOTIF at 14
            _insn(_BPF_JMP | _BPF_JEQ | _BPF_K, 9, 0, connect_nr),
            # 5-7: send-family dispatch (flags word offset differs)
            _insn(_BPF_JMP | _BPF_JEQ | _BPF_K, 2, 0, sendmsg_nr),
            _insn(_BPF_JMP | _BPF_JEQ | _BPF_K, 3, 0, sendto_nr),
            _insn(_BPF_JMP | _BPF_JEQ | _BPF_K, 2, 4, sendmmsg_nr),
            # 8-9: sendmsg flags (args[2]) -> DENY at 15 when MSG_FASTOPEN
            _insn(_BPF_LD | _BPF_W | _BPF_ABS, 0, 0, _SECCOMP_DATA_ARG2_OFFSET),
            _insn(_BPF_JMP | _BPF_JSET | _BPF_K, 5, 2, _MSG_FASTOPEN),
            # 10-11: sendto/sendmmsg flags (args[3]) -> DENY at 15 likewise
            _insn(_BPF_LD | _BPF_W | _BPF_ABS, 0, 0, _SECCOMP_DATA_ARG3_OFFSET),
            _insn(_BPF_JMP | _BPF_JSET | _BPF_K, 3, 0, _MSG_FASTOPEN),
            # 12-15: verdicts
            ...
            _insn(_BPF_RET | _BPF_K, 0, 0, SECCOMP_RET_ERRNO | errno.EPERM),
        ]
    )
...
def seccomp_syscall_nr(machine: str) -> int:
    ...
    raise FenceUnsupportedArch(f"connect fence has no seccomp(2) number for {machine!r}")
                ...
                while True:
                    ...
                    if _hit is not None:
                        try:
                            ...
                            continue
                        print("sandbox: WARNING - connect-fence DENIED "
                              + _hit[0] + ":" + str(_hit[1]),
                              file=sys.stderr)
