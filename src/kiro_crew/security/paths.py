...
_CREW_SECRET_LEAVES: list[str] = [
    ...
    # the kiro-cli binary trust above, and the same class as the Ops Mission
    # Control entries below: it is an INPUT TO AN AUTHORIZATION DECISION.
    # ``devcontainer.is_trusted()`` compares the project's current
    ...
    # including its lifecycle commands and whatever ``runArgs`` / ``privileged``
    # / ``mounts`` it declares. An agent that could write this file would record a
    # matching digest for a config it just authored and self-approve arbitrary
    # container execution, bypassing the human trust prompt that is the entire
    # premise of the feature. Read-blocked as well as write-blocked (unlike the
    # Ops entries, which teammates must read): nothing outside the gateway needs
    # to read it, and the gateway's own reader opens it directly rather than
    # through this gate. Classified as the whole directory so future sidecars are
    # covered too.
    "devcontainers",
