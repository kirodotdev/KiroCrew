...
def _audit_bash_exfiltration(
    ...
    *enabled_ids* lets the caller honour an operator opt-out: a rule whose row
    ...
    for name, value in assignments.items():
        ...
        # `${NAME}` needs no trailing boundary; bare `$NAME` must not eat
        # into a longer name (`$CMDX` is not `$CMD`). The replacement is a
        # LITERAL: pass a function — a raw replacement string would treat
        # backslashes in the value (`\q`, `C:\Users`) as escapes and crash.
        expanded = re.sub(rf"\$(?:\{{{name}\}}|{name}(?!\w))", lambda _m: value, expanded)
