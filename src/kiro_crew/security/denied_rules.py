...
_SELF_PROTECTION_FLOOR_NOTES: dict[str, str] = {
    ...
    "sandbox-escape-ssh-self": (
        ...
        "off-loop check rules out a loopback alias. On Linux the same holds for an IP address "
        "refused right after the gateway starts: until this machine's own address table "
        "has been read, any IP could be one of its secondary addresses, so the refusal "
        "means local-address verification is PENDING — retry it. If an IP address that "
        "is not one of this machine's stays refused on every retry, this machine's address "
        "table could not be read at all, and the refusal holds until it can. A hostname whose lookup keeps "
