...
class ResolverHelper:
    ...
    def realpaths(self, paths: list[str]) -> list[str | None] | None:
        ...
        ``$HOME`` plus the override roots) and the anchor rebuild: single requests
        were measured on Windows CI at ~80,000 round-trips for one test file, where
        each round-trip costs a scheduler tick, so the round-trips were the time.
        One request carries them all, and it carries ONE spelling: anchors use
        ``realpath`` alone, and on Windows each spelling is a handle open per path
        component, so a batch that also computed ``Path.resolve()`` paid twice for
        a value it discarded -- enough to cross the per-thread wait allowance's
        floor on every rebuild. Same contract as ``resolve``: ``None`` per element
        where ``realpath`` raised, ``None`` for the batch on a transport fault.
        """
        if not paths:
            return []
        if self._latched():
            return [_inprocess(p, both=False)[0] for p in paths]
        answers = self._request(paths, paths)
        return None if answers is None else [real for real, _resolved in answers]
    ...
    def _request_locked(
        ...
        # A single path wants both spellings (the candidate check); a list is an
        # anchor batch and wants ``realpath`` alone -- see ``realpaths``.
        both = isinstance(payload, str)
        ...
        try:
            ...
            pairs = [reply["r"]] if both else [[real, None] for real in reply["r"]]
