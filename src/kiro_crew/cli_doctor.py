...
def _doctor_backend_ability_cards(cfg: KiroCrewConfig) -> None:
    ...
    **Values, not prose.** The row prints what the declaration says -- the kind and the
    reach in the registry's own spelling -- rather than an English gloss of it. The
    panel owns the prose; a second wording here would be one declaration speaking with
    two voices, of which only one could ever be translated. It is also the register
    this reader wants: they are in a terminal and the next thing they do is read
