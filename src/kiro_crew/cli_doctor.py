    ...
    * profile installed and attached to the resolved launcher script, and this
    ...
    if not _service_profile_applies(apparmor.PROFILE_PATH, apparmor.PROFILE_NAME):
        ...
        _print_wrapped(
            ...
            f"attached to the kirocrew launcher script this host currently resolves, so "
            f"nothing on this host runs confined by it. Run `kirocrew service install` "
            f"to (re)install the gateway service with the profile attached."
        )
