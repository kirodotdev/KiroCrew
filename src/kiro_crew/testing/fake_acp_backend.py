...
def _session_identity(session_id: str) -> tuple[str, str]:
    """``(session_key, session_token)`` naming the Crew session this ACP session serves.

    The gateway accepts a caller-declared parent session only behind an
    attestation, and a loopback TCP request has no kernel peer to offer, so the
    bridge sends the signed per-session token (``X-Session-Token``) beside the key
    it declares (``X-Session-Key``); the gateway verifies the one back to the
    other. The token reaches this process by one of two roads, by topology:

    * a DEDICATED client (``AcpClient._spawn``) publishes it into the process
      environment, with the session key beside it;
    * the SHARED runtime multiplexes sessions over one process, so identity travels
      per element: the token sits in each ``mcpServers`` entry's ACP ``env`` array
      handed to ``session/new`` (``mcp_gateway.session_servers.attach_stub_session_token``)
      and the key is nowhere in the process — a real MCP child resolves it from
      the token via the MAC-signed mapping the gateway published
      (``session_token_sig.publish_session_token``: ``<home>/session_token_<sha256(token)>.sig``,
      body after the first line). This fake reads that body the same way, WITHOUT
      checking the MAC: it is naming itself to the gateway, which does the check.

    Stdlib-only by this module's contract, so the three names are spelled here.
    """
    import hashlib

    token = os.environ.get("KIROCREW_STUB_SESSION_TOKEN", "")
    if not token:
        servers, _cwd = _SESSIONS.get(session_id, ([], ""))
        for server in servers:
            env = server.get("env") if isinstance(server, dict) else None
            for item in env if isinstance(env, list) else []:
                if isinstance(item, dict) and item.get("name") == "KIROCREW_STUB_SESSION_TOKEN":
                    token = str(item.get("value") or "")
                    break
            if token:
                break
    key = os.environ.get("KIROCREW_SESSION_KEY", "")
    if not key and token:
        home = Path(os.environ.get("KIROCREW_HOME", "") or "~/.kiro/crew").expanduser()
        digest = hashlib.sha256(token.encode("utf-8")).hexdigest()
        try:
            raw = (home / f"session_token_{digest}.sig").read_text(encoding="utf-8")
        except OSError:
            raw = ""
        _mac, _, body = raw.partition("\n")
        key = body.strip()
    return key, token


def _bridge_post(
    path: str, body: dict[str, Any], identity: tuple[str, str] = ("", "")
) -> tuple[int, dict[str, Any]]:
    port = _gateway_port()
    if not port:
        return 0, {"error": "no KIROCREW_BOUND_PORT / KIROCREW_PORT in the fake's environment"}
    headers = {"Content-Type": "application/json"}
    cred = _internal_secret(port)
    if cred:
        headers["X-Internal-Secret"] = cred
    # Sent the way ``session_token_sig.session_token_header`` and ``mcp_core._post``
    # send them; see ``_session_identity`` for where the two values come from.
    session_key, session_token = identity
    if session_key:
...
def _run_directives(directives: list[tuple[str, str]], task: str, session_id: str = "") -> str:
    ...
    for verb, agent in directives:
        if verb == "SPAWN":
            ...
            status, d = _bridge_post("/api/spawn", body, identity)
            run_id = str(d.get("id", ""))
            ...
            status, d = _bridge_post(
                f"/api/spawn/{run}/steer", {"message": task, "mode": mode}, identity
            )
            if status == 200:
        ...
        if directives:
            ...
            _update(
                session_id,
                {
                    ...
                    "content": {
                        "type": "text",
                        "text": _run_directives(directives, task, session_id),
                    },
                },
            )
            _result(req_id, {"stopReason": "end_turn"})
