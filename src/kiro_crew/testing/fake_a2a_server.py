...
* ``[[REVISE]]`` -> an appended draft artifact, then an ``append: false`` update
  that REPLACES it with ``REVISED_TEXT``; the draft must not reach the record.
* ``[[SNAPSHOT]]`` -> no artifact or status events at all: the turn ends with a
  single ``Task`` frame in ``COMPLETED`` state carrying ``SNAPSHOT_TEXT`` inline
  as ``task.artifacts`` -- the reference SDK's ``completed_task()`` shape. The
  client must complete the turn with that text, not report a lost connection.
...
DROP_CHUNKS = 3
#: ``[[REVISE]]`` replaces an appended draft with this text (``append: false``).
REVISED_TEXT = "FINAL: the revised answer, and the only one that counts\n"
#: ``[[SNAPSHOT]]`` delivers this as the inline artifact of a terminal ``Task``.
SNAPSHOT_TEXT = "SNAPSHOT: the whole answer, delivered inside the final Task\n"
...
class FakeA2AServer:
    ...
    async def _send_streaming(
        ...
        # Artifact chunking as the reference SDK's TaskManager reads it: the
        # FIRST update for an artifactId carries ``append: false`` (create); later
        # chunks carry ``append: true``; an ``append: false`` update for an id
        # that already has content REPLACES it. ``append=True`` on an unseen id is
        # a protocol violation the SDK's TaskManager rejects, so the fake tracks
        # what it has created and never emits that shape.
        created: set[str] = set()

        async def artifact(chunk: str, *, replace: bool = False, artifact_id: str = "out") -> None:
            append = artifact_id in created and not replace
            created.add(artifact_id)
            await frame(
                {
                    "artifactUpdate": {
                        "taskId": task.task_id,
                        "contextId": ctx_id,
                        "append": append,
                        "artifact": {"artifactId": artifact_id, "parts": [{"text": chunk}]},
                    }
                }
            )
        ...
        if "[[REVISE]]" in text:
            # A server that corrects itself: an appended draft, then an update
            # with ``append: false`` that REPLACES the artifact's content. Only
            # the replacement is the result; the draft must not reach the record.
            await artifact("DRAFT: this line is superseded\n")
            await artifact(REVISED_TEXT, replace=True)
            await status(_STATE_COMPLETED, final=True)
            await resp.write_eof()
            return resp

        if "[[SNAPSHOT]]" in text:
            # The reference SDK's ``completed_task()`` shape: the ONLY frame
            # after the opening Task is a terminal Task snapshot with the result
            # inline. No artifactUpdate, no statusUpdate. A client that reads a
            # Task frame for its ids alone sees a stream with no terminal state.
            await frame(
                {
                    "task": {
                        "id": task.task_id,
                        "contextId": ctx_id,
                        "status": {"state": _STATE_COMPLETED},
                        "artifacts": [{"artifactId": "out", "parts": [{"text": SNAPSHOT_TEXT}]}],
                    }
                }
            )
            await resp.write_eof()
