...
def _objects_the_kernel_consults(path: Path) -> list[Path] | None:
    ...
    Walks from the root one component at a time, exactly as the kernel resolves a
    pathname, so an intermediate hop is recorded rather than collapsed away by a
    single ``realpath``.

    A symlink's target is SPLICED into the components still to be walked rather than
    joined and normalised. That ordering is the whole point: ``hop/../payload``
    normalised first becomes ``payload`` beside the link, but the kernel resolves
    ``hop`` before applying ``..``, so the real parent is wherever ``hop`` led. A
    lexical collapse would therefore omit the directory an agent actually controls.
    For the same reason ``..`` is applied to the route built so far, never to the
    spelling it was written in.

    ``None`` when the route cannot be traced -- a cyclic chain, one deeper than the
    kernel would follow, or an unreadable component -- which the caller treats as
    untrusted.
    """
    ...
    while remaining:
        ...
        if spelled.is_absolute():
            ...
            remaining = list(spelled.parts[1:]) + remaining
        else:
            current = current.parent
            remaining = list(spelled.parts) + remaining
        objects.append(current)
    return objects
...
def _agent_could_have_written(path: Path) -> bool:
    ...
    every object on the real route, which already holds each directory walked through
    on the way down, so there is nothing further to expand.
