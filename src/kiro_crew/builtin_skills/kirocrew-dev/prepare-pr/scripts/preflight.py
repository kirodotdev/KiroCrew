...
def run(args, extra_env=None):
    ...
    overlays the inherited environment for this one call.
    ...
    try:
        ...
        cwd = os.getcwd()
        try:
            repo_local = os.path.commonpath([exe_abs, cwd]) == cwd
...
def viewer_write_verdict(repo):
    ...
    rc, out, _ = run(["gh", "repo", "view", repo, "--json", "viewerPermission"])
    if rc == 0 and out:
        try:
...
def _push_probe_env():
    ...
    it is left untouched rather than overridden.
    """
...
def push_transport_verdict(branch):
    ...
    Returns (verdict, detail): rc 0 confirms the transport can push; a failed
    dry-run is classified by ``_classify_push_probe_error``. detail is a
    hardcoded-safe label - raw git stderr is never surfaced.
    """
    rc, _, stderr = run(
    ...
    if gh_ok:
        ...
        if cur and cur != "HEAD":
            ...
            if transport_verdict == _WRITE_WRITER:
                write_detail = (
                    ...
                    else "origin accepts push dry-run ({} on gh probe)".format(write_detail)
                )
        ...
        if write_verdict == _WRITE_DENIED:
            if "no non-interactive git push credential" in write_detail:
                ...
                fork_hint = (
                    "configure git push credentials first (gh auth setup-git for HTTPS, "
                    "or load the SSH key into the agent), then re-run this preflight"
                )
            else:
                fork_hint = (
