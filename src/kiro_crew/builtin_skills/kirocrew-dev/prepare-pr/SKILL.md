...
- **Local review is the gate, run EVERY iteration (not once).** Before any push — including every re-push after a failed server round — fan the finished diff out to two **read-only subagents run CONCURRENTLY via two model-pinned `spawn_run` calls** (one call per reviewer, since `spawn_run`'s `model` is batch-wide; both are fire-and-forget so they still run at the same time — this is what makes the local gate fast), each **mirroring a server reviewer with its own contract**: a GPT-class reviewer against `.github/workflows/codex-review.yml`, and a Claude/Opus-class reviewer against `.github/workflows/claude-review.yml` **plus the base-ref `AUTOSDE.yaml` + `website/AUTOSDE.yaml`** (the RULE outranks the prompt) and `CLAUDE.md`/`AGENTS.md`. Fix verified Critical/High (and any `blocking: true` AUTOSDE hit) locally, re-run the gate + one focused verifier, and **only push when locally green**. Local-green is a cost/latency optimization, not a guarantee — the server poll stays as the backstop. If a reviewer's model is unavailable, fall back a tier and warn.
...
1. **Canonical local gates.** Run the KiroCrew worktree skill's Rule 2 gate: pytest / isort / flake8 / mypy, and (for frontend changes) `tsc -b` / vitest. Must be green before review.
2. **Local review that MIRRORS the server reviewers.** Set `BASE_SHA=$(git merge-base HEAD origin/<base>)` and `HEAD_SHA=$(git rev-parse HEAD)`, then dispatch the reviewers as **two separate model-pinned `spawn_run` calls — one per reviewer** (`spawn_run`'s `model` applies batch-wide, so a single batched call cannot give the two reviewers different models). Both calls are fire-and-forget (results arrive as completion events), so firing them back-to-back still runs the reviewers **concurrently**. Each mirrors a specific server gate and reads **that gate's own contract** (they diverge — do not use one rubric for both):
   - **Reviewer G — mirror of the GPT gate.** Pin to a **GPT-5.x-class** model. Read the `SEVERITY + BLOCKING CONTRACT` / `OUTPUT STYLE` sections of `.github/workflows/codex-review.yml`. Charter: reachable correctness/security failures, data loss, crashes/hangs, permission-boundary regressions, cross-OS (macOS/Linux/Windows) breakage.
   - **Reviewer O — mirror of the Claude / Opus 5 gate.** Pin to a **Claude-Opus-class** model. Read `.github/workflows/claude-review.yml` **and — decisively, since the RULE outranks the prompt — the BASE-ref `AUTOSDE.yaml` (backend) + `website/AUTOSDE.yaml` (frontend)**, plus `CLAUDE.md` and `AGENTS.md` (root + `website/`). Block ONLY on **(1)** a `blocking: true` AUTOSDE rule whose file-patterns match a changed file, or **(2)** a reachable residual-class defect. Honor the **≤2 BLOCKING** budget.
