...
| Agent | Use it for |
|---|---|
| `sdpm-spec` | An important deck. Works through a brief, an outline and an art direction with the user, then delegates composition. |
| `sdpm-vibe` | A fast draft from a URL, pasted text, or a one-line brief. Few questions. |
| `sdpm-composer` | Slide composition itself — invoked as a sub-agent by the two above, not directly. |
| `sdpm-style` | Creating a reusable style guide through conversation. |
