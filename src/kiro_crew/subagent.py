...
class SubagentInfo:
    ...
    # True only on the synthetic busy record returned while this conversation's
    # session files are being released. The record keeps the real conversation
    # id so callers never mistake an implementation marker for a live run id.
    _conversation_releasing: bool = False
        ...
        payload_json = json.dumps(
            {
                ...
                "resolved_model": _redact(info.resolved_model),
                "run_id": info.id,
                "silent": info.silent,
                "task": stored_task,
            },
