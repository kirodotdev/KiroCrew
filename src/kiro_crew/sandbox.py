...
def _fence_build_literals(
    ...
    from .security import (
        connect_fence as _cf,  # lazy: this module keeps zero top-level sibling imports
    )
...
def _build_launcher_script(
    ...
    *,
    ...
    uid = os.getuid()
    gid = os.getgid()
    # Connect fence build-time inputs. HARD RULE: this builder stays
    # purely lexical — no stat, no netlink, no DNS (the same reason
    # test_the_builder_does_not_stat_the_hidden_paths forbids even isfile
    # here), so it NEVER resolves addresses inline — the
...
{private_setup}        # Private windows: a directory INSIDE a hidden tree that stays
        ...
        if _libc.prctl:
            ...
            #
            # Two deny sets, kept apart on purpose. _DENY_SYSCALLS is the
            # audited namespace-escape set and nothing else: it is pinned
            # literally by a pentest remediation, and both the escape list and
            # the link/linkat exclusion are read straight off that literal, so
            # an unrelated concern folded in here silently retires the guard.
            # _FENCE_DENY_SYSCALLS is the connect-fence bypass set, one
            # numbering across both arches: io_uring_setup=425,
            # io_uring_enter=426, io_uring_register=427. IORING_OP_CONNECT
            # performs a connect(2) the user-notification fence never sees, so
            # ring setup is answered EPERM -- no ring exists, and liburing and
            # libuv consumers fall back to the plain syscalls that both this
            # filter and the fence do see.
            _FENCE_DENY_SYSCALLS = (425, 426, 427)
            ...
            if _DENY_SYSCALLS:
                ...
                # For each denied syscall: JEQ -> DENY (at index k+4). Both
                # deny sets ride one filter; only their definitions are apart.
                _all_deny = _DENY_SYSCALLS + _FENCE_DENY_SYSCALLS
                _n_deny = len(_all_deny)
                for _i, _nr in enumerate(_all_deny):
        ...
        if not _fence_installed:
            print("sandbox: WARNING - connect fence unavailable on this "
                  "kernel; command-line tier remains the interim cover",
                  file=sys.stderr)
        _fence_csock.close()
