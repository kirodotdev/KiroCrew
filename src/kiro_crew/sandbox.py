...
_RUN_DIR_ARTIFACTS: dict[str, tuple[str, ...]] = {
    ...
    _PI_GATE_ARTIFACT_PREFIX: (".sh", ".cmd", ".ts", ".tmp"),
}
# ``.tmp`` is the mkstemp stage both pi artifacts use before publication.
...
#:
#: The pi gate is exactly that child and has NOT moved yet: ``acp/client.py`` still
#: writes its launcher and sealed extension under ``run``, so a pi session cannot
#: start while this leaf is masked. That is not a consequence of this entry: ``run``
#: sits on the read-gate floor, so it is masked for every enforced harness whether or
#: not it is named here, and the standing fix for the pi gate is its own leaf. Keeping
#: ``run`` masked holds the line that makes such a leaf necessary; exposing it to
#: unbreak one harness would hand the gateway credential to four.
...
_CREW_CHILD_READABLE_LEAVES: tuple[str, ...] = (
    # The governance ceiling and its trust root. Hiding one does not loosen it: the
    # launcher binds an EMPTY file and an empty document makes ``boot_platform()``
    # raise, so an in-sandbox process stops booting rather than running ungoverned.
    "security_policy.json",
    "admission_policy.json",
    "profiles",
    "app_admission.json",
    # Opt-out and consent ceilings the config loader reads in-sandbox. Each holds a
    # decision, never a bearer token; the write side is the risk and ``wrap_argv``
    # re-seals every one of them read-only.
    "denied_commands.json",
    "computer_use.json",
    "oauth_endpoints.json",
    "aws_service_consent.json",
    ...
    "file_delivery_consent.json",
    "ssh_auth_sock_consent.json",
    # The browser launcher an agent browser command must read and execute.
    "playwright-cli",
...
#:    * ``pi-gate`` — an empty dir means no gate artifact exists. The Linux seal
#:      skips an absent target, so materialising it prevents another backend's child
#:      from creating the directory and planting a launcher before the first pi session;
...
_CREW_NOFOLLOW_READONLY_DIR_LEAVES: tuple[str, ...] = (
    "playwright-cli",
    # This leaf holds the launcher the enforced harness's child executes. A symlink
    # would make the seal follow its referent while leaving the lexical name
    # replaceable, letting an agent substitute an executable that bypasses the gate.
    "pi-gate",
)
