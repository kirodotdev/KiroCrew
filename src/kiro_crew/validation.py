...
def section_marker_instant(at: object) -> str:
    """A marker's own request instant, or "" when the value cannot carry one.

    Admitted by SHAPE, not scrubbed: an ISO instant draws from a charset with no
    room for prose, so anything outside it is dropped whole. Two callers need the
    identical rule — the deferred-hold restore and the transfer bundle — and a
    second spelling would let one accept what the other rejects.
    """
    if not isinstance(at, str) or not 8 <= len(at) <= 40:
        return ""
    if set(at) - set("0123456789-:T+.Z"):
        return ""
    return at
