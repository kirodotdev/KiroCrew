...
#: Package attribute -> ``(owning module, symbol on that module)``. The attribute
#: and the symbol are allowed to differ: ``mcp_gateway.stub_main`` is
#: ``mcp_gateway.stub.main``. A re-exported name lives in exactly one place, the
#: submodule that defines it: reads resolve that submodule's current value and
#: writes go to it, so ``mcp_gateway.rewrite_agents`` and
#: ``mcp_gateway.rewriter.rewrite_agents`` cannot hold different values.
