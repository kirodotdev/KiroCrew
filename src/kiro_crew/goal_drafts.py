...
The browser keeps a local copy so the goal editor remains useful during a
short gateway outage, but ``localStorage`` is per browser profile.  This store
is the canonical bridge between desktop and mobile: every record carries the
canonical conflict timestamp.  Live records use server-arrival time; only the
first upload of a browser-only record uses its browser edit time, so opening a
stale client cannot overwrite a newer shared draft.
"""
...
GOAL_DRAFTS_DIR_NAME = "goal-drafts"
_STORE_VERSION = 1
...
GOAL_DRAFT_MAX_MESSAGE_CHARS = 8_000
...
class GoalDraftStore:
    ...
    def __init__(self, base_dir: Path | None = None) -> None:
        self._base_dir = base_dir if base_dir is not None else data_home() / GOAL_DRAFTS_DIR_NAME
    ...
    def _draft(
        ...
        *,
        ...
        if len(message) > GOAL_DRAFT_MAX_MESSAGE_CHARS:
            raise ValueError(f"message too long (max {GOAL_DRAFT_MAX_MESSAGE_CHARS} chars)")
        if (
    ...
    def _prune(
        ...
        *,
        ...
        if len(decoded) > MAX_GOAL_DRAFTS:
            changed = True
            kept = decoded[:MAX_GOAL_DRAFTS]
            if protected_slot is not None and all(slot != protected_slot for slot, _ in kept):
                protected = next(
                    (item for item in decoded if item[0] == protected_slot),
                    None,
                )
    ...
    def get(self, slot_key: str, *, now_ms: int | None = None) -> GoalDraftSnapshot:
        ...
        make_owner_only_dir(self._base_dir)
    ...
    def put(
        ...
        *,
        ...
        """Apply a draft or tombstone and return the post-prune canonical state.

        Live writes are ordered by server arrival time. Migration writes retain
        browser edit-time ordering so an old local-only copy cannot replace a
        newer canonical draft merely by opening.
        """

        slot = self._slot(slot_key)
        now = int(time.time() * 1000) if now_ms is None else now_ms
        submitted_stamp = self._updated_at(updated_at, now_ms=now)
        make_owner_only_dir(self._base_dir)
        with open_lock_file(self._lock_path) as fd:
            with file_lock(fd, exclusive=True, required=True):
                records, changed = self._prune(self._read_unlocked(), now_ms=now)
                ...
                if migration:
                    stamp = submitted_stamp
                    accept = stamp > current.updated_at
                else:
                    accept = True
                    stamp = max(now, current.updated_at + 1)
                incoming = (
                ...
                if accept:
                    ...
                    records, _ = self._prune(
                        records,
                        now_ms=now,
                        protected_slot=slot if not migration else None,
                    )
                    changed = True
                    current = self._decode_record(records.get(slot)) or GoalDraftSnapshot(None, 0)
                if changed:
