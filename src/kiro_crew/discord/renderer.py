...
class DiscordRenderer(Renderer):
    ...
    async def _flush_thinking(self) -> None:
        ...
        if posted is None:
            return
        # This note is a message of its own, so it becomes what the NEXT thing shown
        # is graded against. Without the record the answer's first frame grades
        # against whatever preceded the reasoning, and a preview ending in a
        # credential prefix is completed by the answer's leading characters across
        # two delivered messages. The preview cut runs after redaction, so it can
        # leave that prefix even though the whole key never appeared.
        #
        # Graded on the way in it is not, and deliberately: ``💭 `` is unconditional
        # and survives rendering, so it separates this text from the message above
        # in the only form the reader sees. Grading ``body`` would model an adjacency
        # that does not exist and give up a leading span for a key the emoji already
        # broke.
