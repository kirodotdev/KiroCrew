        ...
        async def _execute() -> None:
            ...
            try:
                ...
                if run.execution_mode == "review_fix":
                    # Review-fix runs attach to an already-prepared candidate
                    # worktree instead of planning a fresh one.
                ...
                save_progress(run)
                await self._apersist_runs()
                if run.branch_name and not workspace_lost and run.execution_mode != "review_fix":
                    try:
    ...
    async def _execute_tasks(self, run: Project, history_key: str) -> None:
        ...
        for group in groups:
            ...
            if len(resolved) == 1:
                ...
                # cannot stall a whole fixed-size batch. The knob is the single
