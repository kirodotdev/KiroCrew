...
class PanelError(ValueError):
    """A panel operation was refused. ``code`` is the machine-readable reason."""
...
def available_templates() -> list[str]:
    ...
    Best-effort for an UNREADABLE directory: it yields what the other one has
    rather than failing a read of somebody's webview. An ALIASED override
    directory is different and deliberate: ``override_templates_dir()`` raises
    ``PanelError("panel_dir_is_a_symlink")`` before either root is read, here and
    in ``resolve_template``, so a linked directory is a coded refusal the operator
    sees on the templates route and on publish, never a silent fall-through to the
    shipped templates that would hide their override being ignored. Pinned by
    ``test_a_linked_template_dir_is_a_coded_refusal_not_a_500``.
    """
    found: set[str] = set()
...
def render_record(record: dict[str, Any] | None) -> str | None:
    """Compose an ALREADY-READ record, or ``None`` when it is absent.
    ...
    takes effect on the next drawer open with no republish. Composition failures
    propagate so callers can distinguish a broken published panel from no panel.
    """
    if record is None:
        return None
