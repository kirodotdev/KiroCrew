...
#: Serializes reexec_lane_guard attempts (see its docstring). LoopBoundLock is
...
@asynccontextmanager
async def reexec_lane_guard() -> AsyncIterator[None]:
    """Drain the history lane around a process-replacing exec — the ONE owner
    of the drain→exec→reopen-on-failure pairing.

    Every survivable re-exec path runs its ``reexec_python_module`` call
    inside this guard: the drain is the LAST await before the exec (any
    await after it can admit a new append the exec then kills), and when
    the exec fails the admission gate is reopened before the exception
    propagates, so a caller that keeps serving is never left with
