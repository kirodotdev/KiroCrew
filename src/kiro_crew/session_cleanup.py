...
class SessionCleanup:
    ...
    async def _expire_idle(self, timeout_secs: int) -> None:
        ...
        for key, is_orphan, scanned in expired:
            ...
            # ``expect_session=None``, so its call stays exactly what it has
            # always been: that axis is not what this change touches, and its
            # reset has never asked about identity.
            if is_orphan:
                ...
                reset_done = await self._owner.reset(key, skip_if_busy=True)
            if not reset_done:
