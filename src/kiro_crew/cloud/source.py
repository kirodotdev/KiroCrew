...
# A `data:<type>/<subtype>;base64,<payload>` URI is an embedded resource that
# DECLARES itself as one in its own syntax. Vite inlines small images and fonts
# this way, and those payloads are long base64 runs that trip the generic
# bare-secret sliding-window heuristic (a real 231-char PNG run from this
# project's built editor CSS does; pinned by
# test_real_bundled_data_uri_does_not_false_positive).
#
# Masking the payload before the credential scan removes the false-positive
# SOURCE, which is what lets every remaining bare-secret warning be treated as
# actionable. The alternative -- exempting the warning whenever the run is not
# exactly 40 chars -- also discarded the true positives, because
# `security._contains_bare_secret` exists precisely to catch a genuine 40-char
# secret glued to adjacent base64 characters ("X" + key, key + "ABC",
# key + "X" + key), all of which report a 41+ char run.
#
# Only the payload is masked, and only for the scan: the bytes shipped in the
# tarball are always the unmodified originals. A credential baked into a bundle
# by a build lands in ordinary JS/CSS, not inside a data: URI, so the leak this
# gate exists to stop is still caught.
_DATA_URI_B64_RE = re.compile(r"data:[\w.+-]+/[\w.+-]+;base64,([A-Za-z0-9+/=]+)")

#: Prefix of the generic entropy-heuristic warning `redact_credentials` emits.
_BARE_SECRET_WARNING = "Redacted bare secret key"

#: Container magic numbers for the asset types a bundler inlines as data URIs.
#: The exemption below is keyed on CONTENT, not position: a payload is only
#: treated as media if it actually decodes to one of these. Masking purely by
#: position would let a bare key parked where a payload goes escape the scan.
_MEDIA_MAGIC = (
    b"\x89PNG\r\n\x1a\n",  # png
    b"\xff\xd8\xff",  # jpeg
    b"GIF87a",
...
def _data_uri_payload_is_media(payload: str) -> bool:
    """True if a base64 data-URI payload decodes to a recognized media container."""
    for pad in ("", "=", "=="):
        try:
            raw = base64.b64decode(payload + pad, validate=True)
        except (binascii.Error, ValueError):
            continue
        return raw.startswith(_MEDIA_MAGIC)
    return False


def _mask_inlined_media(text: str) -> str:
    """Blank out data-URI payloads that are genuinely inlined media."""

    def replace(match: "re.Match[str]") -> str:
        return "data:masked" if _data_uri_payload_is_media(match.group(1)) else match.group(0)

    return _DATA_URI_B64_RE.sub(replace, text)
...
def _read_dist_bundle(dist: Path, dist_resolved: Path) -> list[tuple[str, bytes]]:
    ...
    for file_path in sorted(dist.rglob("*")):
        ...
        if suffix in _DIST_TEXT_SUFFIXES:
            ...
            # The data-URI exemption applies to the GENERIC bare-secret
            # heuristic ONLY -- that is the single check with a false-positive
            # problem on built assets -- and only to a payload that actually
            # DECODES to media (see _mask_inlined_media). Exempting by position
            # alone would let a bare key parked where a payload goes escape. Distinctive and encoded-credential
            # matches have reliable token structure, so they stay actionable
            # everywhere, including inside a data: URI payload (an attacker
            # cannot launder a key by base64-ing it into an inlined image).
            #
            # Every bare-secret warning on the masked text is actionable,
            # including runs longer than 40 chars: that is what a genuine key
            # glued to adjacent base64 characters looks like, and it is
            # precisely what `security._contains_bare_secret` exists to catch.
            _, full_warnings = security.redact_credentials(decoded)
            _, masked_warnings = security.redact_credentials(_mask_inlined_media(decoded))
            actionable = [
                warning for warning in full_warnings if not warning.startswith(_BARE_SECRET_WARNING)
            ] + [warning for warning in masked_warnings if warning.startswith(_BARE_SECRET_WARNING)]
            if actionable:
...
def _build_archived_dist(archive: Path) -> list[tuple[str, bytes]]:
    ...
    with tempfile.TemporaryDirectory(prefix="kirocrew-frontend-") as temp:
        ...
        try:
            ...
            # `build_stock_frontend` resolves npm with shutil.which and reports a
            # MISSING one by returning None, but the spawn itself is unguarded:
            # an npm that resolves yet cannot be executed (non-executable, broken
            # symlink, unlinked between the check and the spawn, EPERM) raises
            # here. Without this branch it would reach `except BaseException:
            # raise` below and abort the launch with a traceback -- exactly the
            # outcome the archive-unchanged fallback exists to prevent. Converted
            # to a refusal so the box builds its own frontend, which is what
            # every other build failure already does.
            raise _AbortInjection(f"could not run the frontend build ({exc!r})") from exc
        if dist is None:
