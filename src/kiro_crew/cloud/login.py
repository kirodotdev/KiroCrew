...
def cancel_device_login(instance_id: str, profile: str = "", region: str = "") -> bool:
    ...
    **Blast radius: every ``kiro-cli login`` under the same uid on that instance.**
    ...
    not a PID this function tracked, because the PID file is a fixed world-writable
    ``/tmp`` path and honouring it would let any local user pick what gets killed.
