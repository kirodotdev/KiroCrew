...
| Runs on the `kiro` (kiro-cli) backend | yes | no: kiro-cli loads JSON only, so an agent that has no JSON spec in either the project's `.kiro/agents` or `~/.kiro/agents` is refused before spawn with a message naming the file. Switch `agent.acp_backend` to `kas` or add a JSON spec in either scope. |
