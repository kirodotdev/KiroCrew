...
- **Timeout**: 3 hours per subagent task (`agent.subagent_timeout_secs`, clamped to 60s..86400s at load; 0 means "use the default"). Enable `agent.subagent_timeout_auto` to let a non-stalled timeout or a successful run using at least 80% of its deadline raise future deadlines by at least 30 minutes, up to `agent.subagent_timeout_max_secs` (default 6 hours). A run already marked or suspected stalled from no stream activity does not train the shared deadline. Learned levels contain no task text, carry gateway-owned provenance, and persist across restarts. The base, opt-in, and ceiling settings are editable from the dashboard. Turning adaptation off immediately returns future runs to the configured base; turning it on again restores valid learned history. Delivery remains capped at 20 minutes (semaphore wait + injection), with 15 minutes per injection attempt (`KIROCREW_INJECTION_TIMEOUT`, clamped to the delivery cap). A **blocking** `spawn_sub_agents` call still collects for at most 2 hours regardless of these settings (`KIROCREW_SPAWN_SUB_AGENTS_MAX_WAIT`, itself capped at 7200s), so use `spawn_run` for work longer than 2 hours and read the results from its completion events
