...
class _PinnedPrivateState:
    """The private hierarchy held open from its canonical skills root."""
...
class SkillsLoader:
    ...
    def _preflight_private_state(self, *, require_sensitive: bool) -> Path:
        """Authenticate existing private ancestry before any state mutation."""
        skills_root_exists = os.path.lexists(self._dir)
        if not skills_root_exists and is_link_or_junction(self._dir):
            raise OSError("skills root is a dangling link or reparse point")
        ...
        projected_private = (
            ...
            else canonical_skills / AUTO_SKILL_NAMESPACE / AUTO_PRIVATE_DIRNAME
        )
        ...
        with contextlib.ExitStack() as stack:
            ...
            for sibling_name in (AUTO_PENDING_DIRNAME,):
                try:
                    stack.enter_context(
                        self._pin_skill_child_parent(
                            auto_parent,
    ...
    def _pin_private_state(
        ...
        *,
        ...
        """Pin every private ancestor from one canonical skills-root resolution."""
        canonical_skills = self._preflight_private_state(
        ...
        with contextlib.ExitStack() as stack:
            ...
            try:
                self._stat_pinned_child(auto_parent, AUTO_PENDING_DIRNAME)
            except FileNotFoundError:
                if create:
                    stack.enter_context(
                        self._pin_skill_child_parent(
                ...
                if not self._pinned_parent_matches(pending):
                    raise OSError("pending root is not a real directory")
            private = stack.enter_context(
                self._pin_skill_child_parent(
                    auto_parent,
                    AUTO_PRIVATE_DIRNAME,
            ...
            if require_sensitive and (
                ...
                raise OSError("private root is not agent-denied")
            resolved_private.relative_to(canonical_skills)
            yield _PinnedPrivateState(private, claims, evidence, locks, claim_locks)
            ...
            if os.name == "nt":
                ...
                if is_link_or_junction(path):
                    ...
                    try:
                        ...
                        verify_fd = platform_compat.open_file_no_reparse(path)
                        try:
                        ...
                        if not self._pinned_parent_matches(parent):
                            return False
                        refreshed = self._stat_pinned_child(parent, name)
                        if not os.path.samestat(current, refreshed):
                            return False
                        os.unlink(path)
            elif directory:
    ...
    def _private_state_roots_safe(self, *, create: bool, require_sensitive: bool = False) -> bool:
        ...
        try:
            ...
            logger.error(
                "Refusing unsafe auto-skill private state under %s. Remove links or junctions "
                "inside the skills tree; to relocate it, link the Kiro Crew data-home root "
                "instead, then retry.",
                self._private_root(),
            )
            return False

    def _open_skill_lock(self, parent: _PinnedSkillParent, name: str) -> int:
    ...
    def _file_lock(self, name: str) -> Iterator[bool]:
        ...
        try:
            private_state = self._pin_private_state(create=True)
    ...
    def _pending_slug_claimed(self, slug: str) -> bool:
        try:
            with self._pin_private_state(create=False) as state:
                if _DIR_FD_SUPPORTED:
                    names = os.listdir(state.claims.fd)
                else:
                    if not self._pinned_parent_matches(state.claims):
                        return False
                    names = os.listdir(state.claims.path)
        except (FileNotFoundError, OSError, RuntimeError, ValueError):
            return False
        return any(name.rsplit("--", 1)[0] == slug for name in names)
    ...
    def _authenticated_claim_restore_state(
        ...
        """Return the durable pending destination and generation for a restore."""
        payload = self._read_authenticated_claim_lock_payload(fd, lock_path, claim_name)
        if payload is None:
            return None
        try:
        ...
        restore_generation = data.get("restore_generation")
        if (
            not isinstance(restore_slug, str)
            or not self._is_pending_slug_safe(restore_slug)
            or not isinstance(restore_generation, str)
            or re.fullmatch(r"[0-9a-f]{64}", restore_generation) is None
        ):
            return None
        return restore_slug, restore_generation
    ...
    def _write_claim_restore_state(
        ...
        *,
        ...
        """Durably bind an imminent pending rename without changing claim state."""
        payload = self._read_authenticated_claim_lock_payload(fd, lock_path, claim_name)
        if payload is None:
            return False
        try:
        ...
        data["restore_generation"] = restore_generation
    ...
    def _commit_claim_evidence_state(
        ...
        """Durably forbid requeue before moving a mutable claim to evidence."""
        ...
        try:
            with self._file_lock("pending.lock") as acquired:
                ...
                with (
                    ...
                    self._pin_private_state(
                        create=False,
                        require_sensitive=True,
                    ) as private_state,
                ):
                    claims_parent = private_state.claims
                    # The pending namespace remains writable to the candidate
                    # producer. A no-reparse read handle permits concurrent
                    # writes on Windows (FILE_SHARE_WRITE) and a retained POSIX
                    # fd survives rename, so the first authoritative generation
                    # must be captured BEFORE moving the directory. The second
                    # capture is comparison-only: it proves the rename did not
                    # race a retained descriptor before the private boundary.
                    candidate = self._pending_root() / slug
                    linked_candidate = is_link_or_junction(candidate)
                    marker_present = not linked_candidate and os.path.lexists(
                        candidate / ".promoted"
                    )
                    claim_snapshot = self._capture_claim_snapshot(candidate)
                    has_snapshot = (
                        claim_snapshot.generation_hash is not None
                        and claim_snapshot.tree is not None
                    )
                    # Linked candidates are never publishable, but dismissal must
                    # unlink their claim without traversing the target. A planted
                    # marker is likewise a refusal-cleanup case. Neither is
                    # permitted to reach a publish path without snapshot authority.
                    ...
                    claimed = True
                    if has_snapshot:
                        observed_claim = self._capture_claim_snapshot(claim)
                        claim_hash = claim_snapshot.generation_hash
                        observed_hash = observed_claim.generation_hash
                        if (
                            claim_hash is None
                            or observed_hash is None
                            or not secrets.compare_digest(claim_hash, observed_hash)
                        ):
                            raise OSError("candidate changed during claim transfer")
        except OSError:
            if claimed and claim_snapshot is not None:
                self._restore_claimed_update(claim, fd, slug, claim_snapshot)
            platform_compat.release_lock(fd)
            os.close(fd)
            if not os.path.lexists(claim) and not os.path.lexists(
    ...
    def _retain_claim_evidence(self, claim: Path, claim_fd: int) -> bool:
        """Durably retire a mutable claim without deleting or requeueing it."""
        lock_path = self._claim_lock_path(claim.name)
            ...
            with self._pin_private_state(
                ...
                try:
                    ...
                    try:
                        ...
                        return False
                    return stat.S_ISDIR(retained.st_mode) and not is_link_or_junction(
                        self._evidence_root() / claim.name
                    )
                ...
                fsync_dir(private_state.evidence.path)
                retained = self._stat_pinned_child(private_state.evidence, claim.name)
                return stat.S_ISDIR(retained.st_mode) and not is_link_or_junction(
                    self._evidence_root() / claim.name
                )
        except (OSError, RuntimeError, ValueError):
            logger.error("Could not move claim to private evidence %s", claim, exc_info=True)
            return False
    ...
    def _restore_claimed_update(
        ...
        """Requeue only an authenticated claim generation through private staging."""
        notify = False
        ...
        with self._file_lock("pending.lock") as acquired:
            ...
            try:
                with (
                    ...
                    if not claim_linked and captured.tree is None:
                        observed = self._capture_claim_snapshot(claim)
                        if (
                            captured.generation_hash is not None
                            and observed.generation_hash != captured.generation_hash
                        ):
                            logger.error(
                                "Claimed skill generation changed after claim; retaining %s",
                                claim,
                            )
                            return None
                        captured = observed
                    if (
                        not claim_linked
                        and captured.tree is not None
                        and self._authenticated_claim_snapshot_state(
                            claim_fd,
                            self._claim_lock_path(claim.name),
                            claim.name,
                        )
                        is None
                        and self._authenticated_claim_publication(
                            claim_fd,
                            self._claim_lock_path(claim.name),
                            claim.name,
                        )
                        ...
                        logger.error("Could not authenticate restored claim generation %s", claim)
                        return None
                    ...
                    for number in [None, *range(2, 51)]:
                        ...
                        restored_meta = {}
                        if not claim_linked:
                            if captured.tree is None or captured.generation_hash is None:
                                logger.error(
                                    "Claimed skill has no immutable restoration authority; "
                                    "retaining %s",
                                    claim,
                                )
                                return None
                            restored_files = dict(captured.tree.files)
                            if metadata:
                                ...
                                if candidate_slug != slug:
                                    ...
                                    restored_meta["name"] = (
                                        f"{AUTO_SKILL_NAMESPACE}/{candidate_slug}"
                                    )
                                    restored_meta["notify_suppressed"] = False
                                    restored_files[Path(".meta.json")] = json.dumps(
                                        restored_meta,
                                        indent=2,
                                    ).encode("utf-8")
                            restored_snapshot = _SkillTreeSnapshot(
                                files=restored_files,
                                file_modes=dict(captured.tree.file_modes),
                                dir_modes=dict(captured.tree.dir_modes),
                                generation_hash=captured.tree.generation_hash,
                            )
                            stage = self._restoration_path(claim.name)
                            if os.path.lexists(stage):
                                if not self._remove_private_tree(
                                    stage,
                                    what="skill restoration artifact",
                                ):
                                    raise OSError("unsafe skill restoration stage remains")
                            self._materialize_skill_tree_snapshot(restored_snapshot, stage)
                            stage_info = self._stat_pinned_child(
                                claims_parent,
                                stage.name,
                            )
                            restored_generation = self._skill_tree_hash(stage)
                            if restored_generation is None or not self._write_claim_restore_state(
                                claim_fd,
                                self._claim_lock_path(claim.name),
                                claim.name,
                                restore_slug=candidate_slug,
                                restore_generation=restored_generation,
                            ):
                            ...
                            try:
                                ...
                                if not self._remove_private_tree(
                                    stage,
                                    what="skill restoration artifact",
                                    expected_identity=(stage_info.st_dev, stage_info.st_ino),
                                ):
                                    raise OSError(
                                        "could not clean refused restoration stage"
                                    ) from exc
                                if not self._clear_claim_restore_state(
                                    claim_fd,
                                    self._claim_lock_path(claim.name),
                                    claim.name,
                                ):
                                    raise OSError(
                                        "could not clear refused restore receipt"
                                    ) from exc
                                continue
                        else:
                            try:
                        ...
                        if not claim_linked and not self._retain_claim_evidence(claim, claim_fd):
                            logger.error(
                                "Could not durably retain restored claim evidence %s",
                                claim,
                            )
                        break
            except OSError:
    ...
    def _cleanup_publication_artifacts(self, claim_name: str) -> bool:
        """Remove only this claim's private stage, backup, and restoration trees."""
        clean = True
        paths = [*self._publication_paths(claim_name), self._restoration_path(claim_name)]
        for path in paths:
            if is_link_or_junction(path):
                clean = False
                continue
    ...
    def _reconcile_prepared_claim(
        ...
        with self._promotion_lock(target_slug) as acquired:
            ...
            # generation only while live is still absent and both private
            # generations match the journal. The no-replace rename is the final
            # compare-and-swap against a concurrent live creator.
            if (
            ...
            # A live generation outside the authenticated before/after set has
            # no recovery ownership. It may be a concurrent writer's complete
            # edit, so neither its current hash nor a second matching hash can
            # authorize moving it into the disposable stage slot. Retain every
            # artifact for operator recovery instead.
            return None
    ...
    def _restore_failed_promotion_claim(
        ...
        if journal is None:
            ...
            return self._authenticated_claim_evidence_state(
                claim_fd,
                lock_path,
                claim.name,
            ) is None and not (claim.exists() or is_link_or_junction(claim))
    ...
    def _recover_recorded_claim_restore(
        ...
        claim: Path,
        claim_fd: int,
        restore_state: tuple[str, str],
    ) -> bool:
        """Complete a durable restore receipt, then move the claim to evidence."""
        restore_slug, restore_generation = restore_state
        pending = self._pending_root() / restore_slug
        if self._skill_tree_hash(pending) == restore_generation:
            return self._retain_claim_evidence(claim, claim_fd)
        stage = self._restoration_path(claim.name)
        if os.path.lexists(pending) or self._skill_tree_hash(stage) != restore_generation:
            return False
        with self._file_lock("pending.lock") as acquired:
            if not acquired:
                return False
            try:
                with (
                    ...
                    try:
                        ...
                        return False
                    if self._skill_tree_hash(stage) != restore_generation:
                        return False
                    self._rename_skill_child_no_replace(
                        private_state.claims,
                        stage.name,
    ...
    def _recover_abandoned_claims(self, *, roots_authenticated: bool = False) -> None:
        ...
        try:
            if _DIR_FD_SUPPORTED:
                claim_names = os.listdir(state.claims.fd)
            else:
                if not self._pinned_parent_matches(state.claims):
                    return
                claim_names = os.listdir(state.claims.path)
            for claim_name in claim_names:
                ...
                claim_linked = stat.S_ISLNK(claim_info.st_mode) or bool(
                    getattr(claim_info, "st_reparse_tag", 0)
                )
                if (
                    not claim_linked and not stat.S_ISDIR(claim_info.st_mode)
                ) or "--" not in claim_name:
                    continue
                ...
                try:
                    ...
                    if restore_state is not None:
                        if not self._recover_recorded_claim_restore(claim, fd, restore_state):
                    ...
                    if not claim_linked and (lock_completed or marker_completed):
                        ...
                        cleanup_claim_lock = not os.path.lexists(claim)
                        continue
                    ...
                    if published is True:
                        if not self._commit_claim_lock_state(fd, lock_path, claim_name):
                    ...
                    if not claim_linked and self._completion_marker_present(claim):
                        if not self._remove_untrusted_completion_marker(claim):
                            continue
                    self._restore_claimed_update(claim, fd, slug, claim_snapshot)
                    cleanup_claim_lock = self._authenticated_claim_evidence_state(
                        fd,
                        lock_path,
                        claim_name,
                    ) is None and not os.path.lexists(claim)
                finally:
                    if acquired:
                        platform_compat.release_lock(fd)
                    os.close(fd)
    ...
    def get_pending_skill(self, slug: str) -> dict | None:
        """Return one snapshot-consistent pending candidate detail payload."""
        if not self._is_pending_slug_safe(slug):
            return None
        pdir = self._pending_root() / slug
        # Detail is user-visible review evidence. Capture body, metadata and
        # scripts once so a retained writer cannot make one response describe
        # different candidate generations across its individual fields.
        tree = self._skill_tree_snapshot(pdir)
        if tree is None:
            logger.warning("Refusing to read pending %s: candidate tree is unsafe", slug)
            return None
        try:
            content = tree.files[Path("SKILL.md")].decode("utf-8")
        except (KeyError, UnicodeDecodeError):
            return None
        meta = self._candidate_metadata_from_bytes(
            tree.files.get(Path(".meta.json")),
            redact=True,
        )
        scripts: list[dict[str, str]] = []
        for relative, payload in sorted(tree.files.items(), key=lambda item: str(item[0])):
            if not relative.parts or relative.parts[0] != "scripts":
                continue
            try:
                script_content = payload.decode("utf-8").replace("\r\n", "\n").replace("\r", "\n")
            except UnicodeDecodeError:
                continue
            scripts.append(
                {
                    "filename": self._redact_text(str(relative.relative_to("scripts"))),
                    "content": self._redact_text(script_content),
                }
            )
        return {
            "slug": slug,
    ...
    def _validate_and_redact_candidate(
        ...
        if tree is None:
            ...
            return None
        return self._validate_and_redact_snapshot(tree, name)

    def _validate_and_redact_snapshot(
        self, tree: _SkillTreeSnapshot, name: str
    ) -> _ValidatedCandidateSnapshot | None:
        ...
        *,
        ...
        try:
            if is_link_or_junction(claim):
                return None
            observed = self._validate_and_redact_candidate(claim, slug)
            if (
                observed is None
            ...
            if claim_snapshot.tree is None:
                ...
                return None
            snapshot = self._validate_and_redact_snapshot(claim_snapshot.tree, slug)
            if snapshot is None:
    ...
    def approve_pending_update(self, slug: str) -> str | None:
        ...
        An immutable pending generation is captured before the public directory
        is renamed. The claimed directory is then compared against that capture;
        publication uses only the captured bytes, while a refusal restores the
        claimed candidate to review without replacing a new public candidate.
        """
    ...
    def _approve_claimed_skill_locked(
        ...
        if claim_snapshot.tree is None:
            ...
            return None
        snapshot = self._validate_and_redact_snapshot(claim_snapshot.tree, name)
        if snapshot is None:
