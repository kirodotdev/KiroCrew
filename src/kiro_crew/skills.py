...
# from exhausting the foreground CLI, the one caller that opts in.
        ...
        *,
        ...
        for root, dirs, files in os.walk(sdir):
            if display_limits and len(out) >= _PENDING_SCRIPT_MAX_ENTRIES:
                # Stop descending once the display budget is spent: count what
                # the cap hides instead of walking an unbounded tree to find it.
                omitted += len(files)
                dirs[:] = []
                continue
        ...
        if omitted:
            out.append(
                {
                    "filename": _VALIDATION_REPORT_TRUNCATION_KEY,
                    "content": f"[at least {omitted} more helper file(s) not shown]\n",
                }
            )
        return out
