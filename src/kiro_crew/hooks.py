...
def _install_by_exchange(
    ...
    The ``"conflict"`` disposal is authorised only once the staging name is
    known to hold the payload again -- the inode *payload_fd* holds open, so
    its number cannot have been recycled either, AND its bytes, re-read through
    that descriptor, still hash to *payload_hash*, because an in-place rewrite
    of the payload while it sat at the original name keeps the identity and
    changes the content. A writer who replaces the
    original name between the check and the exchange back is what that
    exchange moves under the staging name (its ``replace`` already unlinked
    the payload's name), and unlinking it would destroy a save this protocol
    exists to protect. When identity shows the stage is not the payload, one
    more exchange puts that writer's file back at the original name -- it was
    the newest content an instant ago -- and the displaced content stays under
    the staging name: ``"preserved"``, nothing unlinked.

    Returns ``None`` when the first exchange does not happen -- the filesystem
    has no exchange (the call answers ENOSYS/EINVAL/EOPNOTSUPP) or refuses
    this one with any other ``OSError`` -- so the caller can take its plain
    rename with the residual window that path documents. Returns
    ``"preserved"`` in the failures that must not be cleaned up: an exchange
    back raised, or the stage did not come back as the payload. Newer content
    then sits under the staging name and either the payload or a later save
    at the original name. No plain rename is attempted -- without the
    exchange a rename is unconditional, so a writer landing on the original
    name meanwhile would be overwritten -- and unlinking the stage would
    destroy the very file this protocol exists to protect. That state is
    logged at ERROR with both names; the caller reports it as a conflict and
    leaves both files for the user.
    """
    try:
        platform_compat.rename_exchange(tmp_name, base, dir_fd=dfd)
    except (NotImplementedError, OSError):
        # Either the shim's no-exchange errnos or any other refusal from the
        # mount: nothing has moved, so the caller takes its plain rename as it
        # does where no exchange exists. Escaping here would read as a refused
        # write, which installs the store copy and demotes the pointer.
        return None
    ...
    payload_ident = _fstat_ident(payload_fd)
    try:
        platform_compat.rename_exchange(tmp_name, base, dir_fd=dfd)
        ...
        return "preserved"
    # Exchanged back: `base` holds what it held before this call, and the
    # staging name SHOULD hold the payload again. It does not when a further
    # writer replaced `base` between the check above and the exchange: that
    # writer's file is now under the staging name, and its replace already
    # unlinked the payload, so the only two files left are two writers' saves.
    # Identity settles WHICH inode is under the staging name -- the payload's
    # is held open, so the number cannot have been reused -- and its bytes,
    # re-read through that descriptor, settle whether it still IS the payload:
    # a writer who rewrote the inode in place while it sat at the original
    # name (truncate-and-write keeps the identity) leaves a later save behind
    # that must not be unlinked either. Only both together authorise the
    # caller's unlink.
    if (
        payload_ident is not None
        and _stat_ident(tmp_name, dir_fd=dfd) == payload_ident
        and _fd_content_matches(payload_fd, payload_hash, None)
    ):
        return "conflict"
    # A later save is under the staging name. One more exchange puts it back
    # at the original name, where it was an instant ago, and leaves the
    # content it had displaced under the staging name. Nothing is unlinked:
    # the caller does not know which of the two files the user wants.
    try:
        platform_compat.rename_exchange(tmp_name, base, dir_fd=dfd)
    except (NotImplementedError, OSError) as exc:
        logger.error(
            "could not restore %r after a lost compare-and-swap (%s): a later save is "
            "preserved under the staging name %r in the same directory and the displaced "
            "content is at the original name; resolve by hand",
            path,
            exc,
            tmp_name,
        )
        return "preserved"
    logger.error(
        "another save landed on %r while a lost compare-and-swap was being undone: that "
        "save is at the original name and the content it displaced is preserved under the "
        "staging name %r in the same directory; resolve by hand",
        path,
        tmp_name,
    )
    return "preserved"
...
def _fstat_ident(fd: int) -> tuple[int, int] | None:
    """``(st_dev, st_ino)`` of an open descriptor, or ``None`` if it cannot be read."""
    try:
        st = os.fstat(fd)
    except OSError:
        return None
...
def _stat_ident(name: str, *, dir_fd: int) -> tuple[int, int] | None:
    """``(st_dev, st_ino)`` of *name* under *dir_fd* without following links, or ``None``."""
    try:
...
def _pinned_replace(
    ...
    dfd = -1
    created = False
    # The payload's own descriptor, kept open through an exchange install so
    # the inode it names cannot be recycled while the exchange back is checked
    # against it. -1 for the plain-rename paths, which never inspect the stage.
    payload_fd = -1
    try:
        if use_dir_fd:
            try:
        ...
        if use_dir_fd:
            ...
            # Readable as well as writable when an exchange install follows:
            # the stage's bytes are re-read through this descriptor after the
            # exchange back, before its unlink is authorised.
            tfd = os.open(
                tmp_name,
                (os.O_RDWR if held_fd >= 0 else os.O_WRONLY)
                | os.O_CREAT
                | os.O_EXCL
        ...
        try:
            ...
            if held_fd >= 0:
                # An exchange install follows: the stage's identity is checked
                # after the exchange back, and only a held descriptor makes
                # that identity sound. Released in the cleanup below.
                payload_fd = tfd
            else:
                os.close(tfd)
        ...
        # Where no exchange exists (Windows; some network mounts) the plain
        # rename remains, with the residual window it has always
        ...
        if held_fd >= 0:
            ...
            verdict = _install_by_exchange(
                ...
                held_fd,
                payload_fd,
                _hashlib.sha256(encoded).hexdigest(),
                tmp_name,
                base,
                path,
                src_state,
            ...
            if verdict == "preserved":
                # The exchange back failed or never converged: newer content
                # sits under the staging name and MUST NOT be unlinked by the
                # cleanup below; the stale payload stays at the original name
                # (no rename can move it safely). Both are reported for the user.
                created = False
                return "conflict"
        ...
        if held_fd >= 0:
            try:
                ...
                pass
        # Same reason for the payload: the stage's post-exchange identity check
        # authorised the unlink above, and it must refer to the same inode.
        if payload_fd >= 0:
            try:
                os.close(payload_fd)
            except OSError:
