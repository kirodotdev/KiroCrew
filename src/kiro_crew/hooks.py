...
async def run_script_hook(
    ...
    try:
        ...
        # EVERY permitted run, attended included, and AUDIT-OR-DENY: a command running with no
        # permission row leaves the trail unable to say what was allowed, so a failed write refuses.
    ...
    # An unrecordable START refuses only the unattended event, whose row an attended event never
    # writes; an unrecordable PERMIT refuses every event, the grant itself being what went missing.
            ...
            for hook in self._committed_fire_targets(event):
                ...
                if (
                    ...
                    if gov_denied:
                        ...
                        continue
                    # Audit-or-deny here too: the directive is injected under this grant.
...
# latency isolation the at-most-once contract already tolerates.
