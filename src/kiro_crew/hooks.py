...
class ScriptHook:
    ...
    def from_dict(cls, data: dict) -> "ScriptHook":
        ...
        if not recognized:
            ...
            last_error = f"enabled={shown} is not a value this reads: loaded DISABLED"
        return cls(
        ...
        try:
            for hook in self._committed_fire_targets(event):
                ...
                result = await run_script_hook(
                    ...
                    context,
                    hook_event,
                    still_current=lambda: self._is_still_committed_enabled(hid)
                    and (outer is None or outer()),
                )
                results.append(result)
                logger.info(
