...
def _unc_data_home_root() -> Path | None:
    ...
    the resolved-home cache the default branch reads -- the same key
    ``config_dir()`` keys its own memo on -- so an env change, a monkeypatched
    accessor or a reset of the resolution cache all invalidate naturally.
    ...
    if cached is not None and cached[0] == key:
        return cached[1]
    try:
        root: Path | None = _config_paths.data_home()
