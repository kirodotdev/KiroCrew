...
async def process_slack_files(
    ...
    *,
    ...
    after the turn. The event handler passes it for a Slack-native thread whose
    conversation is temporary or incognito (its own ``!incognito`` /
    ``!temporary``): such a conversation persists nothing, and a picture that
    outlived it in the uploads directory would break that promise. A message
    bound for a linked dashboard slot always promotes, whatever that slot's
    privacy mode (see :func:`kiro_crew.slack.events._inbound_images_persist`).
    """
    if not orch.slack:
        return IngestResult()
