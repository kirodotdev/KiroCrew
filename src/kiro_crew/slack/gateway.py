...
from kiro_crew.security.readonly_bash import command_invokes_program, is_read_only_bash
...
# Public alias: the built-in/core allowlist, PROVIDED the qualification/trust
# rule in ``_is_heartbeat_safe_tool`` is honored. This is NOT the full set of
# names the module can approve — an edition's dynamically-contributed
# additions (``SlackEnterpriseGate.heartbeat_safe_tools()``, matched by exact
# ``@server/tool`` identity) approve too, and are deliberately absent here:
# they are read at approval time from a deferred platform-context call, not a
# static name a module-level constant could enumerate. The constant's
# CONTENT (which names it lists) has changed from earlier revisions of this
# module as the matching rules below it were tightened; what has not changed
# is that reading this constant (import, iterate, membership-test) still
# works with no code change required for a caller (tests, operator docs)
# that treats it as "the currently-allowlisted built-in/core names" — the
# read contract is stable even though the set it returns is not frozen.
# Internal matching treats only the builtins (delegated to
# ``hooks._HOST_READ_ONLY_BUILTIN_TOOLS``) as bare-matchable, and requires the
# qualified identity for everything else.
...
def _is_heartbeat_safe_tool(event: LLMEvent) -> bool:
    ...
    if event.is_shell:
        ...
        # identity from that empty-but-"trusted" default. This check is
        # UNCONDITIONAL regardless of mcp_server_name, unlike the one below.
        if not tool_name:
            return False
        # `event.is_shell` is driven by the ACP `kind` field the AGENT
        # declares (acp/_dispatch.py:is_shell_kind), not by whether a server
        # actually backs the call — an edition MCP tool can legitimately
        # declare kind="execute" while carrying a real mcp_server_name. Only
        # a genuine HOST shell tool has no server behind it (same discipline
        # as _is_host_read_only_builtin below), so a present mcp_server_name
        # here means this is an MCP-served tool that merely looks
        # shell-shaped — fall through to the edition allowlist's exact
        # @server/tool pinning below rather than judging it on command text,
        # and never approve it from THIS branch on the command string alone.
        if not mcp_server_name:
            shell_command = event.shell_command
            if not shell_command:
                return False
            # `is_read_only_bash` classifies purely on command-line TEXT. Every
            # git subcommand on its allowlist (status/log/diff/show/branch/tag/
            # remote/rev-parse/describe/ls-files/ls-tree/cat-file/blame) still
            # runs git, and git reads repo-local .git/config on invocation —
            # core.fsmonitor and core.hooksPath name a program from that file,
            # not from the command line, so the classifier (and the flag-based
            # _EXEC_FLAGS fences right above it in state.py, which only close
            # channels a command-line FLAG can select) cannot see either one.
            # A heartbeat cycle's CWD is exactly the kind of place a local
            # writer could plant a malicious .git/config; on the
            # interactive/cron paths a human or an already-attended context
            # backstops this, but heartbeat has no approver at all, ever, so
            # "read-only-looking git command" is not a safe basis for
            # auto-approval here specifically. Exclude ALL git invocations
            # from heartbeat's shell trust rather than the four subcommands
            # actually observed exploitable — the vector is git running at
            # all, not which subcommand was passed.
            #
            # command_invokes_program (not a raw first-word check): a naive
            # `cmd.split()[0] == "git"` is defeated by ANY compound command
            # (`pwd; git status`) or case variation (`GIT status` on a
            # case-insensitive filesystem) — `is_read_only_bash` itself splits
            # on &&/||/;/newline and lowercases each segment's verb before
            # classifying it, so a later `git status` segment still passes as
            # read-only even when the leading token is something else
            # entirely. command_invokes_program shares that exact splitting
            # (state.py) so this exclusion can never be narrower than what the
            # classifier will actually approve.
            if command_invokes_program(shell_command, "git"):
                return False
            return is_read_only_bash(shell_command)
        # Falls through to the tool_name/mcp_server_name matching below.
    elif not tool_name:
        return False
