...
class GatewayOrchestrator:
    ...
    async def _init_services(self) -> None:
        ...
        try:
            ...
            # Off-loop, with the fork refresh deferred and the app-agent
            # leftover-verify skipped: the rebuild gates readiness, and both
            # walks scale with usage (fork count, enabled apps).
            # rebuild_agent_config owns the fork deferral (skip + background
            # schedule together), so this caller cannot get half the pair.
            path = await asyncio.to_thread(
                rebuild_agent_config, refresh_forks="defer", app_agent_refresh=False
            )
