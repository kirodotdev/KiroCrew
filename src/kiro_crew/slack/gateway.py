...
class GatewayOrchestrator:
    ...
    async def _fire_dashboard_nudge(
        ...
        if scheduled:
            try:
                ...
                # A transient filesystem failure is not proof that gateway-owned
                # provenance is absent. Keep the loop and retry on the next beat.
                await self._audit_scheduled_message_refused(
                    loop, "trusted composer provenance temporarily unreadable"
                )
                return _delivery_result(wake_message, MonitorDispatchResult.BUSY)
    ...
    async def _init_autonudge(self) -> None:
        """Purge unsafe scheduled plaintext, then start AutoNudge when enabled."""
        # Scheduled provenance contains replayable user text. Its confinement
        # decision belongs to process boot, not to the optional automation
        # service, so an unconfined host purges it even when AutoNudge is off.
        await asyncio.to_thread(autonudge_selfarm.purge_scheduled_message_records_if_unconfined)
        ...
        def _observer(event: str, loop: NudgeLoop | None) -> None:
            ...
            if self.dashboard_state and loop is not None:
                ...
                if scheduled and event_snapshot != "removed":

                    async def _broadcast_scheduled_message() -> None:
                        try:
                            ...
                            # This projection is advisory. A transient protected-store
                            # failure suppresses the frame but never mutates the loop; a
                            # later observer event or GET can publish it once readable.
                            logger.warning(
                                "scheduled-message provenance temporarily unavailable "
                                "for owner broadcast of %s",
                                loop_id,
                                exc_info=True,
                            )
                            return
                        # A newer update or removal supersedes this detached read.
                        # No await separates this generation check from publish,
                        # so another observer invocation cannot interleave there.
                    ...
                    task = asyncio.create_task(_broadcast_scheduled_message())
                    self._background_tasks.add(task)
                    task.add_done_callback(self._background_tasks.discard)
                    return
                broadcast(
