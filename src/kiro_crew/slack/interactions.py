...
def _inflight_transient_start(msgs: list[dict]) -> int | None:
    ...
    ``mc_inline_stop_*``). Only the handler's in-flight registry establishes
    that a turn is RUNNING — a marker lingering in a fetched thread proves
    nothing (its deletion failure is swallowed, and the no-registry import
    branch treats a lingering marker as a crashed turn's leftover). This
    helper only LOCATES the marker so that branch can drop it. The action_id
    alone identifies it — no other message carries an ``mc_inline_stop_``
    control.
    """
...
async def _handle_inline_stop(
    ...
    # Stale-control guard: ``stop_turn`` is keyed by SESSION, not by turn, so
    # a leftover Stop (its working message outliving the turn through a
    # swallowed deletion failure) would cancel whichever turn runs NEXT. The
    # in-flight registry is the authority: a threaded Stop click is honored
    # only when its message IS the thread's live working marker. A mismatch
    # (another turn's marker is live, or none is) marks the control stale —
    # neutralize it instead of stopping. Threadless payloads cannot be
    # verified and fall through to ``stop_turn``'s own idle handling.
    _click_thread_ts = str((payload.get("message") or {}).get("thread_ts") or "")
    if _click_thread_ts and inflight_turn_marker(_click_thread_ts) != msg_ts:
        if _orch.slack and channel and msg_ts:
            try:
