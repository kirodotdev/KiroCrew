    ...
    *channel_agent* overrides the default agent for this channel (set via
    ...
    if linked_session_key and _dashboard_state and accumulated and not _skip_writes:
        try:
            ...
            if slot:
                # The turn has already run, so this mirror is an EGRESS: persisted, then
                # served to dashboard readers. Scrub it as every other user row is.
                mirrored = redact_row_via_context(text)
                slot.append("user", mirrored, "msg msg-u")
                slot.append("assistant", accumulated, "msg msg-a")
                if slot._on_message:
                    slot._on_message(
                        slot.key, {"role": "user", "content": mirrored, "cls": "msg msg-u"}
                    )
                    slot._on_message(
