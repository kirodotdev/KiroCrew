...
from typing import Any, Iterator, Literal, Mapping, MutableMapping, NamedTuple, cast
...
def _install_heartbeat_agent() -> None:
    ...
    The default is ``{}``, so a standalone install contributes no additional
    MCP server here — the assembled ``kirocrew-core`` entry is still
    unconditionally stripped of ``autoApprove`` below, on every install
    including standalone (the bypass that strip closes is not conditioned on
    a companion existing).
