...
def rebuild_agent_config(
    *, clean: bool = False, refresh_forks: bool | Literal["defer"] = True
    ...
    def _finalize_and_write() -> None:
        ...
        _ownership_fully_read = _ownership_full_at_start and _ownership_full_now
        ...
        for _n in _narrowed_away | _gated_aliases:
            ...
            if _bases:
                ...
                if _n in _vouched_sources or (_n not in _ever_app_owned and _n in _vouched):
                    ...
                    continue
                if all(_base_may_still_mount(_b) for _b in _bases):
                    _exempt_mounts.add(_n)
                if all(_base_still_grants(_b) for _b in _bases):
                    _exempt_grants.add(_n)
            elif _n in _vouched or _ownership_fully_read:
                # No app claims this name or its family. A source that still declares
                # it is enough on its own; otherwise BOTH reads must have seen every
                # claim, since a claim missed at the start and an owner gone by the
                # end leave a name that looks unowned and never was.
                _exempt_mounts.add(_n)
                _exempt_grants.add(_n)
