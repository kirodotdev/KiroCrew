...
class ForeignSpecCeilingUnverified(RuntimeError):
    """A session may not start: the shared agent home is foreign-owned and its
    spec pre-authorizes grants this instance's governance ceiling denies (or
    the spec cannot be read to verify them)."""


class ForeignSpecSnapshot(NamedTuple):
    """What the foreign-ceiling gate VERIFIED, so a later check can prove the
    host consumed those bytes and an in-process projector can hand out exactly
    them. Same bracket doctrine as :class:`DerivedSpecSnapshot`: the window
    between this gate and the host's own read cannot be locked (the reader is
    a subprocess), so it is closed by detection instead."""

    identity: str
    """The spec file's identity; the fingerprint below is of the bytes THAT
    stat described — one observation, never two."""

    fingerprint: str
    """Canonical-JSON content hash of the verified spec."""

    spec: dict[str, Any]
    """The spec, parsed, exactly as the gate verified it — for an in-process
    consumer (the KAS projection) that must hand out the verified bytes rather
    than take a second observation of the file."""

    path: str
    """Where the verified bytes live, for the post-consumption re-check."""


#: Bounds for the foreign-ceiling gate's stat-read-stat bracket, mirroring the
#: derived-spec gate's retry budget: a file that will not hold still yields a
#: refusal, never a torn snapshot.
_FOREIGN_SPEC_OBSERVATION_ATTEMPTS = 3


def require_foreign_spec_ceiling(
    agent: str | None, project_dir: str | Path | None = None
) -> "ForeignSpecSnapshot | None":
    """Fail closed: when this instance DECLINED the shared agent home, refuse
    to start a session against a kept FOREIGN owned spec that pre-authorizes
    grants this instance's own governance ceiling denies.

    The decline arms of :func:`_decline_shared_agent_home` keep another
    instance's specs in place rather than overwriting them. Those specs'
    ``allowedTools``/``autoApprove`` were ceiling-filtered by the WRITING
    instance (:func:`_apply_allowed_tools_ceiling`) under ITS data home's
    policy — not this one's. A pre-authorized grant never reaches the
    PreToolUse gate (that filter's stated contract), so a looser foreign
    ceiling would hand this instance's sessions a gate exemption its own
    policy denies — durably, because the decline arm never rewrites the spec.

    Enforced HERE, at the same spawn seam as :func:`require_fork_governance`,
    because the spec cannot be tightened in place: ``allowedTools`` on the
    shared home is user-owned (a foreign strip would durably narrow the
    OWNER's grants), and writing the shared home from this instance is exactly
    what the guard refuses. The verification is one half of a bracket: it
    returns a :class:`ForeignSpecSnapshot` of exactly what it verified, and
    the caller proves the host consumed THOSE bytes — a disk-reading host via
    :func:`require_unchanged_foreign_spec` once its handshake confirms the
    read, an in-process projector by handing out ``snapshot.spec`` itself —
    because a write landing between this gate and the host's own read would
    otherwise be consumed unjudged.

    Deliberate no-ops answering ``None`` (not applicable, never "verified"),
    each because the exposure this closes cannot exist there: an instance that
    owns its agent home (its own rebuild filtered the spec); an agent whose
    spec is not an OWNED file (a user-authored spec is the user's own grant
    decision, identical on a default-home instance); an absent spec (nothing
    is pre-authorized; the session dies at "Mode not found" regardless); a
    project-shadowed name (kiro-cli resolves ``--agent`` against
    ``<cwd>/.kiro/agents`` first, so the foreign shared spec is never
    consumed — the shadow is user-authored); and an ungoverned host
    (:func:`_may_auto_approve` permits everything without a ceiling). An
    unreadable or unsettled existing spec refuses — verifying nothing and
    proceeding is the bypass itself. Raises
    :class:`ForeignSpecCeilingUnverified` only.
    """
    if not agent:
        return None
    spec_name = next((name for name in OWNED_KIRO_AGENT_FILES if Path(name).stem == agent), None)
    if spec_name is None:
        return None
    if _decline_shared_agent_home(audit=False) is None:
        return None  # this instance owns its agent home; its own rebuild filtered the spec
    spec_path = kiro_agents_dir_path() / spec_name
    if not spec_path.exists():
        return None
    if agent in project_agent_names(
        project_dir, operation="require_foreign_spec_ceiling", source="unknown"
    ):
        return None
    # ONE observation, bracketed stat-read-stat like the derived-spec gate: the
    # bytes verified below and the identity that vouches for them must belong to
    # the same instant, or the snapshot is a torn pair and the post-consumption
    # check built on it proves nothing.
    data: dict[str, Any] | None = None
    identity: str | None = None
    for _ in range(_FOREIGN_SPEC_OBSERVATION_ATTEMPTS):
        identity = _file_identity(spec_path)
        if identity is None:
            return None  # vanished: nothing left to consume, "Mode not found" follows
        data = _read_spec_capped(spec_path)
        if data is not None and _file_identity(spec_path) == identity:
            break
        data = None
    if not isinstance(data, dict) or identity is None:
        raise ForeignSpecCeilingUnverified(
            f"the shared agent spec {spec_path} belongs to another Kiro Crew "
            "instance and could not be read coherently to verify its "
            "pre-authorized grants against this instance's governance ceiling; "
            "refusing to start a session on unverifiable permissions"
        )
    violations: list[str] = []
    allowed = data.get("allowedTools")
    if isinstance(allowed, list):
        violations.extend(
            ref for ref in allowed if isinstance(ref, str) and not _may_auto_approve(ref)
        )
    servers = data.get("mcpServers")
    if isinstance(servers, dict):
        violations.extend(
            f"@{mcp_server_alias(ref)} (autoApprove)"
            for ref, server_spec in servers.items()
            if isinstance(server_spec, dict)
            and server_spec.get("autoApprove")
            and not _may_auto_approve(f"@{mcp_server_alias(ref)}")
        )
    if violations:
        raise ForeignSpecCeilingUnverified(
            f"the shared agent spec {spec_path} belongs to another Kiro Crew "
            "instance and pre-authorizes grants this instance's governance "
            f"ceiling denies ({', '.join(sorted(violations))}); pre-authorized "
            "tools never reach the PreToolUse gate, so refusing to start the "
            "session. Remove the stale specs and restart, or run under the "
            "default data home."
        )
    return ForeignSpecSnapshot(
        identity=identity,
        fingerprint=_foreign_spec_fingerprint(data),
        spec=data,
        path=str(spec_path),
    )


def _foreign_spec_fingerprint(spec: dict[str, Any]) -> str:
    """Content fingerprint of a verified foreign spec — canonical JSON, whole spec.

    CONTENT, not mtime, for the derived gate's reasons: same-tick writes, restored
    backups and stepped clocks all fake a plausible identity. The WHOLE spec rather
    than the grant keys alone, because the snapshot's other consumer (the KAS
    projection) hands out the full dict — a change anywhere in it means the bytes
    projected are not the bytes verified.
    """
...
def require_unchanged_foreign_spec(snapshot: "ForeignSpecSnapshot | None") -> None:
    """Prove the foreign spec has not changed since *snapshot* was taken. Fails closed.

    The second half of the bracket, run once the reader this process does not
    control has consumed the spec — kiro-cli's ``initialize`` (or ``set_mode``)
    response is the earliest reliable signal. A difference means the host may
    have loaded a generation whose grants nobody judged against this instance's
    ceiling, and the spec is already in another process's memory, so the only
    sound answer is to end the session. A re-verification of the CURRENT bytes
    would not do: the host may have consumed an intermediate write this check
    never sees.

    ``None`` short-circuits — the gate answers ``None`` for every state without
    the foreign-spec exposure, so the bracket is not applicable rather than
    satisfied. An identity match passes cheaply; on mismatch the current content
    is re-read and a matching fingerprint passes (a rewrite with identical
    bytes changed nothing the host could have consumed differently). Raises
    :class:`ForeignSpecCeilingUnverified` on any real difference AND on a
    re-check that cannot be performed.
    """
    if snapshot is None:
        return
    spec_path = Path(snapshot.path)
    if _file_identity(spec_path) == snapshot.identity:
        return
    current = _read_spec_capped(spec_path)
    if isinstance(current, dict) and _foreign_spec_fingerprint(current) == snapshot.fingerprint:
        return
    raise ForeignSpecCeilingUnverified(
        f"the foreign shared agent spec {snapshot.path} changed while the session "
        "was loading it, so the host may have consumed pre-authorized grants this "
        "instance's governance ceiling never judged; ending the session "
        f"(verified {snapshot.fingerprint[:12]})"
    )
