...
def markdown_spec_for_agent(agent: str, work_dir: str | Path | None = None) -> Path | None:
    ...
    alone and answers a ``set_mode`` for a markdown-only agent with ``Mode not
    found``, so the kiro-cli harness asks this before spawning and refuses with
    a message that names the file instead. The question is therefore whether
