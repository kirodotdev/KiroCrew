...
@dataclass
class _BindingProof:
    """A post-bind report retained for the lifetime of its exact child handle."""

    port: int
    reported: threading.Event
    root_start_id: str | None = None
...
def _listener_lookup_functional() -> bool | None:
    ...
    """
    global _listener_lookup_self_test_cache
    tool_path = platform_compat.trusted_system_bin(platform_compat.listening_pid_tool())
...
def _port_owner(port: int, proc: subprocess.Popen[bytes] | None) -> str:
    ...
    child. The self-test runs only after the target lookup returns empty. Only a
    completed target negative plus a control lookup that proves the tool functional
    ...
    if not _root_process_identity_matches(proc, "before listener lookup"):
        return _OWNER_FOREIGN
    if not platform_compat.listening_pid_tool_available():
    ...
    if identities is None:
        return _OWNER_UNPROVEN
    if not _root_process_identity_matches(proc, "after ancestry lookup"):
        return _OWNER_FOREIGN

    candidates: list[platform_compat.ProcessDescendantIdentity] = []
    if proc.pid in owners:
        proof = _process_binding_proof(proc)
        root_start = proof.root_start_id if proof is not None else None
        if root_start is None:
            return _OWNER_FOREIGN
        candidates.append(platform_compat.ProcessDescendantIdentity(proc.pid, 0, root_start))
    ...
    stable_before = [
        ...
        if _process_start_identity(identity.pid) == identity.start_time
    ]
    ...
    if not _root_process_identity_matches(proc, "after confirming listener lookup"):
        return _OWNER_FOREIGN
    for identity in stable_before:
        if (
            identity.pid in confirming_owners
            and _process_start_identity(identity.pid) == identity.start_time
        ):
...
def _drain_child_output(
    ...
    fill the pipe. A host without nonblocking pipe reads skips proof parsing but
    still drains stdout in this daemon thread.
    """
    read_chunk = _binding_chunk_reader(stream)
    if read_chunk is None:
        proof.invalidate()
        _discard_child_output(stream.read)
        return
...
def _child_reported_port(proc: subprocess.Popen[bytes], port: int) -> int | None:
    """Return the recognized-address port reported by this exact child."""
    proof = getattr(proc, "_kirocrew_browser_view_binding", None)
    if not isinstance(proof, _BindingProof) or proof.port != port:
        return None
    return proof.reported_port()
...
_STRUCTURAL_BINDING_PORT_ATTR = "_kirocrew_browser_view_structural_binding_port"


@dataclass(frozen=True)
class _StructuralBindingProof:
    """One startup report bound to the process triple that produced it."""
...
def _remember_structural_binding(proc: subprocess.Popen[bytes], port: int) -> None:
    """Bind a startup report to this handle, root start ID, and listener port."""
    binding = getattr(proc, "_kirocrew_browser_view_binding", None)
    root_start_id = binding.root_start_id if isinstance(binding, _BindingProof) else None
    if not isinstance(root_start_id, str) or not root_start_id:
        return
    setattr(
        proc,
        _STRUCTURAL_BINDING_PORT_ATTR,
        _StructuralBindingProof(proc, root_start_id, port),
    )


def _has_structural_binding(proc: subprocess.Popen[bytes], port: int) -> bool:
    """Whether the exact report-bound process is still live and healthy."""
    cached = getattr(proc, _STRUCTURAL_BINDING_PORT_ATTR, None)
    binding = getattr(proc, "_kirocrew_browser_view_binding", None)
    return (
        isinstance(cached, _StructuralBindingProof)
        and cached.handle is proc
        and cached.port == port
        and isinstance(binding, _BindingProof)
        and cached.root_start_id == binding.root_start_id
        and _alive(proc)
        and _process_start_identity(proc.pid) == cached.root_start_id
        and _healthy(port)
    )
...
def _root_process_identity_matches(proc: subprocess.Popen[bytes], phase: str) -> bool:
    """Whether this live handle still names the root process spawned for it."""
    proof = _process_binding_proof(proc)
    captured = proof.root_start_id if proof is not None else None
    alive = _alive(proc)
    current = _process_start_identity(proc.pid) if alive else None
    if captured is not None and current == captured:
        return True
    logger.debug(
        "browser view root identity changed %s: pid=%d, captured=%r, current=%r, alive=%s",
        phase,
        proc.pid,
        captured,
        current,
...
def _verify_child_listener(
    ...
    structurally blind host. Once bound to the exact handle, root start ID, and
    port, that fact stays valid only while the same process remains alive,
    identity-stable, and healthy.
    """
    owner = _port_owner(port, proc)
    fresh_report = _child_reported_binding(proc, port)
    if not _root_process_identity_matches(proc, "after global ownership lookup"):
        return False, False
    if owner == _OWNER_CHILD:
        identity = _take_port_owner_identity_proof(proc, port)
        if identity is None:
            return False, False
        current = _process_start_identity(identity.pid)
    ...
    inconclusive = descendants is None or structurally_blind or per_process is None

    for identity in descendants or []:
        before = _process_start_identity(identity.pid)
        ...
        after = _process_start_identity(identity.pid)
        ...
        if per_process is None:
            inconclusive = True
    if not _root_process_identity_matches(proc, "after listener verification"):
        return False, False
    if not inconclusive:
        return False, False
...
def _recorded_state() -> bool | None:
    ...
    definitive negative. On a structurally blind host, the exact child's startup
    report remains valid for the same handle, root identity, and port while that
    process stays alive and healthy. It cannot authorize a replacement handle.
    ...
    if _proc is None or not _alive(_proc) or _info is None or _child_port is None:
        return False
    if not _root_process_identity_matches(_proc, "before reuse health check"):
        logger.warning(
    ...
    if not _healthy(_info.port):
        ...
        _last_reason = reason
        return False
    if not _root_process_identity_matches(_proc, "after reuse health check"):
        logger.warning(
        ...
        _last_reason = _OWNERSHIP_REASON
        return False
    if _has_structural_binding(_proc, _child_port):
        _last_reason = None
        return True
    ...
    if verified is False:
        ...
        _last_reason = _OWNERSHIP_REASON
        return False
    if _structurally_blind_listener_attribution():
        if _last_reason != _OWNERSHIP_REASON:
            logger.warning(
                "browser view process on port %d has no live handle-bound listener proof",
                _child_port,
            )
        _last_reason = _OWNERSHIP_REASON
        return None
...
def _spawn(command: list[str], port: int) -> subprocess.Popen[bytes] | None:
    ...
    root_start_id = _process_start_identity(proc.pid)
    if root_start_id is None:
        logger.warning(
    ...
    with _lock:
        ...
        while time.monotonic() < deadline:
            ...
            if child_port is None:
                ...
                _remember_structural_binding(proc, child_port)
                if pin_listener is not None:
            ...
            if _healthy(child_port):
                ...
                elif _has_structural_binding(proc, child_port):
                    verified, via_report = True, True
                else:
