...
class WhatsAppConfig:
    ...
    db_path: str = field(
        ...
        metadata=_meta(
            "Session DB Path",
            "Optional override for the pairing session database. Empty (the "
            "default) stores it at <data home>/whatsapp/session.db.",
            tags=["whatsapp"],
        ),
    )
    soft_threshold_pct: int = field(
    ...
    def load(cls) -> KiroCrewConfig:
        ...
        cfg = cls(
            ...
            whatsapp=WhatsAppConfig(
                ...
                soft_threshold_pct=max(
                    1, min(100, _coerce_int(whatsapp_data.get("soft_threshold_pct"), 80))
                ),
                hard_threshold_pct=max(
                    1, min(100, _coerce_int(whatsapp_data.get("hard_threshold_pct"), 95))
                ),
            ),
