...
class KiroCrewAgentConfig:
    ...
    session_color: str = field(
        ...
        metadata=_meta(
            "Session Color",
            "Default session color applied to new sessions using this agent. "
            "Accepts a CSS hex color string (#rrggbb, lowercase). When set, new "
            "sessions created with this agent are automatically tinted with this "
            "color — the same rendering as a manually-picked custom color. "
            "A manual per-session override or the dashboard-level default color "
            "policy still wins if explicitly set. Empty means no agent color.",
        ),
    )
