...
3. On a later turn verify session-bound state and the applied transcript notice. Every cycle checks the exit condition and reports only real signals. A missing permission, credential, configuration, dependency, or failing tool/build/test is remediation work, not a terminal condition: inspect the owning code or config, make the least-privileged allowed fix, verify it, and keep the loop active. Remediation NEVER means granting yourself approvals or permissions, weakening or bypassing approval policy, or editing governance controls (`security_policy.json`, profiles, `denied_commands.json`, trust roots); only an already-allowed least-privilege owner/config repair is permitted. If only the user can grant the next approval, report that once and recheck it on later cycles while doing any other safe work. Call `autonudge_stop` only when the objective or Definition of Done is met, the user asks to stop, a configured STOP sentinel fires, or a host/tooling failure remains unrecoverable after bounded retries and no safe work remains. `max_cycles` is a runaway backstop, NOT success.
