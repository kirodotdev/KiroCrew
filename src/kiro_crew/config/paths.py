...
def default_workspace_dir() -> Path:
    ...
    ``test_no_provider_invents_its_own_no_cwd_fallback`` is what keeps the two in step.
    """
...
def resolved_cwd(cwd: str, session_key: str | None = None) -> str:
    ...
    A provider handed a falsy ``work_dir`` binds to a default, so ``provider.cwd`` reports
    that concrete path and never the empty string, in the platform's own spelling. Comparing
    a raw stored value against it is two bugs waiting: a cleared project's ``""`` never
    matches, and a directory differing only by separator reads as a DIFFERENT one.

    ``session_key`` is REQUIRED for the empty case: the provider FACTORY resolves an empty
    ``cwd`` to a PER-SESSION directory (``workspace_root()/<key>``), not to the shared
    :func:`default_workspace_dir`. Answering the shared root compares a claim against a
    directory no provider binds -- so the slot cold-starts every turn -- and any caller that
    BINDS it collapses sessions meant to be isolated onto one directory.
