...
def resolved_cwd(cwd: str, session_key: str | None = None) -> str:
    ...
    A provider handed a falsy ``work_dir`` binds to a default, so ``provider.cwd``
    reports that concrete path in the platform's own spelling. Comparing a raw stored
    value against it is two bugs waiting: a cleared project's ``""`` never matches, and
    a directory differing only by separator reads as a DIFFERENT directory.

    ``session_key`` is REQUIRED for the empty case, because the provider FACTORY
    resolves an empty ``cwd`` to a PER-SESSION directory (``workspace_root()/<key>``),
    not to the shared :func:`default_workspace_dir`. Answering the shared root would
    compare a claim against a directory no provider binds -- so the slot cold-starts
    every turn -- and would collapse sessions meant to be isolated onto one directory.
