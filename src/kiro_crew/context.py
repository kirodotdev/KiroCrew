...
#: The tags a PERSISTED ``dashboard.language`` may actually resolve to, mirroring
#: the frontend's ``isRestorableLanguage()`` (``website/src/i18n/languages.ts``)
#: for a production build: registered in ``SUPPORTED_LANGUAGES`` and not
#: ``devOnly``. Equal to the frontend's ``DETECTABLE_CODES``.
#:
#: Shape alone is not enough here, and that is the whole of :issue:`1130`. The
#: writer's validator documents the opposite conclusion — "an
...
#: catalog for" (``dashboard/handlers/core.py``). That is true of the SPA and
#: false of this module: ``resolveLanguage()`` gates a stored choice on
#: ``isRestorableLanguage()`` and falls back when it fails, but the block below
#: forwarded the raw tag regardless. A catalog-less ``ja`` therefore rendered
#: English chrome around Japanese tool-call purposes, and those purposes persist
#: in session history, so the mismatch outlived a switch back to a shipped
#: language. Membership is checked where the tag is resolved so the agent and
#: the chrome answer the same question.
#:
#: EXACT membership, deliberately: for a *stored* value the SPA's
#: ``isRestorableLanguage()`` is ``SUPPORTED_CODES.includes(code)``. The loose
#: primary-subtag matching in ``detect.ts`` (``zh-Hans`` → ``zh-CN``) applies
#: only to BROWSER tags inside ``detectBrowserLanguage()``. Mirroring the loose
#: rule here would reintroduce the bug in a new form: a stored ``zh-TW`` would
#: inject ``zh-TW`` while the SPA, finding it unrestorable, renders whatever the
#: browser asked for.
#:
#: Kept in sync by ``test/test_ui_language_catalog_parity.py``, which parses
#: ``SUPPORTED_LANGUAGES`` out of the frontend source and fails naming the gap —
#: the same guard ``test_artifact_import_parity.py`` puts on ``_EXT_KIND_MAP``.
#: A silently drifting copy is what the comments cited above warn against; a
#: copy that cannot drift without failing CI is this repository's usual answer.
_RESTORABLE_UI_LANGUAGES = frozenset(
    {"en", "zh-CN", "hi", "es", "fr", "bn", "pt", "ru", "de", "it"}
)


def ui_language_tag(cfg: "KiroCrewConfig") -> str:
    """Return ``dashboard.language`` as a shipped BCP-47 tag, or ``""``.
    ...
    ``""`` means "the backend does not know" — nothing was chosen (the "follow
    the browser" sentinel, resolved in the SPA's ``resolveLanguage()``), the
    stored value is not tag-shaped, or it is shaped like a tag but names no
    shipped catalog. Callers must treat it as unknown rather than as English.

    That last case is :issue:`1130`. A shape-valid, catalog-less tag is not a
    language this install can render, so the SPA's ``resolveLanguage()`` ignores
    it and falls back — and a caller told otherwise would steer the model into a
    language the surrounding chrome is not in. Returning ``""`` for it makes the
    agent agree with the interface instead, and is the same answer the Auto
    sentinel gets, because both mean the same thing: the backend cannot name the
    rendered language. See ``_RESTORABLE_UI_LANGUAGES``.
    """
    ...
    if not lang or not _UI_LANGUAGE_TAG_RE.match(lang):
        return ""
    if lang not in _RESTORABLE_UI_LANGUAGES:
        return ""
    return lang
...
def _build_ui_language_section(cfg: "KiroCrewConfig") -> str:
    ...
    Returns "" when ``ui_language_tag()`` cannot name the rendered language —
    either ``dashboard.language`` is empty, the "follow the browser" sentinel
    whose resolution happens in the SPA's ``resolveLanguage()``, or it names no
    shipped catalog and so the SPA falls back to detection as well. In both cases
    the backend genuinely does not know the answer, so there is nothing truthful
    to inject. Installs that never picked a language therefore see
    ...
    ``str``, tag-shaped (``_UI_LANGUAGE_TAG_RE``) and a language this install
    ships a catalog for (``_RESTORABLE_UI_LANGUAGES``), so neither a malformed
