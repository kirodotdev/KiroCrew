...
@dataclass
class FeishuInboundMessage(InboundMessage):
    """InboundMessage enriched with Feishu-specific routing fields.

    ``message_id`` is the reply anchor for :meth:`FeishuTransport.send_message`.
    ``chat_type`` / ``chat_id`` let the dispatcher apply group-access policy.
    Consumers use ``getattr(msg, "message_id", "")`` so the neutral
    ``InboundMessage`` stays channel-agnostic.
    """
...
class FeishuTransport(MessagingTransport):
    ...
    def __init__(
        ...
        *,
        ...
        self._allowed_group_ids: frozenset[str] = frozenset(
            g for g in allowed_group_ids if g
        )
    ...
    def configured_targets(self) -> list[ConfiguredChannelTarget]:
        return [
            ConfiguredChannelTarget(
                ...
                available=False,
                unavailable_reason=(
                    "Feishu only allows replies to an inbound message "
                    "(no proactive DM in v1)"
                ),
            )
    ...
    async def receive(self, raw_envelope: Any) -> None:
        ...
        if inbound.chat_type == "group":
            if not (
                self._allow_group and inbound.chat_id in self._allowed_group_ids
            ):
                sel().log_api_access(
