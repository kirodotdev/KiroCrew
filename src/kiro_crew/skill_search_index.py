...
class SkillSearchIndex:
    ...
    def sync(
        ...
        *live_keys*, when given, is the full set of keys currently visible to the
        ...
        """
        with self._lock:
            return self._sync_locked(rows, live_keys)
    ...
    def _read_terms(self, path: str | Path) -> frozenset[str] | None:
        ...
        """
        try:
            raw = safe_read_file_bytes_nolink(str(path), max_bytes=_MAX_INDEXED_BODY_BYTES)
