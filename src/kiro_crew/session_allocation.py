...
class RetireArm:
    ...
    that counter to learn it is stale. Holding them in one record makes that asymmetry a method.

    ``cwd`` is already through ``resolved_cwd``, so a cleared project is the concrete
    default-workspace path, never an empty string. ``None`` means the arm states NO directory,
    which is not the same as stating the default -- see ``_record_arm_cwd``.

    ``requires_sid_clear`` is provenance a directory cannot carry: a discard bumps the generation
    without moving the project, so an arm can name a resolved cwd from an earlier change while
    its conversation was thrown away. Dropped by ``spend`` with the other per-episode facts,
    since the next conversation has its own SID.
    """
...
class SessionRegistryState:
    ...
    # Keys whose next claim must NOT be served a reused session, armed when a teardown was
    # refused. Keyed by string so a cold start that has not registered yet is covered too.
...
class SessionAllocationService:
    ...
    def _generation(self, folded: str) -> int:
        ...
        A read never creates a record, so a key never armed reports zero rather than an entry.
        """
    ...
    def spend_retire_arm(self, key: str) -> None:
        ...
        Called only where the slot itself is gone -- ``destroy``, which deletes the session-map
        entry. The arm names a directory a SUCCESSOR must bind, so once no successor can arrive
        the only claim left is a different slot recreated under the same name, which would
        silently inherit the previous project.

        Cleanup that keeps the slot (``remove``, ``remove_if_unclaimed``) calls nothing: the arm
        is still owed to a real claim, and doubles as the retry target for a start it evicts.

        The GENERATION survives only while a start may still read it: the reap drops the whole
        record once neither a live session nor a claim in flight can.
        """
        folded = self._owner._fold_key(key)
        if (arm := self._arm_if_any(folded)) is not None:
            arm.spend()
            self._reap_arm_if_quiescent(folded)
    ...
    def supersede_arm_for_new_slot(self, key: str, *, only_generation: int | None = None) -> None:
        ...
        Two seams call this, and neither teardown verb can: ``destroy`` spends the arm, while the
        close and sweep paths run ``remove``, which preserves it. So the arm outlives the SESSION
        while the SLOT is gone.

        Slot MINT is the first seam: a freshly minted slot has never been armed, so any arm on
        its key belongs to a previous occupant, whose relative writes would otherwise land in
        that occupant's project. FINAL slot teardown is the second, without which a slot closed
        and never recreated leaks both PENDING entries.

        Reclaims the two pending maps, then bumps the GENERATION and keeps it while a start may
        still read it -- clearing it under one would let that start bind here. ``only_generation``
        narrows this to a RETRACTION of one arm, returning early once another producer has armed.
        """
        folded = self._owner._fold_key(key)
    ...
    def _reap_arm_if_quiescent(self, folded: str) -> None:
        """Drop a spent record once no start can still consult its generation.

        The counter outlives the arm only to refuse a start that began before the change it
        must honour; a key with no session and no claim in flight has none, and a later start's
        zero compares equal.
        """
        arm = self._arm_if_any(folded)
        if arm is None or arm.cwd is not None or arm.agent is not None or arm.requires_sid_clear:
            return
        if folded not in self._sessions and not self._allocation_reservations.get(folded):
            del self.state.retire_arms[folded]
    ...
    def note_conversation_discarded(self, key: str) -> None:
        ...
        Arms no cwd: a clear moves no directory, so the
    ...
    def mark_retire_on_next_claim(
        ...
        For a teardown that had to be REFUSED. The refusal keeps a streaming reply alive, but
        the reason for the teardown does not, so the next claim must not be handed that
        session either.

        Records the KEY, not just the object, because "no session registered" is NOT the same
        as "nothing to protect against": a cold start holds no registry entry until it
        finishes, so a probe in that window sees nothing while a provider bound to the
        pre-change directory is already on its way. `_reacquire_and_validate` consumes the key,
        so a session registering AFTER this call is refused on its next claim just the same.

        A turn already streaming keeps its provider, so the guarantee is that no LATER turn is
        served the stale session. Returns the GENERATION recorded here, which is what lets a
        producer retract its OWN arm via :meth:`supersede_arm_for_new_slot`.
        """
        folded = self._owner._fold_key(key)
    ...
    def transfer_retire_arm(self, from_key: str, to_key: str, cwd: str | None) -> None:
        ...
        For a slot that REBOUND between arming and consuming. Arming the live key alone would
        leave the abandoned key's arm in the map, where nothing drops it: an arm is cleared by
        the claim that satisfies it, and no claim arrives for a key the slot left. The map is
        keyed by STRING, so a later session under that key reads an arm naming a directory
        chosen for a binding that is gone.

        Distinct from :meth:`spend_retire_arm`, which drops an arm no successor is owed; here
        the arm is still owed and only its ADDRESS was wrong. One synchronous step, so the
        requirement is never recorded twice or not at all.

        An EQUIVALENT same-key re-arm keeps the generation rather than bumping it: the
        deferred-reset retry re-enters every few seconds, and each bump rejects a cold start
        still resolving its own model. A DIFFERENT target on the same key must still supersede.
        """
    ...
    async def _reacquire_and_validate(
        ...
        *,
        ...
        ``cwd`` also validates the session's BOUND directory. It is applied when a provider is
        CREATED and never re-applied, so a live session whose project has since changed would
        otherwise be handed back bound to the OLD directory, and the turn's relative writes
        would land there.

        Checked HERE rather than at the reuse decision because this runs with the semaphore
        HELD: any turn that was streaming has finished, so returning False -- which sends the
        caller through ``_evict_stale_session`` -- cannot tear a live reply down mid-stream.

        Guarded on BOTH sides being set, mirroring the pool gate: a caller passing no ``cwd``
        states no requirement and must not evict a session serving others correctly, and a
        provider tracking no real directory string reports no binding to disagree with.
        """
        ...
        try:
            async with self._lock:
                ...
                # The arm names an ALIAS while the session records the RESOLVED agent, so this
                # is the value pre-resolved off-thread, used only while the arm holds it.
        ...
        if not still_valid:
            try:
                if cwd_moved:
                    # Tear down BEFORE the permit is released, and ONLY for a moved directory:
                    # releasing first lets another acquirer win a free permit mid-eviction.
                    await self._evict_stale_session(key, session)
            finally:
                # In a `finally` so a cancellation mid-eviction cannot leave this permit held
                # and wedge the key; safe to release early because the check is IDEMPOTENT.
                session.semaphore.release()
        else:
            session.turn_owner = asyncio.current_task()
        return still_valid
        ...
        try:
            async with self._lock:
                ...
                if existing is not None and not recycling:
                    ...
                    # The cwd match is NOT checked here: this runs before the semaphore is
                    # claimed, so evicting now could tear down a reply that is still streaming.
