...
class RetireArm:
    ...
    The three facts share a key and NOT a lifetime: spending an arm drops the directory and
    the agent it names, while the generation must survive, because a start compares itself
    against that counter to learn it is stale.
    ...
    generation without moving the project, so an arm can hold a resolved cwd from an
    EARLIER change while naming a conversation that was thrown away. A retry reading only
    the directory would state a real path, so the resume guard would not fire.
    """
...
class SessionRegistryState:
    ...
    # refused. Keyed by string so it still covers a cold start that has not registered yet.
...
class SessionAllocationService:
    ...
    def supersede_arm_for_new_slot(self, key: str, *, only_generation: int | None = None) -> None:
        ...
        Neither teardown verb can do this: ``destroy`` spends the arm, while the close and
        sweep paths run ``remove``, which preserves it deliberately -- so the arm
        legitimately outlives the SESSION while the SLOT is gone. Two seams call it. Slot
        MINT, because any arm on a freshly minted slot's key belongs to a previous occupant,
        whose relative writes would land in that occupant's project (slot REUSE returns
        first); and FINAL slot teardown, without which a closed slot leaks both PENDING maps.

        The GENERATION is bumped and RETAINED, never dropped: clearing would let a start
        still in flight compare equal to a fresh key's zero and bind here.

        ``only_generation`` narrows this to a RETRACTION of one producer's own arm. If the
        resident generation has moved on, another producer armed this key inside the
        unwinding window, so this RETURNS EARLY rather than invalidating it.
        """
        folded = self._owner._fold_key(key)
    ...
    def mark_retire_on_next_claim(
        ...
        For a teardown that had to be REFUSED. The refusal keeps a streaming reply alive,
        but the reason for the teardown does not go away, so the next claim must not be
        handed that session either.

        Records the KEY, not just the object, because "no session registered" is NOT the
        same as "nothing to protect against": a cold start holds no registry entry until it
        finishes, so a provider bound to the pre-change directory can already be on its way.

        A turn already streaming keeps its provider, so the guarantee is that no LATER turn
        is served the stale session -- not that an in-flight one is retro-corrected.

        Returns the GENERATION recorded here, which is what lets a producer retract its OWN
        arm and nothing else via :meth:`supersede_arm_for_new_slot`.
        """
        folded = self._owner._fold_key(key)
    ...
    def transfer_retire_arm(self, from_key: str, to_key: str, cwd: str | None) -> None:
        ...
        Distinct from :meth:`spend_retire_arm`: there the arm is owed to nobody, here it is
        still owed and only its ADDRESS was wrong. One synchronous step, so the requirement
        is never recorded twice or not at all.

        An EQUIVALENT same-key re-arm keeps the generation rather than bumping it, because
        the deferred-reset retry re-enters every few seconds and each bump rejects a cold
        start still resolving its own model. A DIFFERENT target is a genuine re-arm.
        """
    ...
    async def _reacquire_and_validate(
        ...
        *,
        ...
        ``cwd`` also validates the session's BOUND directory. It is applied when a provider
        is CREATED and never re-applied, so a live session whose project has since changed
        would otherwise be handed back bound to the OLD directory.

        Checked HERE rather than at the reuse decision because this runs with the semaphore
        HELD: any turn that was streaming on this provider has finished, so returning False
        cannot tear a live reply down mid-stream. It also covers every reuse path in this
        class rather than only the callers that remember to ask.

        Guarded on BOTH being set, mirroring the pool gate: a caller passing no ``cwd``
        ...
        try:
            async with self._lock:
                ...
                # A SEPARATE question from the directory -- a project-scope switch keeps it.
                # The arm names an ALIAS while the session records the RESOLVED agent.
        ...
        if not still_valid:
            try:
                if cwd_moved:
                    # Tear down BEFORE the permit is released: releasing first leaves a FREE
                    # permit on a registered session, so a racing acquirer can win it.
                    await self._evict_stale_session(key, session)
            finally:
                # In a `finally` so a cancellation mid-eviction cannot leave this permit
                # held, which would wedge the key. The check is IDEMPOTENT, so a retry re-evicts.
                session.semaphore.release()
        return still_valid
        ...
        try:
            async with self._lock:
                ...
                if existing is not None and not recycling:
                    ...
                    # The cwd match is NOT checked here: this runs BEFORE the semaphore, so a
                    # turn may be streaming. ``_reacquire_and_validate`` checks it instead.
