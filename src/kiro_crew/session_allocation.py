...
class RetireArm:
    ...
    generation without moving the project, so an arm can name a resolved cwd from an earlier
    change while its conversation was thrown away. Dropped by ``spend`` with the other
    per-episode facts, since the next conversation has its own SID.
    """
...
class SessionAllocationService:
    ...
    def spend_retire_arm(self, key: str) -> None:
        ...
        successor can arrive the only claim left is a different slot recreated under the
        same name, which would silently inherit the previous project.

        Cleanup that keeps the slot (``remove``, ``remove_if_unclaimed``) deliberately
        calls nothing: the arm is still owed to a real claim, and doubles as the retry
        target for a start the cleanup evicts.
    ...
    def supersede_arm_for_new_slot(self, key: str, *, only_generation: int | None = None) -> None:
        ...
        Two seams call this, and neither teardown verb can: ``destroy`` spends the arm, while
        the close and sweep paths run ``remove``, which preserves it deliberately. So the arm
        legitimately outlives the SESSION while the SLOT is gone.
        ...
        and never recreated leaks both PENDING entries until process exit.

        Reclaims the two pending maps only. The GENERATION is bumped and RETAINED, never
        dropped: clearing would let a start still in flight compare equal to a fresh key's zero
        and bind here. ``only_generation`` narrows this to a RETRACTION of one arm, and returns
        early without touching the generation once another producer has armed the key.
        """
        folded = self._owner._fold_key(key)
