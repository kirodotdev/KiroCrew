...
def _destination_fingerprint(cfg: KiroCrewConfig) -> str:
    """A stable, non-reversible tag for the account a heartbeat would go to.

    Buffered activity is stamped with this at enqueue time; the flush drops any
    row whose fingerprint no longer matches the current destination, so a
    heartbeat buffered under one API key or endpoint is never posted to a
    different account after the key or base URL changes between enqueue and a
    delayed flush. The API key is hashed, never stored or sent.
    """
...
def note_coding_activity(
    ...
    *,
    ...
    counts and total line-change count, and triggers a flush when the interval
...
async def _flush(batch: list[tuple[str, dict[str, Any]]]) -> None:
    ...
    Each row was stamped at enqueue with the fingerprint of the account it was
    meant for; a row whose fingerprint no longer matches the current
    destination is DROPPED rather than sent, so activity buffered under one API
    key or endpoint is never posted to a different account after a rotation
    ...
    try:
        ...
        def _load_and_fingerprint() -> tuple[KiroCrewConfig, str]:
            _cfg = KiroCrewConfig.load()
            return _cfg, _destination_fingerprint(_cfg)

        cfg, current_fp = await asyncio.to_thread(_load_and_fingerprint)
        if not (cfg.wakatime.enabled and cfg.wakatime.send_heartbeats):
            return
        rows = [hb for fp, hb in batch if fp == current_fp]
        if not rows:
            return
        client = await asyncio.to_thread(build_client, cfg)
        if client is None:
            return
        for start in range(0, len(rows), MAX_HEARTBEATS_PER_REQUEST):
            chunk = rows[start : start + MAX_HEARTBEATS_PER_REQUEST]
