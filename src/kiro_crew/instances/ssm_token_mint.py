...
def _redacted_tail(text: str, limit: int = _OUTPUT_TAIL_CHARS) -> str:
    ...
    The delegate also bounds its own scan window. That bound is not needed on
    this path -- :func:`cloud.ssm.run_command`'s ``CommandResult`` fields are
    already SSM-invocation-output-sized rather than a blind stream -- but it is
    harmless on text that small, and sharing one implementation is what keeps the
    two transports' scrubs from drifting apart again.
    """
