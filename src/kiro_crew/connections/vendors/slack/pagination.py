...
2. **Legacy page/count** (``page`` + ``count`` in; a ``paging`` object out with
   ...
   ``files.list``, ``search.files`` and ``search.all`` expose no cursor.
   ``search.messages`` additionally offers a cursormark scheme (a ``cursor``
   parameter seeded with ``*``); this slice deliberately models it as legacy
   page/count, a valid and simpler choice for a bounded walk, and does not use
   the cursormark seed.
...
class PageCountPaginator:
    ...
    _done: bool = field(default=False, init=False)
    _malformed_paging: bool = field(default=False, init=False)
    ...
    @property
    def malformed_paging(self) -> bool:
        """Whether the walk stopped because paging was missing or malformed.

        True when the walk terminated because a response's paging was
        missing/malformed rather than reaching its last page. This lets a caller
        tell an anomaly-driven (possibly TRUNCATED) stop from a genuine
        completion; termination still fails closed either way.
        """
        return self._malformed_paging
    ...
    def consume(self, response: dict[str, Any]) -> bool:
        ...
        forever on a response that does not describe its own paging; the
        :attr:`malformed_paging` flag records that case.
        """
        if self.method == "search.all":
