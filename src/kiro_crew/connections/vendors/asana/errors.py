...
shared control-plane error boundary W01 will define. The two are not merged:
this classification is the vendor-specific reading of Asana's HTTP+JSON shape;
a later adapter seam (W01) may map these categories onto a shared envelope, but
that mapping is not this module's concern and is not built here.
...
:func:`classify_cross_workspace` is driven by the caller's own determination
...
def classify_error(status: object, body: Optional[Mapping[str, Any]] = None) -> AsanaError:
    ...
    the dedicated :func:`classify_cross_workspace` instead of guessed from a
    status here.
...
def classify_cross_workspace(
    *,
    requested_gid: str,
    token_workspace: str,
    observed_status: object = None,
) -> CrossWorkspaceDenied:
    """Build the explicit cross-workspace denial from the caller's own facts.
    ...
    The very existence of this dedicated constructor is the enforcement of "no
    silent fallback": the only path to continue past a cross-workspace GID is to
    raise this, never to re-issue the call against a different workspace.
    """
    ...
    return CrossWorkspaceDenied(requested_gid=gid, token_workspace=ws, status=status)
