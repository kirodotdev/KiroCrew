...
class NextPage:
    ...
    ``offset`` is the opaque token to pass on the next request -- the only field
    a caller needs to continue a listing. Asana's ``next_page`` envelope also
    carries ``path``/``uri`` continuation references, but this connector drives
    pagination purely by the ``offset`` cursor and does not model them.
