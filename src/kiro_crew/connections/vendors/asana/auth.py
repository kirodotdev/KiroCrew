...
* ``MCP`` -- the Streamable HTTP MCP server at
  ...
  - does NOT support dynamic client registration (DCR): the app must be
    pre-registered. :data:`MCP_SUPPORTS_DCR` is ``False`` and
    :func:`require_preregistered` enforces it.
...
#: MCP apps cannot use dynamic client registration; pre-registration is required.
MCP_SUPPORTS_DCR = False
...
class AsanaAuthError(ValueError):
    ...
    scopes, a request against the retired SSE endpoint, an MCP request when DCR
    was assumed). Distinct from :class:`AuthSurfaceMismatch`, which is the
    specific cross-surface token misuse.
    """
...
def normalize_rest_scopes(scopes: Optional[Sequence[str]]) -> tuple[str, ...]:
    ...
    The REST surface uses fine-grained ``resource:action`` scopes and REQUIRES
    at least one -- a REST authorization with no scope grants nothing, so an
    empty/None scope list is refused. The ``"default"`` MCP pseudo-scope is
    ...
    if MCP_ONLY_SCOPE in scopes:
        raise AsanaAuthError(
            f"{MCP_ONLY_SCOPE!r} is the MCP surface's pseudo-scope and is not a REST scope; "
            "REST uses fine-grained resource:action scopes"
        )
    cleaned = tuple(s for s in scopes if isinstance(s, str) and s.strip())
    if not cleaned:
...
def require_preregistered() -> None:
    """Assert the MCP surface's no-DCR rule, raising if DCR were assumed.

    Asana's V2 MCP server does not support dynamic client registration: a client
    MUST be pre-registered to obtain a client id/secret. This function exists so
    a wiring layer that reaches for DCR is stopped by a named rule rather than by
    a runtime "Invalid client" surprise. It is unconditional because
    :data:`MCP_SUPPORTS_DCR` is a fixed vendor fact, not a runtime input.
    """

    if not MCP_SUPPORTS_DCR:
        return
    raise AsanaAuthError(  # pragma: no cover - MCP_SUPPORTS_DCR is a fixed False
        "unreachable: MCP DCR support is a fixed vendor fact (False)"
    )
...
class SurfaceToken:
    ...
    def __post_init__(self) -> None:
        if self.surface is AuthSurface.MCP:
            ...
            # A REST token is not workspace-pinned; recording a workspace on it
            # would misrepresent the surface's own scoping model.
            raise AsanaAuthError(
