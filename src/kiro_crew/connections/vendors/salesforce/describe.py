...
class FieldDescribe:
    ...
    #: For a ``reference`` field, the parent-relationship name a SOQL result
    #: nests the related record under (``OwnerId`` -> ``Owner``, ``Parent__c`` ->
    #: ``Parent__r``). ``None`` for a non-reference field or when the describe
    #: omitted it. This is what lets the payload parser tell a nullable parent
    #: relationship (legitimately ``null``) apart from a field-name typo.
...
class ObjectDescribe:
    ...
    #: Parent-to-child relationship names from the describe's ``childRelationships``
    #: array (``Account`` -> ``Contacts``, ``Cases``, ...). A SOQL subquery
    #: (``SELECT Id, (SELECT Id FROM Contacts) FROM Account``) nests a child-query
    #: envelope under one of these names.
    ...
    def relationship_names(self) -> frozenset[str]:
        """The relationship keys a SOQL result may legitimately carry.

        The union of every reference field's parent ``relationship_name`` and
        the object's parent-to-child ``child_relationships``. A key in this set
        is a relationship the describe declares, so a ``null`` (nullable parent)
        or a nested record/child-query envelope under it is expected, not a
        field-name typo. A key NOT in this set (and not a scalar field) is still
        unmodelable and rejected by the payload parser.
        """

        parents = {fd.relationship_name for fd in self.fields.values() if fd.relationship_name}
        return frozenset(parents | set(self.child_relationships))
...
def parse_field_describe(raw: Mapping[str, Any]) -> FieldDescribe:
    ...
    return FieldDescribe(
        ...
        fls=fls,
        relationship_name=rel if isinstance(rel, str) and rel else None,
    )
...
def parse_object_describe(raw: Mapping[str, Any]) -> ObjectDescribe:
    ...
    for child in raw.get("childRelationships", []) or []:
        if not isinstance(child, Mapping):
            raise ValueError("each 'childRelationships' entry must be an object")
        rel = child.get("relationshipName")
        # A childRelationships entry with no relationshipName is not queryable as
        # a subquery (Salesforce omits the name for such relationships), so it is
        # simply not a relationship key a SOQL result can carry -- skip it rather
        # than invent a name.
