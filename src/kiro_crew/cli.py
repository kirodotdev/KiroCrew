...
""",
    ...
    pod_api = pod_sub.add_parser(
        ...
        description=(
            ...
            "exits 1. READ-ONLY by default: GET and HEAD go through, anything that "
            "can change the pod's state needs --allow-write. The token is minted "
            "internally through the same ownership-proof path `pod token` uses, so "
            "no secret is ever read or passed by the caller."
        ),
        epilog=(
            "Examples:\n"
    ...
    pod_api.add_argument(
        ...
        metavar="METHOD",
        help=(
            "HTTP method: GET or HEAD (read-only, the default surface), or POST, "
            "PUT, PATCH or DELETE with --allow-write (case-insensitive)"
        ),
    )
    pod_api.add_argument(
        "path",
    ...
    pod_api.add_argument(
        ...
        action="store_true",
        help=(
            "Permit a state-changing method (POST/PUT/PATCH/DELETE). Off by default "
            "because a pod's API can start agent turns, rewrite config and delete "
            "state, and the path is often taken from content the caller does not "
            "control."
        ),
    )
