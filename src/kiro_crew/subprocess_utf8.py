...
The repo-wide lint gate that enforces it (``scripts/check_subprocess_encoding.py``
plus its committed baseline) landed on ``main`` after this release branch was
cut and is deliberately NOT part of it, so on this branch the mapping is a
convention the call sites opt into rather than something CI can require.
...
one mojibake for another. Those sites keep locale decoding on purpose.
