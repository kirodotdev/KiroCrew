...
def record_activity(
    ...
    * ``"select_crew"`` — the orchestrator judged this member fits the task.
    ...
    try:
        ...
        svc = get_service()
        # ensure FIRST, so the probe below sees the member's whole past. The log
        # may not exist yet, and creating it folds the legacy activity files in:
        # probing before that has an absent ledger answer "nothing recorded", the
        # migration then bring the very record in, and this append duplicate it.
        # ensure raises when it cannot finish, and the caller's except returns
        # False, so a failed migration refuses the append rather than writing
        # against a half-folded log.
        # ensure FIRST, so the probe below sees the member's whole past. The log
        # may not exist yet, and creating it folds the legacy activity files in:
        # probing before that has an absent ledger answer "nothing recorded", the
        # migration then brings the very record in, and this append duplicates it.
        # ensure raises when it cannot finish, and the caller's except returns
        # False, so a failed migration refuses the append rather than writing
        # against a half-folded log.
