...
| `pi` | `mirror` | `pi.py`, wire face only — Crew writes no pi file, so the `session/new` array is its whole channel. No transport is filtered (the adapter accepts stdio, http and sse and skips unknown shapes); names are sanitized the adapter's way and the control plane carries the session key; `disabledTools` is honoured by withholding a third-party server it narrows, and by refusing the call at the approval request (exact `mcp__<server>__<tool>` title match) for Crew's own control plane |
