...
class AcpProvider(LLMProvider):
    ...
    async def _set_effort_config_option(self, level: str) -> None:
        ...
        Every member of ``ACP_BACKENDS_CONFIG_OPTION_TUNING`` arrives here, so
        the name is the channel rather than a harness.
    ...
    async def change_effort(self, level: str) -> bool:
        ...
        ``ACP_BACKENDS_CONFIG_OPTION_TUNING``. Returns False when the current
        ...
        if not model_supports_effort(model):
            ...
            return False
        via_config_option = self._client.backend in ACP_BACKENDS_CONFIG_OPTION_TUNING
