                ...
                async for raw in _iter_sse_data(resp.content):
                    ...
                    if isinstance(art, dict):
                        ...
                        if isinstance(artifact, dict):
                            ...
                            # A2A artifact semantics: ``append: true`` adds to the
                            # artifact of that id; false or absent REPLACES its
                            # prior content. Appends stream live as chunks. A
                            # replacement cannot be streamed (what was shown
                            # before it is now stale and cannot be unsent), so a
                            # replaced artifact is held and delivered once, in its
                            # final form, after the loop -- see the flush below.
                            replaces = not bool(art.get("append"))
                            if replaces:
                                artifacts[aid] = delta
                            ...
                            # artifact text must reach the stream as chunks -- an
                            # artifact reported only on EVENT_COMPLETE
                            # (billing-only) surfaces as "_No response._".
                            if delta and not replaces and aid not in deferred:
