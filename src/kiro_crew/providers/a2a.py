...
#: Distinct artifactIds one turn may name. The text cap alone does not bound the
#: per-artifact table: a stream of empty-text artifacts under fresh ids adds a key
#: per frame while ``"".join(values)`` stays at zero, so the ids are capped by
#: count here and by length in the footprint ``_over_cap`` measures.
_MAX_TURN_ARTIFACTS = 4096
_CAP_ERROR = (
    f"the remote agent's output exceeded {_MAX_TURN_TEXT_CHARS} characters in one turn; "
    "the turn was abandoned"
)
_ARTIFACT_CAP_ERROR = (
    f"the remote agent named more than {_MAX_TURN_ARTIFACTS} distinct artifacts in one turn; "
    "the turn was abandoned"
)
...
#: Comma-separated ``http://`` loopback origins the gateway's ENVIRONMENT vouches
#: for (``http://127.0.0.1:8790,http://[::1]:8790``). Set by whoever starts the
#: gateway -- the test harness, or a developer running a local shim -- never by
#: config: ``config.json`` is agent-writable, the process environment is not, so a
#: config entry alone can never make the gateway talk plaintext to a port on the
#: host. Entries that are not loopback, or not ``http://``, are ignored.
A2A_PLAINTEXT_ORIGINS_ENV = "KIROCREW_A2A_PLAINTEXT_ORIGINS"
...
def _plaintext_origins() -> frozenset[str]:
    """The loopback ``http://`` origins named in :data:`A2A_PLAINTEXT_ORIGINS_ENV`,
    canonicalized like :func:`_origin` so the comparison is exact (host, port)."""
    out: set[str] = set()
    for item in os.environ.get(A2A_PLAINTEXT_ORIGINS_ENV, "").split(","):
        o = _origin(item.strip())
        if o.startswith("http://") and _is_loopback_host(urlsplit(o).hostname):
            out.add(o)
    return frozenset(out)


def _credentials_may_travel(url: str) -> bool:
    """True when this client may send anything to *url*: ``https``, or ``http`` to a
    loopback origin the environment names in :data:`A2A_PLAINTEXT_ORIGINS_ENV`.

    Plain HTTP is otherwise refused even on loopback: a remote registered at
    ``http://127.0.0.1:<port>`` would reach whatever service listens on that port
    of the host, so the operator has to name the exact origin. Anything
    ...
    if scheme != "http":
        return False
    return _origin(url) in _plaintext_origins()
...
def _task_mismatch(locked: str, received: Any) -> str:
    """Error text for a frame naming a task other than the one this turn started.

    One ``message/stream`` call is one task: the first frame that names a task
    fixes the turn's task id, and every later frame (task, status, artifact) must
    name the same one. A frame from another task would otherwise have its text
    recorded as this run's output, or become the target a stop cancels.
    """
    return (
        f"a2a taskId mismatch: this turn is task {locked!r} but the remote sent a frame "
        f"for {str(received)!r}; refusing to record another task's output"
    )
...
class A2AProvider(LLMProvider):
    ...
    def started(self) -> bool:
        """True once :meth:`start` fetched the Agent Card and bound the endpoint."""
    ...
    async def start(self) -> None:
        ...
        # card URL is refused before any request unless the gateway's
        # environment names that loopback origin (the test fixture's case).
        if not _credentials_may_travel(self._agent_card_url):
            raise A2AStreamError(
                ...
                "credentials are only sent over TLS (a plain-http loopback origin must "
                f"be named in {A2A_PLAINTEXT_ORIGINS_ENV})"
            )
    ...
    def _headers(self, base: dict[str, str]) -> dict[str, str]:
        ...
        With credentials configured, a callback that yields no headers (the env
        var was unset between construction and this request, say) is a refusal:
        the request is never sent unauthenticated "to see".
        """
        ...
        if not extra:
            raise A2AStreamError(
                f"agent {self._name!r}: configured credential resolved to nothing at request "
                "time; refusing to send the request unauthenticated"
            )
        return {**base, **extra}
    ...
    async def stream(self, message: str) -> AsyncIterator[LLMEvent]:
        ...
        Emits ``EVENT_TEXT_CHUNK`` per WORKING-status text delta and per appended
        artifact delta, delivers each replaced artifact once after the stream,
        and ends with a single ``EVENT_COMPLETE`` carrying the assembled final
        artifact text (the record). The turn ends on either terminal shape the
        spec allows: a terminal ``statusUpdate``, or a ``task`` snapshot whose
        status is terminal (the reference SDK's ``completed_task()`` shape), whose
        inline ``artifacts`` are then the result. A FAILED / REJECTED / CANCELED
        terminal, a JSON-RPC error frame, ``AUTH_REQUIRED``, a dropped connection
        or a stream with no terminal state RAISES :class:`A2AStreamError` instead
        of completing -- see *Failure discipline* in the module doc.
        """
        ...
        artifact_text = ""
        # Per-artifact content keyed by artifactId (insertion order = first
        # appearance), and the ids whose content was REPLACED and so is delivered
        # once after the loop rather than streamed.
        ...
        saw_terminal = False
        # One stream call is one task. The first frame naming a task locks the
        # turn (a continuation is a new provider, so it locks afresh); a frame
        # for any other task fails the turn rather than being folded in.
        turn_task_id: str | None = None

        def _lock_task(tid: Any) -> None:
            nonlocal turn_task_id
            if not tid:
                return
            tid = str(tid)
            if turn_task_id is None:
                turn_task_id = tid
                self._current_task_id = tid
            elif tid != turn_task_id:
                raise A2AStreamError(_task_mismatch(turn_task_id, tid))

        def _adopt_context(ctx: Any) -> None:
        ...
        def _over_cap() -> bool:
            """Abandon the turn when what this stream holds in memory passes a cap.

            Two bounds, because text alone is not the footprint: the per-artifact
            table is keyed by server-chosen ids, so its size is the ids' count
            and length as much as their content. The count is checked first so
            the length sum below runs over a bounded table.
            """
            nonlocal stop_reason, failed, saw_terminal, error_text
            reason = ""
            if len(artifacts) > _MAX_TURN_ARTIFACTS:
                reason = _ARTIFACT_CAP_ERROR
            elif (
                len(artifact_text) + len(final_text) + sum(len(aid) for aid in artifacts)
                > _MAX_TURN_TEXT_CHARS
            ):
                reason = _CAP_ERROR
            if reason:
        ...
        def _apply_state(state: str, delta: str) -> bool:
            """Record a task state from a status or a Task snapshot.

            Returns True when the state ends the turn. One rule for both frame
            kinds: the spec lets a server finish with a terminal
            ``TaskStatusUpdateEvent`` OR with a ``Task`` whose status is terminal,
            and the outcome must not depend on which shape it chose.
            """
            nonlocal stop_reason, failed, saw_terminal, error_text
            ...
            if state in _TERMINAL_STATES:
                ...
                if state in _FAILURE_STATES:
                    ...
                    failed = True
                    # On failure the status message text (if any) IS the
                    # error surface.
                    if delta and not error_text:
                        error_text = delta
                elif state in _CANCEL_STATES:
                    # Cancelled work is not completed work: the runner calls
                    # record_success on EVENT_COMPLETE, so this must surface
                    # as a failed turn with the cause named.
        ...
        def _status_text(status: Any) -> str:
            ...
            return _parts_text(msg.get("parts")) if isinstance(msg, dict) else ""
            ...
            async with self._session.post(
                ...
                async for raw in _iter_sse_data(resp.content):
                    ...
                    if isinstance(task, dict):
                        _lock_task(task.get("id"))
                        _adopt_context(task.get("contextId"))
                        status = task.get("status") or {}
                        state = str(status.get("state") or "") if isinstance(status, dict) else ""
                        if state not in _TERMINAL_STATES and state != _TASK_STATE_AUTH_REQUIRED:
                            # A non-terminal snapshot (the first frame, typically
                            # SUBMITTED / WORKING) only tells us the ids; content
                            # arrives as artifact and status updates.
                            continue
                        # A Task in a terminal state is a legitimate FINAL event,
                        # not a dropped stream: the reference SDK's
                        # ``completed_task()`` ends a turn with exactly this frame,
                        # its result carried inline as ``task.artifacts``. A
                        # snapshot is authoritative -- each artifact it names is
                        # taken in the form given (replacing anything assembled
                        # from earlier deltas for that id) and delivered once,
                        # after the loop, unless it is already held verbatim.
                        for item in task.get("artifacts") or []:
                            if not isinstance(item, dict):
                                continue
                            aid = str(item.get("artifactId") or "")
                            text = _parts_text(item.get("parts"))
                            if artifacts.get(aid) == text:
                                continue
                            artifacts[aid] = text
                            if aid not in deferred:
                                deferred.append(aid)
                        artifact_text = "".join(artifacts.values())
                        delta = _status_text(status)
                        if delta:
                            final_text += delta
                        if _over_cap():
                            break
                        ...
                        _apply_state(state, delta)
                        break

                    art = result.get("artifactUpdate")
                    if isinstance(art, dict):
                        _lock_task(art.get("taskId"))
                        artifact = art.get("artifact")
                        if isinstance(artifact, dict):
                            delta = _parts_text(artifact.get("parts"))
                            aid = str(artifact.get("artifactId") or "")
                            # A2A artifact semantics, as the reference SDK's
                            # TaskManager applies them: ``append: true`` extends
                            # the artifact of that id; false or absent CREATES it
                            # when the id is new and REPLACES its content when it
                            # already exists. Created and appended text streams
                            # live. A replacement cannot be streamed (what was
                            # shown before it is now stale and cannot be unsent),
                            # so a replaced artifact is held and delivered once,
                            # in its final form, after the loop -- see the flush.
                            replaces = not bool(art.get("append")) and aid in artifacts
                            if replaces:
                                artifacts[aid] = delta
                                if aid not in deferred:
                                    deferred.append(aid)
                            elif not bool(art.get("append")):
                                artifacts[aid] = delta
                            elif delta:
                                artifacts[aid] = artifacts.get(aid, "") + delta
                            artifact_text = "".join(artifacts.values())
                            if _over_cap():
                                break
                            # Artifacts ARE the result in A2A ("results go in
                            # Artifacts, not Messages"), and run.py assembles
                            # the transcript from EVENT_TEXT_CHUNK, so created and
                            # appended artifact text streams as chunks; only an
                            # artifact in replaced state waits for the flush.
                            # Reference-SDK servers stream progress as status
                            # text and then create ONE artifact repeating it all;
                            # that exact recap has been shown and is not repeated.
                            if (
                                delta
                                and aid not in deferred
                                and not (final_text and delta.strip() == final_text.strip())
                            ):
                                yield LLMEvent(kind=EVENT_TEXT_CHUNK, text=delta)
                        continue
                    ...
                    if isinstance(su, dict):
                        _lock_task(su.get("taskId"))
                        _adopt_context(su.get("contextId"))
        ...
        # Replaced artifacts, in their final form, once each. Servers built on
        # the reference SDK stream progress as status-message deltas and then
        # send ONE artifact repeating the whole text; that recap has already been
        # shown, so an EXACT full recap is skipped -- a distinct artifact whose
        # text merely occurs inside earlier status text is output. Done before
        # any terminal outcome below so partial output reaches the transcript on
        # a failed or dropped turn too.
        for aid in deferred:
            text = artifacts.get(aid, "")
            if text and not (final_text and text.strip() == final_text.strip()):
        ...
        if failed:
            ...
            # the transcript because appended deltas were yielded live and
            # replaced artifacts were flushed above.
            raise A2AStreamError(
                "a2a task "
