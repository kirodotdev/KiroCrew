...
#: The status returned on the default (no ``return_body``) path. A FIXED value,
#: never the upstream ``resp.status_code`` — the status code is origin-controlled,
#: so echoing it would leave a covert channel (an origin could encode the injected
#: credential a few bits per call across the status). 0 is not a real HTTP status,
#: so a consumer cannot mistake it for one.
_WITHHELD_STATUS = 0
...
def _secret_reflections(secret_value: str) -> list[str]:
    ...
    base32, hex, percent-encoding) rather than only the raw value.
    ...
    if raw:
        ...
        derived.add(raw.hex())
        derived.add(raw.hex().upper())
        # Base32 (RFC 4648): an echo endpoint may return the credential base32-
        # encoded; cover upper/lower and the padding-stripped forms so a
        # reversible base32 reflection does not slip past the raw/base64/hex set.
        b32 = base64.b32encode(raw).decode("ascii")
        derived.add(b32)
        derived.add(b32.rstrip("="))
        derived.add(b32.lower())
        derived.add(b32.lower().rstrip("="))
...
def _withheld_response(final_origin: str) -> SanitizedResponse:
    """The fixed default result, returned for EVERY outcome once the secret has
    been sent — success, a redirect refusal, a transport error, a timeout — so
    nothing an owner-authorized origin controls (a body, a header, a status, or
    even error-vs-success) can encode a bit of the credential back to the agent.
    Used whenever the owner has not set ``return_body`` for the origin."""
    return SanitizedResponse(
        status=_WITHHELD_STATUS,
        headers={},
        body=(
            "[mediated request completed. The owner has not authorized "
            "returning this origin's response (body, headers, or status); set "
            "'return_body': true in the signed policy for this origin to "
            "receive the real response.]"
        ),
        truncated=False,
...
def _sanitize_response(
    resp: requests.Response, final_origin: str, secret_value: str, *, return_body: bool = False
) -> SanitizedResponse:
    upstream_headers = {
        k: v for k, v in resp.headers.items() if k.lower() in _SAFE_RESPONSE_HEADERS
    }
    ...
    # F2 defense — by default return NOTHING derived from the upstream response.
    #
    # An owner-authorized origin can echo the injected credential back through an
    # UNENUMERABLE transform (a hash, a per-field split, a bespoke encoding) that
    # no substring scrub can catch. That is true of the body, the response
    # HEADERS, AND the response METADATA: the status code, the content-type, and
    # even the body byte length are origin-controlled, so an adversarial agent and
    # a cooperating authorized origin could encode the credential over successive
    # calls into any of them. So by default the mediated result is a CONSTANT that
    # does not vary with ANY upstream response data — no status, no content-type,
    # no length, no body, no headers — leaving zero origin-controlled channel for a
    # reflection to ride out on. The agent learns only that the call completed.
    #
    # An owner who genuinely needs the real body + headers + status sets
    # ``return_body: true`` in the SIGNED policy for that exact origin: a
    # deliberate, owner-authenticated decision to accept the residual reflection
    # risk for a destination they trust. The agent cannot set it (the policy
    # signature is keyed by the sandbox-masked owner key), so the risky path is
    # opt-in and owner-bound. On that path the plaintext + enumerable encodings
    # are still scrubbed from both body and headers as defense-in-depth.
    if not return_body:
        # A fixed constant — independent of resp.status_code, ctype, length, body,
        # or headers. No covert channel survives because nothing here is a function
        # of the upstream response.
        return _withheld_response(final_origin)
    headers = upstream_headers

    # On the opt-in path the origin's bytes are returned, so scrub any reflected
    # credential (plaintext + enumerable encodings) from both body and header
    ...
    try:
        try:
            return _dispatch_with_redirects(
                ...
                target=target,
                timeout=timeout,
                return_body=auth.return_body,
            )
        except (MediationError, PolicyError, SsrfError):
            # Post-send outcome collapse (default path only). Every failure raised
            # from dispatch happens AFTER the credential was injected into a
            # request — a same-origin redirect refusal, a transport error, a
            # timeout, an encode error. All pre-send refusals (method, body size,
            # origin mismatch, initial SSRF/confusable-URL) were already raised
            # ABOVE this call, before the secret was read. So on the default
            # (no ``return_body``) path, converting these to the SAME fixed
            # constant a success returns removes the error-vs-success oracle an
            # owner-authorized origin could otherwise steer on a secret bit. When
            # the owner opted into the real response, surface the real error.
            if auth.return_body:
                raise
            return _withheld_response(auth.origin)
    finally:
...
def _dispatch_with_redirects(
    *,
    ...
    # ONE end-to-end deadline across all redirect hops. Without this the per-hop
    # timeout applied below would let the WHOLE dispatch run up to
    # timeout × (_MAX_REDIRECTS + 1) — which can exceed the caller's loopback
    # timeout, so a slow redirect chain would be cut off at the wrong layer.
    # Each hop gets only the remaining budget; an exhausted budget fails closed.
            ...
            try:
                if resp.status_code in (301, 302, 303, 307, 308) and "location" in resp.headers:
                    ...
                    if code in (301, 302, 303) and current_method not in _SAFE:
                        ...
                        current_json = None
                    continue
                # Enforce the response size cap while streaming, under the same
                # overall wall-clock deadline as the rest of the dispatch.
                _read_capped(resp, deadline)
                return _sanitize_response(
                    resp, authorized_origin, secret_value, return_body=return_body
                )
            finally:
                resp.close()
...
def _read_capped(resp: requests.Response, deadline: float) -> None:
    """Force-read the body up to the cap so ``resp.content`` is bounded, under the
    SAME overall wall-clock ``deadline`` as the rest of the dispatch.

    ``requests``' ``timeout`` bounds only the gap BETWEEN socket reads, and it is
    fixed at request time — so a complicit origin can drip bytes, each read inside
    that per-read timeout, and stretch the total transfer past the overall
    deadline (and past the caller's outer loopback timeout, which would surface an
    error instead of the fixed withheld constant — an outcome oracle). Two guards
    close that: (1) BEFORE each read the underlying socket's own timeout is shrunk
    to the time left on the deadline, so a single blocking read cannot outlast the
    budget; (2) BETWEEN chunks the monotonic deadline is re-checked. Either way
    the stream aborts with the overall-budget error rather than running long.
    """

    def _remaining() -> float:
        return deadline - time.monotonic()

    def _clamp_socket_timeout() -> None:
        # Shrink the live socket read timeout to the remaining budget so the NEXT
        # blocking read cannot exceed the deadline. Best-effort across urllib3
        # internals; the between-chunk check below is the backstop if it is absent.
        left = _remaining()
        if left <= 0:
            raise MediationError("The mediated request exceeded its overall time budget.")
        try:
