...
class SecretAuthorization:
    ...
    #: Default False -- the mediated result then carries only status + safe headers
    #: + a structural body summary, so an owner-authorized origin that reflects the
    #: injected credential through an unenumerable transform cannot hand it back.
...
def load_authorization(config_dir: str | Path, secret_name: str) -> SecretAuthorization:
    ...
    # authorizations map. The signing key (memory_stores/.member-api-key) is
    # masked from the agent sandbox, so a policy an agent planted — or one left
    # writable before an upgrade masked this file — cannot present a valid
