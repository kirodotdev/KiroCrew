...
def _taskrun_dispatch(args: argparse.Namespace) -> None:
    """Dispatch `kirocrew taskrun` subcommands."""
    import asyncio as _asyncio
    import json as _json
    from pathlib import Path as _Path

    from kiro_crew.migration import protocol as _mig
    from kiro_crew.migration.move_plan import plan_unit_move, render_plan, safe_requirement_identity
    ...
    try:
        # `--runs-file` is an arbitrary caller-supplied path, so it goes through
        # the sensitive-path gate rather than a bare read: `--runs-file
        # ~/.aws/credentials` would otherwise be read here, and a JSON decode
        # error quotes the offending text, putting the file's contents on
        # stderr. safe_read_file resolves symlinks and re-checks the resolved
        # target, so a link into a credential directory is refused through it.
        records = _json.loads(safe_read_file(str(runs_path)))
    ...
    # `null`, a number and a bare string are all valid JSON, and iterating any of
    # them raises TypeError past the decode guard above. The registry is a list of
    # run records or it is unusable, so say which file is wrong rather than
    # surfacing a traceback.
    if not isinstance(records, list):
        print(
            f"Error: runs registry {runs_path} is not a list of run records "
    ...
    # transfer needs a task boundary, so a plan for a mid-execution run would
    # describe something that cannot happen.
    try:
        _asyncio.run(adapter.refuse_if_mid_run(task_id))
    except _mig.MidRunError as exc:
        print(f"Error: {exc} — cannot migrate a run with a task mid-execution", file=sys.stderr)
        sys.exit(1)
    except MalformedRunRecord as exc:
        print(f"Error: run {task_id} in {runs_path}: {exc}", file=sys.stderr)
        sys.exit(1)
    ...
    for f in run_fidelity_findings(record):
        # Same reason the renderer sanitizes a requirement identity: a finding's
        # key and sentence are not guaranteed product-authored, and this print is
        # an egress boundary.
        extra.append(
            f"not available from this source: {safe_requirement_identity(f.detail_key)}"
            f" — {safe_requirement_identity(f.detail)}"
        )
    print(render_plan(bundle, target_id, unit_id=task_id, extra=extra))


def _session_dispatch(args: argparse.Namespace) -> None:
    """Dispatch `kirocrew session` subcommands.
    ...
    print(
        ...
        f"can build the plan against the live slot. The transfer itself is not "
        f"implemented on any surface yet.",
        file=sys.stderr,
    )
    sys.exit(2)
...
def _cron_dispatch(args: argparse.Namespace) -> None:
    ...
    if action == "list":
        ...
        from kiro_crew.migration.move_plan import safe_requirement_identity
        ...
        if bundle.requirements:
            ...
            for r in bundle.requirements:
                print(f"    - {r.kind}: {safe_requirement_identity(r.identity)}")
        else:
