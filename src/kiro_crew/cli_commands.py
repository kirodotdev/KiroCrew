...
def _handle_secrets_authorize(args: argparse.Namespace) -> None:
    ...
    # a no-op flag on Windows where the lstat check below is the guard); the
    # regular-file + single-link lstat rejects a swapped alias.
...
def _authorize_txn(args: "argparse.Namespace", cfg_dir, path) -> None:
    ...
    # Audit the authorization change: granting or removing a credential egress is
    # a security-relevant control edit. Secret-free — the NAME, the action, and
    # (on grant) the authorized ORIGIN only; never a secret value. Best-effort so
    # an audit hiccup cannot break the owner's command.
    try:
        if args.remove:
    ...
    except Exception:  # noqa: BLE001 — the audit must not break the authorize command
        pass
    if args.remove:
