...
# Cancellation cannot return until the durable replay-settlement compensation
# lands. Retry pacing is bounded so a transient storage failure neither spins
# nor grows the delay without limit.
...
class SessionMap:
    ...
    def clear_sid(self, key: str) -> None:
        ...
        if not entry:
            return
        changed = _stash_and_clear_sid(entry)
        flags = entry.get("flags")
