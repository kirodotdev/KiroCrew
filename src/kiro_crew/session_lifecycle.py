...
class SessionLifecycleService:
    ...
    async def reset(
        ...
        *,
        ...
        ~46 callers are, and the cost of the two mistakes is not symmetric: a missed
        cancel leaves a bounded child running to its own timeout, while a wrong cancel
        destroys live work for a conversation that comes right back.
        """
        owner = self._owner
        logger = self._deps.logger
