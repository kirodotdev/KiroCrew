...
CRON_DROP_FIELDS: tuple[str, ...] = (
    ...
    "last_retry_count",
    "last_retry_run_ts",
    # The run-ordering counter for those same runs. It is allocated by the
    # SOURCE host's `CronService` and only orders merges of that host's runs;
    # the target allocates its own from the value ITS store holds, so a
    # foreign number seeds the target's ordering with a count it never made.
    "run_generation",
    ...
    "member_id",
    "memory_store",
    # The same memory identity again, as a durable record: it carries
    # ``member_id`` plus the store ref, the agent-namespace selection and the
    # memory mode. Its own invariant refuses a record whose member and store
    # disagree, so a fragment cannot be carried either -- and the target's
    # obligation is already named by the two requirements above, so dropping it
    # discloses nothing less.
    "execution_context",
    # Names a chat folder in the SOURCE sidebar. An id the target cannot resolve
    # is treated as "not filed" by design, so shipping it would at best dangle.
    # Same class as `folder_id`.
    "chat_folder_id",
)
