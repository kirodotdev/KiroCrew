...
class MalformedRunRecord(ValueError):
    """A run record's shape cannot be serialized.

    Raised instead of letting a scalar reach ``dataclasses.asdict``, so a CLI
    caller pointing ``--runs-file`` at a hand-edited or truncated registry gets a
    sentence naming the bad shape rather than a traceback.
    """
...
PROJECT_DROP_FIELDS: tuple[str, ...] = (
    # The run's memory identity as a durable record: the member, the store ref,
    # the agent-namespace selection and the memory mode. Every one of those is
    # resolved against the host, and the record's own invariant refuses a member
    # and store that disagree, so it can be neither shipped nor split.
    "execution_context",
...
def _tasks(run) -> list:
    ...
    The shape is validated HERE rather than at each call site. Every reader of a
    run's tasks comes through this function, so one check covers the plan-time
    mid-run refusal, the discarded-progress description and the serializer alike
    — and a per-call-site check is how the mid-run path was left crashing on a
    scalar after the other two were guarded.
    """
    if isinstance(run, dict):
        tasks = run.get("tasks") or run.get("task_details") or []
    else:
        tasks = getattr(run, "tasks", []) or []
    if not isinstance(tasks, (list, tuple)):
        raise MalformedRunRecord(f"task list is {type(tasks).__name__}; it must be a list of tasks")
    return list(tasks)
...
def serialize_project(run) -> dict:
    ...
    tasks = _tasks(run)
    if tasks:
        # A task entry is a mapping or a dataclass. Anything else -- a scalar in a
        # hand-edited or truncated runs.json -- reaches `dataclasses.asdict` and
        # raises TypeError, which surfaces to a CLI user as a traceback. Name the
        # offending shape instead, so the message says which record is wrong.
        bad = [t for t in tasks if not isinstance(t, dict) and not dataclasses.is_dataclass(t)]
        if bad:
            raise MalformedRunRecord(
                f"task list holds {type(bad[0]).__name__} entries; "
                "each task must be a mapping or a task record"
            )
        payload["tasks"] = [
...
def describe_discarded_progress(run) -> dict:
    ...
    commits = _field(run, "commit_hashes", []) or []
    # `len` on a number raises TypeError, which reaches a CLI user as a traceback
    # from a hand-edited or truncated registry. Same refusal as a malformed task
    # entry: name the shape rather than crash on it.
    if not isinstance(commits, (list, tuple)):
        raise MalformedRunRecord(
            f"commit_hashes is {type(commits).__name__}; it must be a list of commit hashes"
        )
    return {
