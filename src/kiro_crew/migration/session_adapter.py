...
class SessionMigrationAdapter:
    ...
    async def requirements(self, unit_id: str) -> list[P.HostRequirement]:
        ...
        * ``project_checkout`` — advisory. Rematerialization is explicitly out of
        ...
        There is deliberately no ``mcp_server`` requirement. A session's tools
        would be worth naming, but nothing can say WHICH servers a given session
        loaded: ``mcp_discovery.list_servers`` takes no arguments and returns the
        host's whole configured inventory, and its own contract disclaims knowing
        what kiro-cli loads at session time. Emitting that inventory would attach
        a blocking requirement for every server the host happens to configure,
        including the ones this session never touched, so a plan would refuse on
        grounds it cannot support. A requirement whose only possible input is
        fabricated is worse than its absence.
        """
