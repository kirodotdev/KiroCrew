...
def _register_mcp_servers(
    ...
    if withheld:
        ...
        if len(failures) > before:
            ...
            from kiro_crew.agent import _neutralize_app_agent_file, _remaining_app_agent_names

            unneutralized = [
                fn
                for fn in _remaining_app_agent_names(app_name)
                if not _neutralize_app_agent_file(fn)
            ]
            if unneutralized:
                raise RuntimeError(
