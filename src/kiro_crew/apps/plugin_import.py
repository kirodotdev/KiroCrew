...
def find_plugin_manifest(root: Path) -> tuple[Path, str]:
    ...
    for candidate_dir in sorted(root.glob(VENDOR_MANIFEST_DIR_GLOB)):
        if not candidate_dir.is_dir() or is_link_or_junction(candidate_dir):
            continue
        candidate = candidate_dir / MANIFEST_FILENAME
        if candidate.is_file() and not is_link_or_junction(candidate):
...
def resolve_declared_path(root: Path, raw: object) -> Path:
    ...
    probe = root_resolved
    for segment in body.replace("\\", "/").split("/"):
...
def _copy_tree_without_links(src: Path, dst: Path, depth: int = 0) -> tuple[int, list[str]]:
    ...
    for entry in sorted(src.iterdir()):
        ...
        if is_link_or_junction(entry):
            ...
            continue
        if entry.is_dir():
            sub_copied, sub_skipped = _copy_tree_without_links(entry, target, depth + 1)
            copied += sub_copied
...
def _convert_skills(root: Path, declared: object, out_dir: Path, report: ImportReport) -> list[str]:
    ...
    if not roots:
        ...
        if default_root.is_dir() and not is_link_or_junction(default_root):
            roots = [default_root]
    if not roots:
        return []
