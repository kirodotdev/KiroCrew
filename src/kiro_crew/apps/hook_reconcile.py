...
#: + mtime, which every install mints afresh, so a failed attempt against one
#: install does not hold down the install that replaced it, even when the two
#: happen between ticks. Dropped when a process is observed tracked, or when the
#: app is POSITIVELY confirmed removed or disabled -- never on an unknown read.
