...
#: app name -> (monotonic time of the last failed respawn attempt or None if not
#: yet attempted, the install identity that attempt ran against). An entry is
#: recorded the moment this reconciler stops a child it spawned for a rotated
#: secret -- BEFORE the replacement is attempted -- and is what authorises a
#: later tick to spawn; it is what keeps an enabled app from staying down when
#: the replacement cannot be attempted right away (a transient unreadable
#: enablement, a shutdown that began in between). The reconciler never starts a
#: backend it did not itself stop: first starts belong to boot and the dashboard,
#: which vet policy before they spawn. The identity is the ``.app_secret`` inode
#: + mtime. ``install_app`` -- the only path the CLI has to new code, since it
#: refuses an installed name -- mints that file afresh, so a failed attempt
#: against one install does not hold down the install that replaced it, even
#: when the two happen between ticks. (The dashboard's in-place ``update_app``
#: preserves the secret, and that path stops and restarts the backend itself.)
#: Dropped when a process is observed tracked, or when the app is POSITIVELY
#: confirmed removed or disabled -- never on an unknown read.
_pending_respawn: dict[str, tuple[float | None, tuple[int, int] | None]] = {}
...
def _respawn_with_backoff(name: str) -> bool:
    ...
    against an owed replacement (``_pending_respawn``), and only for an app whose
    enablement ``app_enabled_state`` POSITIVELY confirms: the caller read
    ``enabled`` under the lifecycle lock, but a CLI ``disable`` in another
    process can land between that read and the spawn, so enablement is read
    again immediately before spawning and once more after. The re-read is
    tri-state and only a positive ``False`` is terminal: ``None`` (unreadable
    metadata, a write in flight) DEFERS -- the debt stays recorded and the next
    tick asks again -- because acting on it would leave an enabled app whose
    process was just stopped down for good. A child spawned for an app that is
    by then positively disabled is stopped right here rather than serving until
    the next tick. Governance and admission were decided when the reinstalled
    files landed (``install_app`` runs the admission gate in the installing
    process); execution trust for the exact path spawned is vetted inside
    ``start_app_backend``.
    ...
    if marked is None:
        return False  # nothing owed: the reconciler starts nothing it did not stop
    identity = _install_identity(name)
    now = time.monotonic()
    attempted_at, attempted_identity = marked
    if (
        attempted_at is not None
        and attempted_identity == identity
        and now - attempted_at < START_RETRY_BACKOFF_SECS
    ):
        return False
    enabled = app_enabled_state(name)
    if enabled is None:
        return False  # cannot confirm: keep the debt, the next tick re-asks
    if enabled is False:
        _pending_respawn.pop(name, None)  # positively disabled: the debt is void
        return False
    logger.info("app runtime reconcile: starting replacement backend for %s", name)
    try:
        spawned = start_app_backend(name)
    except Exception:
        # A spawn that raises (a governance evaluation error propagates by
        # design) is an attempt like one that returns None: without the record
        # it would be re-attempted and logged on every tick, which is what the
        # backoff exists to prevent.
        _pending_respawn[name] = (now, identity)
        logger.exception("app runtime reconcile: replacement backend for %s raised", name)
        return False
    if spawned is None:
        _pending_respawn[name] = (now, identity)
        return False
...
async def _replace_stale_backend(name: str) -> None:
    ...
    # The proxy refuses the app while this block runs: the stop below pops the
    # tracking record before the child has exited, and an app with a fixed
    # port would otherwise be resolved by name straight back to that child.
    with backend_replacement(name):
        ...
        if get_app_process(name) is not None:
            return
        # The replacement is owed from THIS moment, whatever the attempt below
        # meets: a transient unreadable enablement or a shutdown beginning in the
        # gap must not turn a stopped, enabled app into one that stays down.
        _pending_respawn.setdefault(name, (None, None))
    ...
    async with app_lifecycle_lock(name):
        ...
        if sig == loaded:
            ...
            if loaded_hook_manifest(name) is not None:
                ...
                # a same-mtime rewrite of the secret (a positive generation
                # mismatch the hooks did not notice) and finishes a pending
                # respawn. A record without a digest, or a backend that is not
