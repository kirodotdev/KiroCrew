...
# customizable crons). See the recipes feature design spec for the full design.
# Recipes are *declared* in the manifest but not auto-installed at app install
# time; the built-in ``recipes`` app owns their install/update/cleanup
# lifecycle via ``kirocrew recipes ...``.
# ---------------------------------------------------------------------------
...
class RecipeDependencies:
    """Optional per-recipe dependency declaration.

    Currently supports only ``capabilityPackages`` — a list of capability
    package names that the installer resolves when the recipe is selected.
    Per-recipe (vs app-level) deps allow lazy install: an app with multiple
    recipes only pulls in deps for the recipes the user opts into.
    """

    capabilityPackages: list[str] = field(default_factory=list)  # noqa: N815

    def to_dict(self) -> dict[str, Any]:
        d: dict[str, Any] = {}
        if self.capabilityPackages:
            d["capabilityPackages"] = self.capabilityPackages
        return d

    @classmethod
    def from_dict(cls, data: dict[str, Any]) -> RecipeDependencies:
        return cls(
            capabilityPackages=[  # noqa: N815
                str(p) for p in data.get("capabilityPackages", []) if p
            ],
        )
...
class SlackRecipe:
    ...
    ``description``, ``channelNamePart``, ``purpose``, ``dependencies``.
    ...
    dependencies: RecipeDependencies = field(default_factory=RecipeDependencies)
    ...
    def from_dict(cls, data: dict[str, Any]) -> SlackRecipe:
        ...
        deps = (
            RecipeDependencies.from_dict(deps_raw)
            if isinstance(deps_raw, dict)
            else RecipeDependencies()
        )
        return cls(
...
class CronRecipe:
    ...
    Exactly one of ``schedule`` or ``every_secs`` must be specified.
    ...
    ``promptFile`` / ``promptText``, ``every_secs``, ``persistent_session``,
    ``silent``, ``dependencies``.  Prompted at install: ``agent``, ``schedule``
    (when set, not for ``every_secs``-based recipes), output channel name
    (when ``channelNamePart`` is set).
    """

    name: str = ""  # kebab-case, unique within app
    description: str = ""
    schedule: str = ""  # 5-field cron expression; XOR with every_secs
    every_secs: int = 0  # interval in seconds; XOR with schedule
    agent: str = ""  # default agent (prompt overridable)
    ...
    persistent_session: bool = True  # carry context between runs (matches CronEntry)
    ...
    def from_dict(cls, data: dict[str, Any]) -> CronRecipe:
        ...
        return cls(
            ...
            every_secs=int(data.get("every_secs", 0)),
            agent=str(data.get("agent", "")),
            channelNamePart=str(data.get("channelNamePart", "")),  # noqa: N815
...
class RecipesConfig:
    ...
    def from_dict(cls, data: dict[str, Any]) -> RecipesConfig:
        ...
        crons = [
            CronRecipe.from_dict(r) for r in data.get("crons", []) if isinstance(r, dict)
        ]
...
class AppManifest:
    ...
    def _validate_recipes(self) -> list[str]:
        ...
        for cr in self.recipes.crons:
            ...
            if not cr.agent:
                errors.append(f"{ctx} missing required field: agent")
            # schedule XOR every_secs (exactly one required)
            if bool(cr.schedule) == bool(cr.every_secs):
                if cr.schedule and cr.every_secs:
                    errors.append(
                        f"{ctx} must specify exactly one of 'schedule' or 'every_secs', not both"
                    )
                else:
                    errors.append(f"{ctx} must specify either 'schedule' or 'every_secs'")
