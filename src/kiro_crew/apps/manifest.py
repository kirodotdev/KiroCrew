...
#: Characters in one declared pattern. A pattern is RETAINED in the serialized
#: manifest and re-read on every grant check, so the count cap above is only half
#: a bound: 32 slots that can each hold megabytes is not bounded.
...
class Contributions:
    ...
    def validate(self, app_name: str, known_kinds: frozenset[str]) -> list[str]:
        ...
        *known_kinds* is passed in rather than imported: the manifest layer must
        ...
        # `units` carries KINDS, not `app/`-prefixed patterns, so the ownership rule
        # below does not apply to it -- but the COUNT and LENGTH bounds do, and this
        # list is retained in the installed manifest like any other declared field.
        # Bounded here rather than folded into the loop, because folding it in would
        # apply the prefix rule to a value that is never prefixed.
        ...
        for unit in self.units:
            if len(unit) > _MAX_CONTRIBUTION_PATTERN_CHARS:
                errors.append(
                    f"contributions.units: an entry is {len(unit)} characters, over "
                    f"the limit of {_MAX_CONTRIBUTION_PATTERN_CHARS}"
                )
        ...
        for field_name, patterns in (("events", self.events), ("projections", self.projections)):
            ...
            for pattern in patterns:
                if len(pattern) > _MAX_CONTRIBUTION_PATTERN_CHARS:
                    errors.append(
                        f"contributions.{field_name}: a pattern is {len(pattern)} "
                        f"characters, over the limit of "
                        f"{_MAX_CONTRIBUTION_PATTERN_CHARS}"
                    )
                    continue
                if not app_name:
