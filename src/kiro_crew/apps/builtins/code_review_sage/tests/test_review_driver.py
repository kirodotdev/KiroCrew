...
class TestReviewDriver(unittest.TestCase):
    def setUp(self):
        target = unittest.mock.patch.object(
...
class TestGithubPosting(unittest.TestCase):
    ...
    def test_github_poster_prompt_is_pending_and_never_submits(self):
        ...
        # Without a recorded predecessor the poster is never told to delete
        # anything: a leftover PENDING draft surfaces as the POST's 422, and the
        # poster must stop and report rather than remove a review it cannot
        # prove is stale.
        self.assertIn("Do NOT delete any existing review", p)
        self.assertIn("fail with 422", p)
        self.assertIn("never DELETE", p)
        self.assertNotIn("DELETE repos/", p)
