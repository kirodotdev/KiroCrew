...
# The app's own git repo (for the "open on GitHub" link in the panel).
try:
...
def _redact_thread(thread: object) -> list[dict]:
    ...
    for entry in thread:
        ...
        text, _ = redact_exfiltration_urls(str(entry.get("text", "")))
...
def _proj_for_preview(preview_url: str) -> tuple[dict | None, str]:
    ...
    Static previews are served from this backend's own loopback origin as
    `<static-base><projectId>/<rel>`, so the project id and the served file both
    fall out of the path. Returns `(None, "")` for any URL that isn't ours —
    ...
    The static origin is matched as an EXACT prefix rather than by pattern-matching
    `/<seg>/<rest>`, because a dev-server URL has the identical shape and only the
    origin distinguishes them. Reading a route as `<projectId>/<file>` would hand
    the agent a `sourceFile` that does not exist.
    """
    url = str(preview_url)
    static_base = _static_preview_base()
    if static_base and url.startswith(static_base):
        rel = url[len(static_base) :]
    else:
...
class _DevProxyHandler(BaseHTTPRequestHandler):
    ...
    def _keep_redirect_local(self, location: str) -> str:
        ...
        if (parts.hostname or "").lower() not in _LOOPBACK_HOSTS:
            return location
        if (parts.port or (443 if parts.scheme.lower() == "https" else 80)) != self.upstream_port:
            return location
        rest = parts.path or "/"
        if parts.query:
            rest += "?" + parts.query
...
# The one live static preview server: `{"srv": ThreadingHTTPServer, "url": str}`.
_STATIC_SRV: dict = {"srv": None, "url": ""}


def _static_preview_base() -> str:
    """Base URL of the loopback static preview server, starting it on demand.

    Returns "" if it cannot bind. There is deliberately NO fallback: the panel
    renders its "preview not reachable" state instead. The gateway-proxied route
    this used to fall back to served project files from the DASHBOARD's origin,
    which let a hostile previewed page escape the iframe sandbox, so it has been
    deleted (it answers 410). Do not reintroduce a same-origin fallback.
    """
    if _STATIC_SRV.get("url"):
        return str(_STATIC_SRV["url"])
    try:
        srv = ThreadingHTTPServer(("127.0.0.1", 0), _StaticInjectHandler)
    except OSError:
        return ""
    srv.daemon_threads = True
    threading.Thread(target=srv.serve_forever, daemon=True, name="kiro-static-preview").start()
    _STATIC_SRV["srv"] = srv
    _STATIC_SRV["url"] = f"http://127.0.0.1:{srv.server_address[1]}/"
    return str(_STATIC_SRV["url"])


def _static_preview_url(project_id: str) -> str:
    """URL the iframe should use to preview `project_id` from disk ("" if none)."""
    base = _static_preview_base()
...
_CHILD_ENV_STRIP = ("KIROCREW_PROXY_SECRET", "PORT", "NODE_OPTIONS")
...
def _static_response(
    ...
    try:
        ...
        return 500, "text/plain", str(exc).encode()
    # Content-based backstop for the private-key class: refuse whatever the file is
