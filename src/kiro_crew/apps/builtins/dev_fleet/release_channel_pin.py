...
#: The most mirror refs one resolve will retain, and the most bytes of a ref name.
#:
#: The mirror is fed by the remote, so its population is external and has no
#: natural ceiling. Every listing below asks git for ``--count`` of this plus one
#: -- git truncates before any byte reaches this process -- and a listing that
#: returns the extra row is an OVERFLOW, reported as an error rather than sorted:
#: truncating silently could drop the true tip, so the channel refuses to name one
#: until an operator looks. Two thousand releases is beyond any real remote's tag
#: history; the cap exists to make the bound a fact rather than to be reached.
_MAX_RELEASE_REFS = 2048
_MAX_REF_NAME_LEN = 64


class MirrorOverflow(RuntimeError):
    """The mirror holds more, or longer, refs than one resolve will retain.

    Raised BEFORE any name is filtered or sorted, so an overflowing listing can
    never be read as a smaller population that happened to have no other tags:
    a truncated tail would sort and could name a release that is not the tip.
    """
...
async def fetch_refs(repo: str, *, timeout: int = 120) -> str | None:
    ...
    **``--prune`` is what makes the mirror a fact rather than a habit, and it is
    ...
    owned by :func:`advertised_release_commit`, which asks the configured upstream what it
...
#: Git's answer when ``--worktree`` is probed and ``config.worktree`` does not
#: exist. Anchored on both the file name and the OS reason, so a malformed or
#: unreadable file -- also exit 128 -- never matches.
_WORKTREE_CONFIG_ABSENT_RE = re.compile(
    r"unable to read config file '[^']*config\.worktree': No such file or directory"
)


async def repo_supplied_filter(checkout: str) -> str:
    """Name of a repo-supplied content filter this checkout would run, else ``""``.
    ...
    present in this repository's config, which is the case worth refusing: that
    config is writable by anything with the agent's filesystem access.

    CHECKOUT CONTEXT. Every git read runs with ``-C <checkout>``. That is
    load-bearing for linked worktrees: ``includeIf gitdir:`` is evaluated against
    the gitdir of the worktree being operated on, so a conditional include scoped
    to a staging worktree is invisible when the primary checkout is probed. Create
    first uses ``worktree add --no-checkout``, then passes that empty staging
    checkout here, making the include visible before the same staging checkout is
    populated.

    SCOPES. ``--local`` (the repository's shared ``config``), plus ``--worktree``
    (the supplied checkout's ``config.worktree``) when ``extensions.worktreeConfig``
    makes that scope live. ``--includes`` is mandatory on both: for a
    specific-scope query git defaults include-following OFF, so a driver reached
    through ``include.path`` resolves at checkout time while staying invisible to a
    probe without it. Global and system config are deliberately NOT probed -- that
    is the operator's own machine (``git lfs install`` writes ``filter.lfs.*``
    there), not something a repository supplies, and refusing it would break every
    host with git-lfs installed.
    ...
    :func:`fetch_refs` and :func:`advertised_release_commit` stay standard because
    ...
    if rc == 0 and out.strip() == "true":
        ...
        # the file exist. The scope is probed whenever the extension is on -- never
        # gated on the file existing first, because a file created between an
        # existence check and the populate would be a scope the probe never read
        # and git would still honour. Git answers a missing file with a specific
        # `fatal: unable to read config file '<gitdir>/config.worktree': No such
        # file or directory`, and ONLY that answer is read as an empty scope below;
        # every other failure (a malformed file is also exit 128) stays a failed
        # probe, so treating the missing file as benign does not fail open.
        scopes.append("--worktree")
    for scope in scopes:
        rc, out, err = await runtime._run_cmd(
            ["git", "-C", checkout, "config", scope, "--includes", "--name-only", "--list"],
            timeout=20,
            mode="strict",
        )
        if rc != 0:
            ...
            # produced output or an error message counts as a failed probe. The one
            # message that is NOT a failed probe is the worktree scope's file being
            # absent, which is the ordinary state of a checkout that enabled the
            # extension and never wrote per-worktree config.
            if not out.strip() and not err.strip():
                continue
            if scope == "--worktree" and _WORKTREE_CONFIG_ABSENT_RE.search(err or ""):
                continue
...
async def advertised_release_commit(repo: str, tag: str) -> tuple[str | None, str | None]:
    """The commit oid the CONFIGURED remote advertises for *tag*, or an error.
    ...
    The query names exactly ``refs/tags/<tag>`` and its explicit ``^{}`` pattern. Git
    prints both the tag object and peel line for an annotated tag when given that pair,
    so provenance does not require enumerating every release tag. Parsing accepts only
    those two exact refs and retains at most their two validated, 64-character-bounded
    oids; the peel wins because it is the commit an annotated tag resolves to.

    A tag the remote does not carry is NOT an error from git: ``ls-remote`` exits 0 and
    prints nothing. That returns ``(None, None)``; a non-zero exit is reserved for the
    query itself failing, which fails CLOSED.
    """
    ...
    rc, out, err = await runtime._run_cmd(
        ["git", "-C", repo, "ls-remote", "--tags", remote, direct_ref, peeled_ref],
        timeout=60,
    )
    if rc != 0:
        detail = runtime._redact((err or "").strip())[:200]
...
async def resolve(*, repo: str) -> dict:
    ...
    except MirrorOverflow as exc:
        # Said out loud, once, as the row's error: refusing to name a tip beats
        # sorting a truncated set, which could silently pick the wrong release.
        return {"ok": False, "error": f"{exc}; refusing to resolve the channel"}
    if listed is None:
        return {"ok": False, "error": "cannot list tags (git tag failed)"}
    return await _resolve_tagged(repo, listed)


def _bounded_mirror_rows(listed: str) -> list[str]:
    """Split one bounded mirror listing and reject overflow before filtering."""
    rows = [line.strip() for line in listed.splitlines()]
    if len(rows) > _MAX_RELEASE_REFS:
        raise MirrorOverflow(f"more than {_MAX_RELEASE_REFS} refs under {_TAG_NS}")
    overlong = sum(1 for row in rows if len(row) > _MAX_REF_NAME_LEN)
    if overlong:
        raise MirrorOverflow(
            f"{overlong} ref name(s) under {_TAG_NS} exceed {_MAX_REF_NAME_LEN} bytes"
        )
    return rows
...
async def list_release_tags(repo: str) -> list[str] | None:
    ...
    Bounded at the source: git is asked for ``--count`` of :data:`_MAX_RELEASE_REFS`
    plus one, so at most that many lines ever reach this process. The one extra row,
    or any name longer than :data:`_MAX_REF_NAME_LEN`, is :class:`MirrorOverflow` --
    raised on the RAW rows, before a single name is filtered, so an overlength row
    cannot mask the count and a dropped name cannot pass for an absent one. No
    release tag this channel admits is that long; a longer one is something else
    wearing the mirror namespace, and it is reported rather than skipped.
    ...
    if listed is None:
        return None
    return _bounded_mirror_rows(listed)
...
async def worktree_state(path: str, resolved: dict) -> dict:
    ...
    ``at_tip`` says whether the checkout is at the CHANNEL TIP, not whether it
    tracks ``BASE_BRANCH`` — a release worktree is not trying to track main.
    Together with ``version``, that is the row's complete currency signal.
    ...
    """
    # Three keys, all read by `fleet_state._release_channel`. The worktree's own
    ...
    out: dict = {"at_tip": False, "detached": None, "version": None}
...
async def _release_at_head(path: str) -> str | None:
    ...
    if listed is None:
        return None
    for tag in _bounded_mirror_rows(listed):
