...
def _redact_prose(text: str) -> str:
    ...
    FAILS CLOSED by raising :class:`ProseRedactionUnavailable`. Returning the text
    unscanned because the diff beside it passed a fail-closed scan and the PR is only a
    draft does not hold: the prose is a SEPARATE artifact from the diff, it is the part
    the agent wrote most freely, and the provider CLI publishes it where a description
    cannot be un-published (it persists in the API's edit history even after an edit).
    Every other egress path in this app fails closed for exactly this reason
    (`mcp_server._redact_result`, `routes._redact_for_display`).
