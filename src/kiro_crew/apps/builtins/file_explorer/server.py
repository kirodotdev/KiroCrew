...
try:  # arrives with the DECLARED pdfplumber dependency (setup.cfg), not optional
    from pdfminer.high_level import extract_text as _pdf_extract
except Exception:  # pragma: no cover - only a deliberately stripped install
...
def _pdf_text_units(p: Path) -> list[tuple[str, int, str]]:
    """Per-page PDF text, parsed ONLY from bytes this process already read.

    Everything here goes through :func:`safe_read_file_bytes`. An earlier
    revision fell back to ``doc_parser.extract_text(str(p))`` when pdfminer was
    missing, which REOPENED the user-derived path from disk, outside the
    hardened read gate: between validation and that reopen the path could be
    swapped for a symlink to a sensitive file, and its contents would then
    surface as search results. Parsing bytes closes that window — there is no
    second filesystem access to race.

    The fallback was also unreachable in any supported install: pdfminer.six
    arrives with the DECLARED ``pdfplumber`` dependency (setup.cfg), so the
    import below only fails in a deliberately stripped environment, where
    returning no PDF matches is the correct, safe answer.
    """
    if p.stat().st_size > MAX_PDF_SEARCH_BYTES:
        return []
    if _pdf_extract is None:
        # No parser: skip PDFs rather than reaching for a path-based reader.
        return []
...
class FileExplorerHandler(BaseHTTPRequestHandler):
    ...
    def _h_read(self, qs: dict[str, list[str]]) -> None:
        ...
        body: dict = {
            ...
            "encoding": "utf-8",
            "content": "",
            # STRING, not a number: this is a nanosecond mtime (~1.7e18), and
            # JSON numbers land in a JavaScript double, whose exact-integer
            # range stops at 2^53-1 (~9.0e15). Sent as a number it silently
            # rounds in the browser, comes back changed, and the exact-match
            # guard in /write then rejects EVERY save with 409. It is an opaque
            # token to the client, so text costs nothing and is lossless.
    ...
    def _h_write(self, qs: dict[str, list[str]]) -> None:
        ...
        if base_token:
            # Opaque STRING compare, not int(): the token is a nanosecond mtime
            # rendered as text (see /read). Parsing it back to an int added a
            # ValueError branch that fell through to `pass` — i.e. a malformed
            # token SKIPPED the conflict guard entirely, letting a client
            # overwrite a file that had changed underneath it. Comparing the
            # text has no such hole and needs no parsing.
            if str(st.st_mtime_ns) != base_token:
    ...
    def _locked_write(self, p: Path, body: bytes, base_token: str, base_mtime: str) -> None:
        ...
        st = p.stat()
        if base_token:
            # Same opaque-string compare as the pre-lock check; this one is the
            # authoritative guard, so it must not fail open either.
            if str(st.st_mtime_ns) != base_token:
                raise PathError("file changed on disk since it was loaded", 409)
        elif base_mtime:
            try:
        ...
        return self._json(
            200,
            {
                ...
                # String for the same safe-integer reason as /read: the editor
                # adopts this as its next base token, so rounding it here would
                # 409 the very next save.
                "mtime_ns": str(st2.st_mtime_ns),
            },
        )
