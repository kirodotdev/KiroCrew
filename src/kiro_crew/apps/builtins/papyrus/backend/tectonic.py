...
def redact_url(url: str) -> str:
    ...
    A mirror override may carry credentials in userinfo or a signed query string,
    so neither is ever logged. Mirrors
...
def _provision_job(root: Path | None) -> None:
    ...
    try:
        ...
        _set_state(STATE_ERROR, error=f"provisioning crashed: {exc}")
