...
async def _handle_get_project(request: web.Request) -> web.StreamResponse:
    ...
    def _read() -> tuple[Path, str | None, list[str], bool]:
        ...
        return (
            ...
            store.pdf_path(project, main_file).is_file(),
        )
...
async def _handle_pdf(request: web.Request) -> web.StreamResponse:
    ...
    def _read() -> tuple[Path, bytes | None]:
        """BLOCKING — authorize, resolve, and read the PDF in ONE hop.

        The bytes are read in the same closure as the ``is_file()`` that found
        them, which is strictly narrower than the previous two hops: a gap there
        let the file be deleted between the probe and the read. A read that still
        fails propagates exactly as it did before rather than being converted —
        the 404 remains reserved for "no PDF yet".
        """
        project = _project(name)
        main_file = store.resolve_main_file(project)
        if main_file is None:
        ...
        if not pdf.is_file():
            return project, None
        return project, pdf.read_bytes()

    project, data = await asyncio.to_thread(_read)
    if data is None:
