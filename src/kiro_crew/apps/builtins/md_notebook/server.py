...
async def api_note_delete(request: web.Request) -> web.Response:
    ...
    def _to_trash() -> str:
        # Re-checked here, in the thread that mkdirs and moves, under the vault
        # write lock: the check above ran before an `await`, and a merge landing
        # `.trash -> public` in that gap would otherwise be followed by both.
