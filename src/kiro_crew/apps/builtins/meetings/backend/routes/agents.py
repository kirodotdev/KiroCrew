...
async def handle_toggle_agent(request: web.Request) -> web.Response:
    ...
    enabled_list = list(meta.get("agents_enabled") or [])
    ...
    meta["agents_enabled"] = enabled_list
    ...
    # before it is told about it), so read and write cannot share a thread. The
    # ordering is unchanged from the inline version.
    await asyncio.to_thread(store.write_meeting_meta, meeting_id, meta, root)
    return web.json_response(
