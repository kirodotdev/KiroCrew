...
def is_noise(text: str) -> bool:
    ...
    Speech recognition emits a lot of ``"I"`` / ``"OK I I"`` — three-or-fewer
    words that are all one or two characters. Anything longer, or containing a
    real word, passes.
    """
    words = text.split()
    return bool(words) and len(words) <= 3 and all(len(w) <= 2 for w in words)
    ...
    async def flush_now(self) -> None:
        ...
        for _ in range(_MAX_DRAIN_BATCHES):
            ...
            if len(self.queue) == before:
                break  # a flush that consumed nothing — stop rather than spin
        if self.queue:
            logger.warning(
