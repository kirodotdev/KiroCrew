...
def _delete_oac_for_site(site_id: str, profile: str) -> None:
    """Best-effort: delete the site's OAC (named deploy-web-<site_id>-*). Orphan is harmless/free."""
    try:
        ...
        prefix = f"{_oac_name_prefix(site_id)}-"
        for item in items:
        ...
        logger.warning("deploy-web: OAC cleanup skipped for %s (non-fatal)", site_id)
...
def destroy(site_id: str, profile: str, region: str = DEFAULT_REGION) -> dict[str, Any]:
    ...
    _delete_oac_for_site(site_id, profile)
    if bucket:
        empty_bucket(bucket, profile)
        _checked(["s3api", "delete-bucket", "--bucket", bucket], profile, action="s3:DeleteBucket")
    return {"site_id": site_id, "destroyed": True, "bucket": bucket, "distribution_id": dist_id}
