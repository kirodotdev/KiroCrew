...
_POSIX_SENTINEL_PINNING = {
    ...
    # ``_save_spec_doc`` writes through the SAME pinned descriptor and fails closed
    # without it, so on Windows editing a document (and therefore duplicating a
    # spec, which writes the copies through it) returns
    # ``doc_write_unsupported``/501 by design. These assert the written bytes, so
    # there they are asserting a capability the product deliberately declines.
    # The REFUSAL tests are not listed: not-editable, stale-hash conflict and the
    # symlink refusal are all decided before the platform check, so they are
    # portable and still guard the Windows path.
    "test_doc_write_persists_the_edit_and_returns_the_new_hash",
    "test_doc_write_creates_a_document_only_while_it_is_absent",
