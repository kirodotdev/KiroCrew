...
async def _section_marker(slot: Any, session_key: str, args: dict[str, Any]) -> str:
    ...
    meta = {"label": label}
    if deferred:
        # One seam flushes every held marker, so their stamps are all the flush
        # instant; this is the boundary one, carried for display, never for order.
        meta["at"] = datetime.now(timezone.utc).astimezone().isoformat()
    if not deferred:
