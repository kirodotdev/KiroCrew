...
chat_handlers, chat_persistence, chat_tags, chat_tag_grants) are
...
_USER_ORIGIN_DIRECTIVES = _USER_SURFACE_DIRECTIVES | frozenset(
    # ``ask_question`` is EXCLUDED, and the producer sweep is why: two shipped
    # conductor charters grant it by name so an unattended patrol cycle can ask.
    {"section_marker"}
)
...
def _user_origin_refusal(kind: str, session_key: str, middle: str) -> str:
    ...
    worked, while ``section_marker`` has to say the refusal stands even with a tab
    open, which is the counter-intuitive case. Splitting the shared skeleton out
    keeps that difference deliberate instead of leaving two hand-written sentences
    to fall out of step on the parts that must match.
    """
    return (
...
async def _section_marker(slot: Any, session_key: str, args: dict[str, Any]) -> str:
    ...
    # credential mid-pattern so the regex misses it, which would leave
    ...
    content = f"— End of section: {label} —" if label else "— End of section —"
    ...
    if getattr(slot, "_in_stage_execution", False):
        ...
        # ``if not slot._in_stage_execution``, with ``markers_only=will_synthesize``, and
    ...
    slot._deferred_notes.append(
        {
            # Same shape as a /note hold id: without one a held marker replayed
            # after a rows-only handover AND lost its durable copy at the union.
