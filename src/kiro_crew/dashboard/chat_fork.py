...
async def api_chat_slot_fork(request: web.Request) -> web.Response:
    ...
    if request_app:
        ...
        if slot_is_channel_backed(slot):
            # BEST-EFFORT audit: ``sel()`` can raise on a failed init, and an
            # escaping audit would turn this 404 into a 500 -- distinguishable
            # from a missing slot, which is the enumeration signal the 404
            # exists to hide. The refusal must stand without its audit line.
            try:
                sel().log_api_access(
                    caller=request_app,
                    operation="chat.slot_fork",
                ...
                logger.warning(
                    "slot_fork: denial audit failed for %s (refusal stands)",
                    name,
                    exc_info=True,
                )
            return web.json_response({"error": "not found", "code": "slot_not_found"}, status=404)
            ...
            for _ in range(_SNAPSHOT_ATTEMPTS):
                ...
                try:
                    await save_slot_off_loop(
                        state, slot, best_effort=False, expected_history_key=authorized_history_key
                    )
                except Exception:
                    logger.warning(
