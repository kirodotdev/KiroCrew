...
def _guard(request: web.Request, operation: str) -> Optional[web.Response]:
    ...
    # An app token carries BOTH request["user"] and request["app"] (token_auth
    # sets them together), so checking "user" alone admits an app that declares
    # /api/cloud in its manifest — and these routes create, stop and terminate
    # billable AWS resources. "Owner-only" means the human at the dashboard: a
    # deliberate click is the consent here, and an app acting unattended has
    # none. Deny any app identity outright rather than relying on path scope.
    if request.get("app"):
        _audit(operation, "denied", error="app token rejected")
        return web.json_response(
            {
                "error": "cloud provisioning is owner-only (app tokens cannot reach it)",
                "code": "cloud_owner_only",
            },
            status=403,
        )
...
async def api_cloud_launch_create(request: web.Request) -> web.Response:
    ...
    async with _launch_lock(state):
        existing = await _in_executor((await _astore(state)).list)
