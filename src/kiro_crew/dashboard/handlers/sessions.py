...
async def api_mediated_secret_request(request: web.Request) -> web.Response:
    ...
    # Off the event loop: proof verification is a synchronous HMAC + file read
    # (member_memory_auth reads the masked signing key), so awaiting it via
    # asyncio.to_thread keeps a slow disk read from blocking the gateway loop —
    # the same idiom the handler uses for the vault read/dispatch below.
    proof_ok = await asyncio.to_thread(
        lambda: verify_member_session_proof(proof, session_key, audience="kirocrew-secrets")
    )
    if not proof_ok:
        _sel().log_api_access(
            caller=session_key,
