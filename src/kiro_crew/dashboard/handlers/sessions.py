...
async def _fetch_usage_bg() -> None:
    ...
    try:
        ...
        if api_usage and api_usage.get("credits_plan") is not None:
            ...
            return
        # Fallback: scrape kiro-cli /usage stdout. The free API path above also
        # carries bonus breakdowns; this paid model turn is reached only when
        # that authoritative path is unavailable.
