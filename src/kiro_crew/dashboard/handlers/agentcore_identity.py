...
def apply_agentcore_runtime() -> bool:
    """Deferred so importing the dashboard routes never loads the AWS extra."""
    # optional dependency: kirocrew[agentcore] (botocore) is not a base install;
    # the route module must import without it, so the extra loads on first call.
...
def ensure_extra() -> str:
    # optional dependency: see apply_agentcore_runtime.
...
def _file_posture(row: dict[str, Any] | None = None) -> str | None:
    ...
    if row is None or row.get("enabled") is not True:
        # Only a boolean True is "on": a malformed "false" string is truthy
        # and would otherwise DISPLAY a posture the runtime never enabled.
        return None
...
async def _drop_live_identity(request: web.Request, *, reset_proxy: bool = True) -> None:
    ...
    already-injected Gateway URL and proxy/bearer headers, so Save → Off
    ...
    try:
        ...
        # a raise here would leave the SigV4 listener serving the
...
def _write_home_document(data: dict[str, Any]) -> None:
    ...
    # A link at the destination is refused, not replaced. It could not be
    # snapshotted (reading through it would snapshot the target, not the
    # keystone), so a lockdown failure after the replace would roll back as a
    # FIRST write and unlink the policy path -- leaving no ceiling for the
    # next restart to load. Same disposition as the staging leaf above.
    if platform_compat.is_link_or_junction(path):
        try:
            tmp.unlink()
        except FileNotFoundError:
            pass
        raise FileExistsError(f"security policy path is a link: {path}")
    if path.is_file():
        try:
            ...
            try:
                ...
                pass
            raise
    # The rollback's bytes are staged BEFORE anything changes: a durable,
    # exclusive ``.bak`` leaf on the sensitive floor holding the snapshot.
    # Rolling back is then one atomic replace of a file already on disk --
    # never a write into dest, so no failure between here and the end can
    # leave ``security_policy.json`` truncated for the next boot to load.
    backup = _backup_leaf(path)
    if previous is not None:
        try:
            # A leftover from a rollback that could not land is evidence,
            # not a lock: this Save stages its own snapshot over it.
            unlink_with_retry(backup, missing_ok=True)
            _write_exclusive_tmp(backup, previous)
        except OSError:
            # Nothing has been published yet: refuse the Save whole.
            try:
                tmp.unlink()
            except FileNotFoundError:
                pass
    ...
    try:
        ...
        _rollback_replaced_home_document(path, previous, backup)
        raise
    # Lockdown held: the staged snapshot has nothing left to restore.
    try:
        unlink_with_retry(backup, missing_ok=True)
    except OSError:
        logger.warning("could not remove the policy rollback backup %s", backup, exc_info=True)


def _backup_leaf(path: Path) -> Path:
    """The staged-snapshot sibling of a keystone: ``<name>.bak``, on the same
    sensitive floor and sandbox mask as ``.tmp``/``.lock``."""
    return path.with_name(path.name + ".bak")


def _rollback_replaced_home_document(path: Path, previous: bytes | None, backup: Path) -> None:
    """Undo a committed replace whose dest lockdown failed.

    The snapshot was staged to *backup* BEFORE the forward replace, so the
    rollback is a single atomic replace of bytes already durable on disk;
    dest is never written into. A first write has no previous bytes: unlink
    dest so a failed Save cannot leave the requested policy active. Both
    paths retry the Windows sharing-violation window and verify dest is gone
    or matches the snapshot before ``write_failed`` returns; if the replace
    cannot land after the retries, the error propagates with dest still
    holding the (unlocked) published bytes -- a loud, whole file, never a
    truncated one. The Save still fails ``write_failed``.
    """
    if previous is None:
    ...
    for attempt in range(_REPLACE_MAX_ATTEMPTS):
        try:
            replace_with_retry(backup, path)
            _fsync_parent(path)
