...
def audit_channel_send(
    *,
    ...
    ``chat_note_mirror`` files its ``leg_timeout``, ``leg_error`` and
    ``unbound_at_authoring`` endings through this same emitter, so both stream into
    one filterable place. The one refusal that files NO row is the no-transport
    case, where the install has no channel surface at all: there the refusal is
    about the INSTALL rather than the request, and a row would name no destination.
