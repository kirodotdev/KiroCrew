    ...
    *tool_name* is the SEL caller identity, so an operator can tell which sender
    ...
    def _sample_then_resolve() -> tuple[int | None, Any]:
        ...
        try:
            ...
            before = None
        return before, _resolve_channel_target(state, session_key, link)
    ...
    async def _still_permitted(index: int) -> bool:
        ...
        if not index:
            if governance_ceiling_unchanged(permit_ceiling):
                try:
                    settled_ceiling, settled = await asyncio.to_thread(_sample_then_resolve)
                except Exception:
                    logger.warning(
                        "channel send: pre-part-1 recipient re-ask failed for %s",
                        session_key,
                        exc_info=True,
                    )
                    _audit("error", "resolve_failed_before_part_1")
                    return False
                ...
                if not governance_ceiling_unchanged(settled_ceiling):
                    ...
                    try:
                        ...
                        logger.warning(
                            "channel send: pre-part-1 generation re-confirm failed for %s",
                            session_key,
                            exc_info=True,
                        )
                        _audit("error", "resolve_failed_before_part_1")
                ...
                if settled is None or settled[0].channel_id != resolved.channel_id:
                    ...
                    return False
                # Re-walked like both other arms: the re-ask awaited, and an UNLINK across it
                # leaves the resolved id identical while the binding itself is revoked.
                settled_link, settled_is_origin = _walk_ladder()
                if not _binding_unchanged(settled_link, settled_is_origin, link, link_is_origin):
                    _audit("denied", "link_changed_during_recheck_before_part_1")
                    logger.warning(
                        "channel send: binding for %s changed while the pre-part-1 "
                        "recipient re-ask ran; refusing",
                        session_key,
                    )
                    return False
                return True
            ...
            try:
                confirm_ceiling, confirm = await asyncio.to_thread(_sample_then_resolve)
            except Exception:
                logger.warning(
                    "channel send: pre-part-1 re-confirm failed for %s",
                    session_key,
                    exc_info=True,
                )
                _audit("error", "resolve_failed_before_part_1")
            ...
            if not _binding_unchanged(final_link, final_is_origin, link, link_is_origin):
                _audit("denied", "link_changed_during_recheck_before_part_1")
                logger.warning(
                    "channel send: binding for %s changed while the pre-part-1 "
                    "re-confirm ran; refusing",
                    session_key,
                )
                return False
            return True
        ...
        try:
            recheck_ceiling, recheck = await asyncio.to_thread(_sample_then_resolve)
        except Exception:
            logger.warning(
        ...
        if not governance_ceiling_unchanged(recheck_ceiling):
            try:
                ...
                logger.warning(
                    "channel send: mid-send re-confirm failed for %s",
                    session_key,
                    exc_info=True,
                )
                _audit("error", f"resolve_failed_mid_send_before_part_{index + 1}")
            ...
            if not _binding_unchanged(final_link, final_is_origin, link, link_is_origin):
                _audit("denied", f"link_changed_during_recheck_before_part_{index + 1}")
                logger.warning(
                    "channel send: binding for %s changed while the mid-send re-confirm "
                    "ran; aborted before part %d of %d (%d already delivered)",
                    session_key,
                    index + 1,
                    len(parts),
