...
async def api_directory_upload(request: web.Request) -> web.Response:
    ...
    **Ancestor-swap and short-write safety.** The write goes through the SAME
    ...
    An OVERWRITE carries the replaced file's mode and, where the platform has
    the xattr syscalls, its access-control xattrs. It does NOT reproduce a
    native ACL that lives outside that view -- macOS keeps extended entries
    behind ``acl_get_file`` and Windows has no xattr view at all -- so on
    those platforms a replacement can lose an entry the original carried.
    That residual is shared with every other inode-replacing writer here
    (``_file_write_blocking``, ``pinned_fs.write_file_pinned``) and is not
    specific to uploads.
    """
    caller = request.get("user", "dashboard")
    ...
    def _write() -> str | None:
        ...
        try:
            ...
            #
            # Where the platform has no xattr syscalls the ACL half degrades
            # to a mode-only carry, so a native ACL beyond the mode is not
            # reproduced: on macOS the POSIX bits still carry and only an
            # extended (acl_get_file) entry is lost, while on Windows the
            # DACL is the whole model and the replacement takes the parent's
            # inheritable entries. That residual is the platform's, not this
            # endpoint's -- `_file_write_blocking` and `pinned_fs` carry
            # exactly as much and no more -- so refusing here would take
            # overwrite away on two of the three supported platforms while
            # leaving every other writer in the product unchanged.
    ...
    # would otherwise starve validation behind.
    try:
        outcome = await _run_path_probe(_write, transfer=True)
    except _PathProbeBusy:
        return _probe_busy_response(
