...
async def api_project_git_status(request: web.Request) -> web.Response:
    ...
    #
    # The key is (repoRoot, path, status, staged), NOT path alone: one file with
    # both staged and unstaged changes ("MM", "AM", "MD") legitimately yields two
    ...
    # unstaged lane and undercount the file total. repoRoot is part of the key
    # because sibling packages in a multi-repo answer routinely share a
    # repo-relative path ("src/PkgA/a.txt" and "src/PkgB/a.txt" are both
    # "a.txt"): those are different files, and GitPanel keys its rows
    # repo-qualified (`${group.root}:${path}:${staged}`) to match. A real
    # redaction collision still has an identical tuple, so it still collapses.
    deduped_files: list[dict] = []
    seen_keys: set[tuple[str | None, str, str | None, bool | None]] = set()
    for f in result.get("files", []):
        f["path"] = redact(f["path"])
        if f.get("repoRoot"):
            f["repoRoot"] = redact(f["repoRoot"])
        key = (f.get("repoRoot"), f["path"], f.get("status"), f.get("staged"))
        if key in seen_keys:
            continue
        seen_keys.add(key)
    ...
    # Group rows are the same dict objects as the merged list above, so the
    # per-row redaction already applies here; only the group's own fields are
    # left to redact.
