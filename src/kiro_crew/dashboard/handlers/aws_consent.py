...
async def api_aws_consent_post(request: web.Request) -> web.Response:
    ...
    try:
        ...
        # The compare could not run, so the confirmation is refused rather
        # than recorded unverified. Only that failure carries this code: a
        # grant WRITE that fails, for this or any other service, propagates
        # as the unhandled error it is, the same as every other consent
        # write, instead of reading as a Bedrock source check. The exception
        # text goes to the log, never to the form.
        logger.warning("bedrock_kb one-account check failed at confirm: %s", e)
        return web.json_response(
            {
                "error": (
