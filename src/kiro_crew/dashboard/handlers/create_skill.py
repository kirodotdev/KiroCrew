...
async def api_create_skill_from_session(request: web.Request) -> web.Response:
    ...
    # "default" is the sentinel for the host default agent (kiro-cli's built-in), not a
    # resolvable name in list_agents(); normalize it (and empty) to "" so a default-agent
    # session runs under the host default instead of being refused. A genuinely-unknown
    # NAMED agent is still rejected below -- no silent privilege downgrade.
    if target_agent == "default":
        target_agent = ""
    target_app = getattr(slot, "_app", "") or ""
    agent_prevalidated = False
    if target_agent:
        _resolved, agent_err, _agent_err_code = await asyncio.to_thread(
    ...
    if getattr(info, "queued", False):
        # A queued capture authors at DRAIN time -- after the pre-spawn privacy
        # re-check above -- and the queue drain does not re-check the session's
        # restriction, so a session flipped to incognito while the capture waits
        # in the queue would still have its now-private transcript authored into
        # a skill. Refuse rather than queue (mirroring spawn()'s own no-queue rule
        # for prevalidated app spawns): cancel the queued entry and let the owner
        # retry, so the privacy check runs fresh on the next attempt.
        await state.subagents.cancel(info.id)
        return web.json_response(
            {"error": "subagent queue is at capacity; retry shortly", "code": "at_capacity"},
            status=503,
        )
