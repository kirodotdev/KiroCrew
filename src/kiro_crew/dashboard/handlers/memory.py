...
async def _write_embed_model_config(path: str, dim: int) -> Callable[[], Awaitable[None]]:
    """Persist verified model settings and return a conditional rollback."""
...
def _apply_embedding_model(store: object, raw: str, loop: "asyncio.AbstractEventLoop") -> None:
    ...
    inherited_vectors = bool(_read_memory_config().get("embed_model_legacy_ids"))
    ...
    return web.json_response(
        {
            ...
            "setup_warning": (
                LEGACY_EMBEDDING_WARNING
                if custom is not None and _read_memory_config().get("embed_model_legacy_ids")
                else ""
            ),
            "can_retry": can_retry,
