...
def _redacted(value: object) -> object:
    """Run the shared projection redaction chain over *value*.

    One wrapper rather than the chain itself, so every egress on this surface runs
    the same code the sibling member reads and the WebSocket hub run and the three
    cannot drift apart.

    The import is deliberately function-local. ``eventlog.service`` imports the
    crew-log storage package at module scope, and two launch tripwires
    (``test_crew_log_emit.py``, ``test_crew_log_routes.py``) require this handler's
    import graph to stay clear of it -- a flag-off launch must not load storage.
    ``handlers/members.py`` and ``dashboard/eventlog_ws.py`` import the same name
    the same way, for the same reason.
    """
    from kiro_crew.eventlog.service import _redact_projection_value

    return _redact_projection_value(value)
...
def _type_carries_a_credential(event_type: str) -> bool:
    ...
    """
    return _redacted(event_type) != event_type
...
def _push_projection(
    ...
    if schema is not None:
        ...
        payload["schema"] = _redacted(schema)
    try:
