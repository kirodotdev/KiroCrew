...
def _read_pinned(root: Path, rel: str, *, max_bytes: int | None = None) -> bytes | None:
    """Read *rel* under *root* through a descriptor PINNED to the resolved directory.

    Qualified resolution proves the bundle's identity and then hands back a path. Reading
    that path re-resolves it, so an uninstall-and-reinstall of a different bundle at the
    same canonical name in between is served under the first bundle's key -- the identity
    was checked, but not on the thing finally read.

    Pinning removes the second resolution. ``open_dir_pinned`` walks the ancestor chain
    component by component, so a swapped ancestor cannot redirect it, and the file is then
    opened RELATIVE to that descriptor: ``openat`` stays inside the directory inode the
    resolver validated however the path is later rebound. A replacement necessarily gets
    fresh directory inodes, so it can no longer be reached under the old key at all.

    ``None`` when the directory cannot be pinned, which fails closed exactly as an
    unreadable file already does.
    """
    try:
        fd = pinned_fs.open_dir_pinned(root, what="skill directory")
    except (OSError, RuntimeError, ValueError, pinned_fs.PinnedPathRefusal):
        return None
    try:
        return safe_read_file_bytes_nolink(
            rel, within_root=str(root), max_bytes=max_bytes, dir_fd=fd
        )
    finally:
        os.close(fd)
...
_WITHHELD_MESSAGE = (
    "This skill file was withheld by the file-read gate, so its contents were not "
    "served. The refusal is recorded in the SEL audit log — check it for the decision "
    "on this read."
)
...
async def api_skill_file(request: web.Request) -> web.Response:
    ...
    def _resolve_and_read() -> tuple["Path | None", str | None, str | None]:
        ...
        try:
            data = _read_pinned(r, rel_path, max_bytes=SKILL_FILE_MAX_BYTES)
        except FileTooLargeError:
...
async def api_skill_detail(request: web.Request) -> web.Response:
    ...
    if content is None:
        return web.json_response({"error": "not found"}, status=404)
    if name.startswith(PACKAGE_KEY_PREFIX):
        # Both package reads above — the resolver's and the row's — reach this one exit, and
        # a refusal-only audit makes the COMMON case the unrecorded one.
