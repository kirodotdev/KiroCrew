...
async def api_member_activity(request: web.Request) -> web.Response:
    ...
    def _read_activity_records() -> list[dict]:
        ...
        svc = get_service()
        # No pre-filter limit. A limit here counts events of EVERY type, and the
        # ACTIVITY_RECORD filter runs after, so a member whose log holds more
        # newer message or patrol events than the window shows an EMPTY drawer
        # while `capped` below reports false -- the records are on disk, the
        # window simply never reached them. The display cap still bounds the
        # response; what it must not do is decide which events get to be seen.
        #
        # This is not an unbounded read: history() calls _ensure_loaded() and
        # filters the in-memory event list, so every call holds the whole log
        # regardless of limit. `limit` slices the result, it does not bound the
        # load. Bounding the load itself belongs to the streaming fold in the
        # event log, not here, where a limit only loses records it has already
        # paid to read.
...
async def api_member_history(request: web.Request) -> web.Response:
    ...
    # past the patterns).
    # The SHARED redactor, not a local copy: this one redacts dict KEYS as well
    # as values, which is what a local values-only pass got wrong. A contributor
    # granted `contributions.events` chooses its own `data` keys, so a
    # credential- or exfiltration-URL-shaped KEY crossed to the browser here
    # while the three sibling reads (the catch-up GET, the fold broadcast and the
    # projection push) all scrubbed it. One function, one behaviour at every
    # egress, and it is depth-bounded so a deep payload cannot make it raise.
    from kiro_crew.eventlog.service import _redact_projection_value

    redacted_events = []
    for event in events:
        if isinstance(event, dict) and isinstance(event.get("data"), dict):
            event = {**event, "data": _redact_projection_value(event["data"])}
