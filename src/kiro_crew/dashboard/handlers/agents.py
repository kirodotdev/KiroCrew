...
from kiro_crew.config.loader import _AVATAR_IMAGE_EXTS as _LOADER_AVATAR_IMAGE_EXTS
from kiro_crew.config.loader import (
    ConfigReadError,
    KiroCrewAgentConfig,
    KiroCrewConfig,
...
def _avatar_path(name: str) -> Path | None:
    """Any stored avatar variant for ``name``, or None.

    Fallback resolution only — a config with a valid ``file`` pin resolves
    through :func:`_live_avatar_file` instead, which serves exactly the
    committed file.
    """
...
def _live_avatar_file(name: str, pin: object) -> Path | None:
    ...
    With a valid pin only that exact file counts — an orphaned install left
    by a mid-save crash must not be served in the committed picture's place.
    A field without a pin (hand-edited config) falls back to any stored
    variant.
    """
