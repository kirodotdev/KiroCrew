...
async def sync_source(request: web.Request) -> web.Response:
    ...
    # Validate dependencies before marking the source 'syncing' -- a 503 here
    # must leave sync_status untouched, otherwise every retry hits the 409
    # branch and the source is stranded.
    pipeline = _pipeline(request)
    if not pipeline:
        return web.json_response({"error": "pipeline not configured"}, status=503)
