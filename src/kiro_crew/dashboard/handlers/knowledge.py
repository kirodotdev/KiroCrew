...
# What a ``bedrock_kb`` row RETAINS in ``properties``: exactly the keys the
# connector reads back at every search (``kb_ids`` re-parsed, ``profile`` and
# ``region`` for the client and the one-account check). The form posts these
# three; the insert projects onto them, so a retained row never carries a
# field nothing reads.
_BEDROCK_RETAINED_PROPERTY_KEYS = ("kb_ids", "region", "profile")
    ...
    if source_type == "bedrock_kb":
        ...
        def _locked_insert():
            ...
            with target_change_lock:
                ...
                retained = {
                    key: properties[key]
                    for key in _BEDROCK_RETAINED_PROPERTY_KEYS
