"""Dashboard move endpoints for crew-to-crew work migration.

Three POST routes, one per unit kind, each returning the migration PLAN rather
than performing it: the handoff id, how many allow-listed fields would travel,
the target's blocking requirements, and any advisory findings. The transmit /
quiesce / tombstone steps run over the crew tunnel and land with that wiring;
until then the plan is what the UI shows so a user can see what a move would do.

Why these live in the gateway and not only in the CLI:

  * a SESSION bundle is Layer A plus Layer B joined via session_map.json, and is
    only coherent when taken from the live slot. The gateway holds the slot; the
    CLI does not, so the gateway is the only surface that can plan one.
  * a TASK RUN held in memory carries ``WorkingMemory`` and ``current_task``,
    neither of which ``runs.json`` persists. Planning from the live record is
    therefore higher fidelity than planning from disk.
"""

from __future__ import annotations

import logging
import time
import uuid
from typing import Any

from aiohttp import web

from kiro_crew.migration import protocol as P
from kiro_crew.migration.move_plan import plan_unit_move
from kiro_crew.security import redact_credentials, redact_exfiltration_urls

logger = logging.getLogger(__name__)


def _sel():
    from kiro_crew.sel import sel

    return sel()


async def _read_target(request: web.Request) -> tuple[str, web.Response | None]:
    """Parse and validate the ``to_crew`` body field common to all three routes."""
    try:
        body = await request.json()
    except Exception:
        body = {}
    if not isinstance(body, dict):
        body = {}
    raw_target = body.get("to_crew")
    if raw_target is not None and not isinstance(raw_target, str):
        return "", web.json_response(
            {
                "error": "to_crew must be a string containing the target crew id",
                "code": "migration_invalid_target",
            },
            status=400,
        )
    target = (raw_target or "").strip()
    if not target:
        return "", web.json_response(
            {
                "error": "to_crew is required (the target crew id/label)",
                "code": "migration_no_target",
            },
            status=400,
        )
    return target, None


def _finding_json(f: P.Finding) -> dict[str, Any]:
    """Serialize one finding for the dashboard, scrubbed.

    ``detail`` and ``detail_key`` are not always product-authored strings. A
    ledger finding names the artifact KEY an agent wrote through
    ``session_ledger_record``, and embeds that key in its sentence, so a key
    carrying a token or a credential-bearing URL would otherwise reach the
    browser verbatim. The values behind those keys are already dropped rather
    than reported -- this closes the remaining path, the key itself. Same
    treatment, and the same two redactors, as a requirement identity.
    """
    return {
        "kind": f.kind,
        "detail": _redact_plan_text(f.detail),
        "severity": f.severity,
        "detail_key": _redact_plan_text(f.detail_key),
    }


def _redact_plan_text(text: str) -> str:
    """Scrub one plan string for a consumer-facing response."""
    safe, _ = redact_exfiltration_urls(text)
    safe, _ = redact_credentials(safe)
    return safe


def _redact_requirement_identity(identity: str) -> str:
    """Make a named target requirement safe for a dashboard response.

    Most identities are harmless references (an agent name, repo path). A
    command-policy identity is the source command itself and may contain a token
    or a credential-bearing URL. The receiver still gets the original identity
    inside the bundle for its target-side check; only the consumer-facing PLAN
    is scrubbed here, at the serialization boundary.
    """
    return _redact_plan_text(identity)


def _plan_json(
    bundle: P.MigrationBundle, target: str, *, findings: list[P.Finding] | None = None, **extra: Any
) -> dict[str, Any]:
    return {
        "handoff_id": bundle.handoff_id,
        "bundle_kind": bundle.bundle_kind,
        "bundle_version": bundle.bundle_version,
        "target_crew": target,
        "ships": len(bundle.payload),
        "requirements": [
            {
                "kind": r.kind,
                "identity": _redact_requirement_identity(r.identity),
                "severity": r.severity,
            }
            for r in bundle.requirements
        ],
        "findings": [_finding_json(f) for f in (findings or [])],
        **extra,
    }


def _audit(operation: str, unit_id: str, target: str, handoff_id: str) -> None:
    try:
        _sel().log_api_access(
            caller="dashboard",
            operation=operation,
            outcome="planned",
            source="dashboard",
            resources=f"unit_id={unit_id} target={target} handoff_id={handoff_id}",
        )
    except Exception:  # pragma: no cover - audit must never break the response
        logger.debug("migration move audit failed", exc_info=True)


# ------------------------------------------------------------------ cron (2.6)


async def api_cron_move(request: web.Request) -> web.Response:
    """POST /api/crons/{job_id}/move — plan a cron job's migration."""
    from kiro_crew.migration.cron_adapter import CronMigrationAdapter

    state = request.app["state"]
    job_id = request.match_info["job_id"]
    target, err = await _read_target(request)
    if err is not None:
        return err

    job = state.crons.get_job(job_id)
    if job is None:
        return web.json_response(
            {"error": f"cron job not found: {job_id}", "code": "migration_unit_not_found"},
            status=404,
        )

    adapter = CronMigrationAdapter(job_lookup={job_id: job})
    bundle = await plan_unit_move(
        adapter,
        job_id,
        target=P.CrewRef(crew_id=target, label=target),
        source=P.CrewRef(crew_id="local", label="local"),
    )
    _audit("cron.move", job_id, target, bundle.handoff_id)
    return web.json_response({"ok": True, "plan": _plan_json(bundle, target)})


# --------------------------------------------------------------- taskrun (4.8)


async def api_taskrun_move(request: web.Request) -> web.Response:
    """POST /api/taskrunner/{task_id}/move — plan a task run's migration."""
    from kiro_crew.migration.taskrun_adapter import (
        TaskRunMigrationAdapter,
        describe_discarded_progress,
        run_fidelity_findings,
    )

    state = request.app["state"]
    task_id = request.match_info["task_id"]
    target, err = await _read_target(request)
    if err is not None:
        return err

    runner = getattr(state, "task_runner", None)
    if not runner:
        return web.json_response(
            {"error": "task runner is not available", "code": "migration_no_runner"}, status=503
        )

    # The LIVE record, which carries WorkingMemory and current_task -- state
    # runs.json does not persist. This is the fidelity advantage of planning
    # from the gateway rather than from disk.
    runs = getattr(runner, "_runs", {}) or {}
    run = runs.get(task_id)
    if run is None:
        return web.json_response(
            {"error": f"task run not found: {task_id}", "code": "migration_unit_not_found"},
            status=404,
        )

    adapter = TaskRunMigrationAdapter(run_lookup={task_id: run})
    # Refuse a run with a task in flight before describing a move: the transfer
    # needs a task boundary, so a plan for a mid-execution run would describe
    # something that cannot happen.
    try:
        await adapter.refuse_if_mid_run(task_id)
    except P.MidRunError as exc:
        return web.json_response(
            {
                "error": f"{exc} — cannot migrate a run with a task mid-execution",
                "code": "migration_mid_run",
            },
            status=409,
        )

    bundle = await plan_unit_move(
        adapter,
        task_id,
        target=P.CrewRef(crew_id=target, label=target),
        source=P.CrewRef(crew_id="local", label="local"),
    )
    desc = describe_discarded_progress(run)
    _audit("taskrun.move", task_id, target, bundle.handoff_id)
    return web.json_response(
        {
            "ok": True,
            "plan": _plan_json(
                bundle,
                target,
                findings=run_fidelity_findings(run),
                completed_kept=desc["completed_count"],
            ),
        }
    )


# --------------------------------------------------------------- session (3.8)


async def _build_session_bundle(state, slot) -> dict:
    """Serialize the live slot into a Layer A + Layer B bundle.

    Split out as a module-level seam so the plan endpoint is testable without a
    real DashboardState: the transfer builder is the one piece that needs the
    live gateway.

    The transfer builder carries the transcript and the context window, and of
    the four references ``SESSION_NONPORTABLE`` names it carries only ``agent``.
    The slot's own ``project`` / ``model`` / ``workspace`` are therefore read off
    the slot and merged in, because a plan that cannot see them reports no
    corresponding drop — and for a project-bound session, which is the common
    case, that silently understates exactly what the plan exists to disclose.
    They are inputs to the classifier, which strips them from the payload and
    reports each as a finding, so hydrating them adds nothing to what would
    travel.

    The session ledger is read here for the same reason: ``serialize`` carries it
    as durable working state and reports any host-local artifact path in it, and
    neither happens for a bundle that has no ``ledger`` key at all.
    """
    import asyncio

    from kiro_crew import session_ledger
    from kiro_crew.dashboard.session_transfer import build_transfer_bundle_async

    bundle = await build_transfer_bundle_async(state, slot, origin="local")
    for key in ("project", "model", "workspace"):
        value = getattr(slot, key, "")
        # Absent means "this session has no such reference", which is not a drop.
        # Setting it empty would invent one, so only a real value is carried.
        if value:
            bundle.setdefault(key, value)
    # A file read, so off the event loop: this handler serves the dashboard and a
    # slow disk would stall every other request behind it. The read is
    # best-effort by contract -- an absent or malformed ledger reads as empty --
    # so a session that has none simply contributes no ledger key.
    ledger = await asyncio.to_thread(session_ledger.read_state, slot.key)
    if ledger and any(ledger.get(k) for k in ("goal", "phase", "next", "tried", "artifacts")):
        bundle.setdefault("ledger", ledger)
    return bundle


async def api_session_move(request: web.Request) -> web.Response:
    """POST /api/chat/slots/{slot}/move — plan a chat session's migration."""
    # Local imports: `side` pulls in the ACP client, the config loader and
    # DashboardState, so importing the shared ownership check at module scope
    # would drag all of that into every consumer of this handler module.
    from kiro_crew.dashboard.handlers.side import _check_slot_ownership
    from kiro_crew.dashboard.session_transfer import SnapshotUnstable
    from kiro_crew.migration.session_adapter import (
        SessionMigrationAdapter,
        carry_session_ledger,
        classify_session_portability,
        layer_b_fidelity_findings,
    )

    state = request.app["state"]
    slot_key = request.match_info["slot"]
    target, err = await _read_target(request)
    if err is not None:
        return err

    slot = state.get_slot(slot_key)
    if slot is None:
        return web.json_response(
            {"error": f"session not found: {slot_key}", "code": "migration_unit_not_found"},
            status=404,
        )

    # An app token must not learn that a slot it does not own exists: without this
    # the 404-vs-200 split is itself a disclosure of session existence, and the
    # plan then carries the session's metadata. Same check the other slot-scoped
    # endpoints use, so the boundary is one implementation rather than three.
    denied = _check_slot_ownership(request, slot, "migration.session_move")
    if denied is not None:
        return denied

    try:
        raw = await _build_session_bundle(state, slot)
    except SnapshotUnstable as exc:
        # The transcript is being rewritten underneath us, so a bundle taken now
        # describes a shape the session has already left. That is transient and
        # the caller should retry, which is a 503 rather than the 500 an uncaught
        # exception produces.
        return web.json_response(
            {
                "error": f"session snapshot is unstable, retry: {exc}",
                "code": "migration_snapshot_unstable",
            },
            status=503,
        )
    portable, findings = classify_session_portability(raw)
    findings = findings + layer_b_fidelity_findings(raw)
    if raw.get("ledger"):
        carried, ledger_findings = carry_session_ledger(raw["ledger"])
        portable["ledger"] = carried
        findings = findings + ledger_findings

    # Requirements come from the adapter, not a hard-coded []. It reads the same
    # bundle we already have, so hand it back rather than rebuilding it.
    adapter = SessionMigrationAdapter(session_id=slot_key, bundle_builder=lambda _sid: raw)
    requirements = list(await adapter.requirements(slot_key))

    bundle = P.MigrationBundle(
        bundle_kind="session",
        bundle_version=2,  # matches session_transfer's Layer-B format
        handoff_id=uuid.uuid4().hex,
        created_ts=time.time(),
        source_crew=P.CrewRef(crew_id="local", label="local"),
        payload=portable,
        requirements=requirements,
    )
    _audit("session.move", slot_key, target, bundle.handoff_id)
    return web.json_response(
        {
            "ok": True,
            "plan": _plan_json(bundle, target, findings=findings),
        }
    )
