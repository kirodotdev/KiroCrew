...
async def api_ask_question_slot_answer(request: web.Request) -> web.StreamResponse:
    ...
    if (
        ...
        and not is_owner_dashboard_request(request)
    ):
    ...
    if stream:
        if slot is None:
            return web.json_response(
                {"error": "slot not found", "code": "slot_not_found"}, status=404
            )
        requested_session = request.headers.get("X-ACP-Session-Id", "")
        requested_owner = request.headers.get("X-ACP-MCP-Owner", "")
        current_owner = getattr(slot, "session_mcp_owner", "")
        if requested_session != slot_key or (current_owner and requested_owner != current_owner):
            return web.json_response(
                {
    ...
    held_by_subagents = bool(
        slot is not None
        and subagents is not None
        and subagents.running_agents_for(f"dashboard:{slot.key}")
    )
    if slot is not None and (slot._in_stage_execution or held_by_subagents):
        return web.json_response(
            {"error": "slot prompt is in progress", "code": "slot_busy"}, status=409
        )
    try:
    ...
    if stream:
        acp_mcp_servers = list(slot.session_mcp_servers)
    if slot.running:
        # Circular import: chat delivery imports the dashboard handler graph.
        ...
        # Circular imports: turn execution imports the dashboard handler graph.
