...
async def save_slot_off_loop(
    ...
    *,
    ...
    ``refuse_if_closed``: refuse an OPEN-shaped save whose transcript already
    carries a ``closed`` flag, read under the same lock the write takes. For a
    caller whose save can race a close: ``closed``/``closed_at`` are
    ``SLOT_OWNED``, so an open-shaped rebuild neither writes nor carries them
    back, and committing after the close would erase a dismissal the user already
    saw take effect. Not needed by a save that cannot interleave with a close, and
    it must stay off for a REOPEN, whose whole job is to clear the flag.

    ``expected_tab_id``: the slot GENERATION the caller authorized, refused under
    the same lock when the transcript on disk now carries a different one. A
    close-and-recreate of a channel-, cron- or workflow-born slot keeps
    ``history_key`` and republishes an OPEN line, so it defeats both guards above
    while the file now belongs to a different conversation -- without this the
    caller's window would merge into the replacement's transcript. An absent
    on-disk id (first save, legacy line) does NOT refuse.
    ...
    closed, or ``expected_tab_id`` does not match the transcript's
    generation. None of these skips raises, for either
