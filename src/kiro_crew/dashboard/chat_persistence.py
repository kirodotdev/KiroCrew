...
from kiro_crew.history import (
    ...
    ConversationLog,
    _archive_lines,
    archive_context_entries,
    carry_provenance,
    carry_unowned_metadata,
    latest_transcript_ts,
    pending_context_line_max_bytes,
    same_transcript,
    split_pending_context,
    transcript_sort_key,
    transcript_stems,
...
def _union_pending_context_for_line(
    holder_ctx: object, mine_ctx: object, history_key: str
) -> list[dict]:
    """Union both holders' queues, capped for the metadata line, surplus archived durably.

    FAIL-CLOSED, on the rotation archive's own precedence: archiving is a PRECONDITION of
    shortening the line, never a best-effort side effect. If the archive write does not
    land, the full union is returned and the line is allowed to be oversized -- an
    oversized line is recoverable, an acknowledged entry with no remaining copy is not.
    """
    kept, overflow = split_pending_context(
        holder_ctx, mine_ctx, max_bytes=pending_context_line_max_bytes()
    )
    if not overflow:
        return kept
    try:
        archived = archive_context_entries(history_key, overflow, reason="ctx-line-overflow")
    except Exception:
        archived = None
    if not archived:
        logger.warning(
            "could not archive %d over-ceiling context entr%s for %s; keeping them on the "
            "metadata line rather than dropping them",
            len(overflow),
            "y" if len(overflow) == 1 else "ies",
            history_key,
        )
        return kept + overflow
    logger.info(
        "parked %d over-ceiling context entr%s for %s in %s",
        len(overflow),
        "y" if len(overflow) == 1 else "ies",
        history_key,
        archived,
...
async def rehydrate_slot_from_history_async(
    ...
    *,
    ...
    if gone is not None:
        ...
        return None
    # OFF THE EVENT LOOP, and before the await-free build below: a critical SEL write is
    # inline, so the verdict is resolved here and passed in as a decision.
    ...
    try:
        ...
        for s in sessions:
            ...
            if gone is not None:
                ...
                continue
            # Off the event loop, before the await-free build; see the rehydrate path.
    ...
    try:
        ...
        with state.conversation_log._locked(history_key):
            ...
            if rows_only and existing_meta and not line_is_this_slots:
                ...
                _merged_ctx_line = _union_pending_context_for_line(
                    _holder_ctx, _mine_ctx, history_key
                )
                if _merged_ctx_line:
            ...
            if slot._pending_context_gen != _pending_gen_at_export:
                ...
                if _holder_ctx is not None:
                    _final_ctx = _union_pending_context_for_line(
                        _holder_ctx, _final_ctx, history_key
                    )
                if _final_ctx:
            ...
            if not _suppress_origin:
                ...
                slot._ctx_origin_ids = _committed_ids
