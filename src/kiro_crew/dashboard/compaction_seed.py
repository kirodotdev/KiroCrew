...
three words: ``written``, ``nothing`` (nothing older than the tail, so a
tail-only rotation loses no history) or ``unsupported`` (no transcript here for
this key). The coordinator asks ``supports`` first, before it projects the
rotation at all, so a channel-born session or a key with no tab never reaches
``soft`` by way of a projection the writer was never asked about.
"""
...
def build_seed(
    ...
    """The seed row's ``(content, meta)`` for *rows*, or ``None`` when there is nothing to digest.
...
async def write_seed(
    ...
    ``_seed_slot``), ``SEED_NOTHING`` when nothing is older than the tail the
    successor carries, ``SEED_WRITTEN`` once the row is in the slot. The
    transcript read runs off the event loop: a large session's chained read is
    a file parse.
    """
    ...
    seed = build_seed(rows, window_tokens=window_tokens, method=method)
    if seed is None:
        return SEED_NOTHING
    content, meta = seed
