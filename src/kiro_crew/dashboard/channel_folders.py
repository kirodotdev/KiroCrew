...
from kiro_crew.config.loader import SESSION_FOLDER_NAME_MAX, KiroCrewConfig
...
async def ensure_channel_folder(state: "DashboardState", namespace: str, name: str) -> str:
    ...
    def _create_or_adopt(folders: list[dict]) -> tuple[bool, str]:
        ...
        if existing is not None:
            # Adopting an existing folder: re-engaging it also clears a `hidden`
            # flag, matching the folder CRUD handler's own unhide-on-assign rule.
            # Nothing else changes, so an already-visible folder writes nothing.
