...
async def api_chat_slot_switch_variant(request: web.Request) -> web.Response:
    ...
    async with slot._lock:
        ...
        previous_variant_idx = target_dict.get("variant_idx")
        target_dict["content"] = chosen.get("content", "")
        slot.invalidate_source_links()
        target_dict["ts"] = chosen.get("ts", target_dict.get("ts", ""))
        target_dict["variant_idx"] = idx
        target_meta = target_dict.get("meta")
        if not isinstance(target_meta, dict):
            target_meta = {}
            target_dict["meta"] = target_meta
        chosen_meta = chosen.get("meta")
        chosen_file_changes = (
            chosen_meta.get("file_changes") if isinstance(chosen_meta, dict) else None
        )
        if isinstance(chosen_file_changes, list):
            # New variants always carry a list, including [] for a known
            # no-file answer.
            target_meta["file_changes"] = copy.deepcopy(chosen_file_changes)
        else:
            # A missing legacy snapshot is UNKNOWN. On an actual switch, remove
            # attribution owned by the other selected answer rather than
            # borrowing it for this one; a same-index selection still owns its
            # visible top-level attribution. Unrelated metadata remains intact.
            if previous_variant_idx != idx:
                target_meta.pop("file_changes", None)
        slot._dirty = True
        slot._resumed_count = 0
