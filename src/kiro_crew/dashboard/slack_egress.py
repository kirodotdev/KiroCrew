...
two refusal sites plus this chain). ``upload_destination`` is NOT on that list on
purpose: it keeps its own ``_slack_egress_permitted`` copy of the gate body and says
why at its own definition, so a reader counting consumers here does not conclude the
gate has only two spellings. The hardened chain itself,
...
will assume protection that the other proactive senders do not have.
...
the census that pins them, and the deferred inventory they sit in is recorded in
``docs/system-specs/modules/messaging.md`` rather than by this paragraph.
...
from kiro_crew.messaging.renderer import chunk_text, display_safe, send_parts_revalidating
...
#: site now audits on this leg -- ``chat_note_mirror`` files a transport-leg
#: timeout row and an error row under this operation -- and an
...
def _configured_channel_active(channel_id: str) -> bool:
    ...
    FAIL-CLOSED: any error reading the config is "cannot tell", and this boundary
    refuses those.

    Synchronous by design -- callers run it through ``asyncio.to_thread``, because
    ``KiroCrewConfig.load`` does real filesystem work: a two-``stat`` fingerprint and,
    when that misses, a full read and parse. That is the "unbounded on slow or networked
    storage" cost this module refuses to pay on the event loop, so no caller may invoke
    it directly from a coroutine.

    The re-read is the revocation check, and it is CROSS-PROCESS by construction:
    editing the config busts the stat fingerprint ``KiroCrewConfig.load`` is keyed on,
    so a channel deactivated by any writer stops authorizing on the next read.
    """
    try:
...
def _send_time_authorities(
    session_key: str, channel_id: str, *, tool_name: str, read_configured: bool, app: str = ""
    ...
    THE ACTIVATION VALUE IS THE AUTHORITY, not a counter. An earlier revision
    bracketed this read between two global change-detectors -- the in-process cache
    generation and a stat fingerprint -- and refused if either moved. Both count
    activity on the WHOLE config, so an unrelated save mid-send refused a delivery
    whose own authority never moved, abandoning a multi-part message half-sent. The
    value answers the question actually being asked, and it is cross-process on its
    own: ``KiroCrewConfig.load`` is keyed on a stat fingerprint, so a foreign
    rewrite misses the cache and is re-read from disk.

    The governance ceiling still travels as a generation because it has no
    equivalent per-send value to compare: it is sampled BEFORE the permit and
    returned with it, and the synchronous tail refuses if it moved by the time the
    send is adjacent. ``None`` means the counter could not be read at all, which the
    tail treats as a refusal rather than as unchanged -- an authority that cannot be
    ...
    """
    try:
        # Warm the profile layer first: a cold store's first publication moves the
        # composite, which would otherwise deny this very send.
        poll_profiles_fresh()
        generation: int | None = governance_answer_generation()
    except Exception:
        logger.warning("slack egress: governance generation unreadable", exc_info=True)
        generation = None
    # Re-resolved per chunk and never cached for the send: a ceiling installed between
    # two chunks must refuse the next one, which a cached permit cannot do.
    permitted = channel_egress_permitted(session_key, SLACK_NAMESPACE, tool_name=tool_name, app=app)
    configured: bool | None = None
    if read_configured:
        configured = _configured_channel_active(channel_id)
    return permitted, configured, generation


def channel_egress_permitted(
    session_key: str, channel_type: str, *, tool_name: str, app: str = ""
) -> bool:
    ...
    try:
        ...
        # Re-asked HERE so a revocation during the chunk awaits refuses the next part:
        # `channels` alone cannot see `capabilities.messaging` being lost mid-send.
        if bool(getattr(decision, "permitted", False)):
            decision = vet_and_audit(
...
async def _deliver_slack_governed(
    ...
    *,
    ...
    Other proactive Slack sends post directly instead, and adopting the chain at any
    of them widens that site's refusal set, which is a behaviour change to a surface
    this chain does not otherwise touch, so each belongs in a change where the
    tightening is the subject under review rather than a rider.

    Those sites are enumerated ONCE by symbol in
    ``docs/system-specs/modules/messaging.md``. The tier map is pinned mechanically by
    the census test, so this docstring restates no part of it.
    ...
    *relink* re-reads the binding for the post-await coordinate checks -- the
    ...
    def _coordinates_still_valid(after: str, part: int) -> bool:
        ...
        if (fresh_ts, fresh_channel) != (thread_ts, channel_id):
            audit_egress(
                ...
                session_key=session_key,
                outcome="denied",
                reason=f"link_changed_during_recheck_before_part_{part}",
            )
            logger.info(
        ...
        if is_paused():
            audit_egress(
                ...
                session_key=session_key,
                outcome="denied",
                reason=f"thread_disconnected_during_recheck_before_part_{part}",
            )
            return False
        return True
    ...
    def _authority_still_holds(basis: str, *, configured_active: bool | None) -> bool:
        ...
        roster reads are in-memory, and the last is the nonempty-owner plus DM-prefix
        check described below, which needs no lookup. The configured-channel ACTIVATION read is
        the one that touches the filesystem, so it is NOT performed here: the caller
        reads it off the loop and passes the result in as *configured_active*, and
        that VALUE is what this compares. Nothing on this path stats the filesystem
        from the event loop.
        ...
        if basis == _BASIS_PRINCIPAL:
            # The DM SHAPE is re-asserted, not re-resolved: resolving needs an await,
            # which would reopen the window this function exists to close.
            principal = _session_principal(session_key)
            return bool(principal) and is_allowed_user(principal) and channel_id.startswith("D")
        ...
        if basis == _BASIS_CONFIGURED:
            # The off-loop re-read observes a deactivation; ``None`` is an unreadable
            # config, which this boundary refuses rather than reads as active.
    ...
    async def _permitted_to_send(before: str, part: int) -> bool:
        ...
        if not permitted:
            audit_egress(
                ...
                session_key=session_key,
                outcome="denied",
                reason=f"not_permitted_mid_send_before_part_{part}",
            )
            logger.info(
        ...
        if not _coordinates_still_valid(f"the governance re-check before {before}", part):
            return False
        if not _authority_still_holds(basis, configured_active=configured_active):
            audit_egress(
                ...
                session_key=session_key,
                outcome="denied",
                reason=f"recipient_authorization_revoked_before_part_{part}",
            )
            logger.info(
            ...
            return False
        if not governance_ceiling_unchanged(permit_ceiling):
            audit_egress(
                channel_id=channel_id,
                operation=_OP_SEND,
                session_key=session_key,
                outcome="denied",
                reason=f"governance_changed_mid_send_before_part_{part}",
            )
            logger.info(
    ...
    async def _still_permitted(index: int) -> bool:
        if await _permitted_to_send(f"part {index + 1}/{len(parts)}", index + 1):
            return True
        if index:
            audit_egress(
                channel_id=channel_id,
                ...
                session_key=session_key,
                outcome="denied",
                reason=f"aborted_mid_send_before_part_{index + 1}",
            )
            logger.warning(
...
async def _slack_recipient_basis(
    ...
    if principal and parsed is not None and parsed.surface == SLACK_NAMESPACE:
        if not is_allowed_user(principal):
            _audit("denied", "session_principal")
            return ""
        # Roster membership names a PERSON, not a destination, so it authorizes only
        # this principal's OWN DM -- any other channel_id falls through below.
        principal_dm = ""
        client = getattr(state, "slack_client", None)
        if client is not None and channel_id.startswith("D"):
            try:
                principal_dm = await client.open_dm(principal)
            except Exception:
                # Unresolvable identity is "cannot tell", so it grants nothing here
                # and leaves the channel authorities to answer for this destination.
                logger.debug("slack egress: principal DM resolution failed", exc_info=True)
                principal_dm = ""
        if principal_dm and principal_dm == channel_id:
            _audit("allowed", "session_principal")
            return _BASIS_PRINCIPAL
