...
def register(app: web.Application) -> None:
    ...
    # the id route so "add" cannot be interpreted as a project id. No activate /
    # deactivate: the thin Project installs nothing, so there is no capability
    # trust step to expose (memory activation lands in a later phase).
    ...
    # Re-review: the owner's one trust step over the primary checkout's own
    # executable surfaces, which a sync can move without any review.
    app.router.add_get(
