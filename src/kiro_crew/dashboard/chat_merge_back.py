...
async def _merge_back_locked(
    ...
    # in flight too (GPT round 9, nested chains): merging child→middle while
    ...
    if parent_slot is None:
        ...
        # AUTHORIZE BEFORE REHYDRATING (GPT round 13): rehydration is not a
        ...
        # Absent/unreadable meta falls through: rehydrate returns None for
        # those and the parent_missing 404 answers identically.
        if request_app and state.conversation_log is not None:
            peek_key = slot_transcript_key(_normalize_slot_key(parent_name))
            persisted_meta = await asyncio.to_thread(state.conversation_log.get_metadata, peek_key)
            persisted_app = (persisted_meta or {}).get("app", "") or ""
            if persisted_meta and persisted_app != request_app:
                sel().log_api_access(
                    caller=request_app,
                    operation="chat.slot_merge_back",
                    outcome="denied",
                    ...
                    error="app does not own the persisted parent",
                )
                return web.json_response({"error": "not found", "code": "not_found"}, status=404)
        try:
    ...
    # App ownership check on the RESOLVED parent (GPT round 11), mirroring the
    # fork-side gate above: ``forked_from`` carries a key, and a key can be
    # re-minted onto a different session — including one owned by another app.
    # Without this, App A's merge would persist its summary into App B's
    # transcript. Same 404-not-403 shape (CWE-204: the isolation boundary must
    # not be enumerable); the true reason is recorded server-side via SEL.
    if request_app:
        parent_app = getattr(parent_slot, "_app", "") or ""
        if parent_app != request_app:
            sel().log_api_access(
                caller=request_app,
                operation="chat.slot_merge_back",
                outcome="denied",
                source="app_isolation",
                resources=f"fork={fork_session_key},parent={parent_session_key}",
                error="app does not own the resolved parent",
            )
            return web.json_response({"error": "not found", "code": "not_found"}, status=404)
    ...
    # fork's ``_merging`` flag (GPT round 6): the append-level write gate keys
    ...
    # BOTH spellings are reserved (GPT round 6): the History surface addresses
    ...
    _reserved = _colon | {transcript_stem(k) for k in _colon}
...
async def _merge_back_reserved(
    ...
    if not receipt_exists:
        ...
        try:
            ...
            pass
        try:
            saved = await save_slot_off_loop(state, parent_slot, force=True, best_effort=False)
        except Exception:
            saved = False
            logger.warning(
        ...
        if not saved:
            ...
            # prove a committed write (GPT round 8; the persistence docstring
            # requires republishing callers to check the return). Proceeding
            # would broadcast + archive against a parent that no longer exists,
        ...
        # MODEL VISIBILITY (GPT/Design round 6): the persisted block's
            ...
            parent_slot.append_pending_context(
                {
                    "content": summary_md,
                    "source": (
                        f'merged fork "{fork_title}"' if fork_title else "a merged fork session"
                    ),
                }
            )
        except Exception:
...
async def _archive_fork(
    ...
    (``close_slot``, GPT round 8) rather than a bare save + ``_slots.pop``: the
    ...
    try:
        ...
        # set only here, after that broadcast (GPT round 15). A client acting
