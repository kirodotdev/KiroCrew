...
def _resolve_pending_str_replace(pending: dict[str, Any], after: str) -> str | None:
    ...
    at most three capped strings and never the raw disk read. If the disk
    ...
    ``if_pre_write``. Exactly one match
    proves the state and yields the full-file before — ``before_if_post`` (the
    ...
    if not pending.get("completed"):
        return None
    if_post_write = pending["if_post_write"]
    if_pre_write = pending["if_pre_write"]
    if after == if_post_write and after != if_pre_write:
        return pending["before_if_post"]
    if after == if_pre_write and after != if_post_write:
        return if_post_write
    return None


def _pending_str_replace_payload(content: str, old_str: str, new_str: str) -> dict[str, str]:
    """Precompute the bounded hypotheses a deferred strReplace resolution needs.

    Every string is ``_truncate_snapshot``-capped, so the payload never retains
    the raw disk read (up to ``_MAX_RECONSTRUCT_BYTES``) for a whole turn.
    """
    return {
...
def _bind_pending_str_replace(snapshot: dict | None, tool_call_id: str, slot: "_ChatSlot") -> None:
    ...
    ``tool_call_id`` is recorded on every snapshot, so ``_flush_file_changes``
    can tell one write's repeated snapshots (the same tool call reported at
    both the tool_call and tool_call_update sites) from a second write to the
    same path.

    A pending strReplace payload additionally carries the id, because its
    ...
    already be known here and is applied at once.
    """
    if snapshot is None:
        return
    snapshot["tool_call_id"] = tool_call_id
    if not (tool_call_id and "pending_str_replace" in snapshot):
        return
    snapshot["pending_str_replace"]["tool_call_id"] = tool_call_id
    outcomes = getattr(slot, "_write_tool_outcomes", None)
    if isinstance(outcomes, dict) and tool_call_id in outcomes:
        _apply_write_outcome(snapshot, outcomes[tool_call_id])
...
def _settle_pending_str_replace_outcome(
    ...
    if isinstance(outcomes, dict):
        outcomes[tool_call_id] = completed
    ...
    for fc in fc_changes:
        ...
        if pending and pending.get("tool_call_id") == tool_call_id:
...
def _snapshot_write_target(
    ...
    if diff_old_text is not None:
        ...
        snapshot = {"path": path, "content": before}
...
def _flush_file_changes(slot: "_ChatSlot") -> None:
    ...
    disk, and settles a ``pending_str_replace`` snapshot only when every
    ...
    for fc in slot._file_changes:
        p = fc["path"]
        # An unidentified snapshot ("") counts as its own writer: it cannot be
        # attributed to the pending payload's tool call.
        writers_by_path.setdefault(p, set()).add(str(fc.get("tool_call_id") or ""))
        if p not in deduped:
            deduped[p] = {"path": p, "before": fc["content"], "after": ""}
            if fc.get("pending_str_replace"):
                pending_by_path[p] = fc["pending_str_replace"]
    ...
    try:
        ...
        async for event in event_stream:
            ...
            if event.kind == EVENT_TEXT_CHUNK:
                ...
                if _file_snapshot:
                    _bind_pending_str_replace(_file_snapshot, event.tool_call_id, slot)
                ...
                try:
                    ...
                    if _file_snapshot_upd:
                        _bind_pending_str_replace(_file_snapshot_upd, event.tool_call_id, slot)
                ...
                if _tool_terminal:
                    ...
                    _settle_pending_str_replace_outcome(
                        slot, event.tool_call_id or "", completed=_tool_status == "completed"
                    )
