...
async def _run_chat(
    ...
    *,
    ...
    def _mirror_publishes() -> bool:
        """Whether this turn may publish to the linked Slack thread RIGHT NOW.

        One predicate for every mirror publication -- text chunks, thinking, tool
        cards, the redactor flush, the seal -- so the audience fence
        (:func:`cross_surface_withheld`) cannot be honoured at one leg and missed
        at another. The renderer streams the reply live, so a fence applied only
        at the seal would still let the withheld reply out chunk by chunk. Cheap
        on the ordinary turn: the fence is a dict emptiness test unless a peer
        steered this turn. Callers still test ``_mirror is not None`` themselves
        (the type checker narrows on that, not on a closure).
        """
        return not cross_surface_withheld(state, slot)
    ...
    async def _flush_mirror_text() -> None:
        ...
        there is no mirror, nothing withheld, or the turn is fenced.
        """
        tail = _mirred.flush()
        if tail and _mirror is not None and _mirror_publishes():
            try:
    ...
    try:
        ...
        async for event in event_stream:
            ...
            if event.kind == EVENT_TEXT_CHUNK:
                ...
                # the two appends as one run (see ``_mirred``). Fenced per chunk:
                # the reply streams live, so this is where a withheld turn would
                # otherwise leak.
                if _mirror is not None and _mirror_publishes():
                ...
                # renderer's own display floor before posting. Fenced like text.
                if _mirror is not None and _mirror_publishes():
                    try:
                ...
                # renderer owns display redaction and the length bound for both
                # callers, so this leg passes the fields untouched. Fenced like
                # every other mirror publication: a tool's purpose line is the
                # peer's steer showing through in what the model chose to do next.
                if _mirror is not None and _mirror_publishes():
                    try:
        ...
        # Fenced on the SAME audience check as the channel-neutral leg below:
        # Slack is a cross-surface audience like any other, so a peer steer
        # admitted against an unlinked target must not have its reply sealed here.
        # A withheld seal is not an error -- the transcript keeps the reply and the
        # ``finally`` still stops the stream, without publishing.
        if _mirror is not None and not _retrying_empty and cross_surface_withheld(state, slot):
            logger.info(
                "withholding Slack mirror seal for %s: %d unresolved steer audience fence(s)",
                session_key,
                len(slot._steer_audience_fences),
            )
        elif _mirror is not None and not _retrying_empty:
            try:
        ...
        try:
            if _mirror is not None:
                ...
                try:
                    # Fenced for the same reason the in-progress legs are: a
                    # withheld turn must not publish its buffered text or mark its
                    # card complete (that would publish the title for the first
                    # time), yet its stream must still stop. This runs BEFORE the
                    # fence is cleared below, so it still sees the turn's records.
                    if cross_surface_withheld(state, slot):
                        await _mirror.close(publish=False)
                    else:
                        await _flush_mirror_text()
                        await _mirror.close()
                except Exception:
