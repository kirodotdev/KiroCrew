...
# regardless of request verb; typed provider recovery is host-minted provenance.
# A prefix is recognized only at an actual text boundary (line break or end of
# input); a capitalized suffix with no separator may be legitimate echoed content.
...
def _mentions_provider_budget(message: str) -> bool:
    ...
    not disable suppression. Fixed synthetic provider-recovery text never
    consults this wording gate.
    """
    return _PROVIDER_BUDGET_TOPIC_RE.search(message) is not None


def _classify_provider_budget_banner(
    text: str,
    message: str,
    *,
    ...
    ``bool`` means the matched prefix is provider metadata. Typed provider
    recovery has host-minted ownership, while ordinary transient recovery may
    legitimately repeat the original answer. An explicit capacity request also
    fails open. For an ordinary turn, grammar alone cannot prove authorship: a
    leading prefix before semantic answer text remains byte-for-byte visible.
    A banner-only tail is an artifact only after the turn already delivered
    visible model output. A banner-only turn with no such evidence is ambiguous
    and therefore stays visible, including host-authorized stage answers,
    regenerated answers, and echoes whose prompt uses wording this classifier
    has never seen.
    """
    stripped, matched = _strip_provider_budget_banner(text)
    if not matched:
        return text, False
    if is_transient_recovery:
        return text, False
    if is_provider_recovery:
        return stripped, True
    if _mentions_provider_budget(message):
        return text, False
    if prior_visible_output and not stripped:
        return stripped, True
    return text, False
...
def _flush_segment(
    ...
    *,
    ...
    if owner is not None:
        ...
        owner_was_committed = owner.committed
        ...
        if recovered and not owner_was_committed:
            ledger_sid = session_ledger_emit.session_id_of(getattr(slot, "_acp_client", None))
            session_ledger_emit.on_message_sent(
                ledger_sid,
                session_ledger_emit.live_turn(ledger_sid),
                text=recovered,
...
async def _start_next_queued_turn(state: DashboardState, slot: _ChatSlot) -> bool:
    ...
    # must not be dispatched. The session-scoped twin sees a stop issued on a linked
    # channel surface, which moves only that counter.
...
async def _run_chat(
    ...
    *,
    ...
    _recovery_provenance: RecoveryProvenance | None = None,
) -> None:
    """Stream LLM response into *slot*.  Survives browser disconnect."""

    # The stage controller reads one current-turn predicate. Reset it before
    # any return path so a failed runner entry cannot reuse the prior turn.
    slot._last_turn_stage_answer = False
    ...
    # Queue drain supplies the persisted current row; the stage controller may
    # supply the same typed ownership directly for its bounded second turn. The
    # continuation text is deliberately shared with the ordinary post-token
    # transient ladder, so text equality cannot confer provider-banner stripping,
    # selector ownership, or provider-specific queue policy.
    _provider_budget_recovery_owned = bool(
        _synthetic_payload
        and (
            _recovery_provenance is RecoveryProvenance.PROVIDER_BUDGET_ARTIFACT
            or (
                _current_replay_message is not None
    ...
    def _steer_segment_cut() -> None:
        ...
        if recovery_owner is not None:
            # A selector recovery owns a row that predates this turn. Its
            # buffered pre-steer output belongs above the steer user row, so
            # settle it now, flush its accumulated attribution, and retire that
            # ownership before the handler persists the steer. Subsequent model
            # output is ordinary post-steer output and must not target the older
            # selector or inherit its file changes.
            recovered = _flush_segment(
                state,
                slot,
                assistant_text,
                broadcast=False,
            ...
            _clear_pending_variant_recovery(slot)
            _produced_visible_output = _produced_visible_output or bool(recovered)
    ...
    def _queue_recovery(
        ...
        *,
        ...
        _recovery_qid = slot.queue_insert(
            ...
            # recovery, no kind that names a producer. `queue_meta` carries
            # containment context, caller metadata, and recovery provenance;
            # stamp the trusted actor last so metadata cannot replace it.
            meta={**queue_meta, TURN_ACTOR_META_KEY: _ledger_actor},
    ...
    try:
        ...
        # instruction rather than replaying completed tools. The exact grammar is
        # eligible in ordinary interactive chat regardless of file activity; an
        # explicit capacity topic fails open so a requested byte-identical answer
        # remains visible. A structurally owned ordinary transient continuation is
        # not normal chat: it may legitimately answer the original capacity request
        # with identical text, so only provider-budget ownership may strip it.
        # Typed provider recovery is the only host-minted banner-only artifact
        # ownership; an ordinary transient continuation may legitimately repeat
        # the original capacity answer.
        raw_assistant_text = assistant_text
        assistant_text, _provider_budget_banner = _classify_provider_budget_banner(
            assistant_text,
            message,
        ...
        if _provider_budget_banner:
            ...
            if assistant_text:
                ...
                boundary = min(max(0, _turn_msg_boundary), len(slot.messages))
                _regeneration_variant_target = next(
                    (
                        candidate
                        for candidate in reversed(slot.messages[boundary:])
                        if candidate.get("role") == "assistant"
                        and _active_variant(candidate) is not None
                    ),
                    None,
                )
                if _regeneration_variant_target is None:
        ...
        # Variant-recovery settlement owns the tail when a provider-budget banner
        # recovery is pending; only fall back to the ordinary interrupted
        # partial-reply persist (which also records the interrupted body to the
        # ledger) when no owner claimed it.
        if not _settle_pending_variant_recovery(
            state,
            slot,
            assistant_text,
            purge_chunks=True,
        ...
        if _retry_eligible:
            ...
            # resuming token); otherwise nothing resumes — Stop is active, the
            # stop-generation moved during the backoff, or this is a nested turn —
            # so the row is terminal and says so, with the give-up token so the
            # dashboard keeps it on the ErrorCard (whose Continue affordance is the
            # way forward) instead of a soft notice reading "resuming…" forever.
            _will_recover = (
            ...
            if _will_recover:
                ...
                # this entry. The monotonic generation additionally detects a Stop
                # that resolved back to idle before this check.
                _posttoken_took_over = bool(
                    _has_user_queued_followup(slot) or getattr(slot, "_pending_steers", None)
                )
                _stop_generation_changed = (
                    getattr(slot, "_stop_generation", _stop_gen_turn_start) != _stop_gen_turn_start
                )
                if (
                    _should_suppress_requeue(slot)
                    or _stop_pressed()
                    or _stop_generation_changed
                    or _posttoken_took_over
                ):
        ...
        if _ledger_terminal is not None:
            ...
            _ledger_turn_open = False
        # Publish the stage predicate before teardown awaits. Only a landed,
        # non-synthetic end_turn with normalized model text can complete a stage;
        # visible refusal and permission-denial prose remains non-completing.
        slot._last_turn_stage_answer = bool(
            _turn_landed
            and _turn_had_semantic_answer
            and _stop_reason == STOP_REASON_END_TURN
            and not _terminal_synthetic
            and not (_turn_permission_denied or bool(_refusal_reasons))
        )
