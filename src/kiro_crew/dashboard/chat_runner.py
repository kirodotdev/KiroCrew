    ...
    if slot._pending_reset_history_key:
        ...
        if subagents_attached(state, slot, pending_key, "consume_pending_reset"):
            ...
            try:
                ...
                # A lifetime lease is held for the member's whole life, so refusing on the
                # lease alone would defer this user's project change forever.
                # HELD ACROSS THE AWAIT: the settle's own region closed before this call, so a
                # rebind landing here would reach a session no arm is owed to.
                with settling_key(slot):
                    reset_ok = await state.sessions.reset(
                        pending_key, skip_if_busy=True, refuse_only_on_active_turn=True
                    )
