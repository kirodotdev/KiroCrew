    ...
    *,
    ...
    def _delivery_failed(
        *,
        ...
        if overflow_scope == "global":
            producer = (
                f"parent `{overflow_parent}` / owner `{overflow_owner}`"
                if overflow_parent and overflow_owner
                else "an unknown producer"
            )
            message = (
                f"⚠️ Stage {stage_num} finished its background work, but the global "
                f"report-overflow state from {producer} is blocking every stage "
                "boundary. Auto-run paused before the next stage; discard or Cancel "
                "the paused plan(s) to clear it, then send Go to resume."
            )
        else:
            message = (
                f"⚠️ Stage {stage_num} finished its background work, but its "
                "completion event could not be processed. Auto-run paused before "
                "the next stage. Resolve the session error, then send Go to resume."
            )
        _halt_plan(
            state,
            slot,
    ...
    while not _orchestration_stopped(
        ...
        try:
            ...
            return _delivery_failed(
                overflow_scope=exc.overflow_scope,
                overflow_parent=exc.overflow_parent,
                overflow_owner=exc.overflow_owner,
            )
        except Exception:
            logger.exception(
                "Stage %d: terminal-report handoff failed for slot %s",
