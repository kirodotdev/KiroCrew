...
class SlotBufferCoordinator:
    ...
    def flush_deferred_notes(
        ...
        ``retain_context_owing``: for a caller whose slot is about to be POPPED. The
        visible row is still committed — for a session never reopened it is the only
        copy — but its context half is NOT promoted into ``slot._pending_context``,
        which is never serialized and dies with the discarded frame. A context-only
        residual goes into the durable hold under a FRESH id instead, so the delivered
        row retires the original entry while the residual survives for a later
        ``adopt_closed`` rehydration (an ordinary restore skips a closed session). A
        residual is itself RETAINED by a later retaining flush, so the close-shaped
        flush sites one close runs through cannot drain it back into the dying frame.
        """
