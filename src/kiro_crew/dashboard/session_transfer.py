    ...
    *source* is the optional provenance record of :func:`build_source_record`.
    ...
    for m in all_messages:
        ...
        if role == SECTION_MARKER_ROLE:
            ...
            # Its OWN instant, not the row ``ts``: one seam flushes every held marker,
            # so the row stamps collide and only this separates two breaks in a turn.
            marker_at = validation.section_marker_instant(
                marker_meta.get("at") if isinstance(marker_meta, dict) else None
            )
        ...
        section_markers.append(
            {
                ...
                # A peer bundle is untrusted, so the same shape gate the local restore uses.
                "requested_at": validation.section_marker_instant(sm.get("requested_at")),
            }
        )
    ...
    try:
        ...
        def _emit_markers(index: int) -> None:
            for sm in markers_at.get(index, ()):
                ...
                if sm.get("requested_at"):
                    marker_meta["at"] = sm["requested_at"]
                new_slot.append(
                    SECTION_MARKER_ROLE,
                    sm["content"],
                    "",
