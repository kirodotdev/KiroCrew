...
def _read_layer_b(sm_key: str) -> dict[str, Any] | None:
...
def _materialise_layer_b(sm_key: str, layer_b: dict[str, Any], agent: str) -> bool:
    """Write imported Layer B to disk and join it to the slot. **Blocking IO.**

    Allocates a fresh sid, rewrites the envelope, re-redacts the events (ingress;
    the sender is not trusted), writes ``<sid>.{json,jsonl}`` into
    ``kiro_sessions_dir()``, and points ``session_map[sm_key]`` at it — the join
    the resume path reads to issue ``session/load``. Establishing that entry
    ALSO auto-disables the lossy history-prefix fallback, which only fires when
    no resumable sid exists.

    Best-effort: returns ``False`` on any failure so the caller keeps the
    transcript-only import (Layer A) rather than failing the whole transfer.
    """
    try:
        ...
        SessionMap().set(sm_key, new_sid, provider="acp", cwd="")
        return True
    except Exception:
        logger.warning(
            "session_transfer: Layer B materialisation failed for %s; "
...
def _read_and_assemble(
    ...
    if not history:
        history = list(tail)
    layer_b = _read_layer_b(sm_key) if sm_key else None
...
async def api_chat_slot_import(request: web.Request) -> web.Response:
    ...
    try:
        ...
        # Layer B: if the bundle carried the kiro-cli context, materialise it and
        # join it to this slot so the FIRST turn resumes via ``session/load`` with
        # full fidelity instead of the lossy transcript prefix. Establishing the
        # session_map entry is also what auto-disables that prefix path. Runs in a
        # thread (file writes) and is best-effort BY DESIGN: on any failure the
        # session still opens as the transcript-only copy (Layer A / v1
        # behaviour), so a Layer B problem never fails an already-persisted
        # import.
        layer_b = bundle.get("layer_b")
        if layer_b:
            sm_key = effective_session_key(new_slot)
            resumable = await asyncio.to_thread(
                _materialise_layer_b, sm_key, layer_b, new_slot.agent
            )
            if not resumable:
                logger.info(
