...
def _is_separator_fold(candidate: str, stem: str) -> bool:
    ...
    ``history._safe_key`` substitutes each non-``[\\w\\-.]`` character with ``_``, so it is
    MANY-TO-ONE: ``slack:C123:<ts>``, ``slack:C123_<ts>`` and ``slack/C123:<ts>`` all share
    the stem ``slack_C123_<ts>``. A bare ``fold(candidate) == stem`` test therefore adopts a
    foreign session, and so does asking only whether each folded position held SOME
    non-underscore character -- that refuses an impostor smuggling a literal ``_`` while
    admitting one that SUBSTITUTES a different separator, which folds identically.

    So the rule is positive, not exclusionary: every folded position must have carried the LIVE
    separator, a colon. Asking instead for any character the fold consumes is the wider rule, and
    it is unsafe: ``_safe_key`` substitutes everything outside ``[\\w\\-.]``, so ``slack/C123:<ts>``
    folds to the same stem as ``slack:C123:<ts>`` and the wider rule adopts it -- the foreign-session
    hole this gate exists to close. ``messaging.link``'s ``assert_colon_free`` constrains only ``:``
    inside a segment, so no narrower grammar is available to lean on.

    WHAT THE RESTRICTION COSTS, stated because it is a real refusal and not a theoretical one: a
    conversation id spelled with any other folded character does not resolve here and its slot
    hydrates unbound. Measured shapes: ``+`` in a WhatsApp E.164 id, ``@`` in an iMessage handle or
    a Teams thread id, and a literal ``_`` in a Discord, Webex or WeCom id. The refusal is safe in
    the sense that matters -- entries are held, not dropped, because ``merge_pending_context`` keeps
    the on-disk rows unconditionally -- but that conversation's channel replies stop until its key
    is re-linked. Closing this needs the live spelling persisted beside the stem, which is a state
    format change rather than a predicate tweak.

    A literal ``_`` remains refused WITHOUT a separate clause, because ``_`` is inside the kept
    class and so is not a character the fold consumes: it is byte-identical to the impostor shape
    (``slack:C123_<ts>`` against the transcript of ``slack:C123:<ts>``), which nothing here can
    tell apart from a genuine key spelled that way.
...
def persisted_binding_is_adoptable(
    ...
    recoverable degradation, adopting a foreign key is not. Measured, not assumed:
    admitting the literal case also admitted ``slack:C123_<ts>`` for the transcript of
    ``slack:C123:<ts>``, two distinct live sessions sharing one stem, which is what
    ``test_the_gate_refuses_a_candidate_that_smuggles_a_literal_underscore`` holds.
...
async def preaudit_persisted_binding(meta: object, transcript_key: str) -> bool | None:
    ...
    """
    candidate = str((meta or {}).get("linked_session_key") or "") if isinstance(meta, dict) else ""
    if not candidate:
        return None
    adoptable = persisted_binding_is_adoptable(candidate, transcript_key)
    landed = await asyncio.to_thread(
        audit_persisted_binding, transcript_key, candidate, adopted=adoptable
    )
    return adoptable if landed else False
...
def context_owned_by_previous_binding(slot: _ChatSlot, entry: object) -> bool:
    """True when the drain will WITHHOLD *entry* as belonging to a previous binding.

    Two mechanisms decide whether a seated pending-context entry is this binding's to
    deliver, and they answer differently. ``_note_authorized_elsewhere`` reads a
    ``noteSession`` stamp, which only ``/note`` writes -- a durable ``/context`` entry
    carries none, so that predicate calls it ours. The drain instead judges by ORIGIN
    TRANSCRIPT: after a rebind it withholds everything whose durable copy lives in
    ``_ctx_persisted_key``. Any caller that consults only the stamp therefore treats an
    entry as live that the drain is about to withhold, and a keyed repost matching one
    answers 200 while the new binding receives nothing.

    This is the drain's own rule, shared rather than restated, so the two cannot drift.
    """
    origin = str(getattr(slot, "_ctx_persisted_key", "") or "")
    if not origin:
        return False
    live = slot_history_key(slot)
    if not isinstance(live, str) or not live or same_transcript(origin, live):
        return False
    origin_ids = getattr(slot, "_ctx_origin_ids", None)
    if not isinstance(origin_ids, set):
        origin_ids = set()
    ctx_id = entry.get("ctxId") if isinstance(entry, dict) else None
    # A ctxId-less entry counts as the origin's: the drain cannot tell it apart from one
    # it hydrated, so it withholds it, and matching it here would deliver nothing.
    return not isinstance(ctx_id, str) or ctx_id in origin_ids
