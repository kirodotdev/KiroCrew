...
#: Canonical here because chat_handlers imports this module and the reverse would be circular.
MAX_CONTEXT_CONTENT = 40_000

#: Source-label limits, canonical here for the same non-circular-import reason, so the HTTP
#: boundary and the restore path judge a label by one spelling rather than two.
MAX_SOURCE_LEN = 64
...
def _finite_number(value: object) -> bool:
    ...
    would raise out of the hydrate rather than be rejected. An unconvertible magnitude is not
    a usable TTL, so it reports False.
    """
...
def context_entry_expired(entry: dict, now: float) -> bool:
    ...
    A PRESENT ``maxAge`` that is not a finite number reports EXPIRED rather than doing
    the arithmetic, which would raise ``TypeError`` on ``injectedAt + max_age``
    (``int + str``). That was reachable from every restore path: the boundary validators
    guard only the LIVE enqueue, so an entry rehydrated from an operator-editable
    metadata line never passes through them.

    Reporting EXPIRED rather than "never expires" is the deliberate direction, because
    the callers that already drop expired entries then prune it, whereas immortality
    would have it re-persisted on every save.
    """
