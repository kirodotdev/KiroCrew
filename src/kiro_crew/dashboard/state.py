        ...
        try:
            ...
            if _mslug is not None:
                ...
                if _loop is not None:
                    _loop.run_in_executor(None, _emit_message).add_done_callback(_report)
                else:
                    _emit_message()
        except Exception:
        ...
        try:
            ...
            if current != prev:
                ...
                for _sk, _member in current.items():
                    if _sk not in prev and _sk not in _inflight:
                        _emits.append((_sk, _member, SLOT_OPENED, {"slot_key": _sk}))
                for _sk, _member in prev.items():
                    if _sk not in current and _sk not in _inflight:
                        _emits.append(
