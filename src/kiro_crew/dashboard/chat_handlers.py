...
async def api_chat_slots_cleanup(request: web.Request) -> web.Response:
    ...
    for name in stale_keys:
        ...
        # No begin_close(): `_closing` is a DEPTH and the guard above holds an unreleased
        # acquire on this object, so a second one outlives the restore arm's one release.
        _turn_killed = False
...
def _retract_undurable_note_row(
    ...
    Both failure arms of ``save_slot_off_loop`` leave the transcript untouched — a refusal
    returns False before writing, an exception leaves the atomic rename undone — so the row is
    unpersisted but still in the live window, where the periodic ``flush_slot_now`` would make
    it durable anyway, turning the 503's own re-post instruction into a duplicate row.
...
async def api_chat_slot_note(request: web.Request) -> web.Response:
    ...
    DURABLE: persisted into the slot's own metadata line before the 200, replayed by both
    slot-restore paths after a restart, and retired by the save that commits the delivered rows.
    A caller never needs to re-post after a restart; the one retry signal is a 503
    ``deferred_note_persist_failed``, meaning the hold could not be made durable and was not
    accepted. Delivery stays CONDITIONAL even so: a held note whose authorizing session does not
    match the slot's live one is dropped at the rebind seam and its id recorded.
    ...
    if visible_only:
        # Row alone, so NO entry is built: nothing to queue, nothing for the flush to
        # promote, and `pending` unchanged. The cap check is skipped with it -- an
        # unbuilt entry cannot occupy a bucket, so the cap could only report on a
        # write that is not happening.
    ...
    if deferred:
        ...
        if visible_only and (slot.is_restricted or not state.conversation_log):
            # ``_save_slot_to_history`` returns True WITHOUT writing for a restricted slot
            # or a log-less state, so a 200 would promise durability nothing delivered and
            # the client's rule would close the tab over a row that never reached disk,
            # with no later run to self-correct from. Refused before the append: no row.
            return web.json_response(
                {
                    "error": (
                        "a visible-only note must be durable before it is acknowledged, "
                        "and this slot never persists its transcript"
                    ),
                    "code": "note_not_persistable",
                },
                status=409,
            )
    ...
    def _note_audit(outcome: str, *, source: str = "app_kit", error: str = "") -> None:
        # ONE recorder for every exit: `ok` was written HERE, before the pin resolved.
        sel().log_api_access(
            caller=request_app or request.get("user", "dashboard"),
            operation="note_post",
            outcome=outcome,
    ...
    if not deferred and visible_only:
        ...
        pinned_history_key = slot_history_key(slot)
        pinned_session_key = effective_session_key(slot)
        try:
            pinned = await save_slot_off_loop(
                ...
                closed=False,
                best_effort=False,
                expected_history_key=pinned_history_key,
        ...
        if pinned and (
            ...
            or slot_history_key(slot) != pinned_history_key
            or effective_session_key(slot) != pinned_session_key
        ):
            # Authorized against this generation, so the bytes are fine, but delivering here
            # surfaces them in another slot -- or on a REBIND in another session, which the
            # IDENTITY test cannot see because the object is unchanged, so the live doors
            # (`_pending`, `_on_message`) would hand an old session's note to the new one.
            # NOT the refusal below: the bytes ARE on disk, so a retraction cannot unwrite
            # them and "re-post if still open" would append a SECOND copy on reopen.
            logger.warning("Slot %s: note committed but slot replaced/rebound", name)
            _note_audit("committed_undelivered", error="slot replaced or rebound before delivery")
            return web.json_response(
                {
                    "error": (
                        "the note's row was committed but its slot was replaced before the "
                        "row could be shown; it is on disk, so do not close the slot on "
                        "this response and do NOT re-post the note"
                    ),
                    "code": "note_committed_undelivered",
                },
                status=409,
            )
        ...
        if not pinned:
            if pin_refused:
                ...
                _note_audit("error", error="the note's durable write raised")
