...
async def _cancel_terminal_stop_task(task_at_stop: asyncio.Task[Any] | None) -> None:
    """Settle a captured runner after a terminal outcome with a live task reference.

    The early return on an absent, current, or done reference is the whole
    trigger: the cancel below fires only on a live captured task after the
    provider reports a terminal outcome.
    """
...
async def api_chat_slot_interrupt(request: web.Request) -> web.Response:
    ...
    if outcome == "idle":
        ...
        # stranded. Dispatch it here. Only when no successor turn has claimed
        # the slot: the cancelled loop's exit clears slot.task
        # (chat_orchestrator._stage_loop), so "no successor" is a cleared
        # slot.task or the captured task still in place. Any other live task
        # owns the queue handoff, and dispatching again would double-fire it.
        if slot.task is None or slot.task is task_at_stop:
