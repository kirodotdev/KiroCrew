...
async def api_chat_slot_resume(request: web.Request) -> web.Response:
    ...
    if meta.get("pending_context"):
        ...
        # retire this copy; see the note at the rehydrate site. This meta came from
        # ``get_metadata(history_key)``.
...
def _enqueue_pending_context(
    ...
    ``max_age``), else None on success. The entry is consumed on the next
    ...
    """
    entry, err = _build_pending_context_entry(slot, content, source, max_age, ephemeral)
    if err is not None:
        return err
...
async def api_chat_slot_context(request: web.Request) -> web.Response:
    ...
    Body::

        {
            ...
            "ephemeral": true,         // optional; memory-only, never persisted
    ...
    # A 200 alone does NOT mean the entry survives a crash: it sits in memory until a
    # flush, so a caller recording "delivered" can suppress its own retry for lost content.
    _durable = False
    if not (body.get("ephemeral") is True):
        try:
            # Nothing is refused when this fails: the entry stays queued and the slot stays
            # dirty for the periodic retry. Only the right to REMEMBER delivery depends on it.
            _durable = bool(await save_slot_off_loop(state, slot, force=True))
        except Exception:
            logger.debug("context durability save failed for %s", name, exc_info=True)
            _durable = False
    ...
    return web.json_response(
        {"ok": True, "pending": len(slot._pending_context), "durable": _durable}
    )
