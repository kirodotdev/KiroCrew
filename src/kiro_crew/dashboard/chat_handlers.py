...
async def _apply_source_link_unlink(request: web.Request) -> web.Response:
    ...
    def _publish(phase: str, *, abort: bool = False) -> None:
        nonlocal publication_needed
        # Delivery is not the commit point. After the initial reversible push,
        # no broadcast failure may interrupt persist/compensate/reconcile or
        # change a committed 200 into a 500 that the client rolls back.
        try:
            state.push_slots_update()
            publication_needed = False
        except Exception:
