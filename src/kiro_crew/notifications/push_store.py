...
class PushSubscriptionStore:
            ...
            if path.exists():
                ...
                if isinstance(raw, dict):
                    self._subs = {
                        ...
                        and self._endpoint_is_safe(entry["endpoint"])
                    }
        except Exception:
    ...
    def _endpoint_is_safe(endpoint: str) -> bool:
        """Re-run the SSRF gate on a PERSISTED endpoint; drop it if it fails.

        Defense-in-depth: the only in-band writer (``add``) already validates via
        ``_reject_internal_endpoint``, and the store file is bind-masked HIDDEN in
        every sandbox mode, so an in-sandbox agent cannot write it. But a file
        tampered by some other path must never feed a loopback/private endpoint
        into the fan-out, which POSTs to it — so we validate on load too and drop
        anything that no longer passes (or no longer resolves).
        """
        try:
            _reject_internal_endpoint(endpoint)
            return True
        except PushSubscriptionError:
            logger.warning("Dropping persisted push endpoint that fails the SSRF gate")
            return False
