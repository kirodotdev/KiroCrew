...
def _encrypt(
    ...
    *,
    ...
    ``salt`` and ``server_private`` are injectable ONLY so a test can pin the
    output against the RFC 8291 Appendix A vector byte-for-byte; in production
    both are freshly random per message (a fixed salt/key would be a crypto
    defect), which is the default when they are omitted.
    """
    ...
    # aes128gcm "rs" (record size) header: the maximum record length the
    # receiver should expect, NOT the actual ciphertext length. RFC 8188/8291
    # use 4096; our payloads are truncated well under that so the whole message
    # is one record. Writing the actual length here (a smaller value) is a
    # framing defect some push services reject — the RFC 8291 §A vector pins it
    # to 4096.
    record_size = 4096
...
async def send_web_push(
    ...
    try:
        ...
        # allow_redirects=False: the endpoint was SSRF-validated at subscribe
        # time (push_store._reject_internal_endpoint), but a public endpoint that
        # answers 3xx to an internal address would otherwise let aiohttp follow
        # the hop and turn this POST into an SSRF. A real push service never
        # redirects, so a 3xx is treated as a non-delivery, not followed.
        async with session.post(
            ...
            # (RFC 8030): prune it. 201/200 are success; anything else — a 3xx
            # redirect (never followed, see above), or 413 for a payload over the
            # 4 KB ceiling (guarded upstream by truncating the note in
            # _web_push_payload) — is a transient failure we neither retry here
            # nor treat as gone.
