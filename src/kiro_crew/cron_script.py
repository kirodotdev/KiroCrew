...
def run_command_sandboxed(
    ...
    try:
        ...
        if env_allowlist is not None:
            # ALLOWLIST mode (GPT review, exit gates): the deny-set strips
            # known secret names, but a user's custom `.env` secret has an
            # arbitrary name the deny-set cannot know. Callers whose commands
            # need no gateway environment (a gate runs tests / curls a health
            # check) pass the minimal set they need; everything else —
            # including unknown custom secrets — is withheld.
            clean_env = {k: v for k, v in clean_env.items() if k in env_allowlist}
        proc = subprocess.Popen(
            sandboxed_argv, stdout=subprocess.PIPE, stderr=subprocess.PIPE,
            text=True, env=clean_env, start_new_session=True,
