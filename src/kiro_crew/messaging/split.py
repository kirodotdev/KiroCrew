...
def _made_collapse_clean(text: str, redactor: Callable[[str], str]) -> str:
    ...
    Only when that still collapses into a key does the whitespace itself go -- the
    case of a key no span of the literal text names and no markup reading finds,
    where the alternative is handing the caller a promise that does not hold.
    """
