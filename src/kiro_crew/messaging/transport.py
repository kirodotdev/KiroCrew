...
class TransportCapabilities:
    ...
    * ``supports_session_resume`` — gates whether connecting a channel from the
      dashboard claims INBOUND ownership (``accepts_inbound``). Only a transport
      whose inbound path actually resolves the mirror binding may declare it:
      Discord's dispatcher looks the conversation up (``resumed_session``), while
      Telegram and the rest derive a session key from the route alone and never
      ...
      session instead — which is exactly what this flag exists to prevent.
      Slack is not covered here at all: it routes inbound through its own
      ``_thread_to_session`` index and never sets ``accepts_inbound``.
    ...
    * ``files_inbound`` / ``files_outbound`` — split because one boolean was
    ...
    # inbound-routing policy. Default False: a transport that forgets to declare it
    # gets an outbound-only binding, which is merely less capable — declaring it
    # wrongly would promise a two-way link that silently drops replies.
