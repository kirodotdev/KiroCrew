...
from kiro_crew.uploads import create_upload_file, markdown_image_dest, safe_upload_name
...
def image_row_markdown(path: str) -> str:
    ...
    prompt. The destination goes through the composer's own spelling rule
    (:func:`kiro_crew.uploads.markdown_image_dest`) so a Windows data home or one
    with a space in it renders on the dashboard exactly like a pasted picture.
    Alt text is the constant ``image`` on purpose -- a sender-supplied name
    inside a markdown destination is one more place untrusted text would be
    interpreted, and the filename already sits in the path.
    """
    return f"![image]({markdown_image_dest(path)})"
