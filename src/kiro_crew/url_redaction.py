...
This module exists so that reduction has ONE implementation to converge on. It was
written four times instead: in :mod:`kiro_crew.embeddings`, in papyrus's Tectonic
download, in pptx-maker's engine download, and once more for the whisper catalog.
The copies then disagreed — two of them kept the PATH and two dropped it — and a
mirror that tokenises its path (``/artifactory/api/npm/tok-9f3b2c/…``) had its
token logged by the copies that kept it. Host-only is the surviving policy, and it
lives here.

**Two of the four copies are still in place**, at
``apps/builtins/papyrus/backend/tectonic.py`` and
``apps/builtins/pptx_maker/backend/engine_source.py``. Migrating them is a
mechanical import swap, but it is a separate change: each is a live download path
with its own tests, and folding them in here would put three unrelated features in
one reviewable diff. So read this as the home for that policy, not as proof the
duplication is already gone.
