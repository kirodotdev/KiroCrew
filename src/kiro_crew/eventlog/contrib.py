...
class ExternalProjectionStore:
    ...
    def _flush(self, kind: str, id_: str, rows: dict[str, ExternalRow]) -> None:
        """Caller holds the unit lock. Never raises."""
        ...
        try:
            ...
            # The in-memory row already won; losing the durable copy costs a
            # re-publish after a restart, not correctness now.
            logger.warning("contrib projections could not be persisted to %s", path, exc_info=True)
