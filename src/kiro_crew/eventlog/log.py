...
class MemberLog:
    ...
    def __init__(self, slug: str) -> None:
        ...
        self._ledger: CrewLog | None = None
        self._loaded = False
    ...
    def _ensure_loaded(self) -> None:
        if self._loaded:
            return
        try:
            ledger = CrewLog.open(KIND_MEMBER, self.slug)
        except CrewLogError as exc:
    ...
    def append(self, type: str, data: dict) -> Event:
        ...
        if self.committed_bytes > MAX_UNIT_LOG_BYTES:
            raise UnitLogFull(self.path, self.committed_bytes, MAX_UNIT_LOG_BYTES)
