...
# The member's legacy pre-log files have been folded in and must never be read
# again. data: {} -- its presence is the whole content.
#
# Appended LAST by the migration, after every legacy record has landed, so its
# presence proves they did. It is what makes the legacy files single-use: the
# pre-log files live outside the fence and are agent-writable, so without a
# durable marker every restart re-imports whatever they hold and a row planted
# after the migration becomes authoritative history. Membership in
# ``ALL_EVENT_TYPES`` below is load-bearing for that: it is what stops a
# contributor appending this type itself and silencing the migration.
MEMBER_MIGRATED = "member/migrated"
