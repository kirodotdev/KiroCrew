...
def _manifest(alias: str | None = None, output: str | None = None, url: bool = False) -> None:
    ...
    if not slack_manifest.valid_alias(alias):
        print(
            ...
            f"at most {slack_manifest.ALIAS_MAX} characters (it names the "
            f"/{slack_manifest.SLASH_COMMAND_PREFIX}<alias> slash command).",
            file=sys.stderr,
        )
        sys.exit(1)
...
def _setup_slash_command(*, fresh_install: bool = False) -> None:
    ...
    *fresh_install* is "no Slack tokens were stored before this run": only then
    ...
    if configured:
        ...
        alias = _detect_alias()
        current = (
            slack_manifest.slash_command(alias) if slack_manifest.valid_alias(alias) else "kirocrew"
        )
    else:
        current = "kirocrew"
