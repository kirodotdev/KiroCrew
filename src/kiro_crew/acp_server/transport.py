    ...
    **and** ``method is None`` — the same discipline ``JsonRpcMessage.is_response_for``
    ...
    async def send_request(
        ...
        *,
        ...
        A ``None`` timeout waits until a response, transport EOF, or caller
        cancellation. Finite timeouts raise ``asyncio.TimeoutError`` when the
        peer does not answer; JSON-RPC errors raise ``AcpServerError``.
        """
    ...
    async def run(
        ...
        if on_disconnect is not None:
            try:
                ...
                logger.exception("ACP peer-disconnect cleanup failed")
    ...
    async def _dispatch(
        ...
        *,
        ...
        # Response to one of OUR outbound requests: id present, method absent.
        # A null response id cannot correlate to an outbound request, but it is
        # still a response frame rather than an Invalid Request to answer.
        if msg.method is None and id_present:
            if msg.id is None:
                logger.debug("ignoring response with null id")
                return
        ...
        if msg.method is None:
            # Neither a method nor an id: an Invalid Request per JSON-RPC 2.0.
        ...
        # Notification: method, id member absent. Explicit null is a discouraged
        # but valid request id and MUST receive a response carrying id: null.
        if not id_present:
            await on_notification(msg.method, params)
            return

        # A non-null request id MUST be a string or number.
        if msg.id is not None and _safe_id(msg.id) is None:
            await self.send_error(
                None,
                JSONRPC_INVALID_REQUEST,
    ...
    def _resolve(self, msg: JsonRpcMessage) -> None:
        ...
        if fut is None or fut.done():
            # A response for an id with no active waiter (timed out, or the peer
            # echoed something unsolicited). Dropping is correct.
