...
# Do not let an unreachable gateway keep a dead editor's ACP process alive indefinitely.
_DISCONNECT_CANCEL_TIMEOUT = 10.0
...
class _Session:
    ...
    # The prompt result waits for this backing-stop outcome before acknowledging cancellation.
...
class AcpAgentServer:
    ...
    def __init__(
        ...
        *,
        ...
        self._cancel_tasks: set[asyncio.Task[bool]] = set()
        # session/load and session/resume mutate both local state and the backing
        # slot. Reserve the id across awaits so prompts and sibling lifecycle
        # requests cannot observe or overwrite a half-applied transition.
        self._lifecycle_sessions: set[str] = set()
    ...
    async def serve(self) -> None:
        ...
        await self._transport.run(
            ...
            self._stop_in_flight_on_disconnect,
        )

    async def _stop_in_flight_on_disconnect(self) -> None:
        """Confirm backing stops before local prompt handlers are drained."""
        ...
        if not tasks:
            return
        _done, pending = await asyncio.wait(tasks, timeout=_DISCONNECT_CANCEL_TIMEOUT)
        if pending:
            logger.warning(
                "backing cancellation timed out for %d disconnected turn(s)", len(pending)
            )
            for task in pending:
                task.cancel()
    ...
    async def _selectors_free(self, session: "_Session", req_id: Any) -> bool:
        ...
        """
        if session.session_id in self._lifecycle_sessions:
            await self._transport.send_error(
                req_id,
                JSONRPC_INVALID_PARAMS,
                "a session lifecycle change is in progress",
            )
            return False
    ...
    async def _begin_session_lifecycle(self, session_id: str, req_id: Any) -> bool:
        """Reserve *session_id* for load/resume without racing an active turn."""
        if session_id in self._lifecycle_sessions:
            await self._transport.send_error(
                req_id,
                JSONRPC_INVALID_PARAMS,
                "a session lifecycle change is already in progress",
            )
            return False
        session = self._sessions.get(session_id)
        if session is not None and session.in_flight:
            ...
            return False
        if session is not None and session.selector_in_progress:
            await self._transport.send_error(
                req_id,
                JSONRPC_INVALID_PARAMS,
            ...
            return False
        self._lifecycle_sessions.add(session_id)
        return True

    async def _backing_session_cwd(self, session_id: str, fallback: str | None) -> str | None:
        """Snapshot the backing project for rollback; failure stays best-effort."""
        getter = getattr(self._backend, "get_session_cwd", None)
        if not callable(getter):
            return fallback
        try:
            value = await getter(session_id)
        except Exception:
            logger.warning("failed to snapshot session project for %s", session_id, exc_info=True)
            return fallback
        return value if isinstance(value, str) else fallback

    async def _restore_failed_lifecycle(
        self,
        session_id: str,
        prior: "_Session | None",
        prior_cwd: str | None,
        mcp_snapshot: Any = None,
    ) -> None:
        """Restore local/project/MCP state after load or resume setup fails."""
        if prior is None:
        ...
        restore = getattr(self._backend, "restore_session_cwd", None)
        if prior_cwd is not None and callable(restore):
            try:
                applied = await restore(session_id, prior_cwd)
                if applied is False:
                    logger.info(
                        "session project changed after failed lifecycle for %s; rollback skipped",
                        session_id,
                    )
            except Exception:
                rollback_failed = True
                logger.warning(
                    "failed to restore session project for %s", session_id, exc_info=True
                )

        if prior is not None:
            restore_local_mcp = getattr(self._backend, "restore_local_session_mcp", None)
            if callable(restore_local_mcp):
                try:
                    applied = await restore_local_mcp(session_id, prior.cwd, prior.mcp_servers)
                except Exception:
                    rollback_failed = True
                    logger.warning(
                        "failed to restore prior MCP config for %s", session_id, exc_info=True
                    )
                else:
                    if applied is False:
                        logger.info(
                            "session MCP owner changed after failed lifecycle for %s; "
                            "rollback skipped",
                            session_id,
                        )
                if rollback_failed:
                ...
                return
            logger.warning(
                "backend cannot conditionally restore prior MCP config for %s", session_id
            )
            return
        restore_mcp = getattr(self._backend, "restore_session_mcp", None)
        if mcp_snapshot is not None and callable(restore_mcp):
            try:
                await restore_mcp(session_id, mcp_snapshot)
            except Exception:
                logger.warning(
                    "failed to restore prior MCP registration for %s",
                    session_id,
                    exc_info=True,
                )

    async def _finish_cancellation_cleanup(
        self, cleanup: Awaitable[None], *, description: str
    ) -> None:
        """Finish lifecycle cleanup before the original cancellation propagates."""
        task: asyncio.Future[None] = asyncio.ensure_future(cleanup)
        ...
        while True:
            try:
                ...
                if task.cancelled():
                    logger.warning("%s was cancelled", description)
                    return
                ...
                logger.warning("%s failed", description, exc_info=True)
            return
    ...
    async def _on_notification(self, method: str, params: dict[str, Any]) -> None:
        if method == METHOD_CANCEL:
            ...
            if session is not None:
                session.cancelled.set()
            if self._backend is not None and session is not None and session.in_flight:
                # The prompt result waits for this task so cancellation is
                # acknowledged only after the gateway confirms its backing stop.
                if session.cancel_task is None or session.cancel_task.done():
                    task = asyncio.create_task(self._safe_cancel(session_id))
                    session.cancel_task = task
                    self._cancel_tasks.add(task)
                    task.add_done_callback(self._cancel_tasks.discard)
    ...
    async def _safe_cancel(self, session_id: str) -> bool:
        try:
    ...
    async def _handle_session_new(self, params: dict[str, Any], req_id: Any) -> None:
        ...
        try:
            hosted, _mcp_snapshot = await self._host_session_mcp(session_id, cwd, servers, req_id)
        except asyncio.CancelledError:
            self._sessions.pop(session_id, None)
            if self._backend is not None:
                await self._finish_cancellation_cleanup(
                    self._delete_backing_session(session_id),
                    description=f"backing session cleanup for {session_id}",
                )
            raise
        if not hosted:
    ...
    async def _handle_session_load(self, params: dict[str, Any], req_id: Any) -> None:
        ...
        if cwd is None:
            return
        servers = await self._parse_mcp_or_error(params, req_id)
        if servers is None or not await self._begin_session_lifecycle(session_id, req_id):
            return

        prior = self._sessions.get(session_id)
        prior_cwd = prior.cwd if prior is not None else None
        try:
            prior_cwd = await self._backing_session_cwd(session_id, prior_cwd)
            try:
            ...
            try:
                hosted, mcp_snapshot = await self._host_session_mcp(
                    session_id, cwd, servers, req_id
                )
            except asyncio.CancelledError:
                await self._finish_cancellation_cleanup(
                    self._restore_failed_lifecycle(session_id, prior, prior_cwd, mcp_snapshot=None),
                    description=f"load rollback for {session_id}",
                )
                raise
            if not hosted:
                await self._restore_failed_lifecycle(
                    session_id, prior, prior_cwd, mcp_snapshot=None
                )
                return
            ...
            try:
                ...
                for index, message in enumerate(messages):
                    ...
                    if role == "user":
                        await sink.send_user_text(safe_text, message_id=message_id)
                    elif role in ("assistant", "streaming"):
                        await sink.send_text(safe_text, message_id=message_id)
                fields = await self._selector_fields(session_id, session)
                await self._transport.send_result(req_id, fields)
            except asyncio.CancelledError:
                await self._finish_cancellation_cleanup(
                    self._restore_failed_lifecycle(
                        session_id, prior, prior_cwd, mcp_snapshot=mcp_snapshot
                    ),
                    description=f"load rollback for {session_id}",
                )
                raise
            ...
            self._lifecycle_sessions.discard(session_id)
    ...
    async def _handle_session_resume(self, params: dict[str, Any], req_id: Any) -> None:
        ...
        try:
            ...
            try:
                ...
                await self._finish_cancellation_cleanup(
                    self._restore_failed_lifecycle(session_id, prior, prior_cwd, mcp_snapshot=None),
                    description=f"resume rollback for {session_id}",
                )
                raise
            if not hosted:
                await self._restore_failed_lifecycle(
        ...
        try:
            ...
            if session.cancelled.is_set():
                ...
                if cancel_task is not None:
                    cancel_succeeded = await cancel_task
                if cancel_succeeded:
                    await self._transport.send_result(req_id, {"stopReason": STOP_REASON_CANCELLED})
                else:
                    await self._transport.send_error(
                        req_id, JSONRPC_INTERNAL_ERROR, "Failed to cancel turn"
                    )
                return
    ...
    async def _require_abs_cwd(self, params: dict[str, Any], req_id: Any) -> str | None:
        """Return a non-sensitive absolute ``cwd``, or answer ``-32602``.

        A sandbox mask cannot revoke the process's already-open current-directory
        handle, so protected paths are refused before any provider or backing slot
        is created. Path policy resolution performs filesystem I/O and stays off
        ...
        if not isinstance(cwd, str) or not cwd or not os.path.isabs(cwd):
            ...
            return None
        if await asyncio.to_thread(is_sensitive_path, cwd):
            await self._transport.send_error(
                req_id, JSONRPC_INVALID_PARAMS, "'cwd' must not be a sensitive path"
            )
            return None
        return cwd
    ...
    async def _apply_session_mcp(
        ...
        Returns ``(None, rollback_snapshot)`` on success, or a client-safe error
        with no snapshot when hosting fails. The backend's optional
        ``configure_session_mcp`` hook starts and owns each untrusted child under
        Kiro Crew's sandbox, then registers only its trusted proxy spec on the
        session slot. Provider generations spawn the relay process and reconnect
        through that capability; they never own or launch the untrusted child.
        ...
        try:
            snapshot = await configure(session_id, cwd, servers)
        except asyncio.CancelledError:
            raise
            ...
            return "Failed to start the requested MCP servers", None
    ...
    async def _host_session_mcp(
        ...
        """Host *servers* or fail the request; return success plus rollback state."""
        error, snapshot = await self._apply_session_mcp(session_id, cwd, servers)
        if error is None:
...
def _validate_prompt_blocks(blocks: Any) -> str | None:
    """Validate ``session/prompt`` content blocks before text projection."""
    if blocks is None:
    ...
    for index, block in enumerate(blocks):
        ...
        if btype == "text":
            ...
            required_strings = ("data", "mimeType")
        elif btype == "resource_link":
            required_strings = ("name", "uri")
        elif btype == "resource":
            resource = block.get("resource")
            if not isinstance(resource, dict):
                return f"prompt[{index}].resource must be an object"
            if not isinstance(resource.get("uri"), str):
                return f"prompt[{index}].resource.uri must be a string"
            if not isinstance(resource.get("text"), str) and not isinstance(
                resource.get("blob"), str
            ):
                return f"prompt[{index}].resource must have string 'text' or 'blob'"
            continue
        else:
            return f"prompt[{index}] has unsupported content block type {btype!r}"

        for field_name in required_strings:
            if not isinstance(block.get(field_name), str):
                return f"prompt[{index}].{field_name} must be a string"
    return None
