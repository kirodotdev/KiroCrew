...
from kiro_crew.security import is_sensitive_path, redact_path_segments
...
def _canonicalize(path: str) -> str:
    ...
    denied) fall back to the input. Windows drive-letter paths and POSIX-style
    paths received on Windows pass through because host-local realpath rewriting
    would change their protocol identity.
    """
    if _WIN_DRIVE_RE.match(path) or (os.name == "nt" and path.startswith("/")):
        return path
    try:
...
def _derive_line_from_edit(path: str, params: dict[str, Any]) -> int | None:
    ...
    try:
        ...
        return None
    if not stat.S_ISREG(st.st_mode) or st.st_size > _MAX_EDIT_FILE_BYTES:
        return None
    data = safe_read_prefix(path, _MAX_EDIT_FILE_BYTES + 1)
    if data is None or len(data) > _MAX_EDIT_FILE_BYTES:
        return None
...
def _first_path(params: dict[str, Any]) -> str | None:
    for key in _PATH_KEYS:
        value = params.get(key)
        if isinstance(value, str) and value and "\x00" not in value and _is_absolute(value):
...
def sanitize_tool_locations(
    raw: Any,
    redactor: Callable[[str], str],
) -> list[dict[str, Any]]:
    """Return schema-valid locations whose paths survive egress redaction intact."""
    ...
    for entry in raw:
        ...
        if not isinstance(path, str) or not path:
            continue
        if redact_path_segments(path, redactor) != path:
            continue
    ...
    if not path:
        ...
        if isinstance(files, list):
            ...
            for item in files:
                ...
                elif isinstance(item, str) and "\x00" not in item and _is_absolute(item):
