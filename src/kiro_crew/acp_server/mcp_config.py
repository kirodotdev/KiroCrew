...
# Client session servers may not shadow host-managed identities. App-owned MCP
# servers use the separate ``<app>:<server>`` namespace reserved in the parser.
_RESERVED_MANAGED_SERVER_NAMES = frozenset(name.casefold() for name in KIROCREW_BIN_MCP_SERVERS)
...
def parse_mcp_servers(raw: Any) -> list[StdioMcpServer]:
    ...
    for index, entry in enumerate(raw):
        ...
        if canonical_name in _RESERVED_MANAGED_SERVER_NAMES or ":" in name:
            raise McpConfigError(f"mcpServers[{index}]: reserved server name {name!r}")
