...
class TelegramRenderer(Renderer):
    ...
    async def _rotate_on_length(self) -> None:
        ...
        for ch in chunks[:-1]:
            # A key can be split without either message holding it, and the grade
            # the cut ran saw only the text present then: a segment ending in a
            # credential PREFIX matches nothing, so it seals, and the characters
            # completing the key arrive afterwards. The message already sent cannot
            # be recalled, so the still-unsent side gives up exactly the span that
            # completes it -- off the loop, because the grade scans.
            repaired = await asyncio.to_thread(
                repaired_after_a_sent_tail, self._sent_tail, ch, _default_redactor
            )
            ch = repaired if repaired is not None else ch
            self._buf = [ch]
