...
# digest, so the rotation may proceed. ``nothing``: nothing is older than the
# tail and replay admission included every conversation row, so a tail-only
# rotation loses no history; the coordinator recycles as ``soft`` under the same
# turn hold and tells the compact callback ``all_kept=True``. ``tail_only``: no
# digest was written, but either replay admission cut conversation rows or the
# non-empty dropped slice rendered no digest; the coordinator still recycles as
# ``soft``, with ``all_kept=False`` so its notice names the loss. ``unsupported``:
# this surface holds no transcript it can digest for the key (a channel-born
# session, a key with no tab, a tab that closed under the rotation); the walk
# hands to ``native``, which keeps the history as a summary. Only the literal
# ``nothing`` vouches that every row was carried. The two named no-seed answers
# turn the rotation into ``soft``; any other answer is read as ``unavailable``.
SEED_WRITTEN = "written"
SEED_NOTHING = "nothing"
SEED_TAIL_ONLY = "tail_only"
SEED_UNSUPPORTED = "unsupported"

# What a rotation did with the history older than the tail the successor
# carries, by method, for the notice the surface posts after a compaction: a
# rotation and a native summary reach the user through the same callback, and
# a notice that reads the same for both hides that the pre-tail history is
# gone (``soft``) or survives only as a digest (``shake``). Each value
# completes the notices' "earlier ones are ..." clause, which speaks of the
# agent's MEMORY: the transcript on screen is untouched by a rotation, and a
# bare "dropped" beside an intact transcript reads as a deletion. The digest
# is not a thing the user can open (the seed row draws nothing), so the
# ``shake`` clause says where it went, not that it is kept. ``native`` has no
# entry: it keeps that history as an in-place summary and is not a rotation.
# A ``soft`` rotation the callback reports with ``all_kept=True`` (the seed
# writer answered ``SEED_NOTHING``: the tail covers every row) dropped nothing,
# so the notice writers take their whole-conversation line and never read this
# entry for it.
ROTATION_HISTORY_FATE: dict[str, str] = {
    COMPACTION_METHOD_SOFT: "gone from its memory",
    COMPACTION_METHOD_SHAKE: "condensed into its memory",
}
...
def walk_tail(
    ...
    *,
    ...
    so a tiny budget still seeds one message. With *clip_newest* that one row
    is clipped to *budget_chars* with the truncation mark, so the tail never
    exceeds the budget a rotation is judged against; the replay asks for it
    only when a rotation method is configured, since without one no projection
    charges that figure and the newest row is carried whole, as on ``main``.
    The clip changes the line, never the cut, so the writer's split is the
    same either way.
    ...
    *line_of* renders a row; the replay passes its own to clip inject content.
    ...
    for index in range(len(rows) - 1, -1, -1):
        ...
        if total + len(line) > budget_chars and lines:
            break
        if clip_newest and not lines and len(line) > budget_chars:
    ...
    return TailWalk(lines=lines, cut=cut)
...
def split_tail(
    ...
    *,
    ...
    return TailSplit(dropped=tuple(rows[: walk.cut]), tail=tuple(rows[walk.cut :]))
