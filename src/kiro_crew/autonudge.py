...
class AutoNudgeService:
    ...
    def _load(self) -> None:
        ...
        # Windows and macOS delegated agents can reach Crew hidden leaves. Purge
        # cross-host/legacy composer plaintext before any loop is restored; a
        # failed purge aborts startup rather than bringing agents up beside it.
        autonudge_selfarm.purge_scheduled_message_records_if_unconfined()
        ...
        for raw in data.get("loops", []):
            try:
                ...
                if scheduled_at > 0 and loop.monitor is not None:
                    ...
                    # composer turn. A crash can leave the candidate snapshot
                    # durable before its protected CAS, and a failed CAS plus a
                    # failed rollback write has the same shape. Reconcile while
                    # startup is still serialized and before any timer is armed,
                    # so a losing later deadline cannot delay the authentic turn.
                    try:
                        trusted_scheduled_message = autonudge_selfarm.read_scheduled_message(
                            scheduled_message_trust_id(loop.id), loop.slot_key
                        )
                    except OSError:
                        # A transient protected-store failure says nothing about
                        # authenticity. Keep the mutable metadata row and arm its
                        # timer; the fire-time reader will return BUSY until the
                        # protected record can be read again.
                        logger.warning(
                            "AutoNudge: scheduled provenance temporarily unreadable for "
                            "loop %s; preserving the pending schedule",
                            loop.id,
                            exc_info=True,
                        )
                    if trusted_scheduled_message is not None:
                        # The loop store is agent-readable. Never rehydrate exact
                        # composer text from protected provenance into it.
                        if loop.message:
                            loop.message = ""
                            scheduled_repaired = True
                        if scheduled_at != trusted_scheduled_message.scheduled_at:
                            ...
                            loop.scheduled_at = scheduled_at
                            scheduled_repaired = True
                    trusted_completed = bool(
                        trusted_scheduled_message is not None
                        and trusted_scheduled_message.completed
                    )
    ...
    def _revoke_self_arm_for(
        ...
        loop: NudgeLoop,
        *,
        scheduled_provenance_revoked: bool = False,
    ) -> None:
        ...
        A transactional scheduled removal has already deleted and verified its
        provenance before the mutable row commits. Skipping that duplicate
        best-effort delete prevents its lock acquisition from recreating the
        provenance directory after a platform-wide purge removes it.
        """
        self._revoke_self_arm(
            loop.id,
            scheduled_provenance_revoked=scheduled_provenance_revoked,
        )
    ...
    def _revoke_self_arm(
        ...
        *,
        ...
        def _forget_all_trust() -> None:
            ...
            if not scheduled_provenance_revoked:
                autonudge_selfarm.forget_self_arm(scheduled_message_trust_id(loop_id))
            autonudge_provider_trust.forget_monitor_owner_credentials(loop_id)
    ...
    async def purge_scheduled_messages_for_confinement_change(self) -> None:
        """Remove protected schedules before a sandbox posture change is adopted.

        The epoch is invalidated by the config subscriber before this method is
        entered, so no concurrent mint or verification can succeed. Each row is
        removed through the existing lifecycle transaction; the final directory
        purge catches orphan or out-of-band records after all live rows are gone.
        """
        from kiro_crew import autonudge_selfarm

        for loop in self.list_all():
            if is_scheduled_message(loop):
                removed = await self.discard_scheduled_message(loop.id)
                if not removed and self.get_by_id(loop.id) is not None:
                    raise MonitorUpdateConflict(
                        "scheduled message is in flight during sandbox transition"
                    )
    ...
    async def remove_by_slot(self, slot_key: str) -> NudgeLoop | None:
        ...
        A scheduled message is retired like every other session automation: the
        session close that reaches here is the owner's, and a pending one-shot
        left behind would rebuild the dismissed session via closed-tab
        rehydration -- the outcome the close exists to prevent. Provenance is
        not consulted, matching :meth:`discard_scheduled_message`.
        """
        async with _maintenance_lock(self._base_dir):
            ...
            if is_structured_monitor_loop(loop):
                ...
                await self._remove_unserialized(
                    loop.id, _scheduled_transition=_ScheduledTransition.DISCARD
                )
            return loop
    ...
    async def _revoke_scheduled_provenance_before_removal(loop: NudgeLoop) -> Any | None:
        """Capture and delete replayable composer provenance before removal commits.

        A scheduled message's exact bytes live only in the keystone-gated
        provenance record; the mutable loop row is agent-writable, so a removal
        that commits while its record survives lets a re-minted row replay
        cancelled text as user-authored input. Capturing the authenticated record
        before deleting it lets a failed mutable-store commit restore the exact
        authority before the timer is rearmed.

        Fail closed like the provider-credential denial above: an unverified
        deletion raises, which aborts the removal with the row still stored, so
        success never outruns the revocation. Missing, malformed, and foreign-slot
        records remain removable; their authenticated snapshot is ``None``.
        """
        from kiro_crew import autonudge_selfarm
        ...
        def _capture_revoke_and_verify() -> Any | None:
            ...
            # Supported-host verification also authenticates the surviving
            # record. Unsupported hosts refuse reads by design; the strict
            # deletion helper above is therefore their physical-file proof.
            if (
                autonudge_selfarm.scheduled_message_provenance_supported()
                and autonudge_selfarm.read_scheduled_message(trust_id, slot_key) is not None
            ):
