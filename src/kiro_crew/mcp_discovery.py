...
async def _runtime_grant_present(mcp_url: str, name: str) -> bool | None:
    ...
    """
    try:
        # ``audit_absence``: this caller reads once and renders the answer either
        # way, so an ABSENT grant is acted on just as much as a held one -- it is
        # what turns the row into "Sign-in required". The mint's polling watcher
        # keeps the default, where only the acted-on TRUE is recorded.
        return await grant_observed(mcp_url, audit_absence=True)
    except OSError:
