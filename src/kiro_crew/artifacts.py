...
class ArtifactStore:
    ...
    def update(
        ...
        *,
        ...
        # (the store's own UTF-8), so the text token is the right one; a
        # metadata-only update returns content=None and the handler refetches.
        return self._stamp_token(art)
    ...
    def _read_meta_file(self, path: Path) -> Artifact:
        ...
        return Artifact(
            ...
            content_salt=str(raw.get("content_salt") or ""),
    ...
    def _stage_text(self, path: Path, text: str) -> _Staged:
        ...
        return self._stage_text_in(dfd, parent, dest, text)

    @staticmethod
    def _stage_text_in(dfd: int, parent: Path, dest: Path, text: str) -> _Staged:
        ...
        try:
            ...
            for _ in range(_STAGE_NAME_ATTEMPTS):
                name = f".{dest.name}.{uuid.uuid4().hex}.staged"
                try:
                    fd = os.open(
                        name,
    ...
    def _resolve_write_dest(path: Path) -> tuple[Path, Path]:
        ...
        if is_sensitive_path(str(dest)) or is_sensitive_path(str(parent)):
