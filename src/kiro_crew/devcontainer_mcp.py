    ...
    *,
    ...
    for name, subcommand in _BRIDGED_SERVERS:
        if kind == "tcp":
            ...
            if port is None:
                continue
            args: list[str] = [client, "tcp", "host.docker.internal", str(port)]
        else:
            args = [client, container_socket_path(runtime_token, subcommand)]
...
class McpBridge:
    ...
    def session_servers(self) -> list[dict[str, Any]]:
        if self._unix:
            return session_server_entries(self.runtime_token, transport="unix")
        return session_server_entries(self.runtime_token, transport="tcp", ports=self._ports)
    ...
    async def start(self) -> None:
        ...
        for _name, subcommand in _BRIDGED_SERVERS:
            ...
            if self._unix:
                self._servers.append(await _serve_unix(self._socket_path(subcommand), argv, env))
            else:
                server, port = await _serve_tcp(argv, env)
...
def _on_connect_factory(argv: list[str], env: dict[str, str]) -> Any:
...
async def _serve_unix(path: Path, argv: list[str], env: dict[str, str]) -> asyncio.AbstractServer:
    if path.exists() or path.is_symlink():
        path.unlink()

    server = await asyncio.start_unix_server(_on_connect_factory(argv, env), path=str(path))
...
async def _serve_tcp(argv: list[str], env: dict[str, str]) -> tuple[asyncio.AbstractServer, int]:
    """Listen on 127.0.0.1 with an ephemeral port. Never 0.0.0.0."""
    server = await asyncio.start_server(_on_connect_factory(argv, env), host="127.0.0.1", port=0)
