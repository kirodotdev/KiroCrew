...
class _TaskqBridgeMixin(ManagerComponent):
    ...
    async def taskq_row_is_claimed_unstarted_async(self, agent_id: str) -> bool:
        ...
        """
        # ``ADMITTED`` is declared in the state model and not re-exported by the package,
        # so it is read where it lives rather than re-spelled here.
        from kiro_crew.taskq.model import ADMITTED
        ...
        for attempt in range(_STATE_READ_ATTEMPTS):
            try:
                ...
                return False
            return rec is not None and rec.state == ADMITTED
        return False
