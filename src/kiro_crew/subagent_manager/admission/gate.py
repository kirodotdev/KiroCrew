    ...
    def spawn_impl(
        ...
        *,
        ...
        Args:
            ...
            acp_backend (str | None): Per-spawn ACP backend override (e.g. an
                edition-registered harness id). ``None`` (the default) is no
                override: the subagent runs on the configured default backend,
                not the parent chat's per-chat pick. Any string is a pin --
                ``""`` is kiro-cli's own id -- and must be in
                ...
                pin forces the dedicated-process path: the parent's
        ...
        # ``None`` is no override (the configured default backend, not the
        # parent chat's per-chat pick) and needs no check; ``""`` is a real pin
        # of kiro-cli's own id. A given value is validated against the SAME selectable set the
        ...
        if acp_backend is not None and _gate:
