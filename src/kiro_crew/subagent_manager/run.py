...
class RunEventCoordinator(ManagerComponent):
    ...
    async def _release_direct_provider_impl(self, info: SubagentInfo, *, cancel: bool) -> None:
        ...
        path (a turn may still be in flight) and on a run that ended with an
        error, the wall-clock timeout included; ``cancel=False`` on clean
    ...
    async def _teardown_run_session_impl(self, info: SubagentInfo, session_key: str) -> None:
        ...
        # the release/reset below would never touch it. A clean completion means
        # the remote task already reached its terminal state, so only the session
        # needs closing; a run that ended with an error -- the wall-clock timeout
        # above all, where the remote is still working on a task nobody will
        # read -- issues CancelTask first (best-effort, bounded).
        await self._release_direct_provider_impl(info, cancel=bool(info.error))
        try:
    ...
    async def _run_inner_impl(
        ...
        if _a2a_entry is not None:
            ...
            # Resume honesty: a resumed A2A conversation is live only if a
            # continuation carried a retained contextId AND start() succeeded
            # (the card fetched and the endpoint bound). The build step swallows
            # a failed start on a continuation precisely so this guard, not a
            # blank fresh conversation, is the outcome -- so the started state
            # must be part of the condition, or a retained contextId alone would
            # report a resume that never connected.
            _resumed = (
                bool(info.conversation_key)
                and bool(getattr(client, "context_id", None))
                and bool(getattr(client, "started", False))
            )
            is_cc = False
        ...
        if _a2a_entry is not None:
            ...
            # The transcript is what streamed (appended artifact deltas and
            # status text, live); the RECORD is the artifacts' final content. An
            # A2A artifact update with ``append`` false REPLACES that artifact,
            # and the provider hands the assembled final artifacts in the
            # completion text -- so a draft that streamed and was then replaced
            # is in the transcript but not in the result the parent reads. A
            # completion with no text (nothing produced) keeps the streamed text.
            if _complete_event is not None and _complete_event.text:
                result_text = _complete_event.text
    ...
    async def _build_a2a_provider_impl(self, info: SubagentInfo, entry: Any) -> "LLMProvider":
        ...
        response. Continuation: the contextId the continue caller resolved and
        admission stashed on the record (``_a2a_context``) is used as-is, so the
        conversation the provider joins is the one that was admitted — never a
        second read of state.json that could disagree with the first. ``start()``
        fetches the Agent Card; on failure it raises, and because a continuation
        reports ``_resumed`` off ``context_id`` AND ``started`` the run's
        ``resume_failed`` guard fires rather than silently starting a blank
        conversation.
        """
        ...
        retained_context = (
            str(getattr(info, "_a2a_context", "") or "") if info.conversation_key else ""
        )
