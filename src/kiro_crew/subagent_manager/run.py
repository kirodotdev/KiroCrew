...
class RunEventCoordinator(ManagerComponent):
    ...
    async def _build_a2a_provider_impl(self, info: SubagentInfo, entry: Any) -> "LLMProvider":
        ...
        retained_context = ""
        if info.conversation_key:
            conv_id = subagent_id_from_conversation_key(info.conversation_key)
            if conv_id:
                try:
                    # The one read of the conversation's A2A facts (shared with the
                    # continue callers), synchronous file I/O: off the loop, so a
                    # stalled filesystem cannot freeze every gateway turn.
                    record = await asyncio.to_thread(self._manager.recorded_a2a, conv_id)
                except Exception:  # pragma: no cover - defensive
                    record = None
                retained_context = record.context_id if record is not None else ""
