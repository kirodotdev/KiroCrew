...
def _pop_auto_approve_and_audit(name: str, spec: dict, *, source: str, reason: str) -> dict:
    ...
    try:
        sel().log_api_access(
            ...
            outcome="ok",
            source=source,
            resources=f"@{name} autoApprove removed ({reason}); calls go through the approval gate",  # noqa: E702 — semicolon is inside the f-string literal, not a statement separator
        )
