...
def session_gateway_servers(session_key: str, *, agent: str = "") -> list[dict[str, Any]]:
    ...
    if not session_key:
        return []
    from kiro_crew.platform.agentcore_sigv4 import retire_proxy_session

    # Any inject for this key supersedes whatever session held it before. Retire
    # the predecessor's proxy generation BEFORE the gates below: a successor the
    # gates deny (profile, posture, vault, listener) must still end a stalled
    # predecessor's signed hops, and a replacement is minted only once every
    # gate has passed (``proxy_auth_headers`` in ``_workload_live_proxy_servers``).
    retire_proxy_session(session_key)
    if not _identity_on(session_key, agent=agent):
        return []
    data = read_inbound_sidecar(session_key)
