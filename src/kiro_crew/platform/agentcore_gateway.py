...
def session_gateway_servers(session_key: str, *, agent: str = "") -> list[dict[str, Any]]:
    ...
    if not session_key:
        return []
    from kiro_crew.platform.agentcore_sigv4 import proxy_auth_headers, retire_proxy_session
    ...
    # gates deny (profile, posture, listener) must still end a stalled
    # predecessor's signed hops, and a replacement is minted only once every
    # gate has passed (``proxy_auth_headers``).
