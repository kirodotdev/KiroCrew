...
def ability_payload(backend: str) -> Dict[str, object]:
    ...
    listed = (*ability.withheld, *ability.no_channel)
    keys = spec_keys()
    return {
        ...
        # The SPEC's own spelling of each listed concern, as a render fallback. The
        # kind and the reach already degrade to their declared value when a renderer
        # has no phrase for them; a concern did not, so one added to the vocabulary
        # vanished from the panel while the terminal report named it. A reader seeing
        # `permissions.defaultMode` is a reader who can go and look; a reader seeing
        # nothing has no idea a line was dropped. Only the ids on THIS card, so the
        # map is the row's own and never a dictionary of everything.
        "spec_keys": {cid: keys[cid] for cid in listed if cid in keys},
    }
