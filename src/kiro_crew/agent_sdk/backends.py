...
def register_known_backend(
    ...
    * the **id** into ``_registered_known``, so ``ACP_BACKENDS_KNOWN`` now contains
      it and every subset gate still holds (the capability frozensets never name it,
      so it simply carries the default posture);
...
def acp_runtime_backends() -> FrozenSet[str]:
    ...
    nothing registered and the preview switch off this is exactly
    ``ACP_BACKENDS_ACP_RUNTIME``.
    """
    base = ACP_BACKENDS_ACP_RUNTIME | _registered_runtime
    if codex_runs_on_acp_runtime():
        return frozenset(base | {ACP_BACKEND_CODEX})
