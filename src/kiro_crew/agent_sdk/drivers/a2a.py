...
The provider module is imported at module level: this driver is the sanctioned
SDK-boundary crossing into ``kiro_crew.providers``, and ``run.py`` imports the
driver lazily at the spawn seam, so the HTTP client still stays off the boot path.
"""
