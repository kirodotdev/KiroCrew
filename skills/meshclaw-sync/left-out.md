...
| [`8fb38539`](https://code.amazon.com/packages/MeshClaw/commits/8fb38539) | backend | 49 | **DEFER — needs its own PARTIAL triage** | fix(mcp): reap probe process groups + positively identify escaped aim launcher trees ([CR-290712... internal](https://code.amazon.com/reviews/mainline)). A substantial generic-security fix whose generic pieces are present in the fork (`mcp_discovery.probe_server` `start_new_session=True` + group-kill teardown, `session_pid` orphan sweep + a spawn-env marker constant, `sandboxed_spawn_argv`) but which also touches companion-side / absent surfaces (`providers/claude_code.py`, `mcp_gateway/backend.py`, the aim-launcher cmdline fingerprints). Landed in the batch-49 window but was NOT part of this batch's assigned keeper set; it deserves a careful hunk-by-hunk PARTIAL port (port the generic probe-group reap + the spawn marker; drop the claude_code/mcp_gateway hunks; de-Amazon the aim fingerprint to the launcher-shape + marker heuristic), not a tail-of-batch rush. Boundary advanced past it (to `bb3901c8`); re-scanned next run. |
...
| [`8fb38539`](https://code.amazon.com/packages/MeshClaw/commits/8fb38539) | backend | **DEFER** | Substantial generic-security PARTIAL (probe process-group reap + escaped-launcher positive-ID). See the "Still open" table row above. |
