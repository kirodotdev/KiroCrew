...
> **Batch-49 inherited-upstream finding (CodeQL, HIGH — recorded, NOT patched fork-side):** PR #90's CodeQL run reports one HIGH `js/insecure-randomness` at [`website/src/hooks/usePanelTabs.ts:279`](https://github.com/kirodotdev/KiroCrew/security/code-scanning?query=pr%3A90) — `const sessionId = Math.random().toString(36).slice(2, 14)` in `openTerminal`, "cryptographically insecure random number in a security context." The flagged line is **byte-identical to the upstream post-image** ([`311afeaf`](https://code.amazon.com/packages/MeshClawWebsite/commits/311afeafb188a71cd0b7fa6cea8684cbd6ef3ed6) `src/hooks/usePanelTabs.ts:279`, verified by `git show`), so per DRIFT-PREVENTION it is fixed **upstream-first**, never patched fork-side (patching would diverge the fork and be re-conflicted by the next sync). CodeQL only flags it because it is a *changed* line in this diff — the identical `Math.random().toString(36).slice(...)` id pattern is pervasive and pre-existing in the fork (`RunInTerminalBtn.tsx:40`, `apps/file-explorer/FileExplorerPage.tsx:21/28`, `components/MarkdownPanel.tsx:1015`, `hooks/useSessionGrid.ts:21`) and CodeQL passed on the prior sync PR #46. **Severity is disputed / low real impact:** the `sessionId` is a *local UI correlation key* for a terminal tab↔PTY pairing inside one already-authenticated dashboard session — not an auth token, capability, or cross-user secret (the real security boundary is the backend gateway `mc_token_` auth). Fix shape (upstream-first): mint the id via `crypto.randomUUID()` (already used with a `Math.random` fallback in `useSessionGrid.ts`), flow back via a later sync. | Not a merge blocker — the sync PR is squash-merged by the maintainer (same as PR #46, which also shows CodeQL/PR-Hygiene reds on multi-commit sync branches).
