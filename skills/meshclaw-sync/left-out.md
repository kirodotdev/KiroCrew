...
| Upstream origin | Fork file:sym | Sev | Finding | Fix shape (upstream-first) |
|---|---|---|---|---|
| [`24c320f6`](https://code.amazon.com/packages/MeshClaw/commits/24c320f6efbfe14867efab281105d14bf4f7ec22) | `sandbox.py` launcher `if PID_NAMESPACE:` (~L676) | HIGH | If `sandbox_pid_namespace` is enabled but `unshare(CLONE_NEWPID)` fails, the launcher logs and continues WITHOUT pid isolation (fail-open) rather than aborting, so sandboxed code can still signal host processes. Byte-identical to upstream `sandbox.py:750`. The user+mount namespace (the real credential boundary) is unaffected; pid-ns is best-effort defense-in-depth on top. | Abort the launch unless the operator explicitly disables pid isolation — upstream-first (it is arguably by-design as best-effort). |
| [`ed2f65b0`](https://code.amazon.com/packages/MeshClaw/commits/ed2f65b0fbd0900c471ee6ce0f71d0bc945e61d4) | `cron_script.py` `_resolve_safe_pgid`/`_kill_proc_group` (~L109-165) | MED (Windows-only) | Cron-cancel uses raw `os.getpgid`/`os.killpg`/`signal.SIG*` (POSIX) with `proc.terminate()`/`proc.kill()` fallbacks; the killpg branch is not routed through `platform_compat`, so on Windows the pgid path errors (the fallback still fires). Byte-identical to upstream `cron_script.py:109-165`. | Route the process-group kill through `platform_compat.kill_process_tree` and use portable creation flags — upstream-first (POSIX path is correct as-is). |
| [`0acf7f8f`](https://code.amazon.com/packages/MeshClaw/commits/0acf7f8f) | `mcp_gateway/pool.py` (~L47/L77) `read_buffer_limit_bytes` / `response_spill_threshold_bytes` | MED | The two config keys are parsed into `KiroCrewConfig` but never read by the pool (only the `KIROCREW_MCP_*` env vars take effect), and a configured `0` threshold would spill every response instead of disabling spilling. Faithful port of the batch (config-vs-env wiring gap is upstream). | Wire the loaded config into the gateway and require a positive threshold before spilling — upstream-first. |
