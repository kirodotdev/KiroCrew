...
| [`08e28b8a`](https://code.amazon.com/packages/MeshClawWebsite/commits/08e28b8a7648ddfb739bb33522c1d8043b826401) | frontend | 40 | **DEFER — UNBLOCKED (PR #14 merged 2026-07-18); schedule the dedicated batch** | Electron shell redesign (native-tab shell → SidePanel + BaseWindow menu rework). The new `SidePanel.tsx` hard-imports `src/components/ArtifactPanel.tsx`, which was added by [fork PR #14](https://github.com/kirodotdev/KiroCrew/pull/14) (artifacts-mirror) — **now MERGED, so this port is unblocked**; fork pre-images remain heavily diverged (main.js/App.tsx/ChatPage/index.css hundreds of diff-lines vs upstream pre-image). PARTIAL-extracting the real BaseWindow reload/devtools no-op fix was considered and REJECTED (entangled with redesign-only code: positionTrafficLights, zoomItem, the Cmd+Shift+R accelerator reshuffle; would pre-mutate the viewMenu region the full port must rewrite). **Note: fork Cmd+R / DevTools menu items likely no-op today on the BaseWindow shell — schedule this promptly as a dedicated batch.** All other deps (useIsMobile, useListboxKeyboard, useTouchedFiles, safeSetItem, extractChatLinks, countLines, framer-motion@^12) are already in the fork. No upstream revert. **CHAINED riders to port in the same batch:** (1) [`23f5a314`](https://code.amazon.com/packages/MeshClawWebsite/commits/23f5a314a71197eefe8066bb1622077e967bf613) — take `SidePanel.tsx` from post-fix content (shrink-0 pinned width, [CR-290332522](https://code.amazon.com/reviews/CR-290332522)); (2) [`082f45d5`](https://code.amazon.com/packages/MeshClawWebsite/commits/082f45d5e919c997c1108399217106a1c56622ed) — relocation of the 84px macOS traffic-light inset (`mac-instancebar-inset` root class gated on isMacElectron/macFullscreen/visibleInstanceTabs + the two index.css rules, [CR-290355334](https://code.amazon.com/reviews/CR-290355334)); it has no anchor until the 08e28b8a inset stack exists (batch-41 verdict), so port 08e28b8a and 082f45d5 together, 082f45d5 last. |
