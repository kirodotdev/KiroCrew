...
|---|---|---|---|---|
| [`4a9cc9e3`](https://code.amazon.com/packages/MeshClaw/commits/4a9cc9e3e5b9f83b802fd46895a835f359f611f5) | backend | 53 | **DEFER — needs a from-scratch redesign, not a content port** | fix(browser): guard Playwright MCP launch path against fork bombs ([CR-290811557](https://code.amazon.com/reviews/CR-290811557) / [Mesh-2843](https://taskei.amazon.dev/tasks/Mesh-2843)). A 949-line diff (new `browser/install_guard.py`, a 367-line test, +`_is_unsafe_launcher`/`_circuit_breaker_check`/doctor advisory). The generic core (host-wide launch circuit breaker, install-failure backoff, doctor conflict advisory) is portable-in-principle, BUT the fork's `mcp_playwright_proxy.py` was **rewritten from scratch** for the public `npx @playwright/mcp` path and shares almost none of the pre-image this diff edits: no `_RECURSION_GUARD_ENV`, `_emit_error_and_exit`, `_reinvokes_proxy`, or `_CIRCULAR_LAUNCH_HINT`; `_resolve_playwright_cmd` returns `"npx"` (a bare command) as a VALID launcher and the boot hook has no `ensure_playwright_installed`/AIM install path (`should_skip_auto_install` has nothing to gate). Worse, upstream's `_UNSAFE_WRAPPER_PATTERNS` includes `"npx "` — porting it verbatim would make `_is_unsafe_launcher` **refuse the fork's own legitimate launcher** and break all browser automation. The doctor advisory keys on `~/.aim` (absent). Held for a dedicated human-reviewed triage that redesigns the circuit breaker + backoff against the fork's rewritten npx-based proxy rather than porting the AIM-shim-shaped hunks. |
...
| [`8fb38539`](https://code.amazon.com/packages/MeshClaw/commits/8fb38539) | backend | 49 | ~~**DEFER — needs its own PARTIAL triage**~~ → **PORTED (PARTIAL) 2026-07-21 (PR pending)** | fix(mcp): reap probe process groups + positively identify escaped aim launcher trees ([CR-290696321](https://code.amazon.com/reviews/CR-290696321) / [Mesh-2302](https://taskei.amazon.dev/tasks/Mesh-2302)). Two independent generic-security fixes, both real gaps verified in the fork tree: **(1) probe process-group reap** — `mcp_discovery.probe_server` spawned in the gateway's group and killed only the direct child (`proc.kill()`), leaking launcher grandchildren (the fork's foreign launcher is `npx @playwright/mcp` -> node); now `start_new_session=platform_compat.IS_POSIX` + unconditional `os.killpg(pid, SIGKILL)` teardown with the int/>1 guard. **(2) marked-launcher orphan sweep** — a constant `KIROCREW_SPAWNED=1` env marker injected at every spawn site (`sandboxed_spawn_argv`, `AcpRuntime`, `AcpClient._spawn`, `mcp_gateway/backend.py spawn_backend`), read back from `/proc/<pid>/environ` (Linux-only, fail-closed) so fingerprint-less launcher orphans (`npx @playwright/mcp`) are sweepable via `_is_sweepable_orphan_mcp`. **PARTIAL de-Amazoning:** dropped the `providers/claude_code.py` spawn-site hunk (module absent — KiroACP-only fork) and de-Amazoned the launcher cmdline markers from upstream's `mcp start-server`/`.aim/mcp-servers/` to the fork-relevant `@playwright/mcp` + generic `mcp start-server`. **Ledger correction:** the prior row said "drop the claude_code/mcp_gateway hunks" — but `mcp_gateway/backend.py` **is present** in the fork (only the `spawn_backend` env-inject one-liner), so only `claude_code` was dropped. 24 tests ported (probe group-reap incl. a real-grandchild e2e routed through `platform_compat.pid_liveness` not raw `os.kill(pid,0)`; marked-launcher predicates; environ marker incl. real-child `/proc` reads; find/kill integration; spawn-site drift-guard registry sans `claude_code`). |
...
| Upstream SHA | Repo | Verdict | Why |
|---|---|---|---|
| [`820b1b01`](https://code.amazon.com/packages/MeshClaw/commits/820b1b01) | backend | **SKIP_INTERNAL** | Adds `pipeline-health` to `apps/app-registry.json` pointing at the internal `KiroClawApp-PipelineHealth` GitFarm repo (auto-diagnose via an internal AIM agent). The fork's registry is deliberately `[]`. |
| [`f123cc3f`](https://code.amazon.com/packages/MeshClaw/commits/f123cc3f) | backend | **ALREADY_PRESENT (by architecture)** | Upstream stops the base64-blob heuristic from redacting first-party URLs by making the `_SAFE_DOMAIN_SUFFIXES` branch `continue` past the blob/length checks. The fork's exfil-URL logic **diverged** — it has NO `_SAFE_DOMAIN_SUFFIXES` suffix-list; instead a companion-supplied **exact-host** exemption (`_exfil_url_warning`, the EXTRACTed `CredentialPolicy.exempt_exact_hosts` seam) already skips the blob/length heuristics for exempt hosts (`if domain.lower() not in exempt_hosts:`), with the hard-credential floor still unconditional. The exact bug the upstream hunk fixes cannot occur here; nothing to port. |
| [`8fb38539`](https://code.amazon.com/packages/MeshClaw/commits/8fb38539) | backend | ~~**DEFER**~~ → **PORTED (PARTIAL) 2026-07-21** | Generic-security PARTIAL (probe process-group reap + escaped-launcher positive-ID marker). Ported on its own branch after the batch-49 DEFER; dropped the `claude_code` spawn site, de-Amazoned the launcher markers to `@playwright/mcp`. See the "Still open" table row above (now resolved). |
| [`cbd594fa`](https://code.amazon.com/packages/MeshClaw/commits/cbd594fa) | backend | **ALREADY_PRESENT** | Adds `tool_search` to the base agent tool allowlist in `config/defaults.json`. The commit body itself says "Mirrors the KiroCrew fix"; the fork's `config/defaults.json` already lists `tool_search` (line 17). No-op. |
| [`a39bffa5`](https://code.amazon.com/packages/MeshClaw/commits/a39bffa5), [`bb3901c8`](https://code.amazon.com/packages/MeshClaw/commits/bb3901c8) | backend | **SKIP_INTERNAL** | `CHANGELOG.md`-only v3.3.0 release-note folds. The fork is independently versioned (0.1.0) and keeps its own changelog. |

**Fork-gate adaptation (batch-49, [`84359f45`](https://code.amazon.com/packages/MeshClaw/commits/84359f45713de59c67010d08cb8f29c1fd7d9a6b) terminal port):** the ported `_session_title` helper and its test newly reference the pre-existing POSIX pseudo-terminal file-descriptor field on `_TerminalSession` (an upstream-verbatim, load-bearing name pervasive in `terminal.py`, already accepted in the fork because it predates the changed-lines woke inclusive-language gate added in PR #49). Because the gate (`code-review.yml`) fails on any error-severity finding on ADDED lines, the 4 newly-added references to that field each carry an inline `# wokeignore:rule=<term>` directive — the tool-sanctioned escape hatch — rather than renaming the field (which would diverge from both the fork's own API and upstream). No behavior change; SKILL Step 4 ("the fork's linter can be STRICTER than upstream — fix the ported hunk, don't disable the check").

**Message-vs-code default note (batch-49, [`5db73730`](https://code.amazon.com/packages/MeshClaw/commits/5db73730aecd5b508274d40215b71d96318408a2)):** the commit MESSAGE says the new `knowledge.max_ingest_file_mb` defaults to 20, but every default in the actual diff (config-baseline parent block + schema entry, dataclass field, walrus fallback, module constant) is `100.0`. The fork ports the CODE (`100.0`), matching upstream's shipped behavior; the "20" is a stale message.
...
**PR #97 (batch-51) review round — Codex HIGH, inherited-upstream (recorded, NOT patched fork-side); Claude ✅ no blocking.** Claude Review traced the sandbox delegation and confirmed it holds up (audit-or-deny, fail-closed toward isolation, `safe_read_file` keystone, shared env scrub). Codex's one HIGH is on code **byte-identical to the upstream post-image** it was ported from (verified `git show 507ad772:src/mesh_claw/sandbox.py` — same `return bool(data.get(_KIRO_INTERNAL_SANDBOX_KEY, False))` parse, same `if not isinstance(data, dict)` guard, same `wrap_argv` gate; upstream does NOT write-protect the settings path or require `is True` either). Per DRIFT-PREVENTION it is fixed **upstream-first**, never patched fork-side (a unilateral fork patch would diverge + be re-conflicted by the next sync). Not a merge blocker (multi-commit sync PRs squash-merge; the fork-introduced E2E-assertion regression from the `50a1f96` message-split WAS fixed fork-side in `cron.spec.ts`).

| Upstream origin | Fork file:sym | Tool / Sev | Finding | Fix shape (upstream-first) |
|---|---|---|---|---|
| [`507ad772`](https://code.amazon.com/packages/MeshClaw/commits/507ad772540db0f44b8983f23e22031392fe6874) | `sandbox.py:1586` `wrap_argv` delegation gate + `kiro_internal_sandbox_enabled()` | Codex HIGH | TOCTOU / user-writable settings: a sandboxed agent can write `~/.kiro/settings/amazon-internal.json` (not on the sensitive-path floor, not seatbelt write-denied), toggling `"sandbox"` between the KiroCrew read and the kiro-cli read, so KiroCrew may omit its seatbelt while kiro-cli starts without its own; `O_NOFOLLOW` guards symlinks, not content races, and any truthy non-bool coerces to delegate. Byte-identical to upstream `507ad772` (same `bool(data.get(...))` parse + gate; upstream neither write-protects the path nor requires `is True`). | Write-protect the settings file (sensitive-path floor + seatbelt `file-write*` deny), require `data.get(...) is True`, and close the read-to-exec race by pinning sandbox enablement in the child invocation (atomic/attested) before removing the outer wrap — upstream-first; flow back. |
...
**PR #90 (batch-49) review round — Codex + CodeQL findings, ALL inherited-upstream (recorded, NOT patched fork-side).** Claude Review returned no blocking findings. Every Codex/CodeQL finding below is on code **byte-identical to the upstream post-image** it was ported from (verified by `git show <sha>:<path>` and, for `terminalRegistry.ts`, a whole-file `diff` after the rename map). Per DRIFT-PREVENTION they are fixed **upstream-first** and never patched fork-side — a unilateral fork patch would diverge from upstream and be re-conflicted by the next sync. None is a merge blocker: multi-commit sync PRs are squash-merged by the maintainer (PR #46 showed the same Codex/CodeQL/PR-Hygiene reds and merged).

| Upstream origin | Fork file:sym | Tool / Sev | Finding | Fix shape (upstream-first) |
|---|---|---|---|---|
| [`5db73730`](https://code.amazon.com/packages/MeshClaw/commits/5db73730aecd5b508274d40215b71d96318408a2) + [`84359f45`](https://code.amazon.com/packages/MeshClaw/commits/84359f45713de59c67010d08cb8f29c1fd7d9a6b) | `knowledge/ingestion.py:136/147` (`is_sensitive_path(path)` / `limit_mb = _max_ingest_file_mb()`) + `dashboard/handlers/terminal.py:129` (`os.path.isdir(candidate)` in `_resolve_cwd`) | Codex HIGH (`no-blocking-call-on-event-loop`) | Synchronous FS ops (`is_sensitive_path` resolve, config-loading `_max_ingest_file_mb`, `isdir`) run on the asyncio loop; a file/CWD on an unresponsive NFS/FUSE mount can stall the loop, suppress the heartbeat, and trip the watchdog. Both regions are byte-identical to upstream post-images (ingestion already wraps `p.resolve()` in `asyncio.to_thread` exactly as upstream; the remaining sync calls are upstream-verbatim). | Wrap the sensitive-path check + config lookup + `_resolve_cwd` `isdir` in `asyncio.to_thread()`/`run_in_executor()` upstream; flow back. |
| [`84359f45`](https://code.amazon.com/packages/MeshClaw/commits/84359f45713de59c67010d08cb8f29c1fd7d9a6b) | `dashboard/handlers/terminal.py:129` `_resolve_cwd` | CodeQL #343 `py/path-injection` (HIGH) | The client-supplied `?cwd=` flows into `os.path.isdir(candidate)` with no root restriction. This is upstream's DELIBERATE design (the verbatim comment: "the user's own interactive shell (auth is enforced at the WS handshake), so there is no root restriction beyond isdir") — the WS handshake auth is the control, not a path allowlist. Byte-identical to upstream `84359f45`. | If upstream decides to restrict, confine to the configured/project roots; otherwise it is by-design behind WS auth. Upstream-first. |
| [`311afeaf`](https://code.amazon.com/packages/MeshClawWebsite/commits/311afeafb188a71cd0b7fa6cea8684cbd6ef3ed6) | `website/src/hooks/usePanelTabs.ts:279` `openTerminal` | CodeQL #342 `js/insecure-randomness` (HIGH) | `const sessionId = Math.random().toString(36).slice(2, 14)` — flagged as insecure randomness "in a security context." Byte-identical to upstream. CodeQL only flags it because it is a *changed* line; the identical id pattern is pervasive+pre-existing in the fork (`RunInTerminalBtn.tsx:40`, `apps/file-explorer/FileExplorerPage.tsx:21/28`, `components/MarkdownPanel.tsx:1015`, `hooks/useSessionGrid.ts:21`) and CodeQL passed on PR #46. Low real impact: `sessionId` is a local UI tab↔PTY correlation key inside one already-authenticated dashboard session, not an auth token (the boundary is the gateway `mc_token_` auth). | Mint via `crypto.randomUUID()` (already the primary path in `useSessionGrid.ts`) upstream; flow back. |
| [`311afeaf`](https://code.amazon.com/packages/MeshClawWebsite/commits/311afeafb188a71cd0b7fa6cea8684cbd6ef3ed6) | `website/src/utils/terminalRegistry.ts:102` (persistent-connection manager) | Codex MEDIUM | Deleting a chat unmounts its panel + clears its persisted keys but does not explicitly close each terminal tab's module-level WS, so the backend orphan reaper can't reclaim the PTY until its timeout; repeatedly deleting chats-with-terminals can consume the global session cap (HTTP 429) until reload/restart. The whole file is byte-identical to the upstream post-image (verified by full-file `diff` after the rename map). | On chat delete, enumerate the chat's terminal tabs, call `disposeTerminalConnection()` + the terminal DELETE endpoint per tab, then drop the panel bucket. Upstream-first. |
| [`311afeaf`](https://code.amazon.com/packages/MeshClawWebsite/commits/311afeafb188a71cd0b7fa6cea8684cbd6ef3ed6) | `website/src/hooks/usePanelTabs.ts:274` `openTerminal` | Codex MEDIUM | At the 4-terminal cap `openTerminal()` returns the most-recent existing `sessionId`, and `ChatPage` then `sendToTerminalSession(sessionId, code)` unconditionally — if that terminal is running `python`/`vim`/`cat > file`, the "Run in terminal" text lands as stdin to that process, not a fresh shell. Byte-identical to upstream `311afeaf` (same at-cap return-last-session behavior). | At the cap, return failure without sending (or close+replace an old terminal) before dispatch. Upstream-first. |
| [`4d279fd0`](https://code.amazon.com/packages/MeshClaw/commits/4d279fd0) | `config/loader.py:2813` orchestrator/watchdog parse (`_safe_int`/`_safe_float`) | Codex MEDIUM | `_safe_int`/`_safe_float` validate *conversion* only, not range, so a negative `stage_timeout_seconds` (expires immediately) or negative/non-finite watchdog window (premature oracle check / turn cancel) reaches the newly-wired orchestrator/watchdog dataclasses for the first time in this PR. Byte-identical to upstream `4d279fd0`'s own diff — its commit message states the design intent ("all numeric reads use _safe_int/_safe_float so a malformed value falls back to the field default... consistent with the loader's other coercions"); upstream deliberately does NOT range-check. | Reject negative/non-finite and fall back to documented defaults + boundary tests, upstream-first; flow back. |

**Batch-50 FORK-INTRODUCED defect — fixed fork-side (NOT inherited-upstream).** The batch-50 port of [`33da30e6`](https://code.amazon.com/packages/MeshClaw/commits/33da30e622b93fed5b1364c79fd70d8b8c91a9d9) (commit `3b31ff82`) copied upstream's test helper `_find_dead_pid()` into `test/test_pid_lifecycle.py`, which walks PIDs with a raw `os.kill(pid, 0)`. Codex flagged it `[BLOCK-MERGE]` HIGH. This is a genuine **port mistake, not upstream-verbatim inert code**: upstream MeshClaw is POSIX-only, but this fork supports Windows and forbids raw `os.kill(pid, 0)` (it *terminates* the target on Windows — the CLAUDE.md/AGENTS.md `platform_compat` rule), so on a Windows CI runner the walk could kill an unrelated live process. Per DRIFT-PREVENTION, a fork-introduced defect / port mistake is fixed **fork-side** (only upstream-verbatim findings are deferred). Fixed in `c3d31705`: removed `_find_dead_pid` and mocked `platform_compat.pid_liveness → PID_DEAD` in `test_dead_pid_is_safe_to_untrack` (matching the other three predicate tests). The production `_pid_gone_or_unmanaged` was already Windows-safe (routes through `pid_liveness`); only the test helper had the raw call. This is the same class of fork-side fix as batch-40's `os.kill(pid,0)`→`pid_exists()` sweep correction.
...
| `f8383887` | `deploy_web` hero SVGs (both variants) | ~~**REJECTED (DEFER stands)**~~ → **STALE — closed ALREADY_PRESENT-by-architecture 2026-07-21** | This row's premise ("the workflows heroes 404 today") is **stale**: fork PR #26 (`afe20357`) shipped a Vite `public/app-assets/` hero architecture that does NOT need the `fd633154` `_materialize_builtin_assets` substrate. Verified 2026-07-21: `workflows/app.json` references `/app-assets/workflows/hero-{light,dark}.svg` and both SVGs exist (they render, not 404); `deploy_web` declares no `heroImage`. See the batch-27/28 rows — closed, nothing to port. |
...
| [`3e5d7132`](https://code.amazon.com/packages/MeshClaw/commits/3e5d7132e46c3fd7dd4394dd182fba9f58656025) | backend | ~~**DEFER**~~ → **ALREADY_PRESENT (closed 2026-07-21)** | Reject type-unsafe authored workflow scripts. Fork PR [#57](https://github.com/kirodotdev/KiroCrew/pull/57) MERGED; verified in-tree — `src/kiro_crew/workflows/validate.py` statically rejects all imports (`visit_Import`/`visit_ImportFrom`), rejects unknown ctx methods outside the CM-safe set, and enforces the `META` pure-literal shape (the three fixes). Nothing to port. |
| [`f8383887`](https://code.amazon.com/packages/MeshClaw/commits/f8383887d8dec0f3583a3cbcf674468172e3dd45) | backend | ~~**DEFER**~~ → **ALREADY_PRESENT-by-architecture (closed 2026-07-21)** | Hero SVGs for deploy_web/workflows. Fork PR #26 shipped a native hero architecture (`website/public/app-assets/`, served by Vite — NOT the `fd633154` `_materialize_builtin_assets` substrate). Verified in-tree: `apps/builtins/workflows/app.json` references `/app-assets/workflows/hero-{light,dark}.svg` and **both SVGs exist**; `deploy_web` declares **no** `heroImage` (no broken-image bug). Nothing to port. (Supersedes the stale governance-re-triage note that "the workflows heroes 404 today" — line ~645.) |
...
| [`3e5d7132`](https://code.amazon.com/packages/MeshClaw/commits/3e5d7132e46c3fd7dd4394dd182fba9f58656025) | backend | ~~DEFER~~ → **ALREADY_PRESENT (closed 2026-07-21)** | Reject type-unsafe authored workflow scripts. Verified in-tree — `src/kiro_crew/workflows/validate.py` carries all three fixes (import rejection, ctx-method allowlist, `META` literal shape). See the batch-28 table row for detail. |
| [`f8383887`](https://code.amazon.com/packages/MeshClaw/commits/f8383887d8dec0f3583a3cbcf674468172e3dd45) | backend | ~~DEFER~~ → **ALREADY_PRESENT-by-architecture (closed 2026-07-21)** | Hero images for deploy_web/workflows. Fork PR #26 (`afe20357`) shipped a fork-native hero architecture (`website/public/app-assets/`). Verified in-tree: `workflows/app.json` references `/app-assets/workflows/hero-{light,dark}.svg` and both SVGs exist; `deploy_web` declares no `heroImage`. No broken-image bug, nothing to port. See the batch-28 table row for detail. |
