...
- Data-home migration no longer strands users who already have a `~/.kiro/crew` folder. If that folder pre-exists with different contents (e.g. created by another Kiro tool or a `KIROCREW_HOME` experiment), the migration now completes the switch with your `~/.kirocrew` data authoritative — moving the pre-existing folder aside to a recoverable, keystone-gated + owner-locked `~/.kiro/crew.pre-migration/<timestamp>` backup (the agent's own file tools cannot read secrets from it; its credential leaves auto-expire on the same 7-day schedule as the archive) — instead of silently keeping you on the old `~/.kirocrew` location forever. Which data is live is unchanged; a gateway actively using `~/.kiro/crew` is never disturbed. Note: on this promote-in-place path no `~/.kirocrew.archived` is created — to downgrade, stop KiroCrew and rename `~/.kiro/crew` back to `~/.kirocrew` (your data lives there, not in an archive).
