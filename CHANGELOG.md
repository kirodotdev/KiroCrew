...
- **Importing another agent's setup is no longer a multi-minute wait that imports nothing.** Two independent problems made the first-run import from MeshClaw look hung and then land almost no memory. First, every imported memory chunk was embedded inline on the request: local inference costs roughly 0.4s for a 2000-char chunk and an import writes hundreds of them, so the apply call held for minutes with no progress. Imported memories are now written immediately and embedded afterwards in the background — they are keyword-searchable the moment they land and become semantically searchable shortly after — which took a real import from 214 seconds of waiting down to about 3. (Batching the embeddings is deliberately *not* the fix: measured on real import text it is slower than one at a time, because a single chunk already fills the model's batch.) The background sweep that fills in those vectors also no longer requires the optional `faiss` package, which is not actually a dependency — so on a normal install it had been quietly doing nothing, and any memory waiting on it would have stayed unsearchable by meaning indefinitely. Second, the MeshClaw memory store imported **zero** rows while reporting success: rows were skipped as belonging to a specific project whenever they carried a workspace marker, but a single-workspace install stamps the same placeholder on every row, so the entire store looked project-scoped; and rows the source marks as a learned rule were dropped as unsupported, which discarded exactly the least replaceable thing in it. Placeholder markers are now recognised as "not scoped", genuinely project-scoped rows are still reported as unsupported, and learned rules are imported as high-priority lessons — the same tier, guards, and limits as a rule from a `CLAUDE.md`, so a foreign persona line still cannot slip in as the agent's identity. On a real store that is 45 memories and 21 instructions before, 543 and 36 after.
- **The PR panel no longer reports a red or inflated check status that GitHub itself doesn't show.** GitHub's status-check rollup returns *every* check-run recorded against the head commit, not one per check — dispatch the same workflow twice for one commit (a push immediately followed by an edit event, say) and each run contributes its own row, while the concurrency group cancels the losing suite. The panel rendered those raw rows, so a pull request GitHub summarised as "1 pending, 40 successful" was counted here as 49 checks, and the superseded `CANCELLED` row rolled up to a red failure glyph on both the sidebar chip and the panel header even though its replacement run had passed. Refreshing never cleared it, because the stale row really is still in the provider payload. Both read paths now collapse each check to its latest run the way GitHub's own UI does, keyed on workflow *and* check name so one workflow's later success can never hide another's failure, and preferring a queued re-run over the completed run it supersedes rather than showing a stale pass.
