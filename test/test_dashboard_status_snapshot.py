...
class TestAllStatusSnapshotCallersPassTheUpdateFields:
    ...
    def test_every_status_snapshot_call_site_uses_the_shared_reader(self) -> None:
        ...
        to spread them. The structural contract is therefore: every
        ``status_snapshot(...)`` call lives in ``status_counts.py``. A call
        anywhere else bypasses both the off-loop count cache (reintroducing the
        blocking-call freeze class) and the shared update-fields join.
        """
        import ast
        import pathlib
        ...
        for path in dashboard.rglob("*.py"):
            ...
            for node in ast.walk(tree):
                ...
                # The ONLY permitted status_snapshot call site is the one inside
                # cached_status_snapshot (status_counts.py). Any other file calling
                # it bypasses the off-loop count cache and the shared reader funnel.
                if path.name != funnel:
                    offenders.append(f"{path.name}:{node.lineno}: {srcseg[:90]}")

        assert not offenders, (
            "these status call sites bypass the status_counts funnel "
            "(cached_status_snapshot), so they skip the off-loop count cache "
            "and the shared update reader:\n  " + "\n  ".join(offenders)
        )
