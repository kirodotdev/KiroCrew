...
DEPLOYMENT = "ghe-host-acme"    # the PROVIDER deployment that HOSTS the account


def _verified(principal_id: str) -> QueryPrincipal:
    # A principal the auth layer ESTABLISHED (verified=True) -- what the dashboard
    # handler mints from the middleware identity. principal_is_verified vouches
    # for it; a bare QueryPrincipal(...) with verified=False would be denied.
...
def _seed(store, *, kiro_principal, subject_ref, tenant_ref,
          account=ACCOUNT, deployment=DEPLOYMENT):
    binding = create_binding(
        service_id=SERVICE, claimed_subject="claimed", claimed_tenant="claimed",
        credential_mode="oauth_user", verifier=_verifier_for(subject_ref, tenant_ref),
        slug="salesforce",
    )
...
def _search(kb, bstore, item, principal, term="alpha"):
    r = HybridRetriever(kb, revalidator=_fresh_hook(),
                        binding_resolver=bridge_binding_resolver(_resolver(bstore)))
...
def test_wrong_account_denies(kb, tmp_path):
    ...
    # None -> deny.
    item = _managed_item(kb, "Other", "alpha content", "sf-alice", "t-A",
                         account="other-org")
...
def test_revoked_binding_fences(kb, tmp_path):
    bstore = BindingStore(path=tmp_path / "b.json")
    stored = _seed(bstore, kiro_principal="alice", subject_ref="sf-alice",
                   tenant_ref="t-A")
    item = _managed_item(kb, "Deal", "alpha content", "sf-alice", "t-A")
