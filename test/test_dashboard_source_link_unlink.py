...
class TestPersistenceRoundTrip:
    ...
    def test_bounded_valid_identities_caps_during_iteration_and_drops_junk(self):
        ...
        from kiro_crew.dashboard.handlers.source_providers import bounded_valid_identities
...
def _request(
    slot_key: str, identity: str, slots: dict, *, app: str = "", expect: str | None = None
):
    ...
    # A real query mapping so the optional ``expect`` guard reads a concrete
    # value (or None) rather than an auto-generated MagicMock, which would be
    # truthy and fire the guard on every request. ``multidict``-like ``.get``.
...
async def _delete(
    ...
    with patch("kiro_crew.dashboard.chat_handlers.sel"):
        return await api_chat_slot_source_link_unlink(
            _request(slot_key, identity, slots, app=app, expect=expect)
        )
