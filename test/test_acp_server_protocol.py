...
from kiro_crew.acp_server import server as server_mod
...
class _FailingDrainWriter(_CapturingWriter):
    """Record the frame, then model a broken editor transport."""

    async def drain(self) -> None:
        raise BrokenPipeError("editor disconnected")


class _BlockingDrainWriter(_CapturingWriter):
    """Hold result delivery until the lifecycle task is cancelled."""
...
class TestPermissionGate:
    ...
    @pytest.mark.asyncio
    async def test_session_cancel_beats_buffered_allow(self) -> None:
        class ImmediateTransport:
            async def send_request(self, *_args: Any, **_kwargs: Any) -> dict[str, Any]:
                return {
                    "outcome": {
                        "outcome": OUTCOME_SELECTED,
                        "optionId": OPTION_ALLOW_ONCE,
                    }
                }

        session = _Session(session_id="s1")
        session.cancelled.set()
        sink = SessionSink(ImmediateTransport(), session)  # type: ignore[arg-type]

        assert await sink.request_permission({"toolCallId": "t1"}) is False

    @pytest.mark.asyncio
    async def test_session_cancel_beats_buffered_elicitation_accept(self) -> None:
        class ImmediateTransport:
            async def send_request(self, *_args: Any, **_kwargs: Any) -> dict[str, Any]:
                return {"action": "accept", "content": {"answer": "yes"}}

        session = _Session(session_id="s1", elicitation_supported=True)
        session.cancelled.set()
        sink = SessionSink(ImmediateTransport(), session)  # type: ignore[arg-type]

        result = await sink.create_elicitation({"message": "Continue?"})
...
class TestDrainTasks:
    ...
    @pytest.mark.asyncio
    async def test_editor_eof_stops_in_flight_backing_turn(self) -> None:
        started = asyncio.Event()
        released = asyncio.Event()

        class Backend(_StubMcpBackend):
            def __init__(self) -> None:
                super().__init__()
        ...
        async def handler(_request: PromptRequest, _sink: SessionSink) -> str:
            started.set()
            await released.wait()
            return STOP_REASON_END_TURN

        backend = Backend()
        harness = _Harness(handler, backend)
        await harness.start()
        session_id = await _new_session(harness)
        harness.send(
            {
                "jsonrpc": "2.0",
                "id": 2,
        ...
        assert backend.cancelled == [session_id]
...
class TestFramingStrictness:
    ...
    @pytest.mark.asyncio
    async def test_explicit_null_id_is_answered_as_a_request(self) -> None:
        h = _Harness(_noop_handler)
        await h.start()
        h.send({"jsonrpc": "2.0", "id": None, "method": "totally/unknown", "params": {}})

        frame = await h.wait_for(lambda f: f.get("id") is None and "error" in f)
...
class TestParamValidation:
    ...
    @pytest.mark.asyncio
    async def test_session_new_rejects_sensitive_cwd_off_loop(
        ...
        monkeypatch.setattr(server_mod, "is_sensitive_path", sensitive)
        h = _Harness(_noop_handler)
        await h.start()
        h.send(
            {
...
class _StubMcpBackend:
    ...
    def __init__(self) -> None:
        self.configured: list[tuple[str, str, list[Any]]] = []
    ...
    async def configure_session_mcp(self, session_id: str, cwd: str, servers: list[Any]) -> None:
        self.configured.append((session_id, cwd, list(servers)))
...
class TestAvailableCommandAdvertisement:
    ...
    @pytest.mark.asyncio
    async def test_load_redacts_replayed_history_before_egress(
        self, monkeypatch: pytest.MonkeyPatch
    ) -> None:
        class HistoryBackend(_CommandBackend):
            async def load_session(self, session_id: str, cwd: str) -> list[dict[str, str]]:
                return [
        ...
        monkeypatch.setattr("kiro_crew.acp_server.server.redact_via_context", _redact)
        writer = _CapturingWriter()
        ...
        replay = [
            ...
            for frame in writer.frames
            if frame.get("method") == METHOD_SESSION_UPDATE
            and frame["params"]["update"].get("sessionUpdate")
            in ("user_message_chunk", "agent_message_chunk")
        ]
        assert seen == ["user secret", "assistant secret"]
        assert [update["content"]["text"] for update in replay] == [
            "safe:user secret",
            "safe:assistant secret",
        ]
...
class TestMcpServers:
    ...
    async def test_stdio_server_parsed_and_passed_to_backend(self) -> None:
        ...
        assert backend.configured == [("s-1", "/repo", stored)]
...
class _LifecycleBackend(_StubMcpBackend):
    supports_load = True
    supports_resume = True
    ...
    def __init__(
        ...
        *,
        fail_mcp: bool = False,
        fail_project_restore: bool = False,
        fail_local_mcp_restore: bool = False,
        block_load: bool = False,
        block_mcp: bool = False,
    ) -> None:
        super().__init__()
        self.project = "/old"
        self.fail_mcp = fail_mcp
        self.fail_project_restore = fail_project_restore
        self.fail_local_mcp_restore = fail_local_mcp_restore
        self.block_load = block_load
        self.block_mcp = block_mcp
        self.load_started = asyncio.Event()
        self.load_release = asyncio.Event()
        self.mcp_started = asyncio.Event()
        self.mcp_release = asyncio.Event()
        self.load_calls: list[tuple[str, str]] = []
        self.resume_calls: list[tuple[str, str]] = []
        self.restored: list[tuple[str, str]] = []
        self.deleted: list[str] = []
        self.mcp_snapshot: Any = None
        self.restored_mcp: list[tuple[str, Any]] = []
        self.project_changed = False
        self.mcp_owner_changed = False

    async def get_session_cwd(self, session_id: str) -> str:
        return self.project

    async def restore_session_cwd(self, session_id: str, cwd: str) -> bool:
        if self.fail_project_restore:
            raise ConnectionError("project rollback transport failed")
        if self.project_changed:
            return False
        self.project = cwd
        self.restored.append((session_id, cwd))
        return True
    ...
    async def configure_session_mcp(self, session_id: str, cwd: str, servers: list[Any]) -> Any:
        self.configured.append((session_id, cwd, list(servers)))
        if self.block_mcp:
            self.block_mcp = False
            self.mcp_started.set()
            await self.mcp_release.wait()
        if self.fail_mcp:
            self.fail_mcp = False
            raise _ClientSafeError("MCP server failed to start")
        return self.mcp_snapshot

    async def restore_local_session_mcp(
        self, session_id: str, cwd: str, servers: list[Any]
    ) -> bool:
        if self.fail_local_mcp_restore:
            raise ConnectionError("MCP rollback transport failed")
        if self.mcp_owner_changed:
            return False
        self.configured.append((session_id, cwd, list(servers)))
        return True

    async def restore_session_mcp(self, session_id: str, snapshot: Any) -> None:
        self.restored_mcp.append((session_id, snapshot))


class TestLifecycleTransactions:
    @pytest.mark.asyncio
    @pytest.mark.parametrize("handler", ["_handle_session_load", "_handle_session_resume"])
    @pytest.mark.parametrize("prior_cwd", ["/old", ""])
    async def test_mcp_failure_restores_prior_session_and_project(
        self, handler: str, prior_cwd: str
    ) -> None:
        backend = _LifecycleBackend(fail_mcp=True)
        backend.project = prior_cwd
        writer = _CapturingWriter()
        server = AcpAgentServer(
            AgentTransport(asyncio.StreamReader(), writer),
            _noop_handler,
        ...
        prior = _Session(session_id="existing", cwd=prior_cwd)
        server._sessions["existing"] = prior

        await getattr(server, handler)(
            {
                "sessionId": "existing",
                "cwd": "/new",
                "mcpServers": [{"name": "echo", "command": "/bin/echo"}],
            },
            41,
        )

        frame = writer.find(req_id=41)
        assert frame is not None and frame["error"]["code"] == JSONRPC_INTERNAL_ERROR
        assert server._sessions["existing"] is prior
        assert backend.project == prior_cwd
        assert backend.restored == [("existing", prior_cwd)]
        assert [(cwd, len(servers)) for _, cwd, servers in backend.configured] == [
            ("/new", 1),
            (prior_cwd, 0),
        ]
        assert "existing" not in server._lifecycle_sessions

    @pytest.mark.asyncio
    @pytest.mark.parametrize("handler", ["_handle_session_load", "_handle_session_resume"])
    @pytest.mark.parametrize(
        "rollback_failure",
        ["fail_project_restore", "fail_local_mcp_restore"],
    )
    async def test_rollback_transport_failure_invalidates_local_session(
        self, handler: str, rollback_failure: str
    ) -> None:
        backend = _LifecycleBackend(fail_mcp=True, **{rollback_failure: True})
        writer = _CapturingWriter()
        server = AcpAgentServer(
            AgentTransport(asyncio.StreamReader(), writer),
            _noop_handler,
        ...
        server._sessions["existing"] = _Session(session_id="existing", cwd="/old")
        ...
        frame = writer.find(req_id=54)
        assert frame is not None and frame["error"]["code"] == JSONRPC_INTERNAL_ERROR
        assert "existing" not in server._sessions
        assert "existing" not in server._lifecycle_sessions

    @pytest.mark.asyncio
    @pytest.mark.parametrize("handler", ["_handle_session_load", "_handle_session_resume"])
    async def test_failed_setup_without_local_session_preserves_backing_owner(
        self, handler: str
    ) -> None:
        backend = _LifecycleBackend(fail_mcp=True)
        writer = _CapturingWriter()
        ...
        frame = writer.find(req_id=51)
        assert frame is not None and frame["error"]["code"] == JSONRPC_INTERNAL_ERROR
        assert [(cwd, len(servers)) for _, cwd, servers in backend.configured] == [("/new", 1)]
        assert backend.restored_mcp == []

    @pytest.mark.asyncio
    @pytest.mark.parametrize("handler", ["_handle_session_load", "_handle_session_resume"])
    async def test_late_failure_restores_backing_mcp_snapshot(self, handler: str) -> None:
        backend = _LifecycleBackend()
        snapshot = {
            "servers": [{"name": "old", "command": "/old", "args": [], "env": []}],
            "owner": "adapter-a",
        }
        backend.mcp_snapshot = snapshot
        server = AcpAgentServer(
            AgentTransport(asyncio.StreamReader(), _FailingDrainWriter()),
            _noop_handler,
            session_backend=backend,
        )

        with pytest.raises(BrokenPipeError, match="editor disconnected"):
            await getattr(server, handler)(
                {"sessionId": "existing", "cwd": "/new", "mcpServers": []}, 52
            )

        assert backend.restored_mcp == [("existing", snapshot)]
        assert [(cwd, len(servers)) for _, cwd, servers in backend.configured] == [("/new", 0)]

    @pytest.mark.asyncio
    @pytest.mark.parametrize("handler", ["_handle_session_load", "_handle_session_resume"])
    async def test_cancelled_mcp_setup_restores_prior_lifecycle(self, handler: str) -> None:
        backend = _LifecycleBackend(block_mcp=True)
        writer = _CapturingWriter()
        server = AcpAgentServer(
            AgentTransport(asyncio.StreamReader(), writer),
            _noop_handler,
        ...
        prior = _Session(session_id="existing", cwd="/old")
        server._sessions["existing"] = prior
        lifecycle = asyncio.create_task(
            getattr(server, handler)(
                {
                    "sessionId": "existing",
                    "cwd": "/new",
                    "mcpServers": [{"name": "echo", "command": "/bin/echo"}],
        ...
        await backend.mcp_started.wait()
        ...
        assert server._sessions["existing"] is prior
        assert backend.project == "/old"
        assert backend.restored == [("existing", "/old")]
        assert [(cwd, len(servers)) for _, cwd, servers in backend.configured] == [
            ("/new", 1),
            ("/old", 0),
        ]
    ...
    @pytest.mark.asyncio
    async def test_cancelled_new_mcp_setup_deletes_backing_session(self) -> None:
        backend = _LifecycleBackend(block_mcp=True)
        server = AcpAgentServer(
            AgentTransport(asyncio.StreamReader(), _CapturingWriter()),
            _noop_handler,
            session_backend=backend,
        )
        lifecycle = asyncio.create_task(
        ...
        assert "s-1" not in server._sessions
        assert backend.deleted == ["s-1"]

    @pytest.mark.asyncio
    async def test_failed_new_result_delivery_deletes_backing_session(self) -> None:
        backend = _LifecycleBackend()
        server = AcpAgentServer(
            AgentTransport(asyncio.StreamReader(), _FailingDrainWriter()),
            _noop_handler,
        ...
        with pytest.raises(BrokenPipeError, match="editor disconnected"):
            await server._handle_session_new({"cwd": "/new", "mcpServers": []}, 47)

        assert "s-1" not in server._sessions
        assert backend.deleted == ["s-1"]

    @pytest.mark.asyncio
    async def test_cancelled_new_result_delivery_deletes_backing_session(self) -> None:
        backend = _LifecycleBackend()
        ...
        lifecycle = asyncio.create_task(
            server._handle_session_new({"cwd": "/new", "mcpServers": []}, 50)
        )
    ...
    @pytest.mark.asyncio
    @pytest.mark.parametrize("handler", ["_handle_session_load", "_handle_session_resume"])
    async def test_failed_lifecycle_result_delivery_restores_prior_state(
        self, handler: str
    ) -> None:
        backend = _LifecycleBackend()
        server = AcpAgentServer(
        ...
        with pytest.raises(BrokenPipeError, match="editor disconnected"):
            await getattr(server, handler)(
                {"sessionId": "existing", "cwd": "/new", "mcpServers": []}, 48
            )
    ...
    @pytest.mark.asyncio
    @pytest.mark.parametrize("handler", ["_handle_session_load", "_handle_session_resume"])
    async def test_late_rollback_refuses_newer_adapter_state(self, handler: str) -> None:
        backend = _LifecycleBackend()
        writer = _BlockingDrainWriter()
        server = AcpAgentServer(
            AgentTransport(asyncio.StreamReader(), writer),
        ...
        server._sessions["existing"] = prior
        lifecycle = asyncio.create_task(
            getattr(server, handler)({"sessionId": "existing", "cwd": "/new", "mcpServers": []}, 53)
        )
        await writer.started.wait()

        backend.project = "/newer-adapter"
        backend.project_changed = True
        backend.mcp_owner_changed = True
        lifecycle.cancel()
        with pytest.raises(asyncio.CancelledError):
            await lifecycle

        assert server._sessions["existing"] is prior
        assert backend.project == "/newer-adapter"
    ...
    @pytest.mark.asyncio
    async def test_failed_load_result_delivery_clears_new_local_state(self) -> None:
        backend = _LifecycleBackend()
        server = AcpAgentServer(
            AgentTransport(asyncio.StreamReader(), _FailingDrainWriter()),
            _noop_handler,
        ...
        with pytest.raises(BrokenPipeError, match="editor disconnected"):
            await server._handle_session_load(
                {"sessionId": "existing", "cwd": "/new", "mcpServers": []}, 49
            )
    ...
    @pytest.mark.asyncio
    @pytest.mark.parametrize("handler", ["_handle_session_load", "_handle_session_resume"])
    async def test_lifecycle_rejected_while_prompt_is_in_flight(self, handler: str) -> None:
        backend = _LifecycleBackend()
        writer = _CapturingWriter()
        server = AcpAgentServer(
            AgentTransport(asyncio.StreamReader(), writer),
        ...
        server._sessions["existing"] = _Session(session_id="existing", cwd="/old", in_flight=True)

        await getattr(server, handler)(
            {"sessionId": "existing", "cwd": "/new", "mcpServers": []}, 42
        )

        frame = writer.find(req_id=42)
        assert frame is not None and frame["error"]["code"] == JSONRPC_INVALID_PARAMS
        assert backend.load_calls == []
        assert backend.resume_calls == []

    @pytest.mark.asyncio
    async def test_prompt_rejected_while_load_is_awaiting_backend(self) -> None:
        backend = _LifecycleBackend(block_load=True)
        writer = _CapturingWriter()
        server = AcpAgentServer(
            AgentTransport(asyncio.StreamReader(), writer),
            _noop_handler,
        ...
        load = asyncio.create_task(
            server._handle_session_load(
                {"sessionId": "existing", "cwd": "/new", "mcpServers": []}, 43
            )
        )
        await backend.load_started.wait()

        await server._handle_prompt(
            {"sessionId": "existing", "prompt": [{"type": "text", "text": "hi"}]},
            44,
        )
        frame = writer.find(req_id=44)
        assert frame is not None and frame["error"]["code"] == JSONRPC_INVALID_PARAMS
...
class TestElicitation:
    ...
    @pytest.mark.asyncio
    async def test_form_elicitation_has_no_transport_deadline(
        self, monkeypatch: pytest.MonkeyPatch
    ) -> None:
        seen: dict[str, Any] = {}
        ...
        h = _Harness(handler)
        h.server._form_elicitation_supported = True
        original_send_request = h.transport.send_request

        async def finite_default(
            method: str,
            params: dict[str, Any],
            *,
            ...
            return await original_send_request(method, params, timeout=timeout)

        monkeypatch.setattr(h.transport, "send_request", finite_default)
        await h.start()
        sid = await _new_session(h)
        h.send(
            {
