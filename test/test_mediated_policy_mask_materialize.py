...
def test_refuses_a_symlink_at_the_policy_name(tmp_path, monkeypatch):
    ...
    policy. Cross-platform: the sandbox decision is driven by ``os.lstat``'s
    no-follow mode bits, so the symlink condition is injected by mocking
    ``os.lstat`` for the target rather than planting a real symlink (which needs
    privilege on Windows) — the sandbox logic under test runs identically."""
    target = tmp_path / POLICY_FILENAME
    real_lstat = os.lstat

    def _lstat(path, *a, **k):
        if str(path) == str(target):
            base = real_lstat(tmp_path)  # borrow a real stat_result, flip the type
            fields = list(base)
            fields[0] = (fields[0] & ~0o170000) | stat.S_IFLNK  # st_mode -> symlink
            return os.stat_result(fields)
        return real_lstat(path, *a, **k)

    target.write_text('{"version":1,"authorizations":{}}', encoding="utf-8")
...
def test_refuses_a_hardlinked_policy_file(tmp_path, monkeypatch):
    ...
    the alias writable. It must fail closed. Cross-platform via a mocked
    ``os.lstat`` reporting ``st_nlink == 2`` (a real hard link is POSIX-only in
    practice); the sandbox's regular-file + nlink check runs identically."""
    target = tmp_path / POLICY_FILENAME
    target.write_text('{"version":1,"authorizations":{}}', encoding="utf-8")
    real_lstat = os.lstat
    ...
    def _lstat(path, *a, **k):
        if str(path) == str(target):
            base = real_lstat(target)
            fields = list(base)
            fields[3] = 2  # st_nlink == 2 (same inode reachable at another path)
            return os.stat_result(fields)
        return real_lstat(path, *a, **k)
...
def test_create_race_revalidates_and_fails_closed_on_a_non_regular_winner(tmp_path, monkeypatch):
    ...
    must fail closed rather than be trusted. Cross-platform: the race is lost by
    planting a plain file and the winner's non-regular (symlink) type is injected
    via a mocked ``os.lstat`` — no real symlink needed, and the sandbox's
    re-validation branch runs identically on every platform."""
    import errno
    ...
    def racing_open(path, flags, *a, **k):
        ...
        # then leaving a non-regular winner (what a hostile racer would plant).
        if str(path) == str(target) and (flags & os.O_EXCL):
            target.write_text("", encoding="utf-8")  # lose the race with a real file
            raise FileExistsError(errno.EEXIST, "exists")
        return real_open(path, flags, *a, **k)

    def _lstat(path, *a, **k):
        # The race winner re-validates as a SYMLINK (non-regular) -> must be refused.
        if str(path) == str(target) and target.exists():
            base = real_lstat(target)
            fields = list(base)
            fields[0] = (fields[0] & ~0o170000) | stat.S_IFLNK
            return os.stat_result(fields)
        return real_lstat(path, *a, **k)
