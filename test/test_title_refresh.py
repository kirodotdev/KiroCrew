...
class TestResumeTitleEchoIsNotAPin:
    """The sidebar's resume call ALWAYS sends a title (the existing name, or
    the slot key as placeholder). An echo must NOT be classified as a user
    pin — that would disable the background refresh on every ordinary resume.
    Only an actual override pins."""
    ...
    @pytest.mark.asyncio
    async def test_actual_override_still_pins(self, tmp_path, monkeypatch):
        from aiohttp.test_utils import TestClient, TestServer
        from chat_test_helpers import _make_app, _make_state
        ...
        async with TestClient(TestServer(_make_app(state))) as client:
            resp = await client.post(
                "/api/chat/slots/s1/resume",
                json={"key": "dashboard_s1", "title": "Deliberate new name"},
            )
            assert resp.status == 200
            slot = state._slots["s1"]
            assert slot.title == "Deliberate new name"
            assert slot._title_origin == _TITLE_ORIGIN_USER
