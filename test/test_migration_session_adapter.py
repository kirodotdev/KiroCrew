...
async def test_adapter_conforms_to_the_plan_side_seam():
    ...
    for m in ("requirements", "serialize"):
        assert hasattr(a, m)
...
@pytest.mark.asyncio
async def test_no_requirement_is_invented_for_a_sessions_mcp_servers():
    """Nothing can say which servers a session loaded, so nothing claims to.

    `mcp_discovery.list_servers` takes no arguments and returns the host's whole
    configured inventory, disclaiming any knowledge of what kiro-cli loads at
    session time. A requirement built from that would block on servers the
    session never touched, so the adapter emits none — even when a caller hands
    it a key by that name.
    """
    a = _sess_adapter({"transcript": ["hi"], "mcp_servers": ["kirocrew-core", "kirocrew-cron"]})
    reqs = await a.requirements("sess-1")
    assert [r for r in reqs if r.kind == "mcp_server"] == []
...
async def test_session_requirements_do_not_invent_a_requirement_from_a_blank():
    a = _sess_adapter({"transcript": ["hi"], "agent": "", "project": ""})
    assert await a.requirements("sess-1") == []
