...
calls in general. The tree carries ~116 raw POSIX call sites (``fcntl``,
...
``platform_compat`` shim table in ``AGENTS.md`` and enforced at review time.
What makes signal-0 special is that getting it wrong is *destructive* rather
...
GATED_PROBES: dict[str, str] = {
    ...
    # line of the function, and moot there anyway: kill_process_tree already
    # used `taskkill /T` to walk the whole child tree, so nothing is left to
    # sweep. The function docstring states this.
...
def test_app_backend_pid_alive_uses_the_shim() -> None:
    """Regression pin for the fixed site.

    ``apps/backend.py:_pid_alive`` was the last raw probe in the tree. It is
    called from the stale-reap escalation loop, which is otherwise fully
    shim-routed (``platform_compat.kill_process_tree``) and therefore clearly
    intended to run on Windows. It was only ever *latently* dangerous because
    ...
    mask that a Windows ``_proc_start_time`` would have removed.
    """
    ...
    assert "pid_exists" in ast.get_source_segment(
        src, fn
