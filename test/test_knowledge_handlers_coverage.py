...
class TestSyncSourceAgentBranch:
    ...
    async def test_missing_pipeline_is_503(self, store):
        ...
        # Dependencies are validated before the 'syncing' mark, so a failed
        # sync leaves the source retryable instead of stranded in 'syncing'.
        row = store.db.execute("SELECT sync_status FROM sources WHERE id = ?", (sid,)).fetchone()
        assert row["sync_status"] != "syncing"

    @pytest.mark.asyncio
    async def test_missing_fetch_pool_is_503(self, store):
        ...
        # The 503 must not strand the source in 'syncing' -- a retry is not 409.
        row = store.db.execute("SELECT sync_status FROM sources WHERE id = ?", (sid,)).fetchone()
        assert row["sync_status"] != "syncing"
