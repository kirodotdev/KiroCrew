...
class TestBatchIdentity:
    ...
    async def test_a_drained_rejection_is_announced(self):
        ...
        owned = any(owner.id == "q-reject" for owner in mgr._report_owners.values())
        assert owned or [i.id for i in announced] == [
            "q-reject"
...
class TestWaveDigest:
    ...
    async def test_a_queued_hand_off_is_not_confirmed_until_the_turn_runs(self):
        ...
        with patch(
            ...
            discard()
            await _settle(lambda: bool(settled), what="queued completion discard settled")
