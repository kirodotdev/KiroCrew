...
async def test_workload_m2m_cron_injects_live_loopback_proxy(
    ...
    servers = session_gateway_servers("cron:job1")
    assert len(servers) == 1
    # Each inject registers its own generation, so compare the shape and the
    # bound key rather than a second freshly minted header set.
    expected = acp_http_server(listen, headers=proxy_auth_headers("cron:job1"))
    assert {k: v for k, v in servers[0].items() if k != "headers"} == {
        k: v for k, v in expected.items() if k != "headers"
    }
    got = {pair["name"]: pair["value"] for pair in servers[0]["headers"]}
    assert set(got) == {
        sigv4.PROXY_AUTH_HEADER,
        sigv4.PROXY_SESSION_HEADER,
        sigv4.PROXY_GENERATION_HEADER,
    }
    assert got[sigv4.PROXY_SESSION_HEADER] == "cron:job1"
    assert got[sigv4.PROXY_AUTH_HEADER] == sigv4.bound_proxy_auth_token(
        "proxy-test-token", "cron:job1", "", got[sigv4.PROXY_GENERATION_HEADER]
    )
