"""CLI move verbs for task-run migration.

Task 4.8. Drives the same code path argparse would, with the stores faked, so no
real runs.json / dashboard is touched.

`taskrun move` is fully functional from the CLI: runs.json is on disk, so the
plan is real. A session move has no CLI verb — a session bundle must be
snapshotted from the live slot (Layer A flush consistency plus the Layer B
join), which the CLI cannot do safely against a running session, so the
dashboard is the only surface that plans one.
...
def test_taskrun_move_refuses_a_malformed_task_collection_at_the_mid_run_check(tmp_path, capsys):
    """The mid-run refusal reads the task list BEFORE the serializer does.

    Guarding each reader separately is what left this path crashing after the
    other two were fixed, so the shape is validated once in the shared accessor
    and this test pins the earliest reader.
    """
    import kiro_crew.cli_commands as cc

    rec = _run_record()
    rec["task_details"] = "not-a-list"
    runs = _runs_file(tmp_path, rec)
    args = argparse.Namespace(
        command="taskrun",
        taskrun_action="move",
    ...
    assert "task list is str" in err


def test_taskrun_move_refuses_a_malformed_commit_hash_field(tmp_path, capsys):
    """`len` on a number raises TypeError, which a CLI user sees as a traceback.

    Sibling of the task-list check: the same registry can carry a scalar here.
    """
    import kiro_crew.cli_commands as cc
    ...
    assert exc.value.code == 1
    err = capsys.readouterr().err
    assert "commit_hashes is int" in err


def test_taskrun_move_refuses_a_malformed_task_entry(tmp_path, capsys):
    """A scalar inside the task list must not reach `dataclasses.asdict`.

    The outer list shape is validated, but a scalar ENTRY still crashed the
    serializer, so a hand-edited or truncated registry produced a traceback
    instead of a sentence naming the bad record.
    """
    import kiro_crew.cli_commands as cc

    rec = _run_record()
    rec["task_details"] = ["not-a-task"]
    runs = _runs_file(tmp_path, rec)
    args = argparse.Namespace(
        command="taskrun",
        taskrun_action="move",
    ...
    assert exc.value.code == 1
    err = capsys.readouterr().err
    assert "str entries" in err and "mapping or a task record" in err
