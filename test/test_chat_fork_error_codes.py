...
@pytest.mark.asyncio
async def test_channel_backed_fork_denial_survives_an_audit_failure(tmp_path, monkeypatch) -> None:
    """A raising ``sel()`` must not convert the channel-backed 404 into a 500:
    a 500 for a channel-backed slot next to a 404 for a missing one is exactly
    the distinguishing signal the shared 404 exists to hide."""
    monkeypatch.setattr("kiro_crew.dashboard.state.config_dir", lambda: tmp_path)
    state = _seeded_state(tmp_path)
    ...
    monkeypatch.setattr("kiro_crew.dashboard.chat_fork.sel", _boom)
    ...
    async with TestClient(TestServer(app)) as client:
        resp = await client.post("/api/chat/slots/linkedslot/fork", json={})
