...
def test_the_ratchet_can_actually_fail() -> None:
    ...
    corpus-read refusals answer through ``chat_utils.history_corpus_unreadable``,
    which sets the code by construction, so the scanner does not see them here —
    a stronger guarantee than a per-site scan. There is no ``slot_not_persistent``
    site: an ordinary fork of an incognito or temporary session succeeds and the
    child inherits its mode. The memory-mode-specific refusals are
    ``fork_source_memory_mode_invalid`` (a parent whose persisted mode is outside
    the allowlist) and the promotion escape hatch's own three
    (``slot_already_persistent`` and ``promote_requires_dashboard_user`` from
    ``api_chat_slot_promote``, and ``promote_save_failed`` when the confirmed
    destination save fails and the half-made child is rolled back), among the
    30 total.
    """
