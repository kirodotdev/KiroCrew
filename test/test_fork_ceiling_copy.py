"""The over-capacity fork refusal enforces a ceiling it reports nowhere.

A row count is not an affordance in a transcript that numbers nothing, so the banner
names the ACTION that shrinks the slice and no number at all. The copy states no count,
so the wire field has no reader: the refusal carries only the machine-readable code that
selects that copy. This pins the absence from both sides -- no field, and no catalog
stating a count -- while the ceiling stays a positive constant enforced server-side.
"""
