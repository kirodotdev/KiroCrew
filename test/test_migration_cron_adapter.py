...
@pytest.mark.asyncio
async def test_the_memory_identity_is_reported_rather_than_silently_retired():
    """A dropped host-resolved binding must appear in the plan.

    `member_id` and `memory_store` are resolved against the host, so shipping
    the values either fails closed at fire time or binds the job to a store the
    target's owner never granted. Dropping them is right — dropping them
    SILENTLY is not, because the user would learn only when the job next fires.
    """
    job = _job(id="cj-mem", member_id="member-7", memory_store="store-a")
    adapter = CronMigrationAdapter(job_lookup={"cj-mem": job})

    reqs = await adapter.requirements("cj-mem")
    by_kind = {r.kind: r.identity for r in reqs}
    assert by_kind["crew_member"] == "member-7"
    assert by_kind["memory_store"] == "store-a"
    assert all(r.severity == "blocking" for r in reqs if r.kind in ("crew_member", "memory_store"))

    # And the values themselves do not travel.
    payload = await adapter.serialize("cj-mem")
    assert "member_id" not in payload and "memory_store" not in payload
