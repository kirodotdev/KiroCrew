...
class TestSshTunnelManager:
    ...
    @pytest.mark.asyncio
    async def test_disabling_loopback_during_rearm_keeps_revocation_terminal(
        self, tmp_path, monkeypatch
    ):
        from kiro_crew.config.loader import KiroCrewConfig
        from kiro_crew.instances import ssh_tunnel_manager as stm
        from kiro_crew.instances.registry import CONNECTION_METHOD_LOOPBACK
        ...
        tunnel = stm._SshTunnel(
            "lb-1",
            "",
            53400,
            7901,
            transport=CONNECTION_METHOD_LOOPBACK,
        )
        ...
        monkeypatch.setattr(tunnel, "_wait_until_ready", not_ready)
        tunnel._probe_task = asyncio.create_task(parked_probe())
        await asyncio.sleep(0)
        recovery = asyncio.create_task(
            mgr._restore_for_recovery(
                inst,
        ...
        mgr._track_recovery("lb-1", recovery)
        ...
        assert tunnel.loopback_ownership_ok() is False
        assert tunnel._ownership_condemned is True
        assert "lb-1" not in mgr._tunnels

    @pytest.mark.asyncio
    async def test_rearm_refuses_a_revoked_loopback_tunnel(self, monkeypatch):
        from kiro_crew.instances import ssh_tunnel_manager as stm
        from kiro_crew.instances.registry import CONNECTION_METHOD_LOOPBACK
        ...
        tunnel._forwarder = _Listener()
        tunnel._ownership_condemned = True
        tunnel._revoked = True
        monkeypatch.setattr(tunnel, "_wait_until_ready", ready)
        monkeypatch.setattr(stm, "_PROBE_INTERVAL", 0)

        assert await tunnel.rearm_loopback() is False
        assert tunnel._ownership_condemned is True

    @pytest.mark.asyncio
    async def test_disabling_loopback_cancels_each_in_flight_recovery(self, tmp_path, monkeypatch):
        from kiro_crew.config.loader import KiroCrewConfig
        from kiro_crew.instances.registry import CONNECTION_METHOD_LOOPBACK

        reg, mgr = self._mgr(tmp_path)
        instance_ids = ("lb-1", "lb-2")
        cancelled: list[str] = []
        recoveries = []

        async def parked_recovery(instance_id: str):
            try:
                await asyncio.sleep(30)
            except asyncio.CancelledError:
        ...
        for index, instance_id in enumerate(instance_ids):
            ...
            mgr._tunnels[instance_id] = _FakeTunnel(
                instance_id,
                "",
                53400 + index,
                7901 + index,
            ...
            recovery = asyncio.create_task(parked_recovery(instance_id))
            recoveries.append(recovery)
            mgr._track_recovery(instance_id, recovery)
        await asyncio.sleep(0)
        ...
        try:
            await cleanup
            assert sorted(cancelled) == sorted(instance_ids)
            assert not any(instance_id in mgr._recovery_by_instance for instance_id in instance_ids)
            assert not mgr._reconfiguring.intersection(instance_ids)
        finally:
...
class TestLoopbackTransport:
    ...
    @pytest.mark.parametrize("remote_port", [70000, 0])
    def test_pod_config_rejects_out_of_range_loopback_remote_port(
        self, tmp_path, monkeypatch, remote_port
    ):
        """The params gate rejects an agent-written port before any loopback dial."""
        from kiro_crew.instances.registry import (
            CONNECTION_METHOD_LOOPBACK,
            Instance,
            InstancesRegistry,
        ...
        with pytest.raises(
            ...
            match=rf"loopback remote_port must be 1-65535, got {remote_port}",
        ):
            SshTunnelManager(InstancesRegistry(tmp_path))._resolve_transport(inst)

    def test_pod_config_can_resolve_the_loopback_transport(self, tmp_path, monkeypatch):
        """The pod verification seam remains usable with a valid stored port."""
        from kiro_crew.instances.constants import LOOPBACK_HOST
        from kiro_crew.instances.registry import (
            CONNECTION_METHOD_LOOPBACK,
            Instance,
        ...
        from kiro_crew.instances.ssh_tunnel_manager import SshTunnelManager
        from kiro_crew.instances.validation import validate_loopback_port
        ...
        assert validate_loopback_port("7910") == 7910
