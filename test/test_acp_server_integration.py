...
class _ScriptHooks:
    async def fire(self, _event: str, **_kwargs: Any) -> list[Any]:
        return []
...
class _Svc:
    def __init__(self, sessions: _Sessions, ctx: _Ctx) -> None:
        ...
        self.script_hooks = _ScriptHooks()
...
class TestFullStackDiffReview:
    ...
    async def test_accept_flows_through_to_provider(self) -> None:
        ...
        assert text["params"]["update"]["content"]["text"].rstrip() == "patching now"
