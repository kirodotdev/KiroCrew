...
def _make_dir_link(link: "os.PathLike[str] | str", target: "os.PathLike[str] | str") -> None:
    """Create a reparse point at ``link`` resolving to directory ``target``.

    A directory symlink needs SeCreateSymbolicLinkPrivilege on Windows (WinError
    1314 in an unelevated shell); a junction needs none and is followed by the
    same reparse machinery (``resolve``/``rglob``/``GetFinalPathNameByHandleW``).
    So the containment ratchet stays EXERCISED on an ordinary unelevated Windows
    run instead of silently skipped. Mirrors ``conftest.make_dir_link``.
    """
    if _IS_WINDOWS:
        import _winapi
...
def _make_escaping_file_link(
    link_dir: "os.PathLike[str] | str", link_name: str, outside_file: "os.PathLike[str] | str"
) -> str:
    """Return a path, inside ``link_dir``, to ``outside_file`` reached THROUGH a
    reparse point that escapes ``link_dir``.

    On POSIX this is a direct file symlink ``link_dir/link_name -> outside_file``.
    On Windows a *file* symlink also needs elevation, but a directory JUNCTION
    does not: junction ``link_dir/<link_name>__esc`` at a private outside
    directory holding ONLY a copy of the target file (so the enumerator finds
    exactly one escaping file through it, not whatever else shares tmp_path), and
    reach the file through it. Either way the resolved path escapes ``link_dir``
    and exercises the identical resolve()+containment assertion — unelevated,
    never skipped.
    """
    link_dir = os.fspath(link_dir)
    outside_file = os.fspath(outside_file)
    if _IS_WINDOWS:
        import shutil

        import _winapi

        base = os.path.basename(outside_file)
        # A private outside dir holding only this target, so the junction exposes
        # exactly one file to the enumerator.
        priv = outside_file + "__escdir"
        os.makedirs(priv, exist_ok=True)
        shutil.copy2(outside_file, os.path.join(priv, base))
        junction = os.path.join(link_dir, link_name + "__esc")
        _winapi.CreateJunction(priv, junction)
        return os.path.join(junction, base)
...
class TestOrphanEvidenceScan:
    ...
    def test_symlinked_entry_escaping_root_is_rejected(self, tmp_path):
        ...
        # Reach the outside file through a reparse point that escapes the entries
        # root — file symlink on POSIX, directory junction on Windows (unelevated).
        _make_escaping_file_link(self.entries_dir, "escape.json", outside)
        result = gate.run_scan()
        assert not result.ok
        assert any(
    ...
    def test_symlinked_evidence_log_escaping_root_is_rejected(self, tmp_path):
        ...
        _make_escaping_file_link(self.runs_dir, "github.jsonl", outside)
        result = gate.run_scan()
        assert not result.ok
        assert any(
    ...
    def test_symlinked_evidence_root_escaping_repo_is_not_read(self, tmp_path, monkeypatch):
        ...
        # Replace the runs root with a link pointing outside the repo tree
        # (junction on Windows, symlink on POSIX) so the ratchet runs unelevated.
        self.runs_dir.rmdir()
        _make_dir_link(self.runs_dir, outside_dir)
        opened: list[str] = []
        real_open = open
    ...
    def test_symlinked_entries_root_escaping_repo_is_not_read(self, tmp_path, monkeypatch):
        ...
        _make_dir_link(self.entries_dir, outside_dir)
        opened: list[str] = []
        real_open = open
    ...
    def test_load_jsonl_refuses_symlinked_log(self, tmp_path):
        ...
        outside = tmp_path / "outside_evidence.jsonl"
        outside.write_text('{"run_id": "leaked"}\n', encoding="utf-8")
        # _runs_for_service reads the fixed path runs/github.jsonl directly, so
        # the reparse must sit ON that path. POSIX: a file symlink at it. Windows
        # (file symlink needs elevation): replace the runs dir with a junction to
        # an outside dir that holds github.jsonl, so runs/github.jsonl is reached
        # THROUGH the reparse point and _path_reached_via_symlink still fires.
        if _IS_WINDOWS:
            outside_runs = tmp_path / "outside_runs_dir"
            outside_runs.mkdir()
            (outside_runs / "github.jsonl").write_text('{"run_id": "leaked"}\n', encoding="utf-8")
            self.runs_dir.rmdir()
            _make_dir_link(self.runs_dir, outside_runs)
        else:
            os.symlink(outside, self.runs_dir / "github.jsonl")
    ...
    def test_symlinked_service_subdir_entry_not_read(self, tmp_path, monkeypatch):
        ...
        _make_dir_link(self.entries_dir / "github", outside_svc)
        opened: list[str] = []
        real_open = open
    ...
    def test_symlinked_catalog_evidence_is_rejected(self, tmp_path, monkeypatch):
        ...
        # CATALOG_EVIDENCE points at the fixed link path. POSIX: a file symlink
        # there. Windows (no elevation): make cat_dir a junction to an outside
        # dir holding catalog-evidence.json, so the path is reached THROUGH the
        # reparse point and the containment check still fires.
        if _IS_WINDOWS:
            outside_dir = tmp_path / "outside_cat_dir"
            outside_dir.mkdir()
            (outside_dir / "catalog-evidence.json").write_text(
                '{"shared_contracts": {}}', encoding="utf-8"
            )
            cat_dir.rmdir()
            _make_dir_link(cat_dir, outside_dir)
        else:
...
class TestCatalogEvidenceCounts:
    ...
    def test_unknown_shared_contracts_family_is_rejected(self):
        # An extra key under shared_contracts (outside the nine families and the
        # allowed metadata note) holding contract records must fail closed: the
        # count iterates only the nine families, so an unknown family would go
        # uncounted and let the gate certify more than 72 records.
        cat = self._valid_catalog()
        cat["shared_contracts"]["ROGUE"] = [{"contract_id": "ROGUE-1"}]
        self._point_at(cat)
        result = gate.run_scan()
        assert not result.ok
        assert any(f.field == "shared_contracts.ROGUE" for f in result.findings), result.findings

    def test_allowed_metadata_key_is_not_rejected(self):
        # The one named metadata note is allowed and must NOT trip the unknown-key
        # check (no over-tightening on the real catalog's shape).
        self._point_at(self._valid_catalog())
        result = gate.run_scan()
        assert not any(
            "unknown shared_contracts" in f.message for f in result.findings
        ), result.findings
...
class TestRound26UrlUserinfoRejected:
    """The URL predicate must inspect every component that can carry
    author-supplied data, not only the host: userinfo (username/password) is
    the dedicated credential component, so ``_is_resolvable_https_url`` rejects
    a URL carrying it — a ``https://user:secret@example.com`` citation would
    otherwise write a credential into the public immutable evidence log."""

    def test_userinfo_credential_rejected(self):
        # user:pass@, user@, and :pass@ all carry userinfo -> reject.
        assert not gate._is_resolvable_https_url("https://user:secret@example.com/schema.json")
        assert not gate._is_resolvable_https_url("https://user@example.com/schema.json")
        assert not gate._is_resolvable_https_url("https://:secret@example.com/schema.json")

    def test_legit_url_structure_still_passes(self):
        # PER-COMPONENT decision: path/query/fragment/port are legitimate URL
        # structure and stay unconstrained. A credential-free citation with a
        # path + query + fragment + explicit port must still resolve.
        assert gate._is_resolvable_https_url("https://example.com/schemas/v1.json")
        assert gate._is_resolvable_https_url("https://example.com:8443/schemas/v1.json")
        assert gate._is_resolvable_https_url("https://example.com/a/b.json?ref=abc&v=2#defs")


class TestRound26CrossDriveRelpathSafe:
    """``os.path.relpath(x, REPO_ROOT)`` across drives (Windows repo on one
    drive, toolcache/temp on another) raises ``ValueError``; ``_repo_rel``
    guards every such call uniformly and falls back to the absolute path, so a
    cross-drive input never aborts the gate. Both cross-drive path operations
    (``relpath`` and ``commonpath``) are guarded, not just one."""

    def test_repo_rel_survives_cross_drive(self, monkeypatch):
        # Emulate ntpath's cross-drive ValueError regardless of host platform.
        import ntpath

        monkeypatch.setattr(gate.os.path, "relpath", ntpath.relpath)
        # Two different drives -> ntpath.relpath raises; _repo_rel must NOT.
        result = gate._repo_rel(r"C:\hostedtoolcache\entries\github\op.json")
        assert isinstance(result, str) and result  # fell back, did not raise

    def test_repo_rel_same_drive_returns_relative(self, monkeypatch):
        import ntpath

        monkeypatch.setattr(gate.os.path, "relpath", ntpath.relpath)
        monkeypatch.setattr(gate, "REPO_ROOT", r"D:\a\KiroCrew")
        # Same drive -> a normal relative path, no fallback.
        result = gate._repo_rel(r"D:\a\KiroCrew\connectors\entries\gh\op.json")
        assert result == r"connectors\entries\gh\op.json"


class TestRound28TimestampRequiresTime:
    """A `timestamp` field is a point in time, not a calendar day.
    ``datetime.fromisoformat`` accepts a bare date (no time component) as
    midnight, so ``_is_valid_timestamp`` requires a date-AND-time value and
    rejects a date-only string — the complete predicate for the field, not
    half of it."""

    def test_date_only_rejected(self):
        assert not gate._is_valid_timestamp("2026-01-01")

    def test_real_datetime_forms_accepted(self):
        # Every real datetime form still passes — no over-tightening.
        assert gate._is_valid_timestamp("2026-01-01T00:00:00Z")
        assert gate._is_valid_timestamp("2026-01-01T12:34:56+00:00")
        assert gate._is_valid_timestamp("2026-01-01 12:34:56")

    def test_non_chronological_still_rejected(self):
        assert not gate._is_valid_timestamp("not-a-time")
        assert not gate._is_valid_timestamp("")


class TestRound30ExclusionReasonTypeAnyRow:
    """`exclusion_reason` is typed `string | null` on EVERY matrix row,
    independent of the applicable branch. A type check gated only inside the
    applicable=false branch lets an applicable=true row carry a non-null
    non-string (e.g. 42) unvalidated — a malformed field passing the gate."""
    ...
    def _entry_with_row(self, row):
        return _entry(
            ...
            account_types=["personal"],
            surfaces=["chat"],
            evidence_by_mode_surface_and_auth=[row],
        )

    def test_applicable_true_non_string_exclusion_reason_rejected(self):
        entry = self._entry_with_row(self._row(exclusion_reason=42))
        result = gate.validate_entry(entry, _REF)
        assert any(
            f.field == "exclusion_reason" and "string or null" in f.message for f in result.findings
        ), result.findings

    def test_applicable_true_null_exclusion_reason_has_no_type_finding(self):
        # null is a valid exclusion_reason on an applicable=true row — no over-tightening.
        entry = self._entry_with_row(self._row(exclusion_reason=None))
        result = gate.validate_entry(entry, _REF)
        assert not any(
            f.field == "exclusion_reason" and "string or null" in f.message for f in result.findings
        ), result.findings
...
class TestEntriesNotRemoved:
    """GPT F1 :3253 — the head-only entry enumeration lets a PR delete/replace a
    committed manifest entry without the scan or catalog counts noticing. The
    base-vs-head removal check must reject a disappeared entry, while allowing an
    edited entry and a newly-added one. Uses a real temp git repo so the
    git-ls-tree base listing is exercised."""
    ...
    def _entries_dir(self, tmp_path):
        d = tmp_path / "docs/system-specs/connector-manifest/entries/github"
    ...
    def _point_roots(self, tmp_path, monkeypatch):
        monkeypatch.setattr(gate, "REPO_ROOT", str(tmp_path))
        monkeypatch.setattr(
            gate, "ENTRIES_ROOT", str(tmp_path / "docs/system-specs/connector-manifest/entries")
        )

    def _base_sha(self, tmp_path):
        import subprocess as sp
    ...
    def test_removal_rejected_edit_and_add_allowed(self, tmp_path, monkeypatch):
        run = self._init_repo(tmp_path)
        d = self._entries_dir(tmp_path)
        (d / "op_a.json").write_text('{"operation_id": "op_a"}', encoding="utf-8")
        (d / "op_b.json").write_text('{"operation_id": "op_b"}', encoding="utf-8")
        run("git", "add", "-A")
        run("git", "commit", "-q", "-m", "base")
        base_sha = self._base_sha(tmp_path)
        self._point_roots(tmp_path, monkeypatch)
        monkeypatch.setenv("MANIFEST_APPEND_ONLY_BASE_REF", base_sha)

        # (a) edit op_a's content + add a new op_c — no removal, must NOT flag.
        (d / "op_a.json").write_text('{"operation_id": "op_a", "edited": true}', encoding="utf-8")
        (d / "op_c.json").write_text('{"operation_id": "op_c"}', encoding="utf-8")
        combined = gate.ValidationResult(ok=True)
        gate._check_entries_not_removed(combined)
        assert not any(f.field == "<entries-removal>" for f in combined.findings), combined.findings

        # (b) DELETE op_b (present at base, gone at head) — must be rejected.
        (d / "op_b.json").unlink()
        combined = gate.ValidationResult(ok=True)
        gate._check_entries_not_removed(combined)
        assert any(
            "op_b.json" in f.entry_ref and "REMOVED or renamed" in f.message
            for f in combined.findings
        ), combined.findings

        # (c) RENAME op_a -> op_a_new (a replacement) — old path removal rejected.
        (d / "op_a.json").rename(d / "op_a_new.json")
        combined = gate.ValidationResult(ok=True)
        gate._check_entries_not_removed(combined)
        assert any(
            "op_a.json" in f.entry_ref and "REMOVED or renamed" in f.message
            for f in combined.findings
        ), combined.findings

    def test_no_base_ref_skips(self, tmp_path, monkeypatch):
        self._init_repo(tmp_path)
        self._entries_dir(tmp_path)
        self._point_roots(tmp_path, monkeypatch)
        monkeypatch.delenv("MANIFEST_APPEND_ONLY_BASE_REF", raising=False)
        combined = gate.ValidationResult(ok=True)
        gate._check_entries_not_removed(combined)
    ...
    def test_ls_tree_error_after_resolve_fails_closed(self, tmp_path, monkeypatch):
        run = self._init_repo(tmp_path)
        d = self._entries_dir(tmp_path)
        (d / "op_a.json").write_text('{"operation_id": "op_a"}', encoding="utf-8")
        run("git", "add", "-A")
        ...
        assert not combined.ok
        assert any(
            f.field == "<entries-removal>" and "failing closed" in f.message
            for f in combined.findings
        ), combined.findings
