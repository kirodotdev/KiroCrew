"""The ledger reference documents types the code actually owns, with valid examples.
...
_DOCS = Path(__file__).parent.parent / "docs" / "reference" / "ledger"
...
def test_session_examples_carry_no_crew_only_envelope_fields(block: str) -> None:
    ...
    assert "thread" not in raw, "thread is the crew ledger's shape"
...
def test_session_examples_name_only_a_session_emitter(block: str) -> None:
    ...
    that writes to crew ledgers. That makes it precisely the mistake a reference
...
def _emit_functions_called_in_the_tree() -> frozenset[str]:
    ...
    for path in root.rglob("*.py"):
        if path.name == "session_ledger_emit.py":
            continue
        try:
        ...
        if "session_ledger_emit" not in text:
            continue
