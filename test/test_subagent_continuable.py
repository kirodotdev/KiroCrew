...
class TestContinuationMemoryMode:
    ...
    async def test_fresh_manager_restores_and_tightens_original_mode(self, original, requested):
        ...
        with _mode_publication_completion(conv_id) as mode_published:
            ...
            await _await_continuation_run(manager._tasks[info.id])
        ...
        with _mode_publication_completion(conv_id) as mode_published:
            ...
            await _await_mode_publication(mode_published)
            await _await_continuation_run(restarted._tasks[resumed.id])
    ...
    async def test_missing_original_policy_never_allocates_provider(self):
        ...
        sessions = _mock_sessions(resumed=True)
        manager = _manager(sessions)
        # The ASYNC entry for the same reason as the sibling test above. Its
        # protected-mode attempt has the same explicit completion signal before
        # the terminal run deadline starts.
        with _mode_publication_completion("missing-resume-policy") as mode_published:
            info = await manager.continue_conversation_async(
                "missing-resume-policy", "must not run"
            )
            assert info is not None
            await _await_mode_publication(mode_published)
            # ``.get``, not ``[...]``: this refusal is raised on the run's first
            # steps and can be popped from ``_tasks`` before dispatch returns.
