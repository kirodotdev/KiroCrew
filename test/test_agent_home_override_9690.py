...
class TestAppAgentWritersHonourOwnership:
    ...
    def test_deregister_and_prune_leave_the_shared_dir_alone(self, monkeypatch, tmp_path):
        ...
        denied = self._denials(events)
        assert [e["source"] for e in denied] == ["deregister_agents", "prune_stale_app_resources"]
...
class TestAdoptedHomeKeepsSessionMap:
    ...
    def test_a_failure_while_copying_keeps_the_source_and_leaves_no_partial_file(
        ...
        """The bytes go into a temp file beside the destination and take the
        final name only once complete, so a copy that dies leaves the source in
        place, nothing under the final name and no ``.part`` behind -- the next
