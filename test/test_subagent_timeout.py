...
@pytest.mark.asyncio
async def test_run_timeout_does_not_advertise_uncommitted_future_deadline(timeout_state):
    import asyncio
    import time
...
@pytest.mark.asyncio
async def test_near_limit_success_queues_future_manager_deadline(timeout_state):
    import time
    from unittest.mock import AsyncMock, MagicMock, patch
...
@pytest.mark.asyncio
async def test_failed_persistence_does_not_publish_or_advance_visible_default(
    timeout_state,
    monkeypatch,
):
    import time
    ...
    info = SubagentInfo(id="failed-timeout-persist", task="test", timeout_secs=60)
    info._exec_started = time.time() - 60
    ...
    visible = manager._observe_timeout_usage(info, completed=False)
    tasks = tuple(manager._timeout_persist_tasks)

    assert visible == 60
    assert manager._default_timeout == 60
    assert manager._timeout_policy.current_secs == 60

    await manager._drain_timeout_persist()

    assert timeout.read_learned_timeout() is None
    assert manager._default_timeout == 60
    assert manager._timeout_policy.current_secs == 60
    assert all(not task.cancelled() and task.exception() is None for task in tasks)


@pytest.mark.asyncio
async def test_successful_persistence_publishes_only_after_durable_write(timeout_state):
    import asyncio
    import time
    from unittest.mock import MagicMock
    ...
    async def gated_persist(timeout_secs: int) -> int | None:
        entered.set()
        await release.wait()
        timeout.write_learned_timeout(timeout_secs)
        return timeout.read_learned_timeout()

    manager._persist_timeout_level = gated_persist
    info = SubagentInfo(id="gated-timeout-persist", task="test", timeout_secs=60)
    info._exec_started = time.time() - 60

    visible = manager._observe_timeout_usage(info, completed=False)
    await entered.wait()

    concurrent = SubagentInfo(id="concurrent-timeout-persist", task="test", timeout_secs=60)
    concurrent._exec_started = time.time() - 60
    concurrent_visible = manager._observe_timeout_usage(concurrent, completed=False)

    assert visible == 60
    assert concurrent_visible == 60
    assert len(manager._timeout_persist_tasks) == 1
    assert manager._default_timeout == 60
    assert timeout.read_learned_timeout() is None
    ...
    assert timeout.read_learned_timeout() == 1860
    assert manager._default_timeout == 1860
    assert manager._timeout_policy.current_secs == 1860


@pytest.mark.asyncio
async def test_timeout_persistence_is_ordered_and_consumes_task_failures(timeout_state):
    import asyncio
    from unittest.mock import MagicMock
    ...
    async def ordered_persist(timeout_secs: int) -> int | None:
    ...
    manager._persist_timeout_level = ordered_persist
    manager._schedule_timeout_persist(3600)
    manager._schedule_timeout_persist(5400)
    tasks = tuple(manager._timeout_persist_tasks)
    ...
    assert calls == [3600, 5400]
    assert peak_active == 1
    assert timeout.read_learned_timeout() == 5400
    assert manager._default_timeout == 5400
    assert not manager._timeout_persist_levels
    assert all(not task.cancelled() and task.exception() is None for task in tasks)
...
async def test_cancel_all_drains_in_flight_learned_timeout_write(timeout_state):
    ...
    schedules the detached write, then the process is torn down (``cancel_all``
    -> reexec) before the write lands. The write is manager-owned and drained by
    ``cancel_all``, so the level is published and on disk before the reexec and
    a fresh manager restores it — instead of reading back the stale lower floor.
    """
    import asyncio
    import time
    from unittest.mock import MagicMock
    ...
    async def _gated_persist(timeout_secs: int) -> int | None:
        entered.set()
        await release.wait()
        timeout.write_learned_timeout(timeout_secs)
        return timeout.read_learned_timeout()
    ...
    assert timeout.read_learned_timeout() == 3600
    assert manager._default_timeout == 3600
