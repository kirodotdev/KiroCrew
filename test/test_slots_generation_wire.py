...
A WebSocket frame serialized before a
...
def test_both_emitting_paths_draw_their_stamp_through_the_seam() -> None:
    ...
    Same for the epoch: drawn from the STATE, or a restart reuses one and reads comparable.
    """
    ...
    assert '": _state_slots_epoch(state)' in handlers_src, "the GET header stopped dating"
    assert '": _state_slots_epoch(state)' in ws_src, "the connect frame stopped dating"
    assert state_src.count("slots_epoch=_state_slots_epoch(self)") == 2, "a broadcast stopped"
...
def test_the_connect_time_frame_carries_the_pair_the_client_dates_by() -> None:
    ...
    ``_slots_ws_frame``. The keys ARE the server half of the contract -- no in-repo client
    reads them yet (``dashboardSlice``'s ``slotsGeneration`` is a local Redux counter, not
    this field), and a connect frame omitting them cannot be dated by the client that will.
    """
    ws_src = (
...
def test_the_generation_is_drawn_under_the_stamp_lock() -> None:
    ...
    state = DashboardState.__new__(DashboardState)
    state._slots_stamp_lock = RecordingLock()  # type: ignore[assignment]

    assert state.next_slots_generation() == 1
    assert state.next_slots_generation() == 2
    assert acquisitions == [1, 1], (
        "the counter was drawn without taking the stamp lock, so two emitting "
...
def test_a_closed_row_cannot_arrive_under_a_higher_generation() -> None:
    ...
    Forces the interleave INSIDE the atomic region: the first draw pauses between the
    stamp and the membership read, so a real stamp lock makes the competing emitter wait,
    while dropping the lock lets it draw the HIGHER number over pre-close rows. Its wait
    is BOUNDED, so a version that blocks it cannot deadlock this test.
    """
    import itertools
    ...
    real_draw = state.next_slots_generation
    ...
    state.next_slots_generation = _draw  # type: ignore[method-assign]
    ...
    f.start()
    assert foreign_is_reading.wait(10), "the foreign emitter never drew its stamp"
...
def test_the_stamp_pins_membership_without_serializing_under_the_lock() -> None:
    ...
    repo's no-blocking-call-on-event-loop anchor forbids.
    Asserted by trying to take the STAMP lock -- the one the atomic region holds -- FROM
    ANOTHER THREAD while serialization is in flight: it must succeed.
    """
    ...
    def _serialize(**kw: object) -> list:
        ...
        def _probe() -> None:
            got = state._slots_stamp_lock.acquire(timeout=2.0)
            lock_was_free.append(got)
            if got:
                state._slots_stamp_lock.release()
    ...
    assert lock_was_free == [True], (
        "the stamp lock was still held while the rows were serialized, so a GET or "
...
def test_every_audience_frame_is_serialized_from_one_captured_snapshot() -> None:
    ...
    evicts a live session's cached state.
