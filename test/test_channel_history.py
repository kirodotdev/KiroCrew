...
class TestObserveCompactionOffLoop:
    ...
    async def test_overlapping_reexecs_cannot_reopen_each_others_drain(self, monkeypatch):
        ...
        ``reexec_lane_guard`` holds one loop-bound lock across its whole
        ...
        from kiro_crew import platform_compat as pc_mod
        ...
        async def _attempt(name: str) -> None:
            try:
                async with ex_mod.reexec_lane_guard():
                    pc_mod.reexec_python_module("kiro_crew", [], executable=name)
            except OSError:
