...
def test_loader_init_does_no_private_root_filesystem_work(tmp_path):
    ...
    tree. The spawn-time masking guarantee that eager init used to provide
    now lives in the sandbox launcher itself (see sandbox.py: the child
...
def test_auto_apply_binding_uses_exact_staged_bytes(loader):
...
def test_auto_apply_refuses_body_mutated_after_binding_check(loader, monkeypatch):
    ...
    the current bytes and they no longer match the staged binding."""
...
def test_auto_apply_promotes_verified_bytes_not_post_redaction_file(loader, monkeypatch):
...
def test_staging_binds_validated_bytes_not_file_reread(loader, monkeypatch):
    ...
    monkeypatch.setattr(Path, "write_text", meta_write_then_tamper)
    _stage_update(
        loader,
        slug,
