...
def test_aggregate_cumulative_sums_keep_newest_and_add_across_pids(tmp_path: Path):
    """CUMULATIVE sums: newest snapshot per PID wins; PIDs add together.

    Observable counters (CPU seconds, GC stats) export a lifetime snapshot
    every cycle (temporality 2). Within one process the newest snapshot is the
    truth — re-emissions after a telemetry off/on provider rebuild are
    idempotent — and the cross-process total is the sum of per-PID snapshots.
    """
    ...
    # Newest per PID (150 and 30), summed across PIDs — never 100+150+10+30.
    assert rows[0]["total"] == 180.0
...
def test_aggregate_cumulative_detects_counter_reset_on_pid_reuse(tmp_path: Path):
    ...
    drops, so the earlier process's total is never overwritten: 100 then a
    restart reaching 30 totals 130.
    """
    metric = {
        "name": "kirocrew.process.cpu.seconds",
        "data": {
    ...
    rows = [o for o in result["other"] if o["name"] == "kirocrew.process.cpu.seconds"]
    assert rows and rows[0]["total"] == 130.0
