...
def test_observable_counters_survive_provider_rebuild(tmp_path):
    ...
    snapshot, and the aggregator keeps the newest snapshot per PID, so two
    providers writing to the same shard (an off/on toggle) still aggregate to
    the true value, never a doubled one.
    """
    ...
    # Keep-newest-per-PID: the answer is the newest lifetime snapshot (150) —
    # never 100+150 summed across rebuilds, and never inflated by the four
    # export cycles (100+100+150+150 = 500).
    assert rows[0]["total"] == 150.0, f"got {rows[0]['total']}"
