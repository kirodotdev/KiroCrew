...
class TestSaveOrEnrich:
    ...
    def test_unicode_case_variant_matches(self, tmp_path: Path) -> None:
        """casefold(), not lower(): lower() leaves 'ss' alone, so a stored 'Strasse'
        and a submitted upper-case form compared unequal, the enrich missed, and a
        duplicate was persisted under a different casing."""
        store = LessonStore(base_dir=tmp_path)
        store.save(_make_lesson("Stra\u00dfe", "tool"))

        assert store.save_or_enrich(_make_lesson("STRASSE", "tool", "Do not misspell")) == (
            "enriched"
        )

        records = store.load_all()
        assert len(records) == 1, "must enrich the stored row, not add a duplicate"
        assert records[0].rule == "Stra\u00dfe", "keeps the stored spelling"
