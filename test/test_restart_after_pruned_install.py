...
class TestLauncherOwnership:
    ...
    def test_dot_dot_after_a_hop_resolves_where_the_hop_led(self, tmp_path, monkeypatch):
        """``hop/../payload`` is resolved in kernel order, not collapsed first.

        ``trusted/kirocrew`` points at the RELATIVE ``hop/../payload`` and ``hop`` is
        a link into a directory we own. The kernel resolves ``hop``, then applies
        ``..``, landing in our directory; collapsing the text first yields
        ``trusted/payload`` and never looks there. Only our directory is attributed
        to us, so a lexical collapse answers beyond-reach and the kernel order does
        not.
        """
        ours = tmp_path / "ours"
        (ours / "inner").mkdir(parents=True)
        payload = ours / "payload"
        payload.write_text("#!/bin/sh\nexit 0\n")
        trusted = tmp_path / "trusted"
        trusted.mkdir()
        (trusted / "hop").symlink_to(ours / "inner")
        entry = trusted / _LAUNCHER
        entry.symlink_to(Path("hop") / ".." / "payload")

        route = [str(x) for x in (gateway_restart._objects_the_kernel_consults(entry) or [])]
        assert str(ours) in route, f"the route never reaches the real parent: {route}"

        _own_only(monkeypatch, mine={str(ours)})
        assert gateway_restart._agent_could_have_written(entry) is True

    def test_dot_dot_climbs_above_the_hop_it_followed(self, tmp_path, monkeypatch):
        """``..`` is applied to the route built so far, so it can reach a sibling.

        ``hop`` leads to ``x/deep/inner`` and the link's target then climbs twice and
        descends into ``x/sibling``. That directory is reachable ONLY by applying
        ``..`` to where the hop landed -- walking down never passes through it -- and
        it is the only thing attributed to us here.
        """
        base = tmp_path / "x"
        (base / "deep" / "inner").mkdir(parents=True)
        sibling = base / "sibling"
        sibling.mkdir()
        (sibling / "payload").write_text("#!/bin/sh\nexit 0\n")
        trusted = tmp_path / "trusted"
        trusted.mkdir()
        (trusted / "hop").symlink_to(base / "deep" / "inner")
        entry = trusted / _LAUNCHER
        entry.symlink_to(Path("hop") / ".." / ".." / "sibling" / "payload")

        route = [str(x) for x in (gateway_restart._objects_the_kernel_consults(entry) or [])]
        assert str(sibling) in route, f"dot-dot never climbed out of the hop: {route}"

        _own_only(monkeypatch, mine={str(sibling)})
        assert gateway_restart._agent_could_have_written(entry) is True
