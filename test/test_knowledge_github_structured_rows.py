...
def _commit_payload(**over):
    base = {
        ...
        "commit": {
            "message": "fix: thing",
            "author": {"name": "Octo", "email": "o@example.com", "date": "2026-09-05T00:00:00Z"},
            "committer": {"name": "Octo", "email": "o@example.com", "date": "2026-09-05T00:01:00Z"},
        },
        "parents": [{"sha": "b" * 40}],
...
class TestPrimaryKeys(unittest.TestCase):
    def test_issue_pk_carries_the_full_domain(self):
        ...
        self.assertEqual(row.primary_key, f"{SRC}|{DEFAULT_INSTANCE}|{REPO}|{ENTITY_ISSUE}|42")
    ...
    def test_commit_pk_carries_the_full_domain(self):
        row = mk_commit(_commit_payload())
        self.assertEqual(
            row.primary_key, f"{SRC}|{DEFAULT_INSTANCE}|{REPO}|{ENTITY_COMMIT}|{'a' * 40}"
        )

    def test_check_run_pk_carries_the_full_domain(self):
        row = mk_check(_check_run_payload())
        self.assertEqual(
            row.primary_key, f"{SRC}|{DEFAULT_INSTANCE}|{REPO}|{ENTITY_CHECK_RUN}|987654321"
        )
...
class TestLineage(unittest.TestCase):
    def test_every_row_carries_the_full_lineage_domain(self):
        for row in (
            mk_issue(_issue_payload()),
            mk_commit(_commit_payload()),
...
class TestConnectorContract(unittest.TestCase):
    ...
    def test_validate_config_refuses_creation_until_runtime_wired(self):
        # A well-formed repo passes the shape check but creation is REFUSED
        # (no live runtime): False + a specific reason, not a generic error.
        ok, msg = self.c.validate_config({"uri": REPO})
        self.assertFalse(ok)
        self.assertIn("cannot be created yet", msg)
        self.assertIn("transport executor", msg)

    def test_validate_config_reads_url_key_like_local_folder(self):
        # The add-source POST path passes the location under "url"; validate_config
        # must read it (url first, then uri) or the refusal never sees the value.
        # A malformed value under "url" must surface the SHAPE error (proving the
        # value was read), not the missing-repo error.
        ok, msg = self.c.validate_config({"url": "just-a-name"})
        self.assertFalse(ok)
        self.assertIn("owner/name", msg)

    def test_validate_config_refuses_creation_via_url_key(self):
        # A well-formed repo under "url" (the real POST shape) is still refused
        # at creation with the runtime-not-wired reason.
        ok, msg = self.c.validate_config({"url": REPO})
        self.assertFalse(ok)
        self.assertIn("cannot be created yet", msg)

    def test_validate_config_accepts_shape_for_github_uri(self):
        # github:// form normalises to a valid shape, so it clears the shape
        # check and reaches the creation refusal (not the shape error).
        ok, msg = self.c.validate_config({"uri": "github://kirodotdev/KiroCrew"})
        self.assertFalse(ok)
        self.assertIn("cannot be created yet", msg)

    def test_validate_config_rejects_non_string_uri_without_crashing(self):
        # A non-string value (e.g. JSON `uri: 123`) must be rejected cleanly, not
        # crash on .strip() (which would surface as an HTTP 500 on a request we
        # reject anyway). Guarded for both the url and uri keys.
        for cfg in ({"uri": 123}, {"url": ["not", "a", "string"]}, {"url": {"a": 1}}):
            ok, msg = self.c.validate_config(cfg)
            self.assertFalse(ok)
