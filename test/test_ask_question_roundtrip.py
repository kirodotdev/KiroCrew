...
class TestErrorCodes:
    ...
    **What the codes deliberately do NOT add.** The two ``403``s answer for
    ...
    def test_the_ratchet_can_actually_fail(self) -> None:
        ...
        27, including the three coded streaming-mode refusals for a non-internal
        caller, a missing slot, and a slot that is already running, plus the four
        coded validation paths for the slot-scoped ACP question read and answer
        operations. The non-owner ``403`` is omitted here: its body is produced
...
def _internal_request(state: DashboardState, **match_info: str) -> MagicMock:
    request = MagicMock()
...
@pytest.mark.asyncio
async def test_acp_slot_stream_answer_refuses_non_internal_caller() -> None:
    from kiro_crew.dashboard.handlers.ask_question import api_ask_question_slot_answer
