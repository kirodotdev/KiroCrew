...
class TestErrorCodes:
    ...
    **What the codes deliberately do NOT add.** The two ``403``s answer for
    ...
    def test_the_ratchet_can_actually_fail(self) -> None:
        ...
        29, including the five coded streaming-mode refusals for a non-internal
        caller, a missing slot, stale MCP ownership, a slot that is already
        running, and a slot held by orchestration or subagents, plus the four coded
        validation paths for the slot-scoped ACP question read and answer operations.
        The non-owner ``403`` is omitted here: its body is produced
        by ``handlers._shared._owner_denial_response``. The scanner is per-file,
        so that response remains outside this count.
        """
...
def test_stateless_card_rejected_answers_are_transactional() -> None:
    state = _state()
    slot = SimpleNamespace(_question_pending={})
    state._slots["chat-1"] = slot
    questions = [*_questions("First?"), *_questions("Second?")]
    state.mark_question_pending("chat-1", blocking=False, card_id="card-1", questions=questions)

    with pytest.raises(ValueError, match="one offered option"):
        state.answer_question_card(
            "chat-1",
            "card-1",
            {"First?": "Option A", "Second?": "not offered"},
        )

    assert state.pending_question_cards("chat-1")[0]["answers"] == {}


# ── ACP slot-scoped question bridge ──


def _internal_request(
    state: DashboardState, *, headers: dict[str, str] | None = None, **match_info: str
) -> MagicMock:
    request = MagicMock()
    ...
    request.get = lambda key, default=None: True if key == "internal_auth" else default
    return request


@pytest.mark.asyncio
async def test_acp_slot_pending_lists_stateless_cards() -> None:
    from kiro_crew.dashboard.handlers.ask_question import api_ask_question_slot_pending

    state = _state()
    state._slots = {"chat-1": SimpleNamespace(_question_pending={})}
    state.mark_question_pending("chat-1", blocking=False, card_id="card-1", questions=_questions())

    response = await api_ask_question_slot_pending(_internal_request(state, slot_key="chat-1"))

    assert response.status == 200
    assert json.loads(response.text)["questions"][0]["question_id"] == "card-1"


@pytest.mark.asyncio
async def test_acp_slot_pending_rejects_unknown_slot() -> None:
    from kiro_crew.dashboard.handlers.ask_question import api_ask_question_slot_pending

    state = _state()
    state._slots = {}

    response = await api_ask_question_slot_pending(_internal_request(state, slot_key="missing"))
...
@pytest.mark.asyncio
async def test_acp_slot_answer_rejects_invalid_body() -> None:
    from kiro_crew.dashboard.handlers.ask_question import api_ask_question_slot_answer

    state = _state()
    request = _internal_request(state, slot_key="chat-1", card_id="card-1")
    request.json = AsyncMock(return_value=[])

    response = await api_ask_question_slot_answer(request)

    assert response.status == 400
    assert json.loads(response.text)["code"] == "invalid_answers"


@pytest.mark.asyncio
async def test_acp_slot_answer_reports_missing_card() -> None:
    from kiro_crew.dashboard.handlers.ask_question import api_ask_question_slot_answer

    state = _state()
    state.answer_question_card = MagicMock(side_effect=ValueError("question card not found"))
    request = _internal_request(state, slot_key="chat-1", card_id="missing")
    request.json = AsyncMock(return_value={"answers": {"Question?": "Answer"}})

    response = await api_ask_question_slot_answer(request)

    assert response.status == 404
    assert json.loads(response.text)["code"] == "question_not_found"


@pytest.mark.asyncio
async def test_acp_slot_partial_answer_does_not_dispatch() -> None:
    from kiro_crew.dashboard.handlers.ask_question import api_ask_question_slot_answer

    state = _state()
    state.answer_question_card = MagicMock(return_value=None)
    request = _internal_request(state, slot_key="chat-1", card_id="card-1")
    request.json = AsyncMock(return_value={"answers": {"Question?": "Answer"}})
    ...
    assert response.status == 200
    assert json.loads(response.text) == {"ok": True, "completed": False}


@pytest.mark.asyncio
async def test_acp_slot_answer_queues_behind_running_turn() -> None:
    from kiro_crew.dashboard.handlers.ask_question import api_ask_question_slot_answer
    ...
    state._slots = {"chat-1": slot}
    state.answer_question_card = MagicMock(return_value="Question?: Answer")
    request = _internal_request(state, slot_key="chat-1", card_id="card-1")
    request.json = AsyncMock(return_value={"answers": {"Question?": "Answer"}})

    with patch("kiro_crew.dashboard.chat_delivery.queue_for_next_turn") as queue:
        response = await api_ask_question_slot_answer(request)

    assert response.status == 200
    assert json.loads(response.text) == {"ok": True, "completed": True}
    queue.assert_called_once_with(state, slot, "Question?: Answer", directive_user_origin=True)


@pytest.mark.asyncio
async def test_acp_slot_stream_busy_preserves_pending_card() -> None:
    from kiro_crew.dashboard.handlers.ask_question import api_ask_question_slot_answer

    state = _state()
    slot = SimpleNamespace(running=True, _question_pending={})
    state._slots = {"chat-1": slot}
    state.mark_question_pending("chat-1", blocking=False, card_id="card-1", questions=_questions())
    request = _internal_request(
        state,
        headers={"X-ACP-Session-Id": "chat-1"},
        slot_key="chat-1",
        card_id="card-1",
    )
    request.query = {"stream": "1"}
    request.json = AsyncMock(return_value={"answers": {"Which approach?": "Option B"}})

    response = await api_ask_question_slot_answer(request)

    assert response.status == 409
    assert json.loads(response.text)["code"] == "slot_busy"
    assert state.pending_question_cards("chat-1")[0]["question_id"] == "card-1"
...
@pytest.mark.parametrize(
    ...
    [
        {
            "X-ACP-Session-Id": "other-slot",
            "X-ACP-MCP-Owner": "adapter-b",
        },
        {
            "X-ACP-Session-Id": "chat-1",
            "X-ACP-MCP-Owner": "adapter-a",
        },
    ],
)
async def test_acp_slot_stream_stale_identity_preserves_pending_card(
    ...
    assert response.status == 409
    assert json.loads(response.text)["code"] == "mcp_owner_stale"
    assert state.pending_question_cards("chat-1")[0]["question_id"] == "card-1"


@pytest.mark.asyncio
@pytest.mark.parametrize(
    ("in_stage_execution", "running_agents"),
    [
        (True, []),
        (False, [{"id": "child-1"}]),
    ],
)
async def test_acp_slot_stream_hold_preserves_card_and_plan_task(
    in_stage_execution: bool,
    running_agents: list[dict[str, str]],
) -> None:
    from kiro_crew.dashboard.handlers.ask_question import api_ask_question_slot_answer

    state = _state()
    state.subagents = MagicMock()
    state.subagents.running_agents_for.return_value = running_agents
    slot = SimpleNamespace(
        key="chat-1",
        running=False,
        _in_stage_execution=in_stage_execution,
        session_mcp_owner="",
        task="plan-task",
        _question_pending={},
    )
    ...
    assert json.loads(response.text)["code"] == "slot_busy"
    assert state.pending_question_cards("chat-1")[0]["question_id"] == "card-1"
    assert slot.task == "plan-task"
    state.subagents.running_agents_for.assert_called_once_with("dashboard:chat-1")


@pytest.mark.asyncio
async def test_acp_slot_stream_answer_accepts_owner() -> None:
    from kiro_crew.dashboard.handlers.ask_question import api_ask_question_slot_answer
    ...
    slot = SimpleNamespace(
        ...
        session_mcp_owner="adapter-a",
    )
    state._slots = {"chat-1": slot}
    state.answer_question_card = MagicMock(return_value=None)
    request = MagicMock()
    request.app = {"state": state}
    request.match_info = {"slot_key": "chat-1", "card_id": "card-1"}
    request.query = {"stream": "1"}
    request.headers = {
        "X-ACP-Session-Id": "chat-1",
        "X-ACP-MCP-Owner": "adapter-a",
    ...
    assert response.status == 200
    assert json.loads(response.text) == {"ok": True, "completed": False}
    state.answer_question_card.assert_called_once_with("chat-1", "card-1", {"Question?": "Answer"})


@pytest.mark.asyncio
async def test_acp_slot_stream_answer_freezes_admitted_mcp_servers() -> None:
    from kiro_crew.dashboard.handlers.ask_question import api_ask_question_slot_answer

    state = _state()
    admitted = [{"name": "adapter-a-proxy"}]
    replacement = [{"name": "adapter-b-proxy"}]
    slot = SimpleNamespace(
        key="chat-1",
        running=False,
        _in_stage_execution=False,
        session_mcp_owner="adapter-a",
        session_mcp_servers=admitted,
        _acp_mcp_servers=ContextVar("question_answer_mcp_servers", default=None),
        append=MagicMock(),
    )
    state._slots = {"chat-1": slot}
    state.answer_question_card = MagicMock(return_value="Question?: Answer")

    async def run_background_turn(_slot, turn):  # type: ignore[no-untyped-def]
        return await turn

    state.run_background_turn = run_background_turn  # type: ignore[method-assign]
    request = MagicMock()
    request.app = {"state": state}
    request.match_info = {"slot_key": "chat-1", "card_id": "card-1"}
    request.query = {"stream": "1"}
    ...
    async def run_chat(_state, _slot, _prompt, **_kwargs):  # type: ignore[no-untyped-def]
        task_snapshot = slot._acp_mcp_servers.get()
        observed.extend(task_snapshot if task_snapshot is not None else slot.session_mcp_servers)
        turn_started.set()

    def spawn(_state, _slot, turn):  # type: ignore[no-untyped-def]
        task = asyncio.create_task(turn)
        tasks.append(task)
        return task

    async def stream_response(_request, _slot):  # type: ignore[no-untyped-def]
    ...
    with (
        patch("kiro_crew.dashboard.chat_runner._run_chat", new=run_chat),
        patch("kiro_crew.dashboard.turn_dispatch.spawn_guarded_turn", new=spawn),
        patch("kiro_crew.dashboard.chat_handlers.stream_slot_response", new=stream_response),
    ):
        response = await api_ask_question_slot_answer(request)
    ...
    assert slot._acp_mcp_servers.get() is None


@pytest.mark.asyncio
async def test_acp_slot_answer_starts_idle_turn() -> None:
    from kiro_crew.dashboard.handlers.ask_question import api_ask_question_slot_answer
    ...
    state._slots = {"chat-1": slot}
    state.answer_question_card = MagicMock(return_value="Question?: Answer")
    state.run_background_turn = MagicMock(return_value="background-turn")
    state.push_slots_update = MagicMock()
    request = _internal_request(state, slot_key="chat-1", card_id="card-1")
    request.json = AsyncMock(return_value={"answers": {"Question?": "Answer"}})

    run_chat = MagicMock(return_value="chat-turn")
    with (
        patch("kiro_crew.dashboard.chat_runner._run_chat", new=run_chat),
        patch("kiro_crew.dashboard.turn_dispatch.spawn_guarded_turn", return_value="task") as spawn,
    ):
        response = await api_ask_question_slot_answer(request)

    assert response.status == 200
    slot.append.assert_called_once_with("user", "Question?: Answer", "msg msg-u")
    run_chat.assert_called_once_with(state, slot, "Question?: Answer", _directive_user_origin=True)
    state.run_background_turn.assert_called_once_with(slot, "chat-turn")
    spawn.assert_called_once_with(state, slot, "background-turn")
    assert slot.task == "task"
