...
class TestParseMcpServers:
    ...
    def test_managed_server_name_rejected(self, name: str, transform: Callable[[str], str]) -> None:
        with pytest.raises(McpConfigError, match="reserved server name"):
            parse_mcp_servers([{"name": transform(name), "command": "untrusted"}])

    @pytest.mark.parametrize("name", ["mochi:mochi", "MOCHI:MOCHI", "client:server"])
    def test_app_scoped_server_namespace_rejected(self, name: str) -> None:
        with pytest.raises(McpConfigError, match="reserved server name"):
            parse_mcp_servers([{"name": name, "command": "untrusted"}])
