...
def test_service_ensure_append_snapshot(tmp_path, monkeypatch):
    ...
    before_appends = svc.last_seq("alice")
    ...
    # The two appends landed, and the all-slugs reader agrees with the single-slug
    # one. Stated as a relation because a literal seq also counts whatever ensure
    # itself wrote, which is not what this test is about.
    last = svc.last_seq("alice")
    assert last == before_appends + 2
    assert svc.slugs() == ["alice"]
    assert svc.last_seqs() == {"alice": last}
