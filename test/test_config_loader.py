...
class TestKnowledgePoolEffortConfig:
    """``knowledge.extraction_effort`` parsing.

    It is enum-typed with ``""`` meaning "inherit" — same spelling as
    ``agent.reasoning_effort`` — so the loader coerces anything that is not a
    concrete level to ``""`` instead of passing garbage to the pool.
    """
    ...
    def test_valid_levels_round_trip(self, level: str) -> None:
        cfg = _load_from_dict({"knowledge": {"extraction_effort": level}})
        assert cfg.knowledge.extraction_effort == level

    def test_empty_string_stays_inherit(self) -> None:
        cfg = _load_from_dict({"knowledge": {"extraction_effort": ""}})
        assert cfg.knowledge.extraction_effort == ""

    @pytest.mark.parametrize("bad", ["ultra", "Auto", 7, True, None, ["low"]])
    def test_junk_coerces_to_inherit(self, bad: object) -> None:
        cfg = _load_from_dict({"knowledge": {"extraction_effort": bad}})
        assert cfg.knowledge.extraction_effort == ""

    def test_round_trip_via_to_dict(self) -> None:
        data = _load_from_dict({"knowledge": {"extraction_effort": "low"}}).to_dict()
        assert data["knowledge"]["extraction_effort"] == "low"

    def test_effort_key_is_dashboard_editable_enum(self) -> None:
        from kiro_crew.dashboard.handlers.core import _EDITABLE_CONFIG

        spec = _EDITABLE_CONFIG.get("knowledge.extraction_effort")
...
class TestMigrationWriteBackOrdering:
    ...
    ``load()``'s migration must not call ``cfg.save()``: saving re-serializes
    the whole snapshot this load parsed, so a config write landing after that
    read and before the save is silently replaced by the older snapshot.
    ``load()`` runs
