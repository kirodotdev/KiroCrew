...
def test_bridge_prefers_resolve_ref_and_passes_the_full_ref(store):
    ...
    ref = ProviderResourceRef(provider="sharepoint", account="tenant-A",
                              resource_id="Doc", locator={"endpoint": "host-1"})
    src2 = store.add_source("sp2", "sharepoint", "sharepoint://Doc2")
    item2 = store.add_item("Doc2", "omega content", "doc", source_id=src2)
    store.set_item_acl(item2, ["sp-alice"], tenant="ms-A", managed=True,
...
def test_bridge_resolve_ref_wrong_endpoint_denies(store):
    ref = ProviderResourceRef(provider="sharepoint", account="tenant-A",
                              resource_id="Doc", locator={"endpoint": "host-2"})
...
def test_bridge_falls_back_to_resolve_when_no_resolve_ref(store):
    ...
    item = _managed(store, "Doc", "omega content", provider="sharepoint",
                    account="tenant-A", subjects=["sp-alice"], tenant="ms-A")
    inner = _GrantResolver({("sharepoint", "tenant-A"):
                            AccessContext(subject="sp-alice", tenant="ms-A")})
