...
def found_exe(monkeypatch):
    """The descriptor's executable resolves to a pinned absolute path."""
    monkeypatch.setattr(
...
def test_capabilities_ride_on_the_descriptor():
    d = _descriptor(capabilities=CapabilitySet(session_mcp_array=True))
    assert DescriptorHarness(d)._descriptor.capabilities.session_mcp_array is True
