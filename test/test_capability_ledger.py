...
ENFORCED = {
    ...
    # Gates whether a dashboard connect claims INBOUND ownership
    # (`accepts_inbound`) in dashboard/chat_mirror.py, and therefore whether the
    # slot row reports `direction: both`. Only a transport whose inbound path
    # resolves the mirror binding may declare it.
...
class TestSessionResumeIsDeclaredOnlyWhereItIsHonoured:
    ...
    A dashboard connect on a transport declaring it claims `accepts_inbound`, and the
    slot row then reports `direction: both` — the dashboard tells the user replies
    come back. That is only true where the transport's inbound path resolves the
    mirror binding. Discord's does (`DiscordSessionResume.resumed_session`); every
    other transport builds a session key from the route alone and never looks the
    binding up, so a reply there runs in a SEPARATE session.

    This pins the current set. A new transport declaring the flag fails here, which
    is the point: the author has to come and confirm its inbound path actually
    resolves the binding rather than inheriting a promise the code does not keep.
    """
    ...
    def test_no_other_transport_declares_it(self) -> None:
        ...
        claiming = [
            name for name, caps in others.items() if caps.supports_session_resume
        ]
        assert claiming == [], (
