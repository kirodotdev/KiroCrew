...
A subagent otherwise inherits the parent's backend with no way for a parent to
run a delegated task on a different harness. ``spawn_run`` takes a batch-wide
...
class TestSchema:
    ...
    @pytest.mark.parametrize("value", ["kas", "deepseek", "acme", "kiro", "a", "x-y-z"])
    ...
    @pytest.mark.parametrize("bad", ["KAS", "has space", "under_score", "-lead", "trail-", "a..b"])
...
class TestAdmissionValidation:
    ...
    @pytest.mark.asyncio
    async def test_empty_backend_is_never_checked(self):
        mgr = _mgr()
        mgr._run = AsyncMock()  # type: ignore[method-assign]
        info = mgr.spawn("do the thing")
        assert info is not None
        assert not info.done
        assert info.acp_backend == ""
...
class TestRecordOntoInfo:
    ...
    @pytest.mark.asyncio
    async def test_default_is_empty_meaning_inherit(self):
        mgr = _mgr()
        mgr._run = AsyncMock()  # type: ignore[method-assign]
        info = mgr.spawn("do the thing")
        assert info is not None
...
class TestForcingRuleAndDisqualifier:
    ...
    def _run(self, *, info_backend: str = ""):
