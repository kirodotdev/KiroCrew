...
def test_write_home_document_refuses_link_backed_destination(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    """A symlinked keystone cannot be snapshotted, so a lockdown failure after
    the replace would roll back as a first write and unlink the policy path.
    Refuse the link up front: nothing replaced, nothing rolled back, the link
    and its target untouched, no staging leaf left behind."""
    home = _isolate(monkeypatch, tmp_path)
    ...
    def _restrict(p: Path) -> None:
        if Path(p) == home:
            pytest.fail("must not reach the destination lockdown")
        real_restrict(p)
...
def test_write_home_document_retries_backup_replace_on_windows_share(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    """Rollback is an atomic replace of the pre-staged ``.bak`` snapshot; on
    Windows it retries the sharing-violation window and verifies before
    write_failed returns. Dest is never written into."""
    home = _isolate(monkeypatch, tmp_path)
    original = (
        '{"version": 1, "boot": {"require_sandbox": true, '
        '"allow_terminal": false, "fail_closed": true}, '
    ...
    def _replace(src: Path, dest: Path) -> None:
        replaces["n"] += 1
        if replaces["n"] == 2:
            raise OSError("restore replace sharing violation")
    ...
    assert resp.status == 500
    assert json.loads(resp.text)["code"] == "write_failed"
    assert replaces["n"] == 3  # publish, failed restore, retried restore
    assert home.read_text(encoding="utf-8") == original
    assert not home.with_name("security_policy.json.tmp").exists()
    assert not home.with_name("security_policy.json.bak").exists()  # consumed by the replace
...
def test_write_home_document_never_truncates_dest_when_rollback_replace_fails(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    """When even the atomic restore cannot land, dest is left as the WHOLE
    published document -- never overwritten in place (an ``O_TRUNC`` rewrite
    that dies mid-write leaves a truncated ceiling the next boot cannot load).
    The snapshot stays in ``.bak`` for recovery, the Save fails, and a later
    Save stages its own snapshot over that leftover instead of being bricked."""
    home = _isolate(monkeypatch, tmp_path)
    original = (
        '{"version": 1, "boot": {"require_sandbox": true, '
        '"allow_terminal": false, "fail_closed": true}, '
    ...
    published = json.loads(home.read_text(encoding="utf-8"))  # whole, parseable
    assert published["capabilities"]["agentcore"]["enabled"] is False
    backup = home.with_name("security_policy.json.bak")
    assert backup.read_text(encoding="utf-8") == original
    assert not home.with_name("security_policy.json.tmp").exists()
    # Recovery: with lockdown working again the next Save succeeds and clears
    # the leftover snapshot.
    monkeypatch.undo()
    home2 = _isolate(monkeypatch, tmp_path)
    assert home2 == home
    resp = asyncio.run(mod.api_agentcore_identity_save(_Req({"posture": "none"})))
    assert resp.status == 200
    ...
    async def _body() -> None:
        ...
        class _Sessions:
            ...
            def session_keys(self) -> frozenset[str]:
                return frozenset(f"k{i}" for i in range(len(self._held)))
...
def _reset_harness(providers: list[Any]) -> tuple[Any, list[str]]:
    """A request whose session manager drains exactly *providers*."""
    order: list[str] = []
    ...
    return _ResetReq(), order


def test_reset_all_sessions_revoke_fails_when_a_process_survives(monkeypatch) -> None:
    """Save → Off must not report success while a child still holds its bearer.

    A swallowed shutdown failure with no PID check would let a detached
    kiro-cli keep its injected Gateway credentials while the API says the
    identity is revoked. Shutdown failure escalates to the kill, the PID is
    confirmed, and a survivor makes the awaited revoke raise.
    """
    from kiro_crew.dashboard.handlers import sessions as smod
    ...
    monkeypatch.setattr(smod._h, "_sync_kill_provider", lambda p: killed.append(p._client._pid))
    monkeypatch.setattr(smod, "_pid_terminated", lambda pid: False)
    monkeypatch.setattr(smod.asyncio, "sleep", _instant_sleep)
    req, _order = _reset_harness([_Prov()])
    with pytest.raises(RuntimeError, match="survived shutdown"):
        asyncio.run(smod._reset_all_sessions(req, await_shutdown=True))
    # Once for the raised shutdown, once more when the PID was still alive.
    assert killed == [4242, 4242]


def test_reset_all_sessions_revoke_succeeds_once_the_process_is_gone(monkeypatch) -> None:
    from kiro_crew.dashboard.handlers import sessions as smod
    ...
    monkeypatch.setattr(smod._h, "_sync_kill_provider", lambda p: None)
    monkeypatch.setattr(smod, "_pid_terminated", _terminated)
    monkeypatch.setattr(smod.asyncio, "sleep", _instant_sleep)
    req, order = _reset_harness([_Prov()])
    assert asyncio.run(smod._reset_all_sessions(req, await_shutdown=True)) == 1
    assert order == ["reload", "pool"]
...
def test_reset_all_sessions_revoke_catches_a_cold_start_that_raced_the_drain(monkeypatch) -> None:
    """A Login turn whose allocation was in flight when the snapshot was taken
    registers afterwards. It must still be drained and shut down, the closing
    gate must be up while that happens (so nothing NEW can register), and the
    gate must be down again once the revoke is done."""
    from kiro_crew.dashboard.handlers import sessions as smod
    ...
    class _Sessions:
        ...
        def __init__(self) -> None:
            self._held: list[Any] = [_Prov("early")]
        ...
        def session_keys(self) -> frozenset[str]:
            keys = {p.name for p in self._held}
            if self._late_pending:
                keys.add("late")  # reserved: allocation in flight
            return frozenset(keys)

        async def drain_all_providers(self) -> list[Any]:
            gate_during.append(self._closing)
            out, self._held = self._held, []
            if self._late_pending:
                # The raced allocation lands right after this snapshot.
                self._late_pending = False
                self._held = [_Prov("late")]
            return out
    ...
    monkeypatch.setattr(smod.asyncio, "sleep", _instant_sleep)
    req = _ResetReq()
    count = asyncio.run(smod._reset_all_sessions(req, await_shutdown=True))
    assert count == 2
    assert sorted(shut) == ["early", "late"]
    assert gate_during and all(gate_during)
    assert req.app["state"].sessions._closing is False


def test_reset_all_sessions_revoke_keeps_gate_closed_when_a_reservation_never_lands(
    monkeypatch,
) -> None:
    """A provider spawn that outlives the settle budget must not be waited past
    and then admitted: it was spawned with the pre-save identity. The awaited
    revoke raises (the PUT reports restart_required) and the closing gate stays
    set so the late registration is refused instead of keeping Gateway access."""
    from kiro_crew.dashboard.handlers import sessions as smod
    ...
    class _Sessions:
        ...
        def session_keys(self) -> frozenset[str]:
            # "slow" is reserved for the whole budget and never registers.
            return frozenset({p.name for p in self._held} | {"slow"})
        ...
        async def drain_warm_pool(self) -> list[Any]:
            pytest.fail("must not proceed to the warm pool with a reservation in flight")

        async def reload_provider_factory(self) -> None:
            pytest.fail("must not reload the factory with a reservation in flight")
    ...
    monkeypatch.setattr(smod.asyncio, "sleep", _instant_sleep)
    monkeypatch.setattr(smod, "_REVOKE_SETTLE_ROUNDS", 3)
    monkeypatch.setattr(
        smod._h, "_sync_kill_provider", lambda p: killed.append(p.name), raising=False
    )
    req = _ResetReq()
    with pytest.raises(RuntimeError, match="still in flight"):
        asyncio.run(smod._reset_all_sessions(req, await_shutdown=True))
    # The gate is still up: the slow allocation gets SessionClosingError.
    assert req.app["state"].sessions._closing is True
    # What was drained before the raise did not linger.
    assert killed == ["early"]
