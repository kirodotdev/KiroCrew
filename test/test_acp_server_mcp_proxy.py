...
async def test_run_relays_supervisor_output_to_stdout(monkeypatch) -> None:
    ...
    with (
        tempfile.TemporaryDirectory(prefix="acp-proxy-", dir=short_tmp_base()) as socket_root,
        contextlib.ExitStack() as stack,
    ):
        socket_path = os.path.join(socket_root, "m.sock")
        in_r, in_w = os.pipe()
        stack.callback(close_fd, in_r)
        stack.callback(close_fd, in_w)
        out_r, out_w = os.pipe()
        stack.callback(close_fd, out_r)
        stack.callback(close_fd, out_w)
