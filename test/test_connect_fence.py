...
class TestBpfProgram:
    ...
    def test_program_ends_in_allow_and_notif_returns(self) -> None:
        ...
        allow = struct.unpack_from("<HBBI", prog, 12 * 8)
        enosys = struct.unpack_from("<HBBI", prog, 13 * 8)
        notif = struct.unpack_from("<HBBI", prog, 14 * 8)
        eperm = struct.unpack_from("<HBBI", prog, 15 * 8)
        ...
        assert eperm[3] == cf.SECCOMP_RET_ERRNO | errno.EPERM
    ...
    def test_fastopen_send_family_denied(self) -> None:
        # A MSG_FASTOPEN send opens the TCP connection inside the send path
        # without invoking connect, so the notify trap never adjudicates the
        # target; the program refuses the flag itself with EPERM on every
        # send-family syscall.
        for machine, (sendto_nr, sendmsg_nr, sendmmsg_nr) in {
            "x86_64": (44, 46, 307),
            "aarch64": (206, 211, 269),
        }.items():
            prog = cf.build_connect_notif_prog(machine)
            jeq_sendmsg = struct.unpack_from("<HBBI", prog, 5 * 8)
            jeq_sendto = struct.unpack_from("<HBBI", prog, 6 * 8)
            jeq_sendmmsg = struct.unpack_from("<HBBI", prog, 7 * 8)
            assert (jeq_sendmsg[0], jeq_sendmsg[3]) == (
                cf._BPF_JMP | cf._BPF_JEQ | cf._BPF_K,
                sendmsg_nr,
            )
            assert jeq_sendto[3] == sendto_nr
            assert jeq_sendmmsg[3] == sendmmsg_nr
            # sendmsg carries flags in args[2]; sendto/sendmmsg in args[3].
            ld_arg2 = struct.unpack_from("<HBBI", prog, 8 * 8)
            ld_arg3 = struct.unpack_from("<HBBI", prog, 10 * 8)
            assert (ld_arg2[0], ld_arg2[3]) == (
                cf._BPF_LD | cf._BPF_W | cf._BPF_ABS,
                32,
            )
            assert (ld_arg3[0], ld_arg3[3]) == (
                cf._BPF_LD | cf._BPF_W | cf._BPF_ABS,
                40,
            )
            # Both flag tests match MSG_FASTOPEN and land on the EPERM verdict.
            for idx in (9, 11):
                jset = struct.unpack_from("<HBBI", prog, idx * 8)
                assert jset[0] == cf._BPF_JMP | cf._BPF_JSET | cf._BPF_K
                assert jset[3] == 0x20000000
                target = struct.unpack_from("<HBBI", prog, (idx + 1 + jset[1]) * 8)
                assert target[3] == cf.SECCOMP_RET_ERRNO | errno.EPERM

    def test_fastopen_miss_falls_through_to_allow(self) -> None:
        # A send call without MSG_FASTOPEN reaches the ALLOW verdict: each
        # flag test's false edge resolves to the ALLOW return.
        prog = cf.build_connect_notif_prog("x86_64")
        for idx in (9, 11):
            jset = struct.unpack_from("<HBBI", prog, idx * 8)
            fall = struct.unpack_from("<HBBI", prog, (idx + 1 + jset[2]) * 8)
            assert fall[3] == cf.SECCOMP_RET_ALLOW
...
class TestFenceVerdict:
    ...
    def test_v4_mapped_v6_folds_onto_v4_identity(self) -> None:
        assert cf.fence_verdict(self._target(_sockaddr_in6(22, "::ffff:192.0.2.10")), self.SELF)
...
class TestSupervisorSource:
    ...
    def _shipped_denied(self, ports=frozenset({22}), self_addrs=frozenset()):
        ...
        exec(textwrap.dedent(header), ns)  # nosemgrep: python.lang.security.audit.exec-detected.exec-detected -- executes this repo's own shipped launcher-header source verbatim so the parity assertions below pin what the template bakes in; no external input reaches it  # noqa: S102, E501  # fmt: skip
