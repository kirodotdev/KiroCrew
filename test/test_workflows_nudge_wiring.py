...
class FakeNudgeSvc:
    ...
    async def add(
        ...
        self.added_sentinels.append(stop_sentinel_path)
...
async def test_authz_exit_gate_clean_command_passes_through(monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        "kiro_crew.dashboard.session_directive_apply._gate_anchor_masked",
        lambda state, key: False,
    )
    monkeypatch.setattr(
        autonudge_authz,
        "sel",
...
async def test_authz_gated_add_gets_no_stop_sentinel(monkeypatch) -> None:
    """A GATED loop carries NO stop sentinel (GPT round 18): the sentinel
    path is agent-disclosed and its terminal is ungated, so on a gated loop
    it would be an agent-writable file that ends the loop with no pause and
    no verification. Ungated loops keep the auto-defaulted sentinel."""
    monkeypatch.setattr(
        "kiro_crew.dashboard.session_directive_apply._gate_anchor_masked",
        lambda state, key: False,
    )
    ...
    svc = FakeNudgeSvc()
    _, error, status = await authorize_and_add_nudge(
        svc=svc,
        state=_state(slots={"chat-1-1": SimpleNamespace(workspace="default")}),
        slot_key="chat-1-1",
        message="watch",
        exit_gate_cmd="pytest -q",
        stop_sentinel_path="/tmp/some-stop-file",  # even an explicit one
        source="dashboard",
    )
    assert error is None and status == 200
    assert svc.added_sentinels == [""], "gated loops must carry no sentinel"
    ...
    assert error is None and status == 200
    assert svc2.added_sentinels[0] != "", "ungated loops keep their sentinel"


async def test_authz_gate_arm_denied_when_anchor_is_masked(monkeypatch) -> None:
    """ANCHOR CHECK at arm time (GPT round 22): the default workspace dir
    lives under the masked data home, so a gate armed on an unscoped slot
    would return not_run on every run — the chokepoint refuses the ARM with
    an actionable message instead of arming a gate that can never verify."""
    monkeypatch.setattr(
        autonudge_authz,
        "sel",
        lambda: SimpleNamespace(log_tool_invocation=lambda **kw: None),
    ...
    assert "project directory" in (error or "")
    assert svc.added == [], "nothing armed on a masked anchor"
