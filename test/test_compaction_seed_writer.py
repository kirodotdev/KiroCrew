...
class _Slot:
    def __init__(self, messages: list[dict], key: str = "chat-7") -> None:
        self.key = key
...
def _state(slot, rows_on_disk, *, save_fails: Exception | None = None):
    ...
    def save_slot_strict(
        target, *, expected_slot_name: str | None = None, expected_history_key: str | None = None
    ) -> None:
        if save_fails is not None:
            raise save_fails
        saved.append((target, len(target.messages), expected_slot_name, expected_history_key))
...
class TestWriteSeed:
    ...
    async def test_the_seed_is_on_disk_before_the_writer_answers_written(self, routed):
        ...
        assert [
            (saved_slot is slot, count, expected_slot_name, expected_history_key)
            for saved_slot, count, expected_slot_name, expected_history_key in state.saved
        ] == [(True, 5, slot.key, "hist")], "saved once with the seed and both identity pins"
        assert slot.withdrawn == []

    @pytest.mark.asyncio
    async def test_a_close_during_the_transcript_read_withdraws_the_seed(
        ...
        slot = state.get_or_create_slot("chat-7")
        monkeypatch.setattr(
            "kiro_crew.dashboard.chat_utils.dashboard_slot_key", lambda key: slot.key
        )
        for source in _rows(80):
            slot.append(source["role"], source["content"], "", source["ts"], broadcast=False)
        history_key = slot_history_key(slot)
        await asyncio.to_thread(state.save_slot_strict, slot)
        read_rows = cs.rotation_rows

        def close_then_read(*args, **kwargs):
            removed = state._slots.pop(slot.key)
            assert removed is slot
            assert _save_slot_to_history(state, removed, closed=True, closed_at=1234.0, force=True)
            return read_rows(*args, **kwargs)

        monkeypatch.setattr(cs, "rotation_rows", close_then_read)
        result = await cs.write_seed(state, history_key, "shake", SMALL_WINDOW)

        assert result == cs.SEED_UNSUPPORTED
        assert all(message["role"] != SEED_ROLE for message in slot.messages)
        metadata = state.conversation_log.get_metadata(history_key)
        assert metadata["closed"] is True
        assert metadata["closed_at"] == 1234.0
    ...
    async def test_everything_in_the_tail_is_nothing_to_digest(self, routed):
        ...
        # tail: the one answer that turns the rotation into soft.
        slot = _Slot([])
...
class TestNoGapBetweenDigestAndTail:
    ...
    def test_the_writer_and_the_replay_walk_with_one_set_of_options(self):
        ...
        source = inspect.getsource(cs._build_seed)
