...
class TestInTurnAgentSwitchClearsTheModelPin:
    """A provider-reported agent change is a switch like any other.

    kiro-cli has already moved its own session to the new agent by the time the
    event arrives, so the slot follows. The pin and the fallback walk belonged
    to the OLD agent's provider: leaving them behind sends an id the new
    provider may not serve, and resumes a walk built for a different model.
    """

    @pytest.mark.asyncio
    async def test_provider_reported_switch_clears_the_pin(self, tmp_path, monkeypatch):
        from kiro_crew.dashboard import chat_runner
        ...
        # The turn resolves the slot's agent against the real config, so both
        # names have to be real agents -- the switch still has to differ.
        others = [n for n in KiroCrewConfig.load().agents if n != slot.agent]
        new_agent = others[0] if others else slot.agent
        slot.model = "claude-opus-5"
        slot._fallback_candidate_idx = 3
        ...
        assert slot.model == "", "the switched slot kept the old provider's pin"
