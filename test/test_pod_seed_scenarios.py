...
class TestApiPath:
    ...
    @pytest.mark.parametrize(
        "given",
        [
            "/api/sessions?token=cAlLeRsEcReT",
            "sessions?limit=2&token=cAlLeRsEcReT",
            "http://127.0.0.1:7811/api/health?token=cAlLeRsEcReT",
        ],
    )
    def test_a_caller_supplied_token_refuses_without_echoing_it(self, given: str) -> None:
        """`pod api` injects the credential itself, and this normalized path is
        interpolated into error text and the SEL audit line — forwarding a
        caller's `token` parameter would write their credential into the audit
        trail. The refusal names the parameter, never the value."""
        with pytest.raises(rt.PodError) as excinfo:
            rt.api_path(given)
        message = str(excinfo.value)
        assert "token" in message
        assert "cAlLeRsEcReT" not in message

    def test_other_query_parameters_still_survive_untouched(self) -> None:
        assert rt.api_path("/api/sessions?limit=2&order=desc") == (
            "/api/sessions?limit=2&order=desc"
        )
...
class TestBodySizeCeiling:
    ...
    def test_a_body_that_cannot_be_read_refuses_in_the_pod_vocabulary(self) -> None:
        """A truncated body must not surface as a raw traceback.

        This helper is also called from the ``HTTPError`` handler, and an exception
        raised inside one ``except`` clause is not caught by its siblings — so a
        gateway that truncates a 4xx/5xx body would otherwise print
        ``IncompleteRead`` instead of naming the pod.
        """
        ...
        with pytest.raises(rt.PodError) as excinfo:
            rt._read_capped(_Truncated(), "GET", "/api/sessions", "wt")
        assert "could not " in str(excinfo.value) and "wt" in str(excinfo.value)
...
class _Handler(BaseHTTPRequestHandler):
    ...
    def _respond(self) -> None:
        ...
        # Routed on the PATH component, not the raw request target: the token
        # travels as a query parameter, so a suffix match on `self.path` would
        # stop matching the moment the credential is appended. A real server
        # dispatches on the path alone for the same reason.
        route = self.path.split("?", 1)[0]
        if route.endswith("/boom"):
            self.send_response(503)
            self.send_header("Content-Type", "application/json")
            self.end_headers()
            self.wfile.write(json.dumps({"error": "unavailable"}).encode())
            return
        if route.endswith("/plain"):
    ...
    def do_HEAD(self) -> None:
        # A HEAD response carries no body, so the assertion on this path is the
        # status alone — enough to prove the method reached the gateway.
        self.send_response(200)
        self.send_header("Content-Type", "application/json")
        self.end_headers()
...
class TestPodApi:
    def test_sends_the_token_the_dashboard_actually_reads(self, stub_gateway: HTTPServer) -> None:
        """The credential must travel as ``?token=``, not as a Bearer header.

        The dashboard's auth middleware
        (:mod:`kiro_crew.dashboard.token_auth`) takes a token from the ``token``
        query parameter or from its ``mc_token_<port>`` cookie, and reads no
        ``Authorization`` header at all — so a Bearer credential was
        indistinguishable from sending none and every call answered 403. An
        earlier version of this test asserted the Bearer header, which is why a
        green suite sat in front of a verb that could not authenticate: it pinned
        the transport the code used instead of the one the server accepts.
        """
        status, raw = rt.pod_api(PodConfig.load(), "wt", "GET", "sessions")
        assert status == 200
        echoed = json.loads(raw)
        assert echoed["method"] == "GET"
        assert echoed["path"] == "/api/sessions?token=fixture-token"
        assert echoed["auth"] == ""

    def test_preserves_a_callers_query_string_beside_the_token(
        self, stub_gateway: HTTPServer
    ) -> None:
        # The token is APPENDED, so a path that already carries parameters keeps
        # them — substituting the query would silently drop the caller's filter
        # and answer a different question than the one asked.
        status, raw = rt.pod_api(PodConfig.load(), "wt", "GET", "sessions?limit=2&kind=chat")
        assert status == 200
        assert json.loads(raw)["path"] == "/api/sessions?limit=2&kind=chat&token=fixture-token"

    def test_head_is_available_without_an_opt_in(self, stub_gateway: HTTPServer) -> None:
        status, _raw = rt.pod_api(PodConfig.load(), "wt", "HEAD", "health")
        assert status == 200

    def test_a_write_method_is_refused_without_the_flag(self, monkeypatch) -> None:
        """Default read-only, and refused before the pod is even looked at.

        A pod's API is a full control plane — it can start agent turns, rewrite
        config, delete sessions — and the caller is frequently an agent acting on
        a path it read out of content it does not control. The guard therefore
        runs before liveness and before the mint, so nothing about the target can
        influence whether it applies.
        """
        monkeypatch.setattr(
            rt, "is_active", lambda cfg, name: pytest.fail("refused too late: pod was probed")
        )
        with pytest.raises(rt.PodError) as excinfo:
            rt.pod_api(PodConfig.load(), "wt", "POST", "config", data="{}")
        assert "--allow-write" in str(excinfo.value)

    def test_a_write_method_goes_through_with_the_flag(self, stub_gateway: HTTPServer) -> None:
        status, raw = rt.pod_api(
            PodConfig.load(), "wt", "POST", "config", data='{"key":"x"}', allow_write=True
        )
        assert status == 200
    ...
    def test_a_transport_failure_never_quotes_the_token(self, monkeypatch) -> None:
        # The credential now travels in the URL, and urllib puts the URL into
        # some error strings, so anything derived from an exception is scrubbed
        # before it reaches a message, a log or a terminal.
        secret = "s3cret-pod-token"
        monkeypatch.setattr(rt, "is_active", lambda cfg, name: True)
        monkeypatch.setattr(rt, "mint_token", lambda cfg, name, ttl="2h": secret)

        def _boom(req, timeout=0):  # noqa: ANN001 - urlopen stand-in
            raise OSError(f"connect failed for {req.full_url}")

        monkeypatch.setattr(rt, "loopback_urlopen", _boom)
        with pytest.raises(rt.PodError) as excinfo:
            rt.pod_api(PodConfig.load(), "wt", "GET", "sessions")
...
class TestApiVerb:
    def _args(
        self, path: str, method: str = "GET", data: str = "", allow_write: bool = False
    ) -> argparse.Namespace:
        return argparse.Namespace(
            name="wt", method=method, path=path, data=data, allow_write=allow_write
        )
    ...
    def test_prints_one_fixed_shape_document(
        ...
        # The echoed request carries the credential; the printed document must
        # keep reporting the credential-free path an operator can paste.
        assert doc["body"]["path"] == "/api/sessions?token=fixture-token"

    def test_a_write_without_the_flag_exits_1_naming_it(
        self, monkeypatch, capsys: pytest.CaptureFixture
    ) -> None:
        monkeypatch.setattr(rt, "is_active", lambda cfg, name: pytest.fail("pod was probed"))
        with pytest.raises(SystemExit) as excinfo:
            pod_cli._api(PodConfig.load(), self._args("config", method="POST", data="{}"))
        assert excinfo.value.code == 1
        assert "--allow-write" in capsys.readouterr().err

    def test_an_absent_allow_write_attribute_reads_as_off(
        self, monkeypatch, capsys: pytest.CaptureFixture
    ) -> None:
        # Hand-built Namespaces (tests, older callers) predate the flag, and the
        # safe reading of its absence is the restricted one.
        monkeypatch.setattr(rt, "is_active", lambda cfg, name: pytest.fail("pod was probed"))
        stale = argparse.Namespace(name="wt", method="DELETE", path="sessions/x", data="")
        with pytest.raises(SystemExit):
            pod_cli._api(PodConfig.load(), stale)
        assert "--allow-write" in capsys.readouterr().err
