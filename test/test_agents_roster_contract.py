...
class TestRosterRowKeySet:
    ...
    async def test_project_path_query_scopes_discovery_with_no_slot(
        ...
        def _fake_resolve(raw: str, holds: contextlib.ExitStack | None = None) -> tuple[str, bool]:
            ...
            # directory.
            #
            # Asserting on *holds* rather than just absorbing it is the point: the
            # handler MUST hand the resolver a stack it keeps open across the scan,
            # because the consumer re-checks the path by name and a dropped hold
            # reopens the Windows UNC-junction swap. A double that quietly ignored
            # the argument would keep passing if the handler stopped passing one.
            assert isinstance(holds, contextlib.ExitStack)
            return raw, False
    ...
    @pytest.mark.asyncio
    async def test_validated_root_stays_held_until_after_the_scan(
        self, monkeypatch, tmp_path: Path
    ) -> None:
        """The resolver's holds are STILL OPEN when ``project_agent_names`` runs.

        GPT flagged that ``_resolve_roster_project_path`` validated the owner's
        ``?project_path=`` under a hold but released it on RETURN, so the value
        handed on was only known-good as of resolution time. The consumer then
        re-checks it BY NAME -- ``project_agent_names`` opens with
        ``is_sensitive_path(str(project_dir))`` -- and ``is_sensitive_path``
        RESOLVES what it is given. On Windows, resolving a junction whose target is
        ``\\\\host\\share`` IS the outbound SMB/NTLM probe, so a root swapped in the
        gap turns the consumer's own sensitivity check into a credential probe
        against a caller-chosen host. An SEL round-trip and an executor hop sit in
        that gap, both of which can suspend the coroutine.

        The fix is a lifetime, so the test asserts a lifetime rather than a call:
        the stub registers a release marker in the stack the handler passes, and
        the scan stub records whether that marker has already fired. Released too
        early, ``released`` is True by scan time and this fails -- which is exactly
        what the pre-fix handler did, since its ``with contextlib.ExitStack()``
        lived inside the resolver and unwound at the return.

        Platform-independent on purpose: the harm is Windows-specific but the
        ORDERING this pins is not, so it guards the property on the Linux fleet
        where it would otherwise only be checked by inspection.
        """
        ...
        def _fake_resolve(raw: str, holds: contextlib.ExitStack | None = None) -> tuple[str, bool]:
            assert isinstance(holds, contextlib.ExitStack)
            # Stands in for the real transferred directory handles: whatever the
            # handler does to the stack, it does to this marker too.
            holds.callback(lambda: released.__setitem__(0, True))
            return raw, False

        def _fake_names(project_dir, **kw):
            seen["released_at_scan"] = released[0]
            return frozenset({"held-scope-agent"})
        ...
        try:
            with unittest.mock.patch("kiro_crew.config.loader.config_path", return_value=tmp):
                ...
                app["state"] = types.SimpleNamespace(conversation_log=None)
                async with TestClient(TestServer(app)) as client:
                    resp = await client.get("/api/agents?project_path=/held/project/dir")
                    assert resp.status == 200

            # The scan actually ran (otherwise the ordering claim is vacuous)...
            assert "released_at_scan" in seen
            # ...and the hold had NOT been released when it did.
            assert seen["released_at_scan"] is False
            # Released afterwards, so the handler does not leak the handles for the
            # life of the process -- the other half of a correct lifetime.
            assert released[0] is True
        finally:
            tmp.unlink(missing_ok=True)

    @pytest.mark.asyncio
    async def test_the_holds_are_released_off_the_event_loop(self, monkeypatch, tmp_path: Path):
        """Unwinding the roster holds does NOT run on the event loop thread.

        The holds must outlive the scan (see the test above), but releasing them
        runs ``release_held_directory`` -> ``os.close``/``CloseHandle``, and the
        anchor ``no-blocking-call-on-event-loop`` names ``os.close`` outright
        because it can block on a wedged peer. That is the NORM rather than the
        edge for the paths this branch serves: closing a handle on a mapped network
        drive is an SMB round-trip, so a wedged share would freeze every task on
        the single gateway loop -- the user's turn and the liveness heartbeat alike
        -- until the watchdog kills the process.

        So the release is off-loop, and the oracle is the THREAD it happens on:
        the stub records the releasing thread's ident and the test compares it to
        the loop's own. A ``with`` block around the region (the shape this replaced)
        unwinds inline on the loop thread and fails here.
        """
        loop_thread = threading.get_ident()
        release_thread: list[int | None] = [None]

        def _fake_resolve(raw: str, holds: contextlib.ExitStack | None = None):
            assert isinstance(holds, contextlib.ExitStack)
            holds.callback(lambda: release_thread.__setitem__(0, threading.get_ident()))
            return raw, False
        ...
        monkeypatch.setattr(
            "kiro_crew.dashboard.handlers.agents.project_agent_names",
            lambda project_dir, **kw: frozenset({"off-loop-agent"}),
        )
        tmp = tmp_path / "config.json"
        tmp.write_text(json.dumps(_seed_config_with_every_field_set()))
        try:
            with unittest.mock.patch("kiro_crew.config.loader.config_path", return_value=tmp):
                ...
                app["state"] = types.SimpleNamespace(conversation_log=None)
                async with TestClient(TestServer(app)) as client:
                    resp = await client.get("/api/agents?project_path=/offloop/project/dir")
                    assert resp.status == 200

            # The release ran at all (otherwise the thread claim is vacuous)...
            assert release_thread[0] is not None
            # ...and not on the thread carrying the event loop.
            assert release_thread[0] != loop_thread
        finally:
            tmp.unlink(missing_ok=True)
    ...
    async def test_owner_directed_scan_is_sel_audited(self, monkeypatch, tmp_path: Path) -> None:
        ...
        monkeypatch.setattr(
            "kiro_crew.dashboard.handlers.agents._resolve_roster_project_path",
            lambda raw, holds=None: (raw, False),
        )
        monkeypatch.setattr(
            "kiro_crew.dashboard.handlers.agents.project_agent_names",
...
class TestResolveRosterProjectPathUncGate:
    ...
    def test_unc_path_denied_without_any_open(self, monkeypatch) -> None:
        ...
        monkeypatch.setattr(agents_mod, "hold_directory_tolerating_reparse", _boom)
    ...
    def test_benign_local_path_still_resolves_on_windows(self, tmp_path, monkeypatch) -> None:
        ...
        def _chain_ok(_p):
            yield False, False  # (refused, absent) -- ancestors are not the subject here

        monkeypatch.setattr(agents_mod, "held_verified_chain", _chain_ok)
        monkeypatch.setattr(
            "kiro_crew.dashboard.handlers.agents.is_sensitive_path",
            lambda p: False,
        )
        ...
        monkeypatch.setattr(agents_mod, "hold_directory_tolerating_reparse", lambda _p: 77)
        monkeypatch.setattr(agents_mod, "release_held_directory", lambda _fd: None)
    ...
    def test_root_pin_stays_held_through_realpath_and_sensitivity_check(
        ...
        monkeypatch.setattr(agents_mod, "hold_directory_tolerating_reparse", lambda _p: 77)
        monkeypatch.setattr(agents_mod, "release_held_directory", lambda _fd: order.append("close"))
    ...
    def test_a_cloud_placeholder_root_resolves_instead_of_being_denied(
        self, tmp_path, monkeypatch
    ) -> None:
        """A OneDrive Known Folder Move placeholder directory carries
        ``FILE_ATTRIBUTE_REPARSE_POINT`` and is on by default on Windows 11.
        ``pin_directory``'s blanket refusal of that attribute would deny a
        cloud-synced repo as if it were an attack -- project agents vanish and
        a false security denial is recorded, with no in-product remedy. The
        tolerant hold plus the label-aware check must let it resolve normally.
        """
        import os
        ...
        monkeypatch.setattr(agents_mod, "held_verified_chain", _chain_ok)
        monkeypatch.setattr(agents_mod, "is_unc_shape", lambda _p: False)
        # Label-aware: a cloud-placeholder tag is neither a symlink nor a
        # mount point, so nothing here is a redirect.
        monkeypatch.setattr(
    ...
    def test_a_redirecting_root_is_denied_before_realpath_follows_it(self, monkeypatch) -> None:
        """The owner points the picker at a directory that is itself a junction
        to a UNC share. Resolving it is what sends SMB credentials to the
        attacker's host, so the redirect must be refused BEFORE any realpath --
        asserted by making realpath itself an error.
        """
        import kiro_crew.dashboard.handlers.agents as agents_mod
        from kiro_crew.dashboard.handlers.agents import _resolve_roster_project_path
        ...
        monkeypatch.setattr(agents_mod, "held_verified_chain", _chain_ok)
        monkeypatch.setattr(agents_mod, "is_unc_shape", lambda _p: False)
        monkeypatch.setattr(agents_mod, "path_redirects", lambda _p: True)
        monkeypatch.setattr(agents_mod, "hold_directory_tolerating_reparse", lambda _p: 77)
        monkeypatch.setattr(agents_mod, "release_held_directory", lambda _fd: None)

        def _boom(*_a, **_k):  # pragma: no cover - only hit on regression
            raise AssertionError("realpath followed a redirecting root — SMB probe not prevented")
    ...
    def test_reparse_point_leaf_denied_by_the_pin_itself(self, monkeypatch) -> None:
        ...
        monkeypatch.setattr(agents_mod, "hold_directory_tolerating_reparse", _refuse_reparse_point)
    ...
    def test_missing_root_is_nothing_here_not_a_denial(self, monkeypatch) -> None:
        ...
        monkeypatch.setattr(agents_mod, "hold_directory_tolerating_reparse", _not_found)

        resolved, denied = _resolve_roster_project_path(r"C:\Users\me\does-not-exist")
        assert (resolved, denied) == ("", False)

    def test_the_root_hold_is_still_held_when_a_subdir_is_opened(
        self, tmp_path, monkeypatch
    ) -> None:
        """The root's hold must still be held at the moment each subdir is
        OPENED, not merely before the subdir's own checks.

        An agent-writable project swapped to a UNC junction after the root
        released is traversed by the subdir's own open while resolving the path
        TO it, which sends SMB credentials outbound -- and the subdir's pin
        cannot prevent that, because the traversal happens before its own open
        completes. Asserting "root still held" at subdir-open time is what makes
        ...
        monkeypatch.setattr(agents_mod, "held_verified_chain", _chain_ok)
        monkeypatch.setattr(agents_mod, "is_unc_shape", lambda _p: False)
        monkeypatch.setattr(agents_mod, "path_redirects", lambda _p: False)
        monkeypatch.setattr(
            "kiro_crew.dashboard.handlers.agents.is_sensitive_path", lambda _p: False
        )
        proj = tmp_path / "repo"
        ...
        live: set[int] = set()
        next_fd = [100]
        opened_with_root_released: list[str] = []

        def _hold(p):
            fd = next_fd[0]
            next_fd[0] += 1
            # The ROOT is the first hold taken; every later hold is a subdir, and
            # the root's descriptor must still be live at that moment.
            if live and 100 not in live:
                opened_with_root_released.append(str(p))
            live.add(fd)
            return fd
        ...
        monkeypatch.setattr(agents_mod, "hold_directory_tolerating_reparse", _hold)
        monkeypatch.setattr(agents_mod, "release_held_directory", _release)

        resolved, denied = _resolve_roster_project_path(str(proj))

        assert (resolved, denied) == (os.path.realpath(str(proj)), False)
        assert not opened_with_root_released, (
            "a subdir was opened while the root hold was already released, which is "
            f"the swap window: {opened_with_root_released}"
        )
        assert not live, "every hold must be released by the time resolution returns"
    ...
    def test_subdir_pin_stays_held_through_realpath_and_sensitivity_check(
        ...
        monkeypatch.setattr(agents_mod, "hold_directory_tolerating_reparse", _pin)

        def _close(_fd):
            order.append("close")

        monkeypatch.setattr(agents_mod, "release_held_directory", _close)
        ...
        # -> close, in that order -- proving the subdir pin stayed held through
        # both checks before being released. The ROOT's own close must come
        # LAST, not first: released at the root and re-taken per subdir, the
        # root could be swapped for a UNC junction in that gap and the subdir
        # open would then traverse it, which the subdir's own pin cannot
        # prevent, because the traversal happens while resolving the path TO
        # it. So the trailing "close" is the root's, and every subdir triple
        # precedes it.
        assert (
            order and order[-1] == "close"
        ), f"the ROOT pin must release last, after every subdir: {order}"
