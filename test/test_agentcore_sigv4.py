...
def test_proxy_bounds_inflight_and_inbound_reads() -> None:
    ...
    # Two-phase deadline: untrusted until the token verifies, long budget after.
    assert "self.connection.settimeout(PROXY_PREAUTH_DEADLINE_SECS)" in handle_src
    assert "self.connection.settimeout(PROXY_SOCKET_TIMEOUT_SECS)" in handle_src
    auth_at = handle_src.index("_auth_token_matches(presented, expected)")
    lifted_at = handle_src.index("self._authenticated()")
    assert auth_at < lifted_at
    ...
    assert sigv4.PROXY_PREAUTH_DEADLINE_SECS < sigv4.PROXY_SOCKET_TIMEOUT_SECS


def test_proxy_drops_idle_unauthenticated_connections_and_frees_their_slots(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """Slowloris from the shared loopback: PROXY_MAX_INFLIGHT idle sockets that
    never send a request line must not hold the handler slots for the 300 s body
    budget. Each is cut at the pre-auth deadline (wall clock, so trickling does
    not extend it) and a legitimate caller gets through right after."""
    import socket
    import time

    from kiro_crew.platform import agentcore_sigv4 as sigv4

    monkeypatch.setattr(sigv4, "PROXY_PREAUTH_DEADLINE_SECS", 0.4)
    ...
    proxy = sigv4.GatewaySigV4Proxy(
        "http://127.0.0.1:9/mcp", region="us-east-1", require_https=False
    )
    idle: list[socket.socket] = []
    try:
        listen = proxy.start()
        host, port = urlsplit(listen).hostname, urlsplit(listen).port
        assert host and port
        for _ in range(sigv4.PROXY_MAX_INFLIGHT):
            s = socket.create_connection((host, port), timeout=5)
            idle.append(s)
        # Trickle a byte on one of them: a per-recv timeout would be reset by
        # this; the wall-clock deadline is not.
        time.sleep(0.2)
        idle[0].sendall(b"P")
        # The server must have hung up on all of them by the deadline.
        deadline = time.monotonic() + 5
        closed = 0
        while closed < len(idle) and time.monotonic() < deadline:
            closed = 0
            for s in idle:
                s.settimeout(0.2)
                try:
                    ...
                    closed += 1
        assert closed == len(idle)
        # And a real request is served: the slots were released, not leaked.
        ...
        req = Request(
            ...
            headers={
                ...
                **proxy.session_headers(session_key),
            },
            method="POST",
        )
        with pytest.raises(HTTPError) as failed:
            urlopen(req, timeout=5)  # noqa: S310  # nosemgrep
        # 502: it reached the (dead) upstream, i.e. auth passed and a slot was ours.
...
def test_session_key_is_live_requires_registered_session() -> None:
    ...
    class _Mgr:
        ...
        def has_allocation_reservation(self, key: str) -> bool:
            # ``session/new`` in flight: reserved, not yet registered. The
            # Gateway MCP initialize happens inside that window.
            return key == "dashboard:reserved"
    ...
    try:
        ...
        assert _session_key_is_live("dashboard:reserved") is True
...
def test_proxy_recheck_honours_env_only_launch_configuration(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """A CloudFormation instance configures the posture and Gateway through its
    unit environment and may run with no policy document: the recheck reads the
    EFFECTIVE posture/URL (ceiling first, env only when none is loaded), so the
    documented launch path can sign. Leftover env never outranks a ceiling."""
    from kiro_crew.platform import agentcore_aws
    from kiro_crew.platform import agentcore_sigv4 as sigv4

    _permit_workload(monkeypatch)
    # No ceiling anywhere: the ceiling-only lookup would answer None here.
    ...
    monkeypatch.setattr(agentcore_aws, "authored_gateway_url", lambda: "")
    monkeypatch.setenv(agentcore_aws.ENV_POSTURE, "workload")
    monkeypatch.setenv(agentcore_aws.ENV_GATEWAY_URL, _LIVE_GATEWAY)
    assert sigv4._workload_proxy_still_permitted("dashboard:1", upstream_url=_LIVE_GATEWAY) is True
    # The env URL must still match the handling proxy's upstream.
    monkeypatch.setenv(
        agentcore_aws.ENV_GATEWAY_URL,
        "https://other.gateway.bedrock-agentcore.us-west-2.amazonaws.com/mcp",
    )
    assert sigv4._workload_proxy_still_permitted("dashboard:1", upstream_url=_LIVE_GATEWAY) is False
    # A loaded ceiling that pins login outranks the workload env.
    monkeypatch.setenv(agentcore_aws.ENV_GATEWAY_URL, _LIVE_GATEWAY)

    class _Gov:
        agentcore_gateway_url = _LIVE_GATEWAY

    monkeypatch.setattr(agentcore_aws, "_effective_governance_ceiling", lambda: _Gov())
    monkeypatch.setattr("kiro_crew.platform.governance.agentcore_posture", lambda _gov: "login")
    assert sigv4._workload_proxy_still_permitted("dashboard:1", upstream_url=_LIVE_GATEWAY) is False
...
def test_ensure_workload_proxy_starts_reuses_and_replaces(
    ...
    class _Fake:
        ...
        def session_headers(self, session_key: str, agent: str = "") -> dict[str, str]:
            self._generations[session_key] = "g"
            out = {
                sigv4.PROXY_AUTH_HEADER: sigv4.bound_proxy_auth_token(
                    self.client_token, session_key, agent, "g"
                ),
                sigv4.PROXY_SESSION_HEADER: session_key,
                sigv4.PROXY_GENERATION_HEADER: "g",
            }
            if agent:
                out[sigv4.PROXY_AGENT_HEADER] = agent
            return out
            self.listen_url = ""
            self._alive = False
...
def test_proxy_refuses_a_retired_generation_after_the_key_is_reused(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """The auth digest is bound to the generation the inject registered for the
    session key, and the handler requires THAT generation. A stalled predecessor
    whose key a successor reuses keeps a digest the per-boot token would still
    accept; without the generation check the successor's liveness would authorize
    the predecessor's hops (ABA). The generation header is also never forwarded."""
    from kiro_crew.platform import agentcore_sigv4 as sigv4
    ...
    class _Upstream(BaseHTTPRequestHandler):
        ...
        def do_POST(self) -> None:  # noqa: N802
            length = int(self.headers.get("Content-Length") or "0")
            self.rfile.read(length)
            seen["generation-forwarded"] = self.headers.get(sigv4.PROXY_GENERATION_HEADER)
            payload = b'{"ok":true}'
            self.send_response(200)
            self.send_header("Content-Type", "application/json")
            self.send_header("Content-Length", str(len(payload)))
    ...
    host, port = upstream.server_address[:2]
    monkeypatch.setattr(sigv4, "sign_aws_request", lambda **kw: dict(kw["headers"]))
    monkeypatch.setattr(sigv4, "_workload_proxy_still_permitted", lambda session_key="", **_k: True)
    monkeypatch.setattr(sigv4, "preferred_bind_port", lambda: 0)
    proxy = sigv4.GatewaySigV4Proxy(
        f"http://{host}:{port}/mcp", region="us-east-1", require_https=False
    ...
    try:
        ...
        first = proxy.session_headers(key)
        assert post(first) == 200
        assert seen["generation-forwarded"] is None  # stripped before signing
        # Successor registers the SAME key: the predecessor's headers die.
        second = proxy.session_headers(key)
        assert second[sigv4.PROXY_GENERATION_HEADER] != first[sigv4.PROXY_GENERATION_HEADER]
        assert post(first) == 401
        assert post(second) == 200
        # A digest valid for the old generation cannot be replayed with the new
        # generation header, nor can a missing generation pass.
        forged = dict(second)
        forged[sigv4.PROXY_AUTH_HEADER] = first[sigv4.PROXY_AUTH_HEADER]
        assert post(forged) == 401
        no_gen = {k: v for k, v in second.items() if k != sigv4.PROXY_GENERATION_HEADER}
        assert post(no_gen) == 401
        # Teardown retires the key outright.
        proxy.retire_session(key)
        assert post(second) == 401
    finally:
        proxy.stop()
        upstream.shutdown()
