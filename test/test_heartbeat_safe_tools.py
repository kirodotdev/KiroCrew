...
class TestIsHeartbeatSafeTool:
    ...
    def test_trusted_read_only_shell_command_approved(self) -> None:
        ...
        Uses non-git commands deliberately: every git subcommand is denied
        in heartbeat regardless of read-only classification (see
        ``test_shell_command_git_denied_even_when_read_only``).
        """
        event = _make_event(
            "",
    ...
    def test_shell_command_nonempty_tool_name_still_approves(self) -> None:
        ...
        heartbeat's ability to run a genuinely safe shell command. Uses a
        non-git command deliberately: every git subcommand is excluded from
        heartbeat's shell trust regardless of read-only classification (see
        ``test_shell_command_git_denied_even_when_read_only`` below)."""
        event = _make_event(
            "",
            tool_name="execute_bash",
            mcp_server_name="",
    ...
    def test_shell_command_git_denied_even_when_read_only(self) -> None:
        """Every git invocation is denied in heartbeat regardless of
        ``is_read_only_bash`` classification — ``git status``/``log``/
        ``diff``/etc. are on that classifier's allowlist, but git reads
        repo-local ``.git/config`` on invocation, and ``core.fsmonitor``/
        ``core.hooksPath`` name a program from that file rather than from
        the command line the classifier inspects. A heartbeat cycle's CWD
        is exactly the kind of place a local writer could plant a
        malicious ``.git/config``; unlike the interactive/cron paths,
        heartbeat has no approver at all to backstop that. Exercises
        several allowlisted git subcommands, not just ``status``, since the
        vector is git running at all."""
        for cmd in ("git status --porcelain", "git log -1", "git diff", "git branch"):
            event = _make_event(
                "",
                tool_name="execute_bash",
                mcp_server_name="",
                mcp_identity_trusted=True,
                is_shell=True,
                raw_tool_params={"command": cmd},
            )
            assert not _is_heartbeat_safe_tool(event), cmd

    def test_shell_command_git_exclusion_survives_compound_and_case(self) -> None:
        """The git exclusion must catch ``git`` in ANY segment of a compound
        command, and regardless of case — a naive check of only the whole
        string's raw first token is trivially bypassed by prepending an
        allowlisted verb (``pwd; git status``, ``pwd | git status``) or by
        varying case (``GIT status`` on a case-insensitive filesystem),
        because ``is_read_only_bash`` itself splits on
        ``&&``/``||``/``;``/newline and lowercases each segment's leading
        word before classifying it — so a later ``git`` segment still
        passes as read-only even when the string's first word is something
        else entirely."""
        bypass_attempts = (
            "pwd; git status",
            "pwd && git status",
        ...
        for cmd in bypass_attempts:
            ...
            assert not _is_heartbeat_safe_tool(event), cmd
        # Confidence check: a compound command with NO git anywhere still
        # approves — the exclusion targets git specifically, not every
        # compound command.
        clean_event = _make_event(
            ...
            mcp_identity_trusted=True,
            is_shell=True,
            raw_tool_params={"command": "pwd; ls -la"},
        )
        assert _is_heartbeat_safe_tool(clean_event)
    ...
    def test_mcp_served_shell_event_denied(self) -> None:
        """An MCP-served tool declaring ``is_shell=True`` with an EMPTY
        ``tool_name`` must NOT auto-approve — the unconditional empty-name
        check at the top of the ``event.is_shell`` branch denies before
        server-emptiness or edition-pinning is even considered. (A genuinely
        NAMED edition tool declaring the same ``is_shell=True`` shape is a
        different case — see
        ``TestEditionHeartbeatAllowlist.test_qualified_entry_matches_even_when_declared_execute_kind``,
        which is pinned to approve when the server+name are explicitly
        allowlisted.)"""
        event = _trusted_mcp(
...
class TestEditionHeartbeatAllowlist:
    ...
    def test_qualified_entry_matches_even_when_declared_execute_kind(self, monkeypatch) -> None:
        """A pinned ``@server/tool`` entry still auto-approves when the ACP
        event ALSO carries ``is_shell=True`` (an edition tool declaring
        ``kind="execute"``, which is exactly what makes ``event.is_shell``
        true — see ``acp/_dispatch.py::is_shell_kind``).

        ``event.is_shell`` is driven by the agent-declared ``kind`` field,
        not by whether a server backs the call; before this test's fix, a
        present ``mcp_server_name`` on a shell-shaped event returned False
        immediately instead of falling through to this matching, so a
        legitimately pinned edition tool that happened to declare
        ``kind="execute"`` was denied even though ``@builder-mcp/
        ReadInternalWebsites`` was explicitly allowlisted.
        """
        self._install_gate_extra(monkeypatch, ["@builder-mcp/ReadInternalWebsites"])
        event = _trusted_mcp(
            ...
            "builder-mcp",
            is_shell=True,
            raw_tool_params={"command": "ReadInternalWebsites --url https://example.com"},
        )
        assert _is_heartbeat_safe_tool(event)
...
class TestHeartbeatApproval:
    ...
    async def test_shell_command_still_needs_name_grant_clearance(
        ...
        unconditionally; heartbeat must too. Uses a non-git command
        deliberately: git is denied earlier, by
        ``_is_heartbeat_safe_tool`` itself, before ``name_grant`` is ever
        reached, so a git example here would make ``log_decline_mock``
        never fire and the assertion below moot."""
        sel_mock = MagicMock()
        monkeypatch.setattr("kiro_crew.slack.gateway.sel", lambda: sel_mock)
    ...
    async def test_shell_command_approves_when_name_grant_clears(
        ...
        gate, it does not remove the capability this PR restored. Uses a
        non-git command deliberately: git is excluded from heartbeat's
        shell trust regardless of name-grant clearance (see
        ``test_shell_command_git_denied_even_when_read_only`` in
        ``TestIsHeartbeatSafeTool``)."""
        sel_mock = MagicMock()
        monkeypatch.setattr("kiro_crew.slack.gateway.sel", lambda: sel_mock)
