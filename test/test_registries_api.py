...
class TestSshUrlParity:
    ...
    async def test_accepts_userless_ssh_url(self, tmp_path, monkeypatch):
        # ssh://git.amazon.com/pkg/Name is the canonical GitFarm form — no user@.
        _setup_env(tmp_path, monkeypatch)
        async with TestClient(TestServer(_make_app())) as client:
            resp = await client.put(
                "/api/apps/registries",
                json={
                    "registries": [
                        {"repo": "ssh://git.amazon.com/pkg/MyApps"},
                    ]
                },
            )
            assert resp.status == 200
            data = await resp.json()
            assert data["ok"] is True
            assert data["registries"][0]["repo"] == "ssh://git.amazon.com/pkg/MyApps"

    @pytest.mark.asyncio
    async def test_accepts_user_at_ssh_url(self, tmp_path, monkeypatch):
        # ssh://alias@git.amazon.com/pkg/Name — explicit userinfo form.
        _setup_env(tmp_path, monkeypatch)
        async with TestClient(TestServer(_make_app())) as client:
            resp = await client.put(
                "/api/apps/registries",
                json={
                    "registries": [
                        {"repo": "ssh://alias@git.amazon.com/pkg/MyApps"},
                    ]
                },
            )
            assert resp.status == 200
            data = await resp.json()
            assert data["ok"] is True
            assert data["registries"][0]["repo"] == "ssh://alias@git.amazon.com/pkg/MyApps"
    ...
    async def test_userless_ssh_emits_trust_grant(self, tmp_path, monkeypatch):
        ...
        async with TestClient(TestServer(_make_app())) as client:
            ...
            assert resp.status == 200
            data = await resp.json()
            assert data["newlyTrustedHosts"] == ["git.amazon.com"]
        ...
        assert len(grants) == 1
        assert "host=git.amazon.com" in grants[0].kwargs["resources"]
    ...
    async def test_refresh_accepts_userless_ssh_registry(self, tmp_path, monkeypatch):
        ...
        async with TestClient(TestServer(_make_app())) as client:
            resp = await client.post(
                "/api/apps/registries/refresh",
                json={"repo": "ssh://git.amazon.com/pkg/MyApps"},
            )
            assert resp.status == 200
            data = await resp.json()
            assert data["ok"] is True
            assert data["refreshed"] == ["gitfarm"]
...
class TestIsSafeRepoIdentifier:
    ...
    @pytest.mark.parametrize(
        "url",
        [
            "ssh://git.amazon.com/pkg/MyApps",  # userless
            "ssh://alias@git.amazon.com/pkg/MyApps",  # user@
