...
async def test_cleanup_hook_preserves_active_pid_fence_and_runs_off_loop(monkeypatch, safe):
    import threading
    ...
    boundary = manager._cleanup_boundary()
    monkeypatch.setattr(boundary, "_active_pids", lambda: ({201, 301}, safe))
    loop_thread = threading.get_ident()
    calls = []

    def reap(active_pids):
        calls.append((active_pids, threading.get_ident()))
        return SimpleNamespace(reclaimed=0)

    monkeypatch.setattr("kiro_crew.session.reap_abandoned_agent_scopes", reap)
    await manager._reap_agent_scopes_hook()

    if safe:
        assert len(calls) == 1
        assert calls[0][0] == {201, 301}
