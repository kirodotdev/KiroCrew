...
@pytest.mark.asyncio
async def test_discovery_preserves_existing_global_named_and_private_bindings(monkeypatch):
    from kiro_crew.config.sections import MemoryStoreConfig

    def prepare():
        cfg = _environment(monkeypatch, "linux", "kas", "auto", "namespace", False)
        cfg.memory_stores["legacy-team"] = MemoryStoreConfig(memory_version=1)
        (memory_stores_root() / "legacy-team").mkdir(parents=True)
        cfg.agents["named-member"] = KiroCrewAgentConfig(memory_store="legacy-team")
        cfg.agents["private-member"] = KiroCrewAgentConfig()
        private_store = provision_member_memory(cfg, "private-member")
        cfg.save()
        return private_store

    private_store = await asyncio.to_thread(prepare)
    app = _app(monkeypatch)
    names = ["reviewer", "named-member", "private-member", "new-agent"]
    monkeypatch.setattr(
        handlers,
        "list_agents",
        lambda: [
    ...
    async with TestClient(TestServer(app)) as client:
        response = await client.post("/api/agents/sync")
        assert response.status == 200, await response.text()
        assert (await response.json())["synced"] == ["new-agent"]
    loaded = await asyncio.to_thread(KiroCrewConfig.load)
    assert loaded.memory_stores == before.memory_stores
    for name, store in {
        "reviewer": "default",
        "named-member": "legacy-team",
        "private-member": private_store,
        "new-agent": "default",
    }.items():
        assert await asyncio.to_thread(require_member_memory_store, loaded, name) == store
        if name != "new-agent":
            assert loaded.agents[name] == before.agents[name]
