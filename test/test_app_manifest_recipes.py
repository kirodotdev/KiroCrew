...
Covers SlackRecipe, CronRecipe, RecipesConfig, and RecipeDependencies
...
# ---------------------------------------------------------------------------
# RecipeDependencies  (Kiro Crew uses capabilityPackages)
# ---------------------------------------------------------------------------


class TestRecipeDependencies:
    def test_empty_to_dict_is_empty(self):
        assert RecipeDependencies().to_dict() == {}

    def test_with_capability_packages(self):
        d = RecipeDependencies(capabilityPackages=["FooPkg", "BarPkg"])
        assert d.to_dict() == {"capabilityPackages": ["FooPkg", "BarPkg"]}

    def test_round_trip(self):
        original = RecipeDependencies(capabilityPackages=["A", "B"])
        restored = RecipeDependencies.from_dict(original.to_dict())
        assert restored == original

    def test_from_dict_filters_empty_strings(self):
        d = RecipeDependencies.from_dict({"capabilityPackages": ["", "Foo", None, "Bar"]})
        assert d.capabilityPackages == ["Foo", "Bar"]
...
class TestSlackRecipeRoundTrip:
    ...
    def test_with_dependencies_round_trip(self):
        d = _valid_slack_recipe(dependencies={"capabilityPackages": ["MyAgents"]})
        original = SlackRecipe.from_dict(d)
        assert original.dependencies.capabilityPackages == ["MyAgents"]
        restored = SlackRecipe.from_dict(original.to_dict())
        assert restored == original
    ...
    def test_dependencies_omitted_when_empty(self):
        d = SlackRecipe.from_dict(_valid_slack_recipe()).to_dict()
...
class TestCronRecipeRoundTrip:
    ...
    def test_every_secs_round_trip(self):
        d = _valid_cron_recipe(schedule="", every_secs=300)
        original = CronRecipe.from_dict(d)
        assert original.every_secs == 300
    ...
    def test_persistent_session_default_omitted(self):
        ...
        assert "persistent_session" not in d

    def test_persistent_session_false_serialized(self):
        d = _valid_cron_recipe()
        d["persistent_session"] = False
        original = CronRecipe.from_dict(d)
        assert original.persistent_session is False
        out = original.to_dict()
        assert out["persistent_session"] is False
...
class TestRecipesValidation:
    ...
    def test_valid_cron_recipe_every_secs_passes(self):
        errors = self._validate(crons=[_valid_cron_recipe(schedule="", every_secs=300)])
        assert errors == []
    ...
    def test_cron_missing_schedule_and_every_secs(self):
        errors = self._validate(crons=[_valid_cron_recipe(schedule="", every_secs=0)])
        assert any("schedule" in e or "every_secs" in e for e in errors)

    def test_cron_both_schedule_and_every_secs(self):
        errors = self._validate(crons=[_valid_cron_recipe(schedule="0 12 * * *", every_secs=300)])
