...
def test_locations_import_does_not_load_server_stack(tmp_path) -> None:
    code = """
import sys
import kiro_crew.acp_server.locations
heavy = {
    "kiro_crew.acp_server.gateway",
    "kiro_crew.acp_server.http_backend",
    "kiro_crew.acp_server.mcp_supervisor",
    "kiro_crew.acp_server.server",
    "kiro_crew.acp_server.transport",
}
...
class TestKiroCliFileTools:
    ...
    @pytest.mark.parametrize(
        "raw_params",
        [
            {"path": "/abs/secret-segment/main.py"},
            {
                "path": "/workspace",
                "operations": [{"mode": "Line", "path": "/abs/secret-segment/main.py"}],
            },
            {"files": [{"path": "/abs/secret-segment/main.py"}]},
            {"files": ["/abs/secret-segment/main.py"]},
        ],
    )
    def test_redacted_paths_are_not_emitted(self, raw_params: dict[str, Any]) -> None:
        extracted = extract_tool_locations("fs_read", raw_params)
        assert (
            sanitize_tool_locations(
                extracted,
    ...
    @pytest.mark.parametrize(
        "raw_params",
        [
            {"path": "/abs/bad\x00name.py"},
            {
                "path": "/workspace",
                "operations": [{"mode": "Line", "path": "/abs/bad\x00name.py"}],
            },
            {"files": ["/abs/bad\x00name.py"]},
        ],
    )
    def test_nul_paths_are_dropped(self, raw_params: dict[str, Any]) -> None:
        assert extract_tool_locations("fs_read", raw_params) == []
    ...
    def test_non_regular_edit_target_stays_path_only(self, monkeypatch: pytest.MonkeyPatch) -> None:
        from stat import S_IFIFO
        from types import SimpleNamespace

        from kiro_crew.acp_server import locations
        ...
        monkeypatch.setattr(
            ...
            SimpleNamespace(
                ...
                stat=lambda _path: SimpleNamespace(st_mode=S_IFIFO, st_size=0),
            ),
        )

        def _unexpected_read(*_args: Any, **_kwargs: Any) -> None:
            pytest.fail("non-regular edit target reached safe_read_prefix()")

        monkeypatch.setattr(locations, "safe_read_prefix", _unexpected_read)
        result = extract_tool_locations(
            "str_replace",
            {"path": "/tmp/agent-fifo", "oldStr": "x", "newStr": "y"},
        )
        assert result == [{"path": "/tmp/agent-fifo"}]

    def test_sensitive_edit_target_is_never_read(self, monkeypatch: pytest.MonkeyPatch) -> None:
        from types import SimpleNamespace

        from kiro_crew.acp_server import locations

        monkeypatch.setattr(locations, "is_sensitive_path", lambda _path: True)

        def _unexpected_stat(*_args: Any, **_kwargs: Any) -> None:
            pytest.fail("sensitive edit target reached stat()")

        def _unexpected_read(*_args: Any, **_kwargs: Any) -> None:
            pytest.fail("sensitive edit target reached safe_read_prefix()")
        ...
        monkeypatch.setattr(
            ...
            SimpleNamespace(
                name=os.name,
                path=SimpleNamespace(realpath=lambda path: path),
                stat=_unexpected_stat,
            ),
        )
        monkeypatch.setattr(locations, "safe_read_prefix", _unexpected_read)
        result = extract_tool_locations(
            "str_replace",
            {"path": "/home/user/.ssh/id_rsa", "oldStr": "x", "newStr": "y"},
        )
        assert result == [{"path": "/home/user/.ssh/id_rsa"}]
...
class TestSchemaEnforcement:
    ...
    def test_posix_path_keeps_protocol_identity_on_windows(
        self, monkeypatch: pytest.MonkeyPatch
    ) -> None:
        from kiro_crew.acp_server import locations

        monkeypatch.setattr(locations.os, "name", "nt")
        assert extract_tool_locations("write", {"path": "/remote/main.py"}) == [
            {"path": "/remote/main.py"}
        ]
...
@pytest.mark.skipif(os.name == "nt", reason="drive-letter locations pass through")
