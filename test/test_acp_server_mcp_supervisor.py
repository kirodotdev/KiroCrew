...
def _passthrough_sandbox(monkeypatch):
    ...
    """
    with tempfile.TemporaryDirectory(prefix="acp-sup-", dir=short_tmp_base()) as test_root:
...
class TestCleanup:
    def test_process_group_reap_uses_retained_group_id(self, monkeypatch) -> None:
        ...
        running = _RunningServer(
            name="fixture",
            proc=SimpleNamespace(pid=321, returncode=None),
    ...
    def test_process_group_reap_refuses_reused_leader_pid(self, monkeypatch) -> None:
        ...
        monkeypatch.setattr(platform_compat, "pid_exists", lambda pid: pid == 321)
        running = _RunningServer(
            name="fixture",
            proc=SimpleNamespace(pid=321, returncode=1),
            process_group_id=321,
        )
