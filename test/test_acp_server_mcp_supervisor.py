...
class TestCleanup:
    def test_process_group_reap_uses_platform_compat(self, monkeypatch) -> None:
        calls: list[tuple[int, object]] = []

        def fake_kill_process_tree(pid: int, sig: object) -> None:
            calls.append((pid, sig))

        monkeypatch.setattr(platform_compat, "kill_process_tree", fake_kill_process_tree)
        running = _RunningServer(name="fixture", proc=SimpleNamespace(pid=321))
        running._reap_group()

        assert calls == [(321, platform_compat.SIGKILL)]
