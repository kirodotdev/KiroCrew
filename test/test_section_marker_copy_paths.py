...
class TestTransferPreservesSectionMarkers:
    ...
    @pytest.mark.asyncio
    async def test_imported_markers_land_between_the_turns_they_separated(
        self, tmp_path, monkeypatch
    ):
        """Placement is the whole job: a marker's ``at`` indexes the MESSAGE sequence.

        Import splices markers into the row sequence before the shared materialiser
        hydrates it, so an off-by-one does not error — it silently moves a chapter
        break to the wrong seam, or drops a trailing one whose index equals
        ``len(messages)`` and which no per-message step ever reaches. Pinning the
        whole role order catches both, and catches a reordering that leaves the
        count right.
        """
        ...
        body = {
            ...
            "origin": "peer",
            "messages": [
                {"role": "user", "content": "first", "ts": ""},
                {"role": "assistant", "content": "second", "ts": ""},
            ],
            "section_markers": [
                {"at": 0, "label": "leading", "content": "— End of section: leading —", "ts": ""},
                {"at": 1, "label": "middle", "content": "— End of section: middle —", "ts": ""},
                {"at": 2, "label": "trailing", "content": "— End of section: trailing —", "ts": ""},
            ],
        }
        ...
        roles = [m.get("role") for m in imported.messages]
        assert roles == [
            SECTION_MARKER_ROLE,
            "user",
            SECTION_MARKER_ROLE,
            "assistant",
            SECTION_MARKER_ROLE,
        ], f"markers did not land at the seams they were recorded at: {roles}"
        labels = [
            (m.get("meta") or {}).get("label")
            for m in imported.messages
            if m.get("role") == SECTION_MARKER_ROLE
        ]
        assert labels == ["leading", "middle", "trailing"], (
            f"markers landed in the wrong order: {labels}"
        )
