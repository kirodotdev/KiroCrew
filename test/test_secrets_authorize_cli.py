...
def _run_authorize(monkeypatch, tmp_path, **ns):
    ...
    # signing succeeds (verification derives the same subkey from these bytes).
    # A test that exercises the "no signing key" abort passes with_sel_root=False.
    if ns.get("with_sel_root", True):
        _sel_root = tmp_path / "sel_hmac.key"
        if not _sel_root.exists():
            _sel_root.write_bytes(b"S" * 32)
        monkeypatch.setattr("kiro_crew.sel.sel_hmac_key_path", lambda: _sel_root)
        monkeypatch.setattr("kiro_crew.sel._sel_hmac_key_bytes", lambda: b"S" * 32)
    else:
        _missing = tmp_path / "sel_hmac.key"
        monkeypatch.setattr("kiro_crew.sel.sel_hmac_key_path", lambda: _missing)
    ...
    args = argparse.Namespace(
        ...
        return_body=ns.get("return_body", False),
    )
