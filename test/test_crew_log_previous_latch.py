...
``previous`` edge at all. The superseded store was left unlinked AND its
interrupted tail unclosed, which is the pair of failures the edge exists to
remove, and an append-only record has no later chance to add them.
