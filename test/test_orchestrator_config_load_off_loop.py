...
async def test_plan_cancel_during_the_config_load_does_not_start_the_plan(
    ...
    async def _fake_run_chat(_state: Any, _slot: Any, context: str) -> None:
    ...
    assert await asyncio.to_thread(entered.wait, 10), "the config worker never started"
...
async def test_a_plan_started_after_a_cancel_still_runs(monkeypatch: Any) -> None:
    ...
    plan — arming a new one clears ``slot._orch_tracker``
    (``_reset_auto_run_for_new_plan``), so the next bootstrap publishes a fresh,
    unstopped tracker. This drives that sequence and requires the new plan to
    run.
    """
    threads: list[int] = []
    _record_config_loads(monkeypatch, threads)
    ...
    # Arming a new plan drops it, exactly as _reset_auto_run_for_new_plan does.
    slot._orch_tracker = None
    slot._auto_run = True
