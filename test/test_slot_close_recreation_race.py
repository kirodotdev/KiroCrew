...
async def test_rows_only_handover_persists_a_retained_context_residual(tmp_path) -> None:
    ...
    A close that retains a context-owing note leaves a context-only residual under a
    fresh id and commits the note's visible row. If the hand-over write defers
    ``deferred_notes`` with the other writer's presentation fields, that residual
    never reaches disk while the entry it replaced is carried back verbatim -- and
    the next restore retires that entry against its now-committed row, so the
    context half is gone with nothing left holding it.
