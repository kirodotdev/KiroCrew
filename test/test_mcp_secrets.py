...
def _patch_transport(
    ...
    *,
    ...
    def _urlopen(req: Any, timeout: int = 0, *, socket_path: Any = None) -> Any:
        captured.append(req)
...
def test_self_mints_the_proof_for_a_direct_member_call(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    # Patch it where mcp_secrets looks it up (module-level bound name), not on the
    # source module — mcp_secrets imports the symbol at module scope.
    monkeypatch.setattr(
        mcp_secrets, "issue_member_session_proof", lambda sk, pid, *, audience: "c2VsZg.beef"
    )
