...
class TestEveryForwardableControlIsInheritedOrNamed:
    ...
    #: Controls that ARE inherited but whose restore-path admission is owned
    #: elsewhere, with the reason each is exempt from ``_PEER_OWNED_RESTORE``.
    #: An exemption here is a claim about the OTHER owner's behaviour, so it has
    #: to name that owner -- otherwise it reads as permission to skip the table.
    ADMITTED_ELSEWHERE = {
        "agent": (
            "restored by chat_persistence._restored_agent_name, which prefers a "
            "locally committed protected selection and otherwise returns the "
            "persisted value unrewritten. A peer-bound slot never acquires that "
            "record: api_chat_slot_agent early-returns into _apply_remote_pick, "
            "which persists the accepted pick to transcript metadata and never "
            "calls record_agent_selection -- so the peer's own value is what "
            "comes back, and this table would have no decision to make. It is "
            "NOT re-admitted on read, which is main's pre-existing treatment of "
            "the field on both the local and the remote branch; tracked "
    ...
    def _forwardable() -> set[str]:
        ...
        return set(_PEER_CONTROL_SEGMENTS)
    ...
    def test_every_inherited_control_has_a_peer_owned_admission(self):
        """Inheriting a control is only half of owning it. The other half is what
        happens to the value on the way back off disk: five review rounds of this
        family were a LOCAL vocabulary meeting a peer-owned value at one more site
        — membership, a display-key rejection, a provider-keyed canonicalization,
        a deprecation rename — each of which blanks or rewrites the pin and hands
        the picker back the overwrite the inherit exists to prevent.

        So the guard covers interpretation, not just inheritance: a control that
        is inherited must have an entry in ``_PEER_OWNED_RESTORE``, which is the
        one decision for "this value is the peer's, not ours" — or be named in
        ``ADMITTED_ELSEWHERE`` with the restore path that owns it instead. The
        second branch exists because a control whose restore is owned by another
        function would otherwise force a table entry no caller reaches, and a
        dead entry claims a decision that never happens."""
        from kiro_crew.dashboard.chat_persistence import _PEER_OWNED_RESTORE

        for control in sorted(self._inherited_keys() & self._forwardable()):
            assert control in _PEER_OWNED_RESTORE or control in self.ADMITTED_ELSEWHERE, (
                f"{control!r} is inherited from the peer but has no admission in "
                f"_PEER_OWNED_RESTORE and is not named in ADMITTED_ELSEWHERE, so "
                f"the restore path falls through to this machine's own treatment "
                f"of it."
            )

    def test_the_elsewhere_list_names_only_inherited_forwardable_controls(self):
        """An exemption for a control that is not inherited has no persisted peer
        value to admit anywhere, so it would excuse a decision never reached."""
        assert set(self.ADMITTED_ELSEWHERE) <= (self._inherited_keys() & self._forwardable())

    def test_the_two_exemption_lists_do_not_overlap(self):
        """A control cannot be both deliberately not inherited and inherited-but-
        admitted-elsewhere; an overlap means one of the two reasons is stale."""
        assert not (set(self.ADMITTED_ELSEWHERE) & set(self.NOT_INHERITED))

    def test_the_admission_table_does_not_duplicate_an_elsewhere_exemption(self):
        """The reverse of the exemption: if the table DOES carry an entry, the
        exemption is stale and one of the two is wrong about who admits it."""
        from kiro_crew.dashboard.chat_persistence import _PEER_OWNED_RESTORE

        assert not (set(_PEER_OWNED_RESTORE) & set(self.ADMITTED_ELSEWHERE))

    def test_the_admission_table_names_only_forwardable_controls(self):
        """The reverse direction, so the table cannot rot: an admission for a
        control the peer never owns has nothing to admit."""
        from kiro_crew.dashboard.chat_persistence import _PEER_OWNED_RESTORE

        assert set(_PEER_OWNED_RESTORE) <= self._forwardable()

    def test_the_admission_table_names_nothing_that_is_deliberately_not_inherited(self):
        """A control exempt from inheritance has no persisted peer value to admit,
        so an entry for it would claim a decision that is never reached."""
        from kiro_crew.dashboard.chat_persistence import _PEER_OWNED_RESTORE

        assert not (set(_PEER_OWNED_RESTORE) & set(self.NOT_INHERITED))
