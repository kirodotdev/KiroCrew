...
async def test_handlers_fail_closed_after_ancestor_replacement(campaign, monkeypatch, consumer):
    ...
    knowledge_pipeline = SimpleNamespace(ingest_text=AsyncMock())
    ...
    if consumer == "report":
        ...
        knowledge_pipeline.ingest_text.assert_not_awaited()
    else:
...
def test_truncated_report_never_reads_evidence_from_a_replaced_research_root(campaign, monkeypatch):
    campaign_id, directory = campaign
    report = directory / "FINDINGS.md"
    report.write_bytes(b"x" * (h._REPORT_VIEW_MAX_BYTES + 1))

    replacement_root = directory.parent.with_name("replacement-research")
    replacement_findings = replacement_root / campaign_id / "findings"
    replacement_findings.mkdir(parents=True)
    replacement_finding = replacement_findings / "cycle_999.json"
    replacement_finding.write_text(
        '{"cycle":999,"summary":"replacement evidence must stay private"}',
        encoding="utf-8",
    )
    ...
    finding_reads: list[h._CampaignIdentity] = []
    swapped = False

    def _swap_after_report_read(identity_arg, relative_parts, **kwargs):
        nonlocal swapped
        data = real_read(identity_arg, relative_parts, **kwargs)
        if relative_parts == ("FINDINGS.md",) and not swapped:
            swapped = True
            identity.root.rename(identity.root.with_name("captured-research"))
            replacement_root.rename(identity.root)
        elif relative_parts[:1] == ("findings",):
            finding_reads.append(identity_arg)
        return data

    monkeypatch.setattr(h, "_read_campaign_file_bytes", _swap_after_report_read)
    try:
        rendered = h._read_report_for_identity(identity)
    finally:
        identity.close()
    ...
    assert "Recent cycle evidence" not in rendered
    assert "replacement evidence must stay private" not in rendered
    assert finding_reads == []
    live_replacement = identity.root / campaign_id / "findings" / replacement_finding.name
    assert live_replacement.read_text(encoding="utf-8").endswith('private"}')


def test_truncated_report_reads_latest_evidence_through_its_captured_identity(
    campaign, monkeypatch
):
    campaign_id, directory = campaign
    (directory / "FINDINGS.md").write_bytes(b"x" * (h._REPORT_VIEW_MAX_BYTES + 1))
    findings_dir = directory / "findings"
    for cycle in range(1, 7):
        (findings_dir / f"cycle_{cycle:03d}.json").write_text(
            json.dumps({"cycle": cycle, "summary": f"captured evidence {cycle}"}),
            encoding="utf-8",
        )
    ...
    def _record_identity(identity_arg, relative_parts, **kwargs):
        if relative_parts[:1] == ("findings",):
            finding_reads.append(identity_arg)
        return real_read(identity_arg, relative_parts, **kwargs)

    monkeypatch.setattr(h, "_read_campaign_file_bytes", _record_identity)
    try:
        rendered = h._read_report_for_identity(identity)
    finally:
        identity.close()
    ...
    assert rendered is not None
    assert "captured evidence 1" not in rendered
    assert "captured evidence 2" not in rendered
    evidence_offsets = [rendered.index(f"captured evidence {cycle}") for cycle in range(3, 7)]
    assert evidence_offsets == sorted(evidence_offsets)
    assert len(finding_reads) == h._REPORT_RECENT_CYCLES
    assert all(read_identity is identity for read_identity in finding_reads)
...
@pytest.mark.skipif(
    os.name != "posix",
    reason="the root-link swap requires POSIX symlink semantics",
)
def test_captured_identity_refuses_research_root_link_swap(tmp_path, monkeypatch):
    """A captured campaign must not outlive a changed named research root."""
    from kiro_crew.apps.builtins.auto_research import handlers as h

    original_root = tmp_path / "original-research"
    replacement_root = tmp_path / "replacement-research"
    original_root.mkdir()
    replacement_root.mkdir()
    root_link = tmp_path / "research"
    root_link.symlink_to(original_root, target_is_directory=True)
    monkeypatch.setattr(h, "RESEARCH_DIR", root_link)
    monkeypatch.setattr(h, "DB_PATH", tmp_path / "campaigns.db")
    campaign_id = h.create_campaign(
        {
            "question": "Research root link ownership must remain stable after capture",
            "sources": ["web"],
        }
    )["id"]
    identity = h._campaign_identity(campaign_id)
    ...
    root_link.unlink()
    root_link.symlink_to(replacement_root, target_is_directory=True)
    try:
        with pytest.raises(PermissionError, match="research root identity changed"):
            with identity.pin():
                pass
    finally:
        identity.close()
...
@pytest.mark.skipif(
    os.name != "posix",
    reason="the root replacement requires POSIX rename semantics",
)
def test_fresh_identity_accepts_campaign_under_recreated_root(campaign):
    """A new root does not inherit a prior root's campaign identity record."""
    campaign_id, directory = campaign
    root = directory.parent
    parked = root.with_name("parked-research")
    root.rename(parked)
    root.mkdir()
    (root / campaign_id).mkdir()

    fresh = h._campaign_identity(campaign_id)
...
def test_MUTATION_skipping_live_root_binding_reads_detached_campaign(tmp_path, monkeypatch):
    """Without the root check, a changed link still serves the detached campaign."""
    from kiro_crew.apps.builtins.auto_research import handlers as h
    ...
    campaign_id = h.create_campaign(
        {
            "question": "Detached campaign reads must fail after a root link swap",
            "sources": ["web"],
        }
    )["id"]
    directory = original_root / campaign_id
    (directory / "FINDINGS.md").write_text("detached report", encoding="utf-8")
    identity = h._campaign_identity(campaign_id)
    assert identity is not None

    root_link.unlink()
    root_link.symlink_to(replacement_root, target_is_directory=True)
    monkeypatch.setattr(h._CampaignIdentity, "_validate_live_root", lambda _self: None)
    try:
        assert h._read_report_for_identity(identity) == "detached report"
    finally:
        identity.close()
