...
class TestBuildLauncherScriptCcMode:
    ...
    @pytest.mark.parametrize("private_memory", [False, True])
    def test_docker_config_is_selectively_exposed_when_enabled(self, sandbox_level, private_memory):
        script = _build_launcher_script(
            sandbox_level, private_memory=private_memory, expose_docker_config=True
        )
        compile(script, "<sandbox-launcher>", "exec")
        docker_line = next(
            line for line in script.splitlines() if line.startswith("DOCKER_CONFIG_PATH = ")
        )
        assert ".docker/config.json" in docker_line
        assert '"creating private Docker registry snapshot"' in script
        assert 'b"size=65536,mode=0700"' in script
        assert '"sealing Docker registry config %s" % DOCKER_CONFIG_PATH' in script
        private_marker = "# Private member view: Global V1 remains exclusively gateway-owned."
        assert (private_marker in script) is private_memory
        if private_memory:
            assert script.index("docker_config_data = None") < script.index(private_marker)
            assert script.index(private_marker) < script.index("for d in SENSITIVE_DIRS:")
...
#: and Docker pre-read sit between this block and the credential loop, so the
#: Docker marker is the first stable boundary that excludes both unrelated
#: setup paths from a slice named for the ordinary expose pre-read.
...
_DOCKER_BLOCK_END = "# Private windows: a directory INSIDE a hidden tree that stays"
...
class TestDockerConfigPreRead:
    ...
    def test_fifo_is_refused_without_waiting_for_a_writer(self, tmp_path: Path) -> None:
        source = tmp_path / "config.json"
        os.mkfifo(source)
        real_open = os.open

        def checked_open(path, flags):
            # Fail before opening if the regression would hang the worker.
