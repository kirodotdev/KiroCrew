...
class TestScriptHookDataclasses:
    ...
    def test_a_persisted_enabled_keeps_the_meaning_its_owner_gave_it(self):
        """hooks.json is hand-editable and ``bool("false")`` is True, so a raw read runs it."""
        for raw, want in (
            ("false", False),
            ("off", False),
            ("no", False),
            ...
            assert ScriptHook.from_dict({"enabled": raw}).enabled is want, repr(raw)
    ...
    def test_the_lane_event_stays_audited_and_its_token_charset_stays_narrow(self, monkeypatch):
        """Two one-line mutations that pass silently: one un-audits a run, one admits a glob."""
        assert hooks_mod._audit_gate_row("slot:1", "h") is True

        def _writer_cannot_persist(*a, **k):
            raise OSError(28, "No space left on device")

        monkeypatch.setattr(hooks_mod, "_hook_invocation_writer", _writer_cannot_persist)
        assert hooks_mod._audit_gate_row("slot:1", "h") is False, "a lost row must refuse the run"
        assert not any(hooks_mod._is_token_safe(t) for t in ("*", "?", "a b", "A", "x:y", "p;q"))
        assert all(hooks_mod._is_token_safe(t) for t in ("done", "in.review", "a_b-c", "9"))
...
class TestFireDispatch:
    ...
    def recorder(self, monkeypatch):
        ...
        async def _fake_run(hook, context="", hook_event=None, **kw):
