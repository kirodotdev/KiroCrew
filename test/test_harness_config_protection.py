...
class TestRoutingAttestationsWriteProtection:
    """``backend-routing-attestations.json`` is a selectability grant one step
    downstream of ``harnesses.json`` (``acp.harness.routing_verification``): an
    agent that could write it would make an unverified harness pickable. It sits
    under every fence the descriptor file does -- the tool gate, the sandbox
    read-only seal (with the absent-file pre-create), and the child withhold --
    while staying READABLE, since the gateway loads it on every boot and listing.
    """

    _ATTEST_LEAF = "backend-routing-attestations.json"

    @pytest.mark.parametrize("prefix", _PREFIXES)
    def test_attestations_file_is_write_protected(self, prefix: str) -> None:
        assert is_sensitive_write_path(f"{prefix}/{self._ATTEST_LEAF}")
        assert is_sensitive_path(f"{prefix}/{self._ATTEST_LEAF}") is False

    def test_sandbox_seals_it_beside_the_descriptor_file(self) -> None:
        from kiro_crew import sandbox

        assert self._ATTEST_LEAF in sandbox._CREW_READONLY_LEAVES
        assert self._ATTEST_LEAF in sandbox._CREW_PRECREATE_READONLY_FILE_LEAVES
        assert self._ATTEST_LEAF in sandbox._CREW_CHILD_WITHHELD_LEAVES

    def test_the_leaf_name_is_the_one_the_verifier_writes(self) -> None:
        from kiro_crew.acp.harness.routing_verification import ROUTING_ATTESTATIONS_LEAF

        assert ROUTING_ATTESTATIONS_LEAF == self._ATTEST_LEAF


class TestGrantFilesRefuseAliases:
    """Both grant files are STRICT no-follow leaves and their consumers refuse an alias.

    A bind seal covers the referent, so a symlink at the sealed NAME leaves the name
    replaceable in its writable parent, and a second hardlink reaches the same inode
    from outside the mount. For an execution grant (``harnesses.json``) and the
    selectability grant downstream of it, the next boot would then read a forged grant
    through the sealed name -- so the leaves are on the strict list beside
    ``cloud.json`` (with their overlap wording), and each consumer asks the alias
    question itself and serves NOTHING on refusal.
    """

    _LEAVES = ("harnesses.json", "backend-routing-attestations.json")

    def test_both_files_are_strict_nofollow_leaves_with_overlap_wording(self) -> None:
        from kiro_crew import sandbox
    ...
    def test_a_symlinked_descriptor_file_serves_nothing(self, tmp_path) -> None:
        from kiro_crew.acp.harness.descriptor import load_operator_descriptors
        ...
        link = tmp_path / "harnesses.json"
        try:
            os.symlink(real, link)
        except (OSError, NotImplementedError):
            pytest.skip("cannot create symlinks here")
        valid, invalid = load_operator_descriptors(path=link)
        assert valid == ()
        assert invalid[0][0] == "" and "lone regular file" in invalid[0][1][0]
        # A lone regular file with the same content is served.
        link.unlink()
        link.write_text(real.read_text("utf-8"), encoding="utf-8")
        valid, invalid = load_operator_descriptors(path=link)
        assert [d.id for d in valid] == ["acme"] and invalid == ()

    def test_a_hardlinked_descriptor_file_serves_nothing(self, tmp_path) -> None:
        from kiro_crew.acp.harness.descriptor import load_operator_descriptors

        target = tmp_path / "harnesses.json"
        target.write_text("{}", encoding="utf-8")
        try:
            os.link(target, tmp_path / "alias.json")
        except (OSError, NotImplementedError, AttributeError):
            pytest.skip("cannot create hardlinks here")
        valid, invalid = load_operator_descriptors(path=target)
        assert valid == ()
        assert invalid[0][0] == "" and "lone regular file" in invalid[0][1][0]

    @pytest.mark.skipif(not hasattr(os, "symlink"), reason="symlinks unavailable")
    def test_an_aliased_attestation_store_grants_nothing(self, tmp_path) -> None:
        from kiro_crew.acp.harness.routing_verification import load_attestations

        real = tmp_path / "elsewhere.json"
        real.write_text(json.dumps({"acme": {"fingerprint": "x"}}), encoding="utf-8")
        link = tmp_path / "backend-routing-attestations.json"
        try:
            os.symlink(real, link)
        except (OSError, NotImplementedError):
            pytest.skip("cannot create symlinks here")
        assert load_attestations(link) == {}
        link.unlink()
        link.write_text(real.read_text("utf-8"), encoding="utf-8")
        assert load_attestations(link) == {"acme": {"fingerprint": "x"}}
        os.link(link, tmp_path / "alias.json")
        assert load_attestations(link) == {}
