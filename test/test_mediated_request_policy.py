...
def _sel_root(tmp_path, monkeypatch):
    """The policy signing key is derived from the SEL trust root; install one so
    sign/verify in these tests share the same derived subkey."""
    root = tmp_path / "sel_hmac.key"
    root.write_bytes(b"D" * 32)
    monkeypatch.setattr("kiro_crew.sel.sel_hmac_key_path", lambda: root)
    monkeypatch.setattr("kiro_crew.sel._sel_hmac_key_bytes", lambda: b"D" * 32)
...
def test_normalize_origin_rebrackets_ipv6_literal_and_round_trips():
    # urlsplit.hostname strips the brackets from an IPv6 literal; without
    # re-bracketing, the canonical origin would be "https://::1:8443", which
    # urlsplit re-parses with the trailing ":8443" fused into the host and the
    # port read from the wrong group -> a per-secret PolicyError on reload.
    # Re-bracketing keeps the canonical form parseable, so normalize is
    # idempotent (round-trips) for IPv6 literals.
    assert normalize_origin("https://[::1]:8443/x") == "https://[::1]:8443"
    assert normalize_origin("https://[2001:db8::1]/y") == "https://[2001:db8::1]"
    # Idempotence: feeding the canonical form back in yields the same value.
    once = normalize_origin("https://[2001:db8::1]:8443/a")
