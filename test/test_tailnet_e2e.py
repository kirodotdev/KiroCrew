...
def env(tmp_path, monkeypatch):
    ...
    (home / "config.json").write_text(
        json.dumps({"timezone": "UTC", "dashboard": {"url": f"http://localhost:{_DASH_PORT}"}})
    )
...
class TestRealFailurePropagation:
    def test_a_refusing_daemon_stops_up_and_writes_no_config(self, env, capsys) -> None:
        ...
        # Nothing was recorded, so the host does not claim tailnet access is on.
        assert _config(env).get("dashboard", {}).get("tailscale", {}).get("enabled") in (None, False)
