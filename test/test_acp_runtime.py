...
async def test_runtime_spawn_scrubs_sensitive_env_on_default_auto(monkeypatch):
    ...
    for key in (
        ...
        "SSH_AUTH_SOCK",
        # ``_PYTHON_ENV_PREFIXES`` on this release branch is PYTHONPATH +
        # PYTHONHOME. Main's third key, PYTHONPYCACHEPREFIX, arrived with the
        # pycache-GC work that is not on this branch, so asserting it here would
        # pin a scrub the branch does not perform.
        "PYTHONPATH",
    ):
