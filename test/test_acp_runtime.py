...
@pytest.mark.asyncio
async def test_reader_loop_itself_stages_a_load_window_recap(monkeypatch):
    """The staging branch, driven through the running reader loop rather than
    by appending to the staging list by hand: the recap frame arrives ON THE
    WIRE inside the load window (inits in flight, queue not yet registered),
    and the reader loop must retain it instead of counting it as replay
    flood. Deleting the branch's ``append`` leaves the by-hand test above
    ...
    try:
                ...
                _feed(
                    reader,
                    {
                        ...
                        "params": {
                            ...
                            "update": {
                                "sessionUpdate": "session_info_update",
                                "_meta": {"kiro": {"kind": "recap", "text": "Goal: wire."}},
                            },
                        },
                    },
                )
                await asyncio.sleep(0.05)  # let the reader loop route it
                return {"modes": {"currentModeId": "kirocrew"}, "models": []}
            return {}
        ...
        assert handle._pending_recap == "Goal: wire."
        assert not rt._pending_init_notifications
    finally:
        task.cancel()
        try:
