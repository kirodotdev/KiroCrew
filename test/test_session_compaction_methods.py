...
class TestWalkTail:
    ...
    def test_a_custom_renderer_decides_the_line_and_its_cost(self):
        ...
        assert walk.chars == len("defgh") + len("abc") + 2 * scm.LINE_SEPARATOR_CHARS

    def test_an_oversized_newest_row_is_clipped_to_the_budget(self):
        # The one row the walk admits unconditionally must still fit the
        # budget a rotation is judged against: the fixed-budget projection
        # charges budget_chars for the tail, so the tail may not exceed it.
        budget = 120
        ...
        assert walk.chars <= budget + scm.LINE_SEPARATOR_CHARS
    ...
    def test_a_newest_row_that_fits_is_never_clipped(self):
        ...
        walk = scm.walk_tail(rows, budget_chars=len(line))
    ...
    def test_the_walk_and_the_digest_clip_an_overflowing_line_by_one_rule(self):
        ...
        assert scm.walk_tail(rows, budget_chars=60).lines == [clipped]
...
class TestSplitTail:
    def test_empty_rows_split_to_nothing(self):
        ...
        assert split.dropped == () and split.tail == () and split.tail_chars == 0
    ...
    def test_tail_chars_counts_lines_and_separators(self):
        rows = _rows("ab", "cd")
        split = scm.split_tail(rows, budget_chars=10_000)
        expected = sum(len(scm.replay_line(r)) + 2 for r in rows)
        assert split.tail_chars == expected
