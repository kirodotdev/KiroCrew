...
def _gate_env(monkeypatch, tmp_path, *, declined: bool, spec: dict | None):
    ...
    monkeypatch.setattr(agent, "project_agent_names", lambda *a, **k: set())
    return spec_path
...
def test_project_shadowed_name_stands_aside(monkeypatch, tmp_path):
    """kiro-cli reads the project copy when shadowed; the foreign spec is unused."""
    from kiro_crew import agent
    ...
    monkeypatch.setattr(agent, "_may_auto_approve", lambda ref: False)
    monkeypatch.setattr(agent, "project_agent_names", lambda *a, **k: {"kirocrew"})
    agent.require_foreign_spec_ceiling("kirocrew", tmp_path)  # must not raise
