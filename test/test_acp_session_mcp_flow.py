...
full closed-box gate is Phase 3):
...
from kiro_crew.dashboard.chat import api_chat_slot_delete, api_chat_slot_mcp
...
class TestSlotMcpRoute:
    ...
    @pytest.mark.asyncio
    async def test_stale_owner_cannot_clear_newer_registration(self):
        slot = _ChatSlot("s")
        state = _mock_state(slot)
        async with TestClient(TestServer(_make_app(state))) as client:
            first = await client.post(
                "/api/chat/slots/s/mcp",
                json={"servers": [], "owner": "adapter-a", "mode": "replace"},
            )
            assert first.status == 200
            second = await client.post(
                "/api/chat/slots/s/mcp",
                json={
                    "servers": [{"name": "echo", "command": "/bin/echo", "args": [], "env": []}],
                    "owner": "adapter-b",
                    "mode": "replace",
                },
            )
            ...
            stale = await client.post(
                "/api/chat/slots/s/mcp",
                json={"servers": [], "owner": "adapter-a", "mode": "clear_if_owner"},
            )
            assert stale.status == 200

        assert [server["name"] for server in slot.session_mcp_servers] == ["echo"]
        assert slot.session_mcp_owner == "adapter-b"

    @pytest.mark.asyncio
    async def test_deferred_delete_refuses_changed_project(self):
        slot = _ChatSlot("s")
        slot.project = "/repo"
        state = _mock_state(slot)
        async with TestClient(TestServer(_make_app(state))) as client:
            resp = await client.delete(
                "/api/chat/slots/s",
                headers={
                    "X-ACP-Cleanup-Project-Fingerprint": "0" * 64,
                    "X-ACP-Cleanup-Require-Empty": "1",
                },
            )
            assert resp.status == 409
            data = await resp.json()

        assert data["code"] == "cleanup_stale"
        assert state._slots["s"] is slot

    @pytest.mark.asyncio
    async def test_deferred_delete_survives_gateway_reconstruction(self, monkeypatch):
        slot = _ChatSlot("s")
        slot.project = "/repo"
        state = _mock_state(slot)
        closed: list[str] = []

        async def close(_state, _slot, name: str) -> None:
            closed.append(name)

        monkeypatch.setattr("kiro_crew.dashboard.chat_handlers.close_slot", close)
        fingerprint = hashlib.sha256(b"/repo").hexdigest()
        async with TestClient(TestServer(_make_app(state))) as client:
            resp = await client.delete(
                "/api/chat/slots/s",
                headers={
                    "X-ACP-Cleanup-Project-Fingerprint": fingerprint,
                    "X-ACP-Cleanup-Require-Empty": "1",
                },
            )
            assert resp.status == 200
    ...
    @pytest.mark.asyncio
    async def test_deferred_delete_refuses_slot_with_messages(self):
        slot = _ChatSlot("s")
        slot.messages.append({"role": "user", "content": "adopted"})
        state = _mock_state(slot)
        async with TestClient(TestServer(_make_app(state))) as client:
            resp = await client.delete(
                "/api/chat/slots/s",
                headers={"X-ACP-Cleanup-Require-Empty": "1"},
            )
            assert resp.status == 409
            data = await resp.json()
    ...
    @pytest.mark.asyncio
    async def test_cleanup_guard_cannot_probe_foreign_app_slot(self):
        slot = _ChatSlot("s")
        slot._app = "other-app"
        state = _mock_state(slot)
        async with TestClient(TestServer(_make_app(state, caller="app"))) as client:
            resp = await client.delete(
                "/api/chat/slots/s",
                headers={"X-ACP-Cleanup-Project-Fingerprint": "0" * 64},
            )
    ...
    @pytest.mark.asyncio
    async def test_app_token_cannot_probe_slot_existence(self):
        state = _mock_state()
        async with TestClient(TestServer(_make_app(state, caller="app"))) as client:
            resp = await client.post(
                "/api/chat/slots/missing/mcp",
                json={"servers": []},
            )
            assert resp.status == 403

    @pytest.mark.asyncio
    async def test_internal_replace_can_restore_prior_owner_conditionally(self):
        slot = _ChatSlot("s")
        slot.session_mcp_servers = [{"name": "old", "command": "/old", "args": [], "env": []}]
        slot.session_mcp_owner = "adapter-a"
        state = _mock_state(slot)
        async with TestClient(TestServer(_make_app(state))) as client:
            replaced = await client.post(
                "/api/chat/slots/s/mcp",
                json={
                    "servers": [{"name": "new", "command": "/new", "args": [], "env": []}],
                    "owner": "adapter-b",
                    "mode": "replace",
                    "return_previous": True,
                },
            )
            replaced_data = await replaced.json()
            snapshot = replaced_data["previous"]
            assert snapshot == {
                "servers": [{"name": "old", "command": "/old", "args": [], "env": []}],
                "owner": "adapter-a",
                "expected_generation": replaced_data["generation"],
            }
            ...
            restored = await client.post(
                ...
                json={
                    ...
                    "expected_owner": "adapter-b",
                    "expected_generation": snapshot["expected_generation"],
                },
            )
            assert restored.status == 200

        assert slot.session_mcp_servers == snapshot["servers"]
        assert slot.session_mcp_owner == "adapter-a"

    @pytest.mark.asyncio
    async def test_retried_replace_replays_snapshot_without_overwriting_newer_owner(self):
        slot = _ChatSlot("s")
        slot.session_mcp_servers = [{"name": "old", "command": "/old", "args": [], "env": []}]
        slot.session_mcp_owner = "adapter-a"
        state = _mock_state(slot)
        first_body = {
            "servers": [{"name": "first", "command": "/first", "args": [], "env": []}],
            "owner": "adapter-b",
            "mode": "replace",
            "return_previous": True,
        ...
        async with TestClient(TestServer(_make_app(state))) as client:
            initial = await client.post("/api/chat/slots/s/mcp", json=first_body)
            initial_data = await initial.json()
            await client.post(
                "/api/chat/slots/s/mcp",
                json={
                    "servers": [{"name": "newer", "command": "/newer", "args": [], "env": []}],
                    "owner": "adapter-c",
                    "mode": "replace",
                    "mutation_id": "mutation-newer",
                },
            )
            replay = await client.post("/api/chat/slots/s/mcp", json=first_body)
            assert await replay.json() == initial_data
            assert slot.session_mcp_owner == "adapter-c"
            assert [server["name"] for server in slot.session_mcp_servers] == ["newer"]

    @pytest.mark.asyncio
    async def test_internal_adapter_can_restore_unowned_restart_slot(self):
        slot = _ChatSlot("s")
        state = _mock_state(slot)
        servers = [{"name": "proxy", "command": "/proxy", "args": [], "env": []}]
        body = {
            "servers": servers,
            "owner": "adapter-a",
            "mode": "restore_if_owner",
            "expected_owner": "",
            "mutation_id": "restart-restore",
        }
        async with TestClient(TestServer(_make_app(state))) as client:
            restored = await client.post("/api/chat/slots/s/mcp", json=body)
            assert restored.status == 200
            assert (await restored.json())["applied"] is True
            retried = await client.post("/api/chat/slots/s/mcp", json=body)
            assert retried.status == 200
            assert (await retried.json())["applied"] is True

        assert slot.session_mcp_servers == servers
        assert slot.session_mcp_owner == "adapter-a"

    @pytest.mark.asyncio
    async def test_restore_does_not_overwrite_same_owner_newer_generation(self):
        slot = _ChatSlot("s")
        slot.session_mcp_servers = [{"name": "old", "command": "/old", "args": [], "env": []}]
        slot.session_mcp_owner = "adapter-a"
        state = _mock_state(slot)
        async with TestClient(TestServer(_make_app(state))) as client:
            replaced = await client.post(
                "/api/chat/slots/s/mcp",
                json={
                    "servers": [{"name": "temporary", "command": "/b"}],
                    "owner": "adapter-b",
                    "mode": "replace",
                    "return_previous": True,
                },
            )
            snapshot = (await replaced.json())["previous"]
    ...
    @pytest.mark.asyncio
    async def test_restore_does_not_overwrite_newer_owner(self):
        slot = _ChatSlot("s")
        slot.session_mcp_servers = [{"name": "newer", "command": "/newer", "args": [], "env": []}]
        slot.session_mcp_owner = "adapter-c"
        state = _mock_state(slot)
        async with TestClient(TestServer(_make_app(state))) as client:
            resp = await client.post(
                "/api/chat/slots/s/mcp",
...
class TestGetOrCreateMcpThreading:
    ...
    @pytest.mark.asyncio
    async def test_changed_set_waits_for_active_lease_before_recreating(self, cfg):
        calls: list = []
        mgr = SessionManager(cfg, provider_factory=_recording_factory(calls))
        p1, _, _ = await mgr.get_or_create("k", session_mcp_servers=[_acp_server("echo")])

        replacement = asyncio.create_task(
            mgr.get_or_create("k", session_mcp_servers=[_acp_server("echo", "import sys")])
        )
        await asyncio.sleep(0.1)

        assert not replacement.done()
        p1.shutdown.assert_not_awaited()

        mgr.release("k")
        p2, new2, _ = await asyncio.wait_for(replacement, timeout=3.0)
        mgr.release("k")
