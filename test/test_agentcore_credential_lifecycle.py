...
async def test_pool_hit_discards_when_crew_profile_permits_agentcore(
    ...
    mgr = SessionManager(KiroCrewConfig(), provider_factory=factory)
    # Member-capability preparation refuses an undeclared crew before the pool
    # decision; this test is about the pool discard, so let the crew resolve as
    # a legacy (unenrolled) member.
    from kiro_crew.session_capabilities import CapabilityPreparation

    monkeypatch.setattr(
        "kiro_crew.session_capabilities.prepare_runtime",
        lambda agent, crew_agent, cwd: CapabilityPreparation(crew_agent or ""),
    )
    mgr._pool_size = 1
    mgr._pool_agent = ""
