...
class TestUrlValidation:
    ...
    def test_port_80_is_refused_even_though_the_shared_vet_allows_it(
        ...
        """The shared vet's allow-list is {80, 443} because it also serves plain-http
        link unfurling; THIS caller is https-only, so the stated scope is 443 alone.
        ``https://host:80`` would pass the vet and then fail the TLS handshake with
        a message that does not name the cause — refuse it up front instead."""
        with pytest.raises(cal.CalendarError):
