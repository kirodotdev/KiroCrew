...
The P0 review found one defect class repeatedly: a file or Git coordinate the
agent can write was trusted by a privileged reader -- an unbounded,
link-following read of an install-level JSON file, or a fetch whose URL/branch
came from inside an agent-writable checkout. Instance fixes do not close a
class; these guards do. They fail the build when a new raw read or a
...
_READER_ALLOWLIST = {
    ...
    # The reviewed-surface digest reads only through
    # ``safe_read_file_bytes_nolink`` (link-refusing, byte-bounded), so it needs
    # no exemption at all. Listed so a raw read added here later fails the build.
    "project_review.py": set(),
...
def test_every_install_or_derived_read_goes_through_a_hardened_reader(module: str) -> None:
    ...
    assert not offenders, (
        ...
        "the registry reader / the provenance-record reader:\n  " + "\n  ".join(offenders)
    )
