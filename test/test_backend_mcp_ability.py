...
def test_the_payload_names_the_spec_key_for_every_listed_concern() -> None:
    """The render fallback, so a new concern degrades to a legible key on the panel.

    The kind and the reach already reach a renderer as their declared value, so a
    frontend with no phrase for them prints something rather than nothing. A concern id
    had no such escape: a member added to ``Concern`` rendered blank on the panel while
    ``kirocrew doctor`` named it, and a reader cannot act on a line they never see.

    Only the ids on THIS card, so the map is the row's own rather than a dictionary of
    the whole vocabulary, and every listed id is covered -- a listed id with no key
    would be the same silent drop one layer down.
    """
    for backend in sorted(PROJECTIONS):
        payload = mcp_mod.ability_payload(backend)
        listed = [*payload["withheld"], *payload["no_channel"]]  # type: ignore[misc]
        keys = payload["spec_keys"]
        assert isinstance(keys, dict)
        assert set(keys) == set(listed), backend
        for cid in listed:
            assert keys[cid] == mcp_mod.spec_keys()[cid], (backend, cid)
    assert mcp_mod.ability_payload(STRANGER)["spec_keys"] == {}
