...
class TestArtifactsAndCancelSpelling:
    ...
    async def test_replacement_artifact_supersedes_the_appended_draft(self):
        """``append: false`` (or absent) REPLACES the artifact's prior content.
        The draft streamed live as appends is not the result; only the final
        replacement is delivered for the record, once, and never concatenated
        onto the stale text."""
        done = _sse(
            '{"result": {"statusUpdate": {"taskId": "t-1", "contextId": "ctx-1",'
            ' "status": {"state": "TASK_STATE_COMPLETED"}}}, "id": "1", "jsonrpc": "2.0"}'
        ...
        lines = [
            _STREAM_LINES[0],
            b"",
            self._art("out", "draft one ", append=True),
            b"",
            self._art("out", "draft two", append=True),
            b"",
            self._art("out", "FINAL", append=False),
            b"",
            done,
            b"",
        ]
        ...
        events = await _drain(p.stream("go"))
        chunks = [e.text for e in events if e.kind == EVENT_TEXT_CHUNK]
        # Appends streamed live as they arrived; the replacement came once, last.
        assert chunks == ["draft one ", "draft two", "FINAL"]
        # The record carries the artifact's FINAL content, not draft + final.
        assert events[-1].kind == EVENT_COMPLETE and events[-1].text == "FINAL"

    @pytest.mark.asyncio
    async def test_artifacts_are_tracked_per_id(self):
        # A replacement of artifact "b" must not disturb artifact "a".
        done = _sse(
            '{"result": {"statusUpdate": {"taskId": "t-1", "contextId": "ctx-1",'
            ' "status": {"state": "TASK_STATE_COMPLETED"}}}, "id": "1", "jsonrpc": "2.0"}'
        )
        ...
        p._session = _FakePostSession(lines)
        events = await _drain(p.stream("go"))
        assert [e.text for e in events if e.kind == EVENT_TEXT_CHUNK] == ["alpha", "beta"]
    ...
    async def test_replacement_is_flushed_before_a_failed_turn_is_reported(self):
        ...
        lines = [_STREAM_LINES[0], b"", self._art("out", "partial", append=False), b"", failed, b""]
        p = A2AProvider(name="r", agent_card_url="http://h/.well-known/agent-card.json")
        p._started = True
        p._message_endpoint = "http://h/"
        p._session = _FakePostSession(lines)
...
class TestContinuationStaysWithTheRecordedAgent:
    ...
    @pytest.mark.asyncio
    async def test_the_run_rebuilds_the_provider_from_the_same_record(self, monkeypatch):
        """``run.py`` takes the contextId from ``recorded_a2a`` too -- exercised on
        the REAL rebound manager, where a name run.py imports at module level is
        inert and only ``subagent.py``'s namespace counts."""
        from kiro_crew.subagent import SubagentInfo, SubagentManager

        mgr = SubagentManager(sessions=MagicMock(), ctx_builder=None)  # type: ignore[arg-type]
        monkeypatch.setattr(
            "kiro_crew.subagent.read_state",
            lambda _id: {"provider": "a2a", "agent": "remote-demo", "session_id": "ctx-7"},
        )
        ...
        # A fresh spawn (no conversation) starts with no contextId.
        await mgr._build_a2a_provider(
            SubagentInfo(id="run3", task="t"),
            A2aAgentConfig(name="remote-demo", agent_card_url="https://h/c"),
        )
