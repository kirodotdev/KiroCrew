...
class TestEditionBundleRootsAreOneAuthority:
    ...
    def test_this_package_is_not_trusted_when_the_seam_works(self, home, tmp_path, monkeypatch):
        """The narrowing: a working seam must NOT admit the package directory.

        The walk's own first entry is this package's ``apps/builtins``, so adding
        the package directory on the success leg would only ever admit paths
        OUTSIDE the builtins tree -- every ``.py`` under the installed package
        would resolve as a bundle script, for no builtin bundle that needs it.
        """
        import kiro_crew.apps.execution as execution_mod
        ...
        package_dir = Path(cron_script_mod.__file__).parent.resolve()
        assert (
            package_dir not in roots
        ), f"a working seam must not admit the package directory {package_dir}"
        # The seam's own roots are still there, so this narrows rather than breaks.
        assert edition.resolve() in roots
