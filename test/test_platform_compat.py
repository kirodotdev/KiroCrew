...
class TestProcessDescendants:
    ...
    def test_posix_fallback_excludes_orphan_older_than_recycled_parent(self, monkeypatch):
        ...
        assert pc.process_descendant_identities(10) == [
            pc.ProcessDescendantIdentity(11, 10, "Mon Jan  1 00:00:02 2024")
        ]

    def test_browser_process_start_order_is_three_way(self):
        order = pc._BrowserProcessStartOrder

        assert pc._browser_process_start_order("200", "100") is order.LATER
        assert pc._browser_process_start_order("100", "200") is order.EARLIER
        assert pc._browser_process_start_order("100", "100") is order.INCONCLUSIVE
        assert (
            pc._browser_process_start_order(
        ...
        assert pc._browser_process_start_order("unknown", "tokens") is order.INCONCLUSIVE
    ...
    def test_posix_fallback_requires_identity_stable_across_snapshots(self, monkeypatch):
        ...
        assert pc.process_descendant_identities(10) == [
            pc.ProcessDescendantIdentity(11, 10, "Mon Jan  1 00:00:01 2024"),
            pc.ProcessDescendantIdentity(12, 11, "Mon Jan  1 00:00:02 2024"),
        ]
        assert runs == [
    ...
    @pytest.mark.parametrize("changed_pid", [10, 11], ids=["root", "descendant"])
    def test_posix_fallback_rejects_pid_recycled_after_parentage_snapshot(
        self, monkeypatch, changed_pid
    ):
        before = {
        ...
        after = {
            10: pc._PosixProcessSnapshotRow(1, "root-new" if changed_pid == 10 else "root-old"),
            11: pc._PosixProcessSnapshotRow(10, "child-new" if changed_pid == 11 else "child-old"),
        }
