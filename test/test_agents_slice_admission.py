...
class TestAgentsSliceHeadroom:
    def test_one_cgroup_reader_serves_both_probes(self, tmp_path):
        """``sandbox`` owns the single-value cgroup reader; ``subagent`` binds the
        same function rather than carrying a second spelling of it."""
        assert sa._read_int_file is sb.read_cgroup_int
        (tmp_path / "memory.high").write_text("4096\n", encoding="utf-8")
        (tmp_path / "memory.max").write_text("max\n", encoding="utf-8")
        assert sb.read_cgroup_int(tmp_path / "memory.high") == 4096
        assert sb.read_cgroup_int(str(tmp_path / "memory.high")) == 4096
        assert sb.read_cgroup_int(tmp_path / "memory.max") is None
        assert sb.read_cgroup_int(tmp_path / "absent") is None
...
class TestAgentsSliceThrottling:
    ...
    def test_counter_climb_between_probes_is_throttling(self, tmp_path, monkeypatch):
        ...
        assert sb.agents_slice_throttling() is True
        # A stable counter with usage under the line closes the reading.
        assert sb.agents_slice_throttling() is False
