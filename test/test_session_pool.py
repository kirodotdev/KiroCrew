...
class TestGetOrCreatePoolIntegration:
    ...
    async def test_a_discarded_conversation_stays_discarded_through_an_armed_retry(self):
        ...
        arms.mark_retire_on_next_claim("resurrect-key", "/Users/alice/proj")
        ...
        arm.spend()
        assert (
            not arm.requires_sid_clear
        ), "provenance outliving its episode makes the NEXT conversation's SID the casualty"

    @pytest.mark.asyncio
    async def test_a_spent_arm_is_reclaimed_only_once_the_last_start_settles(self):
        """A spent record is one counter nobody can still compare against -- so it must go.

        ``spend`` deliberately keeps the generation, because a start that snapshotted it
        compares against it later, and that window sits wholly inside the start's allocation
        reservation. Reclaiming EARLIER is the unsafe direction: an absent record reads zero,
        and the comparison is ``started < current``, so a stale start would read as current
        and keep a session bound to the project it was armed away from. Never reclaiming is
        the leak: one entry per key ever armed, until a shutdown nobody reaches.
        """
        mgr, _factory = _make_manager(pool_agent="kirocrew")
        arms = mgr._allocation_boundary()
        folded = mgr._fold_key("reclaim-key")

        arms.mark_retire_on_next_claim("reclaim-key", "/Users/alice/proj")
        spent = arms._arm_if_any(folded)
        assert spent is not None, "nothing was armed, so the reclamation below asserts nothing"
        spent.spend()
        ...
        async def _impl(*_args, **_kwargs):
            in_flight.set()
            await release.wait()
            return _make_provider(), False, False

        with patch.object(arms, "_get_or_create_impl", _impl):
            claim = asyncio.create_task(mgr.get_or_create("reclaim-key"))
            await asyncio.wait_for(in_flight.wait(), timeout=1.0)
            assert folded in arms.state.retire_arms, (
                "reclaimed while a start holds a snapshot of this counter -- the comparison "
                "would read zero and serve the stale start as current"
            )
            release.set()
            await claim

        assert folded not in arms.state.retire_arms

    @pytest.mark.asyncio
    async def test_an_arm_still_naming_a_directory_survives_a_settled_start(self):
        """Only a record naming NOTHING is reclaimable: an unspent arm is still owed a claim.

        The reclamation is gated on the arm being spent rather than on the start settling
        alone, because a start that never reached the armed key leaves the arm owed to the
        real claim -- dropping it there would bind the successor to the old project.
        """
        mgr, _factory = _make_manager(pool_agent="kirocrew")
        arms = mgr._allocation_boundary()
        folded = mgr._fold_key("owed-key")
        arms.mark_retire_on_next_claim("owed-key", "/Users/alice/proj")

        async def _impl(*_args, **_kwargs):
            return _make_provider(), False, False

        with patch.object(arms, "_get_or_create_impl", _impl):
            await mgr.get_or_create("owed-key")

        arm = arms._arm_if_any(folded)
        assert arm is not None and arm.cwd == str(Path("/Users/alice/proj"))
