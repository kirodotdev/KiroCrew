...
def capture_spawn(monkeypatch):
    ...
    async def resolve_installed(*args, **kwargs):
        # main gave _resolve_kiro_bin_for_spawn `environ=`/`home=` kwargs; accept
        # anything so the stub tracks the real signature without pinning it.
    ...
    async def capture_wrap_async(argv, mode, **kwargs):
        # main moved the host path to wrap_argv_async (pod-bundle aware); the
        # container path skips it, so the no-container test still needs this
        # stubbed to observe the host wrappers ran.
        seen["wrap_called"] = True
        return ["/usr/bin/sandbox-wrapper", *argv], None

    def capture_pod_bundle(argv, backend=None):
        # apply_pod_bundle_spawn is SYNC (the host path calls it via
        # asyncio.to_thread); return argv unchanged and is_kiro_cli=None (no pod).
        return argv, None
    ...
    monkeypatch.setattr(runtime_mod, "wrap_argv_async", capture_wrap_async, raising=False)
    monkeypatch.setattr(runtime_mod, "apply_pod_bundle_spawn", capture_pod_bundle, raising=False)
...
class TestSessionCwdOwnership:
    ...
    ``_session_cwd`` is async because it realpaths, and it is reached only from
    ``create_session`` / ``load_session`` -- so a sync version ran two path walks
    on the event loop, which stalls the gateway on a network-backed mount.
    """

    @pytest.mark.asyncio
    async def test_host_runtime_passes_cwd_through(self, tmp_path):
        rt = AcpRuntime(work_dir=tmp_path / "wd")
        assert await rt._session_cwd(None) == str(tmp_path / "wd")
        assert await rt._session_cwd("/some/other/dir") == "/some/other/dir"

    @pytest.mark.asyncio
    async def test_containerized_runtime_maps_to_the_container_workspace(self, tmp_path):
        project = tmp_path / "proj"
        project.mkdir()
        rt = AcpRuntime(work_dir=project)
        ...
        assert await rt._session_cwd(None) == "/workspaces/proj"
        assert await rt._session_cwd(project) == "/workspaces/proj"
        # realpath-equal spellings map too (symlinked/relative forms).
        assert await rt._session_cwd(str(project) + "/.") == "/workspaces/proj"

    @pytest.mark.asyncio
    async def test_foreign_cwd_is_refused_not_silently_mapped(self, tmp_path):
        ...
        with pytest.raises(AcpRuntimeError, match="one runtime has one Dev Container"):
            await rt._session_cwd(other)

    @pytest.mark.asyncio
    async def test_the_path_comparison_is_offloaded(self, tmp_path):
        """The reason it is async at all: both realpath walks must happen in a
        worker, not on the loop. Fails if either call moves back inline.
        """
        project = tmp_path / "proj"
        project.mkdir()
        rt = AcpRuntime(work_dir=project)
        ...
        with patch.object(os.path, "realpath", tracking):
            assert await rt._session_cwd(project) == "/workspaces/proj"

        assert seen, "realpath was never called; the test proves nothing"
        main = threading.main_thread().name
        assert all(t != main for t in seen), f"realpath ran on the loop thread: {seen}"
