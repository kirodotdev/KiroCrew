...
``test_ledger_core.py`` covers the format. This file covers the SPLIT -- the
...
spec text (``docs/system-specs/modules/ledger-core.md`` section 4b) and this file
...
from kiro_crew.ledger.schema import check_ownership, require_src
...
def test_a_patrol_cannot_write_into_a_session_ledger():
...
def test_a_crew_guest_is_refused_by_a_session_ledger():
    session = _session()
    with pytest.raises(LedgerError) as exc:
...
def test_an_app_guest_is_refused_by_a_session_ledger():
    session = _session()
    with pytest.raises(LedgerError) as exc:
...
def test_one_report_type_serves_every_child():
    ...
    # into one parent ledger, and the entries are told apart by src.
    crew = _crew()
...
def test_a_session_ledger_may_cite_a_crew_segment_too():
