...
class TestRosterFreeTextMasking:
    ...
    @pytest.mark.asyncio
    async def test_projected_roster_copy_ships_as_the_sentinel(self, tmp_path):
        """The member/config snapshot in ``projections`` is a second copy of the
        same free text, and the frontend prefers it over the row field-by-field,
        so it carries the same wholesale sentinel -- not an in-place scrub."""
        from kiro_crew.dashboard.handlers.core import _SENSITIVE_MASK
        ...
        with patch(
            ...
            async with TestClient(TestServer(_make_members_app(state))) as client:
                ...
                data = await resp.json()
        rows = {r["name"]: r for r in data["members"]}
        shady_roster = rows["shady"]["projections"]["values"]["roster"]
        benign_roster = rows[CREW]["projections"]["values"]["roster"]
        for field_name in ("kiro_agent", "workspace", "memory_store", "model"):
            assert shady_roster[field_name] == _SENSITIVE_MASK, field_name
            assert benign_roster[field_name] == rows[CREW][field_name], field_name
        assert shady_roster["source"] == "kirocrew"
    ...
    async def test_every_untrusted_text_field_in_the_row_is_guarded(self, tmp_path):
        ...
        state = _make_state(tmp_path)
        # ``member_id`` is not shipped; its only derivative is ``slug``, which
        # ``member_slug`` gates through ``validate_slug`` (lowercase slug
        # grammar) and DROPS the row for anything else -- so a credential-shaped
        # value there tests the slug gate, not this mask, and would empty the
        # roster. Leave it at its default and exempt it below.
        field_kwargs = {
            field_name: _CRED_VALUE
        ...
        # ``telegram_account`` is not in the row at all; ``member_id`` is
        # ``validate_slug``-gated above. Every other class member that the row
        # ships must come back as the sentinel.
        exempt = {"source", "telegram_account", "member_id"}
        for field_name in _AGENT_UNTRUSTED_TEXT_FIELDS:
