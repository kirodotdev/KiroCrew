...
# Every resolver test fakes `runtime._run_cmd`, so no argv reaches git and the
# checkout path is inert — any string does. It is passed explicitly rather than
# left to a default because `resolve(repo=...)` is required: a test that omitted
# it would be resolving against whatever checkout the ambient Dev Fleet
# configuration happens to name, which is a dependency these tests do not want.
_FAKE_REPO = "/fake/checkout"
...
async def test_resolve_stable_skips_newer_prerelease_tags(monkeypatch):
    ...
    got = await rcp.resolve(repo=_FAKE_REPO)
...
async def test_resolve_reports_oid_of_the_tagged_commit(monkeypatch):
    ...
    ``git worktree add`` / ``checkout --detach`` does not identify the commit
    Create must materialize.
...
async def test_the_version_probe_ignores_a_local_tag_on_the_checked_out_commit(monkeypatch):
    ...
    The row is deliberately not at the tip: there the release is known from the
...
# --------------------------------------------------------------------------
@pytest.mark.asyncio
async def test_worktree_state_reports_badge_currency_without_a_commit_count(monkeypatch):
    """``version`` plus ``at_tip`` is the complete channel currency signal.
...
@pytest.mark.asyncio
async def test_worktree_on_a_foreign_tag_reports_no_release(monkeypatch):
...
# --------------------------------------------------------------------------
@pytest.mark.asyncio
async def test_resolve_answers_from_the_mirror_with_the_remote_unreachable(monkeypatch):
    """The mirror's unique job: a published version on the row with no network.

    This is the property that decides whether the mirror may be replaced by the
    ``ls-remote`` answer Create verifies against. It cannot: the fleet is
    deliberately offline-tolerant -- PR lookups degrade to ``None`` on any git or
    ``gh`` failure so the rows still render from local state -- and the release row
    participates in that render only because resolution reads ``.git``. Sourcing it
    from an advertisement would make this the one row that blanks offline, and the
    in-memory fleet cache cannot cover the gap because a cold process starts empty.

    The fake fails every ``ls-remote`` and every ``fetch``, so a resolver that
    reached the network would either be caught by the argv assertion or fail
    outright -- which is what makes this test fail on the substitution rather than
    merely pass alongside it.
    """
    recorded: list[list[str]] = []
    mirror = _fake_git()

    async def offline(cmd, **kw):
        recorded.append(list(cmd))
        if "ls-remote" in cmd or "fetch" in cmd:
            return 128, "", "fatal: unable to access remote: Could not resolve host"
        return await mirror(cmd, **kw)

    monkeypatch.setattr(runtime, "_run_cmd", offline)

    got = await rcp.resolve(repo=_FAKE_REPO)

    assert got["ok"] is True, "the mirror must resolve without reaching the remote"
    assert got["ref"] == "refs/tags/v0.5.0"
    assert got["version"] == "0.5.0"
    assert got["oid"]
    assert not any(
        "ls-remote" in argv or "fetch" in argv for argv in recorded
    ), f"resolve() must read .git only; it issued {recorded}"


# --------------------------------------------------------------------------
# bounds on what a resolve retains
# --------------------------------------------------------------------------
@pytest.mark.asyncio
async def test_the_mirror_listing_is_bounded_by_git_before_it_reaches_this_process(monkeypatch):
    """The listing asks git for ``--count`` of the cap plus one, so the population that
    reaches Python is bounded by git, not by hoping the remote is small."""
    seen: list[list[str]] = []
    ...
    monkeypatch.setattr(runtime, "_run_cmd", fake_run)
    listed = await rcp.list_release_tags(_FAKE_REPO)

    assert listed == ["v0.5.0"]
    listing = next(c for c in seen if "for-each-ref" in c)
    assert f"--count={rcp._MAX_RELEASE_REFS + 1}" in listing


@pytest.mark.asyncio
async def test_an_overflowing_mirror_refuses_to_name_a_tip_instead_of_sorting_a_truncated_set(
    monkeypatch,
):
    """Git returning the extra row means the set is truncated. A truncated set can omit
    the true tip, so the channel reports the overflow rather than picking from it."""
    overflow = [f"v9.{i}.0" for i in range(rcp._MAX_RELEASE_REFS + 1)]
    monkeypatch.setattr(runtime, "_run_cmd", _fake_git(overflow))

    got = await rcp.resolve(repo=_FAKE_REPO)

    assert got["ok"] is False
    assert "unpublished" not in got
    assert str(rcp._MAX_RELEASE_REFS) in got["error"]


@pytest.mark.asyncio
async def test_an_overlong_ref_name_is_overflow_not_a_silent_drop(monkeypatch):
    """No release tag this channel admits is longer than the cap; a longer name is
    something else wearing the mirror namespace. It is REPORTED, not skipped: a
    skipped row would read exactly like a mirror that never held it."""
    long_name = "v" + "9" * rcp._MAX_REF_NAME_LEN + ".0.0"
    monkeypatch.setattr(runtime, "_run_cmd", _fake_git([long_name, "v0.5.0"]))

    got = await rcp.resolve(repo=_FAKE_REPO)

    assert got["ok"] is False
    assert "unpublished" not in got
    assert str(rcp._MAX_REF_NAME_LEN) in got["error"]


@pytest.mark.asyncio
async def test_overflow_is_decided_on_the_raw_rows_before_any_name_is_filtered(monkeypatch):
    """The count bound and the length bound are checked on what git returned, in that
    order, before a name is dropped for any reason -- so an overlength name among the
    first rows cannot pull the visible count back under the cap and let a truncated
    prefix be sorted as if it were the whole population."""
    rows = ["v" + "9" * rcp._MAX_REF_NAME_LEN + ".0.0"] + [
        f"v9.{i}.0" for i in range(rcp._MAX_RELEASE_REFS)
    ]
    assert len(rows) == rcp._MAX_RELEASE_REFS + 1
    monkeypatch.setattr(runtime, "_run_cmd", _fake_git(rows))

    with pytest.raises(rcp.MirrorOverflow) as caught:
        await rcp.list_release_tags(_FAKE_REPO)

    assert str(rcp._MAX_RELEASE_REFS) in str(caught.value)


@pytest.mark.asyncio
async def test_worktree_state_refuses_an_overflowing_points_at_listing(monkeypatch):
    """A truncated points-at prefix cannot be reported as no release at HEAD."""
    rows = [f"v9.{i}.0" for i in range(rcp._MAX_RELEASE_REFS + 1)]
    ...
    with pytest.raises(rcp.MirrorOverflow) as caught:
        await rcp.worktree_state("/wt", {"oid": "tip-oid", "lane": "stable", "version": "9.9999.0"})

    assert str(rcp._MAX_RELEASE_REFS) in str(caught.value)


@pytest.mark.asyncio
async def test_worktree_state_refuses_an_overlength_points_at_row(monkeypatch):
    """An overlength early row is rejected rather than skipped for a later tag."""
    long_name = "v" + "9" * rcp._MAX_REF_NAME_LEN + ".0.0"
    ...
    async def fake_run(cmd, **kw):
        ...
        if "rev-parse" in cmd:
            return 0, "head-oid\n", ""
        if "for-each-ref" in cmd and "--points-at" in cmd:
            return 0, f"{long_name}\nv0.5.0\n", ""
        return 1, "", "unexpected"

    monkeypatch.setattr(runtime, "_run_cmd", fake_run)

    with pytest.raises(rcp.MirrorOverflow) as caught:
        await rcp.worktree_state("/wt", {"oid": "tip-oid", "lane": "stable", "version": "0.6.0"})

    assert str(rcp._MAX_REF_NAME_LEN) in str(caught.value)
