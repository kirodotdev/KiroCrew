...
#: it from each tool's own ``require_strict_session_key`` at entry; a test supplies
#: it directly so these cases exercise rendering rather than identity.
KEY = "dashboard:ui"
...
def test_an_ordinary_session_is_not_refused():
    """``private_memory_store_for_session`` answers ``""`` for an ordinary session."""
    assert s._crew_scope_refusal("") == ""


@pytest.mark.parametrize("boom", [OSError("binding unreadable"), ValueError("bad binding")])
def test_an_unreadable_private_binding_refuses_the_crew_read(monkeypatch: pytest.MonkeyPatch, boom):
    """A binding that cannot be read must FAIL CLOSED, not fall through as unfenced.

    ``_caller_private_store`` returns ``("", refusal)`` on a read failure, and the
    empty store is the same value an ordinary session yields — so a caller that
    ignored the refusal and passed the store straight to the fence would admit the
    read. The refusal is checked FIRST at every call site, and this drives the real
    tool to prove it: an unreadable binding is indistinguishable from a fenced member
    whose store could not be confirmed, and "unknown" on a cross-crew read is refuse.
    """
    ...
    monkeypatch.setattr(mma, "private_memory_store_for_session", _raise)
    monkeypatch.setattr(mcp_core, "_resolve_session_key_strict", lambda: "dashboard:ui")
    reached: list[Any] = []
    monkeypatch.setattr(s, "_crew_search_history", lambda *a, **k: reached.append(a) or "TUNNEL")
    out = s.search_chat_history("search_chat_history", {"query": "q", "crew": "chick"})
    assert out == "Error: this session's private memory is unavailable."
    assert reached == [], "an unreadable binding reached the peer tunnel"
...
def test_a_crew_read_is_refused_when_the_caller_cannot_be_strictly_identified(
    ...
    The resolution now happens once at each tool's ENTRY rather than in a
    crew-specific wrapper: the member-memory simplification landed
    ``require_strict_session_key`` at the top of all three tools, so a separate
    crew-only gate would be a second resolution of the same identity -- the very
    check-then-use split this refusal exists to prevent. This drives the tool.
    """
...
def test_a_strictly_resolved_caller_is_not_refused(monkeypatch: pytest.MonkeyPatch):
    """The control: a properly identified caller is not refused at the gate."""
    monkeypatch.setattr(mcp_core, "_resolve_session_key_strict", lambda: "dashboard:ui")
    sent: list[Any] = []
    monkeypatch.setattr(
        s, "_crew_search_history", lambda *a, **k: sent.append(a) or "REACHED TUNNEL"
    )
    out = s.search_chat_history("search_chat_history", {"query": "q", "crew": "chick"})
    assert out == "REACHED TUNNEL"
    # The key that was CHECKED is the key that goes on the wire.
    assert sent == [("chick", "q", 10, "dashboard:ui")]
