...
class TestSerializeSlotsSubagentsRunning:
    ...
    async def test_flag_true_when_agents_running(self, tmp_path, monkeypatch):
        monkeypatch.setattr("kiro_crew.dashboard.state.config_dir", lambda: tmp_path)
        subs = MagicMock()
        subs.running_agents_rooted_at = MagicMock(return_value=[{"id": "a1"}])
        state = _make_state(tmp_path, subagents=subs)
        ...
        subs.running_agents_rooted_at.assert_any_call("dashboard:s1")

    async def test_flag_counts_the_tabs_whole_tree_not_only_its_own_wave(
        self, tmp_path, monkeypatch
    ):
        """A coordinator that finished before its nested run: the tab's own wave
        is empty, but a grandchild it spawned is still live and its card sits in
        the tab, so the badge stays lit -- the rooted set is what the tab shows."""
        monkeypatch.setattr("kiro_crew.dashboard.state.config_dir", lambda: tmp_path)
        subs = MagicMock()
        subs.running_agents_for = MagicMock(return_value=[])  # own wave done
        subs.running_agents_rooted_at = MagicMock(return_value=[{"id": "grandchild"}])
        state = _make_state(tmp_path, subagents=subs)
        state.get_or_create_slot("s1")
    ...
    async def test_flag_false_when_no_agents_running(self, tmp_path, monkeypatch):
        monkeypatch.setattr("kiro_crew.dashboard.state.config_dir", lambda: tmp_path)
        subs = MagicMock()
        subs.running_agents_rooted_at = MagicMock(return_value=[])
        state = _make_state(tmp_path, subagents=subs)
        state.get_or_create_slot("s1")
