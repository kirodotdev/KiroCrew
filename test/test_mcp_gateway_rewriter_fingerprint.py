...
@pytest.mark.parametrize("old_schema", [4, 5])
def test_pre_casing_schema_forces_regeneration(
    ...
    """Pre-casing overlays can embed the PATHEXT spelling and must be rebuilt."""
    _mk_tree(tmp_path)
    with monkeypatch.context() as patch:
        patch.setattr(rewriter, "_FINGERPRINT_SCHEMA", old_schema)
        _rewrite(tmp_path)
    before = rewrite_counter["n"]
    _rewrite(tmp_path)
    assert rewrite_counter["n"] == before + 2
...
def test_windows_casing_is_revalidated_on_cached_and_kept_overlays(
    ...
    flags = written["mcpServers"]["srv"]["args"]
