...
def test_binding_resolver_none_when_unwired():
    # When nothing is installed on the app, the resolver is constructed lazily on
    # first use ONLY if the control-plane package is importable in this build;
    # otherwise it stays None (fail-closed, managed items denied). The seam
    # contract: no pre-installed resolver + no control-plane package => None.
    import importlib.util

    have_cp = (
        importlib.util.find_spec("kiro_crew.connections.control_plane.acl_binding_resolver")
        is not None
    )
    result = _knowledge_binding_resolver(_request({}))
    if have_cp:
        # Package present: first-use construction yields a (bridged) resolver.
        assert result is not None
    else:
        # Package absent (standalone): stays None.
