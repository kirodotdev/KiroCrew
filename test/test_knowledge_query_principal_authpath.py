...
* an app token -> QueryPrincipal("app:<name>", local_library=False, verified=True);
...
async def test_dashboard_user_token_yields_verified_owner_principal():
    async with TestClient(TestServer(_app())) as client:
        ...
        assert body["local_library"] is False
...
async def test_route_to_resolver_gate_seam_wiring():
    ...
    store.set_item_acl(
        ...
        managed=True,
        fresh_as_of=0.0,
        resource_ref=ProviderResourceRef(provider="sharepoint", account="tenA", resource_id="Doc"),
    )
