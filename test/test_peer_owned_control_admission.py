...
class TestTheChokepointOwnsEveryPeerOwnedControl:
    ...
    def test_the_table_dispatches_to_the_shared_admissions(self):
        ...
        assert "agent" not in _PEER_OWNED_RESTORE  # owned by _restored_agent_name
...
class TestTheAgentRestoreIsOwnedUpstream:
    """The fourth forwardable control, and the one this change does NOT admit.

    ``_restored_agent_name`` owns the agent restore: it prefers a locally
    committed protected selection and otherwise returns the persisted value. That
    is why ``agent`` is named in the guard's ``ADMITTED_ELSEWHERE`` rather than
    carried in ``_PEER_OWNED_RESTORE`` — a table entry no caller reaches would
    claim a decision that never happens.

    These pin the behaviour the exemption ASSERTS. Without them the exemption is
    prose: a later change that made that function canonicalize or blank the value
    would silently reintroduce the class this file exists to close, and no test
    would notice."""

    def test_a_peer_value_comes_back_unrewritten_with_no_protected_record(self, monkeypatch):
        """The property the exemption rests on. A peer-bound slot never acquires a
        protected record — ``api_chat_slot_agent`` early-returns into
        ``_apply_remote_pick``, which persists the accepted pick to transcript
        metadata and never calls ``record_agent_selection`` — so this is the
        branch a restored peer session actually takes."""
        from kiro_crew.dashboard import chat_persistence

        monkeypatch.setattr(chat_persistence, "session_agent_selection_name", lambda _key: None)
        restored = chat_persistence._restored_agent_name(
            "dashboard:1", {"agent": "peer-only-agent"}
        )
        assert restored == "peer-only-agent"

    def test_a_committed_local_selection_still_outranks_the_transcript(self, monkeypatch):
        """Upstream's reason for the function, kept intact: history can retain a
        provisional agent after an interrupted switch, and the committed record is
        the choice. This change must not weaken that precedence."""
        from kiro_crew.dashboard import chat_persistence

        monkeypatch.setattr(chat_persistence, "session_agent_selection_name", lambda _key: "writer")
        restored = chat_persistence._restored_agent_name("dashboard:1", {"agent": "provisional"})
        assert restored == "writer"

    @pytest.mark.parametrize("bad", [5, None, ["writer"]])
    def test_a_non_string_agent_reads_as_absent(self, monkeypatch, bad: object):
        """The one guard this change would otherwise have added itself: a
        hand-edited JSONL cannot put a non-string onto ``slot.agent``, which every
        ``str`` consumer would then trip on. Upstream already does it."""
        from kiro_crew.dashboard import chat_persistence

        monkeypatch.setattr(chat_persistence, "session_agent_selection_name", lambda _key: None)
        assert chat_persistence._restored_agent_name("dashboard:1", {"agent": bad}) == ""
...
class TestAdoptAndRestoreAdmitTheSameValues:
    ...
    @pytest.mark.parametrize("raw", CANDIDATES)
    def test_the_same_holds_for_the_agent(self, raw: str, monkeypatch):
        """Same property, different owner. The agent's restore is
        ``_restored_agent_name`` rather than the table (see
        ``TestTheAgentRestoreIsOwnedUpstream``), so the parity is checked against
        the path a restored peer session actually takes — pinning the table here
        would assert a chokepoint the agent does not flow through and prove
        nothing about the value that reaches the slot."""
        from kiro_crew.dashboard import chat_persistence
        from kiro_crew.dashboard.remote_adopt import peer_row_metadata

        monkeypatch.setattr(chat_persistence, "session_agent_selection_name", lambda _key: None)
        stored = peer_row_metadata({"agent": raw}).get("agent", "")
        assert chat_persistence._restored_agent_name("dashboard:1", {"agent": stored}) == stored
...
#: Every route that mutates a forwardable control: path, request body, the slot
#: field its LOCAL arm writes, and a value standing in for one inherited from the
#: peer. Driving all four is the point — GPT's finding named only ``model`` and
#: ``reasoning_effort``, and fixing the named pair is how this family has drawn a
#: fresh block per round.
_CONTROL_ROUTES = [
    ("model", {"model": "gpt-5.6-sol"}, "model", "claude-opus-4.8"),
    ("reasoning-effort", {"reasoning_effort": "low"}, "reasoning_effort", "turbo"),
    ("agent", {"agent": "writer"}, "agent", "peer-writer"),
    ("workspace", {"workspace": "local-ws"}, "workspace", "peer-ws"),
]


def _half_bound(slot: _ChatSlot, *, missing: str) -> _ChatSlot:
    """A slot the user bound to a crew whose binding never completed.

    Both spellings are real: ``executor`` is persisted and read back before the
    instance and peer-slot fields land, so either can be the absent one.
    """
    slot.executor = "remote"
    slot.instance_id = "" if missing == "instance_id" else "nobita"
    slot.remote_slot = "" if missing == "remote_slot" else "peer-chat-9"
    assert not slot.is_remote, "fixture must model the half-bound state, not a bound one"
    return slot


def _control_app(state: DashboardState) -> web.Application:
    app = web.Application()
    app["state"] = state
    app.router.add_post("/api/chat/slots/{slot}/model", api_chat_slot_model)
...
@pytest.mark.asyncio
class TestAHalfBoundSlotKeepsItsInheritedControls:
    """``is_remote`` requires the WHOLE binding, and its own docstring promises
    callers "False here and a refusal at the dispatch site". The send path kept
    that promise. The four control endpoints did not: each branched on
    ``is_remote`` alone, so a slot carrying ``executor == "remote"`` with an
    incomplete binding skipped the remote arm, committed the control LOCALLY, and
    answered 200.

    The fall-through PREDATES this change — all five branches exist at the merge
    base. What this change alters is its consequence. Before it a bound slot
    carried no inherited value, so the local write clobbered a local default;
    now the same write destroys the value the peer pinned, which is
    ``admit_peer_value``/``admit_peer_effort`` undone from the opposite side. That
    is why it is closed here rather than rebutted as pre-existing.

    These drive each endpoint over HTTP instead of asserting the predicate:
    testing a bound against its own constant passes while the handler still falls
    through, which is exactly how an earlier round of this PR left a clamp alive.
    """

    @pytest.mark.parametrize("missing", ["instance_id", "remote_slot"])
    @pytest.mark.parametrize("segment,body,field,inherited", _CONTROL_ROUTES)
    async def test_the_route_refuses_instead_of_committing_locally(
        self, segment: str, body: dict, field: str, inherited: str, missing: str
    ):
        slot = _half_bound(_ChatSlot("test"), missing=missing)
        setattr(slot, field, inherited)
        state = _mock_state(slot)
        with patch(
            "kiro_crew.dashboard.chat_handlers._apply_remote_pick",
            new=AsyncMock(return_value=web.json_response({"ok": True})),
        ) as pick:
            async with TestClient(TestServer(_control_app(state))) as client:
                resp = await client.post(f"/api/chat/slots/test/{segment}", json=body)
                assert resp.status == 409
                assert (await resp.json())["code"] == "remote_binding_incomplete"
        # Neither arm ran: nothing was forwarded (there is no binding to forward
        # through) and nothing local was written or reset.
        assert pick.await_count == 0
        assert getattr(slot, field) == inherited
        assert state.sessions.reset.await_count == 0

    @pytest.mark.parametrize("segment,body,field,inherited", _CONTROL_ROUTES)
    async def test_a_complete_binding_still_forwards(
        self, segment: str, body: dict, field: str, inherited: str
    ):
        """The guard must refuse the half-bound state ONLY. A bound slot whose
        binding is whole still travels, or the fix has broken every peer pick."""
        slot = _remote(_ChatSlot("test"))
        setattr(slot, field, inherited)
        state = _mock_state(slot)
        with patch(
    ...
    @pytest.mark.parametrize("segment", [r[0] for r in _CONTROL_ROUTES])
    async def test_an_unparseable_body_still_wins_the_precedence(self, segment: str):
        """``read_bounded_json`` is the one validator that precedes the guard on
        ALL four routes, so a body that is not a JSON object is still answered as
        a body problem. That ordering is deliberate: the guard reads
        ``slot.executor``, and a client that sent garbage has to fix the garbage
        whatever the binding holds. Pins it so moving the guard above the parse
        is visible."""
        slot = _half_bound(_ChatSlot("test"), missing="remote_slot")
        state = _mock_state(slot)
        async with TestClient(TestServer(_control_app(state))) as client:
            resp = await client.post(
                f"/api/chat/slots/test/{segment}", json=["not", "an", "object"]
            )
            assert resp.status == 400
    ...
    @pytest.mark.parametrize(
        "segment,body,code",
        [
            ("agent", {"agent": "bad name/../"}, None),
            ("workspace", {"workspace": 17}, "invalid_workspace"),
        ],
    )
    async def test_a_route_level_value_validator_still_precedes_the_guard(
        self, segment: str, body: dict, code: str | None
    ):
        """``agent`` and ``workspace`` validate the SUBMITTED VALUE at route level,
        above the guard, so a bad value there is still a 400. Only these two do —
        see the sibling test for why ``model`` and ``reasoning-effort`` cannot."""
        slot = _half_bound(_ChatSlot("test"), missing="instance_id")
        state = _mock_state(slot)
        async with TestClient(TestServer(_control_app(state))) as client:
            resp = await client.post(f"/api/chat/slots/test/{segment}", json=body)
            assert resp.status == 400
    ...
    @pytest.mark.parametrize(
        "segment,body",
        [("model", {"model": "id\nnewline"}), ("reasoning-effort", {"reasoning_effort": "\n"})],
    )
    async def test_the_refusal_wins_where_the_shape_check_lives_inside_the_arm(
        self, segment: str, body: dict
    ):
        """On ``model`` and ``reasoning-effort`` the value's shape check sits INSIDE
        the ``slot.is_remote`` arm, which a half-bound slot never enters — so the
        409 is the only answer available, and it is also the truthful one: no value
        whatsoever succeeds on this slot, so a 400 would invite a retry that cannot
        pass. Asserted rather than left implicit because it is the one place the
        guard outranks a shape complaint."""
        slot = _half_bound(_ChatSlot("test"), missing="remote_slot")
        state = _mock_state(slot)
        async with TestClient(TestServer(_control_app(state))) as client:
            resp = await client.post(f"/api/chat/slots/test/{segment}", json=body)
    ...
    async def test_a_purely_local_slot_is_untouched_by_the_guard(self):
        """``executor == "local"`` never reaches the refusal, whatever the other
        binding fields hold — the predicate keys on the executor first."""
        from kiro_crew.dashboard.chat_handlers import _remote_binding_incomplete

        local = _ChatSlot("test")
        assert _remote_binding_incomplete(local) is None
        local.instance_id = "nobita"  # stale leftovers, still local
        local.remote_slot = "peer-chat-9"
        assert _remote_binding_incomplete(local) is None
        assert _remote_binding_incomplete(_remote(_ChatSlot("bound"))) is None

    def test_the_send_path_and_the_control_routes_share_one_predicate(self):
        """The rule has ONE home. Five inline copies is what left four of the five
        branches un-guarded, so the send path is a CALLER of the helper rather
        than its sibling — asserted by source so a re-inlined copy is caught."""
        import inspect

        from kiro_crew.dashboard import chat_handlers

        src = inspect.getsource(chat_handlers)
        # The predicate itself appears exactly once: inside the helper.
        assert src.count('slot.executor == "remote" and not slot.is_remote') == 1
        # And every control route plus the send path reaches it.
        assert src.count("_remote_binding_incomplete(slot)") == len(_CONTROL_ROUTES) + 1
