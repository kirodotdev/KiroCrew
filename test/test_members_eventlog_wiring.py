...
class TestHistoryRoute:
    ...
    async def test_newest_first_and_before_paging(self, tmp_path, monkeypatch):
        ...
        # Newest first, stated as the ordering rather than as one fixture's
        # positions. The log also carries entries the service appends for its own
        # bookkeeping, so a literal seq list pins their number too and moves
        # whenever that changes -- which is not what this test is named for.
        seqs = [e["seq"] for e in data["events"]]
        assert seqs == sorted(seqs, reverse=True), f"not newest-first: {seqs}"
        assert data["lastSeq"] == seqs[0] == svc.last_seq(slug)

        # Every message this test appended is on the page, newest first, which is
        # the part a reordering or a dropped record would break.
        previews = [
            e["data"]["preview"] for e in data["events"] if e["type"] == types.MEMBER_MESSAGE
        ]
        assert previews == ["4", "3", "2", "1", "0"]

        # `before` is exclusive and keeps the ordering.
        pseqs = [e["seq"] for e in paged["events"]]
        assert pseqs == sorted(pseqs, reverse=True), f"paging lost the order: {pseqs}"
        assert all(s < 2 for s in pseqs), f"before=2 must exclude 2 and above: {pseqs}"
        assert max(pseqs) == 1, "and must reach the entry just below the bound"
...
class TestActivitySurvivesNewerTraffic:
    """A busy member must not lose its activity drawer to its own chatter.

    The handler reads activity records without a window sized in events of
    every type. A fixed pre-filter window holds no activity at all once more
    newer message or patrol events exist than it admits: the drawer renders
    empty while `capped` reports false, so the response loses the records and
    denies losing them.
    """

    @staticmethod
    def _activity_app(state):
        from kiro_crew.dashboard.handlers.members import api_member_activity
    ...
    @pytest.mark.asyncio
    async def test_activity_older_than_a_thousand_newer_events_is_still_returned(
        self, tmp_path, monkeypatch
    ):
        cfg = _fake_config({CREW: _agent()})
        monkeypatch.setattr("kiro_crew.dashboard.handlers.members.KiroCrewConfig.load", lambda: cfg)
        ...
        svc.append(
            slug,
            types.ACTIVITY_RECORD,
            # ISO-8601, the shape record_activity writes: _parse_activity_ts
            # reads anything else as unplaceable and the handler skips it, so a
            # numeric ts here would make this test pass for the wrong reason.
            {
        ...
        # Bury it under more newer events of another type than any fixed
        # pre-filter window would admit.
        for i in range(1100):
            svc.append(slug, types.MEMBER_MESSAGE, {"text": f"m{i}"})

        state = _make_state(tmp_path)
        async with TestClient(TestServer(self._activity_app(state))) as client:
            resp = await client.get(f"/api/members/{slug}/activity?member={CREW}")
            assert resp.status == 200
            body = await resp.json()

        projects = [e.get("project") for e in body["entries"]]
        assert "/work/early" in projects, (
            "the activity record was dropped by a window sized in events of every "
            f"type: got {len(body['entries'])} entries {projects[:5]}"
        )
        assert body["capped"] is False, (
            "one activity record is not a capped page; `capped` must not be true "
            "and must not be false-by-accident from an empty window"
        )
