...
from kiro_crew.security import ProjectPathVerdict
...
class TestCronJobProjectPath:
    ...
    def test_add_job_sensitive_project_path_rejected(self, tmp_path: Path) -> None:
        """The refusal is what is under test, so the verdict is supplied directly.

        ``_validate_project_path`` delegates to
        :func:`security.resolve_project_path`, which does a real
        ``realpath(expanduser)`` + ``isdir`` against whatever path it is given.
        Stubbing it at ``kiro_crew.cron.resolve_project_path`` -- the name
        ``cron.py`` imports and calls -- keeps the refusal genuinely exercised
        while the path stays synthetic, so no credential directory is reached
        and the test performs no filesystem access at all.
        """
        svc = CronService(base_dir=tmp_path)
        svc._load()
        sensitive_path = "/synthetic/credentials-dir"
        verdict = ProjectPathVerdict(resolved=sensitive_path, sensitive=True, is_dir=True)
        with patch("kiro_crew.cron.resolve_project_path", return_value=verdict):
    ...
    def test_update_job_resubmitting_the_stored_project_path_is_not_a_change(
        self,
        tmp_path: Path,
    ) -> None:
        """Presence is not a change: an equal ``project_path`` is not re-validated.

        The job form resubmits ``project_path`` unchanged on every edit (the same
        way it resubmits ``chat_folder_id``), so once the bound folder is moved or
        deleted -- the state the fire path's vanished-directory skip exists for --
        re-validating the equal value refused every unrelated edit with the
        existing-directory error instead of saving it. Only a DIFFERENT value is
        held to the create-time bar; the sibling test above pins that a changed
        value still is.
        """
        project_dir = tmp_path / "myproject"
        project_dir.mkdir()
        ...
        stored = job.project_path
        project_dir.rmdir()
        updated = svc.update_job(job.id, name="renamed", project_path=stored)
        assert updated is not None
        assert updated.name == "renamed"
        assert updated.project_path == stored, "an equal resubmission must not move the binding"
        # A DIFFERENT missing folder is still refused -- the bar itself is intact.
        with pytest.raises(ValueError, match="existing directory"):
            svc.update_job(job.id, project_path=str(tmp_path / "elsewhere"))
