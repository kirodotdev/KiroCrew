...
class TestSaveSlotStrict:
    ...
    def test_a_replaced_slot_identity_refuses_without_writing(self, tmp_path, monkeypatch):
        state, slot = _slot_with_history(tmp_path, monkeypatch)
        history_key = slot_history_key(slot)
        state.save_slot_strict(slot)
        slot.append("assistant", "stale seed", "msg msg-seed")
        replacement = state.get_or_create_slot("replacement")
        state._slots[slot.key] = replacement

        with pytest.raises(RuntimeError, match="refused the save"):
            state.save_slot_strict(slot, expected_slot_name=slot.key)

        on_disk = state.conversation_log.read_messages_chained(history_key)
        assert [m["content"] for m in on_disk] == ["hello", "world"]
        assert slot._dirty is True

    def test_a_mismatched_history_key_refuses_without_writing(self, tmp_path, monkeypatch):
        state, slot = _slot_with_history(tmp_path, monkeypatch)
        history_key = slot_history_key(slot)
        state.save_slot_strict(slot)
        slot.append("assistant", "stale seed", "msg msg-seed")
        ...
        with pytest.raises(RuntimeError, match="refused the save"):
            state.save_slot_strict(slot, expected_history_key=f"{history_key}:other")
