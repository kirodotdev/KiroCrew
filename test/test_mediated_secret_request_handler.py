...
def _patch_common(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    # By default the per-call member proof verifies (the approved-invocation path).
    # Tests that exercise a forged/missing proof override this.
    monkeypatch.setattr(
        "kiro_crew.member_memory_auth.verify_member_session_proof",
        lambda proof, sk, **_kw: True,
    )
...
async def test_a_missing_or_invalid_member_proof_is_refused(
    ...
    """A caller with the loopback secret + session key but WITHOUT a valid
    gateway-minted member proof (a raw in-sandbox shell) is refused 403 before
    any dispatch — binding the egress to an approved kirocrew-secrets invocation.
    """
    _patch_common(monkeypatch)
    # Override the default: the proof does NOT verify (forged/absent).
    monkeypatch.setattr(
        "kiro_crew.member_memory_auth.verify_member_session_proof",
        lambda proof, sk, **_kw: False,
    )

    def fake_perform(req: Any, _config_dir: Any) -> SanitizedResponse:  # pragma: no cover
        raise AssertionError("dispatch must not run without a valid member proof")
    ...
    assert resp.status == 403
    assert _body(resp)["code"] == "member_proof_required"
