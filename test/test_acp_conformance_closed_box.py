"""Phase 3 closed-box ACP v1 conformance gate for ``kirocrew acp``.
...
* :class:`acp_bb_editor.AcpEditor` is the closed-box client.
...
pytestmark = pytest.mark.xdist_group(name="acp_closed_box")
...
def test_editor_timeout_cleanup_uses_portable_process_tree_kill(
    monkeypatch: pytest.MonkeyPatch, tmp_path: Path
) -> None:
    killed: list[tuple[int, int]] = []
    editor = AcpEditor("http://127.0.0.1:1", home=str(tmp_path))

    class Process:
        pid = 4242

    editor._proc = Process()  # type: ignore[assignment]
    monkeypatch.setattr(
        acp_bb_editor.platform_compat,
        "kill_process_tree",
    ...
    assert killed == [(4242, acp_bb_editor.platform_compat.SIGKILL)]
...
class TestPromptTurn:
    ...
    def test_resource_link_block_preserved_across_boundary(self, gateway, make_editor):
        ...
        editor.prompt(
            sid,
            [
                {"type": "resource_link", "name": "main.py", "uri": uri},
...
class TestJsonRpcErrors:
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            {"type": "unknown", "text": "must not be truncated"},
        ],
    )
    def test_malformed_prompt_block_is_invalid_params(self, gateway, make_editor, block):
        editor = make_editor(gateway)
        editor.initialize()
        sid = editor.session_new()["result"]["sessionId"]
        posts_before = len(gateway.state.chat_posts)
        resp = editor.request(
            "session/prompt",
            {
                "sessionId": sid,
                "prompt": [{"type": "text", "text": "keep"}, block],
            },
        )
        assert _error(resp).get("code") == -32602
        assert len(gateway.state.chat_posts) == posts_before
        editor.assert_conformant()
...
class TestStdioMcp:
    def test_session_new_with_stdio_mcp_preflights_and_registers(self, gateway, make_editor):
        ...
        editor = make_editor(gateway)
        editor.initialize()
        resp = editor.session_new(cwd=tempfile.gettempdir(), mcp_servers=[echo_mcp_server("echo")])
        if "error" in resp:
    ...
    def test_unsupported_transport_rejected_invalid_params(self, gateway, make_editor):
        ...
        resp = editor.session_new(
            ...
            mcp_servers=[{"name": "remote", "url": "https://mcp.example"}],
        )
        assert _error(resp).get("code") == -32602
    ...
    def test_bad_mcp_command_rejected_client_safe(self, gateway, make_editor):
        ...
        editor.initialize()
        resp = editor.session_new(
            cwd=tempfile.gettempdir(), mcp_servers=[{"name": "b", "command": "/no/such/binary-xyz"}]
        )
        assert _error(resp).get("code") == -32603
        assert "failed to start" in _error(resp).get("message", "")
        assert gateway.state.mcp == {"acp-slot-1": []}
        editor.assert_conformant()
...
async def test_official_python_sdk_smoke(gateway, tmp_path) -> None:
    ...
    try:
        ...
        session = await connection.new_session(cwd=tempfile.gettempdir(), mcp_servers=[])
