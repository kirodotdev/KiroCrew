...
class TestRound7Hardening:
    ...
    def test_read_meta_carries_ns_token(self, doc_tree):
        ...
        assert code == 200
        # STRING: a nanosecond mtime is past JS's exact-integer range, so a
        # numeric token rounds in the browser and 409s every subsequent save.
        assert body["mtime_ns"] == str(md.stat().st_mtime_ns)
...
class TestRound9Hardening:
    ...
    def test_pdf_search_never_reopens_the_path(self, doc_tree, monkeypatch):
        """With no pdfminer, PDF search returns nothing rather than falling back
        to a path-based reader.

        The removed fallback called ``doc_parser.extract_text(str(p))``, which
        REOPENED the user-derived path outside the hardened read gate — so a path
        swapped for a symlink between validation and that reopen would surface a
        sensitive file's contents as search results. The tripwire asserts the
        path-based reader is never reached, not merely that the result is empty.
        """
        calls = []

        def tripwire(path):
            calls.append(path)
            return "zanzibar leaked"

        from kiro_crew import doc_parser

        monkeypatch.setattr(server, "_pdf_extract", None)
        monkeypatch.setattr(doc_parser, "extract_text", tripwire)
        assert server._pdf_text_units(doc_tree / "paper.pdf") == []
        assert calls == [], f"reopened the path via {calls}"

    def test_doc_search_skips_symlink_escaping_roots(self, doc_tree):
        # Everything this test creates stays under its OWN tmp_path. The tree the
        # symlink escapes to is outside the allowed ROOT, not outside the test
        # directory: `doc_tree.parent` is pytest's shared numbered dir, so making
        # a sibling there leaves residue that outlives this test's cleanup and can
        # bleed into later ones. Narrowing ALLOWED_ROOTS to a nested directory
        # gets the same escape geometry with no shared state.
        inroot = doc_tree / "inroot"
        inroot.mkdir()
        make_docx(inroot / "report.docx")  # the legit in-root match
        outside = doc_tree / "outside_docs"  # under tmp_path, outside the root
        outside.mkdir()
        make_docx(outside / "leak.docx")  # contains the zanzibar keyword
        (inroot / "leaklink.docx").symlink_to(outside / "leak.docx")
        with patch.object(server, "ALLOWED_ROOTS", [inroot]):
            results = server._search_documents(inroot, "zanzibar", "", "", 50)
        assert results  # the legit in-root report.docx still matches
        assert not any("leaklink" in r["file"] for r in results)

    def test_read_emits_conflict_token_as_string(self, doc_tree):
        """The conflict token crosses the wire as text, never as a JSON number.

        It carries a nanosecond mtime (~1.7e18), past the exact-integer range of
        the JavaScript double a JSON number parses into (2^53-1). Sent as a
        number it silently rounds in the browser, returns changed, and the
        exact-match guard in /write then rejects EVERY save with 409 — i.e.
        markdown editing never works at all.
        """
        target = doc_tree / "notes.md"
        code, body = _json_request(f"/read?path={target}")
        assert code == 200
        assert isinstance(body["mtime_ns"], str), type(body["mtime_ns"])
        assert body["mtime_ns"] == str(target.stat().st_mtime_ns)

    def test_write_rejects_an_unmatched_token_instead_of_failing_open(self, doc_tree):
        """A token that does not match the file must 409.

        The earlier comparison parsed the token with int() inside
        ``try/except ValueError: pass``, so a malformed token SKIPPED the
        conflict guard entirely and the write proceeded — the opposite of what
        the guard exists for.
        """
        target = doc_tree / "notes.md"
        before = target.read_text()
        code, body = _json_request(
            f"/write?path={target}&base_token=not-a-number",
            method="POST",
            post_body=b"clobbered",
        )
        assert code == 409, body
        assert target.read_text() == before, "the write went through anyway"

    def test_write_accepts_the_token_read_gave_it(self, doc_tree):
        """The round trip works: the string token from /read is accepted by
        /write, which is what proves the two ends agree on the format."""
        target = doc_tree / "notes.md"
        _, meta = _json_request(f"/read?path={target}")
        code, body = _json_request(
            f"/write?path={target}&base_token={meta['mtime_ns']}",
            method="POST",
            post_body=b"# Notes\n\nrewritten\n",
        )
    ...
    def test_locked_write_good_token_replaces_atomically(self, doc_tree):
        ...
        # The response token is a string too — the editor adopts it as its next
        # base, so a rounded number here would 409 the very next save.
        assert responses[0][1]["mtime_ns"] == str(md.stat().st_mtime_ns)
