...
class Unguarded(NamedTuple):
    """One CALL a redactor would not help, and the property that makes that true.

    Keyed to a call site rather than to a module. A module-wide key says "trust
    every splitter call in this file", which is not what any of these claims
    actually says: each describes ONE call. ``whatsapp/renderer.py`` holds a
    legitimately unguarded call beside a guarded one in the same function, so the
    function name alone does not separate them either -- ``count`` does, and it is
    what makes the enforcement below a ratchet: the entry licenses exactly that
    many unguarded calls in that function and nothing else in the module, so
    dropping the redactor from the guarded neighbour fails instead of inheriting
    a licence written for its sibling.
    """
...
#: Calls a redactor would not help, keyed by module. An entry is a claim about the
#: named call site, and ``test_no_exception_has_gone_stale`` fails when the claim
#: stops holding, so an entry cannot outlive the code it describes.
NO_SEAM_TO_GRADE = {
    "whatsapp/renderer.py": Unguarded(
        function="render_chunks",
        count=1,
        why=(
...
class TestEveryDeliveryPathIsEnumerated:
    ...
    def test_every_splitter_call_passes_a_redactor(self, path: str) -> None:
        """No module is skipped, so an exemption cannot shelter its neighbours.

        An excepted module is checked here like any other: only the call site its
        entry NAMES may be unguarded, and only as many times as the entry says.
        """
        allowed = NO_SEAM_TO_GRADE.get(path)
        offenders = [
            call
            for call in _unguarded(path)
            if allowed is None or call.function != allowed.function
        ]
        ...
        if allowed is not None:
            inside = [call for call in _unguarded(path) if call.function == allowed.function]
            assert len(inside) == allowed.count, (
                f"{path}.{allowed.function} now has {len(inside)} unguarded splitter "
                f"call(s), not the {allowed.count} its exception describes. The "
                "exception licenses one specific call; a second one in the same "
                "function is a different call and needs its own answer."
            )
    ...
    def test_no_exception_has_gone_stale(self, path: str) -> None:
        ...
        Three ways a claim stops holding, and each fails here: the module reaches no
        splitter at all, the named function is gone, or that function's calls all
        pass a redactor now -- which means the exception grants a licence nothing
        uses. Deleting the entry is then the fix, and leaving it would let a LATER
        unguarded call inherit an exemption written for a call that is gone.
        """
        allowed = NO_SEAM_TO_GRADE[path]
        assert path in SPLITTING_MODULES, (
            f"{path} is excepted but reaches no splitter -- delete its "
            "NO_SEAM_TO_GRADE entry rather than leaving the call site exempt."
        )
        functions = {call.function for call in SPLITTING_MODULES[path]}
        assert allowed.function in functions, (
            f"{path} is excepted at {allowed.function}, which holds no splitter call "
            f"any more (the calls are in {sorted(functions)}) -- delete or re-point "
            "the entry so it cannot cover a call it was not written for."
        )
        inside = [call for call in _unguarded(path) if call.function == allowed.function]
        assert inside, (
            f"{path}.{allowed.function} now passes a redactor on every splitter call "
            "-- delete its NO_SEAM_TO_GRADE entry so a later call cannot inherit the "
            "exemption."
        )
...
class TestDiscordRotatesWithoutHandingOverAKey:
    ...
    def test_the_reasoning_note_becomes_the_predecessor(self) -> None:
        ...
        assert sent_at, "the reasoning note is no longer a message of its own"
        assert recorded_at, (
