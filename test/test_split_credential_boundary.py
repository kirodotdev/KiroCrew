...
class TestTheTerminalKeepsWhatItCan:
    ...
    def test_no_global_whitespace_flush_remains(self) -> None:
        """Nothing strips a valid span's whitespace or markup.

        A sampled budget that misses a safe one costs the reader no formatting:
        every whitespace rewrite in the module sits inside the span repair, which
        only ever touches one key's own span.
        """
        import kiro_crew.messaging.split as split_module

        assert not hasattr(split_module, "_flush_whitespace")

    def test_the_terminal_declines_to_cut_rather_than_strip(self) -> None:
        """When no span names the key the text goes back whole, not flattened."""
        source = (SRC / "messaging" / "split.py").read_text(encoding="utf-8")
        assert "return [safe]" in source, "the terminal returns the text uncut"
...
class TestASealedPrefixCannotBeCompletedLater:
    ...
    def test_both_streaming_legs_offload_and_record_it(self) -> None:
        for path in ("telegram/renderer.py", "whatsapp/turn_renderer.py"):
            ...
            assert "repaired_after_a_sent_tail" in offloaded, f"{path} must offload the repair"
