...
class TestNoteVisibleOnly:
    ...
    async def test_a_failed_durable_write_is_not_answered_as_appended(
        ...
        ``save_slot_off_loop`` returns ``False`` WITHOUT raising when the slot's
        routing moved off the key the call authorized, so that refusal is invisible
        to a ``try``; ``_save_slot_to_history`` writes through an atomic rename, so a
        raise likewise leaves the transcript exactly as it stands. Answering 200 on
        either arm would hand the documented close-on-``appended === true`` rule a
        row that exists only in the live window.

        The row is RETRACTED on BOTH arms, and the arms share one assertion set so
        they cannot drift apart: a row left in the window is unpersisted state the
        periodic ``flush_slot_now`` makes durable anyway -- and the 503's own
        instruction to re-post then lands the note twice. Retracting on the refusal
        arm alone would leave the commoner failure, a lock timeout, duplicating.
        """
        state, slot = _state_and_slot(tmp_path)
    ...
    async def test_a_refused_row_is_never_surfaced_to_a_connected_client(self, tmp_path: Path):
        ...
        ``append`` surfaces through two doors -- the ``chat_message`` broadcast and
        ``_pending``, which an attached stream reader drains -- and both fire before
        the durable write. A row surfaced then retracted stays on screen as a ghost
        that the 503's own re-post instruction duplicates, so delivery waits for the
        commit.
        """
        state, slot = _state_and_slot(tmp_path)
        surfaced: list[dict] = []
    ...
    async def test_a_pinned_row_is_surfaced_through_both_doors(self, tmp_path: Path):
        ...
        Negative control for the test above: with the same instrumentation and a
        committing save, the row reaches the broadcast and the reader queue.
        """
        state, slot = _state_and_slot(tmp_path)
        surfaced: list[dict] = []
        slot._on_message = lambda _key, row: surfaced.append(row)
    ...
    async def test_a_retraction_keeps_a_concurrent_append_and_its_dirty_mark(self, tmp_path: Path):
        ...
        The durable attempt awaits, so position is not a handle on our row and the
        dirty mark is not ours alone to clear: a positional pop would evict the
        newer row, and clearing the mark would strand it with no flush to carry it.
        """
        state, slot = _state_and_slot(tmp_path)
    ...
    async def test_a_close_that_won_the_lock_is_not_undone_by_the_pin(self, tmp_path: Path):
        ...
        Drives the REAL save, because the guard under test lives inside it. The pin
        is open-shaped and ``closed``/``closed_at`` are ``SLOT_OWNED``, so a rebuild
        writes neither and carries neither back: committing after the close erases
        the dismissal, and the holder is popped so nothing rewrites it. The tab
        returns from the archive and the channel reconciler re-arms on it.

        A concurrent POST and DELETE produces this by ordinary interleaving -- the
        handler resolves its slot before the close pops it, and which save reaches
        the lock first is a coin toss.
        """
        state, slot = _state_and_slot(tmp_path)
    ...
    async def test_a_refusal_records_the_app_isolation_denial(self, tmp_path: Path, monkeypatch):
        ...
        The deferred sibling logs ``note_post``/``denied``/``app_isolation`` when a
        rebind takes its hold, and the immediate path refuses for the same reasons
        -- a rebind off the authorized transcript, or a close that committed first.
        A refusal recorded nowhere leaves an app able to hit the guard repeatedly
        with nothing in the log to show it.

        The RAISING arm is the control: an I/O or lock failure is not an
        authorization outcome, and classifying it as one would put a denial in the
        log for every transient disk error.
        """
        state = _make_state(tmp_path)
        events: list[dict] = []
        monkeypatch.setattr(
    ...
    async def test_a_same_key_recreation_does_not_receive_the_pin(self, tmp_path: Path):
        ...
        Drives the REAL save, because the guard under test lives inside it. Neither
        existing guard can see this: a channel-, cron- or workflow-born slot keeps
        its ``history_key`` across a close-and-recreate, so ``expected_history_key``
        matches, and the replacement republishes an OPEN line, so ``refuse_if_closed``
        finds no dismissal. Only the generation distinguishes them -- without it the
        original's window merges into the replacement's transcript.
        """
        state = _make_state(tmp_path)
    ...
    async def test_a_committed_pin_is_not_delivered_to_a_replacement_slot(self, tmp_path: Path):
        ...
        The generation guard settles the DISK; this settles DELIVERY. A recreation
        that happens while the save runs leaves the registry pointing at a new slot,
        and surfacing the row there would show one app's breadcrumb in another's live
        window. The save is made to COMMIT so the identity check is the only thing
    ...
    async def test_a_non_true_visible_only_still_does_both_writes(
        ...
        The three non-true spellings share one assertion set because they are one
        rule, and a case that drifted apart from its siblings would hide exactly
        that. ``null`` is included because it must read as omitted -- the same
        reading ``maxAge`` gives it -- and ``false`` because an explicit false must
        not take a different path from an absent field.
        """
        state, slot = _state_and_slot(tmp_path)
...
class TestDeferredNoteLostOnClose:
    ...
    async def test_a_deferred_note_now_SURVIVES_its_slot_closing(self, tmp_path: Path):
        ...
        deferred breadcrumb remains correct. A close preserves what it held.
        """
        state, slot = _state_and_slot(tmp_path)
        slot.task = asyncio.get_running_loop().create_future()
        assert slot.running is True
    ...
    async def test_a_close_keeps_the_context_half_durable(self, tmp_path: Path):
        ...
        ``flush_deferred_notes`` promotes a context half into ``slot._pending_context``,
        which the persistence layer never serializes, and ``_close_slot`` pops the slot
        after the save. So a close that queued the half destroyed it, while the
        delivered row's ``meta.noteId`` retired the durable entry in the same write --
        leaving nothing for an ``adopt_closed`` rehydration to replay. An ordinary
        restore skips a closed session outright, so the durable residual is the only
        channel that survives the close at all.

        Asserted on the SERIALIZED bytes rather than the live object, because it is
        the on-disk copy the rehydration reads.
        """
        state, slot = _state_and_slot(tmp_path)
        slot.task = asyncio.get_running_loop().create_future()
    ...
    async def test_the_cancelled_turns_own_flush_cannot_drain_the_hold_first(self, tmp_path: Path):
        ...
        ``_close_slot`` pops the slot, then cancels its task. The cancelled turn
        runs its own teardown flush (``chat_runner._finish_queue_cycle``) and, with
        the slot already gone from the registry, takes the branch that hands a
        context half to the next turn -- promoting it into ``_pending_context``,
        which is never serialized and dies with this frame. A retention flush placed
        after the cancel finds an empty hold and retains nothing, so the close still
        loses the half it was changed to keep.

        The tests above use a bare future for ``slot.task``: cancelling one runs no
        teardown, so none of them can see this. This one uses a real task with the
        runner's own flush in its ``finally``.
        """
        state, slot = _state_and_slot(tmp_path)
    ...
    def test_a_residual_is_not_drained_by_a_plain_flush_while_closing(self, tmp_path: Path):
        ...
        A close reaches several flush sites -- its own, the cancelled turn's, the
        hand-over tail's -- and only the first knows it is retaining. A plain flush
        that promoted the residual would undo the retention one call later, so the
        slot's own in-flight close is what decides, not the keyword alone.
        """
        state, slot = _state_and_slot(tmp_path)
