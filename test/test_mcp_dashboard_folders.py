...
class TestTheTwoSidesCompareNamesIdentically:
    ...
    The key consults no Unicode table. Case folding is the operation that would:
    `str.lower()` reads the interpreter's tables and `String.prototype.toLowerCase`
    reads the browser's, so a character whose case mapping differs between those
    two versions would fold differently on each side, and neither side owns both
    tables. Comparing unfolded UTF-16 code units depends only on the encoding.
    """

    def test_case_is_not_folded(self) -> None:
        """Uppercase sorts ahead of lowercase, which is what both sides now do."""
        rows = [
            {"id": "aaaaaaaaaaaa", "name": "apple", "parent_id": "", "order": 0},
            {"id": "bbbbbbbbbbbb", "name": "Apple", "parent_id": "", "order": 0},
        ]
        names = [str(f["name"]) for f in mcp_dashboard._chat_folder_siblings(rows, "")]
        assert names == ["Apple", "apple"]

    def test_the_key_is_the_raw_utf16_encoding(self) -> None:
        """Pins the absence of a fold, so re-adding one is caught here.

        A fold would make this key differ from `text.encode(...)` for any name
        carrying an uppercase letter, and re-introduce the version skew.
        """
        for name in ("Apple", "\u00dfeta", "STRASSE", "\u0130stanbul"):
