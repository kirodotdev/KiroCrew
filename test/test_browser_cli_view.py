...
class FakeProc:
    ...
    def __init__(self, alive: bool = True, pid: int = 424242, stdout: bytes = b"") -> None:
        ...
        setattr(proof, "root_start_id", str(pid))
    ...
    [
        ...
        pytest.param(
            ...
            marks=pytest.mark.skipif(
                ...
                reason="Windows has no independent per-process ownership verdict",
            ),
        ),
    ],
)
...
def test_structurally_blind_status_without_lifetime_proof_names_ownership_failure(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    ...
    assert mod.status()["reason"] == "Couldn't confirm the browser view still owns its port"
...
def test_windows_owner_stability_is_scoped_to_the_listener_chain(
    ...
    maps = iter((first_map, second_map))
...
def test_inconclusive_control_probe_never_claims_foreign(
    ...
    proc = FakeProc(pid=4242)
    probes = iter((([], target_completed), ([], False)))
    monkeypatch.setattr(platform_compat, "listening_pid_tool_available", lambda: True)
    monkeypatch.setattr(
        platform_compat,
        "trusted_system_bin",
        lambda name: "/usr/bin/lsof",
    )
    monkeypatch.setattr(
        platform_compat,
        "probe_port_listeners",
        lambda port: next(probes),
    )
...
def test_listener_self_test_cache_invalidates_when_the_tool_path_changes(
    ...
    tool_path = ["C:/Windows/System32/netstat.exe"]
    ...
    monkeypatch.setattr(
        platform_compat,
        "trusted_system_bin",
        lambda name: tool_path[0] if name == "netstat" else None,
    )
    monkeypatch.setattr(
        mod,
        "_claim_listener",
    ...
    assert mod._listener_lookup_functional() is True
    tool_path[0] = "D:/Tools/netstat.exe"
    assert mod._listener_lookup_functional() is True
...
def test_recycled_descendant_identity_is_denied_around_listener_probe(
    monkeypatch: pytest.MonkeyPatch,
    caplog: pytest.LogCaptureFixture,
    ...
    monkeypatch.setattr(
        platform_compat,
        "process_descendant_identities",
        lambda pid: [platform_compat.ProcessDescendantIdentity(9931, 4242, "old")],
        raising=False,
    )
    ...
    monkeypatch.setattr(
        platform_compat,
        "process_start_time",
        lambda pid: str(pid) if pid == proc.pid else next(reads),
    )
...
def test_same_second_descendant_reuse_with_a_different_start_id_is_denied(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    proc = FakeProc(pid=4242)
    probed: list[int] = []
...
def test_recycled_root_is_reaped_and_respawned_on_reuse(
    monkeypatch: pytest.MonkeyPatch,
    reset_state: list[int],
) -> None:
    children = [FakeProc(pid=4242), FakeProc(pid=4343)]
    spawned: list[FakeProc] = []
    starts = {4242: "4242", 4343: "4343"}
    monkeypatch.setattr(mod, "cli_path", lambda: "/n/pw")
    monkeypatch.setattr(mod, "_healthy", lambda port: True)
...
def test_structurally_blind_start_and_immediate_status_keep_the_url(
    monkeypatch: pytest.MonkeyPatch,
    reset_state: list[int],
) -> None:
    proc = FakeProc(pid=4242)
    spawn_ports: list[int] = []
    reported_ports = iter((45613,))
    monkeypatch.setattr(platform_compat, "IS_WINDOWS", False)
    monkeypatch.setattr(platform_compat, "IS_LINUX", False)
    monkeypatch.setattr(platform_compat, "IS_POSIX", True)
    monkeypatch.setattr(mod, "_structurally_blind_listener_attribution", lambda: True)
    ...
    monkeypatch.setattr(
        mod,
        "_child_reported_port",
        lambda child, requested_port: next(reported_ports, None),
        raising=False,
    )
    monkeypatch.setattr(mod, "_child_reported_binding", lambda child, port: False)
    monkeypatch.setattr(
...
def test_structural_binding_proof_rejects_a_replacement_handle(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    original = FakeProc(pid=4242)
    replacement = FakeProc(pid=4242)
    monkeypatch.setattr(mod, "_healthy", lambda port: True)
    mod._remember_structural_binding(original, 45613)
    cached = getattr(original, mod._STRUCTURAL_BINDING_PORT_ATTR)
    setattr(replacement, mod._STRUCTURAL_BINDING_PORT_ATTR, cached)

    assert mod._has_structural_binding(original, 45613) is True
    assert mod._has_structural_binding(replacement, 45613) is False
...
def test_structurally_blind_dead_child_respawns_from_a_fresh_report(
    ...
    first = mod.ensure_running()
    assert first == mod.ShowInfo("http://127.0.0.1:45613", 45613)
    assert mod._has_structural_binding(spawned[0], 45613) is True
    spawned[0]._alive = False
    spawned[0].returncode = 1
    assert mod._has_structural_binding(spawned[0], 45613) is False
...
def test_transient_listener_probe_failure_still_degrades_without_reaping(
    ...
    assert status["reason"] == "Couldn't confirm the browser view still owns its port"
    assert mod._proc is proc
    assert spawned == [proc]
    assert reset_state == []
...
def test_spawn_captures_root_start_identity(
    ...
    monkeypatch.setattr(mod, "_process_start_identity", lambda pid: "born-at-1")
    ...
    assert proof.root_start_id == "born-at-1"
...
def test_reuse_replaces_a_live_child_after_global_squatter_proof(
    ...
    monkeypatch.setattr(
        ...
        "probe_port_listeners",
        lambda port: (
            [platform_compat.PortListener(listener_owner["pid"], "127.0.0.1", "4")],
            True,
        ),
    )
