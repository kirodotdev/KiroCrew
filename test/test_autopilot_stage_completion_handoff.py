...
@pytest.mark.parametrize(
    ("overflow_scope", "expected", "unexpected"),
    [
        ("", "completion event could not be processed", "global report-overflow state"),
        ("global", "global report-overflow state", "completion event could not be processed"),
    ],
)
@pytest.mark.asyncio
async def test_stage_failure_notice_distinguishes_global_overflow_from_own_debt(
    tmp_path,
    overflow_scope,
    expected,
    unexpected,
):
    """Gateway-wide overflow is not reported as this slot's own debt."""
    from kiro_crew.context_management import OrchestrationTracker
    from kiro_crew.dashboard.chat_orchestrator import _settle_stage_delivery
    ...
    error = SubagentReportDeliveryError(
        "terminal report failed",
        overflow_scope=overflow_scope,
        overflow_parent="dashboard:saturating-parent" if overflow_scope == "global" else "",
        overflow_owner="saturating-owner" if overflow_scope == "global" else "",
    )
    manager.wait_for_parent_reports = AsyncMock(side_effect=error)
    state.subagents = manager
    slot = state.get_or_create_slot("report-failure-notice", mode="orchestrator")
    slot._in_stage_execution = True
    slot.stage_boundary.arm(1, consumed=False)
    ...
    if overflow_scope == "global":
        assert "dashboard:saturating-parent" in text
        assert "saturating-owner" in text
        assert "discard or Cancel the paused plan(s) to clear it" in text
...
async def test_failed_delivery_pause_reaches_transcript_and_linked_channel(tmp_path):
    ...
    manager.wait_for_parent_reports = AsyncMock(
        side_effect=SubagentReportDeliveryError("terminal report failed")
    )
    state.subagents = manager
