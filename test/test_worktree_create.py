...
class TestWorktreeDirtyRefusesFilterRepo:
    """A repo declaring a content-filter driver must not have `status` probed.

    `git status` compares the worktree against the index, which runs a
    `filter.<name>.clean` driver for any tracked file carrying that filter
    attribute -- repo-supplied code, and the one class `_git_no_repo_code`'s `-c`
    overrides cannot disable (driver names are arbitrary), which is exactly why
    the create path refuses such a repo via `_checkout_filter`. The dirty probe
    must make the same call, answering "unknown" (`None`) instead of running it,
    which keeps `remove` refusing without an explicit force.
    """

    def test_filter_driver_repo_reports_unknown_not_clean(self, repo):
        _git("config", "--local", "filter.evil.clean", "cat /etc/hostname", cwd=repo)

        assert _worktree_dirty(str(repo)) is None

    def test_plain_repo_still_probes(self, repo):
        # Guard the test above against passing for the wrong reason: without a
        # filter driver the probe must still run and answer a real boolean.
        assert _worktree_dirty(str(repo)) is False
        (repo / "scratch.txt").write_text("uncommitted\n", encoding="utf-8")
        assert _worktree_dirty(str(repo)) is True

    @pytest.mark.asyncio
    async def test_remove_refuses_unforced_when_filter_blocks_the_probe(self, repo):
        wt = _add_worktree(repo, "proj-wt-filtered", "feat/filtered")
        _git("config", "--local", "filter.evil.clean", "cat /etc/hostname", cwd=repo)
        ...
        async with TestClient(TestServer(_make_app(str(repo)))) as client:
            ...
            assert resp.status == 409
            assert "uncommitted" in (await resp.json())["error"].lower()
        assert wt.exists()
