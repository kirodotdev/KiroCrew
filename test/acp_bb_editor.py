"""Closed-box ACP editor client for the conformance gate.
...
# matching the Phase-2 MCP fixture convention. It proves the adapter's
# session/new MCP preflight performs a genuine spawn + initialize handshake
# through the sandbox chokepoint.
ECHO_MCP_SCRIPT = r"""
import sys, json
def send(o):
...
class AcpEditor:
    """A closed-box ACP client driving one ``kirocrew acp`` subprocess."""
    ...
    def close(self, timeout: float = 10.0) -> int | None:
        """Close stdin, wait for a clean exit, and kill the child tree on timeout."""
        proc = self._proc
        if proc is None:
            return None
