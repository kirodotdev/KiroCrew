...
class TestFireDashboardNudgeDispatch:
    ...
    async def test_scheduled_message_revalidates_after_waiting_for_turn_admission(
        ...
        ds.sessions.get_mirror_link.return_value = None
        admission = session_control.containment_meta(ds, slot)
        mirror = SimpleNamespace(channel_type="slack", channel_id="C2", thread_id="2")

        async def wait_then_run(_slot, turn):
            ds.sessions.get_mirror_link.return_value = mirror
            await turn

        ds.run_background_turn.side_effect = wait_then_run
        orch.dashboard_state = ds
        orch.autonudge_svc = MagicMock()
    ...
    async def test_scheduled_remote_unavailable_stays_pending_without_a_row(
        ...
        orch.dashboard_state = ds
        orch.autonudge_svc = MagicMock()
        monkeypatch.setattr(gw, "peer_is_connected", lambda *_args: connected)
        monkeypatch.setattr(
            gw.autonudge_selfarm,
            "read_scheduled_message",
            lambda _record_id, _slot: gw.autonudge_selfarm.ScheduledMessageProvenance(
    ...
    async def test_scheduled_remote_delivery_retries_without_duplicate_user_rows(
        ...
        orch.autonudge_svc = MagicMock()
        orch.autonudge_svc.settle_unclaimed_scheduled_delivery = AsyncMock(return_value=True)
        monkeypatch.setattr(gw, "peer_is_connected", lambda *_args: True)
        monkeypatch.setattr(
            gw.autonudge_selfarm,
            "read_scheduled_message",
            lambda _record_id, _slot: gw.autonudge_selfarm.ScheduledMessageProvenance(
        ...
        monkeypatch.setattr(gw, "save_slot_off_loop", AsyncMock(return_value=True))
        relay_attempt = 0
        relay_admissions: list[dict] = []

        async def relay(_state, target, _message, *, containment_admission):
            nonlocal relay_attempt
            relay_attempt += 1
            relay_admissions.append(containment_admission)
            if relay_attempt > 1:
                return True
        ...
        monkeypatch.setattr(gw, "relay_remote_turn", relay)
