...
class TestTheGateArtifactsStayReachableInsideTheSandbox:
    ...
    def test_the_gate_leaf_is_the_walks_only_owner_checked_exemption(self):
        """Every exempt leaf owes the walk a reader that refuses the same states."""
        assert sandbox._CREW_OWNER_CHECKED_DIR_LEAVES == ("pi-gate",)
        assert set(sandbox._CREW_OWNER_CHECKED_DIR_LEAVES) <= set(
            sandbox._CREW_PRECREATE_READONLY_DIR_LEAVES
        )

    @pytest.mark.skipif(os.name == "nt", reason="POSIX symlink creation")
    def test_a_squat_on_the_gate_leaf_warns_instead_of_aborting_the_spawn(
        self, monkeypatch, tmp_path, caplog
    ):
        """Direction one: a pi-gate squat may not veto an unrelated backend's session.

        The walk runs on every Linux spawn, so raising here would let anything that can
        write the data home stop kiro, codex and the rest from launching at all. The pi
        session is not let off: ``_pi_gate_artifact_dir`` refuses the identical state,
        which the squat parametrization above pins.
        """
        ...
        assert any("pi-gate" in record.getMessage() for record in caplog.records)

    @pytest.mark.skipif(os.name == "nt", reason="POSIX symlink creation")
    def test_the_same_squat_on_a_shared_leaf_still_aborts_the_spawn(self, monkeypatch, tmp_path):
        """Direction two: the exemption is one leaf wide, not a softening of the walk."""
        leaf = tmp_path / "playwright-cli"
        leaf.symlink_to(tmp_path / "nowhere")
        monkeypatch.setattr(sandbox, "_sealable_absent_ceilings", lambda: ([str(leaf)], []))
