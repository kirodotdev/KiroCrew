...
class TestBroadcastMentionDefang:
    ...
    @pytest.mark.asyncio
    async def test_the_transport_leg_carries_per_app_channel_authorization(
        self, tmp_path, monkeypatch
    ) -> None:
        """A `channels` narrowing that excludes ONE transport must bind the note's app.

        `capabilities.messaging` answers only whether the app may message at all. An app
        permitted to message but excluded from Telegram is narrowed in the `channels`
        scope, which is evaluated per transport -- so dropping the app identity on this
        leg lets the surface profile's own grant admit a send the app's profile refuses.
        """
        ...
        real_vet = runner_mod.vet_and_audit if hasattr(runner_mod, "vet_and_audit") else None

        def _vet(scope, item, **kw):
            seen.append({"scope": scope, "item": item, "app": kw.get("app")})
            return SimpleNamespace(permitted=True)

        monkeypatch.setattr(
            "kiro_crew.platform.governance_profiles.vet_and_audit", _vet, raising=True
        )
        assert real_vet is None or True  # the module imports it lazily inside the resolver

        await _mirror(
            state, slot, "dashboard:chat-1", "note for telegram", "note", request_app="app-x"
        )

        channels = [c for c in seen if c["scope"] == "channels"]
        assert channels, f"the transport leg vetted no `channels` scope at all; saw {seen}"
        assert any(c["app"] == "app-x" for c in channels), (
            "the transport leg's `channels` authorization ran without the authoring app, so "
            "a per-app transport exclusion cannot refuse the note; channels calls were "
            f"{channels}"
        )

    @pytest.mark.asyncio
    async def test_messaging_revoked_mid_send_refuses_the_remaining_parts(
        self, tmp_path, monkeypatch
    ) -> None:
        """`capabilities.messaging` is re-asked PER PART, not once before dispatch.

        A pre-dispatch check cannot see a revocation that lands during the governance and
        chunk awaits, and the per-part revalidation asks only the `channels` scope -- so an
        app whose messaging capability is withdrawn mid-send otherwise keeps posting.
        """
        state = _make_state(tmp_path)
        state.slack_client = None
        tp = _real_caps_transport("telegram")
        ...
        def _vet(scope, item, **kw):
            if scope == "capabilities.messaging":
                asks["messaging"] += 1
                # Permitted for the pre-dispatch gate, revoked for every later ask.
                return SimpleNamespace(permitted=asks["messaging"] <= 1)
            return SimpleNamespace(permitted=True)
        ...
        monkeypatch.setattr(
            "kiro_crew.dashboard.handlers.messaging.vet_and_audit", _vet, raising=True
        )
        monkeypatch.setattr("kiro_crew.dashboard.slack_egress.vet_and_audit", _vet, raising=True)
        monkeypatch.setattr(
            "kiro_crew.dashboard.chat_note_mirror.vet_and_audit", _vet, raising=True
        )
        body = "z" * (tp.capabilities.max_message_chars * 3)

        await _mirror(state, slot, "dashboard:chat-1", body, "note", request_app="app-x")

        assert asks["messaging"] > 1, (
            "`capabilities.messaging` was asked once and never re-asked, so a revocation "
            f"during the send is invisible; asks={asks['messaging']}"
        )
        sent = tp.send_message.await_count if hasattr(tp.send_message, "await_count") else 0
        assert sent < 3, (
            "the note kept posting parts after the app's messaging capability was revoked; "
            f"{sent} parts went out"
        )

    @pytest.mark.asyncio
    async def test_messaging_revoked_mid_send_refuses_an_empty_app_note(
        self, tmp_path, monkeypatch
    ) -> None:
        """The per-part re-ask must not be conditioned on a non-empty app identifier.

        A dashboard-authored note carries NO app, and with an empty app
        `capabilities.messaging` still resolves the per-surface profile and policy ceiling
        -- the enforced path, asked unconditionally by the canonical gate in `mcp_core`.
        Skipping it there exempts the only caller this feature adds from the revalidation.
        """
        state = _make_state(tmp_path)
        state.slack_client = None
        tp = _real_caps_transport("telegram")
        ...
        for target in (
            ...
            "kiro_crew.dashboard.chat_note_mirror.vet_and_audit",
        ):
            monkeypatch.setattr(target, _vet, raising=True)
        body = "z" * (tp.capabilities.max_message_chars * 3)

        await _mirror(state, slot, "dashboard:chat-1", body, "note", request_app="")

        assert asks["messaging"] > 1, (
            "an empty app skipped the per-part `capabilities.messaging` re-ask, so a "
            f"revocation mid-send is invisible to a dashboard note; asks={asks['messaging']}"
        )
        assert tp.send_message.await_count < 3, (
            "the note kept posting parts after messaging was revoked for the surface; "
            f"{tp.send_message.await_count} parts went out"
        )

    @pytest.mark.asyncio
    async def test_an_app_denied_slack_never_reaches_the_dm_open(
        self, tmp_path, monkeypatch
    ) -> None:
        """The FIRST Slack gate carries the app, so no Slack API call precedes the refusal.

        Recipient resolution awaits `open_dm` for the owner-DM basis. An app-blind initial
        gate lets an app denied the Slack `channels` scope clear it, firing that call before
        the later app-aware check refuses -- unauthorized egress, not just a wasted send.
        """
        state = _slack_state(tmp_path)
        _authorize_slack_recipient(monkeypatch, "C123")
        slot = _slot(state, thread="1785370133.085469", channel="C123")
        ...
        def _vet(scope, item, **kw):
            seen.append({"scope": scope, "app": kw.get("app")})
            # Messaging is allowed; the Slack channels scope is denied for this app.
            if scope == "channels" and kw.get("app") == "app-x":
                return SimpleNamespace(permitted=False)
            return SimpleNamespace(permitted=True)
        ...
        await _mirror(
            state, slot, "dashboard:chat-1", "denied for slack", "note", request_app="app-x"
        )

        channels = [c for c in seen if c["scope"] == "channels"]
        assert channels, f"no `channels` gate ran at all; saw {seen}"
        assert channels[0]["app"] == "app-x", (
            "the FIRST Slack channels gate ran app-blind, so an app denied Slack clears it "
            f"and the DM open fires before the app-aware check; channels asks were {channels}"
        )
        assert (
            not state.slack_client.post_message.await_args
        ), "an app denied the Slack channels scope still posted the note"
...
class TestRevocationMidSendStopsRemainingChunks:
    ...
    @pytest.mark.asyncio
    async def test_messaging_revoked_mid_send_stops_an_empty_app_slack_note(
        self, tmp_path, monkeypatch
    ) -> None:
        """The Slack leg's per-chunk re-ask must not be conditioned on a non-empty app.

        `channel_egress_permitted` is asked per chunk, so it is where a mid-send revocation
        is caught. A dashboard note carries no app, and gating the `capabilities.messaging`
        arm on one leaves this leg checking only `channels` for exactly that caller.
        """
        state = _slack_state(tmp_path)
        _authorize_slack_recipient(monkeypatch, "C123")
        slot = _slot(state, thread="1785370133.085469", channel="C123")
        # Revocation is keyed to an observed send, not to an ask ordinal: the capability
        # goes away once chunk 1 is out, whatever number of asks precede it.
        asks = {"messaging": 0}

        def _vet(scope, item, **kw):
            if scope == "capabilities.messaging":
                asks["messaging"] += 1
                return SimpleNamespace(permitted=state.slack_client.post_message.await_count < 1)
            return SimpleNamespace(permitted=True)
        ...
        assert asks["messaging"] > 1, (
            "the Slack leg skipped the per-chunk `capabilities.messaging` re-ask for an "
            f"empty app, so a mid-send revocation cannot stop it; asks={asks['messaging']}"
        )
        assert state.slack_client.post_message.await_count == 1, (
            "Slack kept receiving chunks after messaging was revoked for the surface; "
            f"{state.slack_client.post_message.await_count} posts went out"
        )
    ...
    async def test_transport_unlink_during_the_mid_send_re_resolve_stops_the_rest(
        ...
        def _resolve_then_unlink(st, key, lk, **kwargs):
            calls["n"] += 1
            out = real_resolve(st, key, lk, **kwargs)
            if calls["n"] >= 3:
