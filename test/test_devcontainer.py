...
def _short_bridge_dir(unique: str, tmp_path: Path) -> Path:
    """Gateway-owned MCP bind source for a single test.

    Production keeps this under ``/tmp`` on POSIX because AF_UNIX caps the
    path at ~108 bytes and pytest's tmp_path exceeds that on macOS. Windows
    has no ``/tmp``: ``Path("/tmp")`` becomes a UNC-shaped ``\\tmp\\...``
    and ``Path.write_bytes`` raises FileNotFoundError. ``unique`` must
    differ per test so xdist workers do not share one tree (``home.name``
    is always ``"home"``).
    """
...
def trust_home(tmp_path: Path, monkeypatch: pytest.MonkeyPatch):
    ...
    # The MCP bridge bind lives under /tmp on POSIX (AF_UNIX path length).
    # Keep that directory short and reap it so tests do not leave residue.
...
def test_short_bridge_dir_is_writable_on_this_host(tmp_path: Path) -> None:
    """Windows CI failed here: ``Path("/tmp")/client.tmp`` is not a real file."""
    dest = _short_bridge_dir(tmp_path.name, tmp_path)
    try:
        probe = dest / "client.tmp"
        probe.write_bytes(b"ok")
        assert probe.read_bytes() == b"ok"
        if sys.platform == "win32":
            assert dest == tmp_path / "kc-mb"
        else:
            assert dest.parent == Path("/tmp") / "kc-mb"
    finally:
        if sys.platform != "win32":
...
class TestKillExec:
    ...
    async def test_exec_id_is_interpolated_as_uuid_hex_only(
        ...
        # No spawn path may reintroduce a private EXEC-ID mint. Checked as
        # "nothing assigns an exec_id from a fresh uuid", not "no uuid at all":
        # both spawn paths legitimately mint OTHER ids from uuid4 (a per-spawn
        # _process_instance, the settings-seed owner), so a bare
        # `uuid.uuid4().hex` substring would false-fail on those. The invariant is
        # about the exec id specifically -- it is the value interpolated unquoted
        # into the pidfile path, and it must come from the shared helper.
        exec_mint = re.compile(r"exec_id\s*=\s*uuid\.uuid4\(\)")
        for mod in (acp_client_mod, acp_runtime_mod):
            other = Path(mod.__file__).read_text(encoding="utf-8")
            assert not exec_mint.search(other), (
                f"{mod.__name__} mints its own exec_id; it must come from "
                "devcontainer.containerize_spawn so the injection-safety guarantee "
                "holds on every spawn path"
            )
...
class TestWarmPoolSkipsTrustedDevcontainer:
    ...
    def test_get_or_create_consults_the_trusted_check_before_claim(self) -> None:
        ...
        from kiro_crew import session_allocation as alloc_mod

        # POOL_DECISIONS is still the vocabulary in session.py.
        assert "bypass_devcontainer" in session_mod.POOL_DECISIONS
        # main relocated the pool-decision ladder itself into session_allocation.py
        # (get_or_create is now a thin delegator), so the ordering invariant is
        # asserted against that module: the trusted-devcontainer check must run
        # BEFORE a warm runtime is claimed, or a host-spawned pool runtime would be
        # handed to a work dir that should run in its container.
        src = Path(alloc_mod.__file__).read_text(encoding="utf-8")
        check = src.index("_cwd_has_trusted_devcontainer(cwd)")
        claim = src.index("_drain_and_claim(agent)")
        assert check < claim
