...
class TestDecliningOneBodyDoesNotCostTheCatalog:
    ...
    def test_a_hardlinked_body_stays_searchable(self, tmp_path):
        """The hardened reader refuses a hardlinked inode; search must not regress.

        A global skill body is read unconfined by ``load_skill`` deliberately, so a
        hardlinked SKILL.md was searchable before this index existed. It answers
        from a direct read now, rather than reading as a body with no terms.
        """
        skills = tmp_path / "skills"
        ...
        assert [h["key"] for h in loader.search_skills("linkedneedle")] == ["linked"]
