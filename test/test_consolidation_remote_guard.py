...
async def test_rotation_keeps_pre_transition_rows_behind_the_fence(
    tmp_path,
    monkeypatch: pytest.MonkeyPatch,
    transition: str,
) -> None:
    """Retained private rows stay out of the first local consolidation prompt."""
    log = ConversationLog(base_dir=tmp_path / "sessions")
    log.init()
    _append_rows(log, KEY, 4, prefix="private-row-")
    _make_private(log, transition)

    consolidator = _make_consolidator(log)
    consolidator._call_llm = AsyncMock(return_value={"history_entry": "local summary"})
    assert await consolidator._consolidate(KEY) is None
    consolidator._call_llm.assert_not_awaited()
    assert log.get_metadata(KEY)["last_consolidated"] == 4

    _make_local_again(log, transition)
    _append_rows(log, KEY, 2, prefix="local-row-")

    dropped = _rotate_to_keep(log, 5, monkeypatch)
    assert dropped == 1
    assert log.get_metadata(KEY)["last_consolidated"] == 3

    assert await consolidator._consolidate(KEY) is None
    prompt = consolidator._call_llm.await_args.args[0]
    assert "local-row-0" in prompt and "local-row-1" in prompt
    assert "private-row-" not in prompt
    consolidator._memory.append_history.assert_called_once_with("local summary")
