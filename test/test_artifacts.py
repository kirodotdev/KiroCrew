...
class TestPersistence:
    ...
    def test_recovery_install_is_booked_as_a_content_change(
        ...
        # And a plain metadata-only update with nothing to recover still fires nothing.
        events.clear()
    ...
    def test_recovery_is_booked_even_when_the_triggering_update_conflicts(
        ...
        assert not list(adir.glob(".current.html.*.staged")), "the stage was consumed"
        assert (adir / "current.html").read_text(encoding="utf-8") == "something else"
        assert seen == [("upsert", "k", "something else")], "upsert fired once, after the lock"
