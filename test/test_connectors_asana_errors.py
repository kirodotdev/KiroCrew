...
def test_cross_workspace_denial_is_explicit_raise():
    # The denial is RAISED, never returned: a caller cannot keep it as a value
    # and continue as if the resource were reachable.
    with pytest.raises(CrossWorkspaceDenied) as excinfo:
        raise_cross_workspace_denial(requested_gid="123", token_workspace="900")
    assert excinfo.value.requested_gid == "123"
    assert excinfo.value.token_workspace == "900"
...
def test_cross_workspace_denial_records_observed_status_verbatim_without_asserting_it():
    ...
    with pytest.raises(CrossWorkspaceDenied) as e403:
        raise_cross_workspace_denial(requested_gid="1", token_workspace="9", observed_status=403)
    with pytest.raises(CrossWorkspaceDenied) as e404:
        raise_cross_workspace_denial(requested_gid="1", token_workspace="9", observed_status=404)
...
def test_cross_workspace_denial_status_optional():
    ...
    with pytest.raises(CrossWorkspaceDenied) as excinfo:
        raise_cross_workspace_denial(requested_gid="1", token_workspace="9")
...
def test_cross_workspace_denial_message_states_no_fallback():
    with pytest.raises(CrossWorkspaceDenied) as excinfo:
        raise_cross_workspace_denial(
            requested_gid="123", token_workspace="900", observed_status=404
        )
...
def test_cross_workspace_denial_rejects_empty_gid():
    with pytest.raises(AsanaErrorShapeError):
        raise_cross_workspace_denial(requested_gid="  ", token_workspace="900")
