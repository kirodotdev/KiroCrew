...
class TestCapabilityRow:
    ...
    def test_match_claim_requires_a_named_operation(self):
        # A row cannot claim it matched a manifest operation while naming none:
        # operation_id=null means "no manifest operation matched", so
        # matches_manifest=true beside it is a self-contradiction. Letting it
        # validate would exit 0 and falsely certify a manifest match.
        doc = _good_doc()
        doc["discovery_response"]["capability_rows"] = [
            {
                "operation_id": None,
    ...
    def test_no_match_with_a_named_operation_is_allowed(self):
        # The converse is legitimate: a named operation that did NOT match the
        # manifest shape (matches_manifest=false) is an ordinary probed-but-no-
        # match row, not a contradiction.
        doc = _good_doc()
        doc["discovery_response"]["capability_rows"] = [
            {
                "operation_id": "example.read",
...
class TestSelfTestAndCli:
    ...
    def test_deeply_nested_json_is_usage_error(self, tmp_path):
        # A deeply nested document makes json.load raise RecursionError -- a
        # RuntimeError subclass that escapes OSError / json.JSONDecodeError /
        # ValueError, the same "exception class outside what the handler names"
        # family as the UnicodeDecodeError and NUL cases. It must exit 2, not
        # escape as a traceback. Nesting is built as a raw string so no Python
        # recursion happens before json.load is reached.
        import sys as _sys

        depth = _sys.getrecursionlimit() * 20
        p = tmp_path / "deep.json"
        p.write_text("[" * depth + "]" * depth, encoding="utf-8")
        assert gate.main(["prog", str(p)]) == 2

    @pytest.mark.parametrize("literal", ["NaN", "Infinity", "-Infinity"])
    def test_non_standard_json_constants_are_usage_error(self, tmp_path, literal):
        # CPython's json.load defaults to allow_nan=True, so NaN / Infinity /
        # -Infinity parse without raising even though they are NOT valid JSON
        # (RFC 8259). Such a value would otherwise slip through inside an
        # unchecked array element and let a non-JSON document validate to exit 0,
        # falsely certifying invalid JSON. The reader must reject it as a usage
        # error, not accept it.
        p = tmp_path / "nonstandard.json"
        p.write_text(
            '{"discovery_response": {"scope_snapshot": [' + literal + "]}}",
            encoding="utf-8",
        )
        assert gate.main(["prog", str(p)]) == 2
