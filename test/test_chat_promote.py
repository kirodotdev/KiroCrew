...
# ── regression: promotion preserves persisted non-user/assistant rows ──


@pytest.mark.asyncio
async def test_promote_preserves_persisted_system_rows(tmp_path, monkeypatch) -> None:
    """An ordinary fork keeps only ``user``/``assistant`` rows, but promotion
    copies the WHOLE conversation, so a persisted ``system`` row in the
    ephemeral original (a Stop notice, a compaction card, an error row) must
    survive into the kept session. Dropping it would silently lose transcript
    content, and the ephemeral original is the copy being left behind."""
    monkeypatch.setattr("kiro_crew.dashboard.state.config_dir", lambda: tmp_path)
    state = _make_state(tmp_path)
    slot = state.get_or_create_slot("secret", memory_mode="incognito")
    slot.append("user", "hello", "msg msg-u")
    slot.append("assistant", "hi", "msg msg-a")
    # A Stop between the turns produces a persisted system row, exactly the
    # kind an ordinary fork's user/assistant filter would drop.
    slot.append("system", "Stopped by user.", "msg msg-sys", meta={"kind": "stopped"})
    slot.append("user", "carry on", "msg msg-u")
    ...
    new_slot = state._slots[body["key"]]
    # All five copied rows survive (incl. the Stop system row), then the
    # promotion note. The Stop row keeps its position and content.
    roles = [m["role"] for m in new_slot.messages]
    assert roles == ["user", "assistant", "system", "user", "assistant", "system"]
    stopped = new_slot.messages[2]
    assert stopped["role"] == "system"
    assert stopped["content"] == "Stopped by user."
    assert stopped.get("meta", {}).get("kind") == "stopped"
    # The trailing row is the promotion note, not the copied Stop row.
    assert new_slot.messages[-1].get("meta", {}).get("kind") == "session_promoted"
...
# ── regression: an undurable destination save must not be acknowledged ──


@pytest.mark.asyncio
async def test_promotion_does_not_acknowledge_an_undurable_save(tmp_path, monkeypatch) -> None:
    """Promotion's source is ephemeral, so the destination copy is the only
    durable one. If the destination save fails (e.g. near-full storage), the
    request must NOT return 200 and switch the UI to a session that vanishes on
    restart — the save is confirmed (best_effort=False), so a failure raises and
    the half-made child is cleaned up rather than acknowledged."""
    monkeypatch.setattr("kiro_crew.dashboard.state.config_dir", lambda: tmp_path)
    state = _seed(tmp_path, "secret", "incognito")
    ...
    monkeypatch.setattr("kiro_crew.dashboard.chat_fork.save_slot_off_loop", _boom)

    before = set(state._slots)

    # Snapshot the log's known sessions before the (doomed) promotion so we can
    # prove the rollback left no NEW durable record behind, whatever key the
    # child would have taken.
    def _session_keys() -> set:
        out = set()
        for s in state.conversation_log.list_sessions():
            if isinstance(s, dict):
                out.add(s.get("key") or s.get("history_key") or repr(sorted(s.items())))
            else:
                out.add(s)
        return out
    ...
    # The failed durable save must NOT answer 200. The handler cleans up the
    # half-made child and returns a typed error (promote_save_failed) so the UI
    # can surface it rather than switching to a session that did not land.
    async with TestClient(TestServer(_make_app(state))) as client:
        resp = await client.post("/api/chat/slots/secret/promote", json={})
        status = resp.status
        payload = await resp.json()

    assert status == 500, status
    assert payload.get("code") == "promote_save_failed", payload
    # No half-persisted child left registered, and the ephemeral original stays.
    assert set(state._slots) == before
    assert "secret" in state._slots
    # No ghost transcript: binding the child wrote a durable metadata record
    # before the (failing) save, and rollback must delete it so no metadata-only
    # session lingers in history.
    ghosts = _session_keys() - sessions_before
    assert ghosts == set(), f"aborted promotion left ghost transcript(s): {ghosts}"


@pytest.mark.asyncio
async def test_a_failed_promotion_bind_leaves_no_ghost_transcript(tmp_path, monkeypatch) -> None:
    """The binding step forces the persistent arm for a promotion, so
    ``bind_session_execution`` writes a DURABLE metadata-only transcript record
    BEFORE the post-bind identity re-check. If that check then fails (a
    concurrent agent switch on the source), the rollback must delete that
    durable record too — clearing only the in-memory slot would leave a
    metadata-only ghost session in history."""
    monkeypatch.setattr("kiro_crew.dashboard.state.config_dir", lambda: tmp_path)
    state = _seed(tmp_path, "secret", "incognito")
    ...
    def _bind_then_break(source, child_key, execution, child_mode=None):
        # Perform the REAL bind so the durable metadata-only record lands on
        # disk, then mutate the source slot so the post-bind identity re-check
        # fails — reproducing a concurrent agent switch mid-promotion.
        real_bind(source, child_key, execution, child_mode)
        state._slots["secret"].agent = "switched-mid-promotion"

    monkeypatch.setattr(chat_fork, "_bind_fork_execution", _bind_then_break)

    async with TestClient(TestServer(_make_app(state))) as client:
        resp = await client.post("/api/chat/slots/secret/promote", json={})
        status = resp.status

    # The identity-check failure raises UnknownMemoryStore -> not a 200.
    assert status != 200, status
    # No half-made child registered; the ephemeral original stays.
    assert set(state._slots) == before
    assert "secret" in state._slots
    # And no ghost: the durable record the bind wrote must have been deleted.
    ghosts = _session_keys() - sessions_before
    assert ghosts == set(), f"aborted promotion bind left ghost transcript(s): {ghosts}"


@pytest.mark.asyncio
async def test_an_ordinary_fork_still_tolerates_a_best_effort_save(tmp_path, monkeypatch) -> None:
    """The fail-closed save is promotion-only: an ordinary fork keeps its
    persistent source, so a swallowed best-effort destination save loses
    nothing and must still succeed (periodic flush retries it)."""
    monkeypatch.setattr("kiro_crew.dashboard.state.config_dir", lambda: tmp_path)
    state = _seed(tmp_path, "forkable", None)  # persistent source

    captured = {}

    async def _record(_state, _slot, *_a, best_effort=True, **_k):
        captured["best_effort"] = best_effort
        return True

    monkeypatch.setattr("kiro_crew.dashboard.chat_fork.save_slot_off_loop", _record)

    async with TestClient(TestServer(_make_app(state))) as client:
        resp = await client.post("/api/chat/slots/forkable/fork", json={})
        assert resp.status == 200, await resp.json()
    # An ordinary fork uses the default best-effort save.
    assert captured.get("best_effort") is True
