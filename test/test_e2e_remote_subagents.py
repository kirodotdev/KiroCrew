...
def stack():
    ...
    previous_plaintext = os.environ.get(A2A_PLAINTEXT_ORIGINS_ENV)
    ...
    # Plain HTTP is refused by default; the gateway's ENVIRONMENT (inherited by
    # the child gateway) names the one loopback origin the fake listens on.
    os.environ[A2A_PLAINTEXT_ORIGINS_ENV] = card_origin(sidecar.card_url)
    try:
        ...
        if previous_plaintext is None:
            os.environ.pop(A2A_PLAINTEXT_ORIGINS_ENV, None)
        else:
            os.environ[A2A_PLAINTEXT_ORIGINS_ENV] = previous_plaintext
