...
@pytest.mark.parametrize(
    ...
    [
        ("zh-CN", "zh-CN"),
        (" zh-CN ", "zh-CN"),  # config typo, not a reason to lose the setting
        ("ja", ""),  # tag-shaped but no shipped catalog -> see below
        ("en-XA", ""),  # registered pseudolocale, not restorable in a build
        ("zh-TW", ""),  # regional variant; `resolveLanguage` is exact for stored
...
def test_ui_language_only_accepts_a_shipped_tag(monkeypatch, stored, expected):
    """Shape is necessary but not sufficient: the tag must name a catalog.

    ``ja`` used to be accepted here, on the reasoning that the SPA falls back for
    a code it cannot render so an unrecognised tag is harmless. It is not
    harmless on this path: the sidebar renders in the fallback language while the
    title comes back in the requested one, and the title is persisted, so the
    row stays bilingual (:issue:`1130`). ``context.ui_language_tag`` now answers
    "is this a language this install renders", and the titler inherits that
    because it deliberately shares the one resolver.
    """
