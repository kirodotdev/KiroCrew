...
def _authorize(config_dir, origin="https://api.weather.example", placement=None, return_body=False):
    entry = {"origin": origin, "placement": placement or {"type": "bearer"}}
    if return_body:
        entry["return_body"] = True
...
def test_slow_drip_body_exceeding_the_overall_deadline_is_aborted(tmp_path, monkeypatch):
    # A complicit origin that drips the body — each read inside the per-read
    # socket timeout — must not stretch the transfer past the overall wall-clock
    # budget: the streamed read is held to the same deadline as
    # DNS/connect/redirects, so it aborts with the overall-budget error rather
    # than running to the loopback ceiling (closes the response-timing channel).
    import time as _time

    from kiro_crew.secrets_mediation.dispatch import _read_capped

    class _MultiChunk(_FakeResponse):
        def iter_content(self, chunk_size=65536):
            yield b"first-chunk"
            yield b"second-chunk"

    resp = _MultiChunk(body=b"first-chunksecond-chunk")
    # A deadline already in the past: the very first chunk's deadline check trips.
    past_deadline = _time.monotonic() - 1.0
    with pytest.raises(MediationError, match="overall time budget"):
        _read_capped(resp, past_deadline)


def test_body_within_the_deadline_reads_normally(tmp_path):
    # The deadline guard must not disturb a normal read that finishes in budget.
    import time as _time
    ...
    resp = _MultiChunk()
    _read_capped(resp, _time.monotonic() + 60.0)  # generous deadline
...
def test_unicode_transport_error_becomes_a_secret_free_mediation_error(tmp_path, monkeypatch):
    ...
    _seed_vault(tmp_path)
    _authorize(tmp_path, origin="https://api.weather.example", return_body=True)
    monkeypatch.setattr(
        socket,
        "getaddrinfo",
        lambda h, p, *a, **k: [(socket.AF_INET, socket.SOCK_STREAM, 6, "", ("93.184.216.34", p))],
...
def test_malformed_redirect_location_collapses_to_withheld_not_a_raised_error(
    tmp_path, monkeypatch
):
    # A malformed origin-controlled Location makes urljoin -> urlsplit raise
    # ValueError. On the default (withheld) path that must collapse to the fixed
    # withheld constant, not escape as a raised error / 500 — otherwise an
    # authorized origin could drive an error-vs-withheld oracle by returning a
    # bad Location on a secret-dependent branch.
    _seed_vault(tmp_path)
    _authorize(tmp_path, origin="https://api.weather.example")  # default: return_body False
    cap = []
    # An invalid IPv6 literal in the Location trips urlsplit's ValueError.
    _stub_network(monkeypatch, cap, redirect_to="https://[::bad::]/x")
    resp = perform_mediated_request(
        MediatedRequest(
            secret_name="WEATHER_API_KEY", method="GET", url="https://api.weather.example/redir"
        ),
    ...
    # The withheld constant: sentinel status 0, a fixed message, no upstream data.
    assert resp.status == 0
    assert "owner has not author" in resp.body.lower()
...
def test_secret_absent_from_error_on_transport_failure(tmp_path, monkeypatch):
    _seed_vault(tmp_path)
    _authorize(tmp_path, return_body=True)
    monkeypatch.setattr(
        socket,
        "getaddrinfo",
        lambda h, p, *a, **k: [(socket.AF_INET, socket.SOCK_STREAM, 6, "", ("93.184.216.34", p))],
...
def test_base32_reflected_secret_is_scrubbed(tmp_path, monkeypatch):
    """A reflected credential base32-encoded (RFC 4648) is also scrubbed.

    An echo endpoint that base32-encodes what it received would slip past a
    scrub covering only base64/hex, yet the model could reconstruct the
    plaintext. The scrub covers upper/lower and padding-stripped base32 forms.
    """
    import base64
    ...
    _authorize(tmp_path, return_body=True)
    cap = []
    b32 = base64.b32encode(CANARY.encode()).decode()
    b32_lower = b32.lower()
    b32_nopad = b32.rstrip("=")
    reflected = (
        '{"b32":"' + b32 + '","lower":"' + b32_lower + '","nopad":"' + b32_nopad + '"}'
    ).encode("utf-8")
    _stub_network(monkeypatch, cap, response=_FakeResponse(body=reflected))
    resp = perform_mediated_request(
        MediatedRequest(
    ...
    assert b32 not in resp.body
    assert b32_lower not in resp.body
    assert b32_nopad not in resp.body
...
def test_response_body_is_withheld_by_default(tmp_path, monkeypatch):
    """F2 defense: without an owner ``return_body`` opt-in, the mediated result is
    a CONSTANT independent of all upstream response data (no body, headers, status,
    content-type, or length) — so a credential reflected through ANY channel
    (an unenumerable body/header split, or a metadata covert channel) has nothing
    to ride out on. Fails without the default-off gate."""
    from kiro_crew.secrets_mediation.dispatch import _WITHHELD_STATUS

    _seed_vault(tmp_path)
    _authorize(tmp_path)  # NOT return_body -> everything withheld
    cap = []
    # An adversarial echo splits the credential across fields (defeats scrubbing).
    half = len(CANARY) // 2
    split = ('{"a":"' + CANARY[:half] + '","b":"' + CANARY[half:] + '"}').encode("utf-8")
    _stub_network(monkeypatch, cap, response=_FakeResponse(body=split))
    resp = perform_mediated_request(
        MediatedRequest(
            secret_name="WEATHER_API_KEY", method="GET", url="https://api.weather.example/echo"
        ),
    ...
    # Neither credential fragment (nor any origin bytes) is returned.
    assert CANARY[:half] not in resp.body
    assert CANARY[half:] not in resp.body
    # The result is the fixed constant: withheld status, no headers, fixed message.
    assert resp.status == _WITHHELD_STATUS
    assert resp.headers == {}
    assert "has not authorized returning this origin's response" in resp.body


def test_upstream_headers_are_withheld_by_default(tmp_path, monkeypatch):
    """F2 header channel: without an owner opt-in, NO upstream response data is
    returned — not the headers, not even content-type. A credential the origin
    splits across two allow-listed headers (ETag + Cache-Control) therefore has no
    header channel to escape on. Fails without the default-off gate."""
    _seed_vault(tmp_path)
    _authorize(tmp_path)  # NOT return_body
    cap = []
    half = len(CANARY) // 2
    resp_stub = _FakeResponse(
        body=b'{"ok":true}',
        headers={"etag": CANARY[:half], "cache-control": CANARY[half:]},
    )
    _stub_network(monkeypatch, cap, response=resp_stub)
    resp = perform_mediated_request(
        MediatedRequest(
    ...
    joined = " ".join(resp.headers.values())
    assert CANARY[:half] not in joined
    assert CANARY[half:] not in joined
    # No upstream header value survives at all — headers are empty by default.
    assert resp.headers == {}


def test_post_send_outcomes_collapse_to_the_same_constant_by_default(tmp_path, monkeypatch):
    """F1 oracle defense: on the default (no return_body) path, an outcome the
    origin can steer AFTER the secret is sent — a cross-origin redirect refusal, a
    transport error — must return the SAME fixed constant a success returns, so
    the agent cannot distinguish error from success and read a secret bit out of
    that channel. Fails without the post-send collapse."""
    import requests
    ...
    # Baseline: a plain success returns the constant.
    cap0 = []
    ...
    # A cross-origin redirect (post-send refusal) must look identical.
    cap1 = []
    ...
    # A transport error (post-send) must look identical too.
    monkeypatch.setattr(
        socket,
        "getaddrinfo",
        lambda h, p, *a, **k: [(socket.AF_INET, socket.SOCK_STREAM, 6, "", ("93.184.216.34", p))],
    ...
    # All three are the identical fixed constant — no error-vs-success oracle.
    for r in (ok, redir, err):
        assert r.status == _WITHHELD_STATUS
        assert r.headers == {}
        assert "has not authorized returning this origin's response" in r.body
    assert ok.body == redir.body == err.body


def test_owner_opt_in_returns_the_body(tmp_path, monkeypatch):
    """With ``return_body: true`` in the SIGNED policy, the owner has accepted the
    residual reflection risk for that origin and the (scrubbed) body is returned."""
    _seed_vault(tmp_path)
    _authorize(tmp_path, return_body=True)
    cap = []
    _stub_network(monkeypatch, cap, response=_FakeResponse(body=b'{"temp":"22C","city":"Seattle"}'))
    resp = perform_mediated_request(
        MediatedRequest(
            secret_name="WEATHER_API_KEY", method="GET", url="https://api.weather.example/w"
        ),
        tmp_path,
    )
    assert "Seattle" in resp.body
    assert "response body withheld" not in resp.body


def test_non_boolean_return_body_is_rejected(tmp_path):
    """A malformed non-boolean ``return_body`` in the policy fails closed at load."""
    from kiro_crew.secrets_mediation.policy import PolicyError, load_authorization
