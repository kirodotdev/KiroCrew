...
def test_completion_hook_runs_under_the_pin_and_failure_retains_the_receipt(kernel, monkeypatch):
    observed = []
    root_handles = {100: 1000, 300: 3000}
    kernel.identities[3000] = (300, 30, None)

    def on_complete(root_pid):
        # Check the HANDLE, not its numerically different PID.
        observed.append((root_pid, root_handles[root_pid] not in kernel.closed))
        if root_pid == 100:
            raise RuntimeError("transient completion-hook/write failure")
    ...
    for root_handle in root_handles.values():
        with pytest.raises(OSError, match="temporary discovery refusal"):
            pc.terminate_windows_process_tree_owned(root_handle)
    refuse_scan[0] = False

    # A failed metadata write retains the pin but rotates behind healthy peers.
    assert pc.retry_pending_windows_process_trees(limit=1, on_complete=on_complete) == ()
    assert observed == [(100, True)], "hook ran after close, or the root was not pinned"
    assert kernel.closed == [], "a failed retirement must not close (unpin) any handle"
    assert list(pc._PENDING_WINDOWS_TREE_CLEANUPS) == [(300, 30), (100, 10)]
    assert pc.retry_pending_windows_process_trees(limit=1, on_complete=on_complete) == (300,)
    assert observed == [(100, True), (300, True)]
    assert kernel.closed == [3000], "metadata failure starved a healthy peer"

    # The failed receipt is still usable; a successful retry closes it once.
    succeeded = []
    assert pc.retry_pending_windows_process_trees(
        on_complete=lambda pid: succeeded.append(root_handles[pid] not in kernel.closed)
    ) == (100,)
    assert succeeded == [True], "the root was already closed before the successful hook"
    assert sorted(kernel.closed) == [1000, 2000, 3000]
    assert pc._PENDING_WINDOWS_TREE_CLEANUPS == {}
