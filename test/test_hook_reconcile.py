...
@pytest.mark.asyncio
async def test_a_respawn_that_raises_is_held_by_the_same_backoff(_harness, monkeypatch):
    """``start_app_backend`` can raise (a governance evaluation error propagates
    by design). That is an attempt like a returned None: recorded, so the next
    ticks do not re-attempt and log it every 15s until the window elapses."""
    _, (set_current, _, _) = _harness
    app = _plain_backend_app("cost-ai")
    set_current(app)
    _BACKEND["process"] = _spawned()
    ...
    def raising_start(name):
        _BACKEND["lifecycle"].append("start-raised")
        raise RuntimeError("governance evaluation error: evaluator unavailable")

    monkeypatch.setattr(hr, "start_app_backend", raising_start)
    clock = {"t": 1000.0}
    monkeypatch.setattr(hr.time, "monotonic", lambda: clock["t"])

    await hr.reconcile_once([app])
    assert _BACKEND["lifecycle"] == ["stop", "start-raised"]
    attempted_at, _identity = hr._pending_respawn["cost-ai"]
    assert attempted_at == 1000.0  # the raise was recorded as an attempt

    # Inside the window: not re-attempted, even though a start would now succeed.
    monkeypatch.setattr(hr, "start_app_backend", real_start)
    clock["t"] += 1.0
    await hr.reconcile_once([app])
    assert _BACKEND["lifecycle"] == ["stop", "start-raised"]
    ...
    clock["t"] += hr.START_RETRY_BACKOFF_SECS
    await hr.reconcile_once([app])
    assert _BACKEND["lifecycle"] == ["stop", "start-raised", "start"]
    assert "cost-ai" not in hr._pending_respawn


@pytest.mark.asyncio
async def test_the_proxy_sees_the_replacement_in_flight_from_the_stop_to_the_start(
    _harness, monkeypatch
):
    """``stop_app_backend`` pops the record before the child has exited, so for
    that window no generation check has anything to judge. The reconciler marks
    the app as mid-replacement before the stop and until the start returns, and
    the proxy refuses the name meanwhile (see the routes tests)."""
    _, (set_current, _, _) = _harness
    app = _plain_backend_app("cost-ai")
    set_current(app)
    _BACKEND["process"] = _spawned()
    ...
    def observed_stop(name):
        seen.append(("stop", backend_mod.backend_replacement_in_flight(name)))
        return real_stop(name)

    def observed_start(name):
        seen.append(("start", backend_mod.backend_replacement_in_flight(name)))
        return real_start(name)

    monkeypatch.setattr(hr, "stop_app_backend", observed_stop)
    monkeypatch.setattr(hr, "start_app_backend", observed_start)
    assert not backend_mod.backend_replacement_in_flight("cost-ai")
...
@pytest.mark.asyncio
async def test_the_in_flight_mark_is_cleared_when_the_replacement_raises(_harness, monkeypatch):
    """A raise anywhere inside the replacement must not leave the app refused
    forever: the mark is released on the way out."""
    _, (set_current, _, _) = _harness
    app = _plain_backend_app("cost-ai")
    set_current(app)
    _BACKEND["process"] = _spawned()
    ...
    monkeypatch.setattr(hr, "stop_app_backend", exploding_stop)

    await hr.reconcile_once([app])  # the loop's guard swallows the error

    assert not backend_mod.backend_replacement_in_flight("cost-ai")


@pytest.mark.asyncio
async def test_a_later_tick_respawn_is_also_marked_in_flight(_harness, monkeypatch):
    """The start half of a replacement that a previous tick began is marked
    too, so a fixed-port app is refused rather than resolved by name while the
    replacement spawns."""
    _, (set_current, _, _) = _harness
    app = _plain_backend_app("cost-ai")
    set_current(app)
    _BACKEND["process"] = None
    hr._pending_respawn["cost-ai"] = (None, None)  # owed from an earlier tick
    seen: list[bool] = []
    real_start = hr.start_app_backend

    def observed_start(name):
        seen.append(backend_mod.backend_replacement_in_flight(name))
        return real_start(name)
    ...
    assert seen == [True] and _BACKEND["lifecycle"] == ["start"]
    assert not backend_mod.backend_replacement_in_flight("cost-ai")
...
async def test_a_disable_landing_before_the_respawn_skips_it(_harness, monkeypatch):
    ...
    assert _BACKEND["lifecycle"] == ["stop"]
    assert "cost-ai" not in hr._pending_respawn  # positively disabled: nothing owed


@pytest.mark.asyncio
async def test_a_transient_unknown_enablement_after_the_stop_does_not_strand_the_app(
    _harness, monkeypatch
):
    """The old child is stopped, then the enablement re-read answers ``None``
    (unreadable metadata, a write in flight). That is not a decision: the
    replacement stays owed and the next tick finishes it. Treating the unknown
    as terminal would leave an enabled app whose process the reconciler just
    stopped down until a gateway restart."""
    _, (set_current, _, _) = _harness
    app = _plain_backend_app("cost-ai")
    set_current(app)
    _BACKEND["process"] = _spawned()
    _BACKEND["generation"] = False
    real_enabled = hr.app_enabled_state
    reads = {"n": 0}

    def flaky_enabled(name):
        reads["n"] += 1
        return None if reads["n"] == 1 else real_enabled(name)  # unknown once, then True

    monkeypatch.setattr(hr, "app_enabled_state", flaky_enabled)

    await hr.reconcile_once([app])
    assert _BACKEND["lifecycle"] == ["stop"]  # stopped, respawn deferred
    assert hr._pending_respawn.get("cost-ai") == (None, None)  # owed, not yet attempted
...
@pytest.mark.asyncio
async def test_a_shutdown_beginning_between_stop_and_respawn_keeps_the_debt(_harness, monkeypatch):
    """Nothing is spawned once shutdown began, but the stopped child's
    replacement stays owed for the same reason as above."""
    _, (set_current, _, _) = _harness
    app = _plain_backend_app("cost-ai")
    set_current(app)
    _BACKEND["process"] = _spawned()
    ...
    def stop_then_shutdown(name):
        hr._stopping = True
        return real_stop(name)

    monkeypatch.setattr(hr, "stop_app_backend", stop_then_shutdown)

    await hr.reconcile_once([app])

    assert _BACKEND["lifecycle"] == ["stop"]
    assert "cost-ai" in hr._pending_respawn
