...
async def test_flush_rechecks_opt_in_at_send_time(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    fp = heartbeats._destination_fingerprint(_Cfg())
    await heartbeats._flush([(fp, heartbeats._make_heartbeat("/tmp/repo"))])
    assert stub.batches == []


@pytest.mark.asyncio
async def test_flush_drops_rows_whose_destination_changed(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    # A row buffered for one account must NOT be posted to another after the
    # destination fingerprint changes between enqueue and flush.
    stub = _StubClient()
    monkeypatch.setattr(heartbeats, "build_client", lambda cfg: stub)
    monkeypatch.setattr(heartbeats.KiroCrewConfig, "load", staticmethod(lambda: _Cfg()))
    stale = [("stale-account-fingerprint", heartbeats._make_heartbeat("/tmp/repo"))]
    await heartbeats._flush(stale)
    assert stub.batches == []
...
async def test_flush_chunks_to_the_api_cap(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(heartbeats.KiroCrewConfig, "load", staticmethod(lambda: _Cfg()))
    fp = heartbeats._destination_fingerprint(_Cfg())
    batch = [(fp, heartbeats._make_heartbeat("/tmp/repo")) for _ in range(60)]
