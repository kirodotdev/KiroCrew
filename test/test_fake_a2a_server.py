...
from kiro_crew.agent_sdk.drivers.a2a import card_origin
from kiro_crew.providers.a2a import A2A_PLAINTEXT_ORIGINS_ENV, A2AProvider, A2AStreamError
...
async def _fake():
    ...
    # The fake is plain HTTP; the provider refuses that unless the environment
    # names the exact loopback origin (what the E2E harness and the GUI boot do).
    previous = os.environ.get(A2A_PLAINTEXT_ORIGINS_ENV)
    os.environ[A2A_PLAINTEXT_ORIGINS_ENV] = card_origin(card_url)
    try:
        yield card_url, server
    finally:
        if previous is None:
            os.environ.pop(A2A_PLAINTEXT_ORIGINS_ENV, None)
        else:
            os.environ[A2A_PLAINTEXT_ORIGINS_ENV] = previous
...
class TestFailureDiscipline:
    ...
    async def test_revised_artifact_replaces_the_draft_on_the_wire(self):
        """``append: false`` replaces the artifact: the draft streams live as it
        arrives, the revision is delivered once at the end, and the record
        (EVENT_COMPLETE text) is the revision alone -- never draft + revision."""
        async with _fake() as (card_url, _):
            p = await _provider(card_url)
            try:
                text, events = await _drain(p, "[[REVISE]] anything")
                assert text.endswith(fake_a2a_server.REVISED_TEXT)
                assert text.count("DRAFT") == 1 and text.count("FINAL") == 1
                assert events[-1].kind == EVENT_COMPLETE
                assert events[-1].text == fake_a2a_server.REVISED_TEXT
            finally:
                await p.shutdown()
    ...
    async def test_terminal_task_snapshot_completes_with_its_inline_artifacts(self):
        """[[SNAPSHOT]]: the reference SDK's ``completed_task()`` shape -- no
        artifact or status events, one terminal Task carrying the result. The
        turn completes with that text; it is not a lost connection."""
        async with _fake() as (card_url, _):
            p = await _provider(card_url)
            try:
                text, events = await _drain(p, "[[SNAPSHOT]] anything")
                assert text == fake_a2a_server.SNAPSHOT_TEXT
                assert events[-1].kind == EVENT_COMPLETE
                assert events[-1].text == fake_a2a_server.SNAPSHOT_TEXT
            finally:
                await p.shutdown()
...
class TestBearerOnTheWire:
    ...
    async def test_bearer_configured_client_is_admitted(self, monkeypatch):
        ...
        monkeypatch.setenv(A2A_PLAINTEXT_ORIGINS_ENV, card_origin(card_url))
        try:
    ...
    async def test_edition_registered_scheme_is_what_the_provider_sends(self, monkeypatch):
    ...
    async def test_unconfigured_client_cannot_read_a_protected_card(self, monkeypatch):
        runner, card_url, server = await fake_a2a_server.serve(port=0, require_bearer="s3cret-1")
        monkeypatch.setenv(A2A_PLAINTEXT_ORIGINS_ENV, card_origin(card_url))
        try:
