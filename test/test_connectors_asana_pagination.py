...
def test_parse_next_page_reads_cursor():
    ...
    # path/uri from the envelope are intentionally NOT modeled; only the opaque
    # offset cursor drives continuation.
    assert not hasattr(cursor, "path")
    assert not hasattr(cursor, "uri")
