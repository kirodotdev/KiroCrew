"""Deterministic, offline fake Kiro Crew gateway for the ACP closed-box gate.
...
(:class:`kiro_crew.acp_server.http_backend.HttpGatewayBackend`). The closed-box
...
class FakeGatewayState:
    ...
    def __init__(self) -> None:
        ...
        self.project_generations: dict[str, str] = {}
        self._project_generation_seq = 0
    ...
    class Handler(BaseHTTPRequestHandler):
        ...
        def do_POST(self) -> None:  # noqa: N802 (BaseHTTPRequestHandler override)
            ...
            if path.endswith("/project"):
                ...
                with state.lock:
                    previous_project = state.projects.get(slot, "")
                    current_generation = state.project_generations.get(slot, "")
                    ...
                    if (
                        ...
                        return
                    state._project_generation_seq += 1
                    generation = f"generation-{state._project_generation_seq}"
                    project = str(body.get("project", ""))
                    state.projects[slot] = project
                    state.project_generations[slot] = generation
