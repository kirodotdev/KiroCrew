...
class TestBuildArchivedDist:
    ...
    def test_unexecutable_npm_refuses_instead_of_raising(self, tmp_path, monkeypatch):
        """A resolvable-but-unrunnable npm must refuse, not raise.

        ``build_stock_frontend`` reports a MISSING npm by returning None, but
        the spawn itself is unguarded: an npm that resolves via ``shutil.which``
        yet cannot be executed (non-executable, broken symlink, unlinked between
        the check and the spawn, EPERM) raises OSError. That has to become an
        ``_AbortInjection`` so the caller ships the original archive and the box
        builds its own frontend -- otherwise the launch dies with a traceback,
        which is exactly what the fallback exists to prevent.
        """
        archive = self._archive(
            tmp_path, {"website/package-lock.json": b'{"lockfileVersion": 3}\n'}
        )
        # A real file that resolves but cannot be executed -> Popen raises.
        fake_npm = tmp_path / "npm"
        fake_npm.write_text("not executable\n")
        fake_npm.chmod(0o644)
        monkeypatch.setattr("shutil.which", lambda name, **kw: str(fake_npm))

        with pytest.raises(source._AbortInjection, match="could not run the frontend build"):
            source._build_archived_dist(archive)
...
class TestInjectDist:
    ...
    @pytest.mark.parametrize(
        "run",
        [
            "wJalrXUtnFEMI/K7MDENG/bPxRfiCYEXAMPLEKEYA",
            "XwJalrXUtnFEMI/K7MDENG/bPxRfiCYEXAMPLEKEY",
            "wJalrXUtnFEMI/K7MDENG/bPxRfiCYEXAMPLEKEYABC",
            "wJalrXUtnFEMI/K7MDENG/bPxRfiCYEXAMPLEKEYXwJalrXUtnFEMI/K7MDENG/bPxRfiCYEXAMPLEKEY",
        ],
        ids=["trailing", "leading", "trailing-multi", "two-keys"],
    )
    def test_glued_bare_aws_secret_in_text_asset_aborts_injection(self, tmp_path, run):
        """A real secret glued to adjacent base64 chars must not ship.

        ``security._contains_bare_secret`` exists to catch exactly this: a
        genuine 40-char key with no delimiter on one side reports a 41+ char
        run. An admission gate that only acted on the exact ``(40 chars)``
        warning discarded that signal and shipped the key verbatim into an
        unauthenticated ``/assets/`` file.
        """
        dist = self._make_dist(tmp_path)
        (dist / "assets" / "config.js").write_text(f'window.s="{run}";\n')
        archive = self._make_archive(tmp_path, ["install.sh"])
        original = archive.read_bytes()
    ...
    def test_data_uri_payload_cannot_hide_a_credential_elsewhere(self, tmp_path):
        """Masking a data: URI must not blind the scan to the rest of the file."""
        secret = "wJalrXUtnFEMI/K7MDENG/bPxRfiCYEXAMPLEKEY"
        dist = self._make_dist(tmp_path)
        (dist / "assets" / "editor.css").write_text(
            '.i{background:url("data:image/png;base64,iVBORw0KGgoAAAANSUhEUg")}\n'
            f'.j{{content:"{secret}"}}\n'
        )
        archive = self._make_archive(tmp_path, ["install.sh"])
    ...
    def test_bare_secret_parked_as_a_data_uri_payload_aborts(self, tmp_path, payload):
        """A key where a payload goes is not media, so it is not exempt.

        The exemption exists for genuinely inlined assets, which decode to a
        recognized container. Keying it on POSITION instead of CONTENT would
        turn ``data:image/png;base64,`` into a laundering prefix: writing the
        raw key there would discard the bare-secret warning and ship it.
        """
        dist = self._make_dist(tmp_path)
        (dist / "assets" / "editor.css").write_text(
            f'.i{{background:url("data:image/png;base64,{payload}")}}\n'
        )
        archive = self._make_archive(tmp_path, ["install.sh"])
        original = archive.read_bytes()
    ...
    def test_encoded_credential_inside_data_uri_aborts_injection(self, tmp_path):
        """The data-URI exemption covers the entropy heuristic, not real keys.

        Only the generic bare-secret heuristic is exempted inside a ``data:``
        payload, because that is the one check built assets false-positive on.
        A distinctive credential base64'd into an inlined image must still
        abort -- otherwise the exemption would be a laundering channel.
        """
        payload = base64.b64encode(b'{"accessKey":"AKIAIOSFODNN7EXAMPLE"}').decode()
        dist = self._make_dist(tmp_path)
        (dist / "assets" / "editor.css").write_text(
            f'.i{{background:url("data:image/png;base64,{payload}")}}\n'
        )
