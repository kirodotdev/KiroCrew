...
def _home_env(home: Path) -> dict[str, str]:
    """Env that moves the sensitive-path anchor on EVERY platform.

    ``is_sensitive_path`` anchors on ``Path.home()``, which reads ``USERPROFILE``
    on Windows and never ``HOME``. Patching ``HOME`` alone left the anchor on the
    runner's real profile, so a planted ``~/.ssh`` under a temp dir was not
    sensitive there and the test asserted the opposite of what it meant to.
    """
...
class TestResolveCheckout(unittest.TestCase):
    ...
    def test_a_git_symlinked_at_a_credential_file_is_not_read(self):
        ...
        try:
            ...
            # Creating a symlink needs a privilege the runner may not hold
            # (Windows without developer mode). Skipping is honest: the scenario
            # this asserts cannot be built here.
            self.skipTest(f"cannot create a symlink on this host: {exc}")

        with mock.patch.dict(os.environ, _home_env(home)):
            self.assertIsNone(dispatch.resolve_checkout(str(root)))
...
class TestConfigIsWriteProtected(unittest.TestCase):
    ...
    def test_replacing_the_app_directory_is_refused_too(self):
        """A rule that stops at ``data`` still leaves the store replaceable one
        level up: renaming ``apps/issue-radar`` aside and moving a prepared tree
        into place never names ``data``, and the matcher is at-or-below. The rule
        therefore sits on the APP ROOT, above every directory the store lives
        under, so it holds for paths the app adds later too."""
        from kiro_crew.security import is_sensitive_bash_command, is_sensitive_write_path

        for command in (
            "mv ~/.kiro/crew/apps/issue-radar ~/.kiro/crew/apps/issue-radar.bak",
            "mv /tmp/prepared ~/.kiro/crew/apps/issue-radar",
            "rm -rf ~/.kirocrew/apps/issue-radar",
            # A sibling of ``data`` the app has not created yet: naming the app
            # root means a path added later is covered without another edit here.
            'echo x > "$HOME/.kiro/crew/apps/issue-radar/state.json"',
        ):
            with self.subTest(command=command):
                self.assertIsNotNone(is_sensitive_bash_command(command))

        home = os.path.expanduser("~")
        self.assertTrue(is_sensitive_write_path(f"{home}/.kiro/crew/apps/issue-radar"))

    def test_shell_default_value_expansions_are_refused(self):
        """``${KIROCREW_HOME:-$HOME/.kiro/crew}`` is the documented way to name the
        data home -- it is the spelling this repo's own scripts use -- and the body
        of a default expansion ends in ``}``, so a suffix anchored on ``/`` never
        lines up. Every brace and operator form has to reach the same fence."""
        from kiro_crew.security import is_sensitive_bash_command

        leaf = "apps/issue-radar/data/config.json"
        for command in (
            f'echo x > "${{KIROCREW_HOME:-$HOME/.kiro/crew}}/{leaf}"',
            f'echo x > "${{KIROCREW_HOME:-~/.kiro/crew}}/{leaf}"',
            f'tee "${{KIROCREW_HOME:-$HOME/.kirocrew}}/{leaf}"',
            # The colon-less and assigning operators expand identically.
            f'echo x > "${{KIROCREW_HOME-$HOME/.kiro/crew}}/{leaf}"',
            f'echo x > "${{KIROCREW_HOME:=$HOME/.kiro/crew}}/{leaf}"',
            # The default operator on HOME itself, which the crew prefix follows.
            f'echo x > "${{HOME:-/root}}/.kiro/crew/{leaf}"',
            # Replacing the app root through the same spelling.
            'mv /tmp/p "${KIROCREW_HOME:-$HOME/.kiro/crew}/apps/issue-radar"',
        ):
            with self.subTest(command=command):
                self.assertIsNotNone(is_sensitive_bash_command(command))

        # A read of Kiro Crew's own config stays allowed through the same spelling:
        # the loader clamps it, so only the tool-edit gate needs to refuse it.
        self.assertIsNone(
            is_sensitive_bash_command('cat "${KIROCREW_HOME:-$HOME/.kiro/crew}/config.json"')
        )

    def test_windows_native_spellings_are_refused(self):
        """The module's Windows raw-text branch was keyed to ``_SENSITIVE_HOME_DIRS``
        alone, so the crew-home write-protected leaves had no native coverage: a
        backslash spelling reached the fenced file while the POSIX spelling of the
        same write was refused, and the file-edit gate refused both. Asserted on
        every platform -- the branch reads the RAW command text, so a Windows
        spelling emitted from a POSIX host is refused too."""
        from kiro_crew.security import is_sensitive_bash_command

        for command in (
            r'echo x > "C:\Users\u\.kiro\crew\apps\issue-radar\data\config.json"',
            r'move C:\prepared "C:\Users\u\.kiro\crew\apps\issue-radar"',
            r'echo x > "%USERPROFILE%\.kiro\crew\apps\issue-radar\data\config.json"',
            r'echo x > "%USERPROFILE:~0%\.kiro\crew\apps\issue-radar\x"',
            r"Set-Content $env:USERPROFILE\.kiro\crew\apps\issue-radar\data\config.json",
            r'echo x > "%KIROCREW_HOME%\apps\issue-radar\data\config.json"',
            r"Set-Content $env:KIROCREW_HOME\apps\issue-radar\data\config.json",
            r'echo x > "C:\Users\u\.kirocrew\apps\issue-radar\data\config.json"',
            r'echo x > "C:\Users\u\.kiro\crew\.\apps\issue-radar\data\config.json"',
            # An embedded interpreter literal, which never tokenizes into a path.
            r"""python -c "open(r'C:\Users\u\.kiro\crew\apps\issue-radar\data\config.json','w')" """,
            # The leaves that inherited the same gap.
            r'echo x > "C:\Users\u\.kiro\crew\.data-home-ready"',
            r'echo x > "C:\Users\u\.kiro\crew\apps\ops-mission-control\data\rotation.yaml"',
        ):
            with self.subTest(command=command):
                self.assertIsNotNone(is_sensitive_bash_command(command))

        # Not over-matching: the crew config stays readable, and an ordinary
        # Windows workspace path is untouched.
        self.assertIsNone(is_sensitive_bash_command(r'type "C:\Users\u\.kiro\crew\config.json"'))
        self.assertIsNone(is_sensitive_bash_command(r'echo x > "C:\Users\u\projects\notes.txt"'))

    def test_equivalent_path_spellings_are_refused_too(self):
        """The shell regex is literal, so a dot segment walks straight through it:
        the plain path was blocked while ``.../crew/./apps/issue-radar/data/...``
        and a ``x/..`` hop were allowed. Normalization is the only layer that can
        decide path equivalence, and it consulted the read+write set alone, so a
        write-protected leaf had no normalized check at all.

        Asserted for the PRE-EXISTING leaves as well, because the gap was
        inherited -- covering only this app's entry would leave the data-home
        marker and the Ops Mission Control leaves bypassable the same way."""
        from kiro_crew.security import is_sensitive_bash_command

        for command in (
            'echo x > "$HOME/.kiro/crew/./apps/issue-radar/data/config.json"',
            'echo x > "$HOME/.kiro/crew/apps/./issue-radar/data/config.json"',
            'echo x > "$HOME/.kiro/crew/apps/x/../issue-radar/data/config.json"',
            'echo x > "$HOME/.kiro/crew/./.data-home-ready"',
            'echo x > "$HOME/.kiro/crew/./apps/ops-mission-control/data/rotation.yaml"',
        ):
            with self.subTest(command=command):
                self.assertIsNotNone(is_sensitive_bash_command(command))

    def test_the_crew_config_stays_readable_from_bash(self):
        """The normalized check covers the BASH-leaf set, not every write-protected
        path. Kiro Crew's own ``config.json`` is write-protected but deliberately
        readable from a shell -- the loader clamps any inflated value, so only the
        tool-edit gate needs it. Widening the normalizer to the whole set would
        start refusing this."""
        from kiro_crew.security import is_sensitive_bash_command

        self.assertIsNone(is_sensitive_bash_command('cat "$HOME/.kiro/crew/config.json"'))
    ...
    def test_a_custom_data_home_is_anchored_on_the_shell_gate(self):
        """The shell half of the same question. Skipped on Windows because the
        command this builds is POSIX-shaped (a ``/``-separated temp dir behind a
        ``>`` redirect); the native spellings of the same leaves have their own
        coverage in ``test_windows_native_spellings_are_refused``."""
        from kiro_crew import security as security_mod
