...
pid-incarnation contract — an identity every consumer (sweep entries, the
signed mapping, gatewayd claims, metrics crumbs) picks up with zero changes.
"""
...
def test_zero_creation_filetime_is_unknown_not_a_token(monkeypatch):
    ...
    def _zero_creation_times(_handle, creation_ref, exit_ref, _k, _u) -> int:
        creation_ref._obj.dwHighDateTime = 0
        creation_ref._obj.dwLowDateTime = 0
        exit_ref._obj.dwHighDateTime = 0
        exit_ref._obj.dwLowDateTime = 0
        return 1

    def _exited(_handle, out) -> int:
        out._obj.value = 0
        return 1

    kernel32 = types.SimpleNamespace(
        GetProcessId=lambda _handle: 4242,
        GetProcessTimes=_zero_creation_times,
        GetExitCodeProcess=_exited,
    )
    monkeypatch.setattr(pc, "_open_process_query_handle", lambda _pid: 5)
    monkeypatch.setattr(pc, "_close_process_handle", lambda _handle: None)
    monkeypatch.setattr(pc.ctypes, "WinDLL", lambda _name, **_kwargs: kernel32)
