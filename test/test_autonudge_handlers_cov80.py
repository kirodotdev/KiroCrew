...
async def _scheduled_delete_fixture(
    ...
    """Create a real scheduled row with the requested process-memory state."""
    ...
    autonudge_selfarm.delete_scheduled_message_record(record_id)
    if provenance == "valid":
        autonudge_selfarm.record_scheduled_message(
            record_id, loop.slot_key, "send later", loop.scheduled_at
        )
    elif provenance == "foreign_slot":
        autonudge_selfarm.record_scheduled_message(
            record_id, "chat-9-999", "not yours", loop.scheduled_at
        )
    elif provenance != "missing":  # pragma: no cover - parametrization guard
...
@pytest.mark.asyncio
@pytest.mark.parametrize("provenance", ("missing", "foreign_slot"))
