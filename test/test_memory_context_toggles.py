...
class TestValidationPathIgnoresInjectionConfig:
    ...
    The builder is driven past ``member_for_store`` with a stubbed owner, since
    an unresolvable store returns before the scoping decision is ever made —
    ...
    def _scoping_calls(self, tmp_path, monkeypatch, **kwargs) -> list[object]:
        ...
        calls: list[object] = []
        monkeypatch.setattr(
            "kiro_crew.member_essential_context.member_for_store",
            lambda store, member: ("alice", "template"),
        )
        monkeypatch.setattr(
