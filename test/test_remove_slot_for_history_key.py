...
from kiro_crew.dashboard.handlers import (
    _remove_slot_for_history_key as _remove_slot_for_history_key_impl,
)
from kiro_crew.dashboard.handlers import (
    api_session_delete,
    api_sessions_clear,
...
def _make_state(slots: dict) -> MagicMock:
    ...
    # What a real destroy leaves behind: it removed this key's mapping, and no other
    # key points at that session id. A bare MagicMock would hand back a truthy Mock
    # here, which reads as "another holder still maps this id" and vetoes every
    # ledger removal -- so the default has to state the ordinary case explicitly.
    # The duplicate-mapping case sets its own return value.
    state.sessions.find_key_by_sid = MagicMock(return_value=None)
...
async def _remove_slot_for_history_key(
    ...
    *,
    ...
    """Drive teardown with the same exact owner claim production captures."""
    claim = delete_claim or _capture_history_delete_claim(state, key)
    owner_keys = set(claim.cron_owner_keys)
    owner_keys.update(owner for owner in exact_owner_keys if owner)
    crons = getattr(state, "crons", None)
    if crons is not None and not exact_owner_keys:
        swept = await sessions_module._owner_keys_bound_to_transcript(crons, (key,))
        owner_keys.update(swept.get(key, ()))
    if owner_keys != set(claim.cron_owner_keys):
        claim = replace(claim, cron_owner_keys=frozenset(owner_keys))
    await _remove_slot_for_history_key_impl(state, key, delete_claim=claim)
...
class TestRemoveSlotForHistoryKey:
    ...
    def test_locked_route_capture_binds_exact_runtime_owner(self, tmp_path):
        key = "dashboard_chat-3-300"
        slot = _make_slot(key)
        state = _make_state({key: slot})
        claim = _capture_history_delete_claim(state, key)
        log = ConversationLog(base_dir=tmp_path)
        log.append(key, "user", "hello")

        deleted, resolved_claim = _delete_history_session(log, key, claim)

        assert deleted is True
        assert resolved_claim.cron_owner_keys == frozenset({"dashboard:chat-3-300"})
...
class TestPermanentDeleteReleasesCronOwnership:
    ...
    @pytest.mark.asyncio
    async def test_slotless_live_manager_session_keeps_its_job_owned(self, tmp_path):
        owner_key = "slack:1785370133.085469"
        crons = self._service(tmp_path)
        job = crons.add_job("channel", "ping", every_secs=3600, session_key=owner_key)
        state = _make_state({})
        state.crons = crons
        state.sessions.session_keys.return_value = frozenset({owner_key})

        await _remove_slot_for_history_key(
            state,
            "slack_1785370133.085469",
            exact_owner_keys=(owner_key,),
        )

        assert CronService(base_dir=tmp_path).get_job(job.id).session_key == owner_key
...
class TestAFailedRemovalDoesNotLeakIntoTheNextSave:
    ...
    def test_malformed_persisted_owner_does_not_poison_unrelated_removal(self, tmp_path):
        ...
        setattr(malformed, "session_key", ["not", "a", "string"])
        with crons._file_lock():
            crons._save()

        worker = CronService(base_dir=tmp_path)
        assert worker.remove_job(parent.id, actor="cli", source="cli") is True
...
class TestAJobBornDuringTheDeleteIsNotStranded:
    ...
    async def test_an_add_between_the_sweep_and_the_unlink_ends_ownerless(
        ...
        real_delete = sessions_module._delete_history_session
        born: dict[str, str] = {}

        def _add_then_delete(
            log_arg,
            key,
            claim,
            *,
...
class TestOnlyASTABLELiveKeyRetainsOwnership:
    ...
    @pytest.mark.asyncio
    async def test_old_stateless_run_keeps_every_live_runs_child_owned(self, tmp_path):
        crons = CronService(base_dir=tmp_path)
        parent = crons.add_job(
            "stateless-nightly", "sweep", every_secs=3600, persistent_session=False
        )
        deleted_key = f"cron:{parent.id}:deleted-run"
        pending_key = f"cron:{parent.id}:pending-run"
        current_key = f"cron:{parent.id}:current-run"
        deleted_child = crons.add_job(
            "deleted-child", "ping", every_secs=3600, session_key=deleted_key
        )
        pending_child = crons.add_job(
            "pending-child", "ping", every_secs=3600, session_key=pending_key
        )
        current_child = crons.add_job(
            "current-child", "ping", every_secs=3600, session_key=current_key
        )
        crons.register_active_session_key(parent.id, pending_key)
        crons.register_active_session_key(parent.id, current_key)

        released = await crons.release_jobs_owned_by({deleted_key})

        reloaded = CronService(base_dir=tmp_path)
        assert released == [deleted_child.id]
        assert reloaded.get_job(deleted_child.id).session_key == ""
        assert reloaded.get_job(pending_child.id).session_key == pending_key
        assert reloaded.get_job(current_child.id).session_key == current_key

    @pytest.mark.asyncio
    async def test_release_rechecks_live_keys_after_waiting_for_the_store_lock(
        self, tmp_path, monkeypatch
    ):
        crons = CronService(base_dir=tmp_path)
        parent = crons.add_job(
        ...
        live: dict[str, str] = {}
        real_release = crons._release_jobs_owned_by_locked

        def register_and_persist_before_release(*args, **kwargs):
            live_key = f"cron:{parent.id}:new-live-run"
            crons.register_active_session_key(parent.id, live_key)
            child = crons.add_job("live-child", "ping", every_secs=3600, session_key=live_key)
            live.update(key=live_key, child=child.id)
            return real_release(*args, **kwargs)

        monkeypatch.setattr(
            crons,
            "_release_jobs_owned_by_locked",
            register_and_persist_before_release,
        )
        ...
        reloaded = CronService(base_dir=tmp_path)
        assert released == [deleted_child.id]
        assert reloaded.get_job(live["child"]).session_key == live["key"]
...
class TestAChildCannotBeBornUnderADeadParent:
    ...
    def test_dead_parent_keeps_private_memory_binding_before_owner_release(
        ...
        kwargs = {
            "name": f"late-follow-up-{append_path}",
            "message": "ping",
            "every_secs": 3600,
            "session_key": f"cron:{parent.id}",
        }
        if append_path == "add":
            late = crons.add_job(**kwargs)
        else:
            late = crons.add_job_if_absent(lambda job: job.name == kwargs["name"], **kwargs)
            assert late is not None

        persisted = CronService(base_dir=tmp_path).get_job(late.id)
...
class TestDeferredRemovalTickWriteFault:
    def test_tick_keeps_scanning_after_deferred_removal_save_fault(self, tmp_path, monkeypatch):
        """A requeued ENOSPC removal must not starve every other scheduled job."""
        crons = CronService(base_dir=tmp_path)
        parent = crons.add_job("parent", "sweep", every_secs=3600)
        child = crons.add_job("child", "ping", every_secs=3600, session_key=f"cron:{parent.id}")
        other = crons.add_job("other", "ping", every_secs=3600)
        crons.defer_removal(parent.id)
        ...
        with monkeypatch.context() as patched:
            patched.setattr(crons, "_save", _disk_full)
            snapshot = crons._tick_scan_locked()

        assert {job.id for job in snapshot} == {child.id, other.id}
        assert parent.id in crons._pending_removals
        assert crons.get_job(child.id).session_key == f"cron:{parent.id}"

        crons._tick_scan_locked()
        persisted = CronService(base_dir=tmp_path)
        assert persisted.get_job(parent.id) is None
        assert persisted.get_job(child.id).session_key == ""
        assert not crons._pending_removals
    ...
    async def test_transient_contention_is_retried_until_it_lands(self, caplog):
        ...
        with caplog.at_level(logging.WARNING):
            await _remove_slot_for_history_key(
                ...
                exact_owner_keys=("dashboard:chat-1-100",),
            )
...
class _FakeTranscriptStore:
    ...
    @contextlib.contextmanager
    def locked_stems(self, stems: Collection[str]) -> Iterator[None]:
        """Stand in for ConversationLog's canonical-plus-legacy lock set."""
        ordered = tuple(stems)
        self.calls.append(f"locked_stems:{','.join(ordered)}")
...
class TestSessionLedgerOnPermanentDelete:
    ...
    @staticmethod
    def _ledger(session_id: str, slot: str = "dashboard_chat-1-100"):
        """A session ledger whose header names *slot*, as a real one does.

        The slot is not decoration: the removal requires the unit's own header to
        name a slot this delete proved, because the session id reaches it through
        ``session_map.json`` while the header is written once inside the fenced
    ...
    async def test_a_ledger_the_emitter_itself_opened_is_still_removed(self, monkeypatch):
        ...
        funnel cannot collect a real ledger until that handle is released. What
        releases it is the ``session/closed`` entry landing, which
        ``session_lifecycle.destroy`` writes and this funnel then FLUSHES before
        claiming the lease.

        The stub teardown below emits that close itself, because the real
        ...
        try:
            emit.on_session_opened(
                "acp-emitted", agent="kirocrew", slot="dashboard_chat-1-100", cwd="/tmp"
            )
            assert emit.flush(timeout=5.0)
            assert self._exists("acp-emitted")
            assert (
                remove_unit(lg.KIND_SESSION, "acp-emitted", guard=lambda _dir: True) == REMOVE_OWNED
            ), "fixture did not reproduce the emitter-held lease this test exists for"

            slot = _make_slot("dashboard_chat-1-100")
            state = _make_state({"dashboard_chat-1-100": slot})
            state.sessions.resumable_sid = MagicMock(return_value="acp-emitted")

            async def _destroy_like_the_real_one(*_args, **_kwargs):
                emit.on_session_closed("acp-emitted", END_REASON_DESTROYED)
                return True

            state.sessions.destroy_if = _destroy_like_the_real_one

            await _remove_slot_for_history_key(state, "dashboard_chat-1-100")

            assert not self._exists("acp-emitted")
        finally:
            emit.reset_caches()

    @pytest.mark.asyncio
    async def test_the_teardown_entry_never_starts_a_ledger_for_a_session_without_one(
        self, monkeypatch
    ):
        """Recording the teardown must not CREATE the file it exists to release.

        ``_handle`` never creates a ledger, so a close for a session that has none
        is a policy no-op. Without that, the funnel would write a header for a
        conversation it is deleting and then have to remove it -- and would leave
        one behind on any pass where the removal was refused.

        The removal is refused HERE for that reason. A test that let it succeed
        could not see this property at all: a ledger created and then removed
        leaves exactly the state of one that was never created, so the end-state
        ...
        try:
            slot = _make_slot("dashboard_chat-1-100")
            state = _make_state({"dashboard_chat-1-100": slot})
            state.sessions.resumable_sid = MagicMock(return_value="acp-never-opened")

            await _remove_slot_for_history_key(state, "dashboard_chat-1-100")

            assert not self._exists("acp-never-opened")
        finally:
            emit.reset_caches()

    @pytest.mark.asyncio
    async def test_a_ledger_whose_header_names_another_slot_is_never_removed(self, monkeypatch):
        """The forged-mapping path, closed by the unit's own header.

        The session id reaches this funnel through ``session_map.json``, which lives
        inside the agent-visible tree, so a mapping that named a victim's session
        would aim the removal at the victim's ledger. The header is the independent
        answer: written once inside the fenced ledger tree and never rewritten, so
        it does not move when a mapping does.

        The victim ledger here is a REAL, removable unit -- same kind, closed, no
        writer -- so the only thing standing between the removal and it is the slot
        check. A fixture the removal would have refused anyway could not tell the
        two readings apart.
        """
        _guard_work_ledger_cleanup(monkeypatch)
        self._ledger("acp-victim", slot="dashboard_chat-9-900")
        slot = _make_slot("dashboard_chat-1-100")
        state = _make_state({"dashboard_chat-1-100": slot})
        # The forgery: this slot's mapping points at the victim's session.
        state.sessions.resumable_sid = MagicMock(return_value="acp-victim")

        await _remove_slot_for_history_key(state, "dashboard_chat-1-100")

        assert self._exists("acp-victim"), "a mapping alone redirected the removal"

    @pytest.mark.asyncio
    async def test_a_ledger_with_no_slot_in_its_header_is_left_to_retention(self, monkeypatch):
        """Cannot prove means do not remove, rather than assume it is ours.

        A session that never ran on a dashboard slot writes no slot in its header.
        There is then no second answer to check the mapping against, so the funnel
        removes nothing and the sweep collects the unit on age once its close has
        landed.
        """
        _guard_work_ledger_cleanup(monkeypatch)
        from kiro_crew import ledger as lg
        ...
        assert self._exists("acp-slotless")

    @pytest.mark.asyncio
    async def test_a_ledger_another_key_still_maps_to_is_left_to_retention(self, monkeypatch):
        """A second key on the same session id shares THIS ledger and can resume it.

        The unit is keyed by the ACP id, and the teardown removed only one mapping,
        so a surviving key still resolves that id -- its holder can resume, and the
        log is still needed. Two keys on one sid is a state the system itself
        produces: importing a transferred session twice allocates a new slot key
        each time and deliberately leaves the source intact.
        """
        _guard_work_ledger_cleanup(monkeypatch)
        self._ledger("acp-shared")
        slot = _make_slot("dashboard_chat-1-100")
        state = _make_state({"dashboard_chat-1-100": slot})
        state.sessions.resumable_sid = MagicMock(return_value="acp-shared")
        state.sessions.find_key_by_sid = MagicMock(return_value="dashboard_chat-7-700")

        await _remove_slot_for_history_key(state, "dashboard_chat-1-100")

        assert self._exists("acp-shared"), "a ledger another key can still resume was removed"

    @pytest.mark.asyncio
    async def test_an_unreadable_map_leaves_the_ledger_rather_than_removing_it(self, monkeypatch):
        """Cannot read means cannot prove, and the safe answer keeps the history."""
        _guard_work_ledger_cleanup(monkeypatch)
        self._ledger("acp-unknown-map")
        slot = _make_slot("dashboard_chat-1-100")
        state = _make_state({"dashboard_chat-1-100": slot})
        state.sessions.resumable_sid = MagicMock(return_value="acp-unknown-map")
        state.sessions.find_key_by_sid = MagicMock(side_effect=RuntimeError("map unreadable"))

        await _remove_slot_for_history_key(state, "dashboard_chat-1-100")

        assert self._exists("acp-unknown-map")
