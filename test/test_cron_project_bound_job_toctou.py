...
class TestUpdatePathBindingToctou:
    ...
    async def test_a_concurrent_bind_is_audited_as_denied(self, tmp_path):
        """GPT 5.6 Review F2: the CAS re-check catches exactly the race the
        class docstring describes -- a non-owner's authorization decision
        made against a binding that changed underneath it. Every sibling
        non-owner denial in this handler logs a SEL event; this one must
        too, or the audit trail has a gap for precisely the
        authorization-relevant race this guard exists to close."""
        crons = CronService(base_dir=tmp_path)
