"""Create-a-session-on-a-Project, and the per-turn attachment re-resolve."""

from __future__ import annotations

import json
from pathlib import Path
from types import SimpleNamespace
from unittest.mock import AsyncMock

import pytest
from aiohttp import web
from aiohttp.test_utils import TestClient, TestServer
from chat_test_helpers import _make_state

from kiro_crew.dashboard import chat_runner, handlers_project
from kiro_crew.dashboard.chat import api_chat_slot_create
from kiro_crew.dashboard.state import _ChatSlot
from kiro_crew.project_manifest import create_project_manifest
from kiro_crew.project_registry import ProjectRegistry
from kiro_crew.project_sessions import ProjectSessionError

_PROJECT_UUID = "018f4f4a-760f-7a8b-a5d4-5a7e0f130d4e"


def _registry(tmp_path: Path) -> ProjectRegistry:
    return ProjectRegistry(
        projects_dir=tmp_path / "projects",
        registry_dir=tmp_path / "projects-registry",
    )


def _app(state, registry: ProjectRegistry, *, owner: bool = True) -> web.Application:
    @web.middleware
    async def identity(request: web.Request, handler):
        request["app"] = ""
        request["user"] = "local-app" if owner else "guest"
        return await handler(request)

    app = web.Application(middlewares=[identity])
    app["state"] = state
    services = handlers_project._ProjectServices()
    services._registry = registry
    app[handlers_project.PROJECT_SERVICES_KEY] = services
    app.router.add_post("/api/chat/slots", api_chat_slot_create)
    return app


def _record_slot_frames(state) -> list[list[dict]]:
    frames: list[list[dict]] = []

    def capture(payload):
        if payload.get("_type") == "slots":
            frames.append(json.loads(payload["slots"]))

    state._broadcast = capture
    return frames


@pytest.mark.asyncio
async def test_create_attaches_project_before_first_broadcast(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    monkeypatch.setattr("kiro_crew.dashboard.state.config_dir", lambda: tmp_path)
    bundle = tmp_path / "payments"
    create_project_manifest(bundle, name="Payments")
    registry = _registry(tmp_path)
    project = registry.add_local(bundle)
    state = _make_state(tmp_path / "sessions")
    frames = _record_slot_frames(state)

    async with TestClient(TestServer(_app(state, registry))) as client:
        response = await client.post(
            "/api/chat/slots", json={"name": "payments-chat", "project_id": project.id}
        )
        assert response.status == 200
    slot = state._slots["payments-chat"]
    assert slot.project_id == project.id
    assert slot.project == str(bundle.resolve())
    assert "Project: Payments" in slot._project_brief
    announced = next(row for row in frames[-1] if row["key"] == "payments-chat")
    assert announced["project_id"] == project.id
    assert announced["project"] == str(bundle.resolve())


@pytest.mark.asyncio
async def test_create_rejects_unknown_project_without_creating_slot(tmp_path: Path) -> None:
    state = _make_state(tmp_path / "sessions")
    registry = _registry(tmp_path)

    async with TestClient(TestServer(_app(state, registry))) as client:
        response = await client.post(
            "/api/chat/slots",
            json={"name": "orphan", "project_id": _PROJECT_UUID},
        )
        assert response.status == 404
        assert (await response.json())["code"] == "project_not_found"
    assert "orphan" not in state._slots


@pytest.mark.asyncio
async def test_create_redacts_unavailable_project_error(tmp_path: Path) -> None:
    credential = "AKIAIOSFODNN7EXAMPLE"
    bundle = tmp_path / "payments"
    create_project_manifest(bundle, name=f"Payments {credential}")
    registry = _registry(tmp_path)
    project = registry.add_local(bundle)
    (bundle / "project.yaml").unlink()
    state = _make_state(tmp_path / "sessions")

    async with TestClient(TestServer(_app(state, registry))) as client:
        response = await client.post(
            "/api/chat/slots",
            json={"name": "unavailable", "project_id": project.id},
        )
        assert response.status == 409
        payload = await response.json()

    assert payload["code"] == "project_manifest_invalid"
    assert credential not in payload["detail"]
    assert credential not in payload["error"]
    assert "unavailable" not in state._slots


@pytest.mark.asyncio
async def test_create_project_attachment_is_owner_only(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    bundle = tmp_path / "payments"
    create_project_manifest(bundle, name="Payments")
    registry = _registry(tmp_path)
    project = registry.add_local(bundle)
    state = _make_state(tmp_path / "sessions")
    events: list[dict[str, object]] = []
    monkeypatch.setattr(
        "kiro_crew.dashboard.chat_handlers.sel",
        lambda: SimpleNamespace(log_api_access=lambda **event: events.append(event)),
    )

    async with TestClient(TestServer(_app(state, registry, owner=False))) as client:
        response = await client.post(
            "/api/chat/slots", json={"name": "forbidden", "project_id": project.id}
        )
        assert response.status == 403
        assert (await response.json())["code"] == "owner_only"
    assert "forbidden" not in state._slots
    assert events == [
        {
            "caller": "guest",
            "operation": "project_attach",
            "outcome": "denied",
            "source": "dashboard",
            "resources": "non_owner_block",
        }
    ]


@pytest.mark.asyncio
async def test_create_project_attachment_fails_closed_when_audit_is_unavailable(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    bundle = tmp_path / "payments"
    create_project_manifest(bundle, name="Payments")
    registry = _registry(tmp_path)
    project = registry.add_local(bundle)
    state = _make_state(tmp_path / "sessions")
    attachment_resolutions: list[str] = []
    audit_events: list[dict[str, object]] = []

    def track_resolution(*args, **kwargs):
        attachment_resolutions.append(str(args[0]))
        raise AssertionError("resolution must not run without a recorded audit")

    monkeypatch.setattr(
        "kiro_crew.dashboard.chat_handlers.resolve_project_attachment",
        track_resolution,
    )

    def audit_failure(**event):
        audit_events.append(event)
        raise OSError("audit unavailable")

    monkeypatch.setattr(
        "kiro_crew.dashboard.chat_handlers.sel",
        lambda: SimpleNamespace(log_api_access=audit_failure),
    )

    async with TestClient(TestServer(_app(state, registry))) as client:
        response = await client.post(
            "/api/chat/slots", json={"name": "unaudited", "project_id": project.id}
        )
        assert response.status == 503
        assert (await response.json())["code"] == "project_audit_unavailable"
    assert "unaudited" not in state._slots
    assert attachment_resolutions == []
    assert audit_events[0]["critical"] is True


@pytest.mark.asyncio
async def test_create_cannot_attach_project_to_existing_conversation(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    bundle = tmp_path / "payments"
    create_project_manifest(bundle, name="Payments")
    registry = _registry(tmp_path)
    project = registry.add_local(bundle)
    state = _make_state(tmp_path / "sessions")
    existing = state.get_or_create_slot("existing")
    existing.append("user", "Already working", broadcast=False)
    monkeypatch.setattr(
        "kiro_crew.dashboard.chat_handlers.resolve_project_attachment",
        lambda *_args, **_kwargs: pytest.fail("rejected rebind resolved Project sources"),
    )

    async with TestClient(TestServer(_app(state, registry))) as client:
        response = await client.post(
            "/api/chat/slots", json={"name": "existing", "project_id": project.id}
        )
        assert response.status == 409
        assert (await response.json())["code"] == "project_rebind_requires_new_session"
    assert existing.project_id == ""


@pytest.mark.asyncio
async def test_create_rejects_non_string_project_id(tmp_path: Path) -> None:
    state = _make_state(tmp_path / "sessions")
    registry = _registry(tmp_path)

    async with TestClient(TestServer(_app(state, registry))) as client:
        response = await client.post("/api/chat/slots", json={"name": "invalid", "project_id": 0})
        assert response.status == 400
        assert (await response.json())["code"] == "project_invalid_request"
    assert "invalid" not in state._slots


@pytest.mark.asyncio
async def test_restored_project_attachment_refreshes_workspace_and_brief_before_launch(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    slot = _ChatSlot("restored")
    slot.project_id = _PROJECT_UUID
    slot.project = "/stale/workspace"
    workspace = tmp_path / "current-workspace"
    attachment = SimpleNamespace(workspace_dir=workspace, brief="Current Project brief")
    monkeypatch.setattr(
        "kiro_crew.project_sessions.resolve_project_attachment",
        lambda _project_id: attachment,
    )

    await chat_runner._refresh_project_attachment(slot)

    assert slot.project == str(workspace)
    assert slot._project_brief == "Current Project brief"
    # The first resolution of a restored slot binds without a reset.
    assert slot._pending_reset_history_key is None


@pytest.mark.asyncio
async def test_project_attachment_is_re_resolved_every_turn_and_a_moved_workspace_resets(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    # Project storage moves under a live session (a sync repoints a source, a
    # removal deletes the tree). A cached attachment must not outlive that:
    # every turn re-resolves, and a workspace that moved arms the deferred
    # reset so the provider cold-starts in the new directory.
    slot = _ChatSlot("attached")
    slot.project_id = _PROJECT_UUID
    first = tmp_path / "first"
    moved = tmp_path / "moved"
    calls: list[str] = []
    current = {"attachment": SimpleNamespace(workspace_dir=first, brief="brief v1")}

    def _resolve(project_id: str) -> SimpleNamespace:
        calls.append(project_id)
        return current["attachment"]

    monkeypatch.setattr("kiro_crew.project_sessions.resolve_project_attachment", _resolve)

    await chat_runner._refresh_project_attachment(slot)
    await chat_runner._refresh_project_attachment(slot)
    assert len(calls) == 2  # cached brief does not short-circuit the lookup
    assert slot.project == str(first)
    assert slot._pending_reset_history_key is None  # same workspace: no reset

    current["attachment"] = SimpleNamespace(workspace_dir=moved, brief="brief v2")
    await chat_runner._refresh_project_attachment(slot)

    assert slot.project == str(moved)
    assert slot._project_brief == "brief v2"
    assert slot._pending_reset_history_key == "dashboard:attached"


@pytest.mark.asyncio
async def test_unattached_slot_never_resolves_a_project(monkeypatch: pytest.MonkeyPatch) -> None:
    slot = _ChatSlot("plain")
    slot.project = "/some/dir"
    monkeypatch.setattr(
        "kiro_crew.project_sessions.resolve_project_attachment",
        lambda *_args, **_kwargs: pytest.fail("unattached slot resolved a Project"),
    )

    await chat_runner._refresh_project_attachment(slot)

    assert slot.project == "/some/dir"


@pytest.mark.asyncio
async def test_project_attachment_of_a_removed_project_raises_instead_of_serving_the_cache(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    slot = _ChatSlot("attached")
    slot.project_id = _PROJECT_UUID
    slot.project = str(tmp_path / "gone")
    slot._project_brief = "cached brief"

    def _removed(_project_id: str) -> SimpleNamespace:
        raise ProjectSessionError("project is not registered", code="project_not_found")

    monkeypatch.setattr("kiro_crew.project_sessions.resolve_project_attachment", _removed)

    with pytest.raises(ProjectSessionError) as exc_info:
        await chat_runner._refresh_project_attachment(slot)

    assert exc_info.value.code == "project_not_found"


@pytest.mark.asyncio
async def test_removed_project_fails_the_next_turn_without_binding_a_session(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch, caplog: pytest.LogCaptureFixture
) -> None:
    credential = "AKIAIOSFODNN7EXAMPLE"
    state = _make_state(tmp_path / "sessions")
    slot = _ChatSlot("restored")
    slot.project_id = _PROJECT_UUID
    slot.project = "/stale/workspace"
    get_or_create = AsyncMock()
    state.sessions.get_or_create = get_or_create
    state.sessions.record_failure = AsyncMock()
    monkeypatch.setattr(chat_runner, "_finish_queue_cycle", AsyncMock())
    monkeypatch.setattr(
        chat_runner,
        "_refresh_project_attachment",
        AsyncMock(
            side_effect=ProjectSessionError(
                f"Project {credential} workspace is unavailable",
                code="project_workspace_unavailable",
            )
        ),
    )
    caplog.set_level("WARNING", logger="kiro_crew.dashboard.chat_runner")

    await chat_runner._run_chat(state, slot, "Continue")

    get_or_create.assert_not_awaited()
    state.sessions.record_failure.assert_not_awaited()
    error = next(message["content"] for message in slot.messages if message["role"] == "error")
    assert credential not in error
    assert "[REDACTED" in error
    assert credential not in caplog.text


@pytest.mark.asyncio
async def test_invalid_manifest_refuses_chat_with_parser_message(tmp_path: Path) -> None:
    from kiro_crew.project_manifest import ProjectManifestError, load_project_manifest

    bundle = tmp_path / "payments"
    create_project_manifest(bundle, name="Payments")
    registry = _registry(tmp_path)
    project = registry.add_local(bundle)
    (bundle / "project.yaml").write_text("kind: NotAProject\n", encoding="utf-8")
    with pytest.raises(ProjectManifestError) as error:
        load_project_manifest(bundle)
    state = _make_state(tmp_path / "sessions")

    async with TestClient(TestServer(_app(state, registry))) as client:
        response = await client.post(
            "/api/chat/slots", json={"name": "invalid", "project_id": project.id}
        )
        assert response.status == 409
        assert await response.json() == {
            "error": "project_manifest_invalid",
            "code": "project_manifest_invalid",
            "project_id": project.id,
            "detail": str(error.value),
        }
    assert "invalid" not in state._slots
