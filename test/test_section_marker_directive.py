...
class TestHeadlessProducersAreRefused:
    ...
    async def test_a_provenance_refusal_reaches_the_operator_log(self, caplog):
        ...
        This gate turns a marker drawn by a cron or sub-agent turn into a no-op.
        The refusal text goes to the MODEL and the audit goes to the security
        stream, so an operator asking why their markers stopped appearing has
        nothing in the log they actually read. The sibling gate in the messaging
        driver writes such a line for exactly this reason.
        """
        slot = _FakeSlot(key="dashboard:ridden-by-cron")
        ...
        assert "section_marker" in refused[0].getMessage()
...
class TestTheProvenanceGateIsScopedByProducerEvidence:
    ...
    def test_the_rows_that_write_into_a_transcript_are_origin_gated(self) -> None:
        ...
        assert (
            "section_marker" in _DASHBOARD_ONLY_DIRECTIVES
            and "section_marker" in _USER_ORIGIN_DIRECTIVES
        )
