...
from kiro_crew import mcp_core, session_directive, validation
...
class TestSectionMarkerApplier:
    ...
    async def test_the_flushed_row_carries_its_label_under_meta(self):
        ...
        # Exactly these two, so no unread flag rides along: the label, and the
        # instant that separates two breaks flushed at one seam.
        assert set(row["meta"]) <= {"label", "at"}
    ...
    async def test_content_is_a_human_readable_fallback(self):
        ...
        slot.flush()
        assert slot.messages[0]["content"] == "— End of section: second-item —"
    ...
    async def test_an_unlabelled_marker_still_draws_a_break(self):
        ...
        await _apply(slot, {})
        slot.flush()
        assert slot.messages[0]["content"] == "— End of section —"
    ...
    async def test_a_credential_split_by_the_length_cap_is_still_scrubbed(self):
        ...
        mid-pattern so the regex misses it, leaving a partial secret in
    ...
    async def test_an_ordinary_label_is_left_alone(self):
        ...
        assert row["content"] == "— End of section: review item-42 (part 2/3) —"
    ...
    def test_the_cap_is_read_off_the_schema_not_re_spelled(self, monkeypatch):
        ...
        assert validation.max_section_label() == declared
        ...
        assert validation.max_section_label() == 17
    ...
    @pytest.mark.asyncio
    async def test_the_held_entry_carries_an_id_for_the_recovery_paths(self):
        """Both recovery paths key on this id: the handover dedup matches it against
        the delivered row's ``meta.noteId``, and the persist union retains a disk
        entry only when it has one. Id-less, a held marker replayed after a
        rows-only handover and lost its durable copy at the next union.

        Scope: the HOLD entry only. ``_FakeSlot.flush`` reproduces the write half
        rather than running it, so the row's own stamp is pinned against the real
        flush in ``test_deferred_note_persistence``."""
        slot = _FakeSlot()
        await _apply(slot)
        held_id = slot._deferred_notes[0].get("id")
        assert isinstance(held_id, str) and held_id, "the hold entry carries no id"
...
class TestTheMarkerReachesTheTurnsOwnSave:
    ...
    def _window_before_the_save(self, src: str, occurrence: int = 0) -> str:
        ...
        for _ in range(occurrence + 1):
            save_at = src.index(self.SAVE, save_at + 1)
        return src[max(0, save_at - 1200) : save_at]

    def test_the_marker_flush_sits_in_the_saves_own_block(self):
        src = self._module_source()
        # ONE flush-and-save site, the landing turn. The re-queue seam deliberately
        # has neither: it persists the hold, since its reply is appended afterwards.
        ...
        window = self._window_before_the_save(src, 0)
        assert self.FLUSH in window, "the landing save has no flush before it"

    def test_the_requeue_seam_persists_the_hold_instead_of_releasing_it(self):
        """A released marker at this seam is UNRECOVERABLE: its row lands before the
        auto-continued reply and is then permanently early in the transcript, where a
        merely-undurable marker is only missing. So this seam may make it durable and
        must not place it."""
        src = self._module_source()
        head = src[: src.index("Reset ALL retry budgets")]
        seam = head[head.rindex("\n        else:") :]
        assert "persist_held_markers_sync" in seam, "the re-queue seam lost its durable write"
        assert self.FLUSH not in seam, "the re-queue seam releases the marker again"
        assert self.SAVE not in seam, "the re-queue seam saves, which places the row"
...
class TestRealFlushWritesTheMarkerRow:
    ...
    def _held(slot, **over):
        ...
        The flush drops a held row whose recorded session differs, so an
    ...
    def test_a_held_marker_is_written_with_its_role_and_label(self):
        ...
        assert row["content"] == "— End of section: item-42 —"
        assert row["meta"]["label"] == "item-42"
    ...
    def test_a_marker_row_is_not_stamped_as_a_note(self):
        ...
        slot = self._slot()
        slot._deferred_notes.append(self._held(slot, content="— End of section —"))
        assert slot.flush_deferred_notes(markers_only=False) == 1
...
class TestTheFlushModeIsRequiredNotDefaulted:
    ...
    EXPECTED_SEAMS = {
        ...
        "chat_runner.py": [
            # The landing turn's pre-save flush. The empty-response re-queue is NOT
            # here: it persists the hold, because its reply is appended after it.
            "markers_only=True",
...
class TestTheProvenanceGateIsScopedByProducerEvidence:
    """Whether a dashboard-only directive ALSO needs the origin gate is a question
    about who calls it, not about symmetry. The rows that write into a transcript
    have no legitimate headless producer; a question card does, so the sets differ
    deliberately and this pins which way round."""

    def test_the_rows_that_write_into_a_transcript_are_origin_gated(self) -> None:
        from kiro_crew.dashboard.session_directive_apply import (
            _DASHBOARD_ONLY_DIRECTIVES,
            _USER_ORIGIN_DIRECTIVES,
        )

        for kind in ("section_marker", "suggest_followup"):
            assert kind in _DASHBOARD_ONLY_DIRECTIVES and kind in _USER_ORIGIN_DIRECTIVES
    ...
    def test_ask_question_keeps_the_surface_gate_only(self) -> None:
        ...
        withdrawing it would silently break a documented capability."""
        import pathlib
        ...
        skills = pathlib.Path(__file__).resolve().parents[1] / "src/kiro_crew/builtin_skills"
        granting = sorted(
            p.parent.name
            for p in skills.glob("*/SKILL.md")
            if "ask_question" in p.read_text(encoding="utf-8")
        )
        assert granting, "no charter grants it; the exception above has lost its basis"


class TestClumpedMarkersAreToldApart:
    """Two breaks held by one turn flush at the same instant, so the row's own
    ``ts`` cannot separate them and a repeated label renders twice identically."""

    def test_a_held_marker_records_the_instant_it_was_requested(self) -> None:
        import inspect

        from kiro_crew.dashboard import session_directive_apply

        src = inspect.getsource(session_directive_apply)
        assert 'meta["at"] = datetime.now' in src
        # Display only: an explicit ``ts`` would bypass the monotonic correction
        # and sort the row before rows already written during the turn.
        assert "ts=meta" not in src and 'ts=meta["at"]' not in src

    def test_the_restore_path_admits_only_an_instant_shaped_value(self) -> None:
        from kiro_crew.dashboard.slot_buffers import _restored_marker_at

        assert _restored_marker_at({"at": "2026-09-09T14:31:00+00:00"})
        assert _restored_marker_at({"at": "look at this instead"}) == ""
        assert _restored_marker_at({"at": "2026-09-09T14:31:00+00:00 and more text"}) == ""
        assert _restored_marker_at({"at": 1234567890}) == ""
        assert _restored_marker_at({}) == ""
        assert _restored_marker_at("not a dict") == ""
...
def test_recall_and_replay_skip_section_markers(tmp_path):
    ...
    log.append("k", SECTION_MARKER_ROLE, "— End of section: item-one —")
...
class TestDeliveryIsProviderNeutral:
    ...
    def test_a_marker_is_published_out_of_band(self, monkeypatch):
        from unittest.mock import patch

        from kiro_crew.mcp_tools import control

        # The gateway is sent the CALL for it to re-derive the payload from, and only
        # a dispatch records that call -- without it the emitter reports nothing.
        monkeypatch.setattr(mcp_core, "current_call_name", lambda: "section_marker")
        monkeypatch.setattr(mcp_core, "current_call_raw_args", lambda: {"label": "Phase two"})
        ...
        assert posted == [
            (
                ...
                {"tool": "section_marker", "raw_args": {"label": "Phase two"}},
            )
        ]
...
class TestTransferKeepsEachMarkersOwnInstant:
    """One seam flushes every held marker, so the row ``ts`` values collide and the
    marker's own ``meta["at"]`` is the only thing separating two breaks in a turn.
    A bundle that carries the rows but drops that value re-collapses them on import.
    """

    def test_the_shape_gate_admits_an_instant_and_drops_prose(self) -> None:
        from kiro_crew import validation

        assert validation.section_marker_instant("2026-09-10T09:12:33.481Z") == (
            "2026-09-10T09:12:33.481Z"
        )
        assert validation.section_marker_instant("2026-09-10T09:12:33+02:00") == (
            "2026-09-10T09:12:33+02:00"
        )
        # A rendered value beside the label, so anything outside the ISO charset is
        # dropped WHOLE rather than redacted and shown.
        assert validation.section_marker_instant("about lunchtime") == ""
        assert validation.section_marker_instant("2026-09-10T09:00:00Z <b>x</b>") == ""
        assert validation.section_marker_instant("2026") == ""
        assert validation.section_marker_instant("0" * 41) == ""
        assert validation.section_marker_instant(None) == ""
        assert validation.section_marker_instant({"at": "2026-09-10T09:00:00Z"}) == ""

    def test_one_rule_serves_both_restore_paths(self) -> None:
        """A second spelling would let the bundle accept what the local restore refuses."""
        import inspect

        from kiro_crew.dashboard import slot_buffers

        src = inspect.getsource(slot_buffers._restored_marker_at)
        assert "validation.section_marker_instant" in src
        assert "0123456789" not in src, "the charset is re-spelled instead of delegated"

    def test_the_bundle_carries_the_instant_through_all_three_stages(self) -> None:
        import inspect

        from kiro_crew.dashboard import session_transfer

        src = inspect.getsource(session_transfer)
        assert '"requested_at": marker_at' in src, "export drops the marker's own instant"
        assert (
            'validation.section_marker_instant(sm.get("requested_at"))' in src
        ), "an imported bundle is untrusted; the instant needs the same shape gate"
        assert (
            'marker_meta["at"] = sm["requested_at"]' in src
        ), "the instant is validated and then never restored, so the rows re-collapse"

    def test_the_instant_is_not_the_row_stamp(self) -> None:
        """``ts`` orders the row; it cannot separate markers a single seam flushed."""
        import inspect

        from kiro_crew.dashboard import session_transfer

        marker_block = inspect.getsource(session_transfer._assemble_bundle)
        assert '"ts": m.get("ts", "")' in marker_block
        assert '"requested_at": marker_at' in marker_block
