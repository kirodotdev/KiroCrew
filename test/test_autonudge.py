...
async def test_cancelled_shutdown_stops_waiting_on_a_wedged_write_after_the_grace(
    ...
    the write is neither cancelled nor lost, and that the process-wide teardown
    (`stop()`) still ran."""
...
async def test_uncancelled_shutdown_waits_past_the_grace_for_a_slow_write(tmp_path, monkeypatch):
    ...
    caller. Pinned with a window far shorter than the wait."""
    monkeypatch.setattr(_an, "_DRAIN_CANCEL_GRACE_SECS", 0.01)
    svc = AutoNudgeService(base_dir=tmp_path)
    await svc.start()
    started, release, finished = _wedge_write(svc, monkeypatch)
    ...
    assert not done, "an uncancelled shutdown gave up on a write nobody cancelled"
    assert not finished.is_set()
...
async def test_durability_drain_window_spans_every_settle_on_one_instance(monkeypatch):
    ...
    done, _ = await asyncio.wait({asyncio.ensure_future(drain.settle(second))}, timeout=1)
    assert done, "the second settle ignored the window opened by the first"
    assert done.pop().result() is False
    assert not second.done() and not second.cancelled()

    fresh = _an._DurabilityDrain()
    never = loop.create_future()
    done, still = await asyncio.wait({asyncio.ensure_future(fresh.settle(never))}, timeout=0.2)
    assert not done and fresh.interrupted is False
    never.set_result(None)
    assert await still.pop() is True
...
async def test_update_refuses_a_restart_while_the_runtime_budget_is_still_spent(tmp_path):
    ...
    with pytest.raises(
        MonitorUpdateConflict,
        match="Goal cannot restart because it reached its runtime budget",
    ) as excinfo:
        await service.update(loop.id, active=True)

    assert "Clear stopped goal" in str(excinfo.value)
...
async def test_update_refuses_a_restart_until_the_cycle_cap_is_raised(tmp_path):
    ...
    with pytest.raises(
        MonitorUpdateConflict,
        match="Goal cannot restart because it reached Max cycles",
    ):
        await service.update(loop.id, active=True)
