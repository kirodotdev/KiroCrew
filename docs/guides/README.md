...
| [secrets-env.md](secrets-env.md) | Passing secrets (API keys, tokens) to MCP servers: the encrypted vault (`secret://NAME`, stored via Settings → Secrets) as the primary route, with systemd environment directives or a shell wrapper as fallbacks for non-dashboard setups. |
