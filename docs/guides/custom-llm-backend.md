...
- A `claude` session **does receive Crew's MCP servers**: Crew fills the
  ...
  Crew's control plane is mounted — along with the user-configured servers
  your agent's spec actually references, after the same registry filtering a
  governed install applies — whenever Crew authors the session's
  `.claude/settings.local.json` (see "MCP tools on a Claude session" in
  [claude-code-provider.md](../system-specs/modules/claude-code-provider.md)).
  A project that already carries its own `.claude/settings.local.json` is the
  exception: Crew did not author the file that governs tool use there, so that
  session gets no seed and no `mcpServers` array at all. Tool calls
  pre-approved in your own `~/.claude` settings (including a
  `.claude/settings.json` inside a cloned project) never reach Crew's
  approval path, so Crew's deny rules and audit log do not see them.
