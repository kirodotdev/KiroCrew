...
**Caveats.** `pod api` cannot reach these routes: it authenticates as the
dashboard user (a `?token=` credential), so the handler's internal-auth gate
answers 403 `internal_secret_required`. Drive them from a caller that already
holds the internal secret — the MCP tool path in `src/kiro_crew/mcp_core.py` is
the reference — rather than by extracting one. The surface is behind
...
for i in $(seq 1 40); do
  kirocrew pod api my-wt POST chat/folders --data "{\"name\":\"soak-$i\"}" --allow-write \
...
**`pod api` is read-only unless you say otherwise.** `GET` and `HEAD` go
through; `POST`, `PUT`, `PATCH` and `DELETE` need `--allow-write` and are
otherwise refused before the pod is even contacted. A pod's API is a full control
plane — it can start agent turns, rewrite config and delete sessions — and the
caller is often an agent acting on a path it read out of content it does not
control (an issue body, a page, a log). Read-only by default bounds a steered
path to reading a throwaway instance; the flag is where the operator takes
responsibility for a state change.
