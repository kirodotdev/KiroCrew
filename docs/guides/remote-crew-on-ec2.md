...
3. **A built dashboard frontend (recommended)** — the launcher ships your
   checkout's existing `static/dist` bundle so the box skips the slow,
   failure-prone on-box npm build. Build it once with
   `cd website && npm ci && npm run build` (Node.js 18+); `kirocrew cloud
   doctor` reports whether the bundle will ship. Without it the box builds
   the frontend itself.
