...
| `kirocrew service install` / `stop` / `status` / `uninstall` | works, as a per-user **Task Scheduler** task at `\KiroCrew\gateway`, needing no elevation. The trigger repeats every minute and `MultipleInstancesPolicy` drops a tick while a healthy gateway holds the slot, so a gateway the loop-stall watchdog terminates is back within the minute. Two consequences to know: **`kirocrew service stop` disables the task**, because ending the instance alone would let the next tick undo the stop, and `start` re-enables it; and the task starts at **logon**, not at boot, so it does not serve a headless host nobody signs into. `KIROCREW_HOME` must be in your persistent user environment (`setx`) before installing, because a scheduled task inherits the logon session's environment and not the installing shell's; `install` refuses rather than supervising a different data home |
