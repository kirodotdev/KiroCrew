...
| OpenCode ACP backend (Docker-confined) | works with setup — enforced harnesses are refused by default (no OS sandbox backend exists to apply the credential mask they depend on), but OpenCode has a container path: build the image once with `docker build -t kirocrew-opencode:latest -f src/kiro_crew/agent_sdk/docker/Dockerfile.opencode src/kiro_crew/agent_sdk/docker`, set `agent.sandbox_docker=true` in `%USERPROFILE%\.kiro\crew\config.json`, and select OpenCode in Settings → Agent Backend. The container mounts the session workspace (read-write), a persistent state dir under the data home (read-write, so sessions survive), and — when present — your OpenCode config dir and auth file (read-only); no other host credential path is visible inside, and a missing daemon, a Windows-container-mode daemon, or a missing image refuses the session instead of starting it unfenced. Codex, Pi and goose stay refused (no container path yet) |
