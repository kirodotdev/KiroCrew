...
The redirect URI is fixed per provider (host, port and path) and is shown with a copy button on the provider's card in **Settings → OAuth Apps**. Most providers use the `127.0.0.1` loopback literal; Slack and HubSpot use the host name `localhost` because it is the only plain-http host those vendors accept. GitHub and Asana are the pre-registered providers on this build. The Google Drive, Gmail, Google Calendar, Slack, HubSpot and Box runbooks are written ahead of their Connections entry, so each card appears once that entry ships; Microsoft 365 is a different case, explained in its own runbook.
