...
Every refused ledger operation raises `LedgerError` carrying a stable `code`, and
...
| [`bad_root`](#bad_root) | both | A kind's root under `ledgers` is a link or resolves outside the data home. |
| [`already_exists`](#already_exists) | create | `create` was asked for a ledger whose file is already there. |
...
A unit id names a directory, so a separator in it would let the ledger escape its
root. The raw id is refused loudly rather than folded to something safe: a fold would
silently merge two units into one file, and two units sharing a ledger is worse than
a refused write.
...
A kind's root directory under `ledgers` is a link, or resolves outside the data
home's own ledger tree. The message names the directory.
...
`create` was asked for a ledger whose file is already there. Field: `id`.
...
`open` was asked for a ledger with no content-bearing segment. Field: `id`.
...
Another process owns writes to this unit's ledger, so this one appended nothing.
...
for. A ledger without a readable header is not a ledger. Also raised for a non-integer
...
This ledger kind does not own that `type` domain. The message names the domains the
kind does own.

**Caller action** — Write to the right kind of ledger, or use the right type. See
...
as `app:<name>`, into a crew ledger.

### `bad_thread`

`thread` does not name an existing, earlier `seq` in this same ledger — it is past
the newest entry, or names nothing readable. Field: `thread`.

**Caller action** — Anchor on a `seq` this ledger returned. `thread` is never a
...
A segment's header does not belong to the ledger being read, its schema version
