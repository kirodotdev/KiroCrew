...
Everything here is kind-independent: it holds for a session ledger and a crew
ledger alike. What differs between the two kinds is which `type` domains and which
...
`ledgers` is one shared root for both kinds, which is what lets a single fence
...
| Path | What it is |
|---|---|
| `ledger.jsonl` | The segment whose first entry is `seq` 1. The only segment a writer creates. |
| `ledger.<first seq>.jsonl` | A later segment. The filename's middle is its first entry's `seq`, all digits and greater than 1. |
...
segment is gone is still a readable ledger. A reader validates each segment's
...
| `createdAt` | int | required | Epoch milliseconds the unit's ledger was created. | |
...
| Field | Type | Required | Meaning | Enum |
|---|---|---|---|---|
| `type` | string | required | `domain/action`, partitioned on the first slash. The action may not itself contain a slash, so a namespace is exactly one level deep. A guest form `app:<name>/<action>` exists for crew ledgers. | |
...
| `ref` | object | optional | A citation into another or the same ledger. | |
...
A `ref` is a citation, not a copy: the bytes stay in the ledger they were written
...
| Field | Type | Required | Meaning | Enum |
|---|---|---|---|---|
| `unit` | string | required | The kind of ledger being cited. | `crew`, `session` |
...
ledger may legitimately be gone or pruned. The four statuses are on
...
the same ledger, and a reader pages a group by that anchor. It is validated
against the file at append time, so it can never name a line that does not exist
or one that comes later.

Session entries never carry it. It is the crew ledger's shape, where a report
