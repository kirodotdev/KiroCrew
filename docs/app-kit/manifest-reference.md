...
| `ui.sessionControls[].icon` | string | | Lucide icon name |
| `ui.sessionControls[].placement` | string | `"session-bar"` | Where the chip renders; `session-bar` is the only value today |
...
| `session.agent` | string | Agent selected for the session |
| `session.model` | string | Model selected for the session |
| `session.workspace` | string | Workspace the session belongs to |
...
per app. The dashboard renders at most **3** across all apps, and controls past
that are dropped rather than moved into an overflow menu -- the bar shares one row
with the message input. With four or more contributing apps, a declared control
...
own `backend.routes`, with `session_key` always and `folder_id` / `folder_name`
