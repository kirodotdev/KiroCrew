...
The visible line is **also** mirrored to whatever channel the session is bound to — a Slack thread, a Telegram or Discord conversation, or nothing at all for a dashboard-only session. This exists because the note's two halves reach different audiences: the context half is surface-agnostic, so it reaches the model on whatever surface the next message arrives from, while the visible line is a dashboard broadcast. Without the channel leg a session driven from Slack or Telegram gained the context with no visible provenance where its user was reading. Channel delivery is **best-effort, dispatched in the background, and not reported in the response**: the transcript line and the context entry are the note's contract, and waiting on a channel would put a wedged transport on the POST's critical path, where a client that timed out and retried would write both halves a second time. A governance denial, a transport that cannot send proactively, a paused (disconnected) binding, a recipient whose authorization was withdrawn, an exception, or a transport that answered without a message id all mean the channel simply does not receive it. It does **not** happen at POST time when the visible line is HELD: while a note is held neither half is committed yet, so mirroring it then would publish a channel note for content the transcript may never receive — a foreign binding acquired before the flush drops both halves, leaving the channel asserting a note that does not exist in the session. The mirror is deferred with the halves rather than dropped: the held note carries the destinations it was authored for, and the flush dispatches them once both halves land. A note dropped at the flush for a foreign rebind reaches no channel at all. `deliveryConditional` is true throughout, so the caller is never told delivery happened.
...
**An IMMEDIATE note's 200 carries a durable acknowledgement when it reports `appended:
true`, and it can answer 404.** The forced durable write happens where there is a channel
destination to protect: the visible line is written to the transcript
*before* the channel copy is dispatched, so a channel note can never outlive the line it
asserts. A slot with no channel destination is not put through that write and takes the
ordinary save path, and a write that fails answers `appended: false` — in both cases the
200 promises the transcript append and not the disk. If a permanent session deletion wins
the race against that write, the request
answers `404` with `{ "error": "not found", "code": "slot_not_found" }` rather than a 200
claiming `appended` for a row that will never reach disk. If instead the slot's routing
moves off the key the note was authorized against, the write is refused but the session
still exists, so the request answers `200` with `deliveryConditional` true — the note's
halves may be dropped by the next drain, and the flag is how a caller tells that from a
guaranteed delivery.

**Retry only on the two signals that say the note was NOT accepted:** `503
deferred_note_persist_failed` and `404 slot_not_found`. `appended: true` means the visible line
was written; it does not promise the forced durable write succeeded, and where that write fails
the channel mirror is withheld rather than left asserting a line the transcript may lose.
