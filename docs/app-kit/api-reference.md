...
Body: `{ content, source?, maxAge?, ephemeral?, visibleOnly? }`. By default a note does both writes; `visibleOnly: true` writes the visible transcript row and builds no context entry at all, for a caller dropping a breadcrumb into a slot it is about to close — a context entry queued there would be promoted into state that dies with the closing tab, so the mode exists to not create one. Any non-boolean `visibleOnly` is a `400` with code `invalid_visible_only`; omitted or null means the default both-writes behaviour. There is no context-only mode. The visible line is appended as `role: "inject"` with `cls: "reconcile-note"`, and its content is redacted (credentials, exfiltration URLs) before it reaches the transcript. `maxAge` defaults to 24h for the context half when the key is omitted, so a note nobody follows up on expires instead of attaching to an unrelated message later. An explicit null means no expiry, the same as it does on `/context` — the two endpoints share the field and do not give it opposite meanings. The same `source`/`maxAge`/`content` constraints above apply.

**An IMMEDIATE `visibleOnly` note is committed to disk before its 200, and is therefore reported unconditional.** The row is written and then pinned by a durable save the request awaits, so `deliveryConditional` is false even for an unbound slot: the mode's whole purpose is a row that survives the close that follows it, and a row acknowledged but not yet persisted would not. The pin is what makes the acknowledgement honest, so its failure is a refusal rather than a downgrade — a save the durability guard declines answers `503` with code `note_not_durable`, and the row is taken back out of the live window rather than left for the periodic flush to make durable behind the refusal. Nothing is broadcast to connected clients until the pin commits, so a refused row is never rendered and then withdrawn. Two conditions reach that `503`: the slot was rebound to another session under the write, and the slot was closed under the write (an open-shaped note save that committed after a close would erase the dismissal and return the tab from the archive). Both are authorization refusals rather than transport failures, so each is recorded in the security-event log exactly as the held-note drop above is; a durable write that fails for an I/O or lock reason answers the same `503` and is not recorded as a denial. **Do not close the slot on a `503`** — re-post the note only if the slot is still open. A held (`visibleDeferred: true`) `visibleOnly` note keeps the ordinary hold semantics below, with no context half to deliver.
