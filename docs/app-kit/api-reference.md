...
**An IMMEDIATE note's 200 promises the transcript append, not the disk.** The visible line is
appended and broadcast in one step and the channel copy is dispatched after it — the same crash
posture every other channel-egress path has, so a gateway lost between the append and the next
save can leave a channel note whose transcript line was not yet persisted. If the slot's routing
moves off the key the note was authorized against, the request still answers `200`, with
`deliveryConditional` true — the note's halves may be dropped by the next drain, and the flag is
how a caller tells that from a guaranteed delivery.

**Retry only on the signal that says the note was NOT accepted:** `503
deferred_note_persist_failed`. `appended: true` means the visible line was written to the
transcript window.
