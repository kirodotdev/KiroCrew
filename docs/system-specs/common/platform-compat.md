...
| Decide whether a Nix store path may be PINNED rather than hard-`SHADOWED` | `trusted_nix_store_file(real)` — `real` (already `os.path.realpath`'d by the caller, so a `~/.nix-profile` symlink is collapsed to its store target) is pin-ELIGIBLE only when it is inside `/nix/store` AND `_agent_owns_or_can_write` finds NO entry of `traversed_components` (each hop and the final target) owned or writable by the account Kiro Crew runs as. `_agent_owns_or_can_write` has two arms: an OWNERSHIP arm (same-uid owns it, so it could `chmod`/recreate — refuses a single-user store) and a KERNEL writability arm, `os.access(..., effective_ids=True)` (`faccessat(AT_EACCESS)`), which reads mode bits, group membership AND a POSIX ACL. Fails closed to "writable" (`True`) on an unstattable component, off the `effective_ids` platform, and under root (where `os.access` answers True for everything). `False` on Windows, outside the store, or when the walk cannot be enumerated. The caller does NOT then execute it: it still runs the shebang/dispatcher/interpreter checks and offers a one-time human pin whose approval binds the file's BYTES | treating store immutability as executable IDENTITY (the account controls the name->store mapping through the profile/PATH symlink the collapsed `realpath` never walks, so this gates pin-ELIGIBILITY, never auto-approval); or an `st_mode` group/other bit check such as `path_writable_by_current_user` (inside a user namespace that remaps unmapped groups to the overflow gid `nobody`/65534, a mode-bit test sees a spurious group match against a `nobody`-owned store and calls a store the kernel forbids writing "writable" — `os.access` is not fooled) |
