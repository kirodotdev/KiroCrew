...
| Match an already-open Windows file/directory to its current path spelling | `opened_path_identity_matches(fd, path)`; opens the path with no-reparse/non-delete-sharing semantics. It compares native IDs, and if only one handle supports `FileIdInfo`, re-reads both through `BY_HANDLE_FILE_INFORMATION` before comparing; an unreadable, reparse, share-denied, or differing identity is false. Thus 8.3 and long aliases agree without accepting a changed object. A synthetic `os.stat_result` can expose its Windows-only file-attribute extension as `None`; the no-reparse file opener treats that missing bitset as zero before its mask checks, while real reparse attributes still refuse. | `os.path.samefile`, normalized-string equality, or trusting the descriptor path spelling (all re-open or compare names rather than the held object) |
