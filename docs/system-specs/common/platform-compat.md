...
| Carry a Windows file's DACL across an atomic replace | `snapshot_windows_dacl(fd)` copies the source descriptor's DACL into a detached `WindowsDaclSnapshot` (Python bytes that outlive the handle, so every source handle can be closed before `os.replace`), then `apply_windows_dacl(path, snapshot)` re-installs it on the open-and-pinned staged inode; `atomic_write` drives both through the opt-in `preserve_windows_dacl=` keyword, and an unreadable or unappliable DACL raises `WindowsDaclError` and refuses the write | `restrict_to_owner` (writes an owner-only DACL, never copies the file's existing one); treating an unreadable DACL as empty and letting the replacement fall back to the directory's inherited DACL |
