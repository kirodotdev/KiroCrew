...
| Kill a group whose leader is reaped | `kill_process_group(pgid, sig)` (False once the group is empty or not signalable; refuses `pgid <= 1` and the caller's own group) | `os.killpg(pgid, sig)` |
