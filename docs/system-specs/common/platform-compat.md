...
| Listener-owner ancestry | `process_descendant_identities(pid, candidate_pids=...)` uses a Browser-specific three-way edge comparator: strictly later is accepted, strictly earlier is excluded, and equal coarse or unparseable order is inconclusive. On Windows each candidate-to-root chain must also keep the same PIDs, creation IDs, and edges across two Toolhelp snapshots, while unrelated siblings may churn. The shared `created_after` helper remains numeric-only for its pod and harness callers | requiring the whole process tree to remain unchanged, accepting a bare descendant PID, treating same-second fallback timestamps as foreign, or widening the shared Boolean helper |
...
| Port to PID | `find_listening_pids(port)` / `listening_pid_tool_available()`; `find_port_listeners(port)` when ownership must be scoped to the local address actually probed; `probe_port_listeners(port)` when completed-empty must be distinguished from timeout or execution failure | `lsof` directly |
