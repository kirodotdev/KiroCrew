...
| Restart when the resolved interpreter was PRUNED by the update | `reexec_launcher(launcher, args)` after `gateway_restart.resolve_launch_shim()` validates the pathname this process was launched through (snapshotted at import) | re-reading `sys.argv[0]` at restart time, a `PATH` lookup for the launcher, or `os.execv` on a path already known to be gone |
