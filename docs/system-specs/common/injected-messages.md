...
- There is no continuation cap, for the same reason refusal recovery has none: the
  contract's own use case is a feedback loop, so terminating it is the hook's
  responsibility and Stop stays the breaker.
