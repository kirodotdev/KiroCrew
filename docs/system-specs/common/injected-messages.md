...
- **The row is credential-scrubbed at the sink; the prompt is not.** Both the banner
  row and the no-banner row pass through `redact_exfiltration_urls` then
  ...
  corrupt the instruction the model receives, so it is left alone. For a loop written
  through the arming surfaces below the scrub is a no-op — the authorizer already
  redacts `message` and `banner` on the way in, and redaction is idempotent — so it
...
- **A non-blank banner on a channel-bound loop is refused with 400.** A `slack:` /
  ...
  at load: a non-string or oversized value is blanked. Credential scrubbing runs at the
  SINKS, not at load, so the stored bytes stay as the operator wrote them.
