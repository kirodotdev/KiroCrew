...
7. **Wait for the stage's sub-agents.** Registers one
   ...
   **Settlement invariants.** These ids are stable; code comments and review
   ...
   | S2 | Consumed but interrupted stage work retains its exact retry or continuation obligation until successful settlement and capture; queueing or prompt consumption alone never discharges it. Only the stage turn's own consumption discharges it: a released boundary reports consumption and never stands in for that turn, so the loop pauses rather than advancing past a stage no turn executed. Exhausting the settlement wait ceiling stops auto-run instead of advancing. | `test_autopilot_stage_completion_handoff.py::test_unrelated_completion_does_not_suppress_consumed_stage_continuation`, `::test_exact_preconsumption_recovery_finishes_before_stage_advance`, `::test_unconsumed_stage_pauses_when_settlement_releases_the_boundary`, `::test_settlement_wait_exhaustion_halts_the_plan` | `state.py` (`StageBoundary.continuation_required`, `preserve`, `clear`), `chat_orchestrator.py` (`_queue_consumed_stage_resume`, `_settle_stage_delivery`, `_halt_plan`) |
