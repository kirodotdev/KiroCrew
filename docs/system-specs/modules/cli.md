...
16. **Default crew binding**: verifies the template the default crew's `kiro_agent` binds actually resolves to a spec kiro-cli can dispatch (declared `name` wins; the filename stem counts only when the spec declares no name), and that the resolved spec carries every always-on managed MCP server whose gate is open. Both failure states are typically left behind by a migration that carried config.json over from an install whose template names differ. A binding that resolves to NO spec fails every new session at start — and `kirocrew setup --agent-only --clean` cannot repair it, because setup regenerates only KiroCrew-owned specs; for the same reason the runtime's missing-mode error (`_format_runtime_rpc_error`) prescribes setup only for owned spec names and otherwise says to re-point the binding. A spec that EXISTS but lacks the managed servers runs sessions without lessons/memory (`learn_add`) and cron tools, and never heals on its own because the managed-MCP refresh rewrites only KiroCrew-owned specs. Read-only — doctor names the fix (re-point the default crew's Agent Template in the dashboard, or restore/complete the spec) rather than writing into a file KiroCrew does not own. Silent when the binding is the built-in `kirocrew` template (the Agent and MCP Tools sections cover that spec) and when the Model section already reported the binding as invalid free text. Only the DEFAULT crew is checked: a non-default crew may bind a project agent that resolves only against its own working directory, which doctor cannot see; the default binding is resolved with doctor's own project scope — the same view `resolve_agent_bindings` has for a session started there
