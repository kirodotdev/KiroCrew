...
| `kirocrew doctor --ledger-sweep` | List the session and conductor-work ledgers that look finished — kind, store directory, ledger key, phase or item counts, age and why each qualified. A DRY RUN: it deletes nothing. |
| `kirocrew doctor --ledger-sweep --purge [--older-than-days N] [--include-orphans] [--purge-unreadable]` | Delete the ledgers the dry run listed. Irreversible, explicit, and never automatic — no gateway hook, no history-delete path and no MCP tool reaches it. Each delete is re-decided under the store's own lock and refused if the ledger came back to life. Default idle window 30 days, which must be a finite non-negative number (`nan` and `inf` are refused, because every comparison against `nan` is false and would admit every ledger). An unreadable record is listed but kept unless `--purge-unreadable` is given, and a store is never purged when its `slot_key` breadcrumb is absent or names a key that resolves to a different directory — a copied store is reported for a human instead of aiming the delete at the canonical ledger. `--include-orphans` widens the work half only — an in-flight session ledger is kept whatever its key resolves to. Every one of these four flags is refused without `--ledger-sweep`, so `kirocrew doctor --purge` cannot look like a purge that found nothing. Rules and rationale: [session-work-ledger](session-work-ledger.md#cleanup). |
...
Two flags are MODES rather than checks and short-circuit before the list above runs, because neither answers "is this install healthy": `--bundle` collects the redacted diagnostics zip, and `--ledger-sweep` reports the ledgers that look finished (see the command table and [session-work-ledger](session-work-ledger.md#cleanup)). Neither prints the health report.
