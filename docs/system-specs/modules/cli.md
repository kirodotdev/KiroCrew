...
| `kirocrew app update NAME [--source PATH]` | The App Store's **Sync** from a terminal: re-copy an installed app from its recorded source (a directory, or the registry entry a registry install recorded, which is re-cloned) — or from the given local directory — through the RUNNING gateway: `POST /api/apps/{name}/update` over the owner-only socket, so the backend stop, deregistration, copy with `data/` preserved, re-registration of MCP servers/agents/skills/crons and restart all happen inside the process that owns them. Prints the old → new version and the re-registered counts. Unlike `enable`/`disable`, there is NO file-only fallback: with no gateway reachable it exits **3** with nothing changed, because `update_app` alone would swap the files while the gateway kept serving the old manifest. Exit codes are switched on the gateway's `code`, never its prose: **0** updated · **3** no gateway answered · **4** `app_source_name_mismatch` (the source's `app.json` names another app) · **5** `app_not_installed` / `app_lifecycle_not_gateway` · **1** any other refusal, a `--source` that cannot be resolved, and a timeout whose outcome is unknown. **2** is argparse's own usage error and is deliberately not reused. |
