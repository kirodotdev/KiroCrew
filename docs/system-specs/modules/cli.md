...
The strict-identity check prints an empty-roster upgrade note on every platform.
Older releases could serialize the empty default during an ordinary config save;
the note explains that upgrades preserve it and tells operators with refused
session-ledger tools to enable `kirocrew-core` routing in MCP Management and
restart between tasks. It also names the broker/stub cost. This is guidance,
never an issue that changes doctor's exit status, and it does not rewrite config.
