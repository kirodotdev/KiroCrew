...
| `kirocrew.process.memory.rss_bytes` / `.peak_rss_bytes` | gauge (By) | — | Same module; delegate to `platform_compat.proc_rss_bytes` (current) / `proc_peak_rss_bytes` (high-water mark), both cross-platform. Each callback publishes its own probe's bytes unchanged; these separate observations are not an atomic snapshot and promise no cross-source `peak >= current` ordering. Linux statm can include per-CPU RSS deltas absent from getrusage's approximate high-water accounting. A 0 return maps to None: gap, never a fake zero sample. |
