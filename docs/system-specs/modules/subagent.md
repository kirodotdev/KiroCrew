...
| `_TIMEOUT_SECS` | 1800 | Initial timeout per subagent (30 minutes). With `agent.subagent_timeout_auto=true` (default), a timeout or a successful run that consumes at least 80% of its deadline raises future deadlines by at least 30 minutes, bounded by `agent.subagent_timeout_max_secs` (default 7200). The learned level is stored without task text in owner-only `~/.kiro/crew/subagents/timeout_state.json`; bounded no-follow reads and atomic replacement refuse planted links and aliases. History loading waits until the first post-readiness reaper sweep, and every read/write runs on the maintenance executor rather than the gateway event loop. The configured timeout remains the floor and a configured floor above the adaptive ceiling is honored. Each run captures its deadline at execution start, so learning never changes an in-flight run. |
...
3. `_run()` captures the current effective deadline on `SubagentInfo.timeout_secs` and wraps `_run_inner()` with `asyncio.wait_for(timeout_secs)`
