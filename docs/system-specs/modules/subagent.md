...
**Reconnect recovery**: `subscribe_subagents` in `ws.py` restores both managed and native subagent cards. Managed subagents are authoritative in `SubagentManager`: running records replay as `subagent_snapshot`, and recently completed records replay as `subagent_done`. After replaying in-memory records, the handler falls back to the persistence layer (`subagent_persistence._subagents_dir()`) for agents the manager no longer holds (e.g. after a gateway restart), bounded by `NATIVE_SUBAGENT_TERMINAL_KEEP` (50) and `NATIVE_SUBAGENT_TERMINAL_TTL_SECS` (3600s) to avoid replaying ancient cards. The scan does blocking disk I/O (scandir + stat + JSON reads), so it runs OFF the event loop via `asyncio.to_thread` (`_load_persisted_replay_frames` -> `_collect_persisted_replay_frames`); the directory walk itself is additionally bounded to the newest `_PERSIST_SCAN_CAP` (200) folders by mtime via `heapq.nlargest`, so a long-lived install with thousands of folders does not turn a reconnect into an unbounded stat storm. Persisted running agents emit `subagent_snapshot`; tombstoned agents emit `subagent_done` with outcome derived from `tombstone.cause`. An agent still marked `running` with **no** tombstone whose recorded PID is no longer alive (checked via `platform_compat.pid_exists`, and treated as dead when no PID was ever recorded) is a crashed/orphaned process: it replays as a terminal `subagent_done` (outcome `failed`) so the card is rebuilt rather than dropped or left permanently stuck in `running`. Agents with non-`dashboard:` parent sessions are excluded. Managed results remain disk-backed and are not copied into inline Redux card payloads.

**Incremental update lazy-upsert**: the frontend reducers for `subagent_chunk`, `subagent_tool`, `subagent_stalled`, `subagent_retrying`, and their batched equivalents lazily create a minimal placeholder card (status `running`, empty task, agent `kirocrew`) when the wire id is not yet in the slot's subagent map. This ensures an agent whose `subagent_spawn` frame was missed (e.g. during the `clearSubagentsForSnapshot` reconnect window) still appears in the panel once its first incremental update arrives, rather than being black-holed. The placeholder is filled in if a later spawn or done frame arrives for the same id.
