...
- `context` (str, optional): context-injection mode for the subagent's first message, applied to every task in a batch spawn. `full` (default when omitted) injects memory, lessons, history, episodic memory, and skills; `no_memory` skips the memory-derived blocks (memory/lessons/history/episodic) via the same `blocks_reads` path as dashboard temporary sessions — for independent reviews where stored context could anchor the result; `minimal` injects only date/agent/runtime via the same `minimal_context` path as cron minimal runs — it also drops the skills catalog, triggered skills, and the critical output-format rules, so use it only for work that needs none of those. Maps onto `ContextBuilder.build_message(blocks_reads=..., minimal_context=...)` in `_run_inner`; unknown or absent values fall through to full injection.
