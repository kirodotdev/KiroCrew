...
| `_TIMEOUT_SECS` | 10800 | Default timeout per subagent (3 hours), from `constants.SUBAGENT_TIMEOUT_SECS`. With `agent.subagent_timeout_auto=true` (opt-in), a non-stalled timeout or a successful run that consumes at least 80% of its deadline raises future deadlines by at least 30 minutes, bounded by `agent.subagent_timeout_max_secs` (default 21600). A timeout trains only after `last_activity` advances past the run's captured `_exec_started`; a fully silent run cannot train even when its deadline expires before the idle-stall detector can flag it. A run already marked or suspected stalled from no stream activity also does not train the shared deadline. The learned level is stored without task text as one atomically replaced `~/.kiro/crew/member-memory-bindings/subagent-timeout.json` record under the pre-existing sandbox-read-only root; bounded no-follow reads refuse planted links, aliases, and oversized data. A pre-upgrade agent can therefore plant neither an accepted record nor a parent redirect, and no split state/digest commit can invalidate the last durable level. A qualifying observation computes a candidate but leaves the visible manager default and its return value unchanged; manager-owned detached work serializes candidate writes, reads the atomic record back, and only then publishes the durable level. A failed write or verification keeps the prior visible default, while `cancel_all()` drains pending healthy writes before reexec. History loads on the maintenance executor before the first run captures its deadline, so a restored level applies immediately without constructor or event-loop filesystem I/O. The configured timeout remains the floor and a configured floor above the adaptive ceiling is honored. The ACP prompt transport reserves the larger of the chat-turn ceiling, the configured subagent floor, and — only while adaptation is enabled — the full adaptive ceiling, with its outer margin. It therefore covers the adaptive ceiling while a candidate is waiting to become durable. Each run captures its manager deadline at execution start, resolves one explicit ACP prompt budget around that deadline and the then-current larger chat budget, and reuses it for every original, retry, fallback, and recovery prompt on shared or dedicated providers. A live config cut can change future runs but cannot shorten that captured transport budget. The reaper measures the manager deadline from `_exec_started`, so registration and spawn-approval waits never spend it; learning never changes an in-flight run. Live config changes flow through the same policy and do not erase an enabled learned level. Turning adaptation off immediately resets the active manager deadline for future runs to the configured floor; turning it back on deliberately restores valid history. |
...
| `_STALL_IDLE_SECS` | 120 | Seconds with no stream activity before a running subagent is surfaced as **stalled** in the running-card (configurable via `agent.subagent_stall_idle_secs`). Surface-only — a stalled subagent is never auto-terminated; the user closes it from the UX (per-row stop / Stop-all) and the run's captured adaptive deadline still applies. A WATCHDOG stall (`EVENT_COMPLETE` with `error: tool stall`) is a different signal and IS acted on: see *Stop reason → state*. |
...
3. `_run()` restores adaptive history when needed, initializes the effective deadline on `SubagentInfo.timeout_secs` only while that fresh record still carries the zero sentinel, preserves the nonzero captured deadline when unexpected-cancel recovery respawns the same record, and wraps `_run_inner()` with `asyncio.wait_for(timeout_secs)`
...
7. On timeout: record the run's captured deadline, raise the adaptive deadline for future runs when enabled, and report both values
...
`start_reaper()` launches a periodic loop (60s interval) that force-kills subagents exceeding their captured `timeout_secs` deadline. This is defense-in-depth for cases where `asyncio.wait_for` fails to fire due to event-loop saturation or orphaned tasks. Adaptive learning changes only future runs, so the wrapper and reaper always enforce the same immutable value for a given run.
