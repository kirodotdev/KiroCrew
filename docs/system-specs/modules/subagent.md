...
- **A claimed report is atomic, not merely exclusive.** The claim alone still lost outcomes when the claimer was cancelled mid-report. `_report_terminal` therefore runs on a strongly-referenced task under `asyncio.shield`, spawned by `_run` **before** its teardown awaits so the task is already live wherever a cancellation lands; the caller still receives `CancelledError` while the report completes. Spawn rejections, including approval and queue-drain rejections that never enter `_run`, use this same claimed report path rather than free-floating `_safe_announce` tasks. `cancel_all()` drains outstanding reports with a bounded timeout and then **cancels and gathers** any straggler, so none is left invoking `_on_done` against tearing-down state or killed by a closing loop. Because the awaiter is shielded, shutdown is bounded by that drain rather than the `_ON_DONE_TIMEOUT` injection cap. `SubagentManager.terminal_delivery_inflight_for(parent_session_key)` reads both the report-task owner map and a counted manager-owned completion ledger. The task owner stays true through the whole `_on_done` await, including a rejected completion and the awaited `subagent_injection_failed` fallback when the callback refuses before acceptance. When a dashboard callback accepts an asynchronous completion handoff, it transfers ownership into the ledger before returning; the matching consumption or durable-discard callback releases one count. That transfer survives report-task completion and removal of the materialized slot, so a slot-less Slack, Discord, or Webex stop still sees the debt. Session teardown and goal-stop guards use the combined signal because `info.done` is set before delivery begins and `running_agents_for` intentionally excludes done records. `chat_utils.subagents_attached` is the sole legal reader that interprets the slot's ad-hoc ownership fields (`_subagent_deliveries_inflight`, `_subagent_completion_pending`, and `_pending_subagent_failures`) as evidence of attached child work. Producer and transfer paths may mutate a field and inspect it for their own atomic update, but every teardown, goal-stop, notification, or other consumer MUST call the shared predicate rather than deriving attachment from a field directly; this keeps the fail-closed probes and bounded diagnostic detail in one contract. On dashboard routes ownership then transfers to the slot: `_subagent_completion_pending` remains non-empty until the completion turn is consumed (its callback follows first-empty-response requeue), and an injection failure atomically transfers that fence to `_pending_subagent_failures`, which the next turn drains. A queued completion also carries a one-shot discard callback through queue cancel, hard Stop, rewind and app teardown; on an explicit discard the delivery debt is settled and every `delivered` tombstone read back as durable BEFORE the fence is released, and there is no `await` between that durable evidence and the release. If there is no owed debt the fence is released immediately (nothing durable to lose); but a settlement error, a cancellation, a tombstone that does not read back durable, or a missing `subagent_mgr` when debt is owed all FAIL CLOSED — the fence is retained and no `delivered` tombstone is claimed, so restart recovery re-delivers rather than resurrecting or duplicating a result the user dismissed. The explicit-discard callback is one-shot: after any of those settlement failures there is no in-process retry, and restart orphan reconciliation is the only recovery path; the manager-owned ledger keeps slot-less goal-stop checks fenced until that restart. The manager marks this subset as `retained completion requires restart recovery`, distinct from an ordinary terminal report or completion awaiting consumption, and shared refusal guidance says to restart Kiro Crew for orphan recovery or close the session if abandoning the goal is intentional; it never promises that waiting or another Stop clears the state. Each such retain is also written to the SEL as a `subagent_completion_discard` / `retained` row on the parent session naming the child and the reason (`no_manager`, `settlement_cancelled`, `settlement_failed`, `tombstone_not_durable`), so the retained fence is operator-visible through `kirocrew security events` rather than only in the gateway log; the audit is best-effort and cannot alter the retain. Automatic queue-cap eviction removes the FIFO head only when it is unowned; an owned head makes the incoming notification use its fallback delivery path without growing or skipping through the queue. After queue pop, a pre-entry owner invokes the durable-discard callback if dispatch ends before `_run_chat` begins; once it begins, `_run_chat` transfers that callback to an exact held recovery row when the turn exits before consumption on authentication, approval, tool-authorization, and other error paths. That retained recovery suppresses the direct-injection failure fallback, so one timed-out completion has one recovery owner. Only an explicit Stop, task cancellation, or slot teardown invokes it directly; the recovery row keeps ownership until model consumption or a user-explicit discard. This applies to completed, failed, stopped, and rejected outcomes; only completed outcomes also carry result-file tombstone debt. Enforced by `test_subagent_reap_race.py`, `test_subagent_coverage.py::TestQueueDepth`, `test_subagent_scale.py::TestWaveDigest`, and the queue/stop-directive tests.
