...
- **Failed report settlement is boundary-owned and bounded.** Only a report with a non-empty `_stage_boundary_owner` enters the failure latch. Each retained failure is a frozen compact `_ReportFailureSnapshot`, never the live `SubagentInfo`; it holds exact run/parent/owner-generation identities, timing scalars, and only result, task, raw-task, and error delivery text independently capped at 64 KiB, then reconstructs a temporary record without changing terminal outcome, partial state, or result location. Entries use at most 500 exact scopes plus one bounded global overflow bucket, with each bucket holding at most 64 failures and one saturation marker. When exact-scope capacity is exhausted, the saturating scope enters the global bucket while existing exact buckets stay in place. Its rows and marker retain producer identity for exact redelivery or discard, one bounded identity attributes bystander pause notices until the bucket empties, and every owned barrier fails closed while global debt remains. Explicit finished-run deletion first waits for any active terminal-report task, then redelivers debt for the matching live boundary or discards only that exact inactive boundary before record removal. A hard stop discards the owning stage boundary, while an ordinary non-Autopilot report is unowned, so neither can halt or advance a later stage.
