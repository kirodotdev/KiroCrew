...
### Ledger task dispatch

`spawn_run(work_item_id=...)` dispatches one item from the verified caller's own
ledger. Batch input is refused. The gateway checks that the item is open and
unbound, then carries its id through admission and the run record. Ordinary
spawn approval, capacity checks and private-memory inheritance still apply.
After approval, `_run_inner` rechecks the item before allocating a provider.
Scoped dispatch defaults to `kirocrew-worker`; an explicit agent or named crew's
agent still wins. The selected backend's resolved agent spec must declare
both `kirocrew-work` reporting tools without disabling or excluding either.
The SDK driver checks that configuration after allocation, using the provider's actual directory
and backend rather than guessing from the pool directory. KAS uses its user-level
spec and wildcard projection; the other supported backends use their existing
project-aware resolver. This happens after approval or queueing, before binding.
An incompatible agent fails the run with `work_reporting_unavailable`, leaving
the item available for a fresh dispatch.

Ledger workers always take the dedicated runtime path, even without private
memory. A shared runtime inherits its parent's process session key and cannot
provide the worker's strict `subagent:<id>` MCP identity.

Binding occurs immediately before the first provider stream, after private
memory preparation, provider creation, context building and run publication.
The worker identity is generated by the manager, never supplied by the caller.
Work routes recognize that identity only when the durable binding and the item's
`worker_session_key` agree. The same check protects brief reads and reports for
both spawned and dashboard workers. A partial bind left by a crash grants no
access, while a completed retry or replacement can report normally.
A preparation failure leaves the item available for another dispatch.
Binding is atomic against concurrent dispatch or closure; a transient stream
retry may retain its own binding but cannot take another worker's item.
The worker's `work_brief` and `work_report` routes recognize the durable binding
without requiring a dashboard slot and still verify private process identity.
