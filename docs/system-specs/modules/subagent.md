...
transient retries without double-counting the final completion. Providers that do
not bill in credits report `0`; display surfaces omit the credit label for that
value rather than claiming the run was free. The cumulative credit total is
...
completion, delivery-failure, orphan, and wave-digest messages.
Card headers retain elapsed time only so billing text does not crowd the status
and model chips in a narrow rail. A terminal header without positive reported
