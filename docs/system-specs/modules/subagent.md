...
### `cancel_session(parent_session_key) -> {"cancelled": [ids], "unqueued": [ids]}`
Session-scoped stop backing the chip's header "Stop all" (REST `POST /api/spawn/stop-all`, body `{"slot": "<slot name>"}`, answer `{ok, cancelled: n, unqueued: n}`; the handler resolves the slot to `_active_turn_session_key or effective_session_key(slot)` — the `_cancel_target` idiom: a running turn's captured key outranks the mutable routing, so a mid-turn rebind cannot redirect the stop to the NEXT turn's session — never a raw session key, SEL-audits as `stop_all_subagents` under the resolved session key with the slot name in metadata, and refuses ALL app callers with an anti-enumeration 404, SEL-logged as `subagent.stop_all` denied — deny-by-default rather than ownership-matched, because ownership lives on the RECREATABLE slot (`_app` is set fresh on every recreate) while the cancellation targets the DURABLE `dashboard:<key>` session, which still carries a prior owner's wave after a close + same-key recreate; the chip, a human control, is the only caller). One call cancels every not-done registered run whose `parent_session_key` matches (each a neutral user stop, same path as `cancel()`; the cancellations are all INITIATED before any completion is awaited — `asyncio.gather`, the same shape as `cancel_all` — because a reap's session teardown yields for up to `_RESET_TIMEOUT` and a sequential loop would leave agent N running through N-1 of those windows after Stop all) AND drops the session's `_queue` entries, each routed through `_unqueue`. **`_unqueue` is NEVER SILENT**: a queued member was already counted as submitted (top of `spawn()`), so besides re-emitting the queued depth, every dropped entry is announced as a synthetic `done=True, user_stopped=True` completion through the single completion consumer (same shape as `record_lost_submission`, without re-counting) — the wave's accounting sees the stop, the digest fires and `finalize_batch` prunes, and the parent is never left waiting for a completion event that cannot arrive. Because the entry leaves `_queue` synchronously while the announce runs on a scheduled task, a per-batch `_batch_unqueued_pending` count bridges the window (consulted by `batch_members_pending`, pruned by `finalize_batch`); the announces are serialized under a PER-BATCH lock (`_unqueue_announce_locks`, pruned by `finalize_batch`) whose critical section is decrement + the consumer's accounting prefix, held at most `_UNQUEUE_ANNOUNCE_LOCK_HOLD_SECS` — delivery's long tail runs outside the lock, so one wave's wedged injection can neither finalize a sibling early nor block another wave's announces. The queue is drained FIRST so a slot freed by a cancel cannot drain a queued member of the very wave being stopped. Two exclusions: a run parked on its spawn-approval prompt (`_awaiting_approval` with `_exec_started` unset) stays parked — the approval card is where the user decides it — and other sessions' runs and queue entries are out of scope. Never sets `_shutting_down` (that is `cancel_all`'s shutdown path). Point-in-time semantics: a spawn submission still in flight when the stop runs, or an entry `_drain_queue` popped but not yet registered, can start afterwards — the chip stays mounted and the stop can be repeated, the same property as every other cancel path. Exists because the client cannot do this per-id: queued members reach the UI only as an aggregate count (`subagent_queued`), so the old client-side loop over running ids left them to start after the user asked to stop (#8270). A per-id `DELETE /api/spawn/{id}` on a queued run also works now — the handler falls through to `cancel()`'s unqueue path when the id is not in `_agents` — but the chip has no queued ids to send.
...
- Detection is **surface-only**: `_maybe_flag_stall` never terminates the agent. A genuinely-hung subagent is closed by the user from the UX (per-row stop → `spawnDelete` → `SubagentManager.cancel(agent_id)`, or header Stop-all → `POST /api/spawn/stop-all` → `cancel_session`). The wall-clock reaper at `_TIMEOUT_SECS` remains the only automatic terminator; a `DEAD` liveness verdict deliberately does **not** escalate to a kill, because that would be a change to reap semantics rather than to the signal.
