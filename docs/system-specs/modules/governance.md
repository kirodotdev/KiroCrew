...
stranded exclusive `.tmp` cannot brick later Saves. The rollback's bytes
are staged BEFORE the publish: the snapshot of the existing policy is
written to the exclusive `security_policy.json.bak` leaf (declared by
NAME on the sensitive floor and sandbox mask beside `.tmp`/`.lock` -- not
by adding `.bak` to the keystone artifact-suffix rule, which spans the
crew-home root and would fence the operator's routine `config.json.bak` /
`sessions.db.bak` hand-backups; a leftover from an earlier
failed rollback is replaced, not a lock), and a staging failure refuses
the Save before anything changes. Dest-lockdown failure after replace
rolls back by ONE atomic replace of that `.bak` (Windows sharing-violation
retries, snapshot re-read before `write_failed` returns); dest is never
written into, so no failure can leave a truncated ceiling for the next
boot to load. If even that replace cannot land, the error propagates with
dest still the whole published document and `.bak` left holding the
snapshot for recovery; the next successful Save removes it. An existing
dest that cannot be snapshotted aborts
...
publish, then dest must be gone). A leftover regular
