...
| `TaskArtifactUpdateEvent` | `append: true` → `EVENT_TEXT_CHUNK` per delta, streamed live; `append` false or absent REPLACES that artifact's content and is delivered once, in final form, after the stream (a stale draft cannot be unsent), an exact recap of streamed status text deduplicated |
...
refused with the typed `agent_mismatch` error before any side effect. `run.py`
takes the `contextId` from the same read and rebuilds the provider with it;
