...
| Field | Meaning |
|---|---|
| `name` | The spawnable name. A name that is also a local agent is ambiguous and is refused at the spawn (`_validate_agent`, code `agent_name_collision`), where the local roster is known. |
...
`KiroCrewConfig`: `a2a_agent_names()` and `a2a_agent_by_name(name)`. The loader references the section types in the
...
refuses to start before any request is made, for `none` entries too. Loopback is
**not** exempt by default: a config entry pointing at `http://127.0.0.1:<port>`
would reach whatever service listens on that port of the host. Plain HTTP is
admitted only for the exact loopback origins the gateway's **environment** names
in `KIROCREW_A2A_PLAINTEXT_ORIGINS` (comma-separated, e.g.
`http://127.0.0.1:8790`; non-loopback entries are ignored) — set by whoever starts
the gateway, which an agent that can write `config.json` cannot. The test harness
and the GUI boot name the fake server's origin this way; a developer running a
local shim exports it once. The message endpoint is origin-pinned to the card URL,
so this covers every request.
...
"Once" includes re-entries. The governance vet runs only on the **gated** pass
(first entry, or the pump's re-check of a drained row); an accepted `spawn_async`
row re-entering with its store write done, and a claimed slot re-entering to
register, do not re-run it. A re-entry that resolved the registry afresh would
therefore stash whatever `config.json` says *now*, past the vet. So the gated
pass writes its classification into the row's params as plain data
(`_a2a_vetted`: the entry's name, card URL and auth scheme, or `null` for local —
rows are JSON), and every re-entry rebuilds `_a2a_entry` from that record and
reads the registry not at all. A malformed record reads as local, and a local
entry never reaches the remote route.
...
| one turn | one task (`taskId`) — the first frame naming a task locks the turn; a frame for any other task fails it (`A2AStreamError`), so another task's text is never recorded as this run's and the stop target never moves |
...
| `spawn_steer mode=follow_up` | existing follow-up watcher; delivered as the next task in the context once the current one is terminal. The watcher passes the run's own remote record (agent + persisted `contextId`) as `a2a_record`, the same fact the dashboard handler resolves for `spawn_continue`, so the rebuilt provider resumes the conversation rather than failing the resume guard |
| `spawn_steer mode=interrupt` | `steer_unsupported` (typed), no wire call; `supports_steer` is `False` |
| reap / user stop / timeout | `CancelTask` (best-effort request; the server decides), then the HTTP session is closed — `run.py::_release_direct_provider_impl`, called from normal teardown (`CancelTask` when the run ended with an error, the wall-clock timeout included; shutdown only on a clean completion) and `_force_reap` (cancel + shutdown) |
| `TaskArtifactUpdateEvent` | As the reference SDK's TaskManager reads it: `append` false on a new `artifactId` CREATES the artifact, `append: true` extends it, `append` false on an existing id REPLACES it. Created and appended text streams live as `EVENT_TEXT_CHUNK` (the transcript); a replaced artifact is held and delivered once, in final form, after the stream. The completion event carries every artifact's FINAL content, and `run.py` takes a remote run's record (`info.result`) from that rather than from the streamed chunks — so a draft that streamed and was then replaced is in the transcript but not in what the parent reads. An exact recap of streamed status text is not shown twice. |
| terminal `COMPLETED` / `CANCELED` | `EVENT_COMPLETE` |
| terminal `Task` snapshot | The other terminal shape the spec allows (the reference SDK's `completed_task()`): a `Task` frame whose `status.state` is terminal ends the turn exactly as the matching `TaskStatusUpdateEvent` would, and its inline `task.artifacts` are the result — each taken in the form given (authoritative over deltas assembled for that id) and delivered once, unless already held verbatim. A non-terminal snapshot (the opening `SUBMITTED` / `WORKING` frame) only supplies the ids. |
...
On `spawn_continue`, the conversation's recorded A2A facts (`agent`, `contextId`)
come from one `state.json` read, `SubagentManager.recorded_a2a`, which the async
caller (the dashboard continue handler) resolves under `asyncio.to_thread` and
hands to the synchronous `continue_conversation` — like `cwd`, the sync method
reads nothing itself. A remote conversation belongs to the agent that minted its
`contextId`: an empty `agent` inherits the recorded one and a different one is
refused with the typed `agent_mismatch` error before any side effect. The
admitted `contextId` then travels WITH the run: `continue_conversation` hands it
to `spawn` (`_a2a_context`), admission stashes it on the run record beside the
routing decision (`_a2a_entry`), and `run.py` builds the provider from that value
as-is — there is no second `state.json` read at build time, so the conversation
the provider joins is the one that was admitted. `_resumed` requires both a
retained `contextId` and a provider that actually started (card fetched), so a
card that no longer resolves produces `resume_failed` rather than a silent fresh
conversation.
...
abandoned as a failed turn (`a2a task output cap`). The text cap measures the
whole footprint of the per-artifact table — content **and** the server-chosen
`artifactId`s — and a turn naming more than 4,096 distinct artifacts is abandoned
the same way, so a stream of empty artifacts under fresh ids cannot grow the table
past what the text cap would allow. The HTTP read timeout is an
