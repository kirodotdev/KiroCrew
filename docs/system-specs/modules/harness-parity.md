...
| H8 | New harness identifiers live in `agent_sdk/backends.py` — a LEAF module behind the agent-SDK boundary, so every consumer can name the constants rather than copy them — and are added to `ACP_BACKENDS_KNOWN`; every capability set is a subset of it; and `AcpProvider.__init__` rejects anything outside it. `ACP_BACKENDS_KNOWN` is a live view: the frozen builtin base plus ids `register_known_backend` admitted from `harnesses.json` at boot (id grammar `^[a-z0-9-]{1,32}$`, builtin ids refused, a label/routing/namespace and opt-in capabilities recorded per id). A registered id is spellable exactly as a builtin is, and the capability accessors (`<capability>_backends()`) are what a session-path gate reads so a claim takes effect. `ACP_BACKEND_KIRO` is the empty string, so a value that falls through every identity check spawns `kiro-cli` under a foreign label. `acp/types.py` and the `acp_backends` shim both re-export the vocabulary and remain import sites for existing callers. | `test_harness_parity.py::test_capability_sets_are_subsets_of_known_backends`, `::test_unknown_backend_rejected_at_construction`, `::test_codex_is_selectable_and_answerable`, `test_operator_harness_bootstrap.py::test_no_session_path_module_reads_a_grantable_capability_off_the_frozen_set` | `agent_sdk/backends.py` (`ACP_BACKENDS_KNOWN`, `register_known_backend`), `acp/harness/operator_registry.py`, `providers/acp.py` (`AcpProvider.__init__`), `scripts/check_harness_parity.py` (`VOCABULARY_PATH`) |
...
| H11 | The provider label is a closed mapping over the builtin ids and an absent label means Kiro. It indexes resume compatibility, session-map persistence, and session-file cleanup routing, so a harness with no label of its own persists as a Kiro session and its transcript is pruned for want of a Kiro session file. A builtin's label is its `PROVIDER_LABEL_*` row; a registered id's label is the `display_name` recorded at `register_known_backend`, and `provider_label` reads it after the builtin mapping. Because the label is a key, the operator loader refuses a descriptor whose label a builtin row or another registered id already answers to (`"acp"` would file its sessions as Kiro's). | `test_harness_parity.py::test_every_known_backend_has_a_label`, `::test_codex_carries_its_own_provider_label`, `test_operator_harness_bootstrap.py::test_a_registered_backends_sessions_persist_under_its_own_label`, `::test_a_descriptor_whose_label_is_taken_is_reported_invalid` | `acp/types.py` (`PROVIDER_LABEL_*`), `agent_sdk/backends.py` (`provider_label_for`), `acp/harness/operator_registry.py` (`_label_owner`), `providers/acp.py` (`provider_label`, `cleanup_session`), `session.py` (`detect_provider_switch`) |
...
4. A new harness that ships IN THIS TREE adds rows to `ACP_BACKENDS_KNOWN`, a
   `PROVIDER_LABEL_*`, and an explicit decision for every Group B membership
   set. "Inherited the default" is not a decision. A harness an OPERATOR ships
   as a `harnesses.json` descriptor takes the data path instead: its id, label
   and routing are recorded by `register_known_backend` at boot
   (`docs/system-specs/modules/backend-authoring.md`), and every Group B
   membership set answers "not a member" for it -- a descriptor carries no
   capability claim, so admitting it to a set is the same code-reviewed edit a
   builtin needs. The loader refuses
   an id or label a builtin already owns. `BASELINE_SELECTABLE_BACKENDS` is otherwise
