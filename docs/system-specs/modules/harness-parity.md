...
|---|---|---|---|
| H1 | `agent.acp_backend` defaults to `ACP_BACKEND_KIRO`, and `ACP_BACKEND_KIRO` is in `ACP_BACKENDS_SELECTABLE` unconditionally. An operator who configures nothing, and an operator whose configuration is unusable, both get the Kiro harness. The initial preview set is pinned to Kiro CLI and KAS; discovery cannot widen it without validation evidence and an explicit membership edit. Claude remains described but withheld until session cleanup preserves pre-existing project settings. Codex remains described but withheld because its read-only mode does not permission-route passive reads, while the standard sandbox leaves credential homes readable. | `test_harness_parity.py::test_kiro_is_the_default_backend`, `::test_kiro_is_always_selectable`, `::test_initial_adapter_selection_is_limited_to_reviewed_backends` | `config/loader.py` (`AgentConfig.acp_backend`), `acp/types.py` (`ACP_BACKENDS_SELECTABLE`) |
