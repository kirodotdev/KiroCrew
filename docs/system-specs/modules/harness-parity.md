...
(`ACP_BACKEND_KAS`), OpenCode (`ACP_BACKEND_OPENCODE`), and whatever a
...
| H6 | A capability is granted by opt-in membership, never by negation. `is_session_sharing_eligible` reads `ACP_BACKENDS_SESSION_SHARING` and `supports_steer` reads `ACP_BACKENDS_STEER`, so a harness that has not demonstrated the capability does not inherit it from a set it was never added to. OpenCode is explicitly excluded from both sets and from `ACP_BACKENDS_INTERNAL_SANDBOX` / `ACP_BACKENDS_ACP_RUNTIME`; it uses a separately sandboxed per-session client. | `test_harness_parity.py::test_session_sharing_is_opt_in`, `::test_steer_is_opt_in`, `test_acp_backend_opencode.py::TestOpenCodeProviderBoundaries` | `providers/acp.py` (`AcpProvider.is_session_sharing_eligible`), `acp/client.py` (`AcpClient.supports_steer`), `acp/types.py` |
