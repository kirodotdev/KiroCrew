...
- `meta.sendId` — the scalar, on a row that stands for exactly one send. The two
  ...
  The announce carrying the id reverses a pin from `16ec93beff`, and that commit's
  own wording is what licenses the reversal: it lists "the entry / row shape is
  unchanged. The `queue_push` broadcast is untouched." beneath an **"Additive:"**
  heading, so the pin recorded SHAPE STABILITY rather than a decision to keep the id
  away from other clients. The same commit exists to let a client "prove ITS message
  landed (an unconfirmed-send notice deciding whether to retire)", which is the
  consumer this carry serves — so the carry follows that intent instead of
  contradicting it. The id-absent branch of the pin is kept as a negative control, so
  a send that minted no id still produces the byte-identical announce shape.
...
occur. Resolve them through `deliveryInDoubt` (`utils/sendDelivery.ts`), which
owns the precedence and answers it as ONE boolean — confirmation wins, so a row is
in doubt only when it carries `deliveryUnknown` and no confirmation — and do not
re-derive it per reader: two readers resolving it independently is how a bubble
comes to render muted while the composer beside it reads "delivered".
