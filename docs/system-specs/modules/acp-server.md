...
is actually editing. Production callers run extraction through
`asyncio.to_thread`, so path canonicalization and best-effort edit-line
derivation never block the ACP/dashboard event loop; line derivation reads only
regular files and caps the read itself at the module ceiling. The standalone
