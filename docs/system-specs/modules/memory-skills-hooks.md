...
- **Write-time embedding**: `_write_semantic()` embeds `"<key> <value_json>"` after the upsert (outside `_db_lock`, at `PRIORITY_BULK` — nothing blocks on it and the tail is reached from consolidation/import loops; same space-generation contract as `write_lesson`) and persists the struct-packed, un-normalized vector into the row's `embedding` column. The upsert's conflict clause keeps the stored vector when the value is unchanged (a re-affirmation — the tail then skips the redundant embed) and clears it when the value changed, so a row never ranks by a vector for text it no longer holds. `lesson.*` keys are excluded (`write_lesson` owns their vector — raw rule text). `set_semantic_if_absent()` (bulk import) defers embedding to the backfill sweep, like `write_episodic(defer_embedding=True)`. Rows missed while the model was absent — plus rows cleared by `reconcile_embedding_space()` — are repaired by `_backfill_semantic_kv_embeddings()` inside `backfill_missing_embeddings()`. `set_semantic(..., embedding=..., embedding_space=...)` accepts a pre-computed vector (right-width, finite, float32-packable; anything else is dropped) and persists it verbatim in place of the tail's embed, re-checking the durable space signature inside the locked persist when `embedding_space` is named — `import_memory()` uses it to carry a payload's vectors, but only into a store that cannot re-derive them right now (`embed_fn` unset — the CLI's — or the shared embedder not ready) and whose recorded space matches the payload's `embedding_space_sig` (a fresh unsigned store adopts the payload's space when the active backend produces it); everywhere else rows land through the normal tail, and rows left NULL are counted under `semantic_unembedded`. `kirocrew memory export` ships each semantic and episodic row's vector as base64 of its packed float32 bytes (~5.5 KB per row at 1024 dims; import accepts that form and a plain list; episodic rows are a new per-row payload, up to ~55 MB at the 10,000-episode export cap, streamed through `JSONEncoder.iterencode`; `--no-vectors` omits them) plus the top-level signature and `embedding_model` (the `{model_id, dim}` identity the signature hashes, when this process can name it); `import_memory()` treats a payload whose declared model does not produce its own signature as unsigned, and carries episodic vectors through `write_episodic(embedding_space=...)` under the same gates, counting NULL rows under `episodic_unembedded`.
