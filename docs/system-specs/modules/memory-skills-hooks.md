...
That dimension guard lives in `_cosine_sim` itself, not only in the scorer, so every caller gets it. It matters most on the two paths that compare a similarity against a *threshold* rather than sorting by it — the `write_lesson` semantic dedup (`sim > 0.85`) and `find_contradiction_candidates` (`[0.4, 0.85)`). A truncated score there is not merely mis-ranked: a 384-dim row against a 768-dim query scores ~0.50, which sits inside the contradiction band and is on the same scale as the dedup bar, so an unrelated lesson can be offered as a contradiction or deleted as a duplicate. Both of those loops also build the scorer once and score per row, the same hoisting the ranking paths use. Two properties they depend on that a ranking caller would not notice: the scorer's arithmetic is `float64` (stored blobs are `float32`, and leaving the norm in `float32` lands ~7e-8 off — invisible when sorting, enough to flip a score sitting on a threshold), and the scorer does **not** clamp at 0.0 (the thresholds may be non-positive, so clamping would change which rows qualify). Ranking callers clamp at their own call site where they need it: `get_semantic_context` does, because it passes `query_has_vector=True` and would otherwise feed a negative into the weighted sum; `_rank_lessons` does not need to, because `_hybrid_score` without that flag sends any non-positive vector score to the keyword-only branch.
