...
- The `[PROJECT]` block carries a git line naming the branch and whether the directory is the main checkout or a linked worktree (`_project_git_line`), so a session in a worktree can keep its work on its own branch when several sessions share a repository. Read from `.git`/`HEAD` on the filesystem every turn — no git subprocess, since this runs inline on prompt assembly — and derived from the directory rather than from the session's stored `slot.worktree` binding, so a `git checkout` in a terminal is reflected too. An unreadable or unexpected layout emits nothing rather than a guessed branch.
- A change in that reading is **announced**, not silently swapped: `ContextBuilder._last_git_line` holds the last line injected per session key (bounded, cleared wholesale), and a turn whose reading differs names both sides instead of only the new one. The session key survives the ACP cold start that a project change triggers, so the switch is still announced on the restarted turn. A session with no previous reading never claims a switch.
...
| `project` | `[DOCUMENTATION]` pointer, steering resources (CC backend only), `[PROJECT]` directory line + its git branch/worktree line and switch announcement | yes |
