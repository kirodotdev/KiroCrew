...
- **On-demand:** the `crystallize` builtin skill stages a candidate from the current session. The dashboard also exposes a **"Create skill"** action in the chat header's session actions menu: `POST /api/skills/-/from-session` (owner-only, refuses app and internal-secret callers) shapes the session transcript with `session_summary.extract_turns`/`render_input` and spawns a **background subagent** (`silent=True`) that follows `crystallize`. The subagent is spawned with `approval_mode="spawn"`: the spawn is authorized (a deliberate owner action) but its tool calls stay approval-gated -- never auto, and a trusted parent's auto policy is not inherited -- because it acts on an untrusted transcript. It also runs under the target session's own `agent`/`app` governance identities (not the default agent), so a scoped app/named-agent profile still bounds it. Authoring runs in the background, so it never blocks the ongoing chat. Because the click is a deliberate user action, the candidate marks itself `namespace: "manual"` in `.meta.json`, and `approve_pending_skill` promotes it to the **`manual/<slug>`** namespace (`MANUAL_SKILL_NAMESPACE`) instead of `auto/<slug>` — the same staging/review/approve pattern as auto-generation, but kept out of the `auto/` archive-if-unused lifecycle so a user-captured skill is never confused with, or aged out like, an auto-generated one. Incognito/temporary sessions are refused -- including a linked channel session (e.g. a Slack thread set to `!incognito`), whose restriction lives on the session's canonical state (the `SessionMap`), not on the dashboard slot's `memory_mode`; the guard checks `_session_key_is_restricted(effective_session_key(slot))` so a private linked thread is never persisted into a skill; the check is re-run immediately before spawn (the transcript read is awaited) to close the TOCTOU window. Completion surfaces through the existing `skills.pending_changed` notification.
...
- Author a skill from the current chat: dashboard **Create skill** action (chat header session menu), or `POST /api/skills/-/from-session {session_key, purpose}` (owner-only; purpose required; denials are SEL-audited; spawns a background subagent, non-blocking)
