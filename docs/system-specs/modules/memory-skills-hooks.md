...
- **Staged approval:** new skills route to `auto/.pending/<slug>/`; approve promotes to `auto/<slug>/` (dashboard: Skills → Pending review). Auto-approve for prose-only is opt-in via `approval_required=false`; it covers both new skills and prose-only updates to live auto-skills. An auto-applied update snapshots the prior version under `.versions/` for rollback. Script-bearing candidates and updates always require approval. When `auto_refine_on_deviation=true`, updates remain staged because the refinement writer replaces a full body without a version-aware base check.
- **Atomic candidate claim:** update and new-skill promotion first rename the public pending directory into `auto/.private/claims/` on the same filesystem. The entire `auto/.private/` subtree (claims plus pending, target, and per-claim locks) is on the permanent agent read/write deny floor; before any claim or lock operation, the private root and its `claims/`, `locks/`, and per-claim-lock descendants must be real non-linked directories whose resolved paths remain beneath the authenticated private root. Claiming also verifies both the logical private-root path and its fully resolved physical path against the deny floor, so a linked nested default data home whose physical alias remains agent-accessible fails closed. Dedicated lock files are opened with exclusive no-follow creation for new files, pre-open and opened-inode identity checks for existing files, and regular single-link authentication; on Windows they are then seeded with one inert byte before `msvcrt.locking` takes its byte-range lock. Metadata, stage-token, layout, script, and body inspection use only real-directory claimed snapshots; symlink or junction claims are handled as opaque names, restored or unlinked without reading metadata, writing markers, or recursively deleting through their targets. Per-target locks serialize every live auto-skill mutation (create, full-body edit, refine, pin, injection-mode toggle, archive, restore, and delete) with promotion; dashboard CRUD and toggle handlers offload these bounded waits from the asyncio event loop. An unattended update retains an in-memory SHA-256 binding to the exact staged `SKILL.md` bytes, target, base version, and original live-body digest; under the target lock it rechecks both the private claim binding and current live-body digest, leaving the candidate staged if either changed. Completion markers are created exclusively with no-follow semantics, encode the unpredictable claim name, and are recognized during recovery only when they are regular, single-link, exact-match files; pre-planted symlink, hardlink, and spoofed markers are removed as opaque names and never authorize claim deletion. Marker inspection and removal are skipped entirely for symlink or junction claim parents. If recursive cleanup leaves a completed claim behind after removing its marker, the marker is recreated and authenticated before the claim lock is released, so recovery cannot restore an already-promoted or dismissed partial candidate. Atomic no-replace renames prevent an occupied live or pending destination from being nested into or overwritten. Linux uses the libc `renameat2` wrapper when the symbol is exported; hosts whose libc predates it fail closed — candidate promotion and dismissal are refused and candidates stay staged for review. macOS uses `renamex_np`, and Windows uses `MoveFileW` semantics. Refusal restores the claim to the review queue even when metadata is malformed or valid non-object JSON; abandoned claims and committed dismissal-cleanup leftovers are recovered when the queue is listed. A writer that recreates the original pending slug cannot mutate, replace, or be deleted with the claimed candidate. Dashboard new-versus-update routing is advisory: promotion re-reads kind from the private claim before mutation, and the new-skill path refuses a claimed update so stale pre-claim routing cannot publish it under the candidate slug.
