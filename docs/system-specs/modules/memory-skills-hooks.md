...
- **Staged approval:** new skills route to `auto/.pending/<slug>/`; approve promotes to `auto/<slug>/` (dashboard: Skills → Pending review). Auto-approve for prose-only is opt-in via `approval_required=false`; it covers both new skills and prose-only updates to live auto-skills. An auto-applied update snapshots the prior version under `.versions/` for rollback. Its live-base binding hashes canonical UTF-8 text with CRLF and lone CR normalized to LF at both history staging and apply, while the candidate generation and unattended candidate binding remain byte-exact; newline spelling therefore cannot create a false drift, and a real text or candidate-byte mutation still refuses. Pending detail responses normalize script CRLF and bare CR to LF only after decoding the captured snapshot for display; stored bytes, generation hashes, journals, and publication authority remain unchanged. Script-bearing candidates and updates always require approval. When `auto_refine_on_deviation=true`, updates remain staged because the refinement writer replaces a full body without a version-aware base check.
- **Atomic candidate claim and generation publication:** update and new-skill promotion first capture the public pending generation through authenticated no-follow reads, then durably witness its digest and metadata in the held claim lock before atomically renaming the public pending directory into `auto/.private/claims/` on the same filesystem. The private root, claims, and lock directories are on the permanent agent read/write deny floor. Before a Linux namespace sandbox starts, the host opens the data home and creates/traverses every component of `skills/auto/.private` relative to pinned no-follow directory descriptors with owner-only permissions. It revalidates the lexical identity chain after each mutation and before return; a linked, swapped, non-directory, or uncreatable component refuses the spawn, so an ancestor replacement cannot redirect creation outside the masked tree and even a fresh install gives the launcher a real bind-mask target. Before any product `mkdir` or lock open, a read-only preflight resolves and pins the canonical skills directory, authenticates every existing component below it (including the `.pending` sibling), and rejects links, junctions, reparse points, and non-directories. Before a claim or publication mutation, the same preflight additionally requires both the configured and projected-or-existing canonical private roots to be on the agent-deny floor. A rejected claim preflight creates no private directory, lock, journal, evidence entry, or claim rename. Claim and lock operations independently resolve the supported operator-controlled skills root once, pin that canonical real directory, and create/open `auto`, `.pending`, `.private`, `claims`, `locks`, and `locks/claims` one component at a time while retaining every ancestor handle. POSIX creation, lock open, enumeration, unlink, and claim rename are descriptor-relative; the Windows fallback performs its by-name operation only while existing no-reparse, non-delete-sharing handles pin the complete chain, then compares the current path to the opened descriptor through native file identity. Parent identity is revalidated immediately before and after every final mutation. A validation-only path check never authorizes a later `mkdir`, lock open, or claim rename. Swapping an admitted ancestor therefore either fails closed or leaves the operation bound to the owned object, and creates no private entry in the replacement tree. The logical and resolved private roots must both remain on the agent-deny floor. Dedicated lock files require exclusive creation or an authenticated existing inode, no-follow/reparse refusal, regular/single-link identity, and a bounded authenticated payload. Immediately before the claim rename, the held lock durably records the complete generation digest and the captured metadata bytes; if that record cannot commit, the public candidate is not moved. Every in-process refusal carries that immutable record into restore, and restart recovery reads the same record (or its copy in a prepared publication journal). Restoring to the original pending slug materializes every captured file byte and mode unchanged, including CRLF metadata and the original notification flag. Only a collision that selects a different sibling slug rewrites `slug`, `name`, and `notify_suppressed` from the captured metadata; the mutable claim retains its original bytes as authenticated evidence. Recovery replaces a late metadata symlink or reparse entry without following it, and never reopens mutable `.meta.json` to decide what to restore or notify. The generation digest authenticates every path, entry kind, mode, and byte. Candidate and live generation snapshots share strict ceilings of 1 MiB per file, 8 MiB aggregate file bytes, 512 entries, and 32 relative path components. The descriptor-pinned scanner enforces entry and depth limits while consuming each directory, before retaining the next entry or opening a child; both readers reject a file from its opened size before allocating its payload, cap the read at the remaining aggregate budget, and reject an inode that grows or changes identity, size, mode, or modification time during the read. The claimed tree is observed only to reproduce the pre-rename generation digest; a mismatch from a retained descriptor or by-name replacement refuses the claim. Metadata, layout, body, and scripts are validated and redacted from the pre-rename immutable snapshot, and publication never re-reads mutable candidate inodes. New skills are materialized as a fresh private tree. Updates first capture the complete live generation through a pinned no-follow root and descriptor-relative descendants into one immutable opened-inode snapshot. Links, hardlinks, special files, ancestor substitutions, and listing/open identity changes refuse before target bytes are read; Windows retains the fallback by holding no-reparse directory handles for the complete walk, opening leaves with the no-reparse file primitive, and comparing the current path's no-reparse handle to the already-opened descriptor by native volume serial plus 128-bit file ID. That identity is independent of 8.3 versus long path spelling. Reparse attributes still refuse, regular files still require one link, and the opened inode's size/mode/mtime stability checks still run before bytes are accepted. The private stage is materialized only from that snapshot before applying the rewritten body, version snapshot, validated scripts, and preserved file/directory modes. Every staged file and directory is flushed before the held claim lock records a format-2 prepared journal whose before/after digests cover every relative path, entry kind, permission mode, and file byte. Windows file flushes use writable handles, while every platform retains no-follow open plus lstat/fstat identity checks. An update publishes only by renaming the complete live tree to its claim-specific private backup and renaming the complete stage live; a new skill uses the second rename only. At that mutation boundary the captured backup must still equal the authenticated before digest, and it is checked again after the stage swap for retained-handle writes. Drift restores the concurrent generation without replacing a newer by-name writer and re-queues the candidate only after the prepared journal is durably returned to active state. No script, mode, snapshot, or frontmatter/version file is mutated piecemeal in the live generation. Restart recovery takes the target lock and consumes a claim only when the complete live tree equals the immutable after digest. An exact before tree is re-queued once; a crash between the two update renames restores the exact private backup before re-queuing; a partial, mutated, or otherwise unrecognized live generation remains in place with its claim and private artifacts retained as ambiguous evidence. Any state that cannot be proved remains private rather than risking duplicate application or accepting mixed scripts. Legacy body-only journals may prove a pre-publication rollback but can never prove whole-tree completion. Completion markers remain a no-follow in-tree compatibility record, while the lock's completed state survives partial recursive cleanup; publication reports success only after at least one independently recoverable consumption record exists. A failed marker write leaves the prepared journal untouched for restart classification; a failed completion-state write is covered by the authenticated marker. Private stage/backup trees are removed before the completed claim. On Windows a validated lone regular file whose read-only attribute blocks unlink is opened no-reparse and identity-checked, has only that attribute cleared, is revalidated under its pinned parent, and is then unlinked; a changed object still refuses cleanup and remains private. Per-target locks serialize every live auto-skill mutation with promotion. Atomic no-replace renames prevent occupied live or pending destinations from being nested into or overwritten; unsupported filesystems fail closed and retain the candidate. The refusal directs the operator to move `KIROCREW_HOME` to a local filesystem supporting `renameat2(RENAME_NOREPLACE)` on Linux or `renameatx_np(RENAME_EXCL)` on macOS before retrying. Links or junctions inside the skills tree are refused with the supported relocation alternative: link the complete data-home root instead, leaving every component from `skills/` downward real. A refused in-process promotion materializes the immutable pre-claim tree into a claim-specific private restoration stage, rewrites only its captured metadata bytes for the selected pending slot, and atomically renames that fresh generation into pending. It never rewrites or republishes the mutable claimed tree. After the immutable restoration stage is flushed, the held claim journal records its pending slug and complete-tree digest before the no-replace pending rename. Recovery treats that receipt as authoritative: an exact pending generation means requeue already committed, while an exact private restoration stage with an absent destination is renamed once; a missing/mismatched stage or occupied different destination stays private and is never requeued from mutable claim bytes. After the immutable generation is pending, the mutable claim follows one retention policy whether its bytes still match or a retained writer changed them. While the claim lock is held, its authenticated JSON journal transitions from `claimed` or `prepared` to `evidence` without discarding the captured generation, publication fields, or restoration receipt. The claim directory is then atomically renamed without replacement from `claims/<claim>` to `evidence/<claim>`. A crash after the journal transition but before the rename leaves an active claim whose `evidence` state makes restart recovery complete only that move; it can never requeue or delete the tree. Evidence entries are outside active-claim enumeration, remain agent-denied and discoverable by the same claim name, and keep their authenticated lock journal. Claim-lock cleanup refuses any name present in either the active claims or evidence namespace. No code path claims atomic deletion of a mutable retained-writer generation. A writer that recreates the original pending slug cannot mutate, replace, or be deleted with the claimed candidate, and stale dashboard new-versus-update routing is revalidated from the private claim before mutation. Candidate metadata booleans are enabled only by the literal JSON value `true`; strings such as `"false"` never opt a prose-only candidate into script handling.
#### Prepared claim recovery ownership table

Recovery classifies one immutable filesystem snapshot while holding the target lock. `before` and `after` below mean exact complete-tree digests from the authenticated format-2 journal; `other` includes partial trees and concurrent edits. Only an exact journal-owned generation authorizes cleanup or mutation.

| Kind / format | Live target | Private stage | Private backup | Result and permitted action |
|---|---|---|---|---|
| Any format 2 | `after` | any | any | Committed (`True`). Consume the claim; never rewrite live. A later concurrent edit remains live. |
| Update format 2 | `before` | absent or `after` | absent or `before` | Rolled back (`False`). Live is already the authenticated old generation; clean only the accepted private artifacts and requeue once. |
| Update format 2 | absent | `after` | `before` | Restore `before` with no-replace rename, then return rolled back. If a concurrent writer recreates live first, the rename refuses and the state becomes ambiguous. |
| New format 2 | absent | `after` | absent | Rolled back. Remove the owned stage and requeue the candidate once. |
| New format 2 | any existing value other than `after` | any | any | Ambiguous (`None`). Preserve the live target, claim, and stage. |
| Update format 2 | `other` | any | any | Ambiguous. The live tree has no recovery ownership; never evacuate it into a disposable artifact or replace it with `before`. Preserve all evidence. |
| Any format 2 | any required digest absent, mismatched, linked, reparse, hardlinked, or unreadable | any | any | Ambiguous. Preserve all evidence. |
| Legacy format 1, new | absent | n/a | n/a | Rolled back; otherwise ambiguous. A body-only record cannot prove publication. |
| Legacy format 1, update | live body equals `before` and its optional orphan snapshot is absent or the same body | n/a | n/a | Rolled back after removing only that matching orphan; otherwise ambiguous. |

The live-missing update restore is a compare-and-swap: authenticated `backup == before`, authenticated `stage == after`, and destination absence are all required, while `rename_no_replace` is the final atomic check. There is no check-then-replace path. This both recovers the old generation when no writer exists and preserves a writer that wins the destination race.

- **Scripts:** deterministic procedures may ship a validated **Python** helper (`generate_scripts`, default true); statically validated (regex denylist + AST policy: no dynamic exec/import, destructive fs, process exec, network egress, ≤4 KB) and re-validated at the approve choke point. A candidate keeping **at least one valid** script stays staged for mandatory human review. If every supplied script is rejected, approval-disabled mode drops the candidate rather than auto-applying its prose, while approval-enabled mode keeps the prose staged for a human to inspect.
...
5. **Namespace lock** — `update_auto_skill()` refuses to touch any skill whose name doesn't start with `auto/`, preventing the refine path from ever clobbering hand-authored skills. The bare `auto` namespace and its case-fold or trailing-dot/space filesystem aliases are refused before a generic live mutation can recurse into the namespace; every such refusal emits the same `skill_mutation` permission event with `outcome=denied` and `reason=reserved_auto_namespace` before returning.
