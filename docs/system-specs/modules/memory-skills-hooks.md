...
transition, so an automation can react to a board lane transition without polling.
...
for anything but a refusal it could make no decision about. Each writer calls
`_lane_dispatch_is_permitted` itself and simply does not dispatch on a refusal: the PUT
writer returns early, and the deletion writer folds it into the condition that also owns
the folder cleanup, so a refusal there cannot skip work unrelated to hooks. The gate stays
in the dashboard writer rather than moving into `hooks` because it reads `request["app"]`,
which `hooks` has no business seeing, and because moving it would re-order the audit
relative to the no-op guard that keeps a label-only edit from auditing a refusal it could
never fire. The cost of the argument being gone is that a FIFTH writer of lane membership
could forget the call; the writer table below names every writer that exists so that
omission is visible by inspection.
...
NOT silent to audit: the gate records the permission DECISION on BOTH branches under
`operation="hooks.session_lane_changed"` — `denied` naming the app, and `allowed` for
the dashboard caller — so a refusal is distinguishable from an app that simply never
changed a lane, and a permitted dispatch is not invisible merely because a hook that
actually runs is audited again at dispatch. The check is
...
its own prior snapshot to answer "was Done just added?", which relocates the
polling problem rather than solving it.
...
**The trailing `;` is load-bearing, not decoration.** Matching is `fnmatch`, so
without a terminator a selector written for a short id ALSO matches every longer
id it prefixes -- `*added:abc*` fires on `added:abcdef` -- and the hook that runs
belongs to a different lane. Two real ids are the same length and cannot prefix
each other, but `tags.json` is hand-editable, which is the same path the token
validator guards. `;` is rejected inside an id for that reason: otherwise an id
could forge its own boundary. That validator is an ALLOWLIST (`[A-Za-z0-9_-]+`)
rather than a screen for those separators, because matching is `fnmatch`: an id of
`*` would make the selector written for it (`*added:*;*`) match EVERY lane change
and run that tag's hook on sessions it was never registered for.
...
- **Dispatch is off the request path.** Deltas go onto ONE bounded FIFO queue
  ...
  **The v1 delivery bar is deliberately at-most-once and in-memory.** A gateway
  ...
  its own change. Until then a missed fire is possible, so a correct subscriber
  re-reads live state. A slow hook delays every later
  fire, bounded by the 1-300s hook timeout. That residual delay is tolerated by this
  event's at-most-once contract, which requires a subscriber to reconcile against the
  board rather than trust the stream.
  **Subscribers resolve at DRAIN, from the live store, as every other event does.** A
  ...
  same window does not — both within this event's at-most-once contract, which already
  tells a subscriber to reconcile against the board rather than trust the stream. An
...
- **The lane permit gate audits BOTH branches, and is only reached when a subscriber
  exists.** `_lane_dispatch_is_permitted` records `outcome="allowed"` for the dashboard
  caller and `outcome="denied"` for a caller carrying no dashboard claim, both under
  `hooks.session_lane_changed`, so no permitted dispatch is silent. A denials-only
  variant was carried and withdrawn: it left the allow arm returning `True` with no
  record, which is an un-audited decision on a governance gate. The cost that variant
  was reaching for is instead paid at the two writers, which skip the gate entirely
  unless an enabled `SessionLaneChanged` hook is registered — so the default shipping
  state, where none is, accrues no rows at all rather than one per lane drag. That
  check reads the store's in-memory list and never resolves `capabilities.script_hooks`,
  because resolving it walks `profiles/` and must not happen on a tag write.
- **Only ids are tokenized, and an id must match the `[A-Za-z0-9_-]+` allowlist, so nothing user-controlled reaches the grammar.**
