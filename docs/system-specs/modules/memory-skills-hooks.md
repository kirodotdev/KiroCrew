...
- **Session isolation for the daily-history activity log (issue #655):** at session start, `build_session_context()` calls `MemoryStore.get_context(include_daily_history=False)`, so the `## Recent History` daily log is NOT injected into a new session's context. That log is a per-workspace aggregate of every session's activity with no per-session attribution, so injecting it let a fresh session recall a task that was given in a different, separate chat session. Preferences, projects, semantic memory, and lessons are still injected (preferences and lessons are global BY DESIGN). The daily history stays on disk and remains reachable via the dashboard Memory tab and the memory-search tools when the user explicitly asks to get up to speed. `get_context(include_daily_history=True)` is the default, so direct/API/suggestions/tips callers (`dashboard/handlers/memory.py`, `suggestions.py`, `tips.py` — which use `read_recent_history` directly anyway) are unaffected.
