...
(`app-sdk/ChatEmbed.tsx` the second, wired to a one-row `<textarea>` via the same
`submitOnEnter`/`isComposing` and attaching `textareaRef` for the hook's bounded
auto-grow), and which owns four invariants this file must not re-derive:
