...
- The default golden set is **v2** (68 docs / 46 queries). v1 (18 / 12) is still packaged so archived v1 reports stay reproducible, but it could not discriminate: every class scored 1.000 recall under both a keyword-only and a semantic retriever, because each gold document was the only one in the corpus using its topic's vocabulary, so matching one term was enough to win. v2 adds competing distractors — same-topic documents that differ on the decisive attribute (a different service, environment, cache, or time window) — and the legs then separate. Keyword-only is deterministic (FTS5 + graph, no model) and reproducible: nDCG@3 0.834 / MRR@3 0.804 / MAP@3 0.781. The semantic leg measured with `qwen3-embedding:0.6b` gives nDCG@3 0.917 / MRR@3 0.887 / MAP@3 0.885 — re-measure rather than trust that triple after a model or quantization change, since it moves with both. MAP is the widest of the three separations between the legs (0.104, against 0.083 nDCG and 0.083 MRR): it is the only one of them that reads how late the *second* gold document lands, so it sees the multi-gold classes that MRR — which stops at the first hit — scores identically. Its denominator is `min(|gold|, k)` rather than the textbook `|gold|`, matching the truncation `ndcg_at_k` already applies, so the cut-off must travel with the number and it is not directly comparable to a published MAP over full gold.

Gold labels are blind-audited, not author-asserted. Three annotators on separate model families relabelled all 46 queries from an anonymized packet — document ids masked to `D01`-`D68` (eight real ids name their own answer), query classes withheld, gold sets *and their sizes* withheld, corpus order shuffled — and agreed with the file on 43, 42 and 40 of 46, with 39 unanimous. Two labels the audit overturned are corrected: `q-multi-5` drops `d-mfa-baseline` (the remaining `d-vpn-access` states the VPN and MFA requirement together, so it is self-sufficient) and moves to `clean_fact`, since a single-document lookup sitting in `multi_hop` was inflating that class; `q-reinforcement-1` drops both audit documents, because corroboration is not requirement and the baseline answers the question outright. Both corrections raise `recall_all`/`recall_micro`, and `multi_hop` becomes a harder class once the single-document lookup leaves it, which widens the keyword-versus-semantic separation rather than narrowing it. Agreement is evidence the surviving questions are unambiguous, **not** proof the labels are correct: every annotator was a language model, so a shared blind spot reads as consensus. Five queries where the annotators disagreed with each other remain open, as does external anchoring of the relevance judgments. The coarse separations are the stable evidence: `multi_hop` recall_all@3 0.250 keyword versus 1.000 semantic, and at k=1 neither leg is saturated (recall_any@1 0.650 versus 0.775), which is what makes that cut-off informative. `abstention_rate` is now measured over 6 queries rather than 1; it still reads 0.000 because bare `HybridRetriever.search` has no score floor, so the store never abstains. **A v1 report and a v2 report measure different corpora and must not be differenced** — and nothing mechanical stops it: `bench kb-retrieval` prints its report and writes no file (it has no `--out-dir`), while `bench compare` only diffs saved memory-retrieval reports and never sees a KB run. The one guard is the corpus identity in the printed header, and the corpus *name* alone is not enough of one: the two audit corrections change v2's gold labels under the unchanged name `kb_golden_v2`, so a print scored against the original labels (keyword nDCG@3 0.825, `multi_hop` recall_all@3 0.400) and one scored against the corrected labels (0.834, 0.250) differ by a labelling change, not by retriever movement. The golden set therefore carries a `label_revision` integer (`1` for v2; absent on v1, whose labels are the author's originals) that the header prints beside the name — `KB retrieval eval: kb_golden_v2 (label revision 1)`, or `(unrevised labels)` when the field is absent — and a print made from the original v2 labels reads the bare name. Two prints are comparable only when both name **and** revision match; read both before comparing two of these numbers, and bump `label_revision` with every gold-label edit under an existing name.
