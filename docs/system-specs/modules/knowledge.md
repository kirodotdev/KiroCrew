...
- **A bundle carries per-document ownership** — `export_all` / `import_bundle` round-trip `folder_file_state`, `artifact_item_state` and `agent_item_state` (the `_DOC_STATE_TABLES` registry) alongside the items they own. An item restored without its ownership row is permanently unmanaged: no group label in Sources, no replacement on a later ingest of the same document, and indistinguishable from the residue of an interrupted ingest that the aggregate-source sweep exists to reap. They are imported last, after the sources and items they reference. **A namespace-scoped export narrows them.** A state row names its items in a JSON `item_ids` column that no foreign key reaches, so shipping the rows whole would carry ownership of items the bundle does not contain — which is worse than no ownership, because `ingest_artifact` short-circuits on "hash unchanged AND still holding its items" and a row pointing at absent items satisfies both, silently skipping the document forever in the restored store. A row therefore survives a scoped export only if some of its items do, narrowed to exactly those: dropping it outright would land the exported half *unowned*, which is the state the residue sweep reaps. A row owning nothing (a deduped marker holds a hash claim, not a group) has no item in a scoped bundle and is scoped out, but a full export still carries it. An unreadable group passes through untouched — it cannot be narrowed, and dropping it could unown items that are in the bundle. Columns are intersected with the live schema at import, so a bundle written by a different schema version restores the columns both sides share instead of failing the whole import, and a bundle predating ownership simply contributes none.
- **An export is one coherent snapshot** — `export_all` reads items, entities, relations, sources, locations, mentions and the three ownership tables inside a single transaction. The connection is autocommit, so without it each statement takes its own snapshot and an ingest committing between two of them tears the bundle across tables: items read before the write, ownership read after. The restore then holds ownership naming item ids the bundle never carried, and `ingest_artifact` short-circuits on "recorded hash matches AND the row still holds a group" *without checking the group survived* — so the document is treated as already indexed while searches answer from the older copy. The scoped path tears the other way, dropping ownership for items the bundle does carry and landing them unowned. WAL lets writers continue against the reader's snapshot rather than blocking on it, and the whole export already runs in one `asyncio.to_thread` hop, so every read uses the same thread-local connection.
- **Transient per-document state never crosses the bundle boundary** — a row in `TRANSIENT_DOC_STATUSES` (currently `ingesting`) is not ownership metadata but *authority to delete*, granted by an interruption on one specific machine. `export_all` withholds those rows and `import_bundle` refuses them, so a backup taken while an ingest was in flight cannot hand a restored store a licence to reap knowledge that store's own ingest never produced. Timestamps cannot substitute for this: import preserves each item's original `created_at`, both it and the intent are naive local time, so a bundle from a host running ahead restores items dated after a foreign intent. The accepted cost is that an export racing the window between item commit and ownership finalize carries that residue with nothing able to attribute it — so the restored copy is preserved rather than reaped, the same guarantee any unattributable residue gets, and the safe direction. The source machine keeps its own row and still repairs itself.
