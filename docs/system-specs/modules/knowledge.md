...
- Fuzzy filename matching compares valid ISO dates with their year intact: the month/day suffix is not also parsed as a yearless date. Reports dated a year apart therefore remain distinct even when their embeddings are nearly identical. Malformed ISO-shaped tokens remain available to the generic numeric-date guard, and genuinely separate numeric dates in the filename still participate in the closest-date comparison.
