...
`knowledge.extraction_effort` → `agent.role_efforts.background` → fallback
(`DEFAULT_EXTRACTION_EFFORT` = `high` as the last resort). An invalid or
typed-wrong key falls through to the role chain rather than the hard default, so
a typo cannot silently raise cost. `""` means inherit; the key is
...
both effort and pool size still require a worker restart to reach the live
workers). Both pools drive the same `kirocrew-knowledge` agent and preserve the
...
`/effort` command, gated first by `model_supports_effort` on the model the
session actually serves (`_resolved_model_id`, falling back to `_model`) so an
unsupported model (`auto`, non-reasoning families) skips the push with an INFO
log instead of warning per spawn; Claude ACP keeps its `supports_config_option`
gate and advertised-level descent (the ladder arithmetic is the shared
`select_effort_level` helper from `kiro_crew.effort`). A rejected command still
