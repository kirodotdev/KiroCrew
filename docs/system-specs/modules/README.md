...
| [backend-authoring.md](backend-authoring.md) | The operator guide to config-defined backends: what `harnesses.json` is, why an agent cannot write it, the full field and validation reference, routing as the selectability gate, the edit→restart lifecycle, and per-chat / per-subagent backend selection. |
