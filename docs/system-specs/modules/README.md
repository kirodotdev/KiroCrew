...
| [connector-asana.md](connector-asana.md) | The Asana connector (`W08`): network-free vendor-protocol logic — field rules (opaque GID, `due_on`/`due_at` separation), offset/limit pagination and the legacy truncation ceiling, Asana's HTTP+JSON error taxonomy with the hard cross-workspace denial, no-idempotency attempt-scoped readback and batch partial failure, and the non-interchangeable MCP-vs-REST authorization surfaces. |
