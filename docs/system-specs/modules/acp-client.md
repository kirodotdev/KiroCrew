...
Last Updated: 2026-07-13 (default model 4.8, session_provider module, AcpAuthRequired/AcpPromptBusy exceptions, KIROCREW_KIRO_BIN override, audit_source; verdict-driven liveness-oracle watchdogs on the session handle (acp/liveness.py, watchdog.* config), model-substitution advisory survival, per-turn kiro billing credits, model-registry context-window backfill)
...
- `_resolve_kiro_bin()` checks the `KIROCREW_KIRO_BIN` env override first (an explicit path for environments where the shim is missing/broken), then `~/.toolbox/bin/kiro-cli` and `~/.local/bin/kiro-cli`, before falling back to `shutil.which()` — avoids shell wrappers/aliases. `runtime.py` imports and reuses this same resolver (`from kiro_crew.acp.client import _resolve_kiro_bin`) rather than duplicating a bare `shutil.which()`, so both the per-session client and the multiplexed runtime resolve the binary identically.
