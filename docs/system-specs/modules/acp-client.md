...
- A **sandbox refusal is diagnosed, not swallowed.** `wrap_argv` fail-closing raises the typed `SandboxUnavailableError`, which is caught ahead of the generic handler (it subclasses `RuntimeError`, so the untyped branch would otherwise absorb it) and answered with `code: "model_list_sandbox_unavailable"` plus the error's `kind` and redacted `detail`. The status stays 503 so the degraded-path contract is unchanged — a `transient` denial still self-heals on the 8s poll — while the presentation layer gains what it needs to select the right remedy copy: `no_backend` names the `agent.sandbox_allow_unsandboxed_exec` opt-in (the Windows case, where the picker otherwise silently offers only `auto`), `transient` must not advise weakening the sandbox, and `foreign_sandbox` means this host's sandbox is healthy. Without the typed branch an unsupported host is indistinguishable from a cold-start blip.
