...
- `"opencode"` (`ACP_BACKEND_OPENCODE`): `opencode acp --cwd <work_dir>`. `_resolve_opencode_bin` checks the explicit `OPENCODE_BIN` override, `mise which opencode`, the official `~/.opencode/bin/opencode` location, then the augmented daemon `PATH`. OpenCode uses a separately sandboxed per-session client and is not in the shared-runtime, session-sharing, steer, Kiro-identity-store, or internal-sandbox capability sets. The Kiro-only API key is stripped from its environment. Because OpenCode does not read Kiro agent specs, `session/new` and `session/load` receive only the portable projection of fully exposed Kiro Crew-managed MCP servers; user/app servers and any mount that cannot retain its tool narrowing are omitted rather than broadened.
...
Inline permission arguments accept ACP's `rawInput` field as well as the legacy
`input`/`params` spellings. OpenCode sends its structured permission metadata in
`rawInput`; it is retained for display and argument-derived deny checks, but a
cache miss still leaves `raw_params_trusted=False`.

OpenCode does not emit Kiro's `_meta.kiro` identity. For an OpenCode permission
whose kind is `other`, the direct client may recover `<server>_<tool>` from the
non-model-authored tool registry key in `toolCall.title`. Recovery is restricted
to unique, non-overlapping prefixes derived from managed servers actually sent
in that session's `mcpServers` array. A malformed key, an unknown server, a
sanitization collision, or an overlapping prefix yields no
`mcp_identity_trusted` grant.
...
- `protocolVersion` in the `initialize` request: kiro-cli expects the date string `"2025-08-22"`; claude-agent-acp and OpenCode expect integer `1`.
- Claude and OpenCode skip Kiro's `session/set_mode` and use `session/set_config_option` (`configId: "model"`) instead of `session/set_model`.
- OpenCode advertises models through the `configOptions` entry whose id is `model`, rather than the optional `models.availableModels` object. The client converts those options to the common `{modelId, name, description}` snapshot and refreshes it after a config-option update or model switch. An explicit unadvertised selection raises `AcpModelUnavailable`; an inherited stale selection is withheld and the backend default remains active.
- OpenCode owns its session store. Resume sends `session/load` without probing `~/.kiro/sessions/cli` or attaching private `_kiro.dev/session_file` metadata, and accepts a successful response containing `configOptions` even when it omits Kiro's `modes` object.
- OpenCode receives the same managed MCP projection on new and resumed sessions. The projection and its identity-prefix snapshot are rebuilt together so an old session cannot inherit trust for a server that was not re-registered.
