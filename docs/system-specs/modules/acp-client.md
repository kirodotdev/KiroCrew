...
- `"codex"` (`ACP_BACKEND_CODEX`): the official `@agentclientprotocol/codex-acp` adapter. It is operator-installed rather than a project dependency and resolves through `mise which codex-acp`, mise Node installs, then the augmented daemon `PATH`; on Windows an npm `.cmd` launcher is resolved through the package manifest to `[node.exe, <entry script>]` because CreateProcess cannot execute the batch shim directly. A missing adapter reports `npm i -g @agentclientprotocol/codex-acp`. Codex uses protocol version `1` with fail-closed filesystem and terminal client capabilities, runs on the dedicated `AcpClient` path (no shared `AcpRuntime`, steering, or session sharing), and remains wrapped by Kiro Crew's OS sandbox. `session/new` and `session/load` receive deterministic ACP MCP descriptors built directly from Kiro Crew's managed server registry; pooled stubs override same-named managed descriptors, opt-in servers remain excluded, and no Kiro agent spec is read or written. Resume uses the adapter session id plus `cwd` and MCP descriptors, with no Kiro transcript probe or `_kiro.dev/session_file` metadata. Models and reasoning effort are taken only from the live session's advertised `availableModels` and `reasoning_effort` config option; `auto` means inherit the adapter default and never invents a concrete model. Slash commands use `session/prompt`. Authentication failures are translated to `AcpAuthRequired` with `codex login` remediation. Selecting Codex never runs Kiro prerequisite/auth checks, Kiro usage polling, Kiro identity-store reads, Kiro model listing, Kiro settings overlays, or Kiro agent-spec readiness.

`SessionManager` normalizes and snapshots `agent.acp_backend` when the gateway
starts. Saving another backend changes the persisted target only; live safety
gates, background dispatch, and provider-factory refreshes continue to use the
running snapshot until the gateway restarts. A model or reasoning-effort refresh
therefore cannot hot-switch newly created sessions onto a pending backend.
...
Background one-liners use that shared runtime only when the running backend is
in `ACP_BACKENDS_ACP_RUNTIME`. Codex is deliberately absent: its `_bg` work is
created through the configured provider factory and wrapped by
`_ProviderBgSession`, so it never falls through to an `AcpRuntime` that would
silently launch Kiro instead.
...
- **Poll-driven spawn sites are readiness-gated.** `kiro-cli` auto-launches an
  ...
  readiness gates. Whether the gate applies is decided from
  `request.app["state"].sessions.acp_backend`, the immutable running-backend
  snapshot. It never reloads the persisted target: a pending Kiro-to-Codex switch
  must keep Kiro's guards active, while a gateway restarted onto Codex must not
  inherit stale Kiro checks. Missing or malformed runtime state fails closed.
  Ordinary sends are ungated because a failing ACP attempt
