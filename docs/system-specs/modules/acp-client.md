...
Last Updated: 2026-07-20 (edition-neutral pre-sandbox executable resolution; `/api/models` now checks subprocess exit status, bounded redacted stderr, empty output, JSON validity, and payload shape before returning 200. Prior: default model 4.8, session_provider module, AcpAuthRequired/AcpPromptBusy exceptions, KIROCREW_KIRO_BIN override, audit_source; verdict-driven liveness-oracle watchdogs on the session handle (acp/liveness.py, watchdog.* config), model-substitution advisory survival, per-turn kiro billing credits, model-registry context-window backfill)
...
- `_resolve_kiro_bin()` checks the `KIROCREW_KIRO_BIN` env override first (an explicit path for environments where the normal launcher is missing or broken), then resolves `kiro-cli` from the shared augmented PATH. `runtime.py` imports and reuses this same resolver rather than duplicating a bare `shutil.which()`, so both ACP transports select the binary identically. Immediately before OS sandboxing, `sandbox.py` routes argv[0] through the edition-neutral `PlatformContext.agent_executable` resolver; the public Default is identity and a companion can return a direct executable behind an edition-managed launcher without changing the core.
