...
- Sensitive paths: `~/.aws`, `~/.ssh`, `~/.gnupg`, `~/.gpg`, `~/.config/gcloud`, `~/.azure`, `~/.docker/config.json`, `~/.kube/config`, `~/.npmrc`, `~/.pypirc`, `~/.netrc`, `~/.git-credentials`, `~/.kiro/crew/.env`, `~/.kiro/crew/sel_hmac.key`, `~/.kiro/crew/trust`, `~/.kiro/crew/security_events.jsonl`, `~/.kiro/crew/sel` (the sealed-segment directory), `~/.kiro/crew/app_admission.json`, `~/.kiro/crew/run`
...
- **SEL sealed segments on the floor** — rotation seals the active log into
  numbered segments under `~/.kiro/crew/sel/`, each holding real audit history
  including the oldest evicted-prefix evidence. That directory is a
  `_CREW_SECRET_LEAVES` entry, and because the membership test treats a registered
  path as a subtree (`cand == base` or `cand.startswith(base + os.sep)`),
  `is_sensitive_path()`, `is_sensitive_write_path()` and every bash matcher (POSIX,
  Windows-native, and the relative-traversal symlink-staging matcher) block every
  member — including a segment number no release has emitted yet. That last property
  is why the directory replaced a prefix-family regex: the family had to be injected
  into four separate matchers and exposed through a second accessor with different
  semantics, and it could only ever cover names that already existed. Two consumers
  (the LaTeX hide-set and the posture surface) had already missed it. The entry is
  expanded over the same `_CREW_HOME_PREFIXES` as the other leaves and resolved
  through the same `_home_dir_targets()` helper, so it inherits both-homes coverage
  AND the `KIROCREW_HOME` re-anchoring rather than hardcoding a home-relative
  literal. `sel_hmac.key` lives under `trust/` and stays its own entry.
