...
- **Pass 1 was quadratic in command length; two independent fixes bound it.** The matcher's verb/redirect-anchored branch carried a leading `.*` on all four of its alternatives. A **clean** command (the common case, and the only one that runs to completion, since a match returns early) cost 1.09s at 9KB, 16.03s at 37KB, 65.11s at 75KB: 4x per doubling. `is_sensitive_bash_command` is called synchronously on the event loop from `_resolve_permission`, so that was a loop wedge and the stall watchdog's dump-then-exit — past `LOOP_STALL_EXIT_AFTER_DEFAULT` (25s) at ~45KB of command, past the managed 90s budget at ~87KB. Compounding it, pass 1b re-scans one collapsed variant per separator spelling, and `_SEPARATOR_RUN_RE` matches **any** doubled separator, so the `//` in an ordinary URL tripled the cost (16.06s vs 4.58s for the same 37KB command with a single slash).
  - **The redirect alternative's `.*` was removed** (`.*[<>|]\s*` → `[<>|]\s*`), which is the same redundancy the eleven verb-independent branches had removed earlier: the pattern is only ever used via `.search`, which already retries at every offset, so `.*[<>|]\s*P` matches a subject exactly when `[<>|]\s*P` does. This was the **dominant** term — unlike the three verb alternatives beside it, which only enter where a verb actually matches, it entered at every offset unconditionally. Verified match-set identical, including every shape it exists for (`>~/path`, `>>`, `2>`, `<~/path`, `|~/path`, a piped `tee`), which no other branch matches. The three verb alternatives keep their `.*`, spanning the meaningful gap between verb and path; they cost O(verb occurrences × n) and measure negligible even on a subject that is nothing but `cat` invocations.
  - **Pass 1 and pass 1b additionally run behind a cheap superset pre-filter** (`_build_sensitive_prefilter`), so a command naming no fenced path is not scanned even once, and the URL multiplier disappears with it. The pre-filter is the alternation of the same branches with each leading anchor **removed**, and the `.*`-carrying branch dropped entirely — sound because that branch's tail is character-for-character the tail the verb-independent branch carries, so it cannot match anywhere the pre-filter misses. Both patterns are returned from one `_build_sensitive_patterns()` call and cached together by `_build_sensitive_cache()`, so the pre-filter cannot describe a stale pattern or a stale `Path.home()`. Contract is `_might_contain_credential`'s: may match where the pattern would not, must never fail to match where it would, so **only the cost moves, never the verdict**.
  - The two are complementary: the pre-filter removes the common case, the `.*` removal bounds the worst case (a subject the pre-filter admits but the pattern rejects — a fenced path glued to a name character, with no verb or redirect anywhere — reaches the scan by design, and was still 4x per doubling with the `.*` in place). Measured after both: clean commands 37KB 16.03s → 0.387s and 75KB 65.11s → 0.733s; the adversarial near-miss 0.016s/0.028s/0.055s/0.117s at 2.6/5.4/11/23KB. Both linear. Beyond ~49KB such a command is refused outright by the pre-existing traversal-analysis budget in pass 4, which is fail-closed.
