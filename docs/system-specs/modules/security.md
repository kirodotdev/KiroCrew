...
- **Crew roster serializers** (`dashboard/handlers/_shared.py::redact_record_strings`, #8447): free-text string fields on the crew/agent record (`description`, `triggers`, `workspace`, `memory_store`, …) are agent-writable through `config.json`, and two dashboard endpoints echo that record back to the browser — `GET /api/agents` (`agents.api_kirocrew_agents`, a full `dataclasses.asdict` spread covering BOTH row sources: the `cfg.agents` rows and the project-scope default rows, plus the top-level `default_agent` string, which is the same class of agent-writable config value) and `GET /api/members` (`members.api_members`, an explicit allowlist that stays an allowlist; only its VALUES are scrubbed). Each serialized record funnels through `redact_record_strings`, which applies `redact_exfiltration_urls` then `redact_credentials` (the `members.py` `_sanitize` order) to every string value AND every string dict key (a key serializes into the JSON exactly like a value), recursing into nested dicts/lists so a field or nested structure added to the record later is covered with no code change at either endpoint. Scope is exactly these two endpoints — this chokepoint does NOT cover the wider config surface (e.g. `GET /api/config`-style raw-config reads), and `members.py`'s transcript `last_message` preview keeps its own pre-truncation `_sanitize` pass (already correct; not double-wrapped). Because the crew edit sheet prefills its form from this now-redacted roster and PUTs the prefill fields back unconditionally on save, the write-back path is guarded at three layers: (1) each `/api/agents` row carries a `redacted_fields` manifest (`_shared.py::redact_record_strings_marked`) naming the fields redaction changed; (2) the crew edit sheet omits an untouched redacted prefill from its save (`website/src/utils/redactedPrefillGuard.ts::omitUntouchedRedactedPrefills`, keyed on the manifest, never on marker text) — the client-side omission is what closes the stale-save race, since a server-side comparison at save time cannot tell a stale marker echo from a deliberate edit after a concurrent write; (3) `PUT /api/agents/{name}` drops any body field whose value is exactly the redacted ECHO of the currently-stored value or is a PURE redaction marker (`agents.py::_strip_redacted_echoes`, run before field validation on a pre-lock probe and again against the locked record) as the floor for clients that do not implement omission — a bare marker is never a deliberate stored value, while a deliberately typed new value, including text merely containing a marker alongside other text, writes normally. Residual, stated rather than implied: a stale echo of a PARTIALLY-redacted value (marker embedded in surviving text) from a client that ignores the manifest can still write after a concurrent change — indistinguishable from a deliberate edit at the server. `test_roster_record_redaction.py` pins both halves per endpoint, the project-row source, the nested-value recursion, dict-key coverage, the manifest, and the write-back guards; `redactedPrefillGuard.test.ts` pins the client omission
