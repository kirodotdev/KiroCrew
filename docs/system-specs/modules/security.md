...
**Pre-exec hardlink scan (Linux launcher, Step 7)**: bind-mounting an empty dir over `~/.aws` hides the *path* but not the *inode* - a hardlink planted OUTSIDE the namespace before spawn keeps the credential inode reachable through the link, so a sandboxed agent could read it back. Before `exec`, the launcher walks the agent CWD and `/tmp` for files with `st_nlink > 1` whose `(st_dev, st_ino)` matches a protected credential inode and refuses to `exec` (`sandbox: BLOCKED`) if any is found. This scan is the sole control for that specific "pre-planted hardlink survives the bind-mount" vector. The walk logic lives in the module-level, unit-tested `_scan_credential_hardlinks(scan_roots, protected_inodes, max_scan)`; its source is injected verbatim into the launcher (`inspect.getsource`) so the stdlib-only subprocess and the tests share one implementation. It returns `(dangerous_links, truncated_roots)`.

**Budget: per-root, not shared.** Each scan root gets its OWN independent `_HARDLINK_SCAN_BUDGET` (10000 files) of `lstat` calls. Previously a *single shared* counter was consumed by the CWD first, so a large worktree could zero out the budget and leave `/tmp` - the shared, world-writable root the check exists to guard - never scanned at all. Per-root budgets close that bypass.

**Truncation is observable and the decision is a deliberate escalate, not a silent pass.** Previously, exhausting the budget just `break`-ed and fell through to `exec` indistinguishably from a clean scan - a silent fail-open a long-lived host reaches on its own (a `/tmp` that has accumulated >10000 unrelated files), with no attacker action and no log line. Now a truncated root is returned in `truncated_roots` and the launcher writes a distinguishable `sandbox: WARN hardlink scan INCOMPLETE for root=<r> (examined=<n> budget=<b>)` line to stderr (captured by the parent gateway) before proceeding. The control **escalates (loud, audited fail-open)** rather than **failing closed**: failing closed would convert an environment condition into a guaranteed agent-spawn outage on every launch for any host with a large `/tmp`, which operators would defeat by disabling the sandbox entirely - a strictly worse posture. A legitimate user with a genuinely huge `/tmp` still spawns normally and sees one WARN line per truncated root. Deferred hardening the issue notes: a per-session namespace-private `/tmp` (removes the `/tmp` half of the scan) and a bounded periodic `/tmp` sweep.
