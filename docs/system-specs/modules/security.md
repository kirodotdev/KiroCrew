...
Trust patterns are stored per-slot as session-scoped fnmatch globs (`slot._trusted_patterns`). Pattern matching keys on the event's canonical, provenance-trusted fields, never the LLM-authored display title (`event.title`). Shell tools match `event.shell_command` (recovered from provenance-trusted params; `None` / deny-by-default when the command is unrecoverable). Non-shell tools match a canonical key of `tool_name` plus the provenance-trusted target path (`_nonshell_trust_key`), and ONLY for a file-edit tool (ACP `tool_kind == "edit"`). A non-edit tool that merely carries a `path`/`file_path` param may also carry other security-relevant arguments the key cannot capture (e.g. `file_send(path, channel)` — a grant keyed on `tool_name` + path alone would let a trusted upload to one channel auto-approve the same file to another), so the gate returns `None` for non-edit tools and denies by default. fnmatch is asymmetric — only the pattern interprets metacharacters — so path metacharacters are escaped when the pattern is STORED (`escape=True`) while the MATCH subject stays raw; this makes an exact-file grant on a metacharacter filename (e.g. a Next.js `app/[id].tsx`) match that literal path instead of silently never matching. When the tool identity is missing, or no provenance-trusted path is available (path-less MCP tools such as `cron_add`), the gate returns `None` and denies by default rather than trusting the title or collapsing to a name-only grant that would ignore all arguments. The non-shell key is matched LITERALLY (not run through the shell segment splitter), so a path containing a separator (`&`, `;`, `|`, e.g. `Q&A.txt`) is compared whole instead of being cut into fragments that never re-match. On the STORAGE side, if the display redactors would alter the escaped key (a credential-shaped filename rewritten into a token carrying fnmatch metacharacters that `glob.escape` did not neutralize, which would widen the stored pattern into a matching glob), no trust pattern is stored and the gate falls through to interactive approval. Multi-command shell titles (e.g., `cat,wc`) generate patterns for each binary.
