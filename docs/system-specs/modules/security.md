...
snapshot, HOME isolation, output redaction, and SEL audit are retained). Prior:
pre-request installer redirect validation, direct-CLI ACP attestation, and
fail-closed Windows process enumeration.
...
commands still execute a private snapshot of the exact resolved bytes inside a
sandbox-visible directory below the staging parent rather than the mutable
discovery pathname, binding the process that performs the sign-in to the bytes
just resolved (no stored digest pin — a legitimate self-update must not break
sign-in; the copy keeps the resolved basename so a multiplexer launcher still
dispatches). ACP
...
Candidate discovery spans the inherited `PATH`, interpreter
