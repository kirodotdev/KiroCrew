...
Update 2026-07-28: one-shot `kiro-cli` spawns now request the CONFIGURED
`agent.sandbox` tier via `sandbox.agent_sandbox_mode()` instead of inheriting
`wrap_argv`'s hardcoded `"auto"` default — see "One-shot kiro-cli spawns follow
the configured tier". Managed first-party MCP probes take the trusted-target
Windows carve-out (`mode="off"`) that `kiro_prerequisite` already applied to
`kiro-cli`; third-party probes keep `"standard"` and still fail closed — see
"Trusted-first-party carve-out on a backendless platform".
...
**One-shot `kiro-cli` spawns follow the configured tier**: a caller that shells `kiro-cli` for a short read-only query — `dashboard/handlers/agents.py:api_models` (`chat --list-models --format json`) and `dashboard/handlers/sessions.py:_fetch_usage_bg` (`chat /usage`) — passes `mode=sandbox.agent_sandbox_mode()`, i.e. the SAME `agent.sandbox` value `AcpClient._spawn()` uses for the interactive session, rather than relying on `wrap_argv`'s hardcoded `"auto"` default. Rationale: it is the identical binary the agent session runs, so confining the read-only listing MORE tightly than the session itself buys nothing, while the `"auto"` default combined with the fail-closed raise above silently removed the feature on every host with no sandbox backend. `api_models` swallowed the `RuntimeError` into its degraded 503 branch, the frontend ACP adapter fell back to its auto-only model list, and the dashboard model picker offered nothing but "Auto" with no way to select another model (Windows always; Linux without user namespaces; macOS without `sandbox-exec`). `agent_sandbox_mode()` is **fail-secure**, mirroring `knowledge.llm_pool._get_sandbox_mode`: an absent `agent.sandbox` key resolves to the shipped default `"off"` (isolation delegated to kiro-cli's internal sandbox), while a *present but malformed* value resolves to `"auto"` — a typo is a misconfiguration, never an intent to run unconfined — as does a config-load failure. An operator who sets `agent.sandbox="auto"` still gets KiroCrew's OS sandbox on these spawns; governance may additionally clamp the tier up to a `sandbox.min_level` floor inside `wrap_argv`.

**Trusted-first-party carve-out on a backendless platform**: Windows has **no** OS-level sandbox backend (`detect_backend()` implements Linux user namespaces + macOS `sandbox-exec` only, so it always reports `none`), which means the fail-closed raise above makes *every* non-`off` spawn permanently unavailable there rather than occasionally. Where the spawn target is **first-party/trusted**, the established response is to skip the OS wrap on Windows rather than fail closed — `kiro_prerequisite._run_process` has done this for the trusted `kiro-cli` binary since the Windows port (`if sandboxed and not platform_compat.IS_WINDOWS`), which is why `kirocrew doctor` can report `kiro-cli`/`kiro login` at all on Windows. `mcp_discovery.probe_server` now applies the same carve-out: a server in `agent._MANAGED_MCP_SERVERS` (`kirocrew-cron` / `kirocrew-core` — KiroCrew's own module launched by KiroCrew's own interpreter, the same trust level as the gateway process doing the probing) is probed with `mode="off"` on Windows, restoring first-party MCP diagnostics that previously reported a hard error. The carve-out is deliberately narrow and the posture is unchanged everywhere else: it is gated on `platform_compat.IS_WINDOWS`, **not** on `detect_backend() == "none"` — that value is also returned for a *transient* userns failure, so gating on it would let a momentary Linux hiccup silently drop a probe to an unsandboxed spawn; macOS/Linux behavior is byte-identical to before; **third-party** servers keep `mode="standard"` and therefore still fail closed on Windows (they are untrusted by definition and Windows offers them no confinement); only `_MANAGED_MCP_SERVERS` qualifies, so edition-contributed `_extra_mcp_servers()` entries gain no trust from the public core; `_is_managed_server()` fails **closed** to third-party if the lookup raises; and the spawn still routes through `sandboxed_spawn_argv`, so the credential-env scrub and the `PYTHONPATH`/`PYTHONHOME` strip apply exactly as before. Probe status is display/diagnostics only — `sync_to_agent_config()` does not gate on it — so this never determined whether a tool could run, only whether KiroCrew could report on it.

**Fail-closed guidance is platform-aware**: the `RuntimeError` message tells the operator to "install a supported sandbox backend", which is impossible on Windows and made a documented platform limitation read as a broken install. On Windows the guidance instead states that no backend exists, points at `docs/windows-install.md`, and steers callers toward requesting `mode="off"` for a trusted target rather than setting `agent.sandbox_allow_unsandboxed_exec=true` — that flag removes the check **globally**, for untrusted spawns too, so a per-caller fix is preferred. Message-only; the deny itself is unchanged.
