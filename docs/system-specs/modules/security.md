...
| Node-path trust-root rewrite | Prompt-injected shell rewrites `node-bin-dir`, then Browser Mode runs its fake `npx` outside the agent sandbox | The default-home marker is on the read/write sensitive-path floor; custom-home markers are not consumed because arbitrary shell paths are not fully covered by the static bash matcher |
...
- **Node executable trust root (keystone leaf `node-bin-dir`)** — the saved directory decides which `node`, `npm`, and `npx` Browser Mode later executes in the unsandboxed gateway. A prompt-injected agent that could point it at an agent-writable directory would turn the next setup retry into a sandbox escape. The default-home marker is therefore read+write-blocked under every known default/legacy crew-home prefix, and direct path writes are caught by the existing sensitive-path/bash gates. `is_sensitive_path` also classifies the resolved marker under `KIROCREW_HOME`, but the bash gate's static home-anchored matcher cannot cover every redirect or computed-command form for an arbitrary custom path. Node discovery consequently ignores the marker whenever `KIROCREW_HOME` is set; custom deployments use the process-level `KIROCREW_NODE_BIN_DIR` override instead. The trusted setup script and a human operator writing the default marker outside the agent tool path do not traverse PreToolUse. Browser Mode does not add package-manager-specific fallback directories; Homebrew Node requires an explicit default-home marker or process environment. Automatic version-manager layouts are a residual same-UID trust surface shared with the build resolver, so this discovery policy does not claim a complete same-UID security boundary.
