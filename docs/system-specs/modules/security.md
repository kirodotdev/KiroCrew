...
**Generated Windows CA mirror** — `~/.kiro/crew/trust/ca-bundle.pem` is an input to every later child-process TLS trust decision: an agent that appends a root could make subsequently spawned `kiro-cli` and MCP children trust it. It therefore lives beneath the existing read+write-blocked `trust/` directory rather than as a write-only file in the data-home root. This directory boundary also catches shell commands that enter the data home and name the bundle relatively, including commands that never spell the absolute home-anchored path. Child TLS stacks receive the bundle through `SSL_CERT_FILE` and `REQUESTS_CA_BUNDLE`; a private provenance variable records the same managed path for re-exec refresh. The shell gate treats any explicit command reference to those exact identifiers as another spelling of the protected path, across POSIX, cmd.exe, PowerShell, and embedded interpreter syntax. This leaves ambient inheritance intact for ordinary HTTPS clients while denying indirect writes such as `Add-Content -Path $env:SSL_CERT_FILE`. Kiro Crew's startup publisher writes the file directly with `atomic_write`, outside the agent tool gates. The publisher compares SHA-256 identities over DER: Windows `Disallowed` entries remove matching anchors from both certifi and the Windows `ROOT` additions, duplicates collapse across both sources, and only all-purpose or TLS-server-auth `ROOT` entries are admitted. Every successful store read atomically publishes the complete current set, including certifi-only results, so an old stable-path bundle cannot retain a root removed from Windows; an inherited private provenance marker makes a Kiro Crew re-exec refresh that managed path while an operator-provided `SSL_CERT_FILE` remains untouched. If the Windows stores cannot be read, no partial mirror is published, the last-known-good file remains intact for already-running children, and the new process uses the older certifi/default discovery path; that fallback preserves certificate verification and startup availability but may not reflect Windows explicit distrust, so it is logged as degraded rather than described as fail-closed.

**Write-only runtime trust/config protection** (`is_sensitive_write_path` in `security.py` + `hooks.py`) — security-relevant runtime files are protected against *modification* by agent tools while staying *readable*:
...
- `is_sensitive_write_path(path)` is a superset of `is_sensitive_path(path)`, sharing the same `_path_in_home_dirs` resolve/casefold core so the two gates can't drift. `hooks.on_tool_call` denies a file-EDIT tool call (ACP `edit` kind) whose `path`/`file_path` resolves to a write-protected path.
