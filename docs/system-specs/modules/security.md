...
**Toolbox shim bypass**: On AL2 and macOS, toolbox-installed kiro-cli is a bash shim (`aim sandbox --client kiro-cli`) that creates its own seatbelt/namespace sandbox. Nesting that inside KiroCrew's sandbox-exec fails on macOS 26+ ("sandbox initialization failed: Operation not permitted"). `_resolve_real_kiro_bin()` bypasses the shim by: (1) following symlinks from the supplied path to the final native target (e.g. `~/.local/bin/kiro-cli → .../Kiro CLI.app/Contents/MacOS/kiro-cli`), honoring an explicit `KIROCREW_KIRO_BIN` override first; (2) honoring an explicit non-toolbox `kiro-cli` script override unchanged (only a positively-identified `aim sandbox` shim is redirected); (3) otherwise following the toolbox-managed `~/.local/bin/kiro-cli` symlink (the installer's trusted pointer to the *active* version) and its Linux `$BUNDLE_ROOT/kiro-cli` sibling. Every candidate must pass `_is_native_kiro()` (execute bit + ELF/Mach-O magic). **Security**: resolution never enumerates the user-writable `~/.toolbox/tools/kiro-cli/<version>` directory to pick the highest version — that would let a compromised/sandboxed process plant a higher-version binary and have it exec'd (including under the allow-default capability probe). Resolution is uncached (a few `stat`/`readlink` calls), so a toolbox upgrade — even one that repoints the symlink while retaining the old install — is picked up immediately with no stale-version risk. Called from `namespace_argv()` (Linux) and `sandbox_exec_argv()` (macOS). The capability probe (`_probe_sandbox_exec`) deliberately runs a trusted fixed system binary (`/usr/bin/true`) under `(allow default)` — never the user-writable kiro-cli — so a planted/repointed binary cannot gain unsandboxed execution via the probe.
