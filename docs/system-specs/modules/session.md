...
outage; a landed turn after `escalate` calls `forget(L1, slot.key)` instead, so
the next user turn gets a fresh budget and no recovery duration is recorded for
an outage that never closed.
