...
Dashboard agent picks and accepted remote mirrors clear the model pin and
fallback state through `clear_slot_model_binding`. Provider-reported in-turn
agent switches preserve the active pin, including same-agent events. A name-only
pick also clears the pin when resolution changes its recorded member/template
namespace. An unrecorded kind is backfilled without clearing a same-agent pin.
Local and remote switch responses carry the committed model so the dashboard
updates its pin even when the slots rebroadcast cannot arrive.
Rollback restores a cleared pin only while the request still owns the agent,
model generation and active fallback state; a concurrent agent event wins.
