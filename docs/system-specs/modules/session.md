...
- **Idle cleanup**: expires sessions after `session.timeout_secs` (default
  ...
  last one before its reset. The orphan branch reads it ONCE MORE after the
  sub-agent probe, because that probe suspends and an injection starting inside
  its await is invisible to any earlier read. The guard DEFERS the reap rather
...
- **RSS-threshold recycle** (`_rss_threshold_check`, config
  ...
  an already-committed turn is about to write into either. That predicate is a COROUTINE
