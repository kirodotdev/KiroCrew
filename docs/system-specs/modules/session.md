...
| `cancel_current(key, *, wait_ack_timeout=0.0)` | Cancel in-flight operation without destroying session. Returns `CancelOutcome`. Default `wait_ack_timeout=0.0` preserves fire-and-forget behavior for internal callers (taskrunner, subagent, llm_helpers). Dashboard `_run_chat` calls this seam only after it has acquired the session lease; a coroutine cancelled while waiting behind another holder owns no native turn and must not cancel by key. For an acquired turn it waits up to `agent.soft_stop_budget_secs`, retains the cancellation task across a repeated local cancellation, and surfaces a warning if the native provider does not acknowledge the stop. A local coroutine cancellation alone is not a native ACP cancellation and must not leave tools running after the dashboard reports a timeout. |
