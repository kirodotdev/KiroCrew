...
accepts replay; a landed turn atomically snapshots the fresh SID plus cleared
`replay_pending`, then writes that snapshot from a worker thread before retiring
the live replay lease. The settlement retains a field-level before-image until
