...
| `conversation_root(key)` | Returns a `ConversationRoot(sid, cwd)` observation read as ONE locked pair (`SessionMap.get_sid_and_cwd`, so a writer cannot interleave a sid from one state of the entry with a cwd from another): `cwd` is the directory the key's RESUMABLE conversation would restore on a cwd-less cold start (the session map's persisted cwd for the stored sid), or `""` when nothing is stored or the record carries no cwd; `sid` names that conversation (`""` when nothing is stored). Durable — survives a process recycle and a gateway restart. The public read a caller uses to learn where a conversation is still rooted; a LIVE provider's own `cwd` outranks it when one exists. |
| `discard_conversation_root(key, *, observed)` | Compare-and-clear: drops the key's resume pointer (`SessionMap.clear_sid_if`, read and cleared under the map lock) ONLY if the map still records exactly the `(sid, cwd)` the caller observed via `conversation_root` — so a newer conversation a concurrent cold start published on the key in the meantime keeps its pointer, INCLUDING the ordinary case where that successor adopted the very same stored cwd and republished it under its own sid (a cwd-only compare could not tell the two apart). The entry and its cwd note survive for diagnosis. The no-live-provider counterpart of an `ends_conversation` reset. Returns whether a pointer was dropped; `False` for an empty observation or one that no longer matches. |
| `default_conversation_roots()` | The cwd(s) a session created with no explicit `cwd` can land on: the pool cwd when it resolves, then the ACP runtime's default work dir (`config.paths.default_work_dir()`, the ONE spelling the ACP runtime and client root a cwd-less process at, so the two cannot drift apart). A caller distinguishing "rooted at the gateway default" from "rooted at a stale project" compares against this list. |
| `get_or_create(..., require_cwd=False)` | Opt-in: with `require_cwd=True`, a claim that would hand back a LIVE session rooted at a different directory raises `SessionCwdMismatch` UNDER the registry lock instead — the requested `cwd`, or for a cwd-less claim any of `default_conversation_roots()`, where that claim's own cold start would land. Compared CANONICAL on both sides: the allowed roots are realpath'd off the lock, and the live session's verbatim cwd (the spelling it was rooted with, possibly a symlink to the same directory) is realpath'd off the lock too — before the lock for the entry live at that moment, and outside it on the won-race path — so an equivalent spelling is never refused; only an entry that replaced the peeked one between the peek and the lock falls back to a lexical compare for that one claim. Applied by one helper on EVERY hand-back path: the lock-held reuse branch, the won-race return (a session another claimant installed while this one was building; checked BEFORE queueing on its lease, since an entry's provider never changes while it stays registered), and — because both recursive retries forward the flag — the stale-session and won-race retries. The reuse path otherwise ignores `cwd`, which every existing caller relies on, so the default is off. Consumer: the cron wake for a project-scoped or just-rescoped job (`slack/gateway.py`). |
...
| `reset(key, *, expect_session=None, expect_provider=None, skip_if_busy=False, clear_conversation=False)` | Kill session; returns `bool` (True iff a session was actually torn down). `expect_provider` is the public counterpart of `expect_session` for a caller that decided on a `get_provider` snapshot taken before an `await`: the reset happens only if the key's live entry still carries that provider (identity checked under the lock, atomic with the pop), otherwise `False` and nothing is touched — a session that replaced the observed one in the gap is left for the caller to judge afresh. Does NOT delete session map entry (kiro-cli file persists for future resume). Optional guards evaluated atomically under the lock with the pop, used by the RSS-recycle watchdog: `expect_session` only resets if that exact session object still occupies the key (guards against recycling a reset+recreated session on a stale off-lock RSS reading); `skip_if_busy` skips when the current session's semaphore is held so a live stream is never cut mid-turn. `clear_conversation=True` additionally clears the native resume sid in the SAME event-loop tick as the pop (entry + channel bindings survive, as in `_recycle_held`) — used by the still-critical post-compaction escalation so the overflowed conversation is not reloaded, without a delayed clear ever erasing a racing successor's sid. |
...
| `slack/gateway.py` — `_retire_session_if_cwd_moved` | a cron job's `project_dir` moved or was cleared, so the conversation rooted in the old directory is over and a fresh one cold-starts at the project |
