...
tokens allowed to answer are argv0 and — only when argv0's basename is `node`, `nodejs`
or `node.exe` — the first non-flag token after it, which is the script slot of a
Node-hosted adapter. Node spellings only: every registered backend's bespoke adapter is a
...
The script slot accepts two spellings, because the resolver produces two. The bin shim is
`node /opt/n/bin/codex-acp`, where the basename is the name. The package entry is
`node <pkg>/dist/index.js` (`_CODEX_ACP_PKG_ENTRY`), where the basename is `index.js` and
names nothing — so when the script's basename is a generic package entry file, the package
DIRECTORY above it (stepping over `dist`/`lib`/`build`/`out`/`src`) is tried instead.
Stopping at that one segment is deliberate: matching any path segment would accept a
...
Windows is not covered by the tokenized path. `process_matches` reads only the image
name there, so an interpreter-hosted adapter is `node.exe` whatever names the set
holds; that needs an identity independent of argv and is tracked separately. The
basename split handles `\` as well as `/` regardless, so a Windows-shaped argv0 in a
command line that does get read is not carried whole into an exact-name test.
