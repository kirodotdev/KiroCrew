...
**The `/note` hold now carries two element classes, and a seam must say which it is releasing.** `Slot.flush_deferred_notes` holds `/note` rows for the next USER turn; `section_marker` rows join the same `_deferred_notes` list but obey the opposite policy, so the flush takes a REQUIRED `markers_only` and every caller must state which class it releases:

| element | why it may be held | flush mode at a structural seam |
| ------- | ------------------ | ------------------------------- |
| `/note` | a note is a **message** owed to the next user turn, so delaying it is delivery — it arrives later, addressed to the same turn | stays held |
| `section_marker` | a marker is a **boundary**, so delaying it does not delay a message, it **relocates the seam** onto rows it never covered | released (`markers_only=True`) |

So the rule is directional, not symmetric: a marker may be released early but never held past its successor, while a note may be held but never released early. `markers_only=True` is therefore correct exactly at a seam that owes a note to a turn that has not arrived yet but is about to write rows the marker must precede — the two current callers are the stage-execution exit and the synthesis path, each passing the predicate that detects a structural successor (`chat_runner.py`). The other four seams are teardown paths that owe every held row before the slot goes away, so they pass `markers_only=False`. **`markers_only` is keyword-only with NO DEFAULT, so this is enforced by the signature rather than by convention:** a new seam that says nothing does not silently inherit flush-everything, it fails to call. That matters because neither wrong choice fails visibly — inheriting the flush would move a pending boundary, and an unconditional `markers_only=True` would deliver notes early — and a mistake no test happens to cover would otherwise reach a reader as correct code. This is the one sanctioned qualification of the RFC's "no second notion of held": the hold is still one list with one flush, but its membership is no longer one kind of thing.

**History re-injection counts user and assistant rows only.** When a slot's in-memory
transcript is compared against the archived one to decide whether context needs
re-injecting, both sides count only rows whose `role` is `user` or `assistant`
(`chat_runner.py`, the `disk_count` / `mem_count` pair). A marker is a structural row with
neither role, so counting it would make an unchanged conversation look longer in memory
than on disk and force a re-injection that has nothing to re-inject. The same exclusion
already covers tool and error rows; the marker joins that class rather than starting a
new rule.

The applier reuses the SAME effect cores the HTTP endpoints call — `authorize_and_add_nudge` / `authorize_and_update_nudge` / `svc.remove` for the monitor trio, `slot.project` plus the recent-projects save for `set_project`, `deliver_ws_owners` for `suggest_followup`, and `post_question_card` for `ask_question` — so behavior is unchanged except that `ask_question` is now non-blocking (full contract in `learn-cron-dashboard.md` → "Agent Questions"). `reset_conversation` is the one directive whose core is not reachable inline: it queues `SessionManager.discard_conversation` on the slot for `chat_runner._consume_pending_reset` to apply at a turn boundary, because the discard is a full provider teardown and the producer is mid-turn — the same deferral `set_project` uses, and the reason the immediate route (`POST /api/chat/slots/{slot}/reset-conversation`) answers 409 on a busy slot rather than tearing down a turn mid-write. It queues the session key THIS TURN ran on, passed in by the consumer, never re-resolved from the slot: `linked_session_key` is mutable, so a cron or workflow injection that rebinds the slot between the request and the consume would otherwise discard whatever the slot points at by then and leave the caller's conversation alone. Only the END-OF-TURN consume may apply a discard (`allow_discard`); the two earlier consume points run just before a turn acquires the session, where a teardown lands under a channel turn already streaming on it. Even at that boundary it does not assume: the discard goes through `discard_conversation(..., skip_if_busy=True)`, which refuses under the same session lock that pops the session, mirroring `reset`'s own guard. Probing from the consumer and tearing down afterwards would leave a window in which a channel message acquires the session's semaphore and begins streaming a reply the teardown then destroys — and the semaphore is the stricter signal anyway, since `provider.has_active_turn()` cannot see a turn holding the semaphore with no prompt in flight yet. A refusal returns False and changes nothing, replay flag and session map included, so the consumer leaves the flag armed for a later boundary. The sid clear runs in the SAME tick as the pop, with no await between them — deferring it past the shutdown awaits lets a concurrent channel turn map a SUCCESSOR session under the key while the old provider is still shutting down, and the clear then erases the successor's pointer instead of the discarded one. Sub-agent children are the other wait — `discard_conversation` releases the shared runtime they run on, so a running or queued child, or an in-flight completion-event delivery, also leaves the flag ARMED rather than killing the child's work. Both that consume and the route's 409 read one predicate, `chat_utils.subagents_attached`, so the two cannot drift. The queued flag is in-memory slot state: a gateway restart while it sits armed drops the reset the confirmation promised, which is accepted rather than persisted — the cost is one un-applied reset the caller can ask for again, against durable state for a transient intent. `set_project`, `reset_conversation`, `section_marker` and `suggest_followup` additionally require structural user-turn provenance: injected cron, task-runner, sub-agent, auto-nudge, orchestration, app-authenticated unattended turns, and app-authored Spec Builder seed/handoff prompts cannot retarget a borrowed destination slot even when its session key is user-facing — nor, for the latter two, write a structural row or a follow-up card into a human's session on the strength of a dashboard tab they merely inherited. `ask_question` carries the surface gate WITHOUT the origin one, and a producer sweep is what settles that rather than symmetry: `goal-conductor` and `goal-ledger-conductor` grant it by name precisely so a patrol cycle waking on a nudge with nobody at the keyboard can still put a decision to its operator, answered later from the open tab. Off the dashboard it degrades to an `[OPTIONS:]` steer rather than refusing, so its gate asks whether a tab is open rather than who opened it. `section_marker` carries three gates in total, the most of any directive: the dashboard-only surface gate it shares with `suggest_followup` and `ask_question` (`_DASHBOARD_ONLY_DIRECTIVES` — an open tab must exist for a row to render into, so a tabless cron or sub-agent turn is refused before provenance is even consulted), the human-provenance gate just described, and a third refusal while a multi-stage plan is executing (`_in_stage_execution`), because the whole-plan flush seam would relocate the boundary it marks. The three are independent and ordered surface-then-origin-then-timing: the first asks whether the effect has anywhere to land, the second who is entitled to write it, the third whether the boundary it would mark is stable yet. Spec Builder rejects app-token message and decision submissions before they can enter its human-provenance relay or durable decision ledger. Queue entries preserve this provenance, replacement text adopts the editor's provenance, and mixed or untagged merges fail closed. A refusal at either gate is also reported back to the CALLER, and that reporting is deliberately NOT scoped to `section_marker`: `chat_runner` appends a `[Not applied: …]` note to the tool result for **every** provenance-gated directive, naming which gate declined it — a sub-agent call having no session to act on, or a build that could not verify the call's identity. The harm it removes is that a silent drop is indistinguishable from success to the agent that asked, so the operator-facing `logger.warning` and SEL row reach a human while the model proceeds believing the directive applied and, worse, may report to the user that it did. This widens the result text of `set_project`, `reset_conversation` and `suggest_followup` as well, which is the intended blast radius rather than an incidental one: the note describes the gate, so a directive added to `_USER_ORIGIN_DIRECTIVES` later inherits the disclosure without a second change.
