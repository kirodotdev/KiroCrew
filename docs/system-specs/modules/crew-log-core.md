...
`append_if` adds a THIRD append outcome beside written and refused: **declined**, reported as `None`. It takes a `precondition` and calls it with the newest committed seq once ownership and the per-append lock are both held and the tail has been read, writing the entry only if it answers true. The outcome exists because a decision that governs append ORDER cannot be made outside the hold that assigns the order: another process's entry committed between a caller's decision and its own write lands first, and for a reader that takes the last word per field that ordering is the whole result. A decline appends nothing; it is NOT byte-identical, because the torn-tail repair above it is unconditional and a decline can leave that repair behind.

A precondition carries two obligations, and they are the caller's to keep: it must be BOUNDED, and it must not write to this log. It runs while the lease and the per-append lock are both held, so its cost is a window in which every other process's append to that unit is refused `already_owned` -- and a peer that exhausts its own retry budget loses its entry for good, since this file has no compaction and nothing replays it. Reads inside it are safe and that is structural rather than a courtesy: `iter_from`, `page` and `resolve` take neither the lease nor the lock, per the lazy-claim rule above, and the lease is refcounted per process, so a nested claim from the same process is not contention. A whole-file parse is the obligation's clearest violation: it is unbounded in the unit's length, so a caller that needs one folds BEFORE taking ownership and leaves only a comparison inside.
