...
**Redaction and protocol framing** — before text reaches a renderer, `TurnDriver` first classifies a reserved summary-bearing compaction notice at the start of the turn, then incrementally parses kiro-cli's inline `[STEERING steer-<id>: …]` frame across arbitrary chunk boundaries. Compaction summary bodies become the terse `✅ Context compacted.` receipt. Steering frames never become text: they emit one structured `STEER_CONSUMED` event at the exact boundary (paired with kiro-cli's typed lifecycle event regardless of arrival order). The user-facing `[OPTIONS: …]` trailer is deliberately not part of this filter and passes through unchanged for renderer-native buttons. After framing, `_redact()` delegates to `platform.redact_pako_via_context` for every text chunk, thinking chunk, tool title/purpose, and string field of prompt-choice options. Ordinary outer text receives the same companion-blind exfiltration-plus-credential baseline as before this change; a Mermaid pako link that baseline would now preserve has its decoded views checked by the active `CredentialPolicy` before the renderer sees it. A client cannot un-see a live frame, so a later replacement is not a security boundary.

Authorization must survive presentation. Every `Renderer` instance rechecks
post-`TurnDriver` text through the same narrow seam after display/table/options
transformations, and the channel-local Discord, Telegram, Teams, Webex,
WhatsApp, iMessage and Slack sinks do likewise. The shared helpers keep their
companion-blind baseline as the default because proactive/raw callers also use
them; a renderer opts into restoration explicitly. Dashboard string readback
uses the same rule after its context-aware write/stream boundary, while
structured legacy/corrupt rows retain the public baseline and cannot acquire
restoration from their container shape.
...
- **Redaction is unconditional**: all LLM/tool-originated text flowing through `TurnDriver` passes `platform.redact_pako_via_context` before reaching any renderer. The outer text keeps the prior `redact_exfiltration_urls()` + `redact_credentials()` baseline; decoded pako state is additionally checked under the active policy and fails closed if that policy cannot verify it.
