...
- the operator's `channels` ceiling for the channel, which the client asks the messaging seam for directly. Global to the channel, so it is answerable without a roster and is read even when no predicate is installed;
- the transport's live rosters, installed by `DiscordTransport` as the client's `still_permitted` predicate. Synchronous and in-memory for the same reason `may_send_to` is, and it reads only the client's CACHED channel types: resolving one would re-enter the ladder it guards.

Scope is deliberately narrow in three ways. **Only routes naming a channel are guarded** — a button press's own reply carries its destination in an opaque token, is already re-judged against the live rosters before it is answered, and has a ~3 second deadline. **Only an attempt that actually waited pays for it**, so a send that never blocks costs no governance read. And the predicate **only ever refuses** sends the ladder would otherwise have made: given a channel id alone it settles a withdrawn thread and an allow-list emptied to nobody, while the narrower DM case — one peer removed while others remain — is the same gap `may_send_to` carries and needs the same fix, a `dm_channel_id -> user_id` pairing persisted when the DM is opened.
