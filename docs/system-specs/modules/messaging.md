...
- `provider.supports_steer` — membership in `ACP_BACKENDS_STEER`, or in
  `ACP_BACKENDS_STEERING_REQUEST` on the shared-runtime handle (codex's
  `_session/steering`), since the dormant Claude backend seam has neither. When
  false the message falls through to the queue path.
