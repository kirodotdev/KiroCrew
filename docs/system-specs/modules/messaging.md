...
- **Otherwise a DELIVERED channel send takes it and the Slack owner-DM leg stands
  ...
  stop reading.
- **A multipart cron result settles only as a whole.** Before posting its first Slack part, the gateway persists the exact rendered parts and resolved destination under the result's live-owner claim. Each acknowledged post advances a durable completed-part cursor and the first post also records the parent/thread identity. Every part carries a deterministic UUID `client_msg_id` derived from the job, result generation, and part index, so a process that dies after Slack accepts a post but before the cursor save retries the same bytes under the same Slack duplicate identity. Failure or cancellation releases local ownership but preserves the cursor, parent, pending bytes, and owed occurrence; a replacement runtime resumes at the exact next part without regenerating provider/tool output. Kiro Crew relies on Slack's documented `client_msg_id` duplicate handling for the external post-before-local-save window; it does not claim a stronger exactly-once guarantee than that API contract. Only the final cursor marks the result acknowledged, and only the subsequent whole-result settlement clears the outbox and debt. Single-part delivery is the one-part form of the same state machine. A configured destination remains a destination while the Slack client is temporarily absent, so its generation stays pending rather than being terminalized as dashboard-only.
