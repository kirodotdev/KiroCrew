...
| `messaging/driver.py` | **Layer 2** — `TurnDriver` (channel-neutral turn loop), approval-mode constants, `resolve_transport_approval_mode` (the one transport approval-mode resolver every channel gateway delegates to), `_redact` helper |
...
**One resolver, one answer.** `messaging.driver.resolve_transport_approval_mode(orch, *, yolo_probe=None)` is the single implementation every channel gateway reaches through its own named `_resolve_approval_mode(orch)` seam. It folds three inputs, tightest first: a runtime YOLO probe when the channel has one, the CLI `--approval` override carried on `orch._approval_mode` (including the literal `"yolo"`, which `cli.py` accepts and refuses to start without an isolated `KIROCREW_HOME`), then the configured `agent.approval_mode`. Anything that is not exactly `APPROVAL_AUTO` collapses to `APPROVAL_INTERACTIVE`, so an unrecognized spelling is deny-by-default rather than an accidental auto-approve, and a `yolo_probe` that raises is treated as inactive for the same reason. The per-channel name stays, because channel tests import it: the shared helper is reached *through* that seam, not in place of it. Slack is the one channel that supplies a probe (`is_yolo_mode`, its owner-toggled TTL-capped slash-command override).
