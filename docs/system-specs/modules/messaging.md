...
channel session before a later raw completion is accounted. A fail-closed
one-shot discard is exposed through the same manager probe as `retained
completion requires restart recovery`, and its shared guidance names restart
orphan recovery or intentional session close rather than passive settlement;
legacy nudges retain their existing collector path.
