...
- **Failure is reported, not absorbed.** Every refusal — no link, a link on
  ...
  cannot send proactively, a recipient dropped from the transport's allow-list, a
  transport error — is audited with its reason and reported. `_deliver_to_channel`
  returns `(sent, code, detail)`, and the code is what separates an authorization
  the caller cannot retry from a failure it can: an authorization refused or
  withdrawn surfaces as HTTP 403 `channel_not_permitted`, a resolve or transport
  failure as HTTP 502 `channel_delivery_failed`, and a send that never reached an
  authorization at all (no link, no conversation id, a link on another transport)
  leaves the code empty, which on this leg still answers 502 -- degrading to the bell
  is the addressed DM leg's behaviour, not this one's. One generic
  502 for all three told an LLM-facing caller to retry what a retry cannot fix.
  The bell is not
...
- **`channel_type` on the request is the caller's FILTER, not a destination.** A
  caller that does not constrain the walk sends it empty, so it cannot name where
  a send went; the audit row names the transport the LADDER selected instead.
  Using the filter as the label made every row from an unconstrained caller read
  `channel` regardless of the destination, which left two different destinations
  indistinguishable in the audit trail.
- **Authorization is re-asked before every part of a SESSION-BOUND send.** A
  message chunked to the transport's cap is a sequence of awaits, so a binding
  rebind, a governance narrowing, a revoked app `capabilities.messaging`, or a
  recipient dropped from the allow-list can land between two chunks. Each part
  re-resolves through the ladder, re-walks the binding, and re-decides the recipient
  synchronously with nothing awaited before the send; the grant and the denial are
  both recorded. Parts already sent cannot be recalled, so a refusal aborts the
  remainder rather than pushing more content at a withdrawn authorization.
- **Every leg of a proactive send resolves the SAME principal.** The route picks its
  leg from the request BODY — a bare `channel_type` (session-bound), `channel_type`
  with `target_id` (an addressed target), or `session="<channel>"` (the owner DM) — so
  a gate asked on one leg and skipped on another is escaped by re-spelling the
  destination rather than by defeating the gate. Each leg asks BOTH scopes under the
  gateway-attested app identity (`effective_request_app`): `capabilities.messaging`
  for whether that app may originate a message at all, and `channels` for where it
  may send. Asking only the second resolved the session's surface profile, so an app
  whose own profile denied messaging was refused on one spelling and delivered on the
  other two. The MCP-side messaging vet does not cover this: it is fail-OPEN for
  `send_message` and resolves the stdio subprocess's own app, not the attested one.
- **The addressed DM leg decides once per send, and its revocation window is OPEN.**
  Recorded as accepted, not as answered. Its destination is fixed for the life of the
  request, which disposes of the REBIND case alone; a governance narrowing, a revoked
  app `capabilities.messaging`, or an allow-list drop lands between a DM's chunks
  exactly as it does on the session-bound leg, so that leg can still push content at
  an authority withdrawn mid-message — the same harm the bullet above closes. What
  differs across the legs is the CADENCE, not the gate set: all three ask the same two
  scopes, and only the session-bound leg re-asks them per part.
  Deferred rather than defended: the addressed leg authorizes through a different
  path (`resolve_configured_target` plus `_vet_channel_send`), so converting it is a
  behaviour change on a second egress surface needing its own review, and
  `chat_runner._deliver_cross_surface_reply` carries the identical gap. Anyone
  narrowing this should convert both, not one.
...
- **A proactive send names its destination and fails closed on it**: `send_message`'s Slack fields and its `channel_type` are mutually exclusive families, a refused channel delivery never falls through to Slack, and every refusal is audited and reported — 403 `channel_not_permitted` for an authorization refused or withdrawn, 502 `channel_delivery_failed` for a resolve or transport failure — rather than absorbed into a dashboard notification. The destination comes from gateway-owned state — a cron's job `session_key`, or the kernel-attested `X-Session-Key` header — never from the request body. See § Proactive sends.
