...
**Adopted:** Telegram, Discord (with the thread), Weixin, WhatsApp DMs, Webex, WeCom, iMessage. Each declares its route at its dispatch site using the reply target its own `send_message`/`may_send_to` address — Weixin/WeCom/iMessage the peer id, and Webex the `room_id`. Webex pairs the route to the room type — a DM carries the principal (`user_id`) and no thread; a space carries the `reply_parent` thread and no principal; an unthreaded space carries neither and so falls through to denial. This is because Webex `may_send_to` has a space arm (`room_id in _allowed_rooms`) that falls through to `principal in _allowed`, so a space route carrying a principal would authorize a de-authorized space via its still-allowed sender, and a DM route carrying a thread would be read as threaded and denied for want of a principal. WeCom and iMessage carry no such fall-through: each `may_send_to` grants on the conversation id (userid / handle) alone with no principal arm, so a single peer-id route is authorized or denied on the roster directly. **Teams is excluded on purpose**: its unified-queue drain rebuilds one combined turn that joins several participants' queued messages while keeping the first sender's envelope, so a single route for the combined turn either quotes one participant's message to another (declared) or loses the queued messages on a shutdown-refused drain (withheld). Adopting Teams honestly needs per-message route preservation through the drain, which is a change to the drain itself and belongs in its own PR. **Feishu is excluded on purpose**: `FeishuTransport.may_send_to` returns `False` unconditionally — there is no proactive send in the v1 integration, and `send_message` takes an inbound `message_id` that is stale after a restart — so a declared route would be DROPPED as route-revoked on every replay and the notice could never land; inheriting no route is the honest degradation. **Slack is excluded pending a maintainer packaging call.** `#8912` stays open on Slack's, Feishu's and Teams' account. Attachment re-download is tracked in #8911; re-dispatch and WhatsApp group routes in #9144.
