...
`api_channel_clear_context` discards either one agent conversation or every channel-agent conversation, through `SessionManager.discard_conversation` rather than `reset`, so the persisted resume SID goes with the context instead of reloading it on the next turn. An agent-scope clear preserves shared messages and exchange counts. An all-scope clear also wipes both, persists the channel, and broadcasts `channel_context_cleared` so other browser clients discard stale messages, but only when every member was idle: a partial clear keeps the shared log, because a member refused mid-turn holds the LLM context that references it, and skips the broadcast, whose listener replaces its retained transcript with an empty list.

A member with a declared lifecycle turn in flight is refused rather than destroyed. Any refusal, total or partial, answers 409 carrying `code` `turn_in_flight` and names the refused members in `error`, which the dashboard renders verbatim; only a fully clean clear answers 200. The total refusal is answered before the wipe and the partial path skips the wipe, so no response reports shared state as kept after destroying it.

The handler holds the channel's `_log_lock` across the whole clear, the same lock `Channel.post` holds across its append, delivery and `_save()`, so a post concurrent with an all-scope clear is safe from it: the post either precedes the clear or refuses the member it targets. Each member's discard is bounded by `_CLEAR_DISCARD_TIMEOUT_SECS`, and a cancelled teardown gets `_CANCEL_GRACE_SECS` on top of that. Neither bounds the lock hold: the grace is spent per member, and the wipe's synchronous `_save()` runs with the lock still held. The grace wait returns at its deadline, but the teardown it was watching may still be running when it does. So the constants bound the discard waits only, and not the lock hold: the wipe's `_save()` is synchronous and unbounded, so a waiting post has no guaranteed ceiling at all. An in-flight approval future keeps its member `working`, so a clear REFUSES that member; the future is not cancelled by the handler either way and resolves through the agent task.
