...
The directive is honored on **both** dashboard launch paths so it behaves
consistently regardless of how the run is started: `POST /api/taskrunner`
(`/start`) reads a bounded prefix of the spec file, and the plan→`/execute` path
(`POST /api/taskrunner/{task_id}/execute`) parses the run's in-memory
`spec_content` — the snapshot captured at plan time, so no fresh disk read (and
no read/execute TOCTOU) is introduced on the resume path.
