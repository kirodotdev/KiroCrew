...
- Credential redaction — context-aware egress scrubs route through the single platform-context composition: `kiro_crew.platform.redact_with_findings_via_context` owns the pako-aware baseline, active `CredentialPolicy`, warning order, fallback, and the explicit authorization required to restore a validated pako payload; `redact_via_context` is its text-only projection. Legacy sinks that still call public or low-level baseline redactors never carry that authorization and therefore replace the whole pako candidate even when its decoded state is clean under public patterns. Modules import the latter as `redact`; the two whole-text boundaries that redact a raw provider accumulator and need the warning lists — the dashboard's `_flush_segment` family and Slack's joined `accumulated` rebind — call the former directly instead of sequencing a baseline pass and a second host-aware pass. Sites that were companion-blind on ordinary text before the pako change and are not such a boundary use `redact_pako_via_context`: the live wires (`chat_runner._wsred`/`_thinkred`, the side chat, Slack's `_sred` and cursor edits, `messaging.driver.TurnDriver`'s `_redact`), the direct chat-delivery helper (`chat_delivery.sanitize_outbound`, shared by steer persistence, queue broadcasts, session creation fields, and `session_send`), the deny-row title helper, and the post-authorization presentation/readback passes: `messaging.renderer.Renderer`, each channel-local renderer sink, Slack's native OPTIONS fallback, and dashboard string history/live readback. Shared helpers that also serve raw or proactive callers retain the companion-blind baseline by default and require an explicit redactor from a renderer; Webex and WhatsApp keep separate baseline defaults for their raw transport/client paths. This seam keeps the baseline byte-for-byte on ordinary outer text, but checks every decoded pako view under the active policy before restoring the opaque link; a policy error while inspecting decoded state fails the whole link closed. It is narrower than converting those sites to the full host policy, but unlike a baseline-only pass it cannot disclose a companion-only credential in a live pako link before a later replacement. Covers: `agent.py` SEL-audit callers, `mcp_core.py` chat-history/spawn output, `mcp_cron.py` deny-reason + script-vet + timezone messages, and `dashboard/handlers/files.py` file-content egress (slot append, file-watch, file_read, download gate) as well as the filename/path/description gates. Standalone is byte-for-byte the prior exfil-then-credential two-pass (the Default `CredentialPolicy.redact` delegates to `security.redact`); a loaded companion adds its internal-token regexes uniformly across every egress surface. **The baseline runs exactly once per call.** `_host_delta_redactor` hands the composition a `final_redactor` only when the active policy's `redact` is NOT the unmodified `DefaultCredentialPolicy.redact` — that method IS `security.redact`, the pipeline the seam has just run, so threading it back in would execute every exfil and credential pass a second time on the hottest text path for no change in output. The check is on the method, not the class, so a companion that subclasses the Default and overrides `redact` still runs its delta once after the baseline; a companion whose `redact` itself calls `security.redact` pays that second pass by its own composition, which this seam neither adds to nor can remove. The same active delta is passed independently as `decoded_redactor`, so at a full host-aware boundary a link whose diagram carries a companion-only token is refused before restoration; the narrow pako seam passes only that decoded redactor and omits the outer `final_redactor`. A `StreamRedactor` calls whatever redactor it was given once per commit and composes nothing itself (`test_security.py::TestPakoFragmentRedaction` counts the credential pass through the facade seam on both paths). `platform/context.py` imports the `security` helpers at module scope on purpose: `kiro_crew.platform` already loads `security` at package import (`bootstrap` → `defaults` → `from kiro_crew import security`), so a deferred import there never kept the regex stack off the platform load path and would only re-pay a function-local lookup on every egress call now that the happy path composes the baseline here.
