...
is the sole authority.** The sole enforcer for all four is the connector
**manifest validator** — the checker that owns the manifest schema. That script
(named `check_connector_manifest`, under `scripts/`) is a **named post-S2
sequencing dependency**: it does not exist on `main` yet, so this doc refers to
it by role rather than by a repo path that a reader could try to open today.
...
the manifest validator merges. **Until that step lands, this validator is not a
gate** — it ships its own unit-tested `--test` self-check and test suite, but
nothing in `.github/workflows/` invokes it, so it guards no PR yet. Both the
manifest validator script and the workflow wiring are **named post-S2
sequencing dependencies**, tracked as a named item in the campaign ledger; they
are deliberately not referenced here by a repo path, because neither exists on
`main` yet and a doc must not point a reader at a path that cannot resolve.
