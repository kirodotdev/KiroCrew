...
| Standalone ACP bridge | Every observed tool call (`invoked`) and every hook/editor permission decision (`approved`, `rejected`, or `denied`) with `source=acp`; the daemon-backed adapter does not use this bridge and remains owned by dashboard audit | `acp_server/gateway.py` |
