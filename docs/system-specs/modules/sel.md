...
| Retained sub-agent completion | `subagent_completion_discard` / `retained` (`source=subagent`, `caller_identity` = the parent session) whenever an explicit queued-completion discard FAILS CLOSED and keeps the goal-stop fence: `metadata.reason` is one of `no_manager`, `settlement_cancelled`, `settlement_failed`, `tombstone_not_durable`, with the child in `metadata.subagent_id`. The discard callback is one-shot and only restart orphan reconciliation releases that fence, so the manager marks the owner as `retained completion requires restart recovery`; shared refusal guidance names restart orphan recovery or intentional session close instead of promising that waiting or another Stop clears it. The SEL row tells the operator WHY that retained state exists. Best-effort: an audit failure never alters the retain it describes | `slack/gateway.py` |
