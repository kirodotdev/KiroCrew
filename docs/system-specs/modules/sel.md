...
| Dashboard API | All POST/PUT/DELETE operations via middleware, plus allowed and denied project-skill trust, app-slot, saved-workflow, and strict session-monitor read authorization decisions. Project activation, deactivation, and removal use critical audit-or-deny decisions inside the per-Project mutation lock, before state changes | `dashboard/server.py`, `dashboard/handlers/prompts.py`, `dashboard/handlers/workflows.py`, `dashboard/handlers/autonudge.py`, `dashboard/handlers_project.py` |
