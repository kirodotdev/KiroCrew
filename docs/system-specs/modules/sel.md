...
| Project-bound cron gate | Every owner decision on a job's project directory, on BOTH outcomes — `allowed` and `denied` — so the audit trail shows who was permitted, not only who was refused. Operations: `cron.create.project_path`, `cron.update.project_bound_job`, `cron.update.project_path`, `cron.run.project_bound_job`, `cron.enable.project_bound_job`, and the read side `api_kirocrew_agents.project_path`. `error` carries the reason a denial fired: `not owner`, `sensitive path`, `not a usable directory`, or `stale project binding` (the `expect_project_path` CAS reject on the run and enable paths, answered as 409 `stale_project_binding`; `cron.update.project_bound_job` cannot emit it, because that refusal is a precondition of the write rather than a re-check of an earlier read — see learn-cron-dashboard.md). At fire time, a project definition that would shadow a bound Crew Member emits `cron.fire.project_bound_job` with `outcome=denied`, the job id in `resources`, and `error=member_shadowed_by_project`. Each call is wrapped and its failure logged at debug — the decision has already been made, so a cold or broken SEL must not promote it into a user-visible failure or let a refused fire dispatch | `dashboard/handlers/cron.py`, `dashboard/handlers/agents.py`, `slack/gateway.py` |
