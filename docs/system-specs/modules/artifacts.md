...
| `PATCH` | `/api/artifacts/{slug}` | Partial update; `content` bumps version; optional `folder` key (metadata-only). Optional `expected_token` (optimistic-concurrency token from a prior read's `content_token`): a content write is refused with **409** `{error, current_token, version}` unless live content still mints to it — refetch and re-base; a non-string token is a 400. Token omitted = last write wins (unchanged). Content-carrying responses (here and on `GET /{slug}`) include `content_token`, an opaque keyed digest of the raw content (HMAC under a key persisted at `<store root>/.token_key`, over a per-artifact salt rotated on every content write; never the plain hash, which beside redacted content would let a reader test guesses for the redacted text offline — and the salt denies a reader who can write the same test by saving a guess) to carry back as the next token |
