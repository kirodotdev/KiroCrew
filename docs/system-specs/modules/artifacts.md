...
| `PATCH` | `/api/artifacts/{slug}` | Partial update; `content` bumps version; optional `folder` key (metadata-only). Optional `expected_sha256` (optimistic-concurrency token from a prior read's `content_sha256`): a content write is refused with **409** `{error, current_sha256, version}` when live content no longer hashes to it — refetch and re-base; a non-string token is a 400. Token omitted = last write wins (unchanged). Content-carrying responses (here and on `GET /{slug}`) include `content_sha256`, the raw-content hash to carry back as the next token |
