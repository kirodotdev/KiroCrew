...
Five backends are described. **Kiro, KAS, CC, and Codex are selectable on a plain
public build; OpenCode is known but dormant.** The baseline registry
`BASELINE_SELECTABLE_BACKENDS` (`agent_sdk/backends.py`, re-exported by
`acp_backends.py`) contains the four admitted ids. OpenCode is deliberately absent
until every tool execution can be forced through Crew's gate. There is no frozen
...
Three of the five are *foreign* hosts — **CC, Codex, and OpenCode** — and they
have separate host contracts. KAS is not one of them: it is Kiro's own agent service
...
Codex is in this document for a second reason: it is the
...
the CC and Codex columns as a pair: CC for what a complete answer looks like,
Codex for what a partial one looks like and which rows stay honest by saying
"unmeasured".

OpenCode's column records a different state: the native wire is measured, while
execution admission is refused. Isolated OpenCode 1.18.23 probes used a loopback
fake model, an inert MCP server, temporary homes, and no real credentials. Named
native `allow` rules let write, shell, read, and MCP calls complete without a
`session/request_permission`, despite a wildcard `ask` and a host that rejected
every received request. `agent_sdk.backends.require_backend_admission` therefore
rejects selectable registration and direct `AcpClient`/`AcpRuntime` construction
before I/O. A resolved binary, advertised capability, or green frame replay does
not clear that blocker.

OpenCode evidence labels distinguish **measured** behavior observed by the
isolated probe from **unmeasured** behavior, a **declared** host contract, and
**source-verified** native paths. **Unsupported** means Crew supplies no admitted
implementation. The sanitized
sequences and their provenance are in `test/fixtures/acp_frames/opencode/` and
`test/fixtures/acp_frames/README.md`. They pin parser behavior and preserve the
missing permission requests; they do not exercise a running Crew integration.
...
| **OpenCode** | `ACP_BACKEND_OPENCODE = "opencode"` — native `opencode acp`. Known but dormant: installation and catalog helpers remain available, while registration and both ACP constructors refuse it before I/O because native named permissions can bypass Crew's gate (§7). |

## 1. Agent definition and layout

| | kiro-cli | KAS | CC | Codex | OpenCode |
|---|---|---|---|---|---|
| Where a managed agent lives | `~/.kiro/agents/kirocrew.json` (+ `-lite`, `-conductor`, `-knowledge`, `-research`, `-heartbeat` variants, `agent_files.py`) | Nowhere on disk — agents ride `_meta.kiro.customAgents` on `session/new` (`acp/kas_agents.py`) | `~/.claude/agents/<name>.md`, Markdown with YAML frontmatter (companion: a JSON→Markdown agent translator writes it) | Nowhere — spawned bare, with no `--agent` and no spec on disk (`acp/client.py`) | **unsupported** — no native OpenCode agent file is written; the dormant mirror withholds the agent-definition concerns (`providers/mirrors/opencode.py`). |
| Format | JSON, validated with `deny_unknown_fields`; an unknown key silently falls back to the default agent, which is why bookkeeping lives in an `agent_state` sidecar (`agent.py`) | JSON projected onto the wire; `prompt` must be inlined, and `tools` absent means *no* tools (`kas_agents.py`) | YAML frontmatter (`name/description/model/permissionMode/tools/mcpServers/disallowedTools/hooks`) plus the system prompt as the body | None — no definition is projected in any shape: the spec is not read and no wire equivalent is sent (`acp/client.py`). `providers/mirrors/registry.py` records that as codex's declared state rather than leaving the omission unexplained | **unsupported** — no definition or bookkeeping is projected. Native config validation and shadowing are unmeasured. |
| Selection mechanism | `--agent <name>`; `$PWD/.kiro/agents` shadows `~/.kiro/agents` (`agent_discovery.py`) | `session/set_mode` against a wire-supplied agent | No `--agent` and **no `set_mode` at all**; the agent-activation privilege check is skipped (`acp/client.py`) | No `--agent`, and **no `set_mode`** — activation is gated `if self._is_kiro` (`acp/client.py`). What IS applied before the first prompt is `session/set_config_option("mode", "read-only")`, and a session that never advertised the option is refused (`acp_backends.py`, `acp/client.py`) | **unsupported** in Crew: `require_backend_admission` rejects construction before I/O. **Measured** native sessions defaulted to `build` and advertised `build`/`plan`; those modes are not a verified host gate. |
| Hook event names | Crew's own | Hooks cannot ride an over-the-wire agent (`kas_agents.py`) | Renamed: `PreToolUse` / `PostToolUse` / `UserPromptSubmit` / `SessionStart` / `Stop` (companion) | Crew's own, unrenamed — nothing is projected into the harness, so the gate is reached only through `session/request_permission` → `HookManager.on_tool_call` | **unsupported** — Kiro hooks are not translated into native OpenCode hooks; no guaranteed pre-execution callback has been established. |
| Model pin | `model` in the spec; `"auto"` resolvable | Not projected | Separate `cc_model` sidecar field; cannot resolve `"auto"`, so background agents get a concrete pin (`agent.py`) | over the wire, not in a spec: `session/set_config_option("model", …)` (`acp_backends.py`). `"auto"` is **not** resolved — it is skipped and the adapter's own default stands (`acp/client.py`). Effort takes the same channel; advertised-spelling folding is excluded (`acp_backends.py`) | **measured** — native `session/new` advertises provider-qualified model ids through `configOptions` (`id: model`). The retained catalog helper runs `opencode models`; model switching and `auto` resolution in a real Crew session remain unmeasured and admission-blocked. |
| Protocol version | date-stamped `2025-08-22` | date-stamped | numeric `1` (`acp/client.py`) | numeric `1`, kept as its own literal rather than folded into CC's (`acp/client.py`) | **measured** — numeric `1` from native OpenCode 1.18.23; this does not grant admission. |
...
| | kiro-cli | KAS | CC | Codex | OpenCode |
|---|---|---|---|---|---|
| Replay store | `<kiro home>/sessions/cli/<sid>.json` + `.jsonl`, read to resume (`session_storage.py`) | same store (it *is* kiro-cli) | Its own: `<cc root>/projects/<encoded cwd>/<sid>.jsonl` + `<sid>/`, plus `<cc root>/file-history/<sid>/` (companion: a dedicated transcript-cleanup module exists for exactly this) | None on Crew's side — the adapter keeps its own records and resolves them from the `sessionId` (`acp/client.py`); the session map skips both the transcript pre-check and the prune for any non-kiro label (`session_map.py`) | **unmeasured** — no OpenCode transcript reader or repair path is supplied by Crew; native persistence was not exercised. |
| Store root | `KIRO_HOME` | `KIRO_HOME` | `CLAUDE_CONFIG_DIR` → `<config dir>/cc-config` → `~/.claude` (companion) | not Crew's to compute — `CODEX_HOME` is read only to re-anchor the credential leaf, never to locate a transcript (`security.py`) | **unmeasured** — the isolated probe used a temporary home and XDG roots; no production store location is established here. |
| Directory naming | session id | session id | `realpath(cwd)` with every non-alphanumeric replaced by `-`. **`realpath`, not `abspath`** — on cloud desktops `/home/<user>` is a symlink to `/local/home/<user>`, and an abspath encoding silently misses the transcript directory (companion) | unknown to this repository — nothing here names or reads a codex session directory | **unmeasured** — no native directory-naming contract is relied on by Crew. |
| `session/load` | carries the transcript path, guarded by a file-exists pre-check (`acp/client.py`) | same | carries **no** path; the pre-check is skipped entirely (`acp/client.py`) | carries **no** path and no `_meta`; the file-exists pre-check is skipped, because gating on the kiro transcript would make `file_ok` always False and silently start every resume fresh (`acp/client.py`) | **unmeasured** — `loadSession: true` is advertised by native 1.18.23, but the probe only used `session/new`; resume is not validated. |
| Compaction | asynchronous, reported by `_kiro.dev/compaction/status`; the session id changes | same | `/compact` runs **synchronously inside `session/prompt`**; no status notification ever arrives, and the session id survives (`dashboard/chat_runner.py`) | not supported — excluded from `ACP_BACKENDS_COMPACT`, so a manual `/compact` is refused up front instead of stranding the status waiter (`acp_backends.py`, `acp/session_provider.py`) | **unsupported** in Crew — no compaction capability enrollment; native compaction was disabled in the probe and remains unmeasured. |
| Sessions per process | many (demultiplexed over `AcpRuntime`) | many | **one** (`acp/types.py`) | **one** — not in `ACP_BACKENDS_ACP_RUNTIME`, so it takes the `AcpClient` path, spawned per session (`acp_backends.py`); no shared subagent session either | **unsupported** in Crew — constructors refuse the backend. The dormant transport is per-session; runtime multiplexing and session sharing are not enrolled or measured. |
| History replay | full history every turn, so one oversized image block wedges a transcript permanently and Crew repairs kiro-cli's own file (`session_image_repair.py`) | same | not applicable | not applicable — no Crew-side transcript to replay or repair | **unmeasured** — the corpus contains fresh sessions only; it establishes no full-history replay or transcript-repair behavior. |
...
logout-recycle answer `backends_retired_by_host_logout()`, the `AcpAuthRequired` text
...
| | kiro-cli | KAS | CC | Codex | OpenCode |
|---|---|---|---|---|---|
| `entitlement_source` | `host_identity_store` — `kiro-cli login` writes it; org SSO is `--use-device-flow --license pro` (`kiro_prerequisite.py`) | `host_identity_store` — the same store. Spawned as `kiro-cli acp --agent-engine v3 --auth-method cli`, and that flag is the demonstration: the relay resolves every access token from kiro-cli's own store | `own_credential_file` — brings its own sign-in, and Crew implements no login command for it | `own_credential_file` — brings its own sign-in, and Crew implements neither a login command nor an auth probe | **declared** `own_credential_file` — its own provider sign-in, never Kiro's identity store. Native auth can also arrive through `OPENCODE_AUTH_CONTENT`; Crew does not inspect either source. |
| `credential_leaves` | none of its own. The locations are the HOST's, declared by `identity_stores.IDENTITY_STORE_ROOTS` and spliced onto the floor from there; a harness re-declaring them would hand a driver a say over the host's own store. The store is projected, never copied: identity tables plus `migrations` rows plus selected `state` rows (`kiro_prerequisite.py`) | none of its own — same store, nothing further to fence | `~/.claude/.credentials.json`. Its credential-refresh **command** is named inside its own `settings.json` and copied **verbatim** into the isolated seed at `0o600`; dropping it breaks auth outright, so the seed cannot simply be emptied (companion) | `~/.codex/auth.json`. Crew never reads or copies it and only ever checks that it exists | **declared and source-verified** `~/.local/share/opencode/auth.json` and `mcp-auth.json`, fenced without reading them. Real sign-in remains unmeasured. |
| `home_override_env_vars` | none | none | `CLAUDE_CONFIG_DIR`, `CLAUDE_HOME` | `CODEX_HOME` | `XDG_DATA_HOME` relocates them to `<root>/opencode/<filename>`, not `<root>/<filename>`. Declared relative suffixes feed both the read gate and sandbox mask; OpenCode launches refuse non-absolute overrides. `OPENCODE_TEST_HOME` does not relocate native auth data. |
| `adapter_own_leaves` | none | none | none — outside `tool_gate.ENFORCED_ROUTINGS`, so no credential mask is applied and there is nothing to carve an exception out of. Declaring one anyway would assert something about a control that never runs | `~/.codex/auth.json` — the adapter authenticates ITSELF, so a mask that took its own token away would fail every session at start with an opaque authentication error. The asymmetry is safe: the read gate still refuses that leaf to the AGENT's file tools, so the two controls cover different readers rather than cancelling each other | none — dormant and outside enforced routing, so no credential-mask exemption is granted. |
| `host_logout_retires_children` | `True` — a logout there invalidates a running child | `True` — excluding it would let a KAS session keep serving turns on the previous account's credentials | `False` — a live CC child must survive `kiro-cli logout` | `False` — a `kiro-cli logout` says nothing about whether a running codex child is still authenticated, and retiring its child on that signal would end a live turn for no reason | `False` — Kiro logout says nothing about OpenCode's independent sign-in. |
| `sign_in_remedy` | run `kiro-cli login`, then start a new chat | the same string verbatim — it is kiro-cli's store the relay reads | run `claude` and complete its sign-in, then start a new chat | two branches: complete Codex's own sign-in, or name a model provider in `~/.codex/config.toml` (`CODEX_HOME` moves that folder). Neither is checked here — the adapter reads them | run `opencode auth login` in the operator's terminal, then start a new chat. The declaration is not an auth check or permission to admit a session. |
| `AgentInteractiveLogin` | not implemented — sign-in happens in the operator's own terminal | not implemented — same | not implemented — the harness brings its own | not implemented — the harness brings its own | not implemented — sign-in stays user-driven in the native CLI. |
| Entitlement discovery, model side | account API | account API | runtime, from the advertised model set at session init; the registry is filtered down to it (`dashboard/handlers/agents.py`) | runtime, from the model set advertised at `session/new` / `session/load`. Captured there and persisted to the cross-session provider-model cache under the `codex` namespace, which `GET /api/models` reads back, so a cold dashboard still offers the real list. No account API. No registry filter either — the registry carries no codex provider, so there is nothing to narrow: the picker IS the advertised list plus `auto` (`acp/client.py`, `dashboard/handlers/agents.py`) | **measured** — `configOptions` advertises provider-qualified model ids. The retained `opencode models` catalog is not proof of entitlement or authentication. |
| Readiness probe | `--version` then `whoami`, inside the OS sandbox (`kiro_prerequisite.py`) | same | binary resolution only, but for **both** components and through the spawn's own resolvers, so the answer cannot disagree with what a spawn does (`agent_sdk/backend_install.py`, `agent_sdk/drivers/acp.py`) | one component, adapter resolution only and through the spawn's own resolver: `codex-acp`, reported `installed` / `missing` with the install command, plus `restart_required` when this process cached a negative (`agent_sdk/backend_install.py`). No auth check | **unsupported for session admission** — the shared install resolver can report executable presence, but performs no auth check and cannot clear the admission blocker. |
...
OpenCode's declaration covers the standard absolute-root read floor and the
enforced-adapter mask projection, not every OS subprocess. Generic sandbox masks
and external relative/literal-tilde XDG stores remain outside that guarantee;
[acp-client](acp-client.md) records these unresolved limits. Neither this table
nor its declaration clears the execution-admission blocker.
...
| | kiro-cli | KAS | CC | Codex | OpenCode |
|---|---|---|---|---|---|
| Unit | Kiro credits (`bills_kiro_credits`, membership-only and False for unknown ids) | Kiro credits | US dollars per token | unknown to this repository. `TurnUsage` carries both `credits` and `cost_usd` and each harness fills what it bills in (`acp/types.py`), and the generic `parse_usage_cost` picks up a `cost: {amount, currency}` on `usage_update` from any adapter that sends one, USD only (`acp/_dispatch.py`). Whether codex-acp sends it is not established here | **measured shape only** — `usage_update.cost` carried `{amount: 0, currency: USD}` and the prompt response carried zero token counts from the fake model. Real billing units and unattributable overhead are unmeasured. |
| Quota API | RTS endpoints, target prefix `com.amazon.aws.codewhisperer.runtime.AmazonCodeWhispererService` (`dashboard/handlers/kiro_usage_api.py`) | same | none; cost is computed from token counts | none in core — no codex path in `dashboard/handlers/kiro_usage_api.py` | **unsupported** — no OpenCode quota API integration exists. |
| Token discovery | `data.sqlite3` across four per-OS locations × two product names, four key spellings (`kiro_usage_api.py`) | same | not applicable | not applicable — `.codex/auth.json` is the adapter's own store, classified on the read-gate floor and never read by Crew, which only checks existence to name a sign-in command (`security.py`) | **unsupported** — Crew does not discover a provider billing token; the isolated probe used no real credential. |
| Consumer surface | a boolean flag read by the session readout and `dashboard/state.py` | same | `GET /api/usage/cost` returns `mode="cost"` vs `mode="kiro"`, plus a companion-side budget cap (companion) | none, and this is the one row in this bucket that is a gap rather than a seam: the credit pill's background fetch is not harness-aware — it gates on `kiro-cli` being resolvable, not on the session's backend (`dashboard/handlers/sessions.py`) — so a codex session on a host with kiro-cli installed still shows Kiro credits | **unsupported** — no admitted OpenCode session or verified usage UI. Parser snapshots preserve the measured zero-cost frames without claiming real accounting. |
...
| | kiro-cli | KAS | CC | Codex | OpenCode |
|---|---|---|---|---|---|
| How a tool result arrives | `content[].content.text` blocks, or `rawOutput.items[].Text` / `.Json.stdout` | `content[].content.text` blocks, or a **flat `rawOutput` object with no `items`** (measured: `{output, exitCode, message}`, `{response, imageBase64Urls, message}`, `{kind, retracted}`) — and an MCP result can arrive **already-serialised**, **copied into several fields**, one field **replaced by an offload reference** above a size threshold, and every string **capped at 30k chars** (head 500 + tail 500) | `content[]` text blocks | unmeasured — no codex tool-result shape is recorded. Codex adds no builder of its own and rides the shared `_build_tool_result_event` path, which handles `content[].content.text`, `rawOutput.items[]`, and any other non-empty `rawOutput` object by serialising it (`acp/_dispatch.py`) | **measured** — successful calls use `content[].content.text` plus `rawOutput: {output, metadata}`; failures use the same content nesting plus `rawOutput: {error}`. Read output includes a text wrapper and metadata display object. |
| Directive marker readable in the result | yes | **no**, and it does not need to be | yes | unmeasured, and it does not need to be | **unmeasured** — no control-marker tool was exercised. The mirror withholds Crew tools and no session-directive compatibility is claimed. |
| How the tool call's `rawInput` arrives | complete on `tool_call` | complete on `tool_call`, uncapped (`tool-call-emitter.ts` passes it around its wire cap (kiro-agent `tool-call-emitter.ts`)); carries a `_meta` block the MCP server never sees | empty on `tool_call`, complete on `tool_call_update` | unmeasured — rides the shared `_build_tool_call_event` / `_build_tool_refinement_event`, which record `raw_tool_params` from whichever frame carries it | **measured** — write/read start with `{}` and carry complete arguments on `tool_call_update`; bash starts with `cwd` and later carries command/description. The no-argument MCP tool stays `{}`. Large-input caps are unmeasured. |
| Wire title of an MCP tool call | `_meta.kiro` identity, title not needed | `Running: @<serverName>/<toolName>` (measured; the `Running: ` prefix is the backend's, and `directive_tool_from_call` strips one) | raw Claude tool name `mcp__<server>__<tool>` | unmeasured — a codex MCP call whose title matches neither spelling above resolves to no directive tool and records no digest; declaring the spelling is part of onboarding the backend | **measured** — `probe_ping` with no `_meta.kiro` identity. Crew directive names, normalization collisions, and digest matching are unmeasured; no supported directive channel is claimed. |
...
Codex was the first backend added after that census, so it is the first datapoint
...
narrowed, though: the baseline covers the four admitted backends. The known but
dormant OpenCode id cannot be admitted by registration alone: the same blocker
guards registration and construction. An edition may register other harnesses
through the seam, but installation or naming does not establish enforcement.
Everything below it is still incoherent: the behaviour a companion must
