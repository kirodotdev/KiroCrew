...
| | kiro-cli | KAS | CC | Codex | OpenCode | pi-acp |
|---|---|---|---|---|---| --- |
| Unit | Kiro credits (`bills_kiro_credits`, membership-only and False for unknown ids) | Kiro credits | US dollars per token | unknown to this repository. `TurnUsage` carries both `credits` and `cost_usd` and each harness fills what it bills in (`acp/types.py`), and the generic `parse_usage_cost` picks up a `cost: {amount, currency}` on `usage_update` from any adapter that sends one, USD only (`acp/_dispatch.py`). Whether codex-acp sends it is not established here | tokens, reported per turn as a `usage_update` on `session/update` with a `cost` object beside them; a locally served model reports `cost` `0` | unknown to this repository. `TurnUsage` carries both `credits` and `cost_usd` and each harness fills what it bills in (`acp/types.py`), and the generic `parse_usage_cost` picks up a `cost: {amount, currency}` on `usage_update` from any adapter that sends one, USD only (`acp/_dispatch.py`). Whether pi-acp sends one is not established here — the adapter emits no `usage_update` in slices 1–4 |
