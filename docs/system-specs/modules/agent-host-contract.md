...
| **DeepSeek** | `ACP_BACKEND_DEEPSEEK = "deepseek"` — the `dsh` binary booting its shipped `acp` profile as `dsh --profile acp`. KNOWN but **not selectable** on a public build: it has an install probe, and its tool calls do not reach Crew's permission gate, so offering the switch would offer a harness Crew cannot gate (§7). |

## 1. Agent definition and layout

| | kiro-cli | KAS | CC | Codex | OpenCode | DeepSeek |
|---|---|---|---|---|---|---|
...
| Store root | `KIRO_HOME` | `KIRO_HOME` | `CLAUDE_CONFIG_DIR` → `<config dir>/cc-config` → `~/.claude` (companion) | not Crew's to compute — `CODEX_HOME` is read only to re-anchor the credential leaf, never to locate a transcript (`security.py`) | not Crew's to compute — `XDG_DATA_HOME` is read only to re-anchor the credential leaf, never to locate a transcript (`security/paths.py`) | not Crew's to compute — `DSH_HOME` is read only to re-anchor the credential leaves, never to locate a transcript (`agent_sdk/host_auth.py`) |
| Directory naming | session id | session id | `realpath(cwd)` with every non-alphanumeric replaced by `-`. **`realpath`, not `abspath`** — on cloud desktops `/home/<user>` is a symlink to `/local/home/<user>`, and an abspath encoding silently misses the transcript directory (companion) | unknown to this repository — nothing here names or reads a codex session directory | unknown to this repository — nothing here names or reads an opencode session directory | the harness's own: the session's absolute `cwd` with separators replaced, then the session id beneath it. Observed on disk, and read by nothing in this repository |
| `session/load` | carries the transcript path, guarded by a file-exists pre-check (`acp/client.py`) | same | carries **no** path; the pre-check is skipped entirely (`acp/client.py`) | carries **no** path and no `_meta`; the file-exists pre-check is skipped, because gating on the kiro transcript would make `file_ok` always False and silently start every resume fresh (`acp/client.py`) | carries **no** path and no `_meta`; the file-exists pre-check is skipped, for the same reason as the two adapters above (`acp/client.py`) | **not implemented** — answered `-32601 "Method not found": session/load`, captured in `test/fixtures/acp_frames/deepseek/handshake-live.jsonl`. It advertises `sessionCapabilities.resume` and serves `session/resume` instead, so it is the sole member of `ACP_BACKENDS_RESUME_WITHOUT_LOAD`, which keys BOTH the capability read and the verb sent (`acp/client.py`) |
| Compaction | asynchronous, reported by `_kiro.dev/compaction/status`; the session id changes | same | `/compact` runs **synchronously inside `session/prompt`**; no status notification ever arrives, and the session id survives (`dashboard/chat_runner.py`) | not supported — excluded from `ACP_BACKENDS_COMPACT`, so a manual `/compact` is refused up front instead of stranding the status waiter (`acp_backends.py`, `acp/session_provider.py`) | not supported — it advertises no compaction capability at all, so it is outside `ACP_BACKENDS_COMPACT` and a manual `/compact` is refused up front rather than stranding the status waiter (`acp_backends.py`) | not supported on the wire — it carries compaction internally, and its ACP surface rejects commands and publishes no compaction status, so it is outside `ACP_BACKENDS_COMPACT` and a manual `/compact` is refused up front rather than stranding the status waiter |
...
| | kiro-cli | KAS | CC | Codex | OpenCode | DeepSeek |
|---|---|---|---|---|---|---|
| Unit | Kiro credits (`bills_kiro_credits`, membership-only and False for unknown ids) | Kiro credits | US dollars per token | unknown to this repository. `TurnUsage` carries both `credits` and `cost_usd` and each harness fills what it bills in (`acp/types.py`), and the generic `parse_usage_cost` picks up a `cost: {amount, currency}` on `usage_update` from any adapter that sends one, USD only (`acp/_dispatch.py`). Whether codex-acp sends it is not established here | tokens, reported per turn as a `usage_update` on `session/update` with a `cost` object beside them; a locally served model reports `cost` `0` | tokens, reported per turn as a `usage_update` on `session/update` carrying `used` and `size` (captured in `turn-live.jsonl`). No `cost` object was observed |
