...
- **In-memory first, observe mode persists**: ordinary channels are ephemeral
  ...
  orderly exit/restart path drains the lane
  before `exec`/`os._exit` (the force-exit signal handler is the exception:
  it cannot wait on the loop thread and abandons queued writes), and a
  survivable exec failure must call
