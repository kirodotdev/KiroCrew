...
Heartbeat runs in its own session (`HEARTBEAT_KEY = "_hb"` in `session.py`), distinct from the shared `BACKGROUND_KEY = "_bg"` used by cron / consolidator / chat-title. The session uses the dedicated `kirocrew-heartbeat` agent (installed by `_install_heartbeat_agent` in `agent.py`) — `kirocrew-core` plus any servers an edition names via `McpToolingProvider.extra_heartbeat_mcp_servers()` (public default `{}`, so a standalone install is `kirocrew-core` only) so cycle cold-starts stay cheap. SEL audit logging stays gateway-side in `_heartbeat_approval` regardless; the per-agent narrowing is purely a cold-start cost reduction.
...
Heartbeat is unattended — there is no human to click an approval button. Tool approval uses `HOOK_BASED` policy with a heartbeat-scoped `HookManager` (built once at init by `_build_heartbeat_hooks`) and a custom callback (`GatewayOrchestrator._heartbeat_approval`) that auto-approves only tools whose event carries a positively-verified identity matching `HEARTBEAT_SAFE_TOOLS`' rules and rejects everything else with a SEL audit event (`outcome=denied`, `reason=not_in_heartbeat_safe_tools`). Both approve and deny outcomes emit `log_tool_invocation` so every permission decision is auditable.

The heartbeat-scoped hooks drop the user's `auto_approve_tools` so the allowlist is the **sole approval authority** — `llm_helpers._resolve_permission` would otherwise consult the hooks BEFORE the `_heartbeat_approval` callback, and a user config like `auto_approve_tools=["*"]` would auto-approve any tool, bypassing `HEARTBEAT_SAFE_TOOLS` entirely. The user's `auto_deny_tools` IS preserved (denies can only narrow what runs in heartbeat, never widen). The heartbeat-scoped hooks also set `deny_kind_based_read_only=True`, which suppresses **three** unconditional non-shell auto-approve paths inside `HookManager.on_tool_call`: the ACP `tool_kind in {"read", "fetch"}` branch, the computer-use observe-tool branch (`_cu_read_only_auto_approve`), and the separate kind-**absent** fallback (`_is_read_only_tool(tool_name)`) that runs when `tool_kind` is empty. All three are agent-influenced — `tool_kind` is passed through verbatim from the ACP `kind` field, the computer-use branch's `tool_name` comes from the model-preferred `description`, and `_is_read_only_tool` matches only the model-authored title with no identity check at all — so without this suppression a mutating edition-contributed MCP tool would auto-approve BEFORE `_heartbeat_approval` ever runs, by declaring `kind="read"`, by forging an observe-shaped computer-use title, or by omitting `kind` and relying on a read-shaped title (e.g. `read_all_credentials`), bypassing the allowlist and SEL audit entirely either way. A suppressed match on any of the three falls through to `_heartbeat_approval`'s own decision, same as any other non-match. (The shell arm is a fourth, separately-gated path — see below.)

A hook `TOOL_AUTO_APPROVE` that DOES fire in `_resolve_permission` for a **shell** command (via the name-based grant tiers, e.g. `auto_approve_tools` globs — the read-only tier does NOT survive the heartbeat scoping, per `deny_kind_based_read_only` above; cron and autonudge turns run with the user's full hooks and do see it) is honoured only after the name-grant check confirms each program name still resolves to the program it appears to name; a refusal downgrades to the caller's approver — `_heartbeat_approval` (allowlist) or the interactive card — never a hard block, audited as `outcome=auto_approve_declined` with `reason=name_grant`. On Windows the check declines every name-based shell grant, so unattended cron/autonudge/heartbeat turns there fall to their approver for shell tools the read-only tier used to auto-approve. The check runs UNCONDITIONALLY; a refusal on a caller with no interactive approver rejects the tool (deny-by-default) rather than falling through to the caller-less auto-approve — an unattended turn is exactly where a shadowed name would otherwise run unwatched.

The allowlist is name-based and requires a positively-verified identity — no verb / heuristic fallback, and no fallback to the LLM-authored display title. Heartbeat polls untrusted external content (CR comments, ticket bodies) where prompt-injection could try to widen approval via a clever read-shaped tool name (`get_all_credentials`, `list_env_secrets`, etc.) or a forged title. Strict enforcement is auditable and cannot be widened that way; this is deny-by-default per the security-controls guideline.

The allowlist is curated for read-only / observation tools — the trusted host builtins from `hooks._HOST_READ_ONLY_BUILTIN_TOOLS` minus network egress (`fs_read`, `glob`, `grep` — `web_fetch`/`web_search` are heartbeat-excluded, see below), and side-effect-free reads from `kirocrew-core` (`learn_list`, `spawn_list`, `spawn_status`, `artifact_list`, `artifact_get`, `artifact_versions`, `local_knowledge_search`) and `kirocrew-cron` (`cron_list`), each qualified to its own owning MCP server. A read-only **shell** command with a genuinely non-empty, trusted host identity also auto-approves — see `slack-gateway.md`'s Heartbeat Tool Allowlist section for the exact rule and the name-grant check it goes through. **`git` is excluded from this entirely**, regardless of classification: git reads repo-local `.git/config` on invocation, and `core.fsmonitor`/`core.hooksPath` name a program from that file rather than the command line the read-only classifier inspects — a channel the interactive/cron paths' human backstop covers but heartbeat's zero-approver boundary does not. (The enterprise-internal read APIs — internal code/knowledge search, code-review/ticketing/pipeline/deploy/on-call reads, `recall` — were removed from the public fork's allowlist; an internal companion re-adds them out of band.) Write tools (`send_message`, `file_send`, `cron_add`, `fs_write`) and any shell command the deny-by-default classifier does not confirm read-only are not in the list and are rejected.
