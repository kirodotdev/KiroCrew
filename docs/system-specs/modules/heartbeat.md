...
The heartbeat-scoped hooks drop the user's `auto_approve_tools` so the allowlist is the **sole approval authority** — `llm_helpers._resolve_permission` would otherwise consult the hooks BEFORE the `_heartbeat_approval` callback, and a user config like `auto_approve_tools=["*"]` would auto-approve any tool, bypassing `HEARTBEAT_SAFE_TOOLS` entirely. The user's `auto_deny_tools` IS preserved (denies can only narrow what runs in heartbeat, never widen). The heartbeat-scoped hooks also set `deny_kind_based_read_only=True`, which suppresses **two** unconditional auto-approve paths inside `HookManager.on_tool_call` for a non-shell call: the ACP `tool_kind in {"read", "fetch"}` branch, and the separate kind-**absent** fallback (`_is_read_only_tool(tool_name)`) that runs when `tool_kind` is empty. Both are agent-influenced — `tool_kind` is passed through verbatim from the ACP `kind` field, and `_is_read_only_tool` matches only the model-authored title, with no identity check at all — so without this suppression a mutating edition-contributed MCP tool would auto-approve BEFORE `_heartbeat_approval` ever runs, either by declaring `kind="read"` or by omitting `kind` and relying on a read-shaped title (e.g. `read_all_credentials`), bypassing the allowlist and SEL audit entirely either way. A suppressed match on either path falls through to `_heartbeat_approval`'s own decision, same as any other non-match.
...
The allowlist is curated for read-only / observation tools — the trusted host builtins from `hooks._HOST_READ_ONLY_BUILTIN_TOOLS` (`fs_read`, `glob`, `grep`, `web_fetch`, `web_search`), and side-effect-free reads from `kirocrew-core` (`learn_list`, `cron_list`, `spawn_list`, `spawn_status`, `artifact_list`, `artifact_get`, `artifact_versions`, `local_knowledge_search`), each qualified to its own owning MCP server. (The enterprise-internal read APIs — internal code/knowledge search, code-review/ticketing/pipeline/deploy/on-call reads, `recall` — were removed from the public fork's allowlist; an internal companion re-adds them out of band.) Write tools (`send_message`, `file_send`, `cron_add`, `fs_write`, shell `execute`/`run`) are not in the list and are rejected.
