...
- **Network egress is heartbeat-excluded** (`_HEARTBEAT_EXCLUDED_BUILTIN_TOOLS = {"web_fetch", "web_search"}`): both are trusted read-only builtins on the interactive path (a human approver is present there), but heartbeat's own allowlist never grants outbound network calls — it polls untrusted external content by design, and auto-approving egress on that surface is a materially different risk than auto-approving a local read. The exclusion applies only inside `_is_heartbeat_safe_tool`; `hooks._is_host_read_only_builtin` (the interactive path) is untouched. `HEARTBEAT_SAFE_TOOLS` is subtracted the same way, so the public alias does not advertise names it will never approve.
- **Shell commands** (`event.is_shell`) get a fourth, dedicated branch, reached only past the top-level `mcp_identity_trusted` gate:
  - `mcp_server_name` must be empty (only a true host shell tool has no server behind it — the same discipline as the builtin branch above) AND `tool_name` must be genuinely non-empty. The second check exists because a cache HIT on two empty strings still satisfies `mcp_identity_trusted` (`acp/_dispatch.py`'s identity cache writes `""` unconditionally for a backend that never populates `_meta.kiro` at all, and presence, not value, is what the trust flag checks) — without it, any backend lacking real `_meta.kiro` support would auto-approve a shell command on the recovered text alone, with zero actual identity behind it. The empty-`tool_name` check is UNCONDITIONAL regardless of `mcp_server_name`; a present `mcp_server_name` on its own does not deny outright — `event.is_shell` is driven by the agent-declared ACP `kind` field, not by whether a server backs the call, so an edition MCP tool can legitimately declare `kind="execute"` while carrying a real server. That case falls through past this branch to the core-MCP-tool and edition-pinned matching below, rather than being judged on command text at all.
  - The command is then recovered via `AcpEvent.shell_command` (host-observed bytes from the tool_call cache, never the agent-declared `kind` or the LLM-authored title) and judged by the same deny-by-default classifier (`is_read_only_bash`) the interactive path already trusts for this exact call — EXCEPT `git`. Every `git` invocation is denied here regardless of classification, because git reads repo-local `.git/config` on invocation, and `core.fsmonitor`/`core.hooksPath` name a program from that file, not from the command line the classifier inspects; a heartbeat cycle's CWD is exactly the kind of place a local writer could plant one, and heartbeat has no approver to backstop it the way the interactive/cron paths do. All `git` subcommands are excluded (not only the ones on the read-only allowlist), since the vector is git running at all.
  - `GatewayOrchestrator._heartbeat_approval` adds one more check on top, specific to shell: `name_grant.refusal_for_event(event)`, the same identity-verification every other unattended shell auto-approve path (`llm_helpers._resolve_permission`) runs unconditionally — the shell resolves the program name again through a `PATH` that can lead with agent-writable directories, so a read-shaped command can still execute a shadowed binary. A refusal denies via `name_grant.log_decline`, the shared audit writer; it never falls through to a silent auto-approve.
  - Residual risk accepted, not overlooked: `is_read_only_bash` is a text classifier and cannot see a mutation performed through a side channel a read-shaped command string hides — the same imperfection every other unattended session already accepts from this classifier. The `git`-config channel above was the one instance of this class concrete enough to close outright rather than accept.
