...
20. Strip inline `<thinking>` tags from accumulated text, then rebind `accumulated` through one `redact_with_findings_via_context` boundary — provider chunks stay raw until here so a compressed pako fragment is validated only once complete, and this is the one host-aware call on the answer path before the render, so a link whose decoded state carries a companion-only token is refused rather than logged. Before this boundary the raw buffer reaches only the live cursor edit (wrapped in `redact_pako_via_context`: baseline on ordinary text, active policy on decoded pako state) and the thinking-tag/control-comment/comment-hold transforms; after it nothing rebinds the buffer, and the one raw alias that outlives it, `_untrimmed`, reaches only `extract_options` (body redacted by the host-aware render, choices by `build_options_blocks`) and `_resolve_comment_hold` (released bytes go through `_append_stream`'s redactor). `slack.format.render_for_slack` remains the principal full-context renderer. Its shared overflow helper receives that same redactor explicitly; the `SlackRenderer` transport sibling and native OPTIONS fallback use the narrow seam because their inputs are already authorized. Raw/proactive helpers are not widened. All of it is pinned structurally by `test_raw_turn_egress.py`, the Slack sibling of the dashboard's raw `assistant_text` guard; its allowlists are a security-review surface
