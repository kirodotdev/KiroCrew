...
A key file that stays shorter than 32 bytes is repaired rather than tolerated. When the publish loop exhausts its budget against a *regular* `token_signing.key` shorter than 32 bytes whose mtime is older than `_POISONED_KEY_MIN_AGE_SECONDS` (30 s — long enough that an in-place creator caught between its `O_EXCL` create and its write is left alone), the loader removes it and runs the publish loop once more, so the single-creator election mints the replacement. No boot accepts a file shorter than 32 bytes as a key, and removal avoids preserving potentially sensitive partial key bytes under a sibling name the OS sandbox does not mask. The removal is logged at WARNING with the byte count and age, because every cookie and Slack link signed under the ephemeral secret that preceded it is invalid. The heal runs at most once per load, so a file that cannot be repaired degrades to the ephemeral secret rather than looping.

Both the healer and the in-place creator serialize on an advisory lock (`platform_compat.file_lock`) over the sibling `.token_signing.key.heal.lock`, which the sandbox seals read-only and pre-creates. The lock is opened with `O_NOFOLLOW`, so a symlink at the lock path refuses both the heal and the in-place create rather than locking through it. The lock stops a healer from removing the 0-byte destination an in-place creator is about to write, and stops two healers from removing each other's freshly published key. The in-place creator acquires single-shot because its 50-attempt loop provides the retry. Off the asyncio event-loop thread, the healer waits up to `_HEAL_LOCK_WAIT_SECONDS` (30 s), a ceiling sized against both holders: a healer's critical section is an `lstat` plus an `unlink` and lasts milliseconds, while the in-place creator holds the lock across the owner-only lockdown, the key write and the `fsync`, which on a slow link-less mount can take seconds. That bound lets an ordinary contender re-judge either holder's result under the lock instead of reading a healthy creator's fsync as a stall, while a holder still holding past the ceiling is treated as wedged and the contender runs the bounded extra pass. On the event-loop thread, `platform_compat.file_lock` remains single-shot and a contended healer also requests the extra pass without blocking the loop.
