...
- Superseded attempts collapse to the newest per check identity. A host leaves a
  ...
  comparison that picks the newest run. An absent run conclusion exempts it the same
  way, since `CheckSuite.conclusion` is null while a run is still going and evidence
  the host withheld is not evidence a row was replaced. Over-reporting costs a turn; hiding a
