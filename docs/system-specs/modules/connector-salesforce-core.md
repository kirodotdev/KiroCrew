...
a caller; it defines nothing outside its own `vendors/salesforce/**`. It does no
runtime wiring: it does not import the connections runtime registry or
`platform/governance.py`, and it neither mints a token nor opens a socket. Its
error classifier does import the shared RUN-01 taxonomy the control plane owns
(`kiro_crew.connections.control_plane.ErrorClass` / `ERROR_CLASSES`) and maps
Salesforce failures INTO that closed set rather than forking a second taxonomy —
the same design the sibling `vendors/github/errors.py` follows.
...
`parse_record(describe, record)` types each field of a query/retrieve record
against its describe. A field whose describe `type` is a corroborated
discriminator must arrive as the matching JSON type; a mismatch raises
`PayloadParseError` rather than coercing. An integer numeric value is preserved
as an `int` (no lossy `float()` that would corrupt integers above 2**53), and a
field whose describe `type` is `UNKNOWN` passes through verbatim.

A SOQL result is not always a flat set of the queried object's own scalar
fields, and the parser handles the two non-scalar shapes explicitly rather than
rejecting them as "undescribed":

- **Relationship traversal** (`SELECT Owner.Name FROM Account`) nests a child
  record under the relationship key: `{"Owner": {"attributes": {"type": "User",
  ...}, "Name": ...}}`. The child is a record of a *different* object, named by
  its own `attributes.type`. Offline the core has no describe for that related
  object, so it recurses under `UNKNOWN` semantics — it preserves the child's
  `attributes` and returns the child's own values verbatim, never guessing the
  related object's field types and never coercing a value. A caller that *does*
  have the related describe passes it via `related={type: ObjectDescribe}`, and
  the child is then type-checked against it.
- **Aggregate aliases** (`SELECT COUNT(Id) FROM Account` → `{"expr0": 12}`) are
  computed columns with no field of their own; an `exprN` key is passed through
  verbatim.

Neither shape widens the general rule: a plain scalar key that is not in the
describe, is not an `exprN` alias, and is not a nested relationship record still
raises `PayloadParseError`, so a genuine field-name typo is still caught.
...
- **Page-size hint:** the `Sforce-Query-Options` request header (`batchSize=N`)
  is a documented request-header hint. The offline core encodes no numeric
  bound for it — nothing offline constructs or exercises a batch size, so no
  specific minimum/maximum is asserted here rather than stated untested.
...
- **Authorized `@RestResource`** — a named method at a fixed URL mapping, one of
  a fixed pre-declared set. `register_authorized_rest_resource(...)` admits a
  candidate only if its COMPLETE descriptor `(class_name, method_name,
  url_mapping, http_annotation)` matches an entry in the deployment's explicit
  authorized allow-list exactly. The authorized unit is the whole four-element
  descriptor, not the `Class.method` identifier alone: an authorized method
  re-pointed at a different verb or URL mapping is a different capability and is
  refused. Shape validity (a real Apex verb annotation, a non-empty `/` path) is
  a precondition, not the authorization; the annotation alone does not authorize.
...
| `Sforce-Query-Options` `batchSize` is the page-size request header (no numeric bound asserted offline) | `search_snippet_corroborated` | QueryOptions.batchSize snippet |
