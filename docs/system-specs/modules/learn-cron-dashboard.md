...
**Desktop window and session close** — On macOS, `Cmd+W` is the native
Window → Close Window command in `website/electron/app-menu.js`. It targets the focused
window independently of renderer state or the dashboard shortcut toggle. The main
window follows its existing close handler in `window-lifecycle.js`: hide to the
tray, leaving native fullscreen first, while the gateway and sessions stay alive.
Every secondary dashboard window shows a native confirmation before closing,
including a clean or unresponsive dashboard. Its warning mentions browser pages
only when they are open. No renderer is closed to probe for unsaved work: that
could destroy a consenting page before a later veto or discard a dashboard whose
unload handler times out. Keeping the window open is the default and preserves
every page, control owner and ongoing browser work.
Explicitly closing releases browser control owners before destroying their
pages. New browser pages cannot open while a close decision is pending.
Browser-panel, fullscreen, maximize and theme callbacks tolerate a dashboard
view whose web contents have already been destroyed.
Other windows follow their own close handlers. Session close remains
`Option+Shift+W` on macOS; `lib/shortcutRegistry.ts` advertises that chord and does
not claim `Cmd+W`. Windows/Linux keep `Ctrl+W` for session close,
`Alt+Shift+W` as its alias, and `Ctrl+Shift+W` for window close. The shortcut
reference marks only the primary-modifier defaults as browser-reserved, so the
macOS session-close chord is advertised as usable in both browser and desktop.
