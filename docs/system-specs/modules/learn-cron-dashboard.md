...
- `GET /api/tasks/summary` — `{generated_at, available, depth{by_state,
  ...
  waits for capacity) and `recovering` (it is being retried). `lanes` merges the health monitor's registered cap
...
Backends restarting + Stalled — and no counter can drift from the rows beneath it.
