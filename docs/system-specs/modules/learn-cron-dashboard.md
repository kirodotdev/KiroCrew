...
- **Script-mode HTTP attestation**: the key is the caller's own word, so the launcher also mints one signed token per run (`_publish_script_session_token`: minted with `mcp_gateway.claim.mint_stub_session_token`, published through `session_token_sig.publish_session_token`, retracted in the run's `finally`) and exports it as `KIROCREW_STUB_SESSION_TOKEN`. `ScriptContext` reads that variable at construction without popping it — `McpToolClient` builds every server child's env from the script's `os.environ`, and those children resolve their identity from the same variable — and `_post` sends it as `X-Session-Token` beside `X-Session-Key`, through `session_token_sig.session_token_header` so the header has one spelling. This is what lets a script reach an owner-surface route as its job: `guard_owner_surface_routes` wraps `POST /api/crons/{id}/run` (among others), `member_request_scope` accepts a declared key only when `session_key_is_attested` — kernel peer attestation, which a loopback TCP caller never has, or a token that verifies back to the declared key — and a request carrying the key alone is answered 409 `member_identity_unavailable` and audited as `api_cron_run denied`. `/api/send-message` is not an owner-surface route, so `notify()` works with or without the token. `_post` returns a gateway refusal as `{"error": "HTTP <status>: <gateway reason>", "status_code": <status>, "code": <gateway code>}` (`code` present only when the body carries an identifier-shaped one; the reason is redacted like other inbound text) rather than the bare status line, so a script can tell a refused identity from a job that is already running; a transport failure keeps the plain `{"error": str(exc)}` shape.
