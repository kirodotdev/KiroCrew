...
**OpenCode and Codex model pickers**: GET `/api/models` uses a shared advertised-list
assembler through explicit backend wrappers. It selects the newest non-empty live
advertisement in the selected backend's namespace, then that namespace's
cross-session cache (`opencode` or `codex`); neither can supply the other's list.
Provider-qualified model ids retain their spelling and are deduplicated by exact
id rather than Kiro or Claude's alias normalization. `auto` leads as the
backend-default sentinel, including for `default` aliases; a configured
pin is included only while no advertised list is known. With no session, cache
or pin, the response contains `auto` alone. This route does not consult Kiro
readiness or start a model-catalog subprocess; a subsequent session's advertised
list supplies the real choices through the existing capture/cache mechanism.
