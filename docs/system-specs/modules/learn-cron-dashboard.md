...
supplied `model` before persisting it. A PUT that changes `acp_backend` must
explicitly include both `model` and `reasoning_effort`; empty strings clear
the pins. Missing choices return 400 `backend_choices_required` before field
mutation, private-memory allocation, persistence or session-default refresh.
Existing model and private-memory refusals retain precedence. An unchanged
backend permits partial updates without resupplying the pins.
ACP pins use the shared role-model
