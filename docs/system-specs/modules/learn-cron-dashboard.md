...
**Chat**: POST `/api/chat` (SSE stream, or JSON with `?ws=1` — chunks via WebSocket), `/api/chat/slots` (CRUD, POST accepts optional `agent` field to set agent at creation; list responses include `source_links` extracted from slot messages with cached provider/number/state/CI metadata), resume from history, POST `/api/chat/slots/{slot}/generate-title`, POST `/api/chat/slots/{slot}/agent` (switch agent for slot), POST `/api/chat/slots/{slot}/fork` (fork session — copies visible messages into new slot, body: `{at_message_index?, prompt?}`, returns `{ok, key, title, messages, prompt}`, new slot has `forked_from` metadata), POST `/api/chat/slots/{slot}/merge-back` (merge a finished fork back into its parent — summarizes the fork's conversation, appends a durable `merged_summary` row + pending-context enqueue to the parent, then archives the fork read-only with slot-owned `merged` meta (folded to `closed` on save; a merged slot's history meta carries `merged: true` and resume keeps it read-only); the transition reserves BOTH slots (fork `_merging`, parent `_merge_reserved`) so deletes/closes/turns answer 409 `merge_in_progress` mid-flight, is idempotent across restarts via a content-keyed `merge_key` receipt, and answers 409 `session_merged` on an already-merged fork, 404 on a missing parent, 503 `fork_flush_failed` when the fork cannot be durably persisted first), POST `/api/chat/slots/{slot}/edit-resend` (edit a user message and re-run; in-place truncation of `slot.messages`, body: `{index?, ts?, content}`), POST `/api/chat/slots/{slot}/rewind` (edit any past user message and re-run; fork-and-swap — truncates `slot.messages`, removes the slot's ACP session via `SessionManager.remove`, deletes orphaned kiro-cli session JSONL at `~/.kiro/sessions/cli/<id>.json[l]`, then runs the edited prompt against a fresh ACP session under the same slot key/title/folder. Mirrors kiro-cli `/rewind`. Body: `{at_message_index?, ts?, content}`), PATCH `/api/chat/slots/{slot}/mode` (switch session mode between `""` and `"orchestrator"` — `_VALID_MODES`; 404 missing slot, 400 invalid mode, 409 while the session is running)
