...
recheck it before publication. Two invariants hold across every Project module
and are pinned by `test/test_project_read_guards.py`: (1) every read of an
install-level or derived file the agent could have written (the activation
record under `trust/project-bundles/`, the install `mcp.json`, installed agent
configs, the registry, a checkout's provenance record) goes through one
hardened reader -- link refused in the same operation as the open, one regular
singly-linked file, a fixed byte ceiling -- never a by-name `read_text`; (2) the
Git store never takes a coordinate from inside a checkout. A managed clone's
`remote` and `default_branch` are pinned in its registration at add time, read
from the fresh staging clone before it is published, and `sync` fetches exactly
that URL and branch and fast-forwards `FETCH_HEAD`; the clone's own `origin`
and `HEAD` are agent-writable derived state that an agent can rewrite without
redirecting the owner's sync. A derived repo checkout carries a
`sources/<id>.source.json` record of the remote and branch it was cloned from;
both readers of a checkout (activation and the read-only session attachment)
compare it to the manifest's current declaration and treat a mismatch as
"re-clone" or "unavailable" respectively, never as the declared source.
Project selection, create, and add state live in
