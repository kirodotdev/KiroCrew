...
**Project-scoped roster path safety**: A `GET /api/agents?project_path=...` scan is descriptor-pinned through `kiro_crew.pinned_fs` where the platform supports that walk: the ancestor chain to `<project_dir>/.kiro/agents` is resolved once and opened component by component (`pinned_fs.open_in_pinned_parent` → `pin_parent`), so a component swapped for a link after the resolve is refused rather than traversed, and the leaf's own resolved target — read from the held descriptor via `pinned_fs.fd_real_path`, never re-derived from the name — is what `is_sensitive_path` judges. On that capable path, `handlers/agents.py` passes `raise_unverifiable=True` for an owner-supplied query, and `agent_discovery.ScanUnverifiable` reaches the endpoint as 503 rather than a 200 with an empty roster.

Where `pinned_fs.supports_pinned_walk()` is false, the same handler deliberately leaves `raise_unverifiable=False`. The scan then uses its `unsupported_ok` by-name fallback instead of answering 503, preserving project-agent discovery on that platform. That fallback screens linked ancestors and sensitive targets before enumeration, but retains a check-to-use window: an ancestor swapped for a link between the check and the by-name read can be followed. This is distinct from both an audited denial (`None`, a resolved-sensitive target) and absence (`()`, nothing to enumerate).
