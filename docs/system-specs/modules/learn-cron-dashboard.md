...
mixing a detached campaign with a newly targeted root. Cycle discovery accepts
...
directory pins do not receive DELETE access. Links/reparse points, hardlinks,
unreadable identities, ancestor/root/campaign swaps, or partial child cleanup
still refuse; any refusal keeps the database row so the same campaign deletion
remains retryable.
...
A fork creates a child before its descriptor-bound seed copy, so the route treats that interval as a compensating transaction. It keeps the parent identity pinned, closes the child identity before cleanup, and invokes the normal cleanup-first `delete_campaign` path on every child-identity or parent-copy failure. An oversized parent report returns the explicit `parent_findings_too_large` 413 only after both the child row and owned directory are gone; other copy refusals return `campaign_fork_failed`. If identity-safe cleanup itself cannot finish, the route returns `campaign_fork_cleanup_failed` and retains the row for retry rather than deleting its database owner while leaving unowned bytes on disk.
