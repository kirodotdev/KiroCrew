...
Audio upload accepts `.mp3`, `.m4a`, `.wav`, `.ogg`, `.oga`, `.opus`, and `.flac`.
`_sniff_media_type` validates each container family before any bytes reach the
unpublished temp file;
the composer picker mirrors that exact extension set, and `FileRenderers` routes
each accepted extension to the Range-capable `<audio>` viewer. An uploaded audio
file remains an ordinary attachment path in the agent turn; upload does not imply
transcription.
