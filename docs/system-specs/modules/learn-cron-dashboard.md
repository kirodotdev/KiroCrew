...
**Prompt-loop blocker lifecycle**: a missing permission, credential, configuration,
dependency, or failing tool/build/test is remediation work, not a terminal outcome.
The agent inspects the owning code or configuration, makes and verifies the
least-privileged allowed fix, and keeps the loop active. When only the user can
grant the next approval, it reports that once and rechecks on later cycles while
doing any other safe work. `autonudge_stop` is reserved for a completed goal or
Definition of Done, an explicit user stop, a configured STOP sentinel, or a
host/tooling failure that remains unrecoverable after bounded retries with no
safe work left. Finite cycle and runtime bounds may still deactivate the loop
independently as safety backstops.
