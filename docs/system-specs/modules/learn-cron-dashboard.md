...
- **Command mode**: `command` field specifies a shell command to run (mutually exclusive with `script`). Stdout is captured as the result and goes through `security.redact()` at TWO layers, deliberately. The sink layer is `slack/gateway.py`, whose cron command path redacts for delivery and history (`job.set_run_result(redact(output))`) and again inside `_alert_cron_failure`, whose first act is to redact the detail it is handed. The source layer is `cron_script.run_command_sandboxed`, which redacts before it truncates. That ORDER closes a gap no sink can reach: the 64KB truncation happens inside `run_command_sandboxed` and keeps the head of the text, and a credentialed URL cut between the password and its trailing `@` stops matching, so redacting after the slice can ship that password. It redacts a WINDOW rather than the whole capture -- the cap plus `_REDACT_STRADDLE_MARGIN`, which is the streaming redactor's own longest-credential ceiling -- because `proc.communicate` bounds the captured stdout at nothing, so scanning all of it would let a command streaming gigabytes exhaust memory in redaction that survived capture. The residual is a credential longer than that margin straddling the cut, which is what a sink alone would have delivered anyway. `test_redaction_precedes_the_64kb_slice` pins the order and `test_redaction_input_is_bounded_to_a_window` pins the bound. Under the cap the source layer adds nothing today, both sinks being clean already; it is kept as defence in depth so the guarantee is a property of the VALUE rather than a habit of each reader. Redaction is a shape matcher, not a fence: it masks the credential shapes it knows (PEM private-key blocks, cloud keys, tokens, token-bearing URLs) and a key in an unrecognised encoding still reaches the result.
