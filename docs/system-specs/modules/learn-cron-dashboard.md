...
**File-change snapshots** (`chat_runner.py::_snapshot_write_target`): captures before/after content for write-tool invocations, attached as `file_changes` meta on the last assistant message by `_flush_file_changes`. The 'before' content is sourced in priority order: (1) **strReplace only** — full-file reconstruction via `_classify_str_replace_before` (declines outright for `replaceAll` edits, where oldStr uniqueness is not enforced and reversal would over-revert pre-existing `newStr` occurrences): read the file (regular files only — `S_ISREG` at the stat gate, so `/dev/zero`/FIFOs never read — ≤ `_MAX_RECONSTRUCT_BYTES`, re-checked post-read) through `hooks.safe_read_file` (symlink-safe: resolved-target re-check + `O_NOFOLLOW`) and reverse-apply the substitution from the tool params. Both `_snapshot_write_target` call sites in `_run_chat` are offloaded via `asyncio.to_thread` so a slow/hung filesystem cannot stall the event loop. Classification: `newStr` absent → post-write excluded, pre-write proven iff `oldStr` unique; `newStr` present but not unique (overlap-safe `find()==rfind()`) → post-write can neither be excluded nor reversed → decline; `newStr` unique → the single reversal candidate decides (tool-consistent AND pre-write plausible → undecidable seam (`newStr ⊂ oldStr` / `oldStr ⊂ newStr` edits that drop or add a line beside a kept one, whichever side of the write the snapshot lands on), so the disk content rides along on the fragment entry as `pending_str_replace` ({if_post_write, if_pre_write, before_if_post, tool_call_id} — every hypothesis a `_truncate_snapshot` result (content plus truncation flag, so a resolved before carries its own `truncated` state) built when the payload is built, so a deferred snapshot retains at most three `_MAX_SNAPSHOT`-capped snapshots — bounded like an ordinary snapshot, never the raw ≤ `_MAX_RECONSTRUCT_BYTES` read, which is dropped at once) and `_flush_file_changes` settles it through `_resolve_pending_str_replace` against the turn-end after-content: a path written by more than one tool call in the turn (paths compared in the canonical form `validate_file_path` returns, so two spellings of one file are one path) is never resolved and keeps the fragment because a later write can restore the snapshot content and flip the classification (`_bind_pending_str_replace` stamps every snapshot with its tool call's `_tcid_identity_key` — the fixed-size digest every per-turn id table uses, `""` for an absent, over-long, or redaction-collapsed id, so the turn never retains a backend-authored id verbatim — and one write's repeated snapshots — the tool_call and tool_call_update sites both snapshot the same `rawInput` — do not read as a second writer, while an unidentified snapshot counts as one); otherwise, after == `if_post_write` proves post-write (before = `before_if_post`, the reverse substitution), after == `if_pre_write` proves pre-write (before = `if_post_write`), and neither (the file changed again in the turn) or both (the edit lies past the `_MAX_SNAPSHOT` truncation) keeps the fragment; consistent only → reverse; inconsistent + pre-write plausible → pre-write proven). Resolution is gated on the tool's terminal frame — `_settle_pending_str_replace_outcome` marks the pending payload completed on a `completed` result and deletes it on any other terminal status; it also records the outcome in the slot's per-turn `_write_tool_outcomes` map for a snapshot of the same call that binds after the frame (one `tool_call_update` carrying both the first `rawInput` and the terminal status, whose tool_result the dispatcher emits first), where `_bind_pending_str_replace` consumes it; every uniquely identified tool's terminal frame records there, so the map is keyed by the same `_tcid_identity_key` digest (an absent, over-long, or redaction-collapsed id is recorded nowhere) and capped at `_MAX_WRITE_TOOL_OUTCOMES` entries (oldest evicted) and reset by `_flush_file_changes` on every turn exit — because a rejected or failed write leaves the file unchanged, which reads exactly like the post-write state and would otherwise fabricate a before-diff for an edit that never landed; a pending payload with no terminal frame (turn cancelled) is likewise never resolved, and it never reaches message meta. This exists because kiro-cli's diff content block `oldText` for strReplace is only the replaced **fragment** — using it verbatim diffed a fragment-before against the full-file after and counted the entire file as additions (the #920 race fix's full-file assumption holds for create, not strReplace). Reconstruction declines (falls through) on missing params, empty-`newStr` deletions (position unrecoverable), unreadable files, or neither needle matching; only the undecidable seam above defers instead of declining. (2) ACP diff content block `oldText` carried on the tool-call event (`AcpEvent.diff_old_text`, threaded from `acp/_dispatch.py::_build_tool_call_event`) — authoritative and race-free for create (`''` for a new file, full previous content for an overwrite). (3) Disk read fallback — used only when no diff content block exists (e.g. the blocking `session/request_permission` path where the write has NOT yet executed). Both this fallback and the post-write 'after' read go through `_safe_read_snapshot`, which reads via `hooks.safe_read_file_bytes_nolink` (the descriptor gate described under **Prompt and package-skill read safety** below, `within_root` = the canonical path's own parent, byte cap `4 * _MAX_SNAPSHOT + 4` with truncation allowed so the truncation marker still appears exactly when the file exceeds the character cap, newlines normalized to `\n` as a text-mode read does so a CRLF 'after' diffs cleanly against the text-mode 'before') rather than validating the name and re-opening it — a hardlink alias of a protected file under an innocent name shares the target's inode, and only the opened descriptor's `st_nlink` reveals it. Every refusal is the same `None` an unreadable file produces. No-op entries (before == after) are kept and surfaced — the dashboard renders an explicit "no changes" caption for them (a backend drop would compare post-truncation/post-redaction content and silently discard real changes past the snapshot limit or inside redacted spans). Security invariants apply equally to content-block-sourced and reconstructed text: `validate_file_path` refuses sensitive paths before any source is consulted, `_truncate_snapshot` caps content length (after reconstruction, so the needle can't be cut mid-file), and credential/exfil-URL redaction in `_flush_file_changes` covers all sources. `_truncate_snapshot` also reports whether the cap was exceeded. When either side is incomplete, the persisted entry adds `truncated: true` and `snapshot_limit_chars`; complete entries retain the legacy `{path, before, after}` shape, so an absent flag remains equivalent to `false` for old transcripts. The dashboard treats a flagged entry as incomplete evidence: aggregate and row line counts are suppressed, inline and standalone diff actions are unavailable, and both expanded and minimal chip styles show the localized character-limit notice instead of `no changes` or a partial hunk.
