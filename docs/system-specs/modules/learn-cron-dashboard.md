...
**Per-turn timeout** (`constants.py:CHAT_TURN_TIMEOUT`): every helper-visible `_run_chat` dispatch is wrapped with the shared ceiling. The seven guarded paths are: primary user-typed turns (`chat_handlers.py`), queue drain and post-fan-out synthesis (`chat_runner.py`), cron injection (`handlers/messaging.py`), and Slack cron-script delivery plus subagent recovery re-trigger (`slack/gateway.py`). The structured dashboard-monitor path runs `_run_chat` inside an authorization coroutine passed to `spawn_guarded_turn`; authorization is rechecked after the background permit and the helper still owns the same ceiling. The Slack subagent recovery path hoists `_run_chat` into `_run_chat_coro` and runs its lifecycle wrapper through `bounded_chat_turn`; the source guard follows those local aliases, so the wrapper remains one helper-visible dispatch. The cap (14400s, 4 hours) is sized to match the inner ACP `_DEFAULT_PROMPT_TIMEOUT` so the dashboard layer does not bound below the transport; four hours is the longest single turn the shipped budgets can legitimately produce (the task runner's 90-minute test command plus a fix and a re-run, or a blocking subagent wave at its 2h wait cap), and anything longer belongs to the loop mechanisms, which end the turn between cycles. The `_STALE_TURN_TIMEOUT` (90s, in `acp/client.py`) is the real wedged-session guard — it fires when streaming has gone silent. `CHAT_TURN_TIMEOUT` is the upper safety ceiling for genuinely runaway work, not a "this turn took too long" guard.
