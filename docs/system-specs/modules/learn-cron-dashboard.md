...
Scheduled job execution with three schedule types: `every` (interval, min 60s), `at` (one-shot timestamp), `cron` (5-field expression). Supports natural language parsing via `parse_wakeup()`. The dashboard list exposes `at_ts` alongside the recurring machine fields, so the Schedule editor represents an at-job as **Run once** rather than parsing its localized display string. Deletion-after-run remains internal scheduler state; the dashboard has no consumer for it. Retiming an existing persistent one-shot therefore cannot silently turn it into a self-deleting reminder. PATCH accepts the same `at`/`delay`/`at_time` spellings as create; a same-kind retime preserves the existing deletion-on-run value, while converting to a recurring kind clears it and converting a recurring job to one-shot derives deletion-on-run. One-shots need no special `strict_schedule` default because the scheduler already returns zero jitter for every `at` job; the persisted flag remains caller-controlled.

Composer **Send later** uses an AutoNudge one-shot but does not infer user authorship from timing fields. The record carries a positive `scheduled_message` discriminator, while a gateway-owned, scheduled-message-namespaced provenance entry binds its loop id and slot to the exact composer message and requested instant. The namespace is disjoint from member self-arm records, so rewriting an already trusted self-arm row into scheduled timing fields does not confer user authorship. The feature is available only when the process established a Linux Kiro Crew namespace confinement epoch at boot under a readable, non-degraded agent configuration and the dashboard HMAC key is proven persistent across restart. That epoch is monotonic: an unconfined boot cannot be promoted by a live `off` → `auto` edit, and changing any sandbox mode or no-isolation opt-in invalidates it until restart. The direct config watcher invalidates the epoch and removes scheduled rows before a provider factory can adopt the new posture on every host. On Linux, where a later confined restart can honor a planted pair, it additionally holds every cold-start permit, moves registered providers and detached subagent/background runtimes into a retryable quarantine, and withholds the new factory plus final protected-record purge until every old runtime is verified dead. A failed kill or liveness check retains the exact runtime reference for the next watcher retry. Windows and macOS can never honor provenance, so their sandbox edits preserve live work and use the ordinary factory refresh after the cheap invalidation and cleanup. Explicit sandbox-off, a missing namespace backend, Windows, and macOS refuse mint and verification, and every gateway boot purges legacy composer plaintext before the AutoNudge feature flag can return or agent-capable channel services start. A confined boot preserves hidden records when only the dashboard signing key is ephemeral, so a later restart can recover them. The token system's ephemeral availability fallback also refuses new provenance, but on an otherwise confined, unchanged epoch it preserves existing hidden records and reports reads as temporarily unavailable so a later restart can recover the persistent key. A same-user MAC is not treated as authorship without the boot-established epoch, invariant hidden-leaf boundary, and restart-stable key. At fire time the gateway reads message bytes and timing from that protected entry rather than from the agent-writable loop; a forged mutable message can never become a `user` row, and an earlier forged deadline cannot send before the trusted instant. Authenticated edits serialize mutable persistence, protected-provenance CAS, and timer re-arm inside the service's per-loop transaction. A contended CAS rolls the mutable row back before the lock is released; a crash or rollback-write failure leaves the prior protected content authoritative and therefore fails closed. The ungated registry withholds scheduled composer text. Authenticated per-slot reads overlay the exact protected text and instant, so restart scrubbing cannot feed a lossy mirror back into a time-only edit; PATCH omits text unless the user changed it. Authenticated composer text follows ordinary user-turn semantics and is preserved exactly for the local transcript/model input, but scheduled create and edit reject local `/goal` forms that would add, clear, or replace session automation while the scheduled record owns that slot; read-only `/goal` status and malformed usage-only forms remain valid deferred text. A remote execution retires the one-shot only after the peer stream emits its positive `[DONE]` terminator; 2xx acceptance, truncation, relay failure, and cancellation retain one durable user row and rearm the same message. A delivered scheduled turn remains an unattended self-wake and cannot apply human-only project/reset/tag directives. Agent/workflow automation text remains credential/exfiltration-redacted before persistence, while any later cross-surface mirror of the human message still redacts at the established egress boundary. Unschedule re-checks pending state and captures the authoritative text atomically with removal, returning it for composer restoration; it returns `409 scheduled_message_in_flight` once dispatch has begun.
