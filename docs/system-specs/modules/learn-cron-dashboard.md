...
**AutoNudge goal drafts**: owner-only `GET,PUT /api/autonudge/draft/slot/{slot_key}` share the inactive Set a goal form across dashboard clients. The canonical store is `<data home>/goal-drafts/goal_drafts.json`, replaced atomically under a sidecar lock and restricted to the owner because goal text can contain paths or instructions. It retains at most 50 slots for 30 days within a 1 MiB file. A clear is a timestamped tombstone, not a deletion, so an older local copy cannot resurrect the goal. Capacity pruning always retains a live write the server just accepted and evicts the oldest other slot; a migration may still lose to the bounded set, in which case the browser keeps its local fallback.

Live online edits use a server-monotonic canonical timestamp, and each browser serializes its writes so request arrival is the tiebreak instead of a browser wall clock. A new browser edit advances past the last observed canonical/local timestamp even when that browser's wall clock is behind. The first upload of a browser-only or offline copy is explicitly a migration and keeps its local edit timestamp, with future skew clamped to five minutes. That migration path still depends on the browser clock because no server observation exists for an edit made while disconnected; a browser clock far behind the server can therefore lose a genuinely newer local-only draft on first sync. Opening a browser without editing does not create a fresh timestamp. Every locally accepted message is clamped to the same 8,000-Unicode-code-point limit enforced by the Python store, and the editor exposes that limit through a visible, programmatically-associated count plus a visible polite live-region warning while the limit is hit, so a cut paste is not signalled by the counter's colour alone; idle/cycle values are normalized to the server's integer bounds before persistence, so every saved fallback is remotely representable. Reads and writes run through React Query and share a five-second request bound. The per-slot write queue, quota fallback, and retained-canonical provenance are owned by the goal-draft persistence module, so ordering survives a popover remount and storage-event reconciliation uses the same state as ordinary edits. Every newer cross-tab draft or tombstone accepted into the live editor is either byte-for-byte the server canonical already returned by the open read, or is queued exactly once behind that slot's current remote-write tail; duplicate body/legacy-sidecar events share the first request. A failed cross-tab write keeps the accepted editor state and visible Retry path, while a later local edit supersedes the pending answer and cannot be downgraded by its canonical response. A failed open or live write likewise keeps the local draft editable and exposes an in-place Retry. If local persistence also fails, a module-owned in-memory snapshot remains authoritative across Retry/remount until the server accepts it, so stale localStorage cannot overwrite the editor's only copy. The browser keeps the canonical response only when no newer local edit replaced the submitted snapshot. Clearing a stopped loop follows the same ownership rule across both request boundaries: loop deletion and local/remote tombstone completion reset and close only while every editable field still matches the clear submission, while a newer edit stays visible, enters the ordinary durable per-slot queue behind the tombstone, and owns subsequent failure or Retry state. A failed sync whose newest retained snapshot is a timestamped tombstone reports an unfinished clear with the same Retry, never copy claiming a draft is still held, and Retry keeps that classification because it reconciles the same tombstone; a null draft with no timestamp is no record and keeps the draft copy.

Local durability uses versioned immutable records under the `mc-goal-drafts-v2:` namespace. One `setItem` writes one draft-or-tombstone together with its timestamp and provenance; readers snapshot record names and read each primary key once. A writer never replaces a shared envelope. Compaction removes only exact older keys observed by its scan, so a delayed save, clear, prune, or malformed-record repair cannot erase a concurrent newer record. Canonical responses carry the submitted-record provenance and may supersede that exact local version even when the server normalized its timestamp downward; a concurrent newer record still wins. Equal clocks within the durable journal are ordered deterministically by the unique record key using locale-independent code-unit order. Distinct local and canonical values at an equal timestamp are not treated as the same edit: the local draft or tombstone is reissued above the canonical stamp, and another equal-stamp collision is retried boundedly inside the same queued migration. Retry exhaustion retains the local value and exposes the normal visible Retry path. The former `mc-goal-drafts` raw body + timestamp sidecar is the only read-only migration input, because it is the only shape any released client ever wrote: a pre-upgrade tab may keep writing it but cannot replace v2 state, conversion happens only on an explicit save without rewriting the legacy keys, and a wrapped or otherwise unrecognised body under that key is read as an unknown slot map rather than as a draft. Legacy writers commit the timestamp sidecar first, so the storage listener ignores sidecar events and reconciles only on the following body event that proves both values are visible. A separate immutable retirement journal stores a monotonic per-slot watermark plus exact equal/future-skew identities; it does not expire with executable v2 records, so a migrated, expired, superseded, or capacity-evicted legacy draft/tombstone cannot become current again. Timestamp-less legacy data has ordering stamp zero and receives a fresh migration stamp only after a server read proves the canonical slot empty. Equal-stamp distinct content remains eligible for the normal reissue rule. If retirement-marker persistence fails, the expired v2 record remains as a non-executable carrier and suppresses the legacy input until compaction can durably transfer that state. The shared draft-store factory therefore keeps one raw-body-plus-sidecar format for its other consumers, and exposes only the load/set/save surface those consumers call: a persistence outcome is observed through storage, not returned from save or read back per slot. A TTL load reads sidecar/body as a pair and retries from the newer sidecar when it changes across the body read; load-time expiry filtering never rewrites the shared body, so it cannot erase a concurrent tab's commit. Missing pre-TTL timestamps are merged into the latest sidecar without rewriting body content. Explicit saves retain sidecar-first ordering and rollback guarantees: either write failure restores the prior in-memory timestamp map, and a body-write failure also restores the persisted sidecar. The sandbox precreates the hidden canonical directory before each namespace launch, making the mask effective even before first editor use.
