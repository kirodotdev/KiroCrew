...
the file tools (`hooks.safe_read_file_bytes_nolink`). The captured campaign root
is revalidated against the currently named research root before and after every
campaign pin, so an ancestor replacement or root-link swap refuses instead of
mixing a detached campaign with a newly targeted root. A run/cycle finding
snapshot keeps that one campaign identity live across candidate discovery and
any eager bounded leaf reads, then carries either those captured bytes or the
admitted root/campaign identity tuple into dashboard, stagnation, bookkeeping,
SSE, and completion consumers. A deferred read reopens a leaf only after proving
the named root and campaign still have that exact tuple; none reopens by campaign
id alone. A root/campaign replacement during eager capture makes the whole snapshot
UNKNOWN and cannot update watchdog counters or decide a terminal verdict. A
reachable campaign with no findings is a proven empty `[]` only after a second
root/campaign identity proof following snapshot construction; every later empty
result consumption repeats that proof. An identity or read gap remains UNKNOWN.
Cycle discovery accepts only a single-linked regular
`cycle*.json` leaf beneath the exact absolute
...
evidence so it remains useful without becoming an unbounded response. The report
endpoint returns `200 {"report":""}` only when `FINDINGS.md` is genuinely
missing (or is a genuine empty file); an unavailable campaign identity or a
present leaf that fails metadata/read ownership returns `findings_refused`/409.

Recursive exploration keeps one campaign identity across emergent-question read,
consume, queue load/save, database activation, and brief publication. Queue reads
and atomic writes are descriptor-relative on POSIX; Windows uses the retained
no-share-delete owner. A research-root or campaign-name replacement therefore
cannot redirect `emergent_questions.json`, `subquestion_queue.json`, or `brief.md`
...
Campaign deletion uses that retained identity as mutation authority through the
final empty-directory removal. POSIX keeps the pinned research-root descriptor
and the revalidated campaign descriptor open through descriptor-relative
`rmdir`. Windows captures the campaign with a removal-only handle that requests
DELETE access without permitting `FILE_SHARE_DELETE`; concurrent identities
duplicate one reference-counted owner handle rather than issuing incompatible
independent opens, and the last identity closes the owner. Child cleanup and
final revalidation happen while that handle blocks campaign and ancestor
replacement, and `SetFileInformationByHandle(FileDispositionInfo)` marks that
exact directory object for deletion before any duplicate closes. Ordinary
directory pins do not receive DELETE access. Registry keys include the resolved
research root as well as its device/inode identity, so a removed temporary or pod
root cannot lend an inode-reuse entry to another root; changing roots retires only
inactive entries and preserves any root with a live Windows owner reference. Explicit
`close()` releases the shared owner synchronously. Object finalization detaches its
descriptors without entering the registry lock and delegates the refcount release and
closes to a worker, because finalization can run while the same thread already holds the
non-reentrant acquisition lock. The exact campaign row is selected and its
DELETE committed before any destructive filesystem call; a failed database
mutation therefore leaves every file untouched. Links/reparse points, hardlinks,
unreadable identities, ancestor/root/campaign swaps, or partial child cleanup
still refuse. A refusal after the commit restores the exact owner row, so the same
campaign deletion remains retryable. Once the tree is removed, the registry entry
for that inode is retired whether or not a database row originally remained, so a
later directory reusing the id is not refused by a pin that outlived the directory
it protected.
...
A fork creates a child before its descriptor-bound seed copy, so the route treats that interval as a compensating transaction. It keeps the parent identity pinned, closes the child identity before cleanup, and invokes the normal database-first, row-compensated `delete_campaign` path on every child-identity or parent-copy failure. An oversized parent report returns the explicit `parent_findings_too_large` 413 only after both the child row and owned directory are gone; other copy refusals return `campaign_fork_failed`. If identity-safe cleanup itself cannot finish, the route returns `campaign_fork_cleanup_failed` after the exact child row is restored for retry, rather than leaving unowned bytes without their database owner. The route preallocates the child ID before creation and runs create, identity resolution, seed copy, child-pin release, and compensation as one owned task. A transaction-local allocation claim is marked only after that task's exact child-row INSERT commits; every compensation path requires that claim. An ID collision therefore closes its failed database transaction and propagates without deleting, auditing, or otherwise touching the pre-existing row or directory. Once the claim is marked, later sidecar, identity, seed, and cancellation failures retain the existing safe-compensation contract. Cancellation settles whichever off-loop operation is in flight, closes the child pin, and completes the normal safe-delete attempt before `CancelledError` propagates. An incomplete cancellation cleanup remains retryable and is reported in the operator log and SEL audit even though cancellation has no HTTP response.
...
of readable persisted cycle findings reaches the cap that stopped the loop. Only
current-generation findings count: Start/Resume records the pre-run multiset of
finding-content digests in the transaction that mints `started_at`, and a capture
that cannot pin the campaign directory records UNKNOWN rather than an empty set,
so an unknown boundary refuses cap completion until a later Start/Resume records a
proven one. A
...
never erase a record. On the dashboard, the legacy goal popover writes NO
per-slot goal draft from a clear, confirmed or refused: the confirmation the user
answered was "Clear goal for good", so mirroring the erased record's fields into
the draft store would make the freed slot reopen on the goal that was just
removed. The draft store holds only what the user typed on an empty slot (the
edit-debounce and close-flush writers, which skip while any record is present),
so a confirmed clear leaves that store byte-identical and the reopen seeds from
whatever it already held -- a pre-loop draft or the template. A refused clear
likewise writes nothing and leaves any prior draft byte-identical; a plain stop
never writes a draft, because its record survives and stays authoritative on
reopen.
Clearing is the only way the row is removed, so it is what
