...
The initial optimistic broadcast still rolls back and rethrows on failure, now
with a credential-free `failed/broadcast` SEL event.

The transaction has one publication policy (`_publish`) and one depth-release
`finally`, rather than a separate exception guard at each broadcast. The initial
optimistic push is still reversible: failure rolls back before any disk write and
rethrows. Reconcile, late-alias, rollback, accept-committed and final publication
