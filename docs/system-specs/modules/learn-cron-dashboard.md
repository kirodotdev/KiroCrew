...
still replace an earlier manual app-disable pause. The same rule holds for a
loop one of the timer's terminal bounds already stopped (`cycle_cap`,
`runtime_budget`, `approval_stalled`, `structural_terminal`,
`session_start_failures`): a reasonless `active=False` arriving afterwards --
the goal popover's Pause, pressed off a record that still read running --
keeps the bound instead of stamping `manual` over it, since a manual reading
would offer a resume the still-spent bound turns away before the nudge and
would lose why the loop ended. A pause of a RUNNING loop still records
`manual`, and `active=True` still clears any bound.
