...
**Staged quote annotations** — Selecting assistant text and choosing Quote stages a
structured quote pill above the composer instead of editing the draft text. Quotes
are owned and persisted per chat slot, carry durable message identity for jump-back,
serialize as attributed blockquotes on normal sends and mid-turn steering, and are
restored if the server refuses a send. A quote-only composer is sendable.
