...
**Office previews** (`dashboard/handlers/files.py`, `dashboard/handlers/office_slides.py`; rendered by `components/FileRenderers.tsx`): GET `/api/file-office-preview?path=...` returns `{text, truncated}` for a `.docx`/`.pptx` — plaintext extracted by `doc_parser` — and, for a `.pptx`, `slides: [{index, text}]` beside it (`index` is the deck's own slide number; `text` is `join_slides(slides)`, both from ONE extraction; each slide redacted, the slides sharing the flat field's 512 KB cap). 415 `unsupported_preview_format` for other office extensions.
GET `/api/file-office-slides?path=...` is the rendered-slides manifest for a `.pptx`/`.ppt`: `{status: "ready", digest, count, truncated, width, slides: [{n, width, height}], sha256: {"slide-<n>.png": hex}, sig}` once the deck is rendered, or `{status: "unavailable", reason: "soffice_unavailable", hint}` (200 — a normal state, with the pptx-maker app's per-OS install command) when the gateway host has no LibreOffice; 503 `cache_unsigned` when the SEL trust root cannot be loaded, 502 `convert_failed` when `soffice` fails or times out, 503 `sandbox_unavailable` when the strict sandbox refuses. GET `/api/file-office-slide?path=...&n=<k>[&digest=<sha256>]` serves one PNG (every served slide is a SEL `success` invocation, every refusal a `denied`/`not_found` one; `image/png`, `nosniff`; `Cache-Control: private, max-age=3600` only when the digest is supplied, `no-store` otherwise); a supplied digest that no longer matches the file answers 409 `stale_digest`, an unrendered or unverifiable slide 404 `not_rendered`. Both routes run the file endpoints' shared open-and-check prefix (`_open_checked_file`: path validation, sensitive-path fence, symlink-refusing open, 50 MB `fstat` cap) and audit to SEL under `file_office_slides`, with the same refusal codes as the text preview.
*Rendering.* The deck's bytes are copied from the checked handle into a private work directory under the cache (the child never opens the user's path; re-hashed against the manifest digest before staging, 409 `file_changed` if it moved), converted by `soffice --headless --norestore --convert-to pdf --outdir …` as an argv list through `sandboxed_spawn_argv(…, "strict")` with the trust root in `extra_hidden_dirs`, a throwaway `-env:UserInstallation` profile, a minimal environment (`TMPDIR` inside the work dir, no `DISPLAY`), capped pipes and a 180 s wall-clock timeout that kills the process tree; the PDF's visible text is screened first — the context-aware credential redactor, then the exfiltration-URL redactor, the same two the download path applies — and a deck either would change is not rasterized at all: the manifest answers `{status: "unavailable", reason: "content_redacted"}` (SEL `denied`/`content_redacted`), nothing is published, and the panel shows the redacted text outline under a row that says why (a slide PNG is a picture of the text and cannot be redacted after the fact); otherwise the PDF is rasterized in-process by `pypdfium2` (a core dependency through `pdfplumber`) at 1280 px wide, capped at 500 slides (`truncated`). The cache root is reached without following a link at either name below the data home (`_open_cache_root`, descriptor-relative `mkdir`/`open` with `O_NOFOLLOW`), the deck is hashed WHILE it is staged so the staged copy is the hashed bytes, and the work directory is pinned by descriptors from the moment it is created (`_Staging`): every write into it and read out of it is descriptor-relative (`O_NOFOLLOW` on every open, `O_EXCL` on every create, descriptor-relative unlink/rename), the PDF is opened through it as a regular file, the per-slide hashes come from the bytes written, and the publish rename is verified by inode afterwards — a name swapped for a link mid-render is refused (`convert_failed`), never followed, written through or deleted through. Only the child receives paths. There is no by-name fallback: a platform without descriptor-relative operations (Windows) does not render — the manifest answers `{status: "unavailable", reason: "platform_unsupported"}` (SEL `denied`) and the panel shows the text outline under a row saying so. A conversion failure's diagnostic (the child's stderr tail) is passed through the credential and exfiltration-URL redactors before it is logged or returned as `detail`. Discovery of `soffice` is the pptx-maker app's (`engine.optional_dep_path`, `preview_tools.soffice_hint`), imported lazily.
*Cache.* `<data home>/cache/slide-previews/<sha256 of the deck bytes>/` holds `manifest.json` + `slide-<n>.png`, published by atomic rename, deduped by a refcounted per-digest lock, evicted least-recently-used to a 256 MB budget (the deck just rendered is spared; abandoned work dirs older than twice the timeout are swept). Every request, slide fetches included, hashes the file (no stat-keyed shortcut: a `(size, mtime)` key can collide for a same-size rewrite that keeps the timestamp and would serve the previous deck). **The cache is not a trust root**: it is same-uid agent-writable, so the manifest is HMAC-signed with a subkey derived from the dashboard's `token_signing.key` — gateway-only, masked inside every agent sandbox (the SEL key would not do: in-sandbox MCP servers read it) — under the label `kirocrew.slide_previews.manifest.v1` (the `session_pid_sig` derivation, never the raw root), records every slide's sha256, and every cache read opens `O_NOFOLLOW`, refuses non-regular or oversize files, and serves a slide only when its bytes hash to the signed record. An unsigned, mis-signed, foreign-digest or unverifiable entry is a cache miss; a planted symlink at the deck directory is unlinked (never followed) before publishing. Without a loadable trust root the cache is inert and the manifest route answers 503.
*Frontend contract.* `OfficeViewer` dispatches `.pptx`/`.ppt` to `OfficeSlidesRenderer`: while the manifest loads, the text outline renders under a "Rendering slides…" row (a cold conversion takes up to ~30 s; the outline is instant); `ready` renders `SlidePager` (current slide, prev/next, live `Slide n of N`, keyboard paging, lazy thumbstrip, the card's Open/Download actions), with slide URLs pinned to the manifest digest — a slide whose image request fails (409 after an edit, 404 after eviction) is replaced by an `ErrorNotice` (`askAgent`) with a **Render slides again** action that re-asks for the manifest; `unavailable` renders the outline under a row naming the install command with an **Ask the agent to install it** hand-off (stages a prompt naming the gateway's command into the chat composer via `sendErrorToChat`; the user reads and sends it, nothing runs on the click); for a `.ppt`, which has no text to extract, the same row sits over the download card. The pager footer wraps rather than clips, so a 320 px panel keeps every control visible; any failure renders the outline under an `ErrorNotice` (`askAgent`) carrying the server's reason and code. `.docx` never touches the slides routes.
