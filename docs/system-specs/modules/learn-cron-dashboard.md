...
- Timer loop: non-blocking execution via independent `asyncio.create_task()` per job (via `_run_job_isolated()`), with strong refs in `_running_tasks` to prevent GC. One hung ACP session no longer freezes the entire cron system. Ordinary runs retain both `_executing` admission and their `_running_tasks` entry until the off-loop result merge settles; cancellation drains that non-cancellable worker before token-safe release, so a delayed store lock cannot admit the same still-durable occurrence through either the timer or manual trigger. A completed payload with an exact owed claim treats `CronStoreBusy`, unreadable authority, and save `OSError` as unsettled ownership rather than a logged best-effort failure: the original run task retries off-loop with capped async backoff, preserving token/admission and never rerunning the payload. Save exceptions reload fresh state so commit-then-raise, replacement completion/deletion, or newer sibling debt settles the old claim; exact old debt continues retrying. Cancel/reap may take over through their terminal fence, while shutdown waits for the same run-owned settlement. Never-started/preserved claims and ordinary non-owed merges retain their existing behavior. A cancel/reap arriving during that drain keeps its terminal token fence and remains the sole terminal-history owner. Shutdown awaits the timer and all run tasks, then boundedly retries every retained cancel/reap terminal owner before stabilizing queued owed debt. Any terminal owner or process-only publication that remains unsettled after its bounded attempts makes `stop()` raise instead of reporting a clean cron shutdown; the gateway's existing graceful-shutdown budget still bounds the whole operation. The queued-debt drain repeats the ordinary save/compare path until the pending map is empty, with three attempts covering two successive arrivals during saves plus a final pass. Local producers are joined by then. After the final publication attempt, one locked read-only reconciliation accepts a commit-then-raise, replacement completion/deletion, or newer durable debt without writing again. Per-job timeout increased from 5 min to 30 min (`_JOB_TIMEOUT_SECS`); zombie detection (`is_responsive()` after 10 min inactivity) provides additional defense-in-depth
- **Missed cron-expression occurrence identity**: `owed_fire` remains the backward-compatible persisted boolean, while `owed_fire_id` names the single occurrence it represents by UTC minute. A legacy `owed_fire=true` record with no identity loads as the stable `legacy` sentinel and is rewritten safely on its next save. A run claims the identity it starts with; ordinary completion, explicit user cancellation, and reaping clear the store only when that exact identity is still present, while service-shutdown cancellation preserves its claimed identity so a replacement can retry the unfinished occurrence and a later boundary published by a sibling gateway survives. An inherited scheduled occurrence also retains the immutable `(default timezone, semantic generation)` authority used at capture until its exact run token retires. Replacement-completion arbitration re-evaluates that occurrence under the captured timezone, not the live default, so A → B and A → B → A changes cannot make a drained run miss a valid replacement completion and overwrite its runtime fields; explicit-zone jobs retain their own timezone authority. Before timer dispatch, canonical debt on a job with no explicit timezone is revalidated under the cron store lock against one captured `(timezone, generation)` publication authority; invalid debt is durably cleared only after that authority is re-read and the occurrence re-evaluated. A repeated timezone under a newer generation is treated as ABA churn with no destructive authority. Any authority change during the save restores the exact debt and returns no dispatch authority without overwriting replacement completion or newer debt; save ambiguity is resolved by authoritative reload, and the captured authority is compared again after awaited admission and around token mint so a concurrent publication cannot launch obsolete debt. Explicit-timezone jobs are unaffected and unknown identities remain retained. A shutdown claim that existed only in process is queued before the result worker's first fallible store-lock acquisition, then handled after fresh replacement-completion arbitration: if still owed by the current calendar, only the occurrence identity is merged and saved (newer durable debt wins), then the drained path returns before copying any runtime/result field; save ambiguity queues the claim for the existing post-join shutdown drain. Each admitted in-process run also owns an opaque token, because a manual/no-debt claim is legitimately `None`; isolated, cancel, and reap cleanup mutates occurrence and loop-owned maps only while that token still owns the job id, so a late old path cannot erase a successor's claim. Cancel/reap hold a token-valued admission fence through process cleanup, terminal persistence or rollback, history, refresh, SEL audit, and exactly-once ownership retirement; caller cancellation is re-raised only after that complete operation settles. If cancel's terminal worker fails before it cancels and awaits the run task, the terminal fence and service cancellation marker are withdrawn but the unchanged run token, `_running_tasks` entry, `_executing` claim, run provenance, both watchdog starts, and jitter stay with the still-live isolated task, so its token-safe `finally` remains the cleanup owner; successful terminal cleanup consumes those maps only after the task boundary, and a run that already completed is retired without resurrecting metadata. A SIGTERM already delivered by `kill_running_process` remains a successful cancellation when its escalation thread cannot be constructed or started, but the same broadcast-guarded whole-tree SIGKILL is attempted immediately before returning so a SIGTERM-ignoring executor child cannot survive. Pruned-install launch skips choose the newer of the run's captured occurrence and a boundary matching the launch-time job snapshot. Their queued concrete UTC minute is revalidated after the locked store sync against the fresh enabled state, schedule, timezone, skip dates, and last-run stamp; an obsolete intent retires without changing already-persisted sibling debt, while unknown identities are retained rather than guessed at. A captured `legacy` marker is superseded by a valid current concrete boundary and is preserved only when no current boundary matches. The occurrence is queued before an off-loop locked save and the drained process quiesces only after that save is confirmed. Once the persistence worker is submitted, caller cancellation is deferred until its actual result: success completes relinquishment, the covered local marker, and quiescence before `CancelledError` is re-raised. A reported save failure is resolved under a fresh locked reload: already-durable/newer debt, an obsolete intent, or a replacement completion at or after the occurrence proves the handoff covered; only a definite pre-commit/refused/unreadable result keeps the fallback marker and queued retry without relinquishing or quiescing. Once publication is confirmed durable, the drained run relinquishes both ordinary result and terminal cancel/reap runtime-state merge ownership under its in-process token, so a replacement that consumes the occurrence cannot have its status overwritten or the occurrence republished, either by the old run's direct terminal merge or by a later save of its drained hot cache; an `every`, `at`, or cron skip with no concrete occurrence makes the same transfer before its process-local disable/quiesce, because it needs no occurrence write and the replacement already owns the unchanged schedule. Cancel/reap install their terminal fence before cancellation, cancel and await the isolated run while it drains any submitted handoff worker, and only then consult that run-token fence for hot reflection or locked terminal persistence; they never overtake the handoff decision or release terminal ownership early. A terminal owner clearing a claimed owed occurrence first establishes fresh store authority: lock acquisition and sync/read must complete and `_load_failed` must be false before a missing target can prove deletion or replacement. Lock contention/OSError, sync/read failure, or fail-soft unreadable load becomes the same typed unsettled result before target lookup. It arbitrates any later save failure by resetting the fingerprint and reloading under the same store lock: commit-then-raise, replacement completion, deletion, or a newer sibling occurrence settles the old claim without resurrection, while the exact claimed identity still durable (or an unreadable store) does not. Two short async backoffs give three off-loop attempts. Exhaustion raises before terminal history/audit, retains the token-valued admission fence plus trigger/watchdog metadata, and records whether cancel or reap owns the retry; a later explicit cancel or reaper sweep reuses that same token rather than creating an unowned background task. Caller cancellation remains deferred through the bounded operation and preserves the retry fence if settlement still fails. Non-owed terminal persistence keeps its prior best-effort behavior. Terminal process/task teardown, history, refresh, audit, fencing, and cleanup still settle in full. The command, script, and agent ENOENT paths offload the complete running-install-plus-missing-path ownership classification as one filesystem operation; pathless, user/cwd/provider, and genuine sandbox failures remain ordinary failures. No-service, pre-commit, refused, and exceptional publication retain the local fallback merge, queued retry, and ordinary terminal-state behavior. Before that nondurable fallback copies any runtime state, it yields when the locked fresh target proves the replacement completed the drained run's exact scheduled occurrence. Only covered old process-local identities retire immediately; a newer still-current pending occurrence is persisted against the freshly synced replacement object before return, mutating only owed identity and retiring the queue entry only after the shared save. Obsolete, deleted, paused, rescheduled, skipped, or already-completed pending identities retire without revival; save failures retain the exact retry and reset the fingerprint so a commit-then-raise cannot overwrite a later replacement consumption. Manual runs do not invent debt off-boundary, but a matching, enabled, non-skipped cron minute reached while `_executing` belongs to a manual run is published once under the same identity. A manual payload masking such a minute starts only after the off-loop save succeeds or a fresh locked reload proves the occurrence was already durable, completed, superseded, or made obsolete; an unconfirmed publication retains the exact queued identity and ends as never-started without spending a failure strike. The manual lifecycle also captures at completion before stamping `last_run_ts` or retiring its run metadata, closing a timer snapshot-to-scan gap without republishing an occurrence already captured at admission or by an earlier scan. Repeated scans of that minute are idempotent, and one persisted field bounds catch-up to one make-up run.
...
- Update: `CronService.update_job(job_id, **kwargs)` for partial updates (name, message, schedule, agent, channel, thread_ts, approval_mode, silent) with file locking and cron expression validation. After accepted `cron_expr`, `every_secs`, `timezone`, or `skip_dates` assignments and before the transaction's single save, the pre/post calendar tuples are compared so a dashboard payload repeating unchanged values is not a calendar change. Only an actual change revalidates durable canonical UTC-minute debt, through a calendar-only predicate over schedule, timezone, and skip dates: still-matching debt stays regardless of pause/enabled or a later manual `last_run_ts`, calendar-invalid debt clears, and `legacy`/future-format/unknown identities remain fail-closed. The broader queued-publication predicate deliberately keeps its enabled and last-run checks. Unrelated edits never revalidate debt, rejected updates mutate nothing, and user pause/resume retains debt because pausing changes admission rather than the calendar occurrence owed. A schedule/calendar save exception resets the fingerprint and reloads under the same store lock before propagating: a pre-commit failure restores the old persisted calendar/debt, while commit-then-raise retains the committed new calendar/debt-clear; if reload itself fails, the reset forces the next sync to retry rather than trusting failed hot mutations. `channel` and `thread_ts` are the delivery pair and both clear on a falsy value (`""`/`None` → `None`, matching how `mcp_cron` normalizes a blank `thread_ts`); a field the caps table validates but this method does not assign would be accepted and silently dropped, so every updatable string field needs its own branch here. Re-threading a job that holds an **active `secret_env` grant** deliberately fails its next run **closed**: `thread_ts` is bound into `cron_script.delivery_fingerprint`, which the runner recomputes and the grant pin must match, so the operator has to re-approve before secrets are injected again
...
- **Managed-runtime launch handoff**: script launch preflights the exact `sys.executable` before sandbox construction. On macOS, `sandbox-exec` can itself start after that managed interpreter is pruned and report its inner `execvp` ENOENT only as a nonzero result. On Linux, the outer `systemd-run --user --scope` cgroup wrapper can likewise survive while the namespace launcher's managed interpreter disappears. An exact trusted Seatbelt wrapper whose inner executable is the managed interpreter, or a verified cgroup envelope, receives a parent-created pipe while retaining its original inner argv unchanged. The generated Linux namespace launcher and the direct controlled cron-script launcher remove the fd key, write one byte only after their managed Python process is running, and close the descriptor before execing or loading user payload; no untrusted, malformed, uncgrouped, delegated, or ordinary agent launch receives this channel. Both Seatbelt and cgroup ENOENT reconstruction require authenticated evidence that the original controlled child never started. The runner otherwise reconstructs a typed, pathful `FileNotFoundError` only when the captured argv is that exact trusted Seatbelt profile or Kiro Crew cgroup envelope, its inner executable is the interpreter, stderr is one of that wrapper's exact path-bearing ENOENT spellings, and the interpreter is absent after exit. The gateway remains the final authority: it retains the due occurrence without a failure strike only when both its running interpreter and package module were pruned and the missing filename belongs to that install. A live interpreter, started controlled child, malformed descriptor evidence, unrelated path, untrusted or malformed wrapper, lookalike diagnostic, sandbox refusal, user command/script failure, or uncgrouped failure remains an ordinary failed run.
