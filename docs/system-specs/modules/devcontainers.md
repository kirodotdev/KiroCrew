...
built by the reference `@devcontainers/cli`. This is VS Code parity for the
local tree, **not a sandbox**: image/build, in-container lifecycle hooks, mounts
and `runArgs` are honored after a per-config human trust grant. The grant is the
ceiling for what those bytes declare.

The gateway refuses only what would make that grant uninformative, plus a named
floor that is not a growing docker-alias denylist. The classes are listed under
[Closed screen](#closed-screen). A new spelling of an already-covered class is
canonicalized into that class. A `runArgs` flag that is not in one of those
...
[docs/design/devcontainer-product-readiness.md](../../architecture/design-notes/devcontainer-product-readiness.md).
...
gateway (host)                          container (project toolchain)
  AcpRuntime.spawn / AcpClient._spawn
    ...
    mcpServers → python3 client.py <unix-socket | tcp host.docker.internal port> <secret>
...
1. **Fail to host, never fail the spawn.** `resolve_for_work_dir()` returns
   ...
   eligible when Docker is present. On macOS the official Docker Desktop CLI
   is user-owned; it is accepted only when Developer ID `Docker Inc`
   (`9BNSXJN65R`) still verifies. Spawn must not block on a human decision;
...
10. **The screen is closed by class, not by spelling.** See
    [Closed screen](#closed-screen). A review that finds another docker alias
    of an already-covered class is a canonicalize-into-that-class fix, not a
    licence to add a class.

## Closed screen

Five classes. They are the whole screen. A `runArgs` / Compose / `mounts`
spelling that does not fall into one of them is the human's grant.

| Class | Why it is refused | What belongs |
|---|---|---|
| Digest integrity | The effect is not in the hashed local tree, so the prompt would approve text that is not what runs. | `features` (registry merge); `initializeCommand` (the one spec hook that runs on the host); build inputs, `extends.file`, and compose `include` resolving outside `.devcontainer/`; symlinks; extra hard links; unparseable config; the tree-size ceiling. |
| Isolation voided | The container can reach host storage the approved config never named, so every other check is theater. | `privileged`; mount-capable caps (`SYS_ADMIN`, `ALL`); host namespaces (`--pid=host`, `--network=host` / `--net=host`, and the rest of `_HOST_NAMESPACE_FLAGS` plus their Compose keys). |
| Named host-control escapes | Binding the runtime API or a kernel control tree lets the agent request a fresh container mounting anything. | `docker.sock` / `podman.sock` / `containerd.sock` / `crio.sock` wherever they live; `/proc`, `/sys`, `/dev`, `/run/user`. |
| Sensitive-path floor | The same `is_sensitive_path` predicate the rest of Kiro Crew uses. A named list, not a new class per docker flag. | `~/.aws`, `~/.ssh`, the data-home keystone files, and the other paths that predicate already covers, in every bind / `env_file` / compose `file:` shape that can express a host path. |
| Gateway hygiene | Values the CLI inherits from the gateway process are not in the hashed tree. | `PATH` rebuilt from directories this process cannot write; live forwarded credential-env (`DOCKER_CONFIG`, `DOCKER_CERT_PATH`, CA-bundle names) screened as a bind source. |

`--net` is docker's alias for `--network`. It is canonicalized *before* the
isolation-voided screen, which is the shape every later alias of an existing
class should take. Do not add a sixth class for a newly spotted spelling.
...
the same `sha256(canonical key)[:24]` used for the container's id-label (both
derive from `_project_token_from_canonical()`; `_project_token()` is the
blocking wrapper that realpaths first), and it is what makes a build directory
attributable to a project: with a digest-only path, "this project's superseded
configs" is not an enumerable set, so stale directories could only be reaped by
guessing at unrelated ones. Every trusted config edit produces a new digest, so
without reaping they accumulate for the life of the install.
...
| Concern | Mechanism |
|---|---|
| Identity | `--id-label kirocrew.devcontainer=<_project_token_from_canonical(key)>` (`sha256[:24]` of the already-realpath'd key). Path-charset-safe and short; the same token names the project's build-artifact root. Callers canonicalize off-loop; `_id_label` itself does no I/O. |
...
| Grant revalidation | After the per-project lock is acquired, and again before a newly built container is returned. A revoke during the wait or the build must not hand the container out — digest equality is not enough, because a revoke leaves the bytes alone. |
| Build environment | Allowlisted names only, then `scrub_env`. `PATH` is rebuilt from directories this process cannot write. Path-valued credential env (`DOCKER_CERT_PATH`, `DOCKER_CONFIG`, CA-bundle names) is screened as a bind source by live value, both directions. |
...
run until the user answers. The copy states that this is **not a sandbox**:
trusting runs the setup commands and honors the host mounts and `runArgs` the
file declares. It shows the config path and the first 12 digest characters, and
the raw config is rendered as **text children only** (never
...
- **Native Linux:** `asyncio.start_unix_server` and
  `python3 /tmp/kirocrew-mcp-bridge/client.py <socket> <secret>`. After the screen,
...
- **Docker Desktop (macOS / Windows):** `asyncio.start_server` on
  ...
  `python3 client.py tcp host.docker.internal <port> <secret>`.
...
because `remoteUser` is routinely a different uid than the gateway. Connecting
to the socket or the TCP port is not enough: the host MCP child is spawned
only after the client offers a per-runtime secret (last argv token, first
line on the wire). That secret is what stops a sibling container that can
reach `host.docker.internal` from invoking MCP under this session. An
untrusted local unix co-tenant who can already read the trusted container's
argv is outside the threat model the rest of Kiro Crew defends. The host
...
| `devcontainer_mcp_client.py` | Stdlib-only stdio/socket pipe copied into the bind (unix path or `tcp host port`, plus the handshake secret). |
