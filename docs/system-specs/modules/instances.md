...
used for the peer request never leaves the manager** (§6's invariant holds —
`connect` and `refresh-token` remain the only routes whose response carries the
browser link). The link's positive query probe performs the peer's required
token→session exchange and captures the distinct port-scoped cookie from the
fixed loopback response. Search, transfer, capabilities and proxy calls send
that session token as their cookie, never the browser link that the exchange
revokes on the cookie path. A `401/403` gets exactly one transparent
re-mint-and-exchange retry, because a retained session can go stale while the
tunnel stays `CONNECTED`. A peer predating the exchange returns no distinct
cookie and keeps the legacy link-token fallback.
...
and `_peer_cookie_header` (the exchanged session re-read per attempt, sent as a
cookie, never logged). A fourth caller inherits the invariant instead of copying
it. The shared unauthorized retry goes through `_refresh_peer_credential`, so a
caller cannot accidentally re-mint only a link and immediately replay it as a
cookie.
