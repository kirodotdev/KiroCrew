...
| `GET /api/instances/{id}/capabilities` | Read the connected peer's version, agent roster, model list, effort levels and workspaces through the narrow read-only carrier. Each failed field appears in `unavailable` while successful fields remain usable. |
...
**Capability recovery.** Cheap peer capability reads keep the bounded
`DEFAULT_CAPABILITY_PROXY_TIMEOUT_SECS` budget. `/api/models` gets the separate
`DEFAULT_MODEL_CAPABILITY_PROXY_TIMEOUT_SECS` budget because the peer's own
route can spend five seconds detecting its sandbox backend and then ten seconds
in `kiro-cli chat --list-models`; cutting that request off at the cheap-read
budget guarantees a valid cold discovery is reported unavailable. The dashboard
shows the model list as loading while this field is unavailable and polls the
capability document every eight seconds only when the peer version is known to
match. A disconnected or version-skewed peer does not poll, so recovery cannot
turn a dead tunnel into standing traffic. The interval stops after the first
complete response.
