...
- [17. Crew work migration (move a live unit of work to another crew)](#17-crew-work-migration-move-a-live-unit-of-work-to-another-crew)
...
## 17. Crew work migration (move a live unit of work to another crew)
...
> **Design vs shipped.** §17.1–§17.8 describe the intended design. Only the
> PLAN half of it is in the tree today: given a unit, describe what moving it
> would involve. Nothing transfers ownership. §17.9 is the authoritative list of
> what exists and what does not — read it before relying on any step below.

### 17.1 Why a copy is not enough
...
a crash window in which both ends believe the source owns it; §17.5 closes it.

### 17.2 The five-step handoff
...
| **tombstone / release** | Source becomes retained-but-non-executing, naming its new home. | see §17.5 |
...
### 17.3 What travels: allow-list, never exclude-list
...
audit trail carrying the path itself. A payload secret scan belongs on the source
...
and its refusal must name the matched pattern, never the matched value. That scan
is part of the transfer half and is **not in the tree** (§17.9): with no transmit
there is no payload leaving the host for it to guard.
...
`enabled=False` is persisted; what does not survive is the *reason* (see §17.6).
...
swallowed** — §17.7's non-goal explains why they are not rematerialized. Armed
...
### 17.5 The ack→tombstone crash window
...
### 17.6 Discoverability: the tombstone is a redirect, not a delete
...
A durable tombstone registry — `kind -> unit_id -> tombstone`, in one fsync'd
JSON file at `<config dir>/migration/tombstones.json`, i.e. under `config_dir()`
(`~/.kiro/crew` by default, or whatever `KIROCREW_HOME` overrides it to, the same
base dir `CronService` is given) — is therefore part of the design. **It is not
in the tree yet**: it landed and was then removed with the rest of the transfer
half, because nothing in production wrote a tombstone, which made the redirect
badge that reads one unreachable. It returns with the change that wires transmit,
where a real move finally produces something to record. The design it must
satisfy:
...
Once that registry exists, `kirocrew cron list` and the Schedule page each show a
redirect under a tombstoned row. Neither surface ships in this slice: with no
writer there is nothing to show, so both were removed rather than left rendering
an always-empty state.
...
### 17.9 Current state — what is NOT wired yet

Read this before assuming any part of a move works. **What exists in the tree
today is the PLAN and nothing else.** §17.2 through §17.8 describe the intended
design; the code that implements the transfer is not present.

Implemented:

- `POST /api/crons/{job_id}/move`, `POST /api/taskrunner/{task_id}/move` and
  `POST /api/chat/slots/{slot}/move` return a **plan**: the allow-listed payload
  it would send, and the named requirements the target would have to satisfy.
  Requirement identities are redacted before serialization. Error contract:
  `400` blank / missing / non-string `to_crew`, `404` unknown unit, `409`
  mid-run, `503` no runner.
- `kirocrew cron move` and `kirocrew taskrun move` **print that plan**, with
  requirement identities redacted and terminal controls replaced.
- A session move is planned from the DASHBOARD only, and there is no CLI verb
  for it: a session bundle is coherent only when snapshotted from the live slot
  with turns blocked, and the CLI cannot quiesce a slot it does not own. The
  gateway owns the slot, so it is the surface that can plan one — no surface can
  hand off.
- The per-kind allow-lists (§17.3) and their drift guards.

NOT in the tree, deliberately:

- `MigrationCoordinator`, the five-step handoff of §17.2, and `reconcile()` /
  the crash-window convergence of §17.5.
- `MigrationReceiver`, `accept()`, `lookup()` and the preflight probe of §17.4.
- The in-flight journal, and the durable `TombstoneRegistry` read model and
  redirect surfaces of §17.6.
- The receiver audit events of §17.8.
- The adapters' `quiesce` / `unquiesce` / `materialize` / `tombstone` steps. The
  only refusal that survives is the task-run mid-run check, which the PLAN needs.

All of it was written, tested, and then removed from the shipping change,
because nothing in production called it: every `migrate()` / `reconcile()` /
receiver call site was a test, no production path ever wrote a tombstone (the
adapters were constructed with no registry at all), and the redirect badge that
reads one therefore could not render. A library with no consumer removes none of
the double-execution harm §17.1 names. It returns with the change that wires
transmit over the tunnel, where each piece gains a caller. Tests assert those
names stay absent until then.
