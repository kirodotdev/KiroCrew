...
> **Design vs shipped.** §16.1–§16.8 describe the intended design. Only the
> PLAN half of it is in the tree today: given a unit, describe what moving it
> would involve. Nothing transfers ownership. §16.9 is the authoritative list of
...
is part of the transfer half and is **not in the tree** (§16.9): with no transmit
...
today is the PLAN and nothing else.** §16.2 through §16.8 describe the intended
...
- The per-kind allow-lists (§16.3) and their drift guards.

NOT in the tree, deliberately:

- `MigrationCoordinator`, the five-step handoff of §16.2, and `reconcile()` /
  the crash-window convergence of §16.5.
- `MigrationReceiver`, `accept()`, `lookup()` and the preflight probe of §16.4.
- The in-flight journal, and the durable `TombstoneRegistry` read model and
  redirect surfaces of §16.6.
- The receiver audit events of §16.8.
...
the double-execution harm §16.1 names. It returns with the change that wires
