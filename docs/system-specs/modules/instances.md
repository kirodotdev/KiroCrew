...
- `kirocrew session move` **refuses with exit 2** and says why: a session bundle
  is only coherent when snapshotted from the live slot with turns blocked, and
  the CLI cannot quiesce a slot it does not own. It names the dashboard as the
  surface that can *plan* it — not as one that can hand off, because none can.
