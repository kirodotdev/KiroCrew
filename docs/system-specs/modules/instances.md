...
for `secretary:`/`side:` absence is not evidence, since neither carries an app claim
that could go missing, and `dashboard:ui` is exempt because it names no per-tab slot.
...
caller there is `subagent:`.

Provenance is settled in **two tiers**, because a namespace proves the gateway
minted a key and not *for whom*. `OWNER_MINTED_SESSION_NAMESPACES`
(`dashboard`, `secretary`, `side`) is terminal — the name itself settles it, and
none of the three can be created on somebody else's behalf. `spawn_run`, by
contrast, mints `subagent:<id>` identically whether the parent is the person's tab
or a Slack participant's thread, so a flat roster admitting that prefix hands a
channel-parented child the owner's authority. Those namespaces sit in
`DELEGATED_SESSION_NAMESPACES` (`subagent`, `cron`) instead, where membership is
...
session authority". A cron job's owner is `CronJob.session_key`, and an
**ownerless row is refused** rather than assumed to be the person's. The row does
carry other provenance — `created_by` tags an app job, and
`source_preset`/`source_template_prompt` are written only by the dashboard create
path — but none of it *reliably* marks owner authorship for a row whose
`session_key` is absent: a plain Schedule-page create leaves the template fields
empty, and `created_by` would need an explicit comparison against the configured
owner that this walk does not make. The refusal rests on that, not on the record
being featureless. The workflow family (`wf`,
`wf-pool`, `wf-unpooled`, `wf-worker`, `wf-author`) and `taskrunner` are on
**neither** roster and so take the default refusal — a workflow run's originating
