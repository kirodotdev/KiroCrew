...
`copy2`. When the new home is already populated (the re-migration path — marker
present but legacy still present, so legacy is force-copied over the new home
again), a same-path destination file that is read-only would make `copy2`'s
