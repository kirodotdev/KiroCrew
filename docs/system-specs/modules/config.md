...
That is the **write** path. What is *injected into the agent's context* is gated
more tightly — on catalog membership, not shape — because the client-side
fallback silently split the two apart. See "The tag reaches the agent, too" below.
...
- **A tag with no shipped catalog injects nothing either**, for the same reason.
  `resolveLanguage()` only honours a *stored* tag that passes
  `isRestorableLanguage()` — exact membership in `SUPPORTED_LANGUAGES`, minus the
  dev-only pseudolocale — and otherwise falls back to browser detection. While
  the block forwarded any shape-valid tag, a catalog-less `ja` produced English
  chrome around Japanese purposes, and those purposes persist in session history,
  so the mismatch outlived a switch back to a shipped language. The set the
  builder checks (`context.py::_RESTORABLE_UI_LANGUAGES`) is a deliberate backend
  copy of the frontend catalogue, kept honest by
  `test/test_ui_language_catalog_parity.py`, which parses `languages.ts` and fails
  naming the gap. The cost is that shipping a language now needs that one-line
  backend edit too; the alternative was an agent that confidently writes in a
  language the interface is not rendering.
  Note the matching is **exact**: `zh-TW` and `en-GB` are not restorable stored
  values in the SPA either, so they inject nothing rather than resolving to
  `zh-CN`/`en`. The primary-subtag fallback in `detect.ts` applies only to
  *browser* tags.
