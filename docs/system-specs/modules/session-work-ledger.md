...
When `_coerce_state()` truncates text or ages `tried`, `events`, or `artifacts` past a bound, it logs one warning per read naming each loss and its count. This matters because `record()` persists the coerced state, making those discards permanent; wrong-typed fields remain the documented defaulting contract and are not reported as loss. `TestCoercionIsLoudAboutLoss` pins the retained values, aggregated warning, clean-record silence, and the fact that the warning stops after the first record cycle persists the bounded form.
