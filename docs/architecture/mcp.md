...
merely unlikely. Comparing unfolded consults no table. The tie-break is therefore
case-sensitive, which the ordering does not need: `order` is the sort key, the name
only separates rows sharing one, and every placement or drag renumbers siblings to
distinct values.
