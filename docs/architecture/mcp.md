...
found no owner, and only the second is safe to treat as unowned. Both reads must
therefore have seen every claim before an unclaimed name loses its grant, and an
app's enablement is read as a tri-state so that a metadata read fault is not
recorded as a deliberate disable. Matching is by alias family rather than
equality, because `mcp_server_alias()` is many-to-one and a collision is resolved
by suffixing, so a `base-2` sibling has to inherit its base's answer. A name a
