...
- A remote server that answers the handshake with `401` — or with `403` carrying a
  ...
  **Two pieces of evidence split that ambiguity**, and the wording follows what
  ...
  | challenge, `authGrantPresent` true | **Signed in** | A grant artifact exists. The badge reports THAT, never that the server answers — the probe holds no token, so validity is the one thing it cannot check, and the hover says so. |
  ...
  deliberately toned as a warning rather than a success, and it is not "Online" —
  a stored grant is evidence a sign-in happened, not proof it has not since been
  revoked, and only a probe carrying the runtime's token could close that gap.
