...
|-----------|--------|-------|--------------------|-----------------------|---------------------------|
| `asyncio.wait_for` on `_run_inner` | `subagent.py` | Subagent tasks | Captured per-run deadline: `agent.subagent_timeout_secs` (3 h default), optionally raised toward a 6 h default ceiling (`agent.subagent_timeout_max_secs`) when `agent.subagent_timeout_auto` is enabled | No (see reaper below) | Raises `TimeoutError`, marks subagent failed, learns a higher future deadline only when the run had activity and was not already stalled/suspected stalled, then resets session |
| Periodic reaper loop | `subagent.py` | Subagent tasks | 60s sweep (`_REAPER_INTERVAL`), kills at the run's captured deadline | Yes, runs independently of the spawning session | `_force_reap`: reset, SIGKILL fallback, mark done, optionally learn a higher future deadline, SEL audit, announce |
...
| ACP prompt timeout | `acp/client.py` | Per prompt | 4 h floor (`_DEFAULT_PROMPT_TIMEOUT`); follows the largest configured chat-turn/subagent deadline and, when enabled, the full adaptive subagent ceiling, with a 60s outer margin | No | Raises `AcpTimeoutError`; the transport remains outside any captured adaptive manager deadline |
