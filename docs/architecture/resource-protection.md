...
fails `fork()` with `EAGAIN` in whichever scope asks next, logs nothing, and every agent on
the host hits the same wall at once. What makes it observable is the count on the way up, so
...
gateway's tasks. The reading reaches the `resource_status` pull tool, the dashboard's
`/api/system` payload and the diagnostics bundle's posture block; past
`_SLICE_TASKS_TIGHT_RATIO` (90%) of the ceiling it also rides the injected `[RESOURCES]`
line, and raises that line by itself on a host whose memory is ample — the case the memory
...
refusal keeps naming the GB reading an operator can act on. Where there is no cgroup task
ceiling to approach (macOS, Windows, no delegation) all three figures read `-1` and every
surface omits them rather than reporting an unknown.
