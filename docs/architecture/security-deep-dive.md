...
snapshot is created on a namespace-private tmpfs, so the credential bytes never
appear in a host-visible backing file. It never exposes the host inode or the rest of
