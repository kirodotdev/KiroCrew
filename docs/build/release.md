...
| Workflow | Kind | Role |
|---|---|---|
| `nightly.yml` | trigger (schedule + dispatch) | Derives the date stamp, then calls everything below. `concurrency: nightly-build` with `cancel-in-progress: true`. Its last job, `report`, runs `always()` after every other job and keeps ONE tracking issue (label `nightly-build-failure`, hidden authorship marker in the body): a red run opens it or comments on the open one with the lanes that did not succeed, and the next green run closes it. `build-windows` is `soft_fail`, so its result is not trusted -- the reporter probes the run's artifacts for `build-windows-x64` instead. Pinned by `test_nightly_version_contract.py::TestFailureIssueReporter`. |
