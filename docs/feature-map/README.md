...
| Member event log & contributed views | A per-member append-only ledger drives the Crew Members roster and drawer: roster / activity / wake projections push whole values to the page over the WebSocket so a change (a star toggled elsewhere, a patrol that stopped, a model edited) re-renders in place with no roster refetch and survives a restart with its reason. On top of it, an installed app can — through the contribution protocol — read a member's history, append its own namespaced events, and publish a whole-value projection the drawer renders generically from a small declarative schema (the contributed-view card); the gateway stays the only writer and enforces ownership and quota at the boundary | No new page: it feeds the existing `/members` roster, drawer and card (see **Crew Members**); apps reach it through the protocol, not a dashboard control | `pages/members/ContributedViewCard.tsx`, `pages/members/MembersPage.tsx`, `state/memberProjectionStore.ts`, `state/useMemberProjection.ts`, `hooks/useWebSocket.ts` (`member_projection` frame) | `handlers/eventlog.py`, `dashboard/eventlog_ws.py`, `eventlog/service.py`, `eventlog/grants.py`, `eventlog_hooks.py` | `GET /api/eventlog/{kind}/{unit}/events`, `GET /api/eventlog/{kind}/{unit}/projections/{key}`, `GET /api/eventlog/{kind}/{unit}/projections/{key}/schema`, `POST /api/apps/{app}/token`, `GET /api/ws` (`member_projection`) |
