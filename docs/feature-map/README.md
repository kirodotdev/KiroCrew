...
| Move session to another crew | Plans a live session's handoff to another crew (Layer A transcript + Layer B context window), reporting what would not survive. Plan only — nothing transfers ownership yet | Session `⋯` menu → **Preview a move to crew…** | `components/SessionActionsMenu.tsx`, `components/MoveToCrewDialog.tsx` | `handlers/migration.py` | `POST /api/chat/slots/{slot}/move` |
...
| Move schedule to another crew | Plans a cron job's handoff to another crew: how many fields would ship, and the named requirements the target must satisfy first. Plan only — nothing transfers ownership yet | `/schedule` → row `⋯` → **Preview a move to crew…** | `pages/SchedulePage.tsx`, `components/MoveToCrewDialog.tsx` | `handlers/migration.py` | `POST /api/crons/{job_id}/move` |
...
| Move task run to another crew | Plans a task-runner run's handoff to another crew; refused while a task is mid-execution. Plan only — nothing transfers ownership yet | `/projects` → a non-executing run → `⋯` **More actions** → **Preview a move to crew…** | `pages/ProjectsPage.tsx`, `components/MoveToCrewDialog.tsx` | `handlers/migration.py` | `POST /api/taskrunner/{task_id}/move` |
