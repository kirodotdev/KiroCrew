...
| Frameless Linux menu tip | Desktop-only banner on Wayland/CSD Linux the first time the shell boots without a native window frame: explains that the native app menu (File / View / Content Text Size / Connection) has nowhere to render, offers **Enable window borders** (writes `linuxFrameless: false` and prompts for a Restart-now / Later choice) and a one-time **Don't remind me** dismissal | Appears automatically on Wayland/CSD first-launch, at the top of the app under `<MigrationCheck />` — no navigation | `components/FramelessTipBanner.tsx` (mounted app-wide in `App.tsx`) | `website/electron/ipc-registrar.js`, `website/electron/preload.js` (`framePrefsAPI`), `website/electron/linux-frame.js` (Wayland detection) | IPC, not HTTP: `frame-prefs:get`, `frame-prefs:mark-tip-shown`, `frame-prefs:enable-frames`, `frame-prefs:restart-app` |
