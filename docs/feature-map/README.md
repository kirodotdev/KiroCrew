...
| `instances` | Remote Kiro Crew instances to connect to, and the provisioning lanes that create them (built-in EC2 plus edition lanes from the `remote_provisioners` CPP seam), including regenerating an expired device-code sign-in without re-launching the instance | `RemoteCrewPanel.tsx`, `InstancesPanel.tsx`, `components/remoteProvisionerRenderers.tsx` | `handlers_instances.py`, `handlers_cloud.py` | `GET,POST /api/instances`, `POST /api/instances/{id}/connect`, `GET /api/cloud/provisioners`, `GET /api/cloud/preflight`, `POST /api/cloud/launch/{id}/signin/refresh` |
