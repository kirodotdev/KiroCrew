...
| Files in chat | Browse, attach and upload workspace files; a tree row also accepts a dropped OS file (or the row menu's **Upload files…** picker) and writes it into that directory, prompting instead of silently overwriting on a name collision | Chat left rail → files; composer attach; drag a file onto a directory row, the full-pane drop overlay, or row menu → **Upload files…** | `pages/chat/FileBrowserRail.tsx`, `pages/chat/FilesHomePanel.tsx`, `pages/chat/resolveUploadTargetDir.ts` (internal helper — resolves a drop's target directory from the tree DOM, no new user-facing entry point), `pages/chat/useDirectoryUpload.tsx` (internal helper — the upload/collision-confirm/error hook shared by the drop, overlay and picker paths, no new user-facing entry point) | `handlers/files.py` | `GET /api/file-read`, `POST /api/upload`, `POST /api/upload/file`, `POST /api/directory-upload` |
