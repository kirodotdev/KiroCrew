...
| Files in chat | Browse, attach and upload workspace files; search the rail by file NAME or by file CONTENTS, including the text inside Word/PowerPoint/Excel documents; a tree row also accepts a dropped OS file (or the row menu's **Upload files…** picker) and writes it into that directory, prompting instead of silently overwriting on a name collision | Chat right panel → Files tab, Name/Content toggle below the search field; composer attach; drag a file onto a directory row, the full-pane drop overlay, or row menu → **Upload files…** | `pages/chat/FileBrowserRail.tsx`, `pages/chat/FilesHomePanel.tsx`, `api/fileGrep.ts`, `pages/chat/resolveUploadTargetDir.ts` (resolves a drop's target directory from the tree DOM; no new user-facing entry point), `pages/chat/useDirectoryUpload.tsx` (the upload/collision-confirm/error hook shared by the drop, overlay and picker paths; no new user-facing entry point) | `handlers/files.py` | `GET /api/file-read`, `POST /api/file-grep`, `POST /api/upload`, `POST /api/upload/file`, `POST /api/directory-upload` |
