...
| Files in chat | Browse, attach and upload workspace files; search the rail by file NAME or by file CONTENTS, including the text inside Word/PowerPoint/Excel documents; preview one in the side panel — a .docx renders its headings, lists, tables and bold/italic runs, falling back to extracted plaintext; right-click a file row → Download saves it to the machine running the browser through the credential-gated download endpoint | Chat right panel → Files tab, Name/Content toggle below the search field; right-click a file row → **Download**; composer attach | `pages/chat/FileBrowserRail.tsx`, `pages/chat/FilesHomePanel.tsx`, `api/fileGrep.ts`, `components/FileRenderers.tsx`, `pierre/PierreWorkspaceTreeImpl.tsx` | `handlers/files.py` | `GET /api/file-read`, `GET /api/file-office-preview`, `GET /api/file-download`, `POST /api/file-grep`, `POST /api/upload`, `POST /api/upload/file` |
...
| Workspace fullscreen and panel toggles | Expands the whole right workspace panel over the dashboard's content grid (topbar stays) without remounting its tabs, editor drafts, browser frames or terminals; Escape exits unless a dialog, menu, editor, annotation composer or focused terminal owns it, and the visible control always exits. The chat title row (panel closed) and the panel's own strip (panel open) carry the same 28px Bottom-panel and Side-panel toggles, so the Side toggle keeps one bounding box across open/close; a file hosted inside the workspace shows no file-only fullscreen | Chat right panel strip → **Full screen** (right or bottom dock); title-row **Bottom panel** / **Side panel** toggles | `pages/chat/SidePanel.tsx`, `components/PanelToggles.tsx`, `components/WorkspacePanelContext.ts`, `hooks/useWorkspaceFullscreenPanel.ts`, `pages/ChatPage.tsx`, `App.tsx` (route-level state) | client-only (no handler) | none |
