...
| Merge a fork back | Fold a fork's summary into its parent; the fork becomes a read-only archived card | Session menu (sidebar row / header / context menu) → Merge back; or Split view → fork breadcrumb dropdown → Merge back | `components/SessionActionsMenu.tsx` (default-surface action owner), `components/ChatPane.tsx` (split-view action owner), `pages/chat/MergedSummaryCard.tsx` (archived-result renderer) | `chat_merge_back.py` | `POST /api/chat/slots/{slot}/merge-back` |
