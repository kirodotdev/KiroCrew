...
| Session crew log | The session's own append-only history, read two ways: a page of entries in a seq range with every citation resolved to a verdict, and one of five folds (`status`, `usage`, `timeline`, `tools`, `approvals`) with the seq it was folded through. Owner-gated, because the log carries the session's redacted message bodies. A growth pushes a `session_projection` frame to owner sockets. Written only when `KIROCREW_CREW_LOG` is on; no dashboard page yet, the frame shape is specified so the client can follow | Owner reads; no dashboard page yet | — | `handlers/crew_log.py` | `GET /api/sessions/{id}/crew-log`, `GET /api/sessions/{id}/crew-log/projection/{name}` |
