...
| Hooks | Event-triggered agent runs | `?tab=hooks` | `pages/HooksPage.tsx`, `pages/hookEventWireValues.ts` (internal split of HooksPage — the event id ⇄ wire-value map the picker and the rows both read, no new user-facing destination) | `handlers/hooks.py` | `GET,POST /api/hooks`, `PUT,DELETE /api/hooks/{hook_id}`, `GET /api/kiro-hooks` |
