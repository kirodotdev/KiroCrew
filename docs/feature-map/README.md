...
| Pinned prompt banner | Keeps the turn's own prompt visible in a banner at the top of a long reply while it scrolls; distinct from **Pinned messages** above. The banner is an opaque band with a token-aware lower fade, so transcript text scrolling underneath cannot leak around the card or read as part of it. Worn by the main chat AND by every `ChatPane` host — split-view panes and the Crew Members DM thread — through the same hook | Settings → Chat → **Pin the latest turn** | `pages/chat/PinnedPrompt.tsx`, `pages/chat/usePinnedPrompt.ts` (internal split of `useChatPageTranscriptController` — the host-agnostic pin geometry, scroll recompute and in-place jump shared with `components/ChatPane.tsx`; no new user-facing entry point), `app-sdk/ChatMessageList.tsx` (`onDisplayItems` / `hiddenRow` row-indexing seam) | client-only (no handler) | none |
