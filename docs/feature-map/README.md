...
| `ai-backends` | The AI backends this build offers plus the registry's view of operator-defined ones from `harnesses.json` (write-protected): each backend's id, selectable/known/invalid state, and the reason an entry is unselectable. Backing the per-chat backend picker (composer + Welcome view) and the per-subagent backend override (`spawn_run`), both resolved through the single selection gate above the global default | `pages/settings/BackendsPanel.tsx`, `components/BackendSelector.tsx`, `hooks/useBackends.ts` | `handlers/backends_listing.py`, `handlers/agents.py` (per-backend models) | `GET /api/backends`, `GET /api/models?backend=`, `POST /api/chat/slots` (`backend`), `POST /api/chat/slots/{slot}/backend` |
