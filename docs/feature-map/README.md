...
| Hooks | Event-triggered agent runs, including `SessionLaneChanged` when a session's board lane changes | `?tab=hooks` | `pages/HooksPage.tsx` | `handlers/hooks.py`, `chat_tags.py` (dispatches `SessionLaneChanged` from the tag writes), `chat_handlers.py` (dispatches it for a folder's inherited status tag) | `GET,POST /api/hooks`, `PUT,DELETE /api/hooks/{hook_id}`, `GET /api/kiro-hooks`, `PUT /api/chat/slots/{slot}/tags`, `POST /api/chat/slots/{slot}/drop`, `DELETE /api/chat/tags/{id}`, `POST /api/chat/slots` |
...
| Hooks | Full-page hook manager; the event picker offers `SessionLaneChanged` for board-lane transitions | `/hooks` | `pages/HooksPage.tsx` | `handlers/hooks.py`, `chat_tags.py` (dispatches `SessionLaneChanged` from the tag writes), `chat_handlers.py` (dispatches it for a folder-inherited status tag) | `GET,POST /api/hooks`, `GET /api/kiro-hooks`, `PUT /api/chat/slots/{slot}/tags`, `POST /api/chat/slots/{slot}/drop`, `DELETE /api/chat/tags/{id}`, `POST /api/chat/slots` |
