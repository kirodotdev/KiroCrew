...
| `secrets` | Managed integration credentials plus other encrypted vault entries; a Custom secret the owner authorizes for an exact https origin can also be used by the agent to call that API from chat, mediated host-side so the plaintext never enters the sandbox | `SecretsPanel.tsx` | `handlers/secrets.py`, `handlers/sessions.py` (`api_mediated_secret_request` — host-side vault-read + SSRF-checked dispatch for the in-sandbox `kirocrew-secrets` MCP tool) | `GET,POST /api/secrets`, `DELETE /api/secrets/{name}`, `POST /api/mediated-secret-request` |
