...
| Message font size | Resizes message body text (paragraphs, list items, headings) independently of the rest of the dashboard; sidebar, session list and other chrome are unaffected. Applies in embedded/popout chat sessions too | Settings → Chat → **Message font size** | `pages/ChatPage.tsx`, `components/ChatPane.tsx`, `pages/chat/ChatSettings.tsx`, `pages/chat/UserMessage.tsx`, `pages/chat/AssistantMessage.tsx`, `pages/settings/ChatPanel.tsx`, `styles/message-font-size.css`, `app-sdk/ChatEmbed.tsx`, `app-sdk/messageRenderers.tsx` | client-only (no handler) | none |
