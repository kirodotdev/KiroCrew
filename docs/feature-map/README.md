...
| Release-channel worktree (Dev Fleet) | A detached worktree pinned to the newest published `stable` release, resolved from a private tag mirror (`refs/dev-fleet/release-tags`) so a local tag cannot masquerade as a release, and carried as an ordinary fleet row so a shipped build can be podded beside unreleased work. The pin never moves in place — a newer release is Remove then Create. Never touches `$KIROCREW_HOME/channel`, so it does not change which channel this install updates from | `/dev-fleet` → the `release-channel-stable` row: **Create worktree** on the placeholder | `pages/DevFleetPage.tsx` | `apps/builtins/dev_fleet/http_api.py`, `release_channel_pin.py` (resolve), `worktree_ops.py` (create) | `POST /apps/dev-fleet/api/release-channel/create` |
