...
| Move session to another crew | Plans a live session's single-owner handoff to another crew (Layer A transcript + Layer B context window) | Session `⋯` menu → **Move to crew…** | `components/SessionActionsMenu.tsx`, `components/MoveToCrewDialog.tsx` | `handlers/migration.py` | `POST /api/chat/slots/{slot}/move` |
...
| Move schedule to another crew | Plans a cron job's single-owner handoff to another crew; a moved job carries a redirect badge in its status cell | `/schedule` → row `⋯` → **Move to crew…** | `pages/SchedulePage.tsx`, `components/MoveToCrewDialog.tsx`, `components/MigratedBadge.tsx` | `handlers/migration.py`, `handlers/cron.py` | `POST /api/crons/{job_id}/move` |
...
| Move task run to another crew | Plans a task-runner run's single-owner handoff to another crew; refused while a task is mid-execution | `/projects` → a non-executing run → **Move to crew…** | `pages/ProjectsPage.tsx`, `components/MoveToCrewDialog.tsx` | `handlers/migration.py` | `POST /api/taskrunner/{task_id}/move` |
