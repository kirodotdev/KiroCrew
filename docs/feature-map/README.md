...
| Zero-turn option actions | Model-offered chip that closes the session — not just the tab — with no LLM turn | Follow-up chip row under the last reply → **close** chip | `components/FollowUpBar.tsx`, `pages/ChatPage.tsx` | `chat_handlers.py` | `POST /api/chat/slots/{slot}/note`, `DELETE /api/chat/slots/{slot}` |
