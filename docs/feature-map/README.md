...
| Model picker order | User-authored display order for the model-selection dropdown, honored by every picker (chat composer, agents pages, sidebar bulk switcher, app pickers, cron job form); Auto stays pinned first, unlisted models append in backend order | Composer model chip → **Model order** (drill-in page in the picker) — drag to reorder, Reset to default order | `components/ModelOrderEditor.tsx`, `components/ModelEffortDropdown.tsx`, `providers/modelList.ts` (`applyModelOrder`), `hooks/useAvailableModels.ts` | `handlers/core.py` (`api_kirocrew_config_patch`, `str_list`) | `GET,PATCH /api/config/kirocrew` (`agent.model_order`) |
