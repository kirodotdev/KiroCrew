...
| Work ledger | A shared record between a conductor and its workers. Workers report against their own bound item; the conductor reads progress and records acceptance. Member DMs can dispatch an item with `spawn_run(work_item_id=...)` and publish the read-only `tasks` template. Creation, steering and decisions stay in member chat; the board shows its publication time and keeps worker completion separate from acceptance. `kirocrew-ledger-conductor` and `kirocrew-worker` retain their existing tools; the other shipped conductors keep their transcript procedure | Crew Members → Crew summary → Dashboard → Open dashboard, after asking the member to publish its task board; agent tools on `kirocrew-work` and `kirocrew-panel` | `pages/members/CrewWebview.tsx`; bundled `agent_panel_templates/tasks.html` | `handlers/work_ledger.py`, `handlers/agent_panel.py`, `handlers/messaging.py`, `subagent_manager/run.py` | `GET /api/work-ledger`, `POST /api/work-ledger/record`, `GET /api/work-ledger/brief`, `POST /api/work-ledger/report`, `POST /api/spawn` (`work_item_id`), `POST /api/agent-panel/publish` (`template: tasks`), `GET /api/members/{slug}/panel` |
