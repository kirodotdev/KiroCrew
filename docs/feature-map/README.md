...
| Crew Members | One durable pinned DM thread per crew member; the detail drawer lists the worker sessions the member is driving (live `slots` frames filtered on `created_by`) | `/members` — rail row when the crew preview is on | `pages/members/MembersPage.tsx` | `handlers/members.py`, `slot_projection.py` (`created_by`), `session_control.py` (escalation to the human: `send_to_target` with `target="user"`), `crew_conversation.py` (the per-member conversation index) | `GET /api/members`, `POST /api/members/{slug}/thread`, `GET /api/members/{slug}/activity`, `GET /api/members/{slug}/conversation`, `GET /api/ws` (`slots`), `POST /api/session-control/send` (`target="user"`) |
