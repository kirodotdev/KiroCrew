...
| Files in chat | Browse, attach and upload workspace files; right-click a file row → Download saves it to the machine running the browser through the credential-gated download endpoint | Chat left rail → files; right-click a file row → **Download**; composer attach | `pages/chat/FileBrowserRail.tsx`, `pages/chat/FilesHomePanel.tsx`, `pierre/PierreWorkspaceTreeImpl.tsx` | `handlers/files.py` | `GET /api/file-read`, `GET /api/file-download`, `POST /api/upload`, `POST /api/upload/file` |
