...
| `secrets` | Managed integration credentials plus other encrypted vault entries; a Custom secret the owner authorizes for an exact https origin can also be used by the agent to call that API from chat, mediated host-side so the plaintext never enters the sandbox. The in-sandbox `kirocrew-secrets` MCP server is caller-aware, so the host endpoint authenticates each request with an audience-bound member-session proof (a proof minted for another server, or an unidentified caller, is refused). That proof is only minted for a private (V2) member session, so the mediated call works from a member conversation and a default Global (V1) chat is refused — this is a deliberate scoping of a credential-bearing egress to an authenticated session | `SecretsPanel.tsx` | `handlers/secrets.py`, `handlers/sessions.py` (`api_mediated_secret_request` — host-side vault-read + SSRF-checked dispatch, gated on the audience-bound proof, for the in-sandbox `kirocrew-secrets` MCP tool) | `GET,POST /api/secrets`, `DELETE /api/secrets/{name}`, `POST /api/mediated-secret-request` |
