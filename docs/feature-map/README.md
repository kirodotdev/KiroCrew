...
| Source links | PR/issue/Jira chips derived from the transcript; unlink one to hide it | Session card chip → its ⋮ overflow menu → **Unlink from this session** | `pages/ChatSidebar.tsx` | `chat_handlers.py`, `handlers/source_providers.py` | `GET /api/chat/slots/{slot}/source-links`, `DELETE /api/chat/slots/{slot}/source-links/{identity}` |
