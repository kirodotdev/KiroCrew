...
|---|---|---|---|---|---|
| Org Chart and Guardrails | Persistent member roles, reporting lines, hiring limits, delegated assignments, review and team execution | `/capabilities?tab=organization`; also the crew editor’s Org Chart and Guardrails pane | `pages/OrganizationPage.tsx`, `pages/members/OrganizationMemberWork.tsx`, `api/organization.ts` | `handlers/organization.py`, `handlers/members.py` | `GET,POST /api/organization`, `GET,POST /api/organization-agent` (verified member tools), `POST /api/members/{slug}/thread` |
