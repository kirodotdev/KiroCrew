...
| `security` | Denied commands, sensitive paths, approval posture; this-crew AgentCore identity (**Agent identity** section — Off / Workload / Login posture, workload name and Gateway URL, Save gated on a writable home policy); this-crew AgentCore Gateway catalog (live control-plane snapshot, tools/list through the workload SigV4 proxy, operator-gated target Sync — owner dashboard cookie only); pre-save Gateway preview (what Save would grant for the drafted URL/posture: Gateway + targets from the control plane, nothing persisted, tools deferred to after Save); login-posture consent link (the pending allowlisted 3LO authorization URL, owner dashboard cookie only) | `SecurityPanel.tsx`, `PostureDisclosure.tsx` | `handlers/security.py`, `handlers/tailnet.py`, `handlers/file_delivery_consent.py`, `handlers/agentcore_identity.py`, `handlers/agentcore_inspect.py`, `handlers/agentcore_consent.py` | `GET /api/security/denied-commands`, `PATCH .../builtins/{id}`, `POST .../user`, `GET,POST,DELETE /api/file-delivery/consent`, `GET,PUT /api/agentcore/identity`, `GET /api/agentcore/gateway`, `POST /api/agentcore/gateway/verify`, `POST /api/agentcore/gateway/preview`, `POST /api/agentcore/gateway/sync`, `GET /api/agentcore/consent` |
