...
| Files in chat | Browse, attach and upload workspace files, and preview one in the side panel — a .docx renders its headings, lists, tables and bold/italic runs, falling back to extracted plaintext | Chat left rail → files; composer attach | `pages/chat/FileBrowserRail.tsx`, `pages/chat/FilesHomePanel.tsx`, `components/FileRenderers.tsx` | `handlers/files.py` | `GET /api/file-read`, `GET /api/file-office-preview`, `POST /api/upload`, `POST /api/upload/file` |
