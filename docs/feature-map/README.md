...
| Source links | PR/issue/Jira chips derived from the transcript; unlink one to hide it | Session row ⋮ menu (or right-click; the same menu opens from the chat header title) → **Unlink a PR or Issue chip** → pick a link, then select it again to confirm | `pages/ChatSidebar.tsx`, `components/SessionActionsMenu.tsx`, `components/SourceLinksSubmenu.tsx` | `chat_handlers.py`, `handlers/source_providers.py` | `GET /api/chat/slots/{slot}/source-links`, `DELETE /api/chat/slots/{slot}/source-links/{identity}` |
