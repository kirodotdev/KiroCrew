...
| Advisor | Opt-in cross-model session reviewer: severity-tagged advice cards in the transcript, per-session enable/disable override, global settings (enable, reviewer model) | Model & reasoning popover → **Advisor** (inherit/enabled/disabled); Settings → Chat → **Advisor** (global defaults); advisor cards appear in the transcript when enabled | `pages/chat/AdvisorMessage.tsx`, `components/AdvisorOverrideControl.tsx`, `pages/settings/ChatPanel.tsx` (Advisor section) | `chat_handlers.py` (`api_chat_slot_advisor_override`), `handlers/core.py` (`advisor.enabled` PATCH validator), `server.py` (live re-apply of `advisor.*`, `agent.role_models.advisor`, `agent.acp_backend`), `../advisor/` (native subsystem) | `POST /api/chat/slots/{slot}/advisor-override`, `PATCH /api/config/kirocrew` (`advisor.enabled`, `agent.role_models.advisor`) |
