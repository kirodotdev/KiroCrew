...
| Agent Templates | The harness-level agent definitions crews bind to — viewed and edited inline in each agent's Template pane (`components/crew/AgentTemplateDetail.tsx`); the standalone tab was removed. Includes which skills each template maps by `skill://` URI: a mapping write naming a key the catalogue no longer resolves is refused with code `skills_unknown`, the response reports the keys this install cannot resolve, and a preserved URI is deleted only when the write NAMES it in `removed_unmanaged_skills`. A removal-only PATCH omits `skills` altogether, which states no managed set: the spec re-read under lock then stays authoritative for every mapping the request does not name, so a mapping a concurrent session added cannot be dropped by a client whose keys are stale. `unmanaged_skills` on the request carries what that client SAW as unmanaged — never a keep-list, only a way to tell a mid-flight reclassification from a mapping that was managed all along — and the response returns `unmanaged_skills` so a caller learns which URIs this install holds outside the managed set | `/capabilities?tab=crews` | `pages/KiroCrewAgentsPage.tsx`, `components/crew/AgentTemplateDetail.tsx`, `components/AgentSkillsEditor.tsx` | `handlers/agents.py` | `GET /api/agents/installed`, `GET,PATCH /api/agents/detail/{name}`, `POST /api/agents/detail/{name}/fork`, `POST /api/agents/detail/{name}/publish`, `POST /api/agents/detail/{name}/reset` |
...
| Skills | Installed skills, the public registry, pending candidates. Where several package bundles vendor a skill at the same relative path, each copy lists and opens under its own `package/<qualifier>:<rel>` key, read through the same reader every other skill key uses — on platforms offering a descriptor-relative pinned walk; where none is available the colliding paths are omitted from the catalog rather than listed under a key no read could answer. A copy whose `SKILL.md` is a symlink is omitted for the same reason; one carrying extra HARD links is omitted too, because a hardlink canonicalises to its own in-root path and so cannot be told from a file aliased in from outside the root -- a copy no read can answer is not catalogued as though it could be | `?tab=skills` | `pages/overview/SkillsTab.tsx` | `handlers/prompts.py`, `handlers/discover.py`, `handlers/skill_budget.py` | `GET,POST /api/skills`, `GET /api/skills/-/discover`, `GET /api/skills/-/pending`, `GET /api/skills/{name}/-/tree`, `GET /api/skills/{name}/-/file`, `GET,PUT,DELETE /api/skills/{name}` |
