...
| Session folders | User-defined folders grouping session rows | Sidebar folder header → drag a row; folder ⋯ menu → New ephemeral chat (incognito / temporary / clean) to start a mode-pinned session in the folder | `pages/ChatSidebar.tsx` | `chat_folders.py`, `chat_handlers.py` | `GET,POST /api/chat/folders`, `PATCH /api/chat/slots/{slot}/folder`, `POST /api/chat/slots` |
