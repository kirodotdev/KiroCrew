...
| `security` | Denied commands, sensitive paths, approval posture, Docker registry credential grant | `SecurityPanel.tsx`, `DockerRegistryAccessCard.tsx`, `PostureDisclosure.tsx` | `handlers/security.py`, `handlers/tailnet.py`, `handlers/docker_registry_access.py`, `handlers/file_delivery_consent.py` | `GET /api/security/denied-commands`, `GET,PUT /api/security/docker-registry-access`, `PATCH .../builtins/{id}`, `POST .../user`, `GET,POST,DELETE /api/file-delivery/consent` |
