...
| Folder search | Find a session folder by name and jump to it. The sidebar's search box matches folder names as well as session titles, keeping a matched folder with its whole subtree; the command launcher and the quick-search palette both list folders as their own result group. Choosing a folder makes it visible for the visit, expands its collapsed ancestors, scrolls it into view and flashes it. Lane-dependent: the flat lane renders no folder rows at all, so a match there is visible only through the sessions it keeps and a reveal leaves that lane for the tree; the board lane draws a folder as a column, so a reveal flashes the column rather than a header row | Sidebar search box → type a folder name; ⌘K / Ctrl+K → type a folder name → **Folders** group | `pages/ChatSidebar.tsx`, `apps/command-bar/CommandBarOverlay.tsx`, `apps/command-bar/rootIndex.ts`, `components/commandPalette/providers/foldersProvider.ts`, `components/CommandPalette.tsx`, `store/chatSlice.ts`, `utils/folderTree.ts` | none new — reads the folder tree the sidebar already loads | `GET /api/chat/folders` |
...
| Files in chat | Browse, attach and upload workspace files; search the rail by file NAME or by file CONTENTS, including the text inside Word/PowerPoint/Excel documents; preview one in the side panel — a .docx renders its headings, lists, tables and bold/italic runs, falling back to extracted plaintext; right-click a file row → Download saves it to the machine running the browser through the credential-gated download endpoint; the header carries the per-project actions — reveal the project directory, and open a terminal already in it | Chat right panel → Files tab, Name/Content toggle below the search field; right-click a file row → **Download**; composer attach; Files tab header → reveal button and **Project actions** → **Open terminal in the project directory**, whose row carries a one-line note saying what a terminal is | `pages/chat/FileBrowserRail.tsx`, `pages/chat/FilesHomePanel.tsx`, `api/fileGrep.ts`, `components/FileRenderers.tsx`, `pierre/PierreWorkspaceTreeImpl.tsx` | `handlers/files.py` | `GET /api/file-read`, `GET /api/file-office-preview`, `GET /api/file-download`, `POST /api/file-grep`, `POST /api/upload`, `POST /api/upload/file` |
...
| Today | What was worked on today, with no model call: the Overview summary card counts sessions active in the browser's local day (live slots plus archived rows, incognito/temporary excluded); its drill-in lists them grouped by folder with a click that opens the session, beside the entries already folded into today's daily-history file, whose empty state names the consolidation idle window. Shown in both memory modes, including vector-active installs where the editable Daily history card is hidden | `/settings/overview?view=today` | `pages/OverviewPage.tsx` (`TodaySummaryCard`), `pages/overview/TodayTab.tsx`, `pages/overview/useTodayActivity.ts`, `pages/overview/todayActivity.ts` | `handlers/memory.py`, `handlers/sessions.py`, `chat_folders.py` | `GET /api/memory/history?date=`, `GET /api/sessions?exclude_open=1`, `GET /api/chat/folders`, `GET /api/memory/settings` |
...
| Session crew log | The session's own append-only history, read two ways: a page of entries in a seq range with up to `MAX_PAGE_REFS` citations resolved to a verdict and the rest reported in `refs_unresolved`, and one of five folds (`status`, `usage`, `timeline`, `tools`, `approvals`) with the seq it was folded through. Owner-gated, because the log carries the session's redacted message bodies. A growth pushes a `session_projection` frame to owner sockets. Written only when `KIROCREW_CREW_LOG` is on; no dashboard page yet, the frame shape is specified so the client can follow | Owner reads; no dashboard page yet | — | `handlers/crew_log.py` | `GET /api/sessions/{id}/crew-log`, `GET /api/sessions/{id}/crew-log/projection/{name}` |
...
|---|---|---|---|---|
| `overview` | Health hero, stat cards, memory, usage, WakaTime and today drill-ins | `OverviewPanel.tsx` → `pages/OverviewPage.tsx` | `handlers_system.py` | `GET /api/status`, `GET /api/system` |
...
| `developer` | Developer Mode gate, Feature Previews opt-ins, local-gateway switch. Four of the five previews are per-device client flags; **Decisions (Jev)** is not — its switch writes the gateway's own `decisions.preview` through the shared config route, because the gate that reads the flag runs in the gateway and cannot see this browser's storage. On a gateway whose config carries no `decisions` section that switch renders disabled with the reason beside it rather than offering a write the PATCH allowlist would refuse, and it lists each decision point's arm read-only. `decisionsPreview.ts` is the pure config reader behind that one card — no feature of its own | `DeveloperPanel.tsx`, `FeaturePreviewsSection.tsx`, `pages/settings/decisionsPreview.ts` | `handlers/core.py` (the shared config route) | `GET,PATCH /api/config/kirocrew` |
...
| `agent-backend` | Which agent harness backend is live, and a **capability card** per harness -- available / not-available capability lines, the always-visible tool-approval and security lines, and a described row for a harness this build does not offer, all projected from capability membership by `agent_sdk/backend_cards.py`; hosts the **Kiro sign-in** card (see Standalone operator surfaces) while KAS is an offered backend | `pages/developer/AgentBackendTab.tsx`, `pages/developer/KiroSignInCard.tsx` | `handlers/acp_backend_status.py`, `handlers/kiro_prerequisite.py`, `agent_sdk/backend_cards.py` | `GET /api/acp-backends`, `GET /api/kiro-prerequisite` |
