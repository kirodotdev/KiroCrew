...
| Crew Members | One durable pinned DM thread per crew member, plus a configurable webview each crew fills in; the detail drawer also lists the worker sessions the member is driving (live `slots` frames filtered on `created_by`) | `/members` — rail row when the crew preview is on; the drawer's **Dashboard** section, expandable to full window | `pages/members/MembersPage.tsx`, `pages/members/CrewWebview.tsx` | `handlers/members.py`, `handlers/agent_panel.py`, `slot_projection.py` (`created_by`) | `GET /api/members`, `POST /api/members/{slug}/thread`, `GET /api/members/{slug}/activity`, `GET /api/members/{slug}/panel`, `POST /api/agent-panel/publish`, `GET /api/agent-panel/templates`, `GET /api/ws` (`slots`) |
