...
| Custom titlebar menu on frameless Linux | The in-app hamburger menu that Windows renders (File / Edit / View / Connection / Window / Help) also renders on Linux Wayland/CSD, where the compositor doesn't offer a native window frame. Zero user action required — the hamburger appears in the top-left of the header on first launch at desktop widths (hidden on narrow-window mobile-like layouts where `useIsMobile()` is true; those users have no menu path today, unchanged by this PR). Framed Linux (X11, KDE Wayland with SSD) sees the OS-drawn menu bar as before. Replaces an earlier tip-banner iteration that asked the user to enable OS-drawn window borders and restart | Appears automatically on Wayland/CSD boot — no navigation | `components/WindowsTitlebarMenu.tsx` (rendered under `isWinElectron || isLinuxFramelessElectron` at App.tsx:3353) | `website/electron/ipc-registrar.js`, `website/electron/app-menu.js`, `website/electron/linux-frame.js` (Wayland detection) | IPC, not HTTP: `app-menu:items`, `app-menu:execute` — at their `upstream/main` posture, no sender-origin gate added by this PR |
| Content Text Size ladder | Four-tier `Small (12px) / Medium (16px, default) / Large (20px) / Very Large (24px)` radio group under View → Content Text Size. Persisted per-installation in `displayPreferences.fontSize`. Applies live to every shell-owned window via CDP `Page.setFontSizes` (both standard and monospace); construction-time `webPreferences.defaultFontSize` + `defaultMonospaceFontSize` seed keep restart parity. The embedded browser panel is excluded — its font size follows the site it hosts | View → Content Text Size in the app menu (in-app hamburger on Windows / frameless Linux; OS-drawn menu bar on framed Linux; system menubar on macOS) | `— (no SPA surface; shell-owned)` | `website/electron/display-preferences/schema.js`, `website/electron/display-preferences/index.js`, `website/electron/app-menu.js`, `website/electron/window-lifecycle.js` (WeakSet ownership + construction seeds) | `— (no HTTP or IPC; menu-click dispatches directly)` |
