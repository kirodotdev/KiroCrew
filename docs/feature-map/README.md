...
| Merge a fork back | Fold a fork's summary into its parent; the fork becomes a read-only archived card | Split view → fork breadcrumb dropdown → Merge back | `components/ChatPane.tsx` (action owner), `pages/chat/MergedSummaryCard.tsx` (archived-result renderer) | `chat_merge_back.py` | `POST /api/chat/slots/{slot}/merge-back` |
