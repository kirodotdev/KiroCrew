...
- Loader (`config/sections.py`, `config/loader.py`) validates the shape.
  ...
  credential value is never in config. The variable must be under the
  `KIROCREW_A2A_*` namespace, and its companion `<token_env>_ORIGIN` pins the
  one origin the credential may be sent to (see *Security considerations*).
  Only the object spelling is accepted.
...
| 2 | `providers/a2a.py`, `providers/base.py` | `A2AProvider`, `A2AStreamError`, SSE mapping, origin pin, TLS-only transport with named-loopback plaintext, credential-origin pin, size caps, one-turn-one-task lock, card security-requirement check, cancel; `provider_label` on the ABC | ~640 |
...
| 7 | `subagent_manager/continuation.py` | rebuild provider from the recorded A2A facts (`RecordedA2A`, one off-loop read shared with `run.py`); inherit the recorded agent or refuse another (`agent_mismatch`); `steer_unsupported` for direct providers | ~25 |
| 8 | `platform/governance.py` | `capabilities.remote_spawn` catalog row (opt-in, `agents` and `origins` rulesets) | ~20 |
...
| 11 | tests | `test_a2a_provider.py` (69), `test_fake_a2a_server.py` (16), `test_fake_acp_delegation.py` (15), `TestRemoteSpawnGate` (8), headless E2E (2) | ~1,650 |
...
**Trust model: registering a remote is a configuration decision.** The
registry lives in `config.json`, which is agent-writable. Whoever can edit that
file can register a remote agent — exactly as they can already register an MCP
server or a connection there, and an MCP server likewise receives request
content and is reached over the network. A remote agent is at least as much of
a trust decision as an MCP server (it may receive more context, since a task
description can be longer than a tool argument), and this RFC treats it the
same way: **registry membership is the grant**, remote spawn is permitted by
default, and the controls below exist to make sure a config edit alone cannot
(a) steal a credential the operator issued, (b) send anything unencrypted off
the host, or (c) bypass a policy an operator has written. What a config edit
*can* do — point the gateway at a new HTTPS destination with no credential — is
the same power it already has over MCP servers, and the operator-side lock for
fleets that want to remove it is the `origins` ruleset under
`capabilities.remote_spawn`, not a second registry. Three things therefore live
outside `config.json`, in the gateway's **environment** (set by whoever starts
the process; an agent cannot change a running process's environment, and the
launcher is outside the data home it works in):

- the credential itself (`<token_env>`), because a secret does not belong in
  config;
- the origin that credential may travel to (`<token_env>_ORIGIN`), because a
  config edit must not be able to redirect a token it cannot read;
- the loopback origins that may be reached over plain HTTP
  (`KIROCREW_A2A_PLAINTEXT_ORIGINS`), because a config edit must not be able to
  aim the gateway at an arbitrary local port unencrypted.

A worked example of all three follows the controls.
...
two rulesets: `agents` over registry entry names and `origins` over the
entries' card-URL origins (`scheme://host[:port]`, host-glob matched like
`network.egress`). The layering is:

- `capabilities.spawn` off → no sub-agents at all, local or remote;
- `capabilities.remote_spawn` absent from the policy (or no policy at all) →
  ungoverned and permitted, the standalone default;
- named in a POLICY without `enabled: true`, or `enabled: false` → local
  sub-agents work, every remote spawn is refused with a reason that names the
  agent and the capability;
- `enabled: true` with `scopes.agents` and/or `scopes.origins` → only the
  listed names, and only the listed destinations, may be targeted. The
  `origins` ruleset is what makes the destination an operator decision: a
  policy that only names agents can be satisfied by a config entry that keeps
  an allowed name and points its URL elsewhere; a policy that pins origins
  cannot.
...
both scopes, and that the gate judges the admitted decision.
...
- sends **everything** only over TLS — the task text as much as any credential,
  so this holds for `none` entries too: an `agent_card_url` that is not
  `https://` refuses to start before any socket is opened. Loopback is not
  exempt by default (a config entry at `http://127.0.0.1:<port>` would reach
  whatever listens on that port of the host); plain HTTP is admitted only for
  the exact loopback origins named in the environment variable
  `KIROCREW_A2A_PLAINTEXT_ORIGINS`, which is how the test fixture and a local
  development shim are reached;
- binds each credential to one destination: `token_env` must be under the
  `KIROCREW_A2A_*` namespace, and its companion `<token_env>_ORIGIN` must name
  the origin (`scheme://host[:port]`) the credential may be sent to. The
  driver refuses at construction, and the provider again at `start()`, unless
  the card URL's origin matches exactly — so a config edit that repoints the
  URL while keeping `token_env` cannot redirect a token it cannot read. A
  configured credential that resolves to nothing is a refusal, never an
...
**Worked example.** Three remotes: a hosted agent with a bearer token, a hosted
agent with no authentication, and a local development shim over plain HTTP.

Gateway environment (`~/.kiro/crew/.env`, read at start; operator-owned):

```bash
# Bearer credential for "incident-triage". Config names only the variable.
KIROCREW_A2A_TRIAGE=<token>
# The one origin that token may be sent to; any other card URL refuses at spawn.
KIROCREW_A2A_TRIAGE_ORIGIN=https://triage-agent.example.com
# Plain HTTP is refused everywhere by default; name the local shim's exact
# loopback origin to allow it (loopback only; other entries are ignored).
KIROCREW_A2A_PLAINTEXT_ORIGINS=http://127.0.0.1:9990
```
...
"a2a_agents": [
  { "name": "incident-triage",
    "agent_card_url": "https://triage-agent.example.com/.well-known/agent-card.json",
    "auth": { "scheme": "bearer", "token_env": "KIROCREW_A2A_TRIAGE" } },
  { "name": "docs-librarian",
    "agent_card_url": "https://docs-agent.example.com/.well-known/agent-card.json",
    "auth": { "scheme": "none" } },
  { "name": "local-kiro",
    "agent_card_url": "http://127.0.0.1:9990/.well-known/agent-card.json",
    "auth": { "scheme": "none" } }
]
```

| entry | needs in the environment | why |
|---|---|---|
| `incident-triage` (bearer) | `KIROCREW_A2A_TRIAGE` **and** `KIROCREW_A2A_TRIAGE_ORIGIN`; either missing refuses | the environment is the only credential source; the pin keeps the token on the operator's host |
| `docs-librarian` (`https`, `none`) | nothing | trusted because it is in config — the same footing as an MCP server |
| `local-kiro` (`http`, `none`) | `KIROCREW_A2A_PLAINTEXT_ORIGINS` naming `http://127.0.0.1:9990` exactly | plaintext is refused unless the operator names the loopback port |

Optional enterprise lock, in the keystone-protected `security_policy.json`
(nothing above depends on it):

```json
"capabilities": { "remote_spawn": { "enabled": true,
                                    "origins": { "allow": ["https://*.example.com"] } } }
```

With that policy a config entry pointing anywhere else is refused at spawn,
...
than being reported gone. Cancelled work is not completed work: a
`TASK_STATE_CANCELED` terminal ends the turn as failed, carrying the partial
text. One `message/stream` call is one task: the first frame naming a task
locks the turn and a frame for any other task fails it, so another task's text
is never recorded as this run's and the stop target never moves; likewise a
continuation that receives a `contextId` other than the one it retained fails
rather than recording another conversation's output. A remote conversation
belongs to the agent that minted its `contextId`: `spawn_continue` inherits the
recorded agent and refuses a different one (`agent_mismatch`), before any side
effect. Inbound size is bounded — 256 KiB for the Agent Card, 4 MiB of text per
turn — and an idle read timeout ends a stream that stops sending. The
wall-clock timeout is the bound on a remote run; the turn budget counts
permission events, which a remote never emits.
...
- Conversation memory across `spawn_continue` is the *server's* policy (the
  ...
  forgot.
- Registry membership is the grant, and the registry is in agent-writable
  `config.json` (see *Trust model*): an agent that can edit config can add an
  unauthenticated HTTPS destination, as it can add an MCP server. The
  environment-side controls keep that edit from carrying a credential or
  travelling unencrypted; the `origins` ruleset is the control an enterprise
  relies on to remove the power altogether.
...
**Unit tests** (`test/test_a2a_provider.py`, 69 on the branch): SSE event
...
transport (TLS-only, named-loopback plaintext, credential-origin pin, session closed on a failed start), one-turn-one-task locking, and continuation staying with the recorded agent.
`TestRemoteSpawnGate` (8, in `test_governance_chokepoints.py`) pins the
governance layering.
