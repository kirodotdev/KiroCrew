...
| [rfc-crew-projects.md](rfc-crew-projects.md) | `draft` | Nothing of the bundle. Re-audited 2026-09-17 at `0640c1e0d5`: no project manifest format, registry or Project store exists on main; `slot.project` is a bare directory path whose `.kiro/skills`, project-local agents and `.kiro/steering` are discovered per session. The P0 implementation is open as PR #7181 and is being split per the document's Revision 2 (thin Project: fenced registry + synced workspace + `memory.mode` + MCP by name) |
