...
| [rfc-a2a-remote-subagents.md](rfc-a2a-remote-subagents.md) | `in-progress` | Nothing on main. Verified at `707b8aef2`: `A2AProvider` and `a2a_agents` each return zero hits under `src/`, `agent.provider` is still `enum=["acp"]` (`config/sections.py`), and every subagent run still reaches its worker through `get_or_create` in `subagent_manager/run.py`. The reference implementation (registry, provider, continuation, 15 tests) lives on the author's fork branch against `efb9fc4ba` and is offered as one PR alongside this document; the `fake_a2a_server` fixture and the `test/gui_user` scenarios are unstarted |
