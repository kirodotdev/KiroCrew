...
|---|---|---|
| [rfc-crew-native-append-only.md](rfc-crew-native-append-only.md) | `partial` | Nothing on main; everything it describes as built rides in this document's own PR at `78064f99c`: the per-member append-only ledger and projections (`src/kiro_crew/eventlog/`), the contribution protocol routes and frames (`dashboard/handlers/eventlog.py`, `dashboard/eventlog_ws.py`, `eventlog/grants.py`), and the plugin importer (`apps/plugin_import.py`, `kirocrew app import`). Not started anywhere: attach and dispatch events, grants derived from them, the multi-source tree view, visibility classes and tombstones, any ledger kind other than member |
