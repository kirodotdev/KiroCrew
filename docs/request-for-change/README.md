...
| [rfc-shared-dependency-cache.md](rfc-shared-dependency-cache.md) | `draft` | Phase 1 ships with its own PR: `pod/provision.py::ensure_venv` prefers `uv` (`_find_uv`, `--link-mode hardlink`, `--project`) and falls back to the unchanged pip path; `KIROCREW_PROVISION_PIP_ONLY` opts out. Phase 2 (`make backend`) and Phase 3 (node `hardlink -t` dedup) are proposals only |
