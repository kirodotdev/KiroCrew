...
was added 2026-08-27. The `rfc-crew-agent-sdk-boundary` row was added 2026-08-28 and verified against `dc88f142b`. The `rfc-transcript-section-markers` row was added 2026-08-30 and verified against `202770d13`. The `rfc-mcp-lifecycle-event-log` row was added 2026-09-01 and verified against `1ee69f225`. The `rfc-session-tag-change-event` row was added 2026-09-02 and verified against `6581a04ee`. The `rfc-agent-config-mirror` row was added 2026-09-02 and verified against `f8b3203c0`. The `rfc-chat-core-extraction` row was added 2026-09-05 and verified against `8ed028b0b`. The `rfc-conductor-work-ledger` row was added 2026-09-05 and verified against `049b8c729`. The `rfc-app-session-controls` row was added 2026-08-31 and re-verified against `1d705a03f` on 2026-09-01. The `rfc-central-governance-ceiling` row was added 2026-09-08 and verified against `8a9c269b4`. The `rfc-security-conductor` row was re-audited 2026-09-10 against `f6d38741c`. The `rfc-overload-resilience` row was added 2026-09-12 and verified against `6f056722b`. The `rfc-append-only-ledger` row was added 2026-09-11 and verified against `2f1e2f54a`, and moved to `in-progress` the same day against the open PR named in it rather than against main. The `rfc-webhook-subscriptions` row was added 2026-09-15 and verified against `6163d9a9ca`. The `rfc-arrival-provenance-filing` row was added 2026-09-20 and verified against `f31f25a8a2`. The `rfc-wake-judge` row was added 2026-09-22 and verified against `80bd0a81f`.

| Document | Status | What is actually on main |
|---|---|---|
| [rfc-wake-judge.md](rfc-wake-judge.md) | `draft` | Nothing of the design. Verified at `80bd0a81f`: the `decisions` seam it builds on is real and shipped — `gate.py` declares `DECISION_POINT_NAMES`, `impl_jev.py` holds `JevOracle` and `_to_wire`, and `points/` carries six live points (`skills_select`, `tool_risk`, `model_route`, `memory_recall`, `message_steer`, `compaction_keep`) — but `points/nudge_wake.py`, `impl_llm.py`, the `nudge_evidence` scope and every `decisions.nudge_wake.*` key — including the `quiet_streak_floor` §5 tunes its compounding bound with — do not exist. `types.py` declares one wire type, `class Choice`, aliased as `Question`, so the `noul` and `score` types §3.1 needs are absent. The tick it hooks is `_monitor_tick_is_quiet` in `autonudge.py`, whose quiet-streak floor `_MAX_QUIET_STREAK` is 10, the value §3.1 defers to, and the typed probe it composes with is `PrWatchProbe` in `probes/gh_pr.py`. The Decisions-card shape §3.6 mounts its row on is not on main either: [#12598](https://github.com/kirodotdev/KiroCrew/pull/12598) is open with auto-merge on and no approving review, pending at `c181a6a7b` |
| [rfc-arrival-provenance-filing.md](rfc-arrival-provenance-filing.md) | `accepted` | Nothing; the document lands first and the code follows in its own PR, because §2 records a changed default and the First Principles lane requires the decision to be on the base branch already. Verified at `f31f25a8a2`: `api_chat_slot_import` in `dashboard/session_transfer.py` composes the imported slot's metadata with no `folder_id` and records that the session "lands unfiled just as the tunnel importer always has"; `dashboard/arrival_folders.py` does not exist; and the only name-matched find-or-create folder path is `ensure_channel_folder` in `dashboard/channel_folders.py`, which this design takes its transaction shape from |
| [rfc-solo-spawn-gate.md](rfc-solo-spawn-gate.md) | `in-progress` | Nothing on main; the whole design is open as [#11710](https://github.com/kirodotdev/KiroCrew/pull/11710). Verified at `3865d68b5`: `src/kiro_crew/solo_spawn.py` does not exist, `spawn_run` / `spawn_sub_agents` carry no `solo_reason` field, and the only gate on a one-task spawn is the advisory `GATE:` paragraph in the tool descriptions pinned by `test/test_spawn_single_task_gate.py` |
...
| [rfc-overload-resilience.md](rfc-overload-resilience.md) | `accepted` | Nothing of the design on main. Verified at `6f056722b`: `src/kiro_crew/taskq/` does not exist, `SpawnGate` / `HostBudget` / `SessionStartGate` have zero hits, `REGISTERED_CAPABILITIES` in `mcp_gateway/gatewayd.py` still carries exactly `ensure_backend`, `bridge_ping`, `poolable_ack`, and `BackendPool.acquire_exclusive` still bypasses `_max_backends`. Owner decided 2026-09-12 that the whole design ships as ONE PR on branch `fix/gatewayd-overload-liveness`, whose first slice (supervisor liveness ping, 600s stub reconnect budget) is already on that branch; §13 records the six decisions and the flag that reverses each |
| [rfc-append-only-ledger.md](rfc-append-only-ledger.md) | `in-progress` | Nothing on main; PR 1 is [#10091](https://github.com/kirodotdev/KiroCrew/pull/10091). That PR is `kiro_crew.crew_log` (the per-unit files for both kinds under a masked `crew-log/` root, an envelope carrying `seq`/`thread`/`ref`, the headers, type ownership, guest namespaces, paging, `ok`/`gone` resolution, torn-tail repair with deterministic closers) with the flag-gated session emitter as its first writer; that PR carries its own module specs. Verified at `2f1e2f54a`, before it: `members.py` appends `{ts, member, session}` entries carrying neither `type` nor `seq`, and `work_ledger.py` (`items/<id>.json`) and `session_ledger.py` (`state.json`) each keep a separate head. See the RFC's `Amendment 2026-09-16`: the vocabulary is nine types shorter, the parallel lifecycle-event package is retired, and the format is pre-release rather than frozen. PR 2 — projections, routes, UI, the crew writer and migration — is unstarted |
...
| [rfc-durable-run-coordinator.md](rfc-durable-run-coordinator.md) | `superseded` | Replaced by [rfc-overload-resilience.md](rfc-overload-resilience.md) (its §13 decision Q1): the `runs` / `commands` / `outbox` store proposed here is delivered as tables of `$KIROCREW_HOME/tasks/tasks.db` (`src/kiro_crew/taskq/`), with the outbox as `task_events(kind="deliver")`, on branch `fix/gatewayd-overload-liveness`. The in-memory manager and run folders this document describes are imported into that store by `taskq.migrate.import_legacy` on first boot |
...
that owns the shipped behaviour. A `superseded` row names its replacement — the
