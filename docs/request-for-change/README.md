...
| [rfc-transcript-section-markers.md](rfc-transcript-section-markers.md) | `in-progress` | Phase 1 ships with [#7163](https://github.com/kirodotdev/KiroCrew/pull/7163): the `section_marker` role, its MCP tool and session directive, a turn-boundary append reusing `/note`'s hold, and a labelled-rule renderer in both transcript paths. No collapse behaviour — every collapse affordance on main is still intra-turn (`CollapsibleToolGroup`, `TurnBlock`, `ToolCallLine`) and nothing collapses rows above a point. Phases 2–4 unstarted |
