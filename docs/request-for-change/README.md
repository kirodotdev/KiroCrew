...
| [rfc-app-session-controls.md](rfc-app-session-controls.md) | `in-progress` | Nothing. Verified at `1d705a03f`: `UIConfig` declares `entry`/`pages`/`overlays`/`sidebar` and no session-scoped surface, and the app SDK's public files carry zero `sessionKey`/`session_key`/`activeSession`/`currentSession` hits — so no app can discover the chat it is rendered beside. Implementation (manifest field + composer host + status polling) exists only on an active branch; S1 and S2 are deliberately not separable |
