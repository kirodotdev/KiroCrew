...
| [rfc-crew-work-migration.md](rfc-crew-work-migration.md) | `draft` | Nothing. Re-verified at `992498c`: `git ls-tree -r --name-only main \| grep -c 'kiro_crew/migration'` returns `0`. `session_transfer.py` still documents "Copy, never move", and no code path releases ownership of a cron job, session, or task run |
