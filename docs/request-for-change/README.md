...
|---|---|---|
| [rfc-proactive-slack-egress-consolidation.md](rfc-proactive-slack-egress-consolidation.md) | `draft` | The three tiers it consolidates, and none of the consolidation. Verified at `6d1b51704`. The tier membership itself is enumerated in the RFC and nowhere else in prose; it is pinned mechanically by a census test, and each plain-client site carries a one-line marker naming its own tier. No shared per-chunk revalidation driver exists: `deliver_to_channel` and `_deliver_slack_governed` each re-ask by hand |
