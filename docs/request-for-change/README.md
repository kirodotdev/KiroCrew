...
| [rfc-a2a-remote-subagents.md](rfc-a2a-remote-subagents.md) | `draft` | Nothing on main. Verified at `707b8aef2`: `A2AProvider` and `a2a_agents` each return zero hits under `src/`, `agent.provider` is still `enum=["acp"]` (`config/sections.py`), and every subagent run still reaches its worker through `get_or_create` in `subagent_manager/run.py`. The reference implementation (registry, provider, continuation, fake A2A fixture, `test/gui_user` scenarios) is open as [#10171](https://github.com/kirodotdev/KiroCrew/pull/10171), a fork PR not intended to merge before this RFC is accepted |
...
| [rfc-agent-self-tagging.md](rfc-agent-self-tagging.md) | `in-progress` | Nothing on main; implementation is open PR [#7779](https://github.com/kirodotdev/KiroCrew/pull/7779). On main there is no `chat_tag` MCP directive (`chat_tag` has zero hits in `mcp_tools/control.py`), no protected grants store (`dashboard/chat_tag_grants.py` does not exist), and no `[BOARD]` context line; `dashboard/chat_tags.py` writes are human-only (`api_chat_slot_tags`, `api_chat_slot_drop`) with no agent tag read or write path |
