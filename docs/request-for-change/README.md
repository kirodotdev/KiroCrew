...
| [rfc-chat-core-extraction.md](rfc-chat-core-extraction.md) | `partial` | P1 shipped: `app-sdk/messageRenderers.tsx` is the role registry with a parity contract test ([#5128](https://github.com/kirodotdev/KiroCrew/pull/5128)). P2 shipped for one surface: `website/src/chat-core/transport/sendTurn.ts` exists and `components/ChatPane.tsx` is its only production caller ([#5909](https://github.com/kirodotdev/KiroCrew/pull/5909)); ChatEmbed, SideChat and ChatPage still hand-roll their POST on main (open PRs [#8599](https://github.com/kirodotdev/KiroCrew/pull/8599), [#8655](https://github.com/kirodotdev/KiroCrew/pull/8655), [#8689](https://github.com/kirodotdev/KiroCrew/pull/8689)). P3 shipped for SideChat (`pages/chat/SideChat.tsx` mounts `ChatInput`); `app-sdk/ChatEmbed.tsx` renders its own one-row `<textarea>` on `useComposerDraft`'s auto-grow ref rather than mounting `ChatInput`. Verified at `8ed028b0b`: `ChatPage.tsx` carries 29 `role ===` branches (P5 unstarted) |
