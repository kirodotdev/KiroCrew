...
| Task-run resume from persisted state | **Exists**, at reduced fidelity | `src/kiro_crew/task_reporter.py` `load_checkpoint` and `build_resume_context`; `src/kiro_crew/taskrunner.py` `_RUNS_FILE = "runs.json"` and `_load_runs`. `runs.json` omits `WorkingMemory` and `current_task` — see §2.2 |
...
`src/kiro_crew/dashboard/session_transfer.py`: *"**Copy, never move.** Import
...
**Task-run resume is more feasible than assumed.** `task_models.py` `Project`
...
matters most for `CronJob` (`src/kiro_crew/cron.py`), whose own comments
already mark four fields as *"Runtime-only (never serialized)"*. An
exclude-list would ship the fifth such
