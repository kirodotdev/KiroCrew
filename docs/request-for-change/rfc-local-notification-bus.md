...
1. **Slack escalation for critical notifications: yes, opt-in per channel, Phase 5.** When no dashboard client is connected, a critical notification forwards as a Slack DM through the existing `send_message` path. Default off per channel -- an always-on escalation would double-deliver every critical notification while the dashboard is open and erode trust in both surfaces. Deferred to Phase 5 because disconnect detection needs tuning against real usage.
