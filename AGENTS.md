...
| Product name | The product is **Kiro Crew** — two words, a space, capital `K`. Identifiers keep the spelling their own system gave them: the `kirodotdev/KiroCrew` repo slug, `KiroCrew.dmg`-style artifacts, the channel-qualified `KiroCrew Nightly` OS identifier, the `kirocrew` CLI, `KIROCREW_*` env vars, `kiro_crew` imports. CI-gated on added lines by `scripts/check_brand_name.py`; run `BRAND_BASE_REF=origin/main python3 scripts/check_brand_name.py` before pushing. |
