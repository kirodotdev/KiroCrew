 ...
 *
 * `missing` previously dropped the trailing "Add it to .../.kiro/agents/ or pick
 * another agent." clause -- gateway.py's own docstring calls out that this
 * clause is the whole point of naming the directory (it names the NEXT STEP,
 * not just the diagnosis), so a fixture missing it was showing evidence for a
 * message the shipped code does not produce. `shadowed` carries the plain-fact
 * opening UX Review asked that branch to lead with, not the internal vocabulary
 * it replaces -- a fixture asserting retired copy is the same defect in the
 * other direction.
 */
const REASONS: Record<string, string> = {
  shadowed:
    "This job is bound to a Crew Member, and its project directory "
    + "defines an agent with the same name ('default'). The run was "
    + "skipped instead of letting that agent run with the member's own "
    + "memory. Rename the project's agent or unbind the member.",
  missing:
    "Agent 'release-checklist' not found in this job's project directory. Add "
    + "it under that directory's .kiro/agents/ or pick another agent.",
}
...
window.fetch = (async (input: RequestInfo | URL, init?: RequestInit) => {
  ...
  // JobForm (rendered inside the detail dialog) reads the sidebar's chat-folder
  // list through the shared ['chat-folders'] query. The catch-all below answers
  // `EMPTY` ({jobs: []}), which has no array shape at all, so the ordering
  // helper's `.map` throws inside JobForm and the whole dialog never renders --
  // not just this frame, every scene's dialog. Same trap as
  // cron-project-directory.tsx's `/api/chat/folders` stub.
  if (url.includes('/api/chat/folders')) return json([])
