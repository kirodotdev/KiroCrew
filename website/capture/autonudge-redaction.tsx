/**
 * Verification entry for the auto-nudge REDACTED-PROJECTION surface.
 *
 * Mounts the REAL `AutoNudgePopover` on a loop whose served `message` is a
 * FABRICATED masked goal, so the three states this PR adds can be photographed:
 * the notice explaining why the text reads `[REDACTED: ...]`, the
 * overwrite-confirm row that arms when the user edits that masked text, and the
 * notice shown when a save kept the stored goal.
 *
 * The fixture is invented on purpose: these frames are permanent and
 * outward-facing, and credential-shaped text is this PR's whole subject, so the
 * harness must never be seeded from a real loop store.
 *
 * `?theme=dark|light` selects the palette. The driver asserts each state first.
 */
import { QueryClient, QueryClientProvider } from '@tanstack/react-query'
import { useEffect, useState } from 'react'
import { createRoot } from 'react-dom/client'

import AutoNudgePopover from '../src/components/AutoNudgePopover'
import { initI18n } from '../src/i18n/all'
import '../src/index.css'

const params = new URLSearchParams(location.search)
const theme = params.get('theme') === 'light' ? 'kiro-light' : 'kiro-dark'

document.documentElement.setAttribute('data-theme', theme)
initI18n('en')

/**
 * A masked goal, exactly as the backend's egress scrub would serve it: the
 * operator's own words with the credential-shaped run replaced in place. The
 * key id here is the documented AWS example value, not a real credential.
 */
const REDACTED_GOAL = 'deploy the release with [REDACTED: aws-access-key-id] then post the summary'

const loop = {
  id: 'loop-capture-1',
  slot_key: 'chat-1-123',
  message: REDACTED_GOAL,
  message_redacted: true,
  idle_secs: 300,
  max_cycles: 0,
  cycle_count: 4,
  // `?active=0` photographs the PAUSED arm, whose keep-stored button says "and start
  // loop" -- a different sentence from the active arm's, and one no frame showed.
  active: params.get('active') !== '0',
  // `?parked=1` photographs the restart-parked state: the header chip and the reason the
  // Members patrol row renders from the same `stopped_reason`.
  ...(params.get('parked') === '1'
    ? { active: false, stopped_reason: 'interrupted_cycle' }
    : {}),
  last_fire_ts: 0,
}

const qc = new QueryClient({ defaultOptions: { queries: { retry: false } } })

/**
 * Holds the served loop in state so the driver can move it MID-EDIT.
 *
 * The moved-goal arm needs `loop.message` to change while a local edit is in
 * progress, which a module-level const cannot express. The driver calls
 * `__captureSetGoal` between typing and saving, so the timing is explicit rather
 * than raced against a timer.
 */
function Harness({ width }: { width: number }) {
  const [served, setServed] = useState(loop)
  useEffect(() => {
    ;(window as unknown as Record<string, unknown>).__captureSetGoal = (next: string) =>
      setServed(prev => ({ ...prev, message: next, message_redacted: false }))
  }, [])
  return (
    // Inline width: capture/ sits outside Tailwind's content globs, so an
    // arbitrary-value class written here would never compile.
    <div data-capture-root className="bg-bg text-text p-6" style={{ width, minHeight: 560 }}>
      <AutoNudgePopover
        open
        onOpenChange={() => {}}
        slotKey="chat-1-123"
        loop={served as never}
        onChange={() => {}}
      />
    </div>
  )
}

createRoot(document.getElementById('root')!).render(
  <QueryClientProvider client={qc}>
    <Harness width={Number(params.get('w')) || 520} />
  </QueryClientProvider>,
)
