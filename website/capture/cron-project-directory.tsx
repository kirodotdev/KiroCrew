...
window.fetch = ((input: RequestInfo | URL, init?: RequestInit) => {
  ...
  if (url.startsWith('/api/agents')) {
    ...
    // Only the demo checkout declares agents. Any OTHER bound directory answers
    // the global roster, which is what makes a project-only pick stop resolving
    // when the directory moves -- the reset notice has nothing to fire on
    // otherwise, because the same rows would come back for every path.
    const declares = bound && url.includes(encodeURIComponent(PROJECT))
    return json({ agents: declares ? PROJECT_ROWS : GLOBAL_ROWS, default_agent: 'default' })
  }
  ...
  // The sidebar's folder list, which the form's own folder picker reads through
  // the shared ['chat-folders'] query. The catch-all below answers `{}`, and the
  // ordering helper calls `.map` on it, so leaving this to the catch-all throws
  // inside JobForm and no frame renders at all.
  if (url.startsWith('/api/chat/folders')) return json([])
