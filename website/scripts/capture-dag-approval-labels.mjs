 ...
 *     Approve / Deny buttons, the warn halo AND the "Needs Approval" word; the
 *     pending gated node reads "Approval gate", has no halo and no buttons; the
 *     legend names both states.
 *   --before: the pre-change checkout. The buttons and halo sit on the
 *     in_progress node, but that node reads "in progress" while the pending
 *     gated node is the one that says "needs approval".
...
for (const theme of ['dark', 'light']) {
  await page.goto(`${BASE}/capture/dag-approval-labels.html?theme=${theme}`)
  ...
  const halo = async title => card(title).locator('rect[stroke="var(--warn)"][stroke-width="3"]').count()
  ...
  const state = {
    implement: { word: await word('Implement'), buttons: await buttons('Implement'), halo: await halo('Implement') },
    deploy: { word: await word('Deploy'), buttons: await buttons('Deploy'), halo: await halo('Deploy') },
  ...
  const ok = dialogs === 0 && (await cards.count()) === 4 &&
    ...
    (BEFORE
      ...
      : state.implement.word === 'Needs Approval' && state.deploy.word === 'Approval gate' &&
        state.plan === 'Done' && state.announce === 'Pending' && /Needs Approval/.test(legend) && /Approval gate/.test(legend))
