 ...
 * drill-in's two empty states) and the morning case (sessions live, nothing
 * folded into memory yet), land in temp-screenshots/overview-today-card/
...
// Three data shapes: the busy day the feature exists for, a quiet day
// (nothing yet -- the drill-in shows both empty states and the card its zero
// line), and the by-design morning case (sessions running, nothing folded
// into memory yet because no session has idled the configured hours).
const SCENARIOS = {
...
async function main() {
  ...
  console.log('OK — 7 screenshots in', OUT)
}
