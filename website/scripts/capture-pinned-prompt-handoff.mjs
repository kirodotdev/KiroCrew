...
const VIDEO_DIR = join(OUT, `${LABEL}-${THEME}-video`)
// Second gate on the destructive step. `safeName` already makes a traversal
// unrepresentable, but the recursive delete below is the one irreversible thing
// this script does, so it is checked against the resolved path rather than trusted
// to the name rule alone.
{
  const outAbs = resolve(OUT)
  const dirAbs = resolve(VIDEO_DIR)
  if (dirAbs !== join(outAbs, `${LABEL}-${THEME}-video`) || !dirAbs.startsWith(outAbs + sep)) {
    console.error(`FAIL: video dir escaped the output directory: ${dirAbs} not inside ${outAbs}`)
    process.exit(2)
  }
}
