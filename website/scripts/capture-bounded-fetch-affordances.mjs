...
async function main() {
  ...
  // Fork + plan, both kept in place and disabled with the reason on their tooltip.
  ...
  for (let i = 0; i < n; i++) {
    ...
    // The reason is a STATE, not an imperative the click refuses to honour -- and it is
    // VISIBLE text referenced by aria-describedby, so a keyboard user reads it too.
    ...
    assert(`action ${i} shows the state reason`, /unavailable/i.test(reasonText))
