...
// One record, one age: the theme switch re-minted the SAME record, so the bar's
// chip keeps the plain "Published" label and agrees with the band's age. UX
// review's third round saw "New version published 3d ago" above "Showing the
// version from 3d ago" here and asked which copy was on screen; the "new
// version" label now fires only when the record's publish instant is later than
// the shown document's, a state the unit tests reach by republishing. Resolved
// through the collapse button's row so the docked card's chip is not the one read.
const barChip = collapse.locator('xpath=..').locator('[data-testid="crew-webview-age"]')
check('the bar chip is present in the failed-refresh state', (await barChip.count()) === 1)
const chipText = (await barChip.textContent()) || ''
check('the bar chip keeps the plain label for a same-record re-mint', chipText.startsWith('Published '))
check('the bar chip does not claim a new version exists', !chipText.startsWith('New version'))
