 ...
 * Frame 06b: the same root field in the FLAT LIST view — the page places it,
 *           not the tree, so it is there when there is no tree at all.
 ...
 * Frame 08b: the rename field previews a cleaned name (`Inbox` → `2026/Q1`)
 *           before Enter, like the folder field does.
...
async function main() {
  ...
  try {
    ...
    // 06b — the root field in list view: placed by the page, not the tree.
    await rootField.press('Escape')
    await page.getByRole('button', { name: 'Sort and view' }).click()
    await page.getByRole('button', { name: 'Flat list' }).click()
    await page.getByRole('button', { name: 'Inbox', exact: true }).waitFor({ timeout: 5000 })
    await page.getByRole('button', { name: 'New note or folder' }).click()
    await page.getByRole('button', { name: 'New folder at the top level' }).click()
    const listRootField = page.getByRole('textbox', { name: 'Folder name' })
    await listRootField.waitFor({ timeout: 5000 })
    await listRootField.fill('Archive')
    await page.waitForTimeout(200)
    await page.screenshot({ path: `${OUT}/06b-root-field-list-view.png`, clip: await railClip(page) })
    console.log('wrote', `${OUT}/06b-root-field-list-view.png`)
    await listRootField.press('Escape')
    await page.getByRole('button', { name: 'Sort and view' }).click()
    await page.getByRole('button', { name: 'Folders', exact: true }).click()
    await page.getByRole('button', { name: PARENT, exact: true }).waitFor({ timeout: 5000 })
    await page.getByRole('button', { name: 'New note or folder' }).click()
    await page.getByRole('button', { name: 'New folder at the top level' }).click()
    await rootField.waitFor({ timeout: 5000 })
    ...
    await page.getByText("This name can't sync to a Windows clone (CON, NUL, LPT1…)").waitFor({ timeout: 5000 })
    ...
    // 08b — the rename field previews a cleaned name before Enter.
    await renameField.fill('2026/Q1')
    await page.getByText('Will be renamed to “2026Q1”').waitFor({ timeout: 5000 })
    await page.waitForTimeout(200)
    await page.screenshot({ path: `${OUT}/08b-rename-cleaned-preview.png`, clip: await railClip(page) })
    console.log('wrote', `${OUT}/08b-rename-cleaned-preview.png`)
