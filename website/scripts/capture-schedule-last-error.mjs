 ...
 *                                  directory declares its agent: states the
 *                                  collision in plain words, then both remedies
...
const SHADOW_FRAGMENTS = [
  'This job is bound to a Crew Member',
  'the same name',
  "Rename the project's agent or unbind the member",
]
/** The internal vocabulary UX Review had this refusal open on, which the shipped
 *  sentence no longer uses anywhere -- a frame carrying it is stale evidence. */
const SHADOW_RETIRED_FRAGMENTS = ['would shadow', 'private memory']
/** The directory is named POSSESSIVELY, not by path: the absolute folder was
 *  stripped from every skip reason because `last_error` reaches a non-owner. */
const MISSING_FRAGMENT = "not found in this job's project directory"
/** The clause a prior fixture dropped -- the NEXT STEP, not just the diagnosis. */
const MISSING_REMEDY_FRAGMENT = "Add it under that directory's .kiro/agents/ or pick another agent."
...
await scene('shadowed', '09-last-error-member-shadowed', {
  assert: (text) => {
    ...
    // The second defect: opening on internal vocabulary rather than the state.
    for (const f of SHADOW_RETIRED_FRAGMENTS) {
      if (text.includes(f)) {
        throw new Error(`frame carries retired copy ${JSON.stringify(f)}`)
      }
    }
  },
})
...
await scene('missing', '10-last-error-agent-missing', {
  assert: (text) => {
    ...
    // The defect this frame is evidence against: a fixture that claimed to be
    // verbatim but silently dropped the "Add it to .../.kiro/agents/" clause --
    // the diagnosis without the remedy, which gateway.py's own docstring says
    // is the entire reason the directory is named at all.
    if (!text.includes(MISSING_REMEDY_FRAGMENT)) {
      throw new Error(`missing shipped remedy clause ${JSON.stringify(MISSING_REMEDY_FRAGMENT)}`)
    }
  },
})
