...
export async function openAgentsPage({ crews }) {
  ...
  try {
    ...
    const ctx = await browser.newContext({ locale: 'en-US', viewport: { width: 1440, height: 900 }, baseURL: base })
    ...
    await stubDashboardApi(page, {
      extra: async (path, route) => {
        ...
        if (path === '/api/agents/installed') { await json(route, INSTALLED_FIXTURE); return true }
