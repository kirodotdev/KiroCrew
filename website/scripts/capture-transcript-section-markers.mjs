...
const run = async () => {
  // A version-managed Node (mise, asdf, nvm) puts its own lib dir on LD_LIBRARY_PATH, and
  // the browser inherits it and loads that older libstdc++ instead of the system one.
  const browserEnv = { ...process.env }
  delete browserEnv.LD_LIBRARY_PATH
  // Headless rasterization falls back to software GL, so name it rather than letting the
  // shell probe a GPU it has no access to and fail the capture at screenshot time.
