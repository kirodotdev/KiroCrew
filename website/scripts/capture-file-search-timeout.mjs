...
async function captureArm(context, base, mode, expected, shot, opts = {}) {
  ...
  // Seeds ride INSIDE the stub's init script, which applies them after its own
  // localStorage.clear(); a separately registered addInitScript races that clear.
  const localStorageEntries = { 'mc-active-slot': SLOT }
  if (opts.ctrlEnter) {
    ...
    localStorageEntries['mc-chat-config'] = JSON.stringify({ sendOnEnter: 'ctrl-enter' })
  }

  await stubDashboardApi(page, { slots, theme: 'dark', extra, localStorageEntries })
...
async function captureRailArm(context, base, expected, shot) {
  ...
  await stubDashboardApi(page, {
    ...
    localStorageEntries: {
      ...
      'mc-files-rail-open': '1',
      ['mc-panel-tabs:' + SLOT]: JSON.stringify({
        activeId: 'file:' + FILE,
        tabs: [{ id: 'file:' + FILE, kind: 'file', title: 'index.ts', path: FILE, slot: SLOT }],
      }),
    },
  })
  await page.goto(base + '/', { waitUntil: 'domcontentloaded' })
...
async function captureFolderPanelArm(context, base, mode, expected, shot) {
  ...
  await stubDashboardApi(page, {
    ...
    localStorageEntries: {
      ...
      ['mc-panel-tabs:' + SLOT]: JSON.stringify({
        activeId: 'folder:' + DIR,
        tabs: [{ id: 'folder:' + DIR, kind: 'folder', title: 'src', path: DIR, slot: SLOT }],
      }),
    },
  })
  await page.goto(base + '/', { waitUntil: 'domcontentloaded' })
