...
async function capturePickerArm(context, base, which, expected, shot, opts = {}) {
  ...
  // Same shape as the @-menu's denied arm, and the same caveat: no `X-Auth-Required`, or the
  // client reads a session expiry and renders the generic copy over a 403 that looks right.
  ...
  const extra = async (path, route) => {
    if (path === '/api/recent-projects') {
      ...
      // Loading… state: every settled arm has already left it by the time it paints. Waited
      // out, the same open request is what lets the CLIENT deadline fire for the timed-out arm.
    ...
    if (path === '/api/browse-dirs') {
      ...
      // 'listing-denied' is the coded refusal: the one listing arm that names its cause AND
      // withholds Retry, since re-asking a refusal returns the same answer.
      if (which === 'listing-denied') { await deny(route); return true }
  ...
  // notice is not mounted at all -- the listing arms have to switch to it to be photographed.
  if (opened && (which === 'listing' || which === 'listing-hang' || which === 'listing-denied')) {
 ...
 */
async function captureFolderPanelArm(context, base, mode, expected, shot, opts = {}) {
  const page = await context.newPage()
  logPageProblems(page)
  ...
  const found = await page.locator(`text=${expected}`).first()
    .waitFor({ timeout: 20000 }).then(() => true).catch(() => false)
  await page.waitForTimeout(400)
  // A string that must NOT share the frame with `expected`. Counted after `expected` landed and
  // the listing settled, so an absence is the panel's decision rather than a pending read.
  const stacked = opts.absent ? await page.locator(`text="${opts.absent}"`).count() : 0
  await page.screenshot({ path: `${OUT}/${shot}.png` })
  console.log('wrote', `${OUT}/${shot}.png`, { mode, found, ...(opts.absent ? { stacked } : {}) })
  await page.close()
  return found && stacked === 0
}
...
async function main() {
  ...
  // The tree arm's listing stub is EMPTY on purpose: with the tree notice up, the panel must not
  // also say "Empty folder" -- the listing is a fallback for the tree, not a second finding.
  const fpTreeError = await captureFolderPanelArm(
    context, base, 'tree-hang', "Couldn't load the file tree — Refresh to retry", '20-panel-tree-error',
    { absent: 'Empty folder' })
  ...
  // -- suffixed the same way too, since the rail's header Refresh refetches that very read.
  const railTree = await captureRailArm(
    context, base, "Couldn't load the file tree — Refresh to retry", '16-rail-tree-timed-out')
  ...
  // The recents TIMED-OUT arm, which ux-review found in no frame: the same hanging route as
  // the Loading… arm, but waited out, so the production BROWSE_FILES_TIMEOUT_MS deadline is
  // what fires and the cause-keyed timeout wording renders rather than the generic failure.
  // The listing beneath it succeeds, so the frame carries exactly the one recents Retry.
  const pickerRecentTimeout = await capturePickerArm(
    context, base, 'recent-hang', 'Loading recent projects timed out', '23-picker-recents-timed-out',
    { expectRetries: 1 })
  // The listing's coded REFUSAL, which ux-review found in no frame: the arm a typed inaccessible
  // path lands in. It renders the cause-specific denied copy, not the generic failure -- and
  // NO Retry, because re-asking a refusal returns the same answer. The recents beneath succeed,
  // so `expectRetries: 0` asserts the withholding rather than a control below the fold.
  const pickerListingDenied = await capturePickerArm(
    context, base, 'listing-denied', 'No access to this folder', '24-picker-listing-denied',
    { expectRetries: 0 })

  await browser.close()
  srv.close()

  console.log({ timedOut, failed, empty, denied, notFound, pickerRecent, pickerListing, pickerBoth, pickerLoading, pickerRecentTimeout, pickerListingDenied })
  if (!timedOut || !failed || !empty || !denied || !notFound
      || !pickerRecent || !pickerListing || !pickerBoth || !pickerLoading || !pickerRecentTimeout
    || !pickerListingDenied
