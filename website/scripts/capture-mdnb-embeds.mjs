 ...
 *
 * The feature resolves an embed the way Obsidian does by default — through the
 * vault's `attachmentFolderPath` setting, then the note's folder, then the vault
 * root — while a standard `![alt](src)` image keeps resolving against the note.
 * Two frames, one claim each:
 *
 ...
 *   02 fallback  - an embed whose file is gone shows the file name with the
 *                  broken-image glyph rather than a broken frame; a note
 ...
 * and the vault's attachment folder setting through
...
/** An Obsidian vault whose images live in `z-assets/`: the setting the attachments route reports. */
...
Both keep the note readable instead of showing a broken frame.
`
...
/** The attachment folder setting, answered before the shared stub's catch-all reaches it. */
async function attachments(path, route) {
  if (!path.startsWith('/apps/md-notebook/api/attachments')) return false
  json(route, { attachmentFolderPath: ATTACHMENT_FOLDER })
  return true
}

/**
 * Grow the viewport until the note pane needs no scrollbar, so the clipped frame
 * shows the WHOLE note. `notePaneClip` clips to the pane's on-screen box, and the
 * pane is the scroll container: at 900px the third embed (the `|360` one) sat
 * below the fold and the frame stopped being evidence of the width suffix.
 */
async function fitNotePane(page) {
  const needed = await page.evaluate(() => {
    ...
    if (!el || el === document.body) return window.innerHeight
    return Math.ceil(el.getBoundingClientRect().top + el.scrollHeight)
  })
  await page.setViewportSize({ width: 1280, height: Math.min(Math.max(900, needed + 24), 4000) })
...
async function shoot(browser, base, doc, { file, fallback = false }) {
  ...
  if (fallback) {
    // The embed resolves to a file the endpoint 404s, so the frame is evidence
    // once the file name stands in for the image and no note image is left in
    // the DOM. A miss carries no tooltip: the name is all there is to say.
    ...
    if (await page.getByText('deleted-diagram.svg', { exact: true }).getAttribute('title'))
      throw new Error('plain miss carries a tooltip')
...
async function main() {
  ...
  try {
    ...
    await shoot(browser, base, mdnbNoteDoc(NOTE_PATH, FALLBACK_NOTE), {
      file: '02-fallbacks-missing-transclusion.png',
      fallback: true,
    })
  } finally {
    await browser.close()
