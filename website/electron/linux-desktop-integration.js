...
function integrateLinuxDesktop(deps) {
  ...
  // Deferred reconciliation does not prove the canonical launcher exists or is current.
  if (launcherResult.status === "deferred") {
    note(`launcher at ${launcherPath} reconciliation deferred; retrying on a later launch`);
    return { skipped: true, reason: "launcher deferred" };
  }
