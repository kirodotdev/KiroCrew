...
4. One entry in `_RESTORABLE_UI_LANGUAGES` (`src/kiro_crew/context.py`), so the
   agent is told about the language too.

The fourth edit is the odd one out and worth the sentence it costs. Everything
the SPA needs is data; the backend needs the set as well because it injects the
configured tag into the agent's context, and shape alone is not enough to decide
whether a stored tag names a language this install can render. Forwarding an
unrenderable tag produced English chrome around foreign-language tool-call
purposes, persisted in session history (#1130). Leaving the backend ignorant of a
newly shipped language is the same defect pointing the other way: the dashboard
renders it while purposes revert to the conversation's language.
`test/test_ui_language_catalog_parity.py` parses `SUPPORTED_LANGUAGES` out of this
tree and fails naming the missing side, so a half-added language cannot ship
quietly — but CI naming the gap is a slower teacher than this list being right.
