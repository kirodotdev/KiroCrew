...
Split view, Bottom panel, then Side panel. The session-window pair and the
workspace-visibility pair are separate regions, divided by a hairline and an 8px
...
focuses that window and toggles nothing here) is the pointer entry point to the
docked terminal; the rebindable `terminal` panel chord is the keyboard one. It sits next
to the Side-panel toggle because the two panels share the workspace edge and are
opened and closed together. The nav rail carries no Terminal row: the rail lists
routes, and a row that toggled a panel instead of navigating had to hardcode its own
lit state because it had no route to derive one from. Putting the control on the
workspace edge it opens leaves one pointer entry rather than two that can disagree.
