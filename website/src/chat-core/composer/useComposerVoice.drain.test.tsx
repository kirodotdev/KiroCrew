...
/** A composer wired the way the real hosts are: the setter the hook is given IS the
 *  host's change handler, and that handler reports every edit back to the hook. Both
 *  production hosts do exactly this, so a dictation write and a keystroke arrive at
 *  the same function -- which is the condition the self-write depth exists for. */
function mountWithReportingHost(caretRef?: { current: { start: number; end: number } | null }) {
  const inputRef = { current: '' }
  const slot = { current: SESSION }
  const api: { current: ReturnType<typeof useComposerVoice> | null } = { current: null }
  fx.engine.streamEnabled = true
  const write = (v: string) => {
    const before = inputRef.current
    inputRef.current = v
    api.current?.noteComposerEdit(before, v)
  }
  const hook = renderHook(
    () => useComposerVoice({ sessionId: slot.current, inputRef, setInput: write, caretRef }),
    { wrapper: makeWrapper(STT_STREAMING) },
  )
  api.current = hook.result.current
  /** What the user typing does: the host's own handler, same as the hook's writes. */
  const typeInComposer = (v: string) => { act(() => { write(v) }) }
  return { hook, inputRef, typeInComposer }
}
...
describe('useComposerVoice — a reported edit settles what the values cannot', () => {
  /** Dictate `ship` into `please ` through a host that reports its edits, and release. */
  function dictateThenRelease() {
    const h = mountWithReportingHost()
    h.inputRef.current = 'please '
    fx.engine.recording = true
    fx.engine.sessionOwner = SESSION
    act(() => { h.hook.rerender() })
    act(() => { void h.hook.result.current.startVoice() })
    fx.engine.partial = 'ship'
    act(() => { fx.captured.onPartial?.('ship', SESSION) })
    enterDrain(h.hook)
    fx.engine.partial = 'ship'
    expect(h.inputRef.current).toBe('please ship')
    return h
  }

  it('leaves the draft alone once the user has retyped the run at its own offset', () => {
    // The shape no pair of values can resolve: delete the dictated word, type it again
    // byte for byte in the same place, and the composer reads exactly as the write left
    // it. The locator therefore places the run and would remove characters the user
    // authored, through a programmatic write the browser's undo does not reverse. The
    // deletion was reported when it happened, so the answer is known rather than
    // inferred.
    const { hook, inputRef, typeInComposer } = dictateThenRelease()
    typeInComposer('please ')
    typeInComposer('please ship')
    act(() => { hook.result.current.cancelVoice() })
    expect(inputRef.current).toBe('please ship')
    // The engine is still released -- the microphone is not held hostage to the refusal.
    expect(fx.engine.cancel).toHaveBeenCalledTimes(1)
  })

  it('still takes the run back when the reported edits never reached it', () => {
    // The ordinary thing to do during a minutes-long wait. Typing in front of the run
    // leaves its characters untouched, so provenance is intact and the discard acts.
    const { hook, inputRef, typeInComposer } = dictateThenRelease()
    typeInComposer('PLEASE please ship')
    act(() => { hook.result.current.cancelVoice() })
    expect(inputRef.current).toBe('PLEASE please ')
  })

  it('does not read its own correction as the user reaching into the run', () => {
    // The hook writes through the host's change handler -- the same function the user's
    // typing arrives at -- so a correction replacing the run would otherwise report
    // itself as a touch and cost every corrected dictation its discard.
    const { hook, inputRef } = dictateThenRelease()
    act(() => { fx.captured.onPartial?.('ship it', SESSION) })
    expect(inputRef.current).toBe('please ship it')
    act(() => { hook.result.current.cancelVoice() })
    expect(inputRef.current).toBe('please ')
  })

  it('does not read a correction that REWRITES the run as the user reaching into it', () => {
    // The case above only appends: an insertion at the run's far edge leaves the run's
    // own characters alone, so the touch test answers no whether or not the hook's write
    // is skipped. A correction that changes what the run SAYS replaces characters inside
    // it, which is the one shape the self-write depth decides -- reported, it would look
    // exactly like the user overtyping the dictation.
    const { hook, inputRef } = dictateThenRelease()
    act(() => { fx.captured.onPartial?.('sail', SESSION) })
    expect(inputRef.current).toBe('please sail')
    act(() => { hook.result.current.cancelVoice() })
    expect(inputRef.current).toBe('please ')
  })

  it('keeps the refusal once touched, across the corrections that follow', () => {
    // The touch here lands while capture is still LIVE, which is the reachable order:
    // after the release a user edit stops later partials from landing at all, so the
    // question only arises when corrections are still arriving. A correction rebuilds
    // the region from the snapshot, so the characters there are the machine's again --
    // but the value alone cannot show that, and the sibling path splices ahead of the
    // live tail and KEEPS what the user typed. Carrying the touch forward costs a
    // residue the user can see and delete; dropping it risks taking characters that
    // were theirs, so the utterance keeps the answer.
    const h = mountWithReportingHost()
    h.inputRef.current = 'please '
    fx.engine.recording = true
    fx.engine.sessionOwner = SESSION
    ...
    act(() => { fx.captured.onPartial?.('ship', SESSION) })
    expect(h.inputRef.current).toBe('please ship')
    h.typeInComposer('please shXip')
    fx.engine.partial = 'ship it'
    act(() => { fx.captured.onPartial?.('ship it', SESSION) })
    expect(h.inputRef.current).toBe('please ship it')
    enterDrain(h.hook)
    act(() => { h.hook.result.current.cancelVoice() })
    expect(h.inputRef.current).toBe('please ship it')
  })
})
