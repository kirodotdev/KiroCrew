 ...
 *
 * This file originally failed in BOTH directions, on the theory that the
 * downward half is what keeps a budget shrinking. It does not, and #1060
 * retired that model repo-wide: a stored count is regenerated by one command, so
 * the downward half only ever costs a ledger hunk that reads like routine churn,
 * while every parallel i18n branch has to edit the same line and conflict. The
