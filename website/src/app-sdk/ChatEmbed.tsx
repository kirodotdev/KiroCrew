...
function ChatEmbed({ slotKey, agent, placeholder, frameless, startAtBottom, onSend, aboveComposer }: ChatEmbedProps) {
  ...
  const sendMutation = useMutation({
    mutationFn: async ({ msg, override, seenCount, sendId }: { msg: string; override: boolean; seenCount: number; sendId: string }) => {
      const fail = (reason?: string) => {
        ...
        if (!override) restoreIntoComposer(msg)
      }
      const unconfirmed = () => {
        const undo = override ? undefined : restoreIntoComposer(msg)
        setSendTail({
          role: 'notice',
  ...
  const send = useCallback((override?: string) => {
    ...
    // poll show THIS send".
    sendMutation.mutate({ msg, override: override != null, seenCount: messageCountRef.current, sendId: mintSendId() })
