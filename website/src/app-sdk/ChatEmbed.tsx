 ...
 *  user row appearing past that point -- by identity first, never by bare text,
 *  which a manual resend or a duplicate injection could share. `sentText` is
 *  the trimmed wire text, kept ONLY as the queued-drain fallback below: when
 *  the slot was busy the server queues the message and drains it into a user
 *  row WITHOUT the `meta` it was posted with (`sendId` is lost), so a genuinely
 *  delivered late/queued send would otherwise never match `sendId` and the
 *  notice would stick forever. `undo` is the composer before/after an
 *  unconfirmed restore, so that proof can take the text back out. */
interface SendTail {
  role: 'error' | 'notice'
  content: string
  seenCount: number
...
function ChatEmbed({ slotKey, agent, placeholder, frameless, startAtBottom, onSend, aboveComposer }: ChatEmbedProps) {
  ...
  // already have shown it), recognised two ways, both scoped to this send:
  // (1) by identity -- a row carrying this send's `meta.sendId`, which a
  // dispatched send keeps, so a same-text resend or injection cannot be
  // mistaken for it; (2) by the queued-drain fallback -- a busy slot queues
  // the message and drains it into a row that has NO `meta` at all, dropping
  // the id, so a row past the start point that is sendId-less AND carries this
  // send's exact wire text is that same send arriving late. Only that proof
  // retires the notice and takes back the text it handed back -- and only if
  // the composer still holds exactly what the restore produced; anything typed
  // or edited since stays. Transcript growth from anything else (a prior turn
  // still streaming, a cron or sub-agent injecting DIFFERENT text into the
  // slot) is NOT proof: the send may still be undelivered, so the notice and
  // the restored text both stay -- withdrawing the text on unproven delivery
  // would re-create the silent loss this surface is being fixed for. An
  // `error` tail is never retired here: nothing arriving later makes a refused
  // send less refused.
  useEffect(() => {
    if (sendTail?.role !== 'notice' || messages.length <= sendTail.seenCount) return
    const fresh = messages.slice(sendTail.seenCount)
    // Primary proof: THIS send's own user row, by identity. A dispatched send's
    // row carries the `meta.sendId` it was posted with, so it is matchable even
    // against a same-text resend or injection.
    const byId = fresh.some(m => m.role === 'user' && m.meta?.sendId === sendTail.sendId)
    // Queued-drain fallback: a busy slot queues the message and drains it into a
    // user row WITHOUT `meta` (the server does not carry it through the queue),
    // so `sendId` is gone and identity can never match -- the notice would stick
    // on a send that DID land. Recognise that one shape narrowly: a user row past
    // the send-start point that carries NO `sendId` at all AND whose content is
    // exactly this send's wire text. It is scoped so a duplicate cannot false-
    // retire -- a foreign send that DID keep its `sendId` is not sendId-less, and
    // a same-text row that kept a DIFFERENT `sendId` is likewise excluded; only a
    // meta-stripped drain of this exact text passes, which is the queued send.
    const byQueuedDrain = fresh.some(m => m.role === 'user' && m.meta?.sendId == null && (m.content ?? '').trim() === sendTail.sentText)
    if (!byId && !byQueuedDrain) return
    setSendTail(null)
