...
function ChatEmbed({ slotKey, agent, placeholder, frameless, startAtBottom, onSend, aboveComposer }: ChatEmbedProps) {
  ...
  const { draft, setDraft, picked, toggleOption, composition, submitOnEnter, textareaRef } =
    ...
    <div className={`flex flex-col h-full min-h-0 overflow-hidden ${frameless ? '' : 'border border-border rounded-lg bg-bg'}`}>
      ...
      <div className={`flex items-end gap-2 px-3 py-2 shrink-0 ${frameless ? '' : 'border-t border-border bg-bg-accent'}`}>
        <textarea
          ...
          className={/* The field's edge is an inset RING (box-shadow), not a border: the
              auto-grow effect writes a border-box height taken from scrollHeight, which
              excludes borders, so a real border would leave the box 2px short of its
              content at every height and clip the last line's descenders. Ring takes no
              layout space (OAuthRelayAffordance's field precedent; an outline is ruled
              out on a scroller — SessionSummaryTab documents it clipping to a hairline).
              forced-colors keeps a real border (the FileCard/PinnedPrompt convention):
              ring is invisible there, and the cost is bounded — at rest the 38px floor
              absorbs the border exactly, and past one line the 2px surfaces as
              scrollable overflow, not a clipped descender. The min-h floor reproduces
              the old input's 38px resting geometry (the hook delegates the empty size
              to the element's own CSS floor) and guards the hidden-mount case: a
              display:none mount measures scrollHeight 0 and would otherwise write a
              0px-tall composer that stays collapsed until the draft changes. */
            'flex-1 min-w-0 px-3 py-2 min-h-[38px] text-sm bg-bg-elevated ring-1 ring-inset ring-border forced-colors:border rounded-md text-text outline-none focus-visible:ring-accent transition-shadow resize-none overflow-y-auto'}
          value={draft}
