 ...
 * available to its own pattern; both markers therefore parse from one line.
 *
 * The lookahead spans the WHOLE remaining chain rather than one sibling head, and that
 * is load-bearing rather than defensive. Accepting a bare `(?=\[HEAD)` made
 * `[OPTIONS: A] [OPTIONS: B] in docs` parse its FIRST marker (a sibling head follows)
 * while the second declined (prose follows), so `[OPTIONS: A] ` was deleted from the
 * message and came back as a spurious `A` chip — the silent-prose-deletion class the
 * `$` anchor exists to prevent, reintroduced by the escape hatch. Requiring
 * `(?:SIBLING)+$` makes the chain all-or-nothing: every marker on the line parses, or
 * none does and the whole line stays visible prose.
 *
 ...
 *
 * Linear still. Each repetition must open on a literal head at a fixed position and the
 * body is tempered against every head, so it cannot cross into the next sibling: the
 * partition of the line into siblings is UNIQUE, leaving nothing for the engine to
 * re-try. A chain that fails to reach `$` fails once per position, not per split.
 */
