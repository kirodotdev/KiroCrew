// Canonical marker regexes — import these rather than re-spelling one, or the grammar drifts
// between parsers. The CONTENT head mirrors the backend's; the ACTION head has no twin yet.
//
 ...
 *  by both patterns below. Longest-distinguishing first, so a head that is a prefix of
 *  another cannot shadow it. `OPTIONS?:` covers `[OPTIONS:` and the single-choice
 ...
 * forbids only its OWN head still happily consumes the OTHER one. MEASURED here, against a
 * body tempered against one head only — given
 ...
 * anchor is satisfied, and only the capture is wrong. Not reproducible on the backend, which
 * has one head and so cannot exhibit it — this is the frontend's own measurement.
 *
...
const NEWLINE = /\n/.source
const BODY_CHAR = `[^${OPENER_CHARS}${CLOSER_CHARS}${NEWLINE}]`
...
const CONTINUES = `${SEPARATOR}|${CLOSER_CLASS}`
const CONTINUES_AT = new RegExp(CONTINUES, 'y') // sticky, so no slice; lastIndex set per use
/**
...
/** The body's alternation, group-free, so the sibling lookahead below can reuse it
 *  without shifting any capture number. */
const BODY_RUN = `(?:${LABEL_PAIRS}|${TEMPER}|${LABEL_CONTINUES}|${BODY_CHAR})*`
/** The captured label body, single-line. Exactly one capture group. */
const BODY_LINE = `(${BODY_RUN})`
 ...
 * SIBLING MARKER — but only when the siblings run out the LINE.
 *
 * Requiring `$` alone meant that on a shared line only the TRAILING marker could
 * match: `[OPTIONS: A] [OPTION-ACTIONS: close=B]` matched the action marker and left the
 * content marker unmatched, so its pills were dropped AND its raw text rendered as prose
 * — and the affordance that survived was the one that deletes the tab. A LOOKAHEAD fixes
 * that, since the sibling is not consumed and remains available to its own pattern.
 *
 * It spans the WHOLE remaining chain rather than one sibling head, which is load-bearing:
 * a bare `(?=\[HEAD)` made `[OPTIONS: A] [OPTIONS: B] in docs` parse its FIRST marker (a
 * head follows) while the second declined (prose follows), so `[OPTIONS: A] ` was deleted
 * from the message and came back as a spurious `A` chip. `(?:SIBLING)+$` makes the chain
 * all-or-nothing: every marker on the line parses, or none does and the line stays prose.
 *
 ...
 * written. Only a chain of siblings reaching the line end terminates early.
 *
 * NOT linear: the partition inside one lookahead is unique, but the lookahead runs at EVERY
 * match and re-tiles the line's remainder, so k markers cost O(k²). `HEAD_CAP` bounds k.
 */
const SIBLING_MARKER = `\\[(?:${MARKER_HEADS})${BODY_RUN}${TAIL_CLOSER}`
const SIBLING_CHAIN = `(?=(?:${SIBLING_MARKER})+$)`
const TAIL_LINE = `${TAIL_CLOSER}(?:$|${SIBLING_CHAIN})`
/** The wrapper-tolerant tail: the closer, then the run that only a marker which
 *  OPENED one may carry, then the same line end. */
const TAIL_LINE_WRAPPED = `${TAIL_CLOSE_RUN}${WRAP_CLASS}{0,3}${TAIL_SPACE}(?:$|${SIBLING_CHAIN})`
...
const HEAD_FRAME = -1 // a stack frame for a marker HEAD; no opener index is negative
...
/** The choices a matched CONTENT marker OFFERS, in the order the agent listed them.
 *
 * Exported so `parseOptions` and `stripMarkersForDisplay` cannot disagree about it. The
 * display rule keys on a marker rendering at least one pill, and answering that with a
 * second copy of this split is how an empty-labelled `[OPTIONS:]` counted as its own
 * renderer while the pills it implied were dropped: accepted by the grammar, rendered by
 * nothing, and the row emptied between the two. `|` wins when present so a label may
 * contain a comma. */
export function optionsOf(match: RegExpMatchArray): string[] {
  const labels = labelsOf(match)
  const sep = labels.includes('|') ? '|' : ','
  return labels.split(sep).map(o => o.trim()).filter(Boolean)
}

/** Line index of each offset, ascending, in ONE pass. Offsets MUST already be sorted. */
function lineIndices(text: string, offsets: number[]): number[] {
  let line = 0
  let at = 0
  return offsets.map(off => {
    for (; at < off; at++) if (text[at] === '\n') line++
    return line
  })
}

/** Lines whose marker chain carries a member that acceptance refuses.
 *
 * `SIBLING_CHAIN` admits a sibling through the LOOSE tempered body, while acceptance re-checks
 * each marker's OWN balance with `labelsHaveUnmatchedOpener`. The two disagree, so without a
 * chain-level verdict `[OPTIONS: A] [OPTIONS: B then arr[0]` stripped the first marker and left
 * the second as prose: the deleted span came back as a chip nobody wrote. That is the
 * all-or-nothing invariant `SIBLING_CHAIN` claims, so the verdict belongs to the whole line —
 * and to BOTH kinds at once, since a chain may mix them and each pattern sees only its own. */
function refusedChainLines(text: string): Set<number> {
  const out = new Set<number>()
  const kinds: [string, (m: RegExpMatchArray) => string][] = [
    [OPTION_MARKER_PATTERN_SOURCE, labelsOf],
    [OPTION_ACTION_MARKER_PATTERN_SOURCE, m => m[1] ?? ''],
  ]
  for (const [source, labelsFor] of kinds) {
    const matches = [...text.matchAll(new RegExp(source, MARKER_PATTERN_FLAGS))]
    const offsets = matches.map(m => m.index ?? 0)
    const flags = unclosedMarkerFlags(text, offsets)
    const lines = lineIndices(text, offsets)
    matches.forEach((m, i) => {
      if (labelsHaveUnmatchedOpener(labelsFor(m)) || flags[i]) out.add(lines[i] ?? 0)
    })
  }
  return out
}
 ...
 * caller. Depth-filtered like the action path: the tempered body stops one CANDIDATE
 * containing another head, but an UNCLOSED head is no candidate and still nests one. */
export function findOptionMarkers(content: string): RegExpMatchArray[] {
  if (overHeadCap(content)) return []
  const matches = [...content.matchAll(new RegExp(OPTION_MARKER_RE))]
  // No candidate, so the refusal passes below have nothing to judge — and every one of them
  // scans the whole text. An ordinary row without a marker is the overwhelming common case.
  if (matches.length === 0) return matches
  const offsets = matches.map(m => m.index ?? 0)
  const flags = unclosedMarkerFlags(content, offsets)
  const refused = refusedChainLines(content)
  const lines = lineIndices(content, offsets)
  return matches.filter((m, i) =>
    !labelsHaveUnmatchedOpener(labelsOf(m)) && !flags[i] && !refused.has(lines[i] ?? 0))
}
...
/** `content` as a surface should DISPLAY it: accepted markers removed, UNLESS removing them
 *  would leave the user nothing to read.
 *
 * A content marker's labels survive the strip as pills, so emptying that row loses nothing.
 * An action marker has no renderer until the chip layer lands, so the same strip erased an
 * action-only message outright: a row with no text, which `searchableText` could not find
 * either because the index removes the same span. Retaining the marker is the lesser of the
 * two — raw syntax the user can read and search beats a row that silently holds nothing —
 * and it mirrors the rule a refused candidate already follows.
 *
 * Both the transcript and the search index call THIS, so the two cannot drift: a marker kept
 * for display is kept for search, and keying the decision on one head at one site is the
 * repeat defect this module exists to prevent.
 *
 * The predicates that MEASURE a row (`hasOptionsMarker`, `substantiveLength`) keep calling
 * `stripOptionMarkers` deliberately: they ask whether a marker is present and what substance
 * stands beside it, and a marker retained for display is still not substance of its own. */
export function stripMarkersForDisplay(content: string): string {
  ...
  // A CONTENT marker earns the empty row only by RENDERING something, so the test is whether
  // the marker that renders OFFERS a pill — not whether any marker is present. `[OPTIONS:]`
  // is accepted by the grammar (empty labels are balanced) yet offers nothing. It must be the
  // LAST marker, because that is the one whose options the pills come from: polling all of
  // them let an earlier `[OPTIONS: A]` vouch for a trailing `[OPTIONS:]` that renders none,
  // leaving a row with neither prose nor pills — the erasure this retention exists to stop.
  const renders = findLastOptionMarker(content)
  return renders && optionsOf(renders).length > 0 ? stripped : content
}
...
/** Marker heads one message may carry before EVERY marker in it is refused: the chain tail is
 *  re-evaluated per match, so k same-line markers cost O(k²). `{1,N}` would let the tail parse. */
const HEAD_CAP = 256
const overHeadCap = (text: string): boolean =>
  [...text.matchAll(new RegExp(HEAD_RE))].length > HEAD_CAP
 ...
 * head still open.
 *
 * Depth counts HEAD brackets only, and which frame a closer pops differs by frame: a NESTED
 * one only on its own bracket type, since a pair must match; the HEAD on any closer type,
 * since the terminator is a closer CLASS so `[OPTIONS: a】` does close — but never on a
 * CONTINUATION closer, which the label grammar treats as label text rather than a terminator.
 * A bare count was wrong the same way one step down: a citation `[1]` inside an open head
 * cancelled it, so `[OPTIONS: see [1] for details [OPTION-ACTIONS: close=X]` rendered a live
 * close chip from syntax matching no content marker. A stray closer pops an empty stack, a
 * no-op, and the stack resets at each newline because both heads are LINE forms.
 */
function unclosedMarkerFlags(text: string, starts: number[]): boolean[] {
  // Nothing to flag, so none of the four full-text scans below are worth paying for. This is
  // the common case on the render path: `hasOptionsMarker` asks about every transcript row.
  ...
  let closeI = 0
  let lineI = 0
  // Innermost-last: `HEAD_FRAME` marks a marker head, anything else the opener's index in
  // `PAIR_OPENERS`. `depth` counts the head frames, so the flag stays O(1) per offset.
  const stack: number[] = []
  let depth = 0
  return starts.map(start => {
    ...
    for (;;) {
      ...
      if (next === n) {
        ...
        stack.push(isHead ? HEAD_FRAME : PAIR_OPENERS.indexOf(text[next] ?? ''))
        openI++
      } else {
        CONTINUES_AT.lastIndex = next + 1
        const top = stack[stack.length - 1]
        const headEnds = top === HEAD_FRAME && !CONTINUES_AT.test(text)
        if (headEnds || top === PAIR_CLOSERS.indexOf(text[next] ?? '')) {
          if (stack.pop() === HEAD_FRAME) depth--
        }
        closeI++
      }
    }
...
/** Spans the transcript renders VERBATIM: fenced code blocks and `<mcwidget>` bodies.
 *
 * A marker inside one is an agent DOCUMENTING the syntax, not offering an action — the fence
 * is why the reader sees it as text — so stripping it deleted the writer's own example from
 * the transcript and, because the index removes the same span, from search as well. An
 * UNTERMINATED region runs to the end of the text on purpose: a streaming turn has not closed
 * its fence yet, and eating the contents until it does is the same deletion mid-flight.
 *
 * Refused HERE, in the one function that decides what is a marker, so what is OFFERED and
 * what is HIDDEN stay keyed on a single decision — `stripActionMarkers` takes its keep-set
 * from this, so a refusal added to one side alone could not drift. */
const VERBATIM_RE = /```[\s\S]*?(?:```|$)|<mcwidget\b[^>]*>[\s\S]*?(?:<\/mcwidget>|$)/gi
const verbatimSpans = (text: string): [number, number][] =>
  [...text.matchAll(new RegExp(VERBATIM_RE))]
    .map(m => [m.index ?? 0, (m.index ?? 0) + m[0].length] as [number, number])
const inVerbatimSpan = (spans: [number, number][], at: number): boolean =>
  spans.some(([start, end]) => at >= start && at < end)

/** Every action marker in `text` that is genuinely a marker. TWO refusals, answering
 *  different questions: `unclosedMarkerFlags` asks whether an EARLIER head is still open at
 *  this offset, and `labelsHaveUnmatchedOpener` asks whether this marker's OWN body balances.
 *
 * The second is not redundant: the nesting check measures depth AT the offset, so a stray `[`
 * later in the body escapes it — `[OPTION-ACTIONS: close=Close after checking arr[0]` ran its
 * body to `arr[0]`'s closer and rendered the empty string while offering a chip built from the
 * deleted words. Failing closed matters more here than on the content path, which has refused
 * that shape since #9284, because the affordance that survives tears the tab down. */
export function matchActionMarkers(text: string): RegExpMatchArray[] {
  if (overHeadCap(text)) return []
  const matches = [...text.matchAll(new RegExp(OPTION_ACTION_MARKER_RE))]
  if (matches.length === 0) return matches
  const offsets = matches.map(m => m.index ?? 0)
  const flags = unclosedMarkerFlags(text, offsets)
  const refused = refusedChainLines(text)
  const lines = lineIndices(text, offsets)
  const verbatim = verbatimSpans(text)
  return matches.filter((m, i) =>
    !labelsHaveUnmatchedOpener(m[1] ?? '') && !flags[i] && !refused.has(lines[i] ?? 0)
    && !inVerbatimSpan(verbatim, offsets[i] ?? 0))
}

/** Remove every action marker that is genuinely a marker, leaving a rejected span visible.
 *
 * Takes its keep-set FROM `matchActionMarkers` rather than re-deriving the refusals, so what
 * is OFFERED and what is HIDDEN cannot disagree — that disagreement is how a chip appears for
 * text still on screen, or text vanishes with no chip. It once held its own copy of the
 * nesting check, so a refusal added to one side alone kept deleting the prose it refused. */
export function stripActionMarkers(text: string): string {
  if (overHeadCap(text)) return text
  const accepted = new Set(matchActionMarkers(text).map(m => m.index))
  return text.replace(OPTION_ACTION_MARKER_RE, (m: string, _body: string, offset: number) =>
    accepted.has(offset) ? '' : m,
  )
}
