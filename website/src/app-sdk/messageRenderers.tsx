...
export const defaultMessageRenderers: readonly MessageRenderer[] = [
  ...
  },
  {
    ...
    render: (m, ctx) => {
      ...
      // Every break carries its time. The row's OWN instant is the visible one, so a
      // deferred break never reads earlier than the row above it.
      const at = (m.meta?.at as string | undefined) || m.ts
      // The requested instant stays reachable in the title: one seam releases every
      // marker a turn held, so landed stamps clump and requested ones separate them.
      const requested = fmtMessageTimeFull(at)
      return ctx.row(
        <SectionMarkerRow
          label={label}
          fallback={m.content}
          ...
          timeTitle={
            at === m.ts
              ? requested
              : i18nT('pages.chat.sectionMarkerRow.time_pair', {
                  requested,
                  landed: fmtMessageTimeFull(m.ts),
                })
          }
        />,
        true,
