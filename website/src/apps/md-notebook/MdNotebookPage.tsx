...
export default function MdNotebookPage() {
  ...
  // The rename typed so far, held here as well as in the field: the row sits
  // in a list that unmounts while a search is typed.
   ...
   */
  /**
   * Resolves true once the note exists on the backend — whatever happens to the
   * reload and open that follow — and false when the create itself was refused.
   * The folder field keys on that: it closes on true and keeps its draft on false.
   */
  const newNote = useCallback(async (folder = ''): Promise<boolean> => {
    const vault = vaultRef.current
    let created = false
    try {
      ...
      created = true
      if (vaultRef.current !== vault) return created
      await loadNotes()
      if (vaultRef.current !== vault) return created
  ...
  const newFolder = useCallback(
    async (parent: string, name: string): Promise<boolean> => {
      const clean = portableName(name)
      // Nothing usable in the name: nothing to create, nothing to keep open.
      if (!clean) return true
      return newNote(parent ? `${parent}/${clean}` : clean)
    },
    [newNote],
  )
