...
export function NewFolderRow({
  ...
  const commit = () => {
    ...
    // The field closes only once the folder exists. A refused creation (a
    // read-only vault, a lost connection) raises the error notice from the
    // page; the field stays with its draft so the name is not typed twice.
    // `done` holds during the request — a second Enter or the blur that
    // follows a click elsewhere must not create the folder twice — and is
    // released on failure so the retry (or Escape) works again.
    void actions.onNewFolder(parent, name).then((created) => {
      if (created) actions.onNewFolderEnd()
      ...
      <NameField
        ...
        onKeyDown={e => {
          ...
          if (e.key === 'Enter') {
            ...
            // Escape during an IME composition belongs to the IME (it drops
            // the candidate, not the field): acting on it here would unmount
            // the field and lose the whole draft.
...
export function NoteRow({
  ...
  const [problem, setProblem] = useState<string | null>(null)
  const inputRef = useRef<HTMLInputElement | null>(null)
  // Read by the seeding effect below without being one of its dependencies:
  // the page's actions bundle changes on every mirrored keystroke, and
  // re-seeding (and re-selecting) the field on each one would fight the user.
  ...
  // or from the draft mirrored on the page, when this row is remounting after a
  // search unmounted it mid-rename.
  useEffect(() => {
    if (!renaming) return
    setDraft(actionsRef.current?.renameDraft ?? note.title)
    setProblem(null)
  ...
  // The same preview the folder field gives: when the cleaner would change the
  // typed name, say what the note will be called before Enter.
  const typedRename = draft.trim()
  const cleanRename = portableName(typedRename)
  const renameHint =
    renaming && typedRename && typedRename !== note.title && !problem && cleanRename && cleanRename !== typedRename
      ? i18nT('apps.mdNotebook.row.nameWillBe', { name: cleanRename })
      : undefined
    ...
    >
      {renaming ? (
        <NameField
          ...
          onChange={e => {
            setDraft(e.target.value)
            actions?.onRenameDraft(e.target.value)
            setProblem(null)
          }}
          onClick={e => e.stopPropagation()}
          ...
          onKeyDown={e => {
            ...
            if (e.key === 'Enter') {
              ...
              // Same IME rule as the folder field: a composition's Escape is
              // the IME's, and ending the rename on it would drop the edit.
              if (!ime.claimKey(e)) return
              e.preventDefault()
              actions?.onRenameEnd()
            }
