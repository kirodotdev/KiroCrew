 ...
 *
 * Two shapes reach the device behind a name that does not look like one:
 * `CONIN$` and `CONOUT$` are consoles too, and Windows strips the spaces
 * before an extension, so `CON .txt` resolves to `CON`. The ` *` before the
 * extension covers the second — an interior space survives `cleanComponent`,
 * which strips only a TRAILING run, so without it `CON .txt` is created on
 * POSIX and no Windows clone can check the vault out again.
 */
 ...
 * The backend applies the same rule to every path it is about to create, on
 * every platform; the client repeats it for an earlier, local message at the
 * field, and it is the wider of the two — the server's device-name set does
 * not list the superscript ports (`COM¹`–`LPT³`) yet.
 */
