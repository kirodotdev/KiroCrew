...
export interface NoteActions {
  ...
  onRenameEnd: () => void
  /**
   * The rename typed so far, mirrored here by the field on every keystroke and
   * read back as its initial value; null until the first keystroke, so the
   * field seeds from the title. The row unmounts while a search is typed —
   * kept only in the field, a half-typed (or refused) name would come back as
   * the original title once the search is cleared.
   */
   ...
   * Resolves true once the note exists on the backend; false when the request
   * was refused (the error notice says why), so the field can stay open with
   * its draft instead of making the user retype a name the server rejected.
   */
