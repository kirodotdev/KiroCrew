...
import { withPreviewCsp } from './previewCsp'
 ...
 * could exfiltrate the rendered document. `withPreviewCsp` additionally stops
 * PASSIVE loads (tracking pixels, loopback probes), which the sandbox does not.
 * Interactive pages show as layout only; the source toggle carries the rest. */
export function HtmlViewer({ content }: { content: string }) {
  const [mode, setMode] = useState<'preview' | 'source'>('preview')
  const blobUrl = useMemo(
    () => URL.createObjectURL(new Blob([withPreviewCsp(content || '')], { type: 'text/html' })),
    [content],
  )
  return (
...
export function OfficeViewer({ path }: { path: string }) {
  ...
  if (error || !data) {
    // A failed extraction is a FAILURE, not an empty state — the document
    // exists and could not be read — so it renders through the shared
    // ErrorNotice (AUTOSDE `errors-use-error-notice`) rather than an
    // EmptyState subtitle, which dropped the structure and the hand-off.
    // askAgent is ON: unlike an unsaved draft, a document that will not parse
    // is exactly the kind of thing an agent can usefully look into.
    return (
