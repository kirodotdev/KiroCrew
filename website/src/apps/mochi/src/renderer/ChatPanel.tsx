...
import { reportStat, isMochiBindError, isMochiSlotOccupiedError } from '../../panel/panelBridge'
...
export const ChatPanel: React.FC<ChatPanelProps> = ({ onToggleWatch, watchPanelVisible, onTogglePinned, pinnedPanelVisible, pinnedFileCount }) => {
  ...
  const sendText = useCallback(async (text: string) => {
    ...
    try {
      ...
      // send them debugging the wrong thing. The occupied-slot refusal gets a
      // string of its own again: it is PERMANENT until the user clears the
      // populated slot, so "try again in a moment" would loop them on advice
      // that can never work.
      setIsWaiting(false)
      setTurnActive(false)
      setInput((prev) => (prev ? prev : text))
      setDropError(i18nT(isMochiSlotOccupiedError(err)
        ? 'apps.mochi.chat.slot_occupied'
        : isMochiBindError(err)
