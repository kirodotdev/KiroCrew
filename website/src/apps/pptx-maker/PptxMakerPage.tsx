 ...
 * (`sdpm-spec`, `sdpm-vibe`, `sdpm-composer`, `sdpm-style`) and the page links
...
const CHAT_AGENTS = [
  {
    agent: 'pptx-maker/sdpm-spec',
    labelKey: 'apps.pptxMaker.pptxMakerPage.mode_spec',
    hintKey: 'apps.pptxMaker.pptxMakerPage.mode_spec_hint',
  },
  {
    agent: 'pptx-maker/sdpm-vibe',
    labelKey: 'apps.pptxMaker.pptxMakerPage.mode_vibe',
    hintKey: 'apps.pptxMaker.pptxMakerPage.mode_vibe_hint',
  },
  {
    agent: 'pptx-maker/sdpm-style',
    labelKey: 'apps.pptxMaker.pptxMakerPage.mode_style',
    hintKey: 'apps.pptxMaker.pptxMakerPage.mode_style_hint',
  },
] as const
            ...
            <Card className="mb-4">
              ...
              <div className="flex items-start gap-3 flex-wrap">
                {CHAT_AGENTS.map((entry) => (
                  <div key={entry.agent} className="flex flex-col gap-1 max-w-[230px]">
                    <SendBtn
                      onClick={() => startChat.mutate(entry.agent)}
                      disabled={startChat.isPending}
                    >
                      {entry.agent.endsWith('sdpm-style') ? (
                        <Sparkles className="lucide-inline" />
                      ) : (
                        <MessageSquarePlus className="lucide-inline" />
                      )}
                      {i18nT(entry.labelKey)}
                    </SendBtn>
