...
interface Options {
   ...
   */
  onFinal?: (text: string) => boolean | void
  ...
  const start = useCallback(async () => {
    ...
    ws.onmessage = ev => {
      ...
      if (msg.type === 'final') {
        ...
        if (onFinalRef.current?.(text) === false) return
        // This is the line that reaches the agents. A dropped dispatch must not
        // tear down the stream — the caption already shows it and the next
        // segment still lands.
        meetingsApi.dispatch(meetingId, text).catch(() => {})
        return
      }
      if (msg.type === 'error') {
    ...
    bufferedBytes = 0
    ready = true
  }, [cleanup, clearWatchdog, meetingId])
