...
// questions differ: a pull/merge request is about landing it, a finding is about
...
// agent that publishes a PR or MR, or edits the measurement harness, has defeated
// the point of it. Kept in one builder so a new surface cannot forget them.
import type { SubjectKind } from './agentSession'

/** True when a change-request URL is a GitLab merge request rather than a GitHub
 *  pull request. The `/-/merge_requests/` path segment is GitLab's canonical web
 *  route, so the URL itself is the provider signal — no host list to maintain. */
export function isMergeRequestUrl(url?: string): boolean {
  return Boolean(url && url.includes('/-/merge_requests/'))
}

/** The provider's own noun for a drafted change: "merge request" on GitLab,
 *  "pull request" on GitHub. Spliced into English prompts, so English on purpose. */
export function changeRequestNoun(url?: string): string {
  return isMergeRequestUrl(url) ? 'merge request' : 'pull request'
}

/** The abbreviation practitioners use for the same thing: "MR" vs "PR". */
export function changeRequestAbbrev(url?: string): string {
  return isMergeRequestUrl(url) ? 'MR' : 'PR'
}

/** The non-negotiables appended to every seed prompt. Parameterized on the
 *  subject URL so a prompt about a specific GitLab MR says "merge request";
 *  surfaces with no subject URL (ruler, run) get the neutral both-providers form. */
function constraintsFor(url?: string): string {
  const lead = url
    ? `Constraints: this ${changeRequestNoun(url)} is a DRAFT by design — never publish it, mark it ready for review, merge it, or enable auto-merge; that is the human's call.`
    : 'Constraints: every pull or merge request this loop drafts is a DRAFT by design — never publish one, mark it ready for review, merge it, or enable auto-merge; that is the human\'s call.'
  return [
    lead,
...
export function prPrompt(pr: PrSubject): string {
  const lines = [
    `Help me land this draft ${changeRequestNoun(pr.url)}: ${pr.url}`,
    '',
    `Title: ${pr.title}`,
  ]
  ...
  lines.push(
    '',
    `This ${changeRequestAbbrev(pr.url)} was drafted by the auto-improvement loop, which already verified the change and reproduced its result independently. Your job is what is left: read the current CI state and review threads, fix what is genuinely failing, and rebase if it conflicts.`,
    '',
    'Start by telling me what is actually blocking it, before changing anything.',
    '',
    constraintsFor(pr.url),
  )
  return lines.join('\n')
}
...
export function findingPrompt(f: FindingSubject): string {
  ...
  if (f.pr) lines.push(`${isMergeRequestUrl(f.pr) ? 'Merge request' : 'Pull request'}: ${f.pr}`)
  lines.push(
    '',
...
export function runPrompt(r: RunSubject): string {
  ...
  if (r.drafted !== undefined) lines.push(`Pull or merge requests drafted: ${r.drafted}`)
  lines.push(
    '',
...
/** Human-readable default label for a subject kind, used in slot titles.
 *  For the `pr` kind, prefer `kindLabel` with the subject URL so a GitLab
 *  merge request is titled "MR", not "PR". */
...
/** KIND_LABEL, provider-aware: derives PR vs MR from the subject URL. */
export function kindLabel(kind: SubjectKind, url?: string): string {
  if (kind === 'pr') return changeRequestAbbrev(url)
