...
/**
 * The notice copy for a SEARCH failure, keyed by the same cause the listing arm uses.
 *
 * Diverges on `timed_out` alone, because that is the one string naming the operation
 * that expired. A refusal is the same fact whichever read hit it, so the other three
 * share the listing arm's strings.
 */
export const SEARCH_FAILURE_KEYS: Record<SearchErrorCause, string> = {
