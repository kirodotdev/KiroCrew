...
interface CleanupFailure {
  key: string
  /** `revive`: the existence check that gates reviving this chat's retired
   *  terminals failed for a reason other than 404, so they stay hidden until
   *  the check is retried. It carries no lease. */
  phase: 'delete' | 'cleanup' | 'terminals' | 'revive'
...
export async function closePreparedTerminals(lease: TerminalSessionLease): Promise<void> {
  ...
  if (existing && existing.phase !== 'delete' && existing.phase !== 'revive') return retryHistoryTerminalCleanup(lease.scope)
...
/** The active chat's terminals are retired (its history was deleted, or a
 * deletion was interrupted). Confirm the chat still exists before reviving
 * them, capturing the activation BEFORE the request. A 404 means the chat is
 * gone and the terminals stay hidden by design — not a failure. Any other
 * failure of the check is recorded as a `revive` failure for the cleanup
 * notice to show with a retry, so the request's rejection is never swallowed
 * (`errors-use-error-notice`). Deliberately leaves another phase's failure in
 * place: that notice already owns this key. */
export async function reviveRetiredTerminals(key: string): Promise<void> {
  const started = resetGeneration
  const activation = captureTerminalActivation(key)
  const existing = failures.get(identity(key))
  if (existing && existing.phase !== 'revive') return
  if (existing) { failures.set(identity(key), { ...existing, busy: true }); publish() }
  try {
    await api.chatSlotDetail(key, 1, 0)
  } catch (error) {
    if (started !== resetGeneration) return
    if (error instanceof ApiError && error.status === 404) failures.delete(identity(key))
    else failures.set(identity(key), { key, phase: 'revive', ids: null, busy: false })
    publish()
    return
  }
  if (started !== resetGeneration) return
  await activateTerminalsAfterNavigation(key, activation)
  // Clear the notice only once the terminals are back, so its Retry stays busy
  // for the whole recovery rather than for the check alone.
  if (existing && started === resetGeneration) { failures.delete(identity(key)); publish() }
}
