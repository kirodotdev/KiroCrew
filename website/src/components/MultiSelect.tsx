...
export interface MultiSelectOption {
  ...
  description?: string
  locked?: boolean
  /** The label is an identifier shown as-is (a model id with no vendor name),
   *  not copy: rendered in the mono stack and marked opaque for the i18n render
   *  scan, the way this codebase marks code and paths. */
  opaqueLabel?: boolean
}
          ...
          return (
            ...
            >
              <Checkbox
                ...
                data-i18n-opaque={option.opaqueLabel || undefined}
                onChange={event => onToggle(option.value, event.target.checked)}
              />
              <span className="min-w-0 flex-1">
                <span
                  data-i18n-opaque={option.opaqueLabel || undefined}
                  className={`block truncate text-[13px] font-medium text-text ${option.opaqueLabel ? 'font-mono' : ''}`}
                >
                  {option.label}
                </span>
