 ...
 *
 * ENTRANCE IS ANIMATED (UX round 28, lens-13): when `merged: true` lands while
 * the fork is still on screen (second tab, split pane, embed), the composer is
 * replaced by this bar — animating the bar in from the composer's position
 * makes that read as the composer BECOMING the bar rather than "that vanished
 * and something else appeared". The animation lives here, on the appearing
 * element, so every host surface gets the same transition without each swap
 * site wrapping its (large) composer subtree in an AnimatePresence pair.
 */
