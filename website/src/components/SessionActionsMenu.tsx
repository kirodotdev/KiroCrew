...
export default function SessionActionsMenu({
  ...
  const groups = collapseGroups<React.ReactNode>([
    ...
    [
      <Item key="close" {...offlineItem(i18nT('utils.offline.close_sessions'), i18nT('components.sessionActionsMenu.close_session'), () => close(slotKey))} className={connected ? 'text-danger focus:text-danger' : 'opacity-40 text-muted'}>
