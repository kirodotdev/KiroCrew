...
function QueueStackInner({ messages, onCancel, onInterrupt, onEdit, onReorder, fuseBelow = true, pendingIds,
  editError, editRejected, onDismissEditError }: {
  messages: ChatMessage[]
  ...
  /** The text each rejected edit threw away, keyed by card. Reopens that card's input seeded. */
  ...
  // A rejected edit closed its input before the failure landed, so the typed text existed nowhere.
  // Reopen that card seeded with it rather than making the user retype work the server never took.
  useEffect(() => {
    if (!editRejected) return
    const target = messages
      .map(m => m.meta?.queueId as string | undefined)
      .find(id => !!id && id in editRejected)
    if (target) setEditingId(target)
  }, [editRejected, messages])
            ...
            return (
              ...
              >
                <span className="flex items-center gap-1.5 h-full">
                  ...
                  {isEditing && onEdit ? (
                    <EditInput initial={(queueId && editRejected?.[queueId]) ?? m.content} onCommit={v => commitEdit(queueId!, v)} onCancel={cancelEdit} />
                  ) : (
                    <>
...
export default memo(QueueStackInner, (prev, next) =>
  ...
  prev.editRejected === next.editRejected &&
