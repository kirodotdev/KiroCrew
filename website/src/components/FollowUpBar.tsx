...
function Chip({ option, recommended, isPicked, picked, quickSend, onSelect, onSend, className, index, animating, sourceKey }: ChipProps) {
  ...
  const hint = chipTitle(isPicked, quickSend, picked, !!onSend, sendRefused(option))
  ...
  // A habituated quick-send user clicks expecting a SEND and gets a composer fill. The dashed
  // edge and the note are at-rest signals, so neither marks the moment the gesture differed.
  ...
  const acknowledgeHold = () => {
    if (!sendRefused(option)) return
    setAckHeld(true)
    if (ackTimerRef.current) clearTimeout(ackTimerRef.current)
    ackTimerRef.current = setTimeout(() => setAckHeld(false), 1400)
  }
    ...
    return (
      <>
        <button
          ...
          onClick={(e) => { acknowledgeHold(); onSelect(option, e) }}
          className={`${className} ${entrance.className}${ackHeld ? ' ring-2 ring-accent' : ''}`}
          style={entrance.style}
          {...tipHandlers}
          data-held={ackHeld ? 'true' : undefined}
        >
