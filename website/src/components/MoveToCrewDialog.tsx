...
interface Props {
  // No `unitKind`: the heading named it by interpolating the raw enum, so every
  // locale rendered "moving taskrun …" untranslated. The id identifies the unit,
  // the menu it was opened from supplies the kind, and the rendered plan states
  // the bundle kind — so the token was implementation vocabulary with no reader.
  unitId: string
  onPlan: (toCrew: string) => Promise<{ ok?: boolean; plan?: MovePlan; error?: string }>
  onClose: () => void
}

export default function MoveToCrewDialog({ unitId, onPlan, onClose }: Props) {
  ...
  // The suggestions notice is derived from a query, not stored, so its dismissal
  // needs its own bit — there is no message string to clear.
  ...
  return (
    <Dialog open onOpenChange={open => { if (!open) onClose() }}>
      <DialogContent maxWidth={640} hideClose>
        ...
        <DialogBody>
        {/* The rhythm between the field, the notice and the plan panel belongs to
            this content, not to DialogBody -- that primitive owns its own padding,
            so a spacing class on it would restyle the component from a call site. */}
      ...
      {crews.length > 0 && (
        ...
        {/* No agent hand-off here: `toCrew` is an unsaved local draft. Leaving
            ...
            Dismiss has to clear BOTH sources. `suggestionsError` is derived from
            the crew-list query rather than held in state, so clearing only
            `error` leaves the X inert whenever the suggestions notice is the one
            on screen -- a control that is present and does nothing. */}
        <ErrorNotice
          message={error || (suggestionsHidden ? '' : suggestionsError)}
          onDismiss={() => {
            setError('')
