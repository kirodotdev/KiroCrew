...
describe('the delivery warning strip decides once for both surfaces', () => {
  ...
  it('states the consequence in every state that drops a send', () => {
    // Keyed on delivery ALONE: the handler drops any row the reducer will take, and the reducer
    // refuses only a delivered one, so suppressing the note promised a silence never given.
    for (const delivered of [false, true]) {
