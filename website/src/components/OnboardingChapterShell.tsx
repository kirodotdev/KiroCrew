...
function StockOnboardingDecorations() {
  return (
    <>
...
export function ShellAside({ copy }: { copy: ShellAsideCopy }) {
  ...
  const branding = theme ? getThemeBranding(theme.colorTheme) : undefined
  const brandLogo = branding?.logo
  const OnboardingDecorations = branding?.onboardingDecorations
    ...
    <aside className="relative flex min-h-[248px] w-full shrink-0 overflow-hidden bg-accent text-accent-fg sm:min-h-0 sm:w-[36%]">
      {OnboardingDecorations ? (
        <div className="pointer-events-none absolute inset-0 z-0" aria-hidden="true">
          <ErrorBoundary
            scope="theme-onboarding-decorations"
            fallback={<StockOnboardingDecorations />}
          >
            <OnboardingDecorations />
          </ErrorBoundary>
        </div>
      ) : (
        <StockOnboardingDecorations />
      )}
      <div className="relative z-10 flex w-full flex-col p-7 sm:p-10">
        <div className="flex items-center gap-3">
          {brandLogo ? (
            <img src={brandLogo} alt="" aria-hidden="true" className="h-8 w-8 shrink-0 object-contain" />
          ) : (
