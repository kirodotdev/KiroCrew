 ...
 */
import { BadgeCheck, Check, Database, Plus, ShieldCheck, Users } from 'lucide-react'
      ...
      <div>
        ...
        {sources.map(s => (
          ...
          >
            ...
            {/* Toggle affordance: a selected source row carries a check so the
                multi-select model reads as independent toggles, distinct from
                the single-select CATEGORIES list above. Decorative — the row's
                aria-pressed already exposes the selected state to assistive tech. */}
            {selectedSources.includes(sourceRowKey(s)) && (
              <Check size={14} className="text-accent shrink-0 ml-auto" aria-hidden />
            )}
          </Clickable>
        ))}
        <button
