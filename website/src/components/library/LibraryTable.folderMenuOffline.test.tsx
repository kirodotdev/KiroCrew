...
describe('artifacts FolderMenu – the reason row must be true of the whole menu', () => {
  ...
  /** The trigger keeps its own node offline — dimmed and refused — so a
   *  disconnect with the menu open does not unmount the chevron under the
   *  pointer. `disabled` alone cannot carry that: on a coarse pointer the
   *  SubTrigger renders a role="button" div (PhoneSubTriggerDiv) that spreads
   *  unrecognised props onto the <div> and calls onToggle unconditionally, so
   *  `disabled` means nothing there. The controlled `open` closes both paths. */
  const moveRow = () =>
    screen.getByText(/^Move to folder/).closest('[role="menuitem"],[role="button"]') as HTMLElement

  it('offline the Move row is still the trigger, keeping its chevron, and cannot expand', () => {
    const { actions } = renderMenu(false)
    const row = moveRow()
    expect(row.getAttribute('aria-disabled')).toBe('true')
    expect(row.className).toContain('opacity-40')
    expect(row.className).toContain('cursor-not-allowed')
    // Two icons: the folder AND the chevron. The superseded design swapped in a
    // plain row carrying only the folder, which is the unmount this pins shut.
  ...
  it('offline the Move row stays inert on a coarse pointer, where `disabled` is not honoured', () => {
  ...
  /** The control for the two above: the same node, undimmed, really does expand —
   *  so their "did not expand" arms are measuring the gate and not a dead render. */
  it('connected the Move row is the same node and does expand', () => {
    renderMenu(true)
    const row = moveRow()
...
describe('artifacts row writes – a dimmed control must still say why', () => {
  ...
  /** The star compounds a different way: element `opacity-40` multiplies with the
   *  UNPINNED colour's own alpha (`text-muted/40`), so 0.4 × 0.4 ≈ 16% and a
   *  control meant to read "inert but present" reads as absent. Offline the
   *  unpinned star takes full-alpha `text-muted` so `opacity-40` is the only dim.
   *  The pinned arm never had this: `text-accent` carries no alpha of its own. */
  it('offline the UNPINNED star dims once, not opacity times colour alpha', () => {
    renderWithStore(false, <table><tbody><ArtifactRow a={artifact} onOpen={vi.fn()} onDelete={vi.fn()} deletingSlug={null} onTogglePin={vi.fn()} /></tbody></table>)
    const star = screen.getByRole('button', { name: /^star artifact disabled/i })
    expect(star.className).toContain('opacity-40')
    expect(star.className).not.toContain('text-muted/40')
    expect(star.className).toMatch(/\btext-muted\b/)
  })

  it('connected the unpinned star keeps its lighter resting colour — the control', () => {
    renderWithStore(true, <table><tbody><ArtifactRow a={artifact} onOpen={vi.fn()} onDelete={vi.fn()} deletingSlug={null} onTogglePin={vi.fn()} /></tbody></table>)
    const star = screen.getByRole('button', { name: /^star artifact$/i })
    expect(star.className).toContain('text-muted/40')
    expect(star.className).not.toContain('opacity-40')
  })

  /** The pinned star was already single-layer, so the fix above must not have
   *  quietly changed it: `text-accent` plus exactly one `opacity-40`. */
  it('offline the PINNED star keeps its accent colour and one dim', () => {
    ...
    expect(star.className).toContain('opacity-40')
    expect(star.className).not.toContain('text-muted')
  })
...
describe('FolderNameInput – an offline commit must not consume the draft', () => {
  ...
  /** An empty field holds no draft, so the offline hold must not swallow the
   *  abandon too. The callers that open this editor are ungated, so a held-open
   *  empty editor leaves Escape as the only exit — which a coarse pointer lacks. */
  it('offline an EMPTY field still abandons, so the editor cannot strand open', () => {
    const { field, onCommit, onCancel } = renderInput(false)
