  ...
  return (
    <>
      <Input
        ...
        {...ime.bindEnter<HTMLInputElement>({
          ...
          onBlur: () => {
            ...
            // and its draft intact until the connection is back.
            if (!connected) return
      ...
      {/* A held draft the user cannot see reads as a dead field, which is the
          ...
          note asks for the field to be kept open rather than claiming the name
          is saved. */}
      {!connected && (
  ...
  return (
    <DropdownMenu>
      ...
      <DropdownMenuContent align="end" onClick={(e) => e.stopPropagation()}>
        <DropdownMenuItem
          className={connected ? undefined : 'opacity-40 text-muted cursor-not-allowed'}
        ...
        <DropdownMenuSeparator />
        <DropdownMenuItem
          className={`text-danger${connected ? '' : ' opacity-40 cursor-not-allowed'}`}
        ...
        >
          <td className="px-2.5 py-2 border-b border-border text-center">
            <button
              ...
              className={`p-0.5 rounded transition-colors bg-transparent border-none disabled:cursor-default ${rowConnected ? 'cursor-pointer ' : 'opacity-40 cursor-not-allowed '}${a.pinned ? 'text-accent' : rowConnected ? 'text-muted/40 hover:text-accent' : 'text-muted/40'}`}
...
export function SessionDocStar({ d, busy, onMaterialize }: { d: SessionDoc; busy: boolean; onMaterialize: (path: string, sessionKey?: string) => void }) {
  ...
  return (
    <IconButton
      variant="accent"
      disabled={busy}
      onClick={() => { if (!connected) return; onMaterialize(d.path, d.session_key) }}
