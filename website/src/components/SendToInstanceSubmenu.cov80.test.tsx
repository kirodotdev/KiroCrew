...
describe('SendToInstanceSubmenu', () => {
  ...
  it('the page notice keeps the peer\'s own sentence, so the what-to-do survives the menu closing', async () => {
    // The gateway answered with the peer's reason: that sentence was written for
    // the reader, and the row that shows it is gone once the menu closes.
    sendSessionToInstance.mockRejectedValue(new ApiError(502, 'zzq instance is running an older Kiro Crew — update it, then reconnect'))
    mount([instance()])
    fireEvent.click(await screen.findByTitle('zzq-peer'))
    const { result } = renderHook(() => useActionFailure())
    await waitFor(() => expect(result.current.failure?.message)
      .toBe('The copy was not delivered — this session is unchanged. zzq instance is running an older Kiro Crew — update it, then reconnect'))
  })
  ...
  it('a resend to the same peer that lands takes the page notice down', async () => {
    sendSessionToInstance
      .mockRejectedValueOnce(new Error('zzq-peer-refused'))
      .mockResolvedValue({ resume_mode: 'session_load' } as never)
    mount([instance()])
    fireEvent.click(await screen.findByTitle('zzq-peer'))
    const { result } = renderHook(() => useActionFailure())
    await waitFor(() => expect(result.current.failure?.message)
      .toBe('The copy was not delivered — this session is unchanged.'))
    fireEvent.click(screen.getByTitle('zzq-peer'))
    await waitFor(() => expect(sendSessionToInstance).toHaveBeenCalledTimes(2))
    await waitFor(() => expect(result.current.failure).toBeNull())
    expect(await screen.findByText('Sent')).toBeInTheDocument()
  })

  it('a copy that lands on ANOTHER peer leaves the refused peer\'s notice up', async () => {
    // Still true, and its detail names the refusing peer's problem.
    sendSessionToInstance.mockImplementation((id: string) =>
      id === 'i1' ? Promise.reject(new Error('zzq-peer-refused')) : Promise.resolve({ resume_mode: 'session_load' } as never))
    mount([instance(), instance({ id: 'i2', name: 'zzq-peer-2' })])
    fireEvent.click(await screen.findByTitle('zzq-peer'))
    const { result } = renderHook(() => useActionFailure())
    await waitFor(() => expect(result.current.failure?.message)
    ...
    fireEvent.click(screen.getByTitle('zzq-peer-2'))
    expect(await screen.findByText('Sent')).toBeInTheDocument()
