...
export default function SourceLinksSubmenu({ slotKey, variant }: {
  ...
  const unlink = useMutation({
    ...
    mutationFn: (identity: string) => api.unlinkSourceLink(slotKey, identity, slotGeneration(slotKey)),
    // Capture BOTH the slot snapshot and the session generation at mutate-start.
    ...
    onMutate: () => ({
      ...
      generation: slotGeneration(slotKey),
    }),
    onSuccess: (_result, identity, context) => {
