...
export default function ChatPane({
  ...
  const handleUserInput = useCallback((next: string | ((prev: string) => string)) => {
    ...
    // Report the edit to the voice atom, which is the only place that can tell a
    // deletion-then-identical-retyping of a dictated run from a run nobody touched:
    // the two leave the same pair of values, so provenance has to be observed as it
    // happens. Skipped for the atom's own writes, which come through this same
    // handler -- it is the setter the atom was given.
    composerRef.current?.voice()?.noteComposerEdit(before, inputRef.current)
    setInput(next)
  }, [])
