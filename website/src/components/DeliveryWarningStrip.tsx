...
/** Whether the dismiss exit REMOVES the row, which decides its label and whether the consequence is
 *  stated. Keyed on delivery ALONE: the handler drops any row the reducer will take, and the reducer
 *  refuses only a delivered one -- so gating on `discardable` too promised a silence it never gave. */
const dismissDropsBubble = (delivered: boolean, _discardable: boolean): boolean => !delivered
