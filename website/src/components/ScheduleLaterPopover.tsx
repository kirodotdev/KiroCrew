  ...
  return createPortal(
    ...
    >
      <div className="flex min-w-0 flex-col gap-2 p-0.5">
        ...
        {/* Disclosed at the moment of choosing, not after the fact: a pending
            message lives only in this gateway process (the accepted
            restart-invalidation contract), and a user who learns that from a
            missing message after a restart has already lost it. Named as the
            confirm button's description so a screen reader hears it before
            committing. */}
        <p
          id="schedule-later-restart-note"
          className="text-[11px] text-muted"
          data-testid="schedule-later-restart-note"
        >
          {t('components.chatInput.schedule_restart_clears_note')}
        </p>
        <Btn
          primary
          onClick={() => {
          ...
          aria-describedby="schedule-later-restart-note"
