/** Wrap a bare patch body in the `diff --git` / `---` / `+++` headers Pierre
 * needs to identify a file. These strings are parser wire format, not
 * user-visible copy; this module is listed in `eslint.i18n.config.js`. */
...
/** The `---`/`+++` pair for the synthetic unnamed-snippet file section. */
