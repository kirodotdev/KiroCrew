...
export default function AutoNudgePopover({ slotKey, loop, open, onOpenChange, onChange, onSetUpBoundedMonitor, writeDisabled = false, interrupted = false, trigger, content }: Props) {
   ...
   *  nudge goes out. The cap is judged on the FORM, because the raised cap
   *  rides the resume: the moment the field holds a value the timer would let
   *  through, Play is back. The budget has no field here (`max_runtime_secs`
   *  is measured from the record's creation, and neither is in this form), so
   *  a loop stopped on it has nothing to revive it with from this surface. Read
   ...
   *
   *  Pause follows the same gate. A running loop sits at its cap for a whole
   *  idle interval after the capping fire (the timer's cap check runs on the
   *  NEXT tick), and a pause landing then would record `manual` over a bound
   *  that is spent: the record stops saying why it is done, reads Paused, and
   *  its Play is one the timer turns away. So a pause this surface can SEE
   *  would hide a spent bound is not sent; the one it cannot see -- a stop the
   *  frame has not yet delivered -- the service keeps by itself, and its
   *  answer (the record as it stands) is what `pause` hands up. */
  const boundSpent = !!loop && (
    loop.stopped_reason === RUNTIME_BUDGET_STOP_REASON
    || (parseCycles(maxCyclesInput) > 0 && loop.cycle_count >= parseCycles(maxCyclesInput))
  )
  ...
  /** Save, on a RUNNING loop only: persist the fields the user edited, nothing
   *  else (`editedFields`; a pristine form sends an empty PATCH, which writes
   *  nothing and returns the record as it stands).
   *
   ...
   *  stale running reading ends in Stopped, never in Paused. And a pause this
   *  surface can see would land on a spent bound (`boundSpent`) is not sent at
   *  all -- the control is disabled, and this refuses too. The body carries
  ...
  async function pause() {
    if (!loop || writeDisabled || boundSpent) return
    setSaving(true)
    setError('')
    try {
   ...
   *  form it fires and writes nothing (`triggerNow`). Play with no loop
   *  creates the loop from the form and fires the id the server returned
   *  (`startNow`) -- a create has no pristine case.
   ...
   *  holds AND what the user last saw, so they join the pristine baseline -- a
   *  second press with no further edit sends nothing again. The fields NOT
   *  sent keep their baseline: they were not shown anew, and the record may
   *  hold another writer's value for them. Hands NOTHING up itself -- see
   *  `handUpAfterFire`. */
  async function writeLoop(write: { url: string, method: 'PATCH' | 'POST', body: unknown }, fieldsSent: Partial<LoopFields> | null): Promise<AutoNudgeLoop> {
    ...
    if (fieldsSent) seeded.current = { ...(seeded.current ?? formFields()), ...fieldsSent }
    return written
  }
   ...
   *  the store) and the bridge's `onChange` guard then drops any later
   *  hand-off still running in this pressed render's closure. Two hand-offs --
   *  the written record, then the fired one -- lost the second: the schedule
   *  kept reading a full countdown and Trigger never disabled on the due cycle.
   *  A create handed up before the fire also re-keyed and remounted this
   *  popover mid-press, so a refusal on the fire leg landed on an instance
   *  that was gone. So: the fired record (armed deadline on it) when the fire
   *  lands; the WRITTEN record when the fire is refused -- the loop is saved,
   *  resumed or created either way, and the parent must learn that -- with
   *  the refusal rethrown into the inline notice. `written` is null for a
   *  press that wrote nothing (a pristine Trigger): a refused fire then hands
   *  up nothing, since nothing changed. */
    ...
    <Popover open={open} onOpenChange={onOpenChange}>
      ...
      >
        ...
        {/* THE ACTION ROW -- every control on ONE lane.
            ...
            IT (`editedFields`, see `runControl`): Play on a paused or stopped
            loop writes the edited fields with `active: true` and fires, or
            ...
            loop writes the edited fields and fires, or just fires. So an
              ...
              {loop?.active ? (
                <>
                  ...
                  {/* A write (`active: false` and nothing else), so it follows
                      ...
                      a pause sends no fields. It does follow Play's spent-bound
                      gate: a pause on a loop whose count sits at the form's cap,
                      or on a record carrying the budget reason, would record
                      `manual` over a bound that is spent -- see `boundSpent`. */}
                  <Btn
                    type="button"
                    className="!px-1.5"
                    ...
                    onClick={pause}
                    disabled={saving || writeDisabled || boundSpent}
                  >
                ...
                <Btn
                  ...
                  /* `boundSpent`: a revive the timer would turn away before
                     the nudge is not offered; the field that clears it brings
                     the control back. */
                  disabled={saving || writeDisabled || !message.trim() || boundSpent}
                >
