...
interface Props {
  ...
  /** Why those writes are disabled, rendered beside them. Without it the form is dead with no reason on screen. */
  writeDisabledReason?: ReactNode
  /**
...
export default function AutoNudgePopover({ slotKey, loop, open, onOpenChange, onChange, onSetUpBoundedMonitor, writeDisabled = false, writeDisabledReason, interrupted = false, trigger, content }: Props) {
    ...
    <Popover open={open} onOpenChange={onOpenChange}>
      ...
      >
        ...
        {/* The reason the fields below are dead. `writeDisabled` alone renders a
            ...
            reason attached. Passed in rather than derived here, so this editor
            keeps knowing nothing about session modes. */}
        {writeDisabledReason ? (
          <p
            role="status"
