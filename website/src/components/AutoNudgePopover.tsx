...
export default function AutoNudgePopover({ slotKey, loop, open, onOpenChange, onChange, onSetUpBoundedMonitor, writeDisabled = false, interrupted = false, trigger, content }: Props) {
  ...
  // Whether the USER changed each number this open, tracked PER FIELD: one shared flag marked
  // both dirty, so an untouched field's stale value rode out over a concurrent change to it.
  ...
  // Both arms require an actual edit: arming without one promises a destruction that
  // cannot happen.
  ...
  async function save(opts?: { keepStoredGoal?: boolean }) {
    ...
    if (needsOverwriteConfirm && !confirmOverwrite && !keepStoredGoal) {
      ...
      // Recomputed, never latched: a dismiss or reopen re-enters here, and a stale true announces a change that did not happen.
      setConfirmRearmed(fingerprintAtOpen !== null && liveFingerprint !== fingerprintAtOpen)
    ...
    try {
      ...
      // An omitted PATCH field means "unchanged" server-side (the handler reads a bare
      // `body.get`; `_update_unserialized` guards each assignment on `is not None`).
      ...
      if (loop && !keepStoredGoal) {
        ...
        const baseline = armed ?? loop.message_fingerprint
      ...
      if (resp.status === 409) {
        ...
        // REFETCH first: the compare gate keys on the served goal having changed, so without
        // this the next Save repeats the 409. Via the SLOT route -- a bare loop id 405s.
        try {
      ...
      // Read BEFORE the reset below: the notice reports what this save actually carried,
      // and a flag cleared first would make every kept-goal save claim settings changed.
      ...
      if (data.message_ignored === true) {
        // Split on the same flag the kept-goal arm uses: the settings half of this notice
        // would otherwise report a change the user never made on a goal-only save.
      ...
      setSaveNotice(null)
      if (goalEdited.current && data.loop?.message_redacted === true) {
        // The store kept text the user did not type. Closing here would let the reader nudge
        // with masked text and tell them only on reopen; the notice above says so in place.
        textareaRef.current?.focus()
        return
      }
      onOpenChange(false)
  ...
  return (
    <Popover open={open} onOpenChange={next => {
      // The trigger toggle closes through HERE, reaching neither hook below, so a guard
      // placed only on Escape and outside-click lets that one gesture discard silently.
          ...
          <div className="flex flex-col items-end gap-2 mt-2">
            ...
            <div className="flex flex-col gap-2 w-full">
              ...
              <Btn
                ...
                // Truly disabled, not aria-disabled: a click landing during an in-flight
                // save would submit the irreversible overwrite twice.
