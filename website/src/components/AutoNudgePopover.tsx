...
export default function AutoNudgePopover({ slotKey, loop, open, onOpenChange, onChange, onSetUpBoundedMonitor, writeDisabled = false, interrupted = false, trigger, content }: Props) {
  ...
  // `ignored` is the server declining the submitted text; `kept` is the user declining to overwrite. One state, not two booleans, since the two can never be true at once.
  const [saveNotice, setSaveNotice] = useState<'ignored' | 'ignored-goal-only' | 'kept' | 'kept-goal-only' | null>(null)
  ...
  // WHERE the refusal came from, not just that one happened: the explanation has to land next to the control the user actually pressed, or the X reads as a dead button.
  const [refusedFromClose, setRefusedFromClose] = useState(false)
  // Whether the USER typed in the goal textarea this open. The goal patch is gated on this, never on comparing against a `loop` a live update can replace underneath us.
  const goalEdited = useRef(false)
  // Whether the USER changed each number this open, tracked PER FIELD: one shared flag marked both dirty, so an untouched field's stale value rode out over a concurrent change to it.
  const idleEdited = useRef(false)
  const cyclesEdited = useRef(false)
  const anyNumericEdited = () => idleEdited.current || cyclesEdited.current
  const clearNumericEdited = () => { idleEdited.current = false; cyclesEdited.current = false }
  const parseIdle = (s: string) => parseInt(s, 10) || 60
  const parseCycles = (s: string) => parseInt(s, 10) || 0
  // Latched at OPEN, because both inputs below are live: a websocket update replacing `loop` with a newer clean goal must neither disarm the gate nor become its own baseline.
  // No serializer emits `message_redacted` (`_serialize` is asdict(NudgeLoop)), so derive it from the served text. All three write-path redactors share one prefix: `[REDACTED: credential]`, `[REDACTED: encoded credential]` (security/redaction.py:898,911) and `[REDACTED: suspicious URL to ...` (security/exfil.py:1078).
  const isRedacted = (text?: string) => !!text && text.includes('[REDACTED: ')
  const loopRedacted = Boolean(loop?.message_redacted) || isRedacted(loop?.message)
  const [redactedAtOpen, setRedactedAtOpen] = useState(false)
  const [servedAtOpen, setServedAtOpen] = useState<string | null>(null)
  // Latched at OPEN like ``servedAtOpen``: arming on the LIVE token let a goal replaced before the first Save click become its own baseline, so the 409 could never fire.
  const [fingerprintAtOpen, setFingerprintAtOpen] = useState<string | null>(null)
  // The goal the CONFIRM was armed on, latched when the gate appears. A render-time check cannot close the race: the live goal can move between the read and the click.
  const [confirmArmedFor, setConfirmArmedFor] = useState<string | null>(null)
  // Re-arming is otherwise signalled ONLY by the preview text changing, which a screen reader is never told, so the gate silently swallows the click that looked like a yes.
  const [confirmRearmed, setConfirmRearmed] = useState(false)
  // The BASELINE the confirm was armed on. Latched apart from the text above, which is a redacted projection two different goals can share and so cannot identify one.
  const [confirmArmedFingerprint, setConfirmArmedFingerprint] = useState<string | null>(null)
  // Both arms require an actual edit: arming without one promises a destruction that cannot happen.
  const editsRedactedGoal =
    goalEdited.current &&
    (loopRedacted || redactedAtOpen) &&
    message !== (loop?.message ?? '')
  // The stored goal changed under an edit already in progress, so saving the typed text would discard a goal this user never saw. Same irreversibility, same explicit act.
  ...
  // The confirm lives BELOW the action row, never in Save's position: swapping it in where Save was let a double-click land on it, defeating the gate.
  const confirmPending = confirmOverwrite && needsOverwriteConfirm
  const dismissRef = useRef<HTMLButtonElement | null>(null)
  const textareaRef = useRef<HTMLTextAreaElement | null>(null)

  // Radix routes Escape, an outside click and the X through three separate hooks; ONE flag backs all three, since a per-path copy leaves one path discarding silently.
  const dismissWouldDiscard = useCallback(() => {
    // The numbers are lost by the same gesture as the text, so both compare against the open-edge seed (`||`, so a stored 0 matches the shown default).
    const numericUnsent =
      anyNumericEdited() &&
      !!loop &&
  ...
  const refuseDismissOnce = useCallback(() => {
    ...
    // Unconditional: gating this on the gate being DOWN made an armed-gate dismissal cost three presses, the first of which collapsed the gate and explained nothing.
    setEscapeWarned(true)
    textareaRef.current?.focus()
  }, [])

  // Disabling Save drops focus to <body>, so land it on the arm that WRITES NOTHING -- the keep-stored arm still PATCHes, so a habituated second Enter there commits a partial save.
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    clearNumericEdited()
    // Latched HERE, at open, not at the first keystroke: a live update landing between the render and that keystroke would otherwise become its own baseline and pass unnoticed.
    setRedactedAtOpen(loopRedacted)
    ...
    setConfirmClear(false)
    // Same reason for the overwrite gate: an armed confirmation surviving a dismiss would let the next Save replace a redacted goal with no fresh confirmation.
    setConfirmOverwrite(false)
  ...
  useEffect(() => {
    if (!open || !loop) return
    if (!idleEdited.current) setIdleInput(String(loop.idle_secs || 60))
    if (!cyclesEdited.current) setMaxCyclesInput(String(loop.max_cycles || 0))
  }, [open, loop])
  ...
  async function save(opts?: { keepStoredGoal?: boolean }) {
    ...
    // ``=== true`` on purpose: a call site that forwards a DOM event as the first argument must never enable this, only an explicit caller.
    const keepStoredGoal = opts?.keepStoredGoal === true
    // The overwrite is IRREVERSIBLE and the server cannot return the original, so an edit to a redacted goal needs an explicit act, not passive copy the user skims.
    if (needsOverwriteConfirm && !confirmOverwrite && !keepStoredGoal) {
      const liveFingerprint = loop?.message_fingerprint ?? ''
      // The stored goal can move BEFORE this first click; adopting that token made ``expect_fingerprint`` match server-side, so the 409 could not fire. Recomputed, never latched.
      setConfirmRearmed(!!liveFingerprint && fingerprintAtOpen !== null && liveFingerprint !== fingerprintAtOpen)
      setConfirmArmedFor(loop?.message ?? '')
      // Latch ONLY a truthy token. '' is non-nullish, so storing it defeats both the armed baseline and the open-edge fallback, and the fence stops being offered anything at all.
      if (liveFingerprint) { setConfirmArmedFingerprint(liveFingerprint); setFingerprintAtOpen(liveFingerprint) }
      setConfirmOverwrite(true)
      return
    }
    // Past the gate, the stored goal must still be the one the confirm was armed on: a click answering a question about text no longer there re-arms instead of committing.
    if (confirmOverwrite && !keepStoredGoal && (loop?.message ?? '') !== confirmArmedFor) {
      setConfirmArmedFor(loop?.message ?? '')
      if (loop?.message_fingerprint) { setConfirmArmedFingerprint(loop.message_fingerprint); setFingerprintAtOpen(loop.message_fingerprint) }
    ...
    try {
      ...
      // An omitted PATCH field means "unchanged" server-side (the handler reads a bare `body.get`; `_update_unserialized` guards each assignment on `is not None`).
      const patch: Record<string, unknown> = { active: true }
      if (idleEdited.current) patch.idle_secs = idle_secs
      if (cyclesEdited.current) patch.max_cycles = max_cycles
      if (!keepStoredGoal && goalEdited.current) patch.message = message
      if (loop && !keepStoredGoal && patch.message !== undefined) {
        // The baseline the confirm was ARMED on, never the live one, which carries the newer goal's own token; read only while the gate is up so no latch outlives its confirm.
        const armed = confirmOverwrite ? confirmArmedFingerprint : null
        // A live `autonudge_state` frame is nine hand-named keys with no goal_token (slack/gateway.py:7885), so it replaces `loop` with a fingerprint-less copy and the fence would stop being offered a baseline mid-edit. The open-edge latch still holds the authoritative one.
        const baseline = armed || loop.message_fingerprint || fingerprintAtOpen
        if (baseline) patch.expect_fingerprint = baseline
      }
      const resp = loop
      ...
      if (resp.status === 409 && data.code !== 'structured_monitor_requires_monitor_api') {
        ...
        // REFETCH first: the compare gate keys on the served goal having changed, so without this the next Save repeats the 409. Via the SLOT route -- a bare loop id 405s.
        try {
          ...
          // SURFACED, not swallowed: the stale fingerprint makes the next Save repeat this same 409, so hiding the refetch failure hides why the retry never converges.
          throw new Error(i18nT('components.autoNudgePopover.conflict_reload_failed'))
        }
        setError(data.error || i18nT('components.autoNudgePopover.confirm_overwrite_moved_question'))
        return
      }
      if (!resp.ok) throw new Error(data.error || `HTTP ${resp.status}`)
      // Read BEFORE the reset below: the notice reports what this save actually carried, and a flag cleared first would make every kept-goal save claim settings changed.
      const numericWasEdited = anyNumericEdited()
      setConfirmOverwrite(false)
      // The response IS the new served state. Leaving the open-edge latches behind would arm the moved-goal gate against the user's own just-saved text on every stay-open path below.
      setServedAtOpen(data.loop?.message ?? '')
      setFingerprintAtOpen(data.loop?.message_fingerprint ?? data.loop?.goal_token ?? '')
      // Clear a flag only while its input still holds what THIS save submitted: a keystroke landing during the in-flight PATCH is newer than the response, and clearing would let the reseed below overwrite it.
      if (parseIdle(latest.current.idleInput) === idle_secs) idleEdited.current = false
      if (parseCycles(latest.current.maxCyclesInput) === max_cycles) cyclesEdited.current = false
      if (data.message_ignored === true) {
        // Split on the same flag the kept-goal arm uses: the settings half of this notice would otherwise report a change the user never made on a goal-only save.
        setSaveNotice(numericWasEdited ? 'ignored' : 'ignored-goal-only')
        // The confirm button the user pressed unmounts with the gate, so focus would fall to <body> here too; the notice below is announced via role="status".
        textareaRef.current?.focus()
        onChange(data.loop)
        return
      }
      onChange(data.loop)
      if (keepStoredGoal) {
        // NOT `ignored`: nothing was ignored, the user declined the overwrite, and that notice would blame a text match and ask for the retry they just refused.
        setSaveNotice(numericWasEdited ? 'kept' : 'kept-goal-only')
        // The user is still editing: closing reseeds the textarea from the served projection on reopen, and no draft covers it while a loop exists.
        textareaRef.current?.focus()
        return
      }
      setSaveNotice(null)
      if (goalEdited.current && (data.loop?.message_redacted === true || isRedacted(data.loop?.message))) {
        // The store kept text the user did not type. Closing here would let the reader nudge with masked text and tell them only on reopen; the notice above says so in place.
        textareaRef.current?.focus()
        return
      }
      // A keystroke that landed during the in-flight PATCH is newer than what this save carried, and no draft covers the goal while a loop exists, so closing would discard it.
      if (latest.current.message !== message) {
        textareaRef.current?.focus()
        return
      }
      onOpenChange(false)
  ...
  return (
    <Popover open={open} onOpenChange={next => {
      // The trigger toggle closes through HERE, reaching neither hook below, so a guard placed only on Escape and outside-click lets that one gesture discard silently.
      if (!next && dismissWouldDiscard()) {
        setRefusedFromClose(false)
        refuseDismissOnce()
        return
      }
      ...
      >
        ...
        <textarea
          ...
          onChange={e => {
            ...
            // A fresh edit re-arms the discard warning: the text at stake is not the text the previous warning was about.
            setEscapeWarned(false)
            setRefusedFromClose(false)
            // An armed confirmation answers the text it was armed FOR. Reverting to the redacted copy and editing again reused it, overwriting with no second ask.
            setConfirmOverwrite(false)
            setSaveNotice(null)
        ...
        <div className="flex flex-col gap-3 mb-3 sm:flex-row">
          <div className="flex-1">
            ...
            <input
              ...
              onChange={e => {
                hasEdited.current = true
                idleEdited.current = true
                // A fresh edit re-arms the discard warning, exactly as the goal textarea does: the value at stake is not the one the previous warning was about.
                setEscapeWarned(false)
                setRefusedFromClose(false)
        ...
        {(saveNotice === 'ignored' || saveNotice === 'ignored-goal-only') && (
          <div role="status" data-testid="autonudge-ignored-fields" className="text-warn text-[11px] mb-2">
            {i18nT(saveNotice === 'ignored'
              ? 'components.autoNudgePopover.ignored_fields_notice'
              : 'components.autoNudgePopover.ignored_fields_only_notice')}
          </div>
        )}
        ...
        {saveNotice === 'kept-goal-only' && (
          <div role="status" data-testid="autonudge-kept-stored-goal" className="text-muted text-[11px] mb-2">
            {i18nT('components.autoNudgePopover.kept_stored_goal_only_notice')}
          </div>
        )}
          ...
          {loop && (
            loop.active ? (
              ...
              /* On an already-stopped loop this press REMOVES the record, which
                 ...
                 `Btn danger` colours it rather than leaving it to :hover,
              ...
              <Btn
                type="button"
                danger={!confirmPending}
                onClick={() => {
                  // The two gates are mutually exclusive: an armed overwrite beside this erase puts two destructive asks on one screen, so the erase also recedes while that gate holds the screen.
                  setConfirmOverwrite(false)
          ...
          <div className="flex flex-col items-end gap-2 mt-2">
            {confirmRearmed && (
              <p
                ...
                className="m-0 self-stretch text-warn text-[12px] font-medium"
              >
            ...
            {goalMovedUnderEdit && (
              // The choice is irreversible and the newer text is in hand here, so show it rather than asking the user to discard something they have never read.
              <p
                data-testid="autonudge-moved-goal-preview"
                // eslint-disable-next-line jsx-a11y/no-noninteractive-tabindex -- it scrolls, so with no tab stop a keyboard reader cannot reach the tail
                tabIndex={0}
                role="region"
              ...
              >
                ...
                {i18nT('components.autoNudgePopover.moved_goal_preview', {
                  // Shown IN FULL and scrolled rather than truncated: the click authorises destroying all of it, so an elided tail hides part of the decision.
                  goal: confirmArmedFor ?? '',
                })}
                {loopRedacted && (
                  // The preview is the SERVED projection, so this arm can ask the user to discard a goal they were shown only as a mask. Say so at the decision.
            ...
            <div className="flex flex-col gap-2 w-full">
              <Btn
                ...
                onClick={() => {
                  // The WRITE-FREE exit: both other buttons PATCH, and the only silent dismissal was Escape, which DISCARDS the typed goal.
                  setConfirmOverwrite(false)
                  textareaRef.current?.focus()
                }}
                onKeyDown={e => {
                ...
                // GHOST, not filled: this is the only arm that writes NOTHING, so it must not share a shape with the arm that saves the other settings.
              ...
              <Btn
                ...
                onClick={() => {
                  // Dismiss the gate ONLY. Restoring the served text here discarded the user's typed goal, which no draft covers while a loop exists.
                  setConfirmOverwrite(false)
                  // This button unmounts with the gate, so focus would fall to <body> on the gate's own SAFE path. Land it on the text the user was editing.
                  textareaRef.current?.focus()
                  // Answers the GOAL question, not the whole form: persist the other settings so a changed interval is not silently dropped.
                  save({ keepStoredGoal: true })
                }}
                onKeyDown={e => {
                  // Same guard as the overwrite button: this one is focused on mount, so a repeating Enter from Save would otherwise dismiss the gate unseen.
                  if ((e.key === 'Enter' || e.key === ' ') && e.repeat) e.preventDefault()
                }}
                // OUTLINED: it does write (the other settings), so it is not the ghost arm, and it is not destructive, so it is not the danger arm.
                className="bg-card"
              >
                {/* On a PAUSED loop this arm also resumes it, so it must say so --
                    the same split base Save makes between save and start_loop. The
                    parenthetical is dropped when no numeric edit is pending, because
                    then this arm saves nothing the user typed. */}
                {loop?.active
                  ? i18nT(
                      anyNumericEdited()
                        ? 'components.autoNudgePopover.keep_original_goal'
                        : 'components.autoNudgePopover.keep_original_goal_only'
                    )
                  : i18nT(
                      anyNumericEdited()
                        ? 'components.autoNudgePopover.keep_original_goal_start'
                        : 'components.autoNudgePopover.keep_original_goal_only_start'
                    )}
              </Btn>
              ...
              <Btn
                ...
                // Truly disabled, not aria-disabled: a click landing during an in-flight save would submit the irreversible overwrite twice.
