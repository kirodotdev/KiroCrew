...
describe('MoveToCrewDialog', () => {
  it('keeps a blank target out of the failure surface', async () => {
    ...
    renderDialog(<MoveToCrewDialog unitId="j1" onPlan={onPlan} onClose={() => {}} />)
  ...
  it('dismisses the crew lookup failure notice, which is derived rather than stored', async () => {
    // The notice text comes from the query error, not from state, so a dismiss
    // wired only to the stored error leaves a visible X that does nothing.
    listInstances.mockRejectedValue(Object.assign(new Error('HTTP 500'), { status: 500 }))
    const onPlan = vi.fn().mockResolvedValue({ ok: true, plan })
    renderDialog(<MoveToCrewDialog unitId="j1" onPlan={onPlan} onClose={() => {}} />)
  ...
  it('shows advisory findings so a degraded move is never silent', async () => {
    ...
    renderDialog(<MoveToCrewDialog unitId="chat-3" onPlan={onPlan} onClose={() => {}} />)
  ...
  it('surfaces a planner error instead of a blank dialog', async () => {
    ...
    renderDialog(<MoveToCrewDialog unitId="j9" onPlan={onPlan} onClose={() => {}} />)
    await userEvent.type(screen.getByLabelText(/target crew/i), 'dst')
    await userEvent.click(screen.getByRole('button', { name: /preview move/i }))
  ...
  it('closes on cancel without planning', async () => {
    const onClose = vi.fn()
    const onPlan = vi.fn()
    renderDialog(<MoveToCrewDialog unitId="j1" onPlan={onPlan} onClose={onClose} />)
  ...
  it('names the outcome, so no label promises a move the code cannot perform', () => {
    renderDialog(<MoveToCrewDialog unitId="j1" onPlan={vi.fn()} onClose={() => {}} />)
  ...
  it('uses the shared modal primitive and closes on Escape', async () => {
    const onClose = vi.fn()
    renderDialog(
      <MoveToCrewDialog unitId="j1" onPlan={vi.fn()} onClose={onClose} />,
    )
  ...
  it('uses the shared error notice for planner failures', async () => {
    ...
    renderDialog(
      <MoveToCrewDialog unitId="j1" onPlan={onPlan} onClose={() => {}} />,
    )
    await userEvent.type(screen.getByLabelText(/target crew/i), 'dst')
    await userEvent.click(screen.getByRole('button', { name: /preview move/i }))
