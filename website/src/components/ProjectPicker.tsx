...
export default function ProjectPicker({ open, onOpenChange, anchorRef, anchorRect, onSelect }: Props) {
  ...
  // Only a RECOVERY unmounts the button, so a repeated failure must hand focus back once the
  // re-enabling render commits. Keyed by which Retry was pressed: both can be on screen.
  const refocusRetry = useRef<null | 'recent' | 'listing'>(null)
  useEffect(() => {
    if (retrying || !refocusRetry.current) return
    const target = refocusRetry.current === 'recent' ? recentRetryRef : listRetryRef
    refocusRetry.current = null
  ...
  const { listError, browse, retry } = useBrowseDirs(open, (d, preserveInput) => {
      ...
      {(tab === 'recent' || !picked) && (recentError || retrying === 'recent') && (
        <div className="px-3 py-2 border-b border-border flex items-center gap-2">
          ...
          <Btn
            type="button"
            ref={recentRetryRef}
            // preventDefault on mousedown only, so the pointer does not blur the input; the
            // work stays on click, which is also what Enter on the focused button fires.
            ...
            onClick={() => {
              ...
              void refetchRecent().then(
                ...
                if (ok) {
                  ...
                  target?.focus()
                } else refocusRetry.current = 'recent'
              })
            }}
          ...
          >
            <RefreshCw
              size={10}
              className={retrying === 'recent' ? 'animate-spin inline-block mr-1' : 'invisible inline-block mr-1'}
            />
            ...
            {listBusy && (
              <div className="px-3 py-4 flex items-center gap-2">
                ...
                {listRecoverable && <Btn
                  ...
                  onClick={() => {
                    ...
                    void retry().then(ok => {
                      ...
                      else refocusRetry.current = 'listing'
                    })
                  }}
                ...
                >
                  <RefreshCw
                    size={10}
                    className={retrying === 'listing' ? 'animate-spin inline-block mr-1' : 'invisible inline-block mr-1'}
                  />
                  {i18nT('components.projectPicker.retry')}
                </Btn>}
              </div>
