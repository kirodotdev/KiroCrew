...
export default function FilePickerMenu({ query, anchorRef, open, onSelect, onClose, onFileOpen, project, sendOnEnter = 'enter' }: Props) {
  ...
  // The `_enter_sends` half promises a key, so it may only be spoken while that key is released.
  // Focus must not change it: rewriting an alert region mid-read moves text under a reader.
  ...
  const retryButton = (shownCause !== 'timed_out' && shownCause !== 'failed') ? null : (
    <RetryControl
      busy={retrying}
      controlRef={retryRef}
      label={i18nT('components.filePickerMenu.retry')}
      ...
      onRetry={() => {
        ...
        return refetch().then(r => !r.isError, () => false).then(ok => {
          setRetrying(false)
