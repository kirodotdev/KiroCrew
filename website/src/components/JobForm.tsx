...
import { defaultFireLocal, fireTimeLocal } from './ScheduleLaterPopover'
...
function parseJobDefaults(job?: CronJob) {
  ...
  return { name: job.name, message: job.message, agent: job.agent || '', model: job.model || '', channel: job.channel || '', approvalMode: job.approval_mode || '', silent: job.silent || false, strictSchedule: job.strict_schedule || false, hideInChat: job.hide_in_chat || false, minimalContext: job.minimal_context || false, chatFolderId: job.chat_folder_id || '', jobKind: jobKindOf(job), schedMode, intVal, intUnit, weekDays, weekTime, cronExpr: cronRaw, onceLocal, onceLocalInitial: onceLocal }
}
...
export default function JobForm({ job, prefill, agents, defaultAgent, rosterFailure, lockedAgent, memberId, memberNoun, providerAgent, onSaved, layout = 'horizontal', externalSubmit, submitRef, onSavingChange, onSubmitError, onDirtyChange }: Props) {
  ...
  const submit = async () => {
    ...
    try {
      ...
      if (!job) { setName(''); setMsg(''); setWeekDays([]); setIntVal(1); setOnceLocal(''); setChannel(''); setModel(''); setApprovalMode(''); setSilent(false); setStrictSchedule(false); setHideInChat(false); setMinimalContext(false); setChatFolderId('') }
