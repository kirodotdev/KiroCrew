...
export function FolderPickerItems({ folders, onPick, currentFolderId, rootLabel = 'No folder (root)', Item, offlineVerb }: {
  ...
  // Read here: a prop defaulting to connected left ArtifactDetailPage's picker ungated.
  ...
  return (
    <>
      <Item
        ...
        onSelect={(event) => { refuse(event); if (connected) onPick(null) }}
      >
      ...
      {ordered.map(({ folder: f, depth, path }) => (
        <Item
          ...
          onSelect={(event) => { refuse(event); if (connected) onPick(f.id) }}
        >
  ...
  return (
    <Sub>
      ...
      <SubContent className="min-w-[170px] max-h-[280px] overflow-y-auto">
        ...
        {/* LAST, as the top-level menu does: a reconnect unmounts this row, and above the destinations it would shift them under a mid-aim pointer. */}
        {!connected && <OfflineMenuReason testId="folder-move-offline-reason" reason={i18nT('utils.offline.gateway_offline_reconnect', { action: verb })} />}
      </SubContent>
    </Sub>
  )
}
