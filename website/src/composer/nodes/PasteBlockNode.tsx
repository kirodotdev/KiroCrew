...
function PasteBlockChipInEditor({
  ...
  const { openPreview, removeBlock } = usePasteBlocks()
  ...
  const onOpen = useCallback(
    (el: HTMLElement) => {
      ...
      // parks the caret right after it again.
      setSelected(true)
      openPreview(seq, el)
    },
    [openPreview, seq, setSelected],
  )

  const onRemove = useCallback(() => {
    // Remove the node from the tree; the host's snapshot drops the block.
    editor.update(() => {
      $getNodeByKey(nodeKey)?.remove()
    })
    removeBlock(seq)
  }, [editor, nodeKey, removeBlock, seq])
