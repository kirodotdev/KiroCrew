...
export interface PastePreviewEditorProps {
  ...
  lines: number
  /** Save the (possibly edited) content. */
  onSave: (content: string) => void
  /**
   * Dismiss without saving. Reached from Cancel, from Escape (twice when there
   * are unsaved edits — see below), or from a pointerdown outside the panel
 ...
 * - A pointerdown outside the panel SAVES unsaved edits (`onSave`) and closes;
 ...
 *   Escape (or Cancel) discards. Escape the IME owns (cancelling a candidate
 *   list) is claimed by the shared latch and never reaches the panel.
    ...
    open && anchorRect ? (
        ...
        >
          ...
          <textarea
            ...
            value={value}
            onChange={(e) => { setValue(e.target.value); setDiscardArmed(false) }}
            aria-label={i18nT('components.pastePreviewEditor.editor_aria')}
