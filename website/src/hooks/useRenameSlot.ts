/**
 * The single rename commit path, shared by the sidebar row and ChatPage editors.
 *
 * ONE commit per slot at a time: two PATCHes of one field racing on the wire let
 * the server apply the older one last, and no client bookkeeping can reorder what
 * already arrived. Hence no sequence numbers and no per-rename ack cache.
 *
 * That does not answer whether the server spoke while the request was open: a
 * pushed title event can carry the string that was typed, so comparing values
 * cannot tell an independent confirmation from this client's own guess. The
 * generation counters answer that, and only that.
 *
 * The in-flight set is MODULE-level, not `useRef`: either surface can rename a
 * slot, and per-component state would let each start one.
 */
