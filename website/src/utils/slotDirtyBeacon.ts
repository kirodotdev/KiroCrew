 ...
 */
type DirtyClaim = { s: string; t: number }
...
export function publishSlotDirty(
  ...
  const claim: DirtyClaim = { s: slot, t: Date.now() }
...
export function retractSlotDirty(composerId: string): void {
  publishSlotDirty(composerId, null, false)
...
export function anyWindowSlotDirty(slot: string): boolean {
  ...
  try {
    for (const key of claimKeys()) {
      ...
      if (now - claim.t < CLAIM_TTL_MS) return true
    }
  } catch {
...
export const WORST_CLAIM_WRITE_BYTES =
  ...
  + JSON.stringify({ s: 'p'.repeat(PROBE_SLOT_PAD), t: Date.now() }).length
