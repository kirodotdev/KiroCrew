 ...
 *
 * NOT lowercased. `toLowerCase` here and `str.lower()` in the Python reader each
 * consult their own runtime's Unicode tables, so a character whose case mapping
 * differs between the browser's version and the interpreter's folds differently on
 * the two sides — and neither side owns both tables, so no code can close that
 * skew. Comparing unfolded never consults them. The tie-break is therefore
 * case-sensitive, which the ordering does not need: `order` is the sort key and
 * the name only separates rows that share one.
 *
 ...
 */
const folderName = (f: ChatFolder): string => (typeof f.name === 'string' ? f.name : '')
