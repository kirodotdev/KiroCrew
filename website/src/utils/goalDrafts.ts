...
function readLegacyInput(slot: string): LegacyGoalDraftInput | null {
  ...
  try {
    ...
    const isAtomicEnvelope = row.__slotDraftStore === 1
      && row.drafts && typeof row.drafts === 'object' && !Array.isArray(row.drafts)
      && row.timestamps && typeof row.timestamps === 'object' && !Array.isArray(row.timestamps)
    const drafts = isAtomicEnvelope
      ? row.drafts as Record<string, unknown>
      : row
    const stored = sanitizeStoredGoalDraft(drafts[slot])
    if (!stored) return null
    let timestampValue: unknown
    if (isAtomicEnvelope) {
      timestampValue = (row.timestamps as Record<string, unknown>)[slot]
    } else {
