...
export function settlePinnedArrangementAgainstMembership(pinnedKeys: readonly string[]): void {
  ...
  // Skipped, not remembered: retaining it needed a settled-batch hook that fires before the commit
  // seam, so it forgot the very marker that seam was about to justify keeping.
 ...
 */
/** Requested in review on this PR by bolichen97, who declined the upgrade trade that drops an
 * order a user dragged before the marker existed. */
