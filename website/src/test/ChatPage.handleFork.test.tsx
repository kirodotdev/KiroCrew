...
describe('handleFork surfaces its failure on the shared action-error notice', () => {
  ...
  it('carries no retry affordance: one consumer did not justify a permanent wire field', () => {
    ...
    // Direction-specific on purpose: the generic wording asked the reader to move in a
    ...
    // The action row is opacity-0 until hover OR focus (ACTIONS_REVEAL_CLS), and the
    // quoted string is only its tooltip: no visible "fork icon" is on screen to find.
    for (const copy of [
      chat.fork_too_large_error_head,
      chat.fork_too_large_error_tail,
      chat.fork_too_large_offsite_unknown,
    ]) {
      expect(copy).not.toMatch(/fork icon/i)
      expect(copy).toMatch(/hover or focus/i)
    }
  })
  ...
  it('inherits the shared notice clearing rule instead of restating it', () => {
    ...
    expect(handler).not.toContain('setActionError(')
    const switcher = src.slice(src.indexOf('knowledgeFetchRef.current.clearResults()'))
    // A pattern, not a literal: the switcher clears conditionally so an error flagged
    // preserveOnSwitch survives, and this test only needs it to still resolve to null.
    expect(switcher.slice(0, 600)).toMatch(/setActionError\([^)]*null/)
  })
})
