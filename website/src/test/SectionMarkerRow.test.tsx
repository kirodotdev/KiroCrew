...
describe('the SDK path draws the rule', () => {
  it('titles a deferred marker with both the requested and the append instant', () => {
    const rows: React.ReactNode[] = []
    const ctx = { row: (node: React.ReactNode) => rows.push(node) } as never
    const entry = resolveRenderer(
      marker({ ts: '2026-03-20T14:41:09Z', meta: { label: 'item-42', at: '2026-03-20T14:38:07Z' } }),
      defaultMessageRenderers,
    )
    entry!.render(
      marker({ ts: '2026-03-20T14:41:09Z', meta: { label: 'item-42', at: '2026-03-20T14:38:07Z' } }),
    ...
    expect(stamp!.getAttribute('title')).toContain(' · ')
    expect(stamp!.getAttribute('title')).toMatch(/Requested/)
    expect(stamp!.getAttribute('title')).toMatch(/added/)
    expect(stamp!.getAttribute('title')!.split(' · ').length).toBe(2)
  })
