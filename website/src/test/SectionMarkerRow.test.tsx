...
describe('the capture scene applies the theme the stylesheet keys on', () => {
  // A frame is only evidence if it photographs the shipped theme. An earlier draft of this
  // scene set a bare `data-theme`, so its light frame showed a row no user sees.
  const scene = (name: string): string =>
    readFileSync(resolve(here, `../../capture/${name}.tsx`), 'utf8')

  it('sets the mode attribute the stylesheet keys on', () => {
    const name = 'transcript-section-markers'
    expect(scene(name), `${name} does not set data-mode`).toMatch(/dataset\.mode\s*=/)
  })

  it('sets the prefixed theme value, not the bare one', () => {
    const name = 'transcript-section-markers'
