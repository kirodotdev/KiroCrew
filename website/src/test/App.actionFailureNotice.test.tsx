...
import instancesReducer, { setActiveId } from '../store/instancesSlice'
import App from '../App'
...
describe('a rejection that lands while a remote instance fills the window', () => {
  /**
   * The local pane is hidden, not unmounted, while a remote tab is active
   * (App.tsx: `display: activeInstanceId === null ? 'block' : 'none'`), and the
   * store never auto-clears. So a pin rejected after the user switched tabs is
   * DEFERRED — reported into the hidden pane and shown the moment Local is back —
   * never lost. It is deliberately NOT painted over the remote pane: that pane is
   * another crew's whole dashboard, with its own copy of this sink, so a local
   * failure shown there would read as that crew's.
   */
  it('is kept in the local pane and shown again on return to Local', async () => {
    const { store } = renderAt('/capabilities')
    await waitFor(() => { expect(screen.getByTestId('capabilities-page')).toBeInTheDocument() })
    const localPane = screen.getByTestId('dashboard-shell').parentElement!
    expect(localPane.style.display).toBe('block')
    act(() => { store.dispatch(setActiveId('crew-a')) })
    expect(localPane.style.display).toBe('none')

    __resetActionFailureForTests()
    act(() => { reportActionFailure("Couldn't change this session's pin — the change was undone.", 'Research notes') })
    const notice = await screen.findByTestId('session-action-error')
    // Still rendered, still carrying the report — inside the hidden local pane,
    // and only there.
    expect(notice.textContent).toContain('Research notes')
    expect(localPane.contains(notice)).toBe(true)
    expect(screen.getAllByTestId('session-action-error')).toHaveLength(1)

    act(() => { store.dispatch(setActiveId(null)) })
    expect(localPane.style.display).toBe('block')
    const shown = screen.getByTestId('session-action-error')
    expect(shown).toBe(notice)
    expect(shown.textContent).toContain('Research notes')
  })
})
