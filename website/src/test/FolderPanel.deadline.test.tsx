...
describe('FolderPanel — bounded /api/file-search', () => {
  ...
  it('names a listing 400 as a missing folder, offering no remedy it cannot honour', async () => {
    // A path that is no longer a directory answers `not_a_directory`, a code the shared map does
    // not carry — so it degraded to a RETRYABLE failure and promised a Refresh that re-fails.
    const { ApiError } = await import('../api/apiError')
  ...
  it('leaves an UNNAMED 400 from the listing generic, rather than guessing from the status', async () => {
    // The bare `status === 400` sniff this replaced would have called ANY 400 a missing folder,
    // so a future 400 from this endpoint would have inherited the wrong copy silently.
    const { ApiError } = await import('../api/apiError')
    vi.spyOn(api, 'browseFiles').mockRejectedValue(
  ...
  it('reserves the Refresh label while a SEARCH is in flight, not only once it fails', async () => {
    // `refreshBusy` tracks the listing read alone, so the label grew only after the search had
    // already failed — the morph landed under a cursor reaching for the control it names.
    const label = () => screen.getByLabelText('Refresh').querySelector('span')
    let release: (v: unknown) => void = () => {}
    vi.spyOn(api, 'fileSearch').mockImplementation(
      (() => new Promise(r => { release = r })) as never)
    renderPanel()
    // Control: listing settled, nothing in flight, nothing failed — width unreserved.
    expect(await screen.findByText('README.md')).toBeInTheDocument()
    await waitFor(() => expect(label()).toBeNull())

    await search('zz')
    await waitFor(() => expect(label()).not.toBeNull())
    release({ results: [] })
  })
