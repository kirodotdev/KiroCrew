...
import { useFileScrollMemory } from '../apps/file-explorer/useFileScrollMemory'
import {
...
import { withPreviewCsp } from '../apps/file-explorer/previewCsp'
...
describe('OfficeViewer', () => {
  ...
  it('surfaces extraction failures through ErrorNotice, not an empty pane', async () => {
    vi.stubGlobal('fetch', vi.fn(async () => ({
      ok: false, status: 415, json: async () => ({}),
      text: async () => 'zzz not extractable',
    }) as unknown as Response))
    render(withQueryClient(<OfficeViewer path="/zzz/f.docx" />))
    // A document that exists but will not parse is a FAILURE, so it must carry
    // the shared error surface (with its agent hand-off), not an EmptyState.
    const notice = await waitFor(() => screen.getByTestId('fe-extract-error'), { timeout: 5000 })
    expect(notice.textContent).toMatch(/zzz not extractable|415/)
  })
})

describe('withPreviewCsp', () => {
  // sandbox="" stops scripts but NOT passive loads, so a saved page's
  // <img src="http://…"> would still beacon out or probe the local network on
  // preview. These assert the policy lands where the parser will honour it.
  // Position of the META TAG, not of the header name inside it: the name sits
  // 18 chars into `<meta http-equiv="…`, so measuring the name made "prepended"
  // read as index 18 and the first version of these assertions was simply wrong
  // about its own helper.
  const CSP_AT = (s: string) => s.indexOf('<meta http-equiv="Content-Security-Policy"')

  it('keeps a leading doctype first and puts the policy straight after it', () => {
    const out = withPreviewCsp('<!DOCTYPE html><html><head><title>t</title></head><body>x</body></html>')
    expect(out.indexOf('<!DOCTYPE html>')).toBe(0)   // no quirks mode
    expect(CSP_AT(out)).toBe('<!DOCTYPE html>'.length)
    expect(out).toContain("default-src 'none'")
  })

  it('is not fooled by a <head> inside a comment (the bypass)', () => {
    // The regression: a free /<head\b[^>]*>/ match landed the meta INSIDE this
    // comment, where it is inert, and the image then fired.
    const evil = '<!-- <head> --><img src="https://attacker.example/px.gif">'
    const out = withPreviewCsp(evil)
    expect(CSP_AT(out)).toBe(0)                       // policy is first, not in the comment
    expect(CSP_AT(out)).toBeLessThan(out.indexOf('<!--'))
    expect(CSP_AT(out)).toBeLessThan(out.indexOf('<img'))
  })

  it('prepends for a bare fragment and for a commented doctype', () => {
    expect(CSP_AT(withPreviewCsp('<p>hello</p>'))).toBe(0)
    // A doctype that is not really leading must not be treated as one.
    const out = withPreviewCsp('<!-- <!DOCTYPE html> --><img src="http://x/">')
    expect(CSP_AT(out)).toBe(0)
    expect(CSP_AT(out)).toBeLessThan(out.indexOf('<img'))
  })

  it('tolerates leading whitespace before the doctype', () => {
    const out = withPreviewCsp('\n  <!doctype html><html><body>x</body></html>')
    expect(out.trimStart().startsWith('<!doctype html>')).toBe(true)
    expect(CSP_AT(out)).toBeLessThan(out.indexOf('<body>'))
  })

  it('permits only what a self-contained page needs to look right', () => {
    const csp = withPreviewCsp('<p>x</p>')
    expect(csp).toContain("style-src 'unsafe-inline'")
    expect(csp).toContain('img-src data:')
    expect(csp).not.toContain('http:')
    expect(csp).not.toContain("script-src 'unsafe-inline'")
    for (const directive of ['connect-src', 'frame-src', 'media-src']) {
      // Covered by default-src 'none'; assert none was re-opened.
...
describe('useMarkdownEditor', () => {
  ...
  it('shows the conflict warning on a 409 without clobbering state', async () => {
    ...
    vi.spyOn(fileExplorerApi, 'write').mockRejectedValue(new ApiError(409, 'changed'))
    render(<Harness path="/zzz/notes.md" />)
    fireEvent.click(screen.getByText('start'))
    fireEvent.click(screen.getByText('type'))
    await act(async () => { await vi.advanceTimersByTimeAsync(1300) })
...
describe('useFileScrollMemory', () => {
  function Harness({ path }: { path: string }) {
    const ref = useRef<HTMLDivElement | null>(null)
    const onScroll = useFileScrollMemory(ref, path, true)
    return <div data-testid="body" ref={ref} onScroll={onScroll} style={{ overflow: 'auto', height: 50 }} />
  }

  it('remembers the offset (debounced) and restores it for the same file', async () => {
    vi.useFakeTimers()
    ...
    // Restoration across the unmount IS the observable behaviour; the store is
    // deliberately in-memory, so there is nothing in localStorage to assert on
    // (a persisted pixel offset would restore into a file that may have changed
    // on disk since — see the hook's own note).
    expect(localStorage.getItem('kc:file-explorer:scroll:v1')).toBeNull()
  ...
  it('does not restore one file\'s offset into a different file', async () => {
    vi.useFakeTimers()
    const { unmount } = render(<Harness path="/zzz/a.md" />)
    const body = screen.getByTestId('body')
    ...
    render(<Harness path="/zzz/b.md" />)
    const other = screen.getByTestId('body')
    Object.defineProperty(other, 'scrollTop', { value: 0, writable: true })
    await act(async () => { await vi.advanceTimersByTimeAsync(300) })
...
describe('FileViewer shell', () => {
  ...
  it('offers Edit for markdown that decoded losslessly', () => {
    renderViewer()
    expect(screen.getByRole('button', { name: /Edit/i })).toBeInTheDocument()
  })

  it('refuses to edit a .md whose bytes did not decode losslessly', () => {
    // /read serves a binary or undecodable .md as empty/replacement-char
    // content; editing that buffer would write the lossy rendering back over
    // the original bytes. The pencil must not appear at all.
    for (const bad of [
      { binary: true, encoding: 'none' },      // binary blob named .md
      { binary: false, encoding: 'latin-1' },  // decoded, but not losslessly
      { binary: false, encoding: 'utf8', truncated: true }, // capped read
    ]) {
      const { unmount } = renderViewer({ fileMeta: { ...meta, ...bad } as never })
      expect(screen.queryByRole('button', { name: /Edit/i })).toBeNull()
      unmount()
    }
  })

  it('accepts either spelling of the utf-8 label', () => {
    for (const encoding of ['utf8', 'utf-8', 'UTF-8']) {
      const { unmount } = renderViewer({ fileMeta: { ...meta, encoding } as never })
      expect(screen.getByRole('button', { name: /Edit/i })).toBeInTheDocument()
      unmount()
    }
  })
