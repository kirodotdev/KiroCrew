...
describe('SessionActionsMenu – offline affordance is not rename-only', () => {
  ...
  it('the destructive row dims like the rest offline, rather than staying red as the one live-looking write', () => {
    renderMenu(false)
    const close = screen.getByText(/^Close session$/).closest('[role="menuitem"]') as HTMLElement
    expect(close.className).toContain('opacity-40')
    expect(close.className).toContain('text-muted')
    expect(close.className).not.toContain('text-danger')
  })

  it('keeps the destructive colour when connected — the control for the row above', () => {
    renderMenu(true)
    const close = screen.getByText(/^Close session$/).closest('[role="menuitem"]') as HTMLElement
    expect(close.className).toContain('text-danger')
    expect(close.className).not.toContain('opacity-40')
  })
  ...
  it('the reason names the dimmed rows, not session changes at large, while two session rows stay live', () => {
    renderMenu(false)
    const reason = screen.getByTestId('menu-offline-reason')
    for (const live of [/^Mark as unread$/, /^Tags…$/]) {
      expect(screen.getByText(live), String(live)).toBeInTheDocument()
    }
    expect(reason.textContent).not.toMatch(/session changes/i)
    expect(reason.textContent).toMatch(/dimmed/i)
  })
