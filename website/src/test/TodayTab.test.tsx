...
describe('TodayTab', () => {
  ...
  it('states the count basis as visible text, not only a hover title', async () => {
    mount([{ key: 'live-1', title: 'Overview Today card', messages: 41, running: true, last_turn_ts: nowIso() }])
    expect(await screen.findByText('Overview Today card')).toBeInTheDocument()
    // The Usage card beside this one counts sessions STARTED today; the two
    // numbers disagree by design, and a keyboard or touch reader never sees a
    // title attribute, so the basis is rendered as text.
    expect(screen.getByTestId('today-sessions-basis')).toHaveTextContent(
      'Sessions with any activity today, including ones started earlier. The Usage card counts sessions started today.',
    )
  })
  ...
  it('surfaces a failed sessions read as an error notice, not an empty list', async () => {
    ...
    mount()
    expect(await screen.findByTestId('today-sessions-error')).toBeInTheDocument()
