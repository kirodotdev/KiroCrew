...
function mount(config: Record<string, unknown> = {}, opts: { pairWithheld?: boolean } = {}) {
  ...
  if (opts.pairWithheld) {
    // What a non-owner dashboard session is served: the gateway OMITS both keys
    // rather than emptying them, and that difference is the whole signal.
    delete served.custom_model_url
    delete served.custom_model_sha256
  }
  mockApi.sttConfig.mockResolvedValue(served)
...
describe('SttSettings custom-model pair explanation', () => {
  ...
  it('clears a digest the backend rejected, so the explanation is not masked', async () => {
    ...
    const sha = await screen.findByLabelText(/SHA-256/i)
  ...
  it('stays silent for a caller the pair was withheld from, whose rows read empty', async () => {
    // A non-owner dashboard session with custom IN FORCE: the gateway omits the pair,
    // so both rows are empty and the old condition fired - telling the one caller who
    // cannot see the pair that the default model was still in force, which is the
    // opposite of the state the gateway holds. Empty rows are absence of evidence
    // here, not evidence of an unfinished pair.
    mount({ model: 'custom' }, { pairWithheld: true })
    await waitFor(() => expect(modelSelect()).toBeTruthy())
    expect(explanation()).toBeNull()
  })
  ...
  it('leaves a sibling row alone when the other half is saved', async () => {
    ...
    const url = await screen.findByLabelText(/model URL/i)
...
describe('SttSettings custom-model draft hand-off guard', () => {
  ...
  async function failSaveOf(label: RegExp, value: string) {
    ...
    // The custom rows mount on the config query resolving, which is a tick LATER
    // than the model combobox the caller waited for -- so address the row itself.
