 ...
 * MemberProjectionFrame / MembersSubscribedFrame.
 */
...
describe('useWebSocket member projection frames', () => {
  ...
  it('members_subscribed refetches the roster so a key deleted while away is dropped', () => {
    // Truncation is keyed by SLUG, so a contributed KEY removed during the
    // outage survives it: the deletion frame was missed and the held row's seq
    // is at or below the baseline, so nothing drops it. Only the roster knows
    // the key is gone, so the baseline has to go back and ask.
    ...
    expect(keys).toContain(JSON.stringify(MEMBERS_ROSTER_QUERY_KEY))
  })

  it('a baseline with no lastSeqs refetches nothing', () => {
    const spy = vi.spyOn(qc, 'invalidateQueries')
    const ws = open()
    act(() => {
      ws.simulateMessage({ type: 'members_subscribed', data: {} })
    })
    const keys = spy.mock.calls.map((c) => JSON.stringify((c[0] as { queryKey?: unknown })?.queryKey))
    expect(keys).not.toContain(JSON.stringify(MEMBERS_ROSTER_QUERY_KEY))
  })
})
