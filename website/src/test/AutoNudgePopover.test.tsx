...
describe('AutoNudgePopover goal persistence', () => {
  ...
  it.each([7_999, 8_000, 8_001])(
    ...
    length => {
      ...
      // One element carries the limit warning for everyone: it is the polite
      // live region screen readers hear when a paste is cut, and it is visible
      // to sighted users, who otherwise learned of the cut only from the small
      // counter turning colour.
      const warning = screen.getByTestId('goal-message-limit-warning')
      expect(warning).toHaveAttribute('role', 'status')
      expect(warning).toHaveAttribute('aria-live', 'polite')
      if (expectedLength === GOAL_DRAFT_MAX_MESSAGE_CHARS) {
        expect(warning).toHaveTextContent(/Maximum 8000 characters/i)
  ...
  it('keeps a local clear-write failure visible instead of closing as if both stores cleared', async () => {
    ...
    try {
      ...
      // The editor shows the cleared template, so the failure must describe the
      // CLEAR that did not finish, not a retained draft that no longer exists.
      expect(screen.getByTestId('goal-draft-sync-error')).toHaveTextContent(
        "Couldn't clear this goal on every device yet",
      )
      expect(screen.getByTestId('goal-draft-sync-error')).not.toHaveTextContent('copy is still here')
    } finally {
      Storage.prototype.setItem = orig
    }
  ...
  it('keeps a remote clear-write failure visible with a durable local tombstone', async () => {
    ...
    expect(goalBox().value).toContain('north star')
    expect(screen.getByTestId('goal-draft-sync-error')).toHaveTextContent(
      "Couldn't clear this goal on every device yet. Retry to finish clearing it.",
    )
  ...
  it('preserves a failed clear tombstone and visible failure across remount', async () => {
    ...
    expect(goalBox().value).toContain('north star')
    await waitFor(() => expect(screen.getByTestId('goal-draft-sync-error')).toBeInTheDocument())
    // The remount reconciles the durable tombstone, so its failure is still a
    // clear that has not finished, not a draft this device is holding.
    expect(screen.getByTestId('goal-draft-sync-error')).toHaveTextContent(
      "Couldn't clear this goal on every device yet",
    )
    expect(loadGoalDraftSnapshot(slot).draft).toBeNull()
  ...
  it('keeps Retry failed when a local clear failure is followed by a remote failure', async () => {
    ...
    expect(screen.getByTestId('popover-open')).toHaveTextContent('true')
    expect(goalBox().value).toContain('north star')
    // Retry re-runs the open reconciliation over the retained tombstone; a
    // second failure must keep describing the clear, not fall back to draft copy.
    expect(screen.getByTestId('goal-draft-sync-error')).toHaveTextContent(
      "Couldn't clear this goal on every device yet",
    )
  })
  ...
  it('does not remount an expired timestamp-less migration after server eviction', async () => {
    const nowValue = Date.now()
    const now = vi.spyOn(Date, 'now').mockReturnValue(nowValue)
    const legacyDraft = { message: 'raw legacy migration', idleSecs: 60, maxCycles: 0 }
    localStorage.setItem(LEGACY_GOAL_DRAFTS_KEY, JSON.stringify({ [SLOT]: legacyDraft }))
...
describe('AutoNudgePopover {{STOP_FILE}} help line (#10458)', () => {
  ...
  it('explains the raw token under the default template and names it verbatim', () => {
    ...
    // Screen readers get both the character-count contract and the stop-token
    // explanation; merging the two features must not orphan either description.
    const describedBy = goalBox().getAttribute('aria-describedby')!.split(/\s+/)
    expect(describedBy).toContain(screen.getByTestId('goal-message-character-count').id)
  ...
  it('does not render the help line for a goal that carries no token', () => {
    ...
    expect(goalBox().getAttribute('aria-describedby'))
      .toBe(screen.getByTestId('goal-message-character-count').id)
  ...
  it('Start loop posts the message with the token intact (display never rewrites what is stored)', async () => {
    renderPopover(null)
    const startLoop = screen.getByRole('button', { name: /Start loop/i })
    await waitFor(() => expect(startLoop).toBeEnabled())
    await act(async () => { fireEvent.click(startLoop) })
    const calls = (fetch as unknown as { mock: { calls: [string, { body?: string }?][] } }).mock.calls
    const save = calls.find(c => String(c[0]).startsWith('/api/autonudge') && c[1]?.body)
    expect(save, 'no /api/autonudge write was issued').toBeTruthy()
    const body = JSON.parse(save![1]!.body!)
