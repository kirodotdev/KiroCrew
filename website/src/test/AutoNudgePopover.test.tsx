...
describe('AutoNudgePopover — the confirm is bound to the goal it was served for', () => {
  ...
  it('never advises leaving the text untouched once the text has been touched', async () => {
    ...
    // ux-review: the question must name the STAKE rather than restate the verb -- what the
    // reader cannot know is that the stored goal is neither shown nor recoverable.
  ...
  it('omits every field the user did not touch, so a concurrent change to it survives', async () => {
    // GPT 5.6 (BLOCKING): a settings-only Save sent the goal seeded at open together with the
    // LIVE fingerprint, so the 409 matched and the newer goal was replaced by the stale one.
    const patches: string[] = []
    vi.stubGlobal(
      'fetch',
      vi.fn((_url: string, init?: RequestInit) => {
    ...
    await waitFor(() => expect(patches.length).toBe(1))
    // An omitted `message` is what protects the moved goal: the server leaves the field
    // alone, so no value of ours -- stale seed OR live copy -- can land on top of it.
    ...
    // A goal CLEARED elsewhere is a deliberate act, and resending any goal would have
    // resurrected it; omission leaves the cleared goal cleared.
  ...
  it('refuses the first trigger click that would discard an unsaved goal edit', async () => {
    // GPT 5.6 (BLOCKING): the trigger toggle closes through the ROOT onOpenChange, which
    // neither dismiss hook sees, so that one gesture discarded the edit without warning.
...
describe('AutoNudgePopover — the keep-stored arm claims only what it saved', () => {
  ...
  it('does not claim other settings were saved when only the goal was edited', async () => {
    ...
    // ONLY the goal is touched. The numeric inputs are never edited, so a notice that
    // reports settings being saved describes a change the user did not make.
    fireEvent.change(screen.getByRole('textbox'), { target: { value: 'my replacement' } })
    fireEvent.click(screen.getByRole('button', { name: /save/i }))
    await screen.findByTestId('autonudge-confirm-question')
