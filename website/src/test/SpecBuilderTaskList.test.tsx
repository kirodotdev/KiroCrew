...
describe('DocView tasks tab', () => {
  const detail = {
    ...
    phase: 'tasks',
    running: false,
    files: { 'requirements.md': null, 'design.md': null, 'tasks.md': '- [ ] add the tests\n' },
    docs: { 'tasks.md': { hash: 'd'.repeat(64), editable: true } },
  ...
  it('falls back to the raw markdown while editing, so the file stays reachable', () => {
    render(
      <DocView
        detail={detail}
        tab="tasks"
    ...
    act(() => { fireEvent.click(screen.getByRole('button', { name: /edit tasks/i })) })

    expect(screen.getByRole('textbox', { name: /edit tasks/i })).toHaveValue('- [ ] add the tests\n')
