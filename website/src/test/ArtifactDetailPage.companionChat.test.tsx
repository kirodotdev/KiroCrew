...
describe('ArtifactDetailPage companion chat', () => {
  ...
  it('attaches the artifact the user acted on, not the one on screen when the POST goes out', async () => {
    // The context used to be built INSIDE mutationFn, which React Query resolves from the
    // observer's LATEST render options -- so moving to another document during the create
    // round-trip made this slot's durable entry describe that other document, silently and for
    // the full TTL.
    let resolveCreate!: (v: { key: string; title: string }) => void
    vi.mocked(api).createChatSlot = vi.fn().mockReturnValue(
      new Promise<{ key: string; title: string }>((res) => { resolveCreate = res }),
    )
    const { queryClient } = renderPage()
    await waitForLoaded()
    fireEvent.click(screen.getByLabelText('Toggle agent chat'))
    await waitFor(() => expect(vi.mocked(api).createChatSlot).toHaveBeenCalledTimes(1))
    // The page re-renders against a different document while the create is still in flight. The
    // fetcher has to move too: the test client treats data as instantly stale, so a refetch would
    // otherwise restore the first artifact and hide the very read this test is about.
    const other = mkArtifact({ slug: 'other-doc', name: 'Other Doc', version: 9 })
    vi.mocked(api).artifact = vi.fn().mockResolvedValue(other)
    await act(async () => {
      await queryClient.refetchQueries({ queryKey: ['artifact', 'cr-queue'] })
    })
    expect(await screen.findByText('Other Doc')).toBeInTheDocument()
    await act(async () => { resolveCreate({ key: 'slot-new', title: 'Artifact: CR Queue' }) })
    await waitFor(() => expect(vi.mocked(api).chatSlotContext).toHaveBeenCalledTimes(1))
    const [, content, opts] = vi.mocked(api).chatSlotContext.mock.calls[0]
    expect(content).toContain('cr-queue')
    expect(content).not.toContain('other-doc')
    // The version was already pinned in a local before the await; the content now matches it.
    expect((opts as { contextKey?: string }).contextKey).toBe('2')
  })
