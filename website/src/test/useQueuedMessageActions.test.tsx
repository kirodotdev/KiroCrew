...
describe('useQueuedMessageActions — an edit that COMMITS while its response is lost', () => {
  ...
  /** The edit id the client actually sent, so the echo models OUR edit rather than a foreign one. */
  const ourEditId = (n = 0) => apiMocks.editQueuedMessage.mock.calls[n][3] as string
  const echo = (slot: string, queueId: string, content: string, editId: string) =>
    applyQueueEdit({ slot, queue_id: queueId, content, editId }) as never
  ...
  it('retires a banner already shown when the echo confirms afterwards', async () => {
    ...
    await act(async () => { store.dispatch(echo('chat-1', 'q1', 'the committed text', ourEditId())) })
  ...
  it('still reports a LATER edit failing after an earlier edit was confirmed', async () => {
    ...
    await act(async () => { store.dispatch(echo('chat-1', 'q1', 'first text', ourEditId())) })
...
describe('useQueuedMessageActions — a REJECTED edit keeps the typed text recoverable', () => {
  ...
  it('reports the rejected text against its card, so the input can reopen seeded', async () => {
    ...
    expect(get().editRejected,
      ...
      .toEqual({ q1: 'the text the user typed' })
  })
  ...
  it('reports nothing when the echo already committed the edit', async () => {
    ...
    expect(get().editRejected, 'a committed edit was not rejected').toEqual({})
  })
  ...
  it('reports nothing on a SUCCESSFUL edit', async () => {
    ...
    expect(get().editRejected).toEqual({})
  })
  ...
  it('clears the rejected text once the banner is dismissed', async () => {
    ...
    act(() => { get().onEdit('q1', 'the text the user typed') })
    await act(async () => { d.reject(new Error('rejected outright')) })
    await waitFor(() => expect(get().editRejected.q1).toBeTruthy())

    act(() => { get().dismissEditError() })

    expect(get().editRejected,
      'dismissing is the user declining the handback, so it must not reopen later').toEqual({})
  })
})

describe('useQueuedMessageActions — two cards edited at once keep separate records', () => {
  /** The edit id the client actually sent, so the echo models OUR edit rather than a foreign one. */
  const ourEditId = (n = 0) => apiMocks.editQueuedMessage.mock.calls[n][3] as string
  const echo = (slot: string, queueId: string, content: string, editId: string) =>
    applyQueueEdit({ slot, queue_id: queueId, content, editId }) as never

  it('starting an edit on ANOTHER card does not erase the first card&#39;s rejected text', async () => {
    const a = deferred()
    apiMocks.editQueuedMessage.mockReturnValue(a.promise)
    const { get } = renderActions({})

    act(() => { get().onEdit('q1', 'card A typed text') })
    await act(async () => { a.reject(new Error('rejected outright')) })
    await waitFor(() => expect(get().editRejected.q1, 'premise: A holds the sole copy').toBeTruthy())

    // B starts while A's failure stands. A single settlement record made this erase A's only copy.
    const b = deferred()
    apiMocks.editQueuedMessage.mockReturnValue(b.promise)
    act(() => { get().onEdit('q2', 'card B typed text') })

    expect(get().editRejected.q1,
      'a second card&#39;s edit must not throw away the first card&#39;s unrecoverable text')
      .toBe('card A typed text')
  })

  it('a confirmation for ANOTHER card does not clear the first card&#39;s rejection', async () => {
    const a = deferred()
    apiMocks.editQueuedMessage.mockReturnValue(a.promise)
    const { get, store } = renderActions({})
    ...
    apiMocks.editQueuedMessage.mockReturnValue(b.promise)
    act(() => { get().onEdit('q2', 'card B typed text') })
    await act(async () => { store.dispatch(echo('chat-1', 'q2', 'card B typed text', ourEditId())) })
    await act(async () => { b.resolve({ ok: true, content: 'card B typed text' }) })

    expect(get().editRejected.q1,
      'B settling says nothing about A, whose failure is still unresolved').toBe('card A typed text')
  })

  it('still clears the rejection of the card that WAS confirmed', async () => {
    // Positive control: keying per card must not stop a real confirmation retiring its own record.
    const d = deferred()
    apiMocks.editQueuedMessage.mockReturnValue(d.promise)
    const { get, store } = renderActions({})
    ...
    await waitFor(() => expect(get().editRejected.q1,
      'the card now shows the committed edit, so its rejection is spent').toBeUndefined())
  })
})

describe('useQueuedMessageActions — a FOREIGN edit cannot discard a failed local edit', () => {
  /** Another client's edit of the same card: same queue id, a DIFFERENT edit identity. */
  const foreignEcho = (slot: string, queueId: string, content: string) =>
    applyQueueEdit({ slot, queue_id: queueId, content, editId: 'edit-from-another-client' }) as never
  const ourEditId = (n = 0) => apiMocks.editQueuedMessage.mock.calls[n][3] as string

  it('keeps the typed text when another client&#39;s edit settles the same card first', async () => {
    const d = deferred()
    apiMocks.editQueuedMessage.mockReturnValue(d.promise)
    const { get, store } = renderActions({})
    ...
    act(() => { get().onEdit('q1', 'the text the user typed') })
    await waitFor(() => expect(apiMocks.editQueuedMessage).toHaveBeenCalled())
    expect(ourEditId(), 'premise: this edit carries its own identity').toBeTruthy()

    // A DIFFERENT client edits the same card, so a settlement is published for this queue id with a
    // higher sequence -- but it says nothing about whether OUR request was taken.
    await act(async () => { store.dispatch(foreignEcho('chat-1', 'q1', 'someone else&#39;s text')) })
    await act(async () => { d.reject(new Error('our request was rejected')) })

    expect(get().editRejected.q1,
      'a foreign edit must not discard the only copy of what this user typed')
      .toBe('the text the user typed')
    expect(get().editError, 'and the failure must still be reported').toBeTruthy()
  })

  it('does not let a foreign settlement retire a banner already shown', async () => {
    const d = deferred()
    apiMocks.editQueuedMessage.mockReturnValue(d.promise)
    const { get, store } = renderActions({})
    ...
    act(() => { get().onEdit('q1', 'the text the user typed') })
    await act(async () => { d.reject(new Error('our request was rejected')) })
    await waitFor(() => expect(get().editRejected.q1, 'premise: the rejection is held').toBeTruthy())

    await act(async () => { store.dispatch(foreignEcho('chat-1', 'q1', 'someone else&#39;s text')) })

    expect(get().editRejected.q1,
      'the recovery survives a settlement that was never about our edit')
      .toBe('the text the user typed')
  })

  it('still suppresses on OUR OWN edit&#39;s confirmation', async () => {
    // Positive control: identity matching must not stop a genuine self-confirmation suppressing the
    // contradictory banner, which is the case the guard exists for.
    const d = deferred()
    apiMocks.editQueuedMessage.mockReturnValue(d.promise)
    const { get, store } = renderActions({})
    ...
    await waitFor(() => expect(apiMocks.editQueuedMessage).toHaveBeenCalled())
    await act(async () => {
      store.dispatch(applyQueueEdit({
        slot: 'chat-1', queue_id: 'q1', content: 'the committed text', editId: ourEditId(),
      }) as never)
    })
    await act(async () => { d.reject(new Error('network died after the commit')) })

    expect(get().editError, 'our own confirmed edit must not be reported as failed').toBeNull()
    expect(get().editRejected.q1, 'and needs no handback').toBeUndefined()
  })
})
