...
describe('ProjectPicker', () => {
  ...
  describe('selection', () => {
    ...
    it('drops the recents notice off a Browse tab the user chose deliberately', async () => {
      ...
      expect(await screen.findByText('Loading recent projects timed out')).toBeInTheDocument()

      fireEvent.mouseDown(await screen.findByText('Browse'))
      expect(screen.queryByText('Loading recent projects timed out')).not.toBeInTheDocument()

      fireEvent.mouseDown(await screen.findByText('Recent'))
      expect(screen.queryByText('Loading recent projects timed out')).toBeInTheDocument()
    })

    it('names a recent-projects deadline instead of silently landing on Browse', async () => {
    ...
    it('names a recent-projects access refusal and withholds Retry', async () => {
      ...
      expect(await screen.findByText('No access to recent projects')).toBeInTheDocument()
      expect(screen.queryByText('Recent projects unavailable')).not.toBeInTheDocument()
    ...
    it('keeps focus in the picker when the recents Retry recovers into an EMPTY list', async () => {
      ...
      fireEvent.mouseDown(await screen.findByText('Recent'))
      const retry = await screen.findByRole('button', { name: /^Retry: Loading recent projects/ })
      // Recovers, but with NO rows: the search field never mounts.
      vi.mocked(api.recentProjects).mockResolvedValue({ dirs: [] })
      fireEvent.click(retry)

      await waitFor(() => expect(screen.queryByText('Loading recent projects timed out')).not.toBeInTheDocument())
  ...
  describe('Browse tab trailing-slash auto-drill', () => {
    ...
    /**
     * The four causes the shared classifier can report, each with the copy the notice owes it.
     * An auto-drill is how the two REFUSALS are reached in practice -- a user types a path into
     * a directory she may not read, or a path that names a file -- and the notice used to fold
     * both into the generic failure while (correctly) withholding Retry, so it stated neither
     * a reason nor a remedy. Rendering the same string as the sibling picker keeps one refusal
     * from reading two ways depending on which picker the user happened to be in.
     */
    it.each([
      ['a denied directory', 403, 'access_denied', 'No access to this folder', false],
      ['a path that is not a directory', 400, 'not_a_directory', 'Folder not found', false],
      ['a deadline', null, null, 'Folder listing timed out', true],
      ['a codeless failure', 500, null, 'Unable to list folder', true],
    ])('names the cause when an auto-drill lands in %s', async (_label, status, code, copy, retryable) => {
      const { ApiError } = await import('../api/apiError')
      const browseSpy = vi.mocked(api.browseDirs)
      browseSpy.mockResolvedValue(mockBrowseDirs('/home/u', []))
      renderWithProviders(
      ...
      const rejection = status === null
        ? Object.assign(new Error('deadline exceeded'), { name: 'TimeoutError' })
        : new ApiError(status, 'nope', JSON.stringify(code ? { error: 'nope', code } : { error: 'nope' }))
      browseSpy.mockRejectedValue(rejection)
      fireEvent.change(input, { target: { value: '/home/u/secret/' } })
      await act(async () => { await vi.advanceTimersByTimeAsync(250) })
      expect(browseSpy).toHaveBeenCalledWith('/home/u/secret', expect.any(AbortSignal))
      await act(async () => { await vi.advanceTimersByTimeAsync(0) })

      expect(screen.getByRole('alert')).toHaveTextContent(copy)
      // Retry only where re-asking can answer differently; its accessible name carries the SAME
      // cause as the notice, so the two cannot drift apart.
      const retry = screen.queryByRole('button', { name: `Retry: ${copy}` })
  ...
  describe('a background refetch does not rewrite what the user typed', () => {
    ...
    it('withholds the listing Retry on a refusal but offers it on a transient failure', async () => {
      ...
      fireEvent.mouseDown(screen.getByText('Browse'))
      // The refusal is NAMED, not folded into the generic failure: a notice that withholds
      // Retry while saying only "unable" leaves the user with neither a reason nor a remedy.
