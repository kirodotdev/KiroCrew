...
describe('MdNotebookPage — settings, guarded mutations and editor keys', () => {
  ...
  it('keeps a refused rename draft across a search that unmounts the row', async () => {
    api.search.mockResolvedValue({ results: [] })
    await mount()
    await screen.findByRole('button', { name: 'One' }, TREE_READY)
    rowAction('One', 'Rename note')
    let field = await screen.findByRole('textbox', { name: 'Note name' })
    await userEvent.clear(field)
    await userEvent.type(field, 'CON{Enter}')
    expect(screen.getByText("This name can't sync to a Windows clone (CON, NUL, LPT1…)")).toBeTruthy()
    // Typing in Search replaces the list with results: the row, field and all, is gone.
    const search = screen.getByRole('textbox', { name: 'Search notes' })
    await userEvent.type(search, 'zzz')
    expect(await screen.findByText('No matches')).toBeTruthy()
    expect(screen.queryByRole('textbox', { name: 'Note name' })).toBeNull()
    // Clearing the search brings the row back in rename mode — with the draft, not the title.
    await userEvent.clear(search)
    field = await screen.findByRole('textbox', { name: 'Note name' })
  ...
  it('keeps the folder field and its draft when the backend refuses the creation', async () => {
    api.newNote.mockRejectedValueOnce(new Error('vault is read-only'))
    api.newNote.mockResolvedValueOnce({ path: 'Projects/Untitled.md' })
    await mount()
    await screen.findByRole('button', { name: 'One' }, TREE_READY)
    fireEvent.click(screen.getByRole('button', { name: 'New note or folder' }))
    fireEvent.click(screen.getByRole('button', { name: 'New folder at the top level' }))
    ...
    // Refused: the error notice says why, the field is still there with the name.
    await screen.findByText('vault is read-only')
    expect(screen.getByRole('textbox', { name: 'Folder name' })).toBe(field)
    expect(field).toHaveValue('Projects')
    // Retrying is one more Enter, not a retype.
    await userEvent.type(field, '{Enter}')
    await waitFor(() => expect(api.newNote).toHaveBeenCalledTimes(2))
  ...
  it('refuses a folder name Windows cannot check out and says so; strips a trailing dot', async () => {
    ...
    for (const bad of ['CON', 'lpt1', 'Nul.txt', ' . . ']) {
      ...
      const reason = bad.trim().startsWith('.')
        ...
        : "This name can't sync to a Windows clone (CON, NUL, LPT1…)"
