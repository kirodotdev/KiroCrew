...
describe('ChatPage file-chip remove parity', { timeout: 15_000 }, () => {
  ...
  it('shortening a mention to a suffix that uniquely resolves among staged paths keeps the file staged (fork GPT review)', async () => {
    // Pick `@src/main.ts`, hand-shorten it to the still-valid `@main.ts`:
    // the reference was rewritten, not deleted, yet every EXACT recorded
    // alias now fails -- the old rule silently unstaged the attachment and
    // the send omitted it. The uniqueness-scoped carve-out keeps the file
    // staged because `main.ts` path-suffix-matches exactly ONE staged path.
    const store = makeStore('slot-a', [{ key: 'slot-a', project: '/repo' }])
    await renderPage(store)
    ...
    // Still staged: the shortened suffix uniquely resolves to this file.
    await new Promise(r => setTimeout(r, 50))
    expect(screen.getByLabelText('Remove')).toBeTruthy()
    expect(ta.value).toBe('please check @main.ts ')
  })

  it('shortening a mention to a suffix shared by TWO staged paths still unstages (the carve-out is uniqueness-scoped)', async () => {
    // The ambiguous case keeps the author's original rule: with both
    // /repo/src/main.ts and /repo/other/src/main.ts staged, a hand-typed
    // `@main.ts` suffix-matches BOTH, so it proves nothing about which file
    // the user means -- the edited file unstages exactly as before the
    // carve-out, and the wrong-file hazard cannot come back.
    const store = makeStore('slot-a', [{ key: 'slot-a', project: '/repo' }])
    await renderPage(store)
  ...
  it('a mention rewritten to the CURRENT project\'s own rel form after re-rooting the slot stays staged (fork GPT review)', async () => {
    // Stage under /repo, re-root the slot upward, hand-rewrite the token
    // to the now-valid `@repo/src/main.ts`. Neither the recorded alias nor
    // any shorter suffix matches, and revival never inspects still-staged
    // paths -- the file was silently dropped from the send despite the
    // text plainly referencing it. The freshly derived current-project rel
    // is now accepted and recorded, so the remove control strips the live
    // form too.
    const store = makeStore('slot-a', [{ key: 'slot-a', project: '/repo' }])
    await renderPage(store)
    ...
    // Still staged: the mention is this file's own rel under the CURRENT root.
    await new Promise(r => setTimeout(r, 50))
    expect(screen.getByLabelText('Remove')).toBeTruthy()

    // The accepted form was recorded, so remove strips the live mention.
    fireEvent.click(screen.getByLabelText('Remove'))
    await waitFor(() => expect(screen.queryByLabelText('Remove')).toBeNull())
    expect(ta.value).not.toContain('@repo/src/main.ts')
  })
  ...
  it('removing a uniquely shortened mention also strips its live alias', async () => {
    const store = makeStore('slot-a', [{ key: 'slot-a', project: '/repo' }])
    await renderPage(store)
