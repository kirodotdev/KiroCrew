 ...
 * temporary, clean) so a mode-pinned session can be started INSIDE a folder in
 * one step. The create call must carry both the memory/clean mode and the
 ...
 *   (1) incognito / temporary create with their memory_mode, clean with
 *       clean_mode, and every one rides the CREATE call with the folder id;
 *   (2) none of them smuggle the `defaultAutopilot` preference in as
 *       'orchestrator' — they name a memory/clean type, not a run mode;
...
describe('folder menu: ephemeral chat creation', () => {
  ...
  it('clean creates with clean_mode and no memory_mode in the folder', async () => {
    cfg.value = { tagColumnsEnabled: false, confirmCloseSession: false, defaultAutopilot: true }
    renderSidebar()
    openFolderMenu()
    openEphemeralSubmenu()
    fireEvent.click(screen.getByTestId(`folder-new-clean-${FOLDER_ID}`))
    await waitFor(() => expect(mocks.createChatSlot).toHaveBeenCalledTimes(1))
    const call = mocks.createChatSlot.mock.calls[0]
    expect(call[ARG_CLEAN_MODE]).toBe(true)
    expect(call[ARG_MEMORY_MODE]).toBeUndefined()
    expect(call[ARG_FOLDER_ID]).toBe(FOLDER_ID)
    expect(call[ARG_MODE]).not.toBe('orchestrator')
  })
  ...
  it('lists the modes inline under a caption at phone width, with no flyout', () => {
    ...
    expect(screen.getByTestId(`folder-new-clean-${FOLDER_ID}`)).toBeInTheDocument()
    // The label is a caption here, not a sub-trigger.
    expect(screen.queryByTestId(`folder-new-ephemeral-${FOLDER_ID}`)).toBeNull()
  })

  it('creates with clean_mode from the inline row at phone width', async () => {
    mobile.value = true
    renderSidebar()
    openFolderMenu()
    fireEvent.click(screen.getByTestId(`folder-new-clean-${FOLDER_ID}`))
