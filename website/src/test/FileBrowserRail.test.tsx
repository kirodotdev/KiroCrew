...
describe('FileBrowserRail resize grip', () => {
  ...
  it('names the header Refresh in the tree notice, which is icon-only here', async () => {
    H.api.projectTree.mockReset().mockRejectedValue(
  ...
  it('withholds the remedy from a refusal, which answers the same on every Refresh', async () => {
    H.api.projectTree.mockReset().mockRejectedValue(
      new ApiError(403, 'nope', JSON.stringify({ code: 'access_denied' })))
    mount()

    const alert = await screen.findByRole('alert')
    expect(alert).toHaveTextContent('No access to this folder')
    expect(alert).not.toHaveTextContent(/Refresh to retry/)
    expect(alert).not.toHaveTextContent(/Couldn't load the file tree/)
  })

  it('gives the rail control a VISIBLE Refresh label while the notice names it', async () => {
    H.api.projectTree.mockReset().mockRejectedValue(
      Object.assign(new Error('deadline exceeded'), { name: 'TimeoutError' }))
    mount()
    await screen.findByRole('alert')

    expect(screen.getByLabelText('Refresh').querySelector('span')).not.toHaveClass('invisible')
  })

  it('keeps the rail control compact while the tree read is healthy', async () => {
    mount()
