...
describe('AutoNudgePopover — the redacted projection is marked, not silent', () => {
  ...
  it('tells the user in prose that the goal is unchanged, naming no API key', async () => {
    ...
    expect(text).toMatch(/goal unchanged/i)
  ...
  it('declining keeps the stored goal but still persists the other settings', async () => {
    ...
    fireEvent.change(screen.getByRole('spinbutton', { name: /Seconds between nudges/i }), {
      target: { value: '180' },
    })
    fireEvent.click(screen.getByRole('button', { name: /save/i }))
    ...
    expect(body.idle_secs).toBe(180)
  ...
  it('offers the safe path in the redaction notice, and only one path', async () => {
    renderPopover({ ...BASE, message_redacted: true } as AutoNudgeLoop)
    const notice = await screen.findByTestId('autonudge-redacted-notice')
    const text = notice.textContent ?? ''
    expect(text).toMatch(/retype the goal in full/i)
    expect(text).toMatch(/saved as literal text/i)
    // Two sentences describing the SAME visible state with opposite outcomes let a reader
    // follow either one, and one of those outcomes commits the mask.
    expect(text).not.toMatch(/leave the text untouched/i)
  })
