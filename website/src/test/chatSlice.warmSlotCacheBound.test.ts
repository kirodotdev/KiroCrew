...
describe('warmSlotCache hydrate bound', () => {
  ...
  describe('a pane holding rows warms through a count-matched window (#10005)', () => {
    ...
    it('replaces the cache with the marked page when a FULL window has moved wholly past it -- no unbounded read', async () => {
      // A pane holding 600 identified rows whose slot then produced 1,400 more
      // before this warm (a long-running loop; #12907's 5,894-row session). The
      // newest-500 window is rows 1500..1999: its oldest row anchors nowhere and
      // the cache's oldest row is not on the page. That is the window having
      // MOVED, not a hole -- the page replaces the cache with the door marked and
      // the rows behind stay reachable through the pane's load-earlier bar.
      // Before: the stitch check declined and the thunk pulled all 2,000 rows,
      // on every `done` of the busiest transcript on the host.
      const held = rows(600)
      const window = rows(500, 1500)
      ;(api.chatSlotDetail as ReturnType<typeof vi.fn>).mockResolvedValue({ ...detail, messages: window, has_more: true, total: 2000 })
      const store = makeStore('active-slot', { slotMessages: { 'bg-slot': held }, slotHydrated: { 'bg-slot': true } })
      await store.dispatch(warmSlotCache('bg-slot') as never)
      expect(api.chatSlotDetail).toHaveBeenCalledTimes(1)
      expect(api.chatSlotDetail).toHaveBeenCalledWith('bg-slot', 500)
      const chat = store.getState().chat
      const contents = chat.slotMessages['bg-slot'].map(m => m.content)
      // Contiguous, ends at the newest row, no spliced hole.
      expect(contents).toEqual(window.map(m => m.content))
      expect(contents[0]).toBe('row 1500')
      expect(contents.at(-1)).toBe('row 1999')
      expect(chat.slotPaneHasMore['bg-slot']).toBe(true)
      expect(chat.slotPaneBounded['bg-slot']).toBe(500)
    })

    it('replaces a SHORTER cache the same way when a full window moved past it', async () => {
      // 40 cached rows, 200 produced since: the newest-50 window is rows 190..239.
      // Same verdict from the other side of the length comparison.
      const held = rows(40)
      const window = rows(50, 190)
      ;(api.chatSlotDetail as ReturnType<typeof vi.fn>).mockResolvedValue({ ...detail, messages: window, has_more: true, total: 240 })
      const store = makeStore('active-slot', { slotMessages: { 'bg-slot': held } })
      await store.dispatch(warmSlotCache('bg-slot') as never)
      expect(api.chatSlotDetail).toHaveBeenCalledTimes(1)
      expect(api.chatSlotDetail).toHaveBeenCalledWith('bg-slot', PANE_HYDRATE_LIMIT)
      expect(store.getState().chat.slotMessages['bg-slot'].map(m => m.content)).toEqual(window.map(m => m.content))
      expect(store.getState().chat.slotPaneHasMore['bg-slot']).toBe(true)
    })

    it('still takes the unbounded retry for a SHORT wholly-newer page: only a full window is the window having moved', async () => {
      // 3 rows on the page against a 50-row request: not a window the handler
      // produced (it slices after `_collapse_wire_rows`, so `has_more` means
      // exactly `limit` rows). The append/retry behaviour is unchanged.
      const held = rows(600)
      ;(api.chatSlotDetail as ReturnType<typeof vi.fn>)
        .mockResolvedValueOnce({ ...detail, messages: rows(3, 1500), has_more: true, total: 2000 })
        .mockResolvedValueOnce({ ...detail, messages: [...held, ...rows(3, 1500)], has_more: false, total: 603 })
      const store = makeStore('active-slot', { slotMessages: { 'bg-slot': held } })
      await store.dispatch(warmSlotCache('bg-slot') as never)
      expect(api.chatSlotDetail).toHaveBeenCalledTimes(2)
      expect(store.getState().chat.slotMessages['bg-slot']).toHaveLength(603)
    })
