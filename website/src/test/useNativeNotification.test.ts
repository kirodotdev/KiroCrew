...
beforeEach(() => {
  ...
  // isWindowAway() gates delivery on `document.hidden || !document.hasFocus()`.
  // jsdom reports the window visible AND focused, so force the "away" axis on:
  // these tests exercise the delivery branch, not the visibility gate.
