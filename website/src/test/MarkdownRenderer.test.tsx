...
describe('MarkdownRenderer LaTeX-native delimiters (#7803)', () => {
  ...
  it('handles a flood of empty display pairs in linear time (no per-closer slice)', { timeout: 120000 }, () => {
    ...
    // guard, not a proof of linearity.
    const time = (s: string) => {
      const t0 = performance.now()
      render(<MarkdownRenderer content={s} />)
      return performance.now() - t0
    }
    const base = time('\\[ '.repeat(300000))
    const flood = time('\\[ \\] '.repeat(150000))
    expect(flood / base).toBeLessThan(1.8)
  })
  ...
  it('pairs thousands of unclosed verbatim openers in linear time (one pass, not a scan per opener)', () => {
    ...
    // render absorbs JIT and module-load cost from the first measurement.
    const time = (n: number) => {
      const t0 = performance.now()
  ...
  it('handles an ampersand flood beside math in linear time (the reference scan is bounded)', () => {
    ...
    const content = 'x \\(a\\) ' + '&'.repeat(100_000)
    const t0 = performance.now()
    const { container } = render(<MarkdownRenderer content={content} />)
