 ...
 * fixture out of `docs/system-specs/modules/decisions.md` § 7 — the document
...
const SECTION = "## 7. The decision strip's record and feedback"

/** The first fenced JSON block inside § 7. */
...
function specFeedbackVocabulary(): string {
  const text = readFileSync(SPEC, 'utf-8')
  const start = text.indexOf('`POST /api/decisions/feedback`')
  if (start < 0) throw new Error('the spec no longer names POST /api/decisions/feedback')
