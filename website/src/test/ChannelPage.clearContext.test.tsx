...
describe('ChannelPage — Clear Context', () => {
  ...
  it('names the partial outcome in the clear-all confirm, not a no-op', async () => {
    // "kept instead" reads as NOTHING HAPPENED, when the idle agents are in fact cleared --
    // a user who reads it as an abort re-runs a destructive act they already completed.
    ...
    expect(copy, 'the confirm never says the idle agents ARE cleared').toContain(
      'only the agents that are idle are cleared',
    )
  })
