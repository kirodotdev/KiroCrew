...
vi.mock('../components/WelcomeView', () => ({
  default: ({ backend, onSelectBackend, backendError }: { backend?: string | null; onSelectBackend?: (id: string) => void; backendError?: string | null }) => (
    <div>
      {/* Three states are distinguishable on purpose: '(null)' = inherit the
          default, '' = pinned to kiro-cli, 'acme' = pinned to acme. */}
      <span data-testid="welcome-backend-value">{backend === null ? '(null)' : backend ?? '(undefined)'}</span>
      <button data-testid="pick-acme" onClick={() => onSelectBackend?.('acme')}>pick acme</button>
      <button data-testid="pick-kiro" onClick={() => onSelectBackend?.('')}>pick kiro</button>
      {/* The refused-switch message the page hands the surface (the real
          WelcomeView renders it through ErrorNotice). */}
      {backendError && <span data-testid="welcome-backend-error">{backendError}</span>}
    </div>
  ),
}))
...
function makeStore(acp_backend?: string | null) {
...
async function renderChat(acp_backend?: string | null) {
...
describe('ChatPage — welcome-screen backend pick applies to the current slot', { timeout: 15_000 }, () => {
  ...
  it('shows no backend chip on the welcome surface: the picker above the composer is the control there', async () => {
    await renderChat('acme')
    // The welcome view (stubbed here) carries the picker; a chip beside it that
    // says the backend "can't be changed" would contradict a control that
    // changes it. The chip appears once the first message pins the binding.
    expect(screen.queryByTestId('chat-input-backend-chip')).toBeNull()
  })

  it('hands a refused switch to the welcome surface as an error, not the shared switch toast', async () => {
    const { api } = await import('../api/client')
    ;(api.chatSlotBackend as unknown as ReturnType<typeof vi.fn>).mockImplementationOnce(
      async () => ({ ok: false, error: 'backend \'acme\' is not selectable' }),
    )
    const store = await renderChat('')
    await act(async () => { fireEvent.click(screen.getByTestId('pick-acme')) })
    // The failure reaches the surface that owns the picker (WelcomeView renders
    // it through ErrorNotice with the agent hand-off)...
    await waitFor(() => expect(screen.getByTestId('welcome-backend-error').textContent).toMatch(/not selectable/))
    // ...and NOT the hand-written agent/model/project switch toast.
    expect((store.getState() as RootState).chat?.agentSwitchNotice ?? null).toBeNull()
    // The stored slot value is untouched: the chip keeps showing what runs.
    const slot = (store.getState() as RootState).dashboard.slots.find(s => s.key === 'slot-a')
    expect(slot?.acp_backend).toBe('')  // still the seeded pin, not 'acme'
  })
  ...
  it('displays the inherit-default state for a slot with no pin', async () => {
    // Both "no field" and an explicit null are inherit: the picker sees null,
    // never '' (which would claim a kiro-cli pin the slot does not have).
    await renderChat(undefined)
    expect(screen.getByTestId('welcome-backend-value').textContent).toBe('(null)')
    cleanup()
    await renderChat(null)
    expect(screen.getByTestId('welcome-backend-value').textContent).toBe('(null)')
  })

  it("displays a kiro-cli pin ('') as a pin, distinct from inherit", async () => {
    await renderChat('')
    expect(screen.getByTestId('welcome-backend-value').textContent).toBe('')
  })

  it("writes a kiro-cli pick as '' through the mutation endpoint, not as a clear", async () => {
    const { api } = await import('../api/client')
    const store = await renderChat('acme')
    await act(async () => { fireEvent.click(screen.getByTestId('pick-kiro')) })
    await waitFor(() => expect(api.chatSlotBackend).toHaveBeenCalledWith('slot-a', ''))
    await waitFor(() => {
      const slot = (store.getState() as RootState).dashboard.slots.find(s => s.key === 'slot-a')
      expect(slot?.acp_backend).toBe('')
    })
  })
})
