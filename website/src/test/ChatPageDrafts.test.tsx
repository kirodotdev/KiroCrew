...
describe('ChatPage composerSlotRef effect ordering', () => {
  ...
  it('syncs the composer mirror in onChange, before the queued state update', () => {
    // Deliberately brittle, like its sibling above: UPDATE the substrings if the
    // prop is reformatted, never delete the guard.
    const here = dirname(fileURLToPath(import.meta.url))
    const src = readFileSync(resolve(here, '../pages/ChatPage.tsx'), 'utf8')
    const eager = src.indexOf('inputRef.current = v')
    const queued = src.indexOf('setInput(v)', eager)
    expect(
      eager,
    ...
    expect(queued, 'the composer onChange no longer sets the state').toBeGreaterThan(eager)
    // The same handler reports the edit to the voice atom, which is what lets a
    // deletion-then-identical-retyping of a dictated run be told from a run nobody
    // touched -- the values alone cannot. It has to read the PRE-change value, so it
    // sits after the mirror is captured and before the state update.
    const noted = src.indexOf('noteComposerEdit(before, v)', eager)
    expect(noted, 'the composer onChange must report the edit to the voice atom').toBeGreaterThan(eager)
    expect(noted, 'the edit must be reported before the queued state update').toBeLessThan(queued)
  })
