...
describe('SessionColorSwatches – a panel opened online must not write once the gateway drops', () => {
  ...
  it('reports a drag commit dropped when the gateway goes mid-debounce', async () => {
    // The one path `disabled` cannot cover: the PATCH is queued behind a 300ms
    // timer, so the drop lands between the gesture and the write — and a drop the
    // user is not told about is the silent loss this reporting store exists for.
    vi.useFakeTimers()
    try {
      ...
      expect(mocks.setSlotColorHex).not.toHaveBeenCalled()
      expect(probe.result.current.failure?.message).toMatch(/gateway offline/i)
    } finally {
      vi.useRealTimers()
    }
  })
