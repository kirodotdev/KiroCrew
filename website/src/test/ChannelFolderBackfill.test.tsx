...
describe('ChannelFolderBackfill', () => {
  ...
  it('invalidates a query key the app actually registers', async () => {
    // A wrong key is a SILENT no-op: `invalidateQueries` neither throws nor logs
    // for a key nothing is registered under, so an open sidebar kept showing every
    // session this just filed as unfiled. This said `['slots']`, which no
    // `useQuery` in the app defines.
    ...
    try {
      vi.stubGlobal('fetch', answer(report({ moved: [{ key: 'slack:1', title: 'A', label: 'Slack' }] })))
      renderButton()
      ...
      expect(keys, 'the slot list must be invalidated').toContain('["chat-slots"]')
      // The key that looked right and matched nothing.
