...
describe('save conflict token', () => {
  ...
  it('409 shows the changed-on-disk notice and keeps the buffer', async () => {
    ...
    // The Save button now performs an informed overwrite (the 409 armed its
    // separate overwrite token), so its label must say so while the banner is
    // up — a reflex second Cmd+S reads as the overwrite it is.
    expect(
  ...
  it('a metadata edit after the 409 keeps the warning and the overwrite label', async () => {
    // The 409 armed a separate overwrite token, so Save overwrites until
  ...
  it('holds the edit base and takes the overwrite token only from the 409 body', async () => {
    ...
    // edit base stays TOKEN_V1, while a 409 whose body names TOKEN_LIVE arms
    // only the informed-overwrite Save to send TOKEN_LIVE.
    const updateSpy = vi
      .fn()
      .mockRejectedValueOnce(
        new ApiError(
  ...
  it('a held Cmd+S never uses the 409 overwrite token on its key-repeat', async () => {
    // The OS auto-repeats keydown while the key is held. The first keydown's
    // save comes back 409 and arms the labelled Save with TOKEN_LIVE so that
    // the NEXT save is an informed overwrite -- and the repeat keydown, still
    // part of the same hold, would be that next save: an overwrite of content the
  ...
  it('a second save while one is in flight is dropped before an overwrite is armed', async () => {
    // A click or a distinct keystroke landing while the first PATCH is still
    // pending must not queue. If it did, a 409 from the first save could arm
    // the overwrite token before the second runs, turning the queued save into
    // an overwrite the user did not choose. The in-flight guard drops it instead.
  ...
  it('after a 409 Snapshot is a no-op that leaves the banner alone; only Save overwrites', async () => {
    // Snapshot never consumes the overwrite token, so with the banner up it
    // would send the edit base and 409 again -- and the save path clears the
    // banner before the request and re-arms it after, an unexplained flicker
    // whose text ("saving again will overwrite") is not true of Snapshot.
    // Neither Cmd+Shift+S nor the button may issue a PATCH or touch the banner
    // while the conflict is armed; plain Save keeps its informed overwrite.
    const updateSpy = vi
      .fn()
      .mockRejectedValueOnce(
        new ApiError(
    ...
    // Cmd+Shift+S with the banner up: no PATCH, banner untouched.
    ...
    // The Snapshot button is disabled and its title says only Save overwrites.
    const snapshotButton = screen.getByRole('button', { name: /^Snapshot$/ })
    expect(snapshotButton).toBeDisabled()
    expect(snapshotButton).toHaveAttribute('title', expect.stringMatching(/only Save \(Cmd\+S\) overwrites/))
    fireEvent.click(snapshotButton)
    await new Promise(r => setTimeout(r, 50))
    expect(updateSpy).toHaveBeenCalledTimes(1)
    expect(screen.getByText(/Content changed since you loaded it/)).toBeInTheDocument()

    // Plain Save is unchanged: it sends the overwrite token from the 409 body.
    fireEvent.click(screen.getByRole('button', { name: /Save — overwrite newer content/ }))
    await waitFor(() => expect(updateSpy).toHaveBeenCalledTimes(2))
    expect(updateSpy).toHaveBeenNthCalledWith(2, 'cr-queue', {
      content: '# v1 edited',
      snapshot: false,
      expected_token: TOKEN_LIVE,
    })
  ...
  it('a sync retry keeps the edit base and only labelled Save uses the conflict token', async () => {
    const published = mkArtifact({
      live_dirty: true,
      publication: {
        artifact_id: 'pub-1',
    ...
    const conflict = new ApiError(
      409,
      'artifact content changed since it was read',
      JSON.stringify({ error: 'conflict', current_token: TOKEN_LIVE, version: 2 }),
    )
    const updateSpy = vi
      .fn()
      .mockRejectedValueOnce(conflict)
      .mockRejectedValueOnce(conflict)
      .mockResolvedValueOnce(mkArtifact({
        content: '# v1 edited',
    ...
    await editAndDirty()
    const syncButton = await screen.findByRole('button', { name: /Snapshot to publish/ })
    ...
    fireEvent.click(syncButton)
    await waitFor(() => expect(updateSpy).toHaveBeenCalledTimes(2))
    expect(updateSpy).toHaveBeenNthCalledWith(1, 'cr-queue', {
      content: '# v1 edited',
      snapshot: false,
      expected_token: TOKEN_V1,
    })
    ...
    // Both sync actions stopped at the refused flush: neither reached its
    // snapshot=true mutation, and the dirty buffer remains intact.
    expect(screen.getByTestId('editor-stub')).toHaveValue('# v1 edited')
    expect(screen.getByText(/Content changed since you loaded it/)).toBeInTheDocument()
    ...
    expect(updateSpy).toHaveBeenNthCalledWith(3, 'cr-queue', {
      content: '# v1 edited',
      snapshot: false,
      expected_token: TOKEN_LIVE,
    })
