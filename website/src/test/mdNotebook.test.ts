...
describe('md-notebook/misc helpers', () => {
  it('cleans a typed name into a component every clone of the vault can hold', () => {
    ...
    // The consoles are devices under their own names too.
    for (const bad of ['CONIN$', 'conout$', 'ConIn$.md']) expect(portableName(bad)).toBe('')
    // Windows strips the spaces before an extension, so these resolve to the
    // device. `cleanComponent` strips only a TRAILING run, so an interior space
    // reaches the regex and the name would otherwise be created on POSIX and
    // block every Windows clone's checkout.
    for (const bad of ['CON .txt', 'con  .md', 'LPT1 .md', 'nul .', 'COM¹ .md'])
      expect(portableName(bad)).toBe('')
    // Not devices: the space is inside the stem, or the stem is only a prefix.
    expect(portableName('CON X.txt')).toBe('CON X.txt')
    expect(portableName('CONIN.md')).toBe('CONIN.md')
    expect(portableName('conception .md')).toBe('conception .md')
