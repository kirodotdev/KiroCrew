...
describe('the agent-switch notice', () => {
  ...
  it('offers the agent hand-off, which this surface has nothing to lose to', () => {
    // Required by the errors-use-error-notice rule for a surface whose inputs are persisted:
    // the alternative branch demands a comment naming a draft, and this notice protects none.
    render(<AgentSwitchNotice message="A turn is running." onDismiss={() => undefined} />)
    const notice = screen.getByTestId('agent-switch-error')
