...
describe('the agent-switch notice', () => {
  ...
  it('keeps danger severity for a switch that genuinely broke, not only withheld', () => {
    render(
      <AgentSwitchNotice
        message={agentSwitchFailureMessage(new ApiError(400, 'no such agent: nope', '{}'))}
        onDismiss={() => undefined}
      />,
    )
    const notice = screen.getByTestId('agent-switch-error')
  ...
  it('hands the agent the structured context, not only the sentence', () => {
    // The point of routing this refusal through the shared notice. The hand-off is worth
    // offering only if it arrives carrying what a diagnosis needs -- which endpoint answered,
    // with what status and `code`. Asserted on the STAGED PROMPT rather than on a prop, because
    // the notice shows LOCALIZED copy while the journal is keyed on the backend's prose: the
    // component's own message lookup cannot bridge that, so the context is present here only
    // while the report is passed explicitly.
    ...
    render(<AgentSwitchNotice message={agentSwitchFailureMessage(WORKSPACE_503)} onDismiss={() => undefined} />)
    fireEvent.click(screen.getByText(i18nT('components.askAgent.ask_the_agent')))

    const staged = sessionStorage.getItem(ERROR_HANDOFF_KEY) || ''
    expect(staged).toMatch(/\/api\/chat\/slots\/chat-1\/agent/)
    expect(staged).toMatch(/HTTP 503/)
    expect(staged).toMatch(/workspace_unavailable/)
  })
})
