...
describe('FolderConfigModal', () => {
  ...
  it('flags an uninstalled agent and blocks Save until it is changed', async () => {
    ...
    // renamed since save) is a VALIDATION ERROR at save time: the orphan stays
    // selectable and flagged so the user sees why, but Save is blocked until
    // they pick a valid agent (or clear to inherit). The form's contract is that
    // it only ever persists a valid agent; run-time resolution handles a saved
    // agent that later disappears by falling back to the global default.
    ...
    // Save is BLOCKED while the orphan is selected.
    expect((screen.getByTestId('folder-config-submit') as HTMLButtonElement).disabled).toBe(true)
    // Picking a valid agent clears the error and re-enables Save.
    fireEvent.click(await screen.findByRole('option', { name: 'kirocrew-dev' }))
    await waitFor(() =>
      expect((screen.getByTestId('folder-config-submit') as HTMLButtonElement).disabled).toBe(false),
    )
    fireEvent.click(screen.getByTestId('folder-config-submit'))
    expect(onSubmit).toHaveBeenCalledWith(expect.objectContaining({ defaultAgent: 'kirocrew-dev' }))
  })
  ...
  describe('project-scoped agent roster', () => {
    ...
    it('flags and blocks Save when the dir changes and the agent is gone', async () => {
      ...
      // cleared — it stays, flagged "(not installed)", and Save is blocked.
      ...
      await waitFor(() => expect(agentTrigger()).toHaveTextContent(/repo-dev.*not installed/i))
      expect((screen.getByTestId('folder-config-submit') as HTMLButtonElement).disabled).toBe(true)
    ...
    it('does not dead-end Save when a re-scoped roster scan errors', async () => {
      // GPT span=FolderConfigModal.tsx:275 / Opus: a terminal scan error
      // (retry:false) must NOT hold Save disabled forever. On error the roster
      // falls back to the global prop and the orphan flag is suppressed, so
      // there is no stale pick to guard — Save stays usable.
      mockAgents.mockImplementation((_sk?: string, dir?: string) =>
        dir === '/repo/ok'
      ...
      const { onSubmit } = open()
      fireEvent.change(screen.getByTestId('folder-config-name'), { target: { value: 'X' } })
      // Re-scope to a dir whose scan ERRORS.
      ...
      // Save must recover to enabled (no agent selected, global fallback) — not
      // stay dead-ended on the terminal error.
      await waitFor(() =>
        expect((screen.getByTestId('folder-config-submit') as HTMLButtonElement).disabled).toBe(false),
      )
      fireEvent.click(screen.getByTestId('folder-config-submit'))
    ...
    it('flags and blocks Save when the dir is cleared and the agent is gone', async () => {
      ...
      // The pick is NOT silently cleared: it stays, flagged, and Save is blocked.
      fireEvent.change(screen.getByTestId('folder-config-project-dir'), { target: { value: '' } })
      await waitFor(() => expect(agentTrigger()).toHaveTextContent(/repo-dev.*not installed/i))
      expect((screen.getByTestId('folder-config-submit') as HTMLButtonElement).disabled).toBe(true)
    })
    ...
    it('flags and blocks Save on an INHERITED re-scope (parent dir changes)', async () => {
      ...
      // agent it is flagged "(not installed)" and Save is blocked, so a save
      // cannot persist a pick the new scope does not contain.
      mockAgents.mockImplementation((_sk?: string, dir?: string) =>
        Promise.resolve({
