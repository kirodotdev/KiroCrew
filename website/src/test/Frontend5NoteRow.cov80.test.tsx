...
describe('md-notebook/NoteRow — inline rename', () => {
  ...
  it('seeds the field from the mirrored draft when there is one, and mirrors every keystroke back', () => {
    // The row unmounts during a search; the rename typed so far comes back
    // from the page rather than resetting to the title.
    const { a, input } = renderRenaming({ renameDraft: 'Renam' })
    expect(input.value).toBe('Renam')
    fireEvent.change(input, { target: { value: 'Renamed' } })
    expect(a.onRenameDraft).toHaveBeenCalledWith('Renamed')
    expect(input.value).toBe('Renamed')
  })

  it('previews the cleaned name under the field when the cleaner would change it', () => {
    const { container, input } = renderRenaming()
    const hint = () => {
      const id = input.getAttribute('aria-describedby')
      return id ? container.querySelector(`#${CSS.escape(id)}`)?.textContent : undefined
    }
    // The untouched title carries no hint at all.
    expect(hint()).toBeUndefined()
    fireEvent.change(input, { target: { value: '2026/Q1' } })
    expect(hint()).toBe('Will be renamed to “2026Q1”')
    // A name the cleaner leaves alone: no hint.
  ...
  it('abandons the edit on Escape and swallows other keys', () => {
    const { a, input } = renderRenaming()
    fireEvent.change(input, { target: { value: 'Renamed' } })
    // Escape while an IME composition is in flight is the IME's (it drops the
    // candidate): the field stays, the edit with it.
    fireEvent.keyDown(input, { key: 'Escape', isComposing: true })
    expect(a.onRenameEnd).not.toHaveBeenCalled()
...
describe('md-notebook/NoteRow — folder row actions and the new-folder field', () => {
  ...
  it('keeps a refused folder name in the field, marked invalid with the reason, until it is edited', async () => {
    const { container, a } = renderFolders({
  ...
  it('commits a typed name on Enter, once, and ends the field only once the folder exists', async () => {
    const calls: string[] = []
    const { container } = renderFolders({
      newFolderParent: 'beta',
      onNewFolderEnd: vi.fn(() => calls.push('end')),
      onNewFolder: vi.fn(async () => { calls.push('create'); return true }),
    })
    const input = container.querySelector('.mdnb-row input') as HTMLInputElement
    fireEvent.change(input, { target: { value: '  Projects ' } })
    fireEvent.keyDown(input, { key: 'Enter' })
    // A second Enter, or the blur a click elsewhere produces while the request
    // is in flight, must not create the folder twice.
    fireEvent.keyDown(input, { key: 'Enter' })
    fireEvent.blur(input)
  ...
  it('keeps the field and its draft when the creation is refused, and lets the user retry', async () => {
    const outcomes = [false, true]
    const { container, a } = renderFolders({
      newFolderParent: 'beta',
      onNewFolder: vi.fn(async () => outcomes.shift() as boolean),
    })
    const input = container.querySelector('.mdnb-row input') as HTMLInputElement
    fireEvent.change(input, { target: { value: 'Projects' } })
    fireEvent.keyDown(input, { key: 'Enter' })
    await act(async () => {})
    // Refused by the backend: the field is still there with the name typed.
    expect(a.onNewFolderEnd).not.toHaveBeenCalled()
    expect(container.querySelector('.mdnb-row input')).toBe(input)
    expect(input.value).toBe('Projects')
    // Escape works again after the refusal; so does a second Enter.
    fireEvent.keyDown(input, { key: 'Enter' })
    await act(async () => {})
    expect(a.onNewFolder).toHaveBeenCalledTimes(2)
    expect(a.onNewFolderEnd).toHaveBeenCalledTimes(1)
  })
  ...
  it('abandons on Escape without creating anything — but not on a composition\'s Escape', () => {
    const { container, a } = renderFolders({ newFolderParent: 'beta' })
    const input = container.querySelector('.mdnb-row input') as HTMLInputElement
    fireEvent.change(input, { target: { value: 'Projects' } })
    // An IME's Escape drops the candidate, not the field: the draft survives.
    fireEvent.keyDown(input, { key: 'Escape', isComposing: true })
    expect(a.onNewFolderEnd).not.toHaveBeenCalled()
    expect(input.value).toBe('Projects')
    fireEvent.keyDown(input, { key: 'Escape', keyCode: 229 })
