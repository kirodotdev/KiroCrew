...
describe('interactive model visibility', () => {
  ...
  it('is wired only into ChatPage and ChatPane consumers', () => {
    ...
    expect(chatPage).toContain('useFilteredDropdown(modelPickerModels, m => modelDisplayName(m.name))')
    ...
    expect(chatPane).toContain('useFilteredDropdown(modelPickerModels, m => modelDisplayName(m.name))')
