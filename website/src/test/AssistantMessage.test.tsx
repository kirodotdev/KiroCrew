...
import { OPTION_MARKER_PATTERN_SOURCE, OPTION_ACTION_MARKER_PATTERN_SOURCE, MARKER_PATTERN_FLAGS, matchActionMarkers } from '../app-sdk/protocol/optionMarker'
...
describe('parseOptions', () => {
  ...
  it('does not catastrophically backtrack on adversarial `[OPTIONS:` input', () => {
    ...
    for (const [name, re] of patterns) {
      ...
      //
      // Each pair arm is pinned to the COMPLETE head alternation and to its OWN closer.
      // `${opener}(?!` alone did not bind them: an arm narrowed to `(?!OPTIONS?:)` still
      // contains `\[(?!`, and the head sweep below still passes because `OPTION-ACTIONS:`
      // reaches the source through `SIBLING_MARKER` no matter what any pair arm guards —
      // so the ratchet read green while a nested action head could be eaten as a label.
      // The body between guard and closer is deliberately NOT pinned: that is the part a
      // legitimate improvement rewrites, and a ratchet that fails on one gets deleted.
      const headGuard = `(?!${heads.join('|')})`
      const continuesGuard = '(?![ \\t]*[|,]|[\\]\\u3011\\uFF3D\\u3015])'
      const pairs: [string, string][] = [
        ['\\[', '\\]'], ['\\u3010', '\\u3011'], ['\\uFF3B', '\\uFF3D'], ['\\u3014', '\\u3015'],
      ]
      for (const [opener, closer] of pairs) {
        expect(src, `${name} guards opener ${opener} against EVERY head`)
          .toContain(`${opener}${headGuard}`)
        expect(src, `${name} closes ${opener} on its OWN paired closer`)
          .toContain(`${closer}${continuesGuard}`)
      }
      // And the body's ARM SET exactly, respelled with the generalised guard. Every check
      // here is satisfied by any SUPERSET of the arms, so presence-only assertions let an
      // arm be APPENDED — `|[${closerChars}]`, the unconditional closer that ran a label body
      // to the line's last closer and deleted the sentence after it (#9284) — and the
      // widening would ship green. Pinning the set is what makes this a ratchet.
      const openerChars = pairs.map(([o]) => o).join('')
      const closerChars = pairs.map(([, c]) => c).join('')
      const bodyChar = `[^${openerChars}${closerChars}\\n]`
      const continues = `[ \\t]*[|,]|[${closerChars}]`
      const armSet = [
        ...pairs.map(([o, c]) => `${o}${headGuard}${bodyChar}*${c}(?!${continues})`),
        `[${openerChars}]${headGuard}`,
        `[${closerChars}](?=${continues})`,
        bodyChar,
      ].join('|')
      expect(src, `${name} pins the body's arm SET exactly`).toContain(`(?:${armSet})*`)
    ...
    // And the parse itself stays linear on an adversarial body. The fixture is sized BELOW
    // `HEAD_CAP`: past it both entry points short-circuit on the head count and return before
    // either pattern runs, so a 20k-head input passes even with a catastrophically
    // backtracking body — that half of the ratchet asserted nothing. One head, a long
    // unterminated body of openers, and the elapsed time is what a wedge would break.
    const adversarial = (head: string): string => `${head} ${'a['.repeat(4000)}`
    const started = performance.now()
    expect(parseOptions(adversarial('[OPTIONS:')).options).toEqual([])
    expect(matchActionMarkers(adversarial('[OPTION-ACTIONS:'))).toEqual([])
    expect(performance.now() - started, 'tempered body stays linear').toBeLessThan(500)
    // Over the cap is a SEPARATE property: every marker refused wholesale rather than parsed.
