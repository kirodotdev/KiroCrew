...
vi.mock('../api/client', () => ({
  api: {
    ...
    // Stubbed so the ONLY failing fetch in this file is the roster one under
    // test: `useAgents` (in a child) calls `agentCatalog`, and the chat-folder
    // picker calls `chatFolders`. Left unstubbed, the folder query fails and
    // renders its own `role="alert"` notice, which the alert assertion below
    // would then attribute to the form-wide notice.
