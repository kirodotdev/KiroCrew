...
describe('McpTab needs_auth status', () => {
  ...
  it('reports a held runtime grant as signed in, without claiming it still works', async () => {
    ...
    expect(screen.queryByText('Not verified')).not.toBeInTheDocument()
    // Warn, not ok: a stored grant is evidence a sign-in happened, never proof it
    // has not since been revoked, so the tone must not read as certified-working.
    expect(screen.getByText('Signed in').className).toContain('text-warn')
  ...
  it('tells the user where the sign-in happens, and offers no control that cannot do it', async () => {
    ...
    expect(screen.getByText(/start a new session, and send a message/)).toBeInTheDocument()
    // The flow still has an ending, but it rides the badge's hover rather than the
    // always-visible cell: it is worth nothing until the sign-in is done, and this
    // is a dense table where every extra sentence is paid for by every row.
    expect(screen.queryByText(/use the Probe MCP servers button/)).not.toBeInTheDocument()
