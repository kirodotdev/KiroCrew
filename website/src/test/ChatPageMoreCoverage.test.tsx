...
describe('ChatPage run-in-terminal reuse-current (#11641)', () => {
  ...
  it('opens a fresh terminal without a reuse-revert notice when the config fetch fails (F1 reversed)', async () => {
    // A failed kirocrewConfig read means the saved reuse setting is unknown.
    // We have no positive proof reuse was ever enabled, so minting a fresh tab
    // matches the reuse-off default for the majority and must NOT surface a
    // "reuse silently reverted" notice — that would assert a preference the
    // user never set. The command still runs: a fresh tab is minted, quietly.
    ...
    try {
      ...
      await waitFor(() => expect(dock.result.current.tabs.length).toBe(1))
      // No reuse-revert notice: the config-load failure is silent.
