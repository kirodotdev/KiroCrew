...
describe('WorktreePicker', () => {
  ...
  it('a 409 is recognised by its code, not by English prose in the message', async () => {
    // The endpoint answers with `code: 'worktree_dirty'`; the confirm row must
    // key off that rather than matching /uncommitted/ in the human message,
    // which would also match the OPPOSITE ("could not check") case below.
    const err = Object.assign(new Error('localised text with no English keyword'), {
      status: 409,
      body: JSON.stringify({ error: 'localised', code: 'worktree_dirty' }),
    })
    ;(api.worktreeRemove as ReturnType<typeof vi.fn>).mockRejectedValue(err)
    setup()
    fireEvent.click(await screen.findByRole('button', { name: /remove worktree/i }))
    expect(await screen.findByRole('button', { name: /remove anyway/i })).toBeInTheDocument()
    expect(await screen.findByText(/will be lost/i)).toBeInTheDocument()
  })

  it('an unverifiable dirty state does not claim there ARE uncommitted changes', async () => {
    // `worktree_dirty_unknown` means git could not be asked. Reporting it as
    // "uncommitted changes" would talk the user into a destructive click on a
    // fact the backend explicitly reported as unknowable.
    const err = Object.assign(new Error('Could not check for uncommitted changes; not removing.'), {
      status: 409,
      body: JSON.stringify({ error: 'could not check', code: 'worktree_dirty_unknown' }),
    })
    ;(api.worktreeRemove as ReturnType<typeof vi.fn>).mockRejectedValue(err)
    setup()
    fireEvent.click(await screen.findByRole('button', { name: /remove worktree/i }))
  ...
  it('a non-dirty failure surfaces as an error, not as a force prompt', async () => {
    const err = Object.assign(new Error('Access denied'), {
      status: 403,
      body: JSON.stringify({ error: 'Access denied', code: 'worktree_path_sensitive' }),
    })
    ;(api.worktreeRemove as ReturnType<typeof vi.fn>).mockRejectedValue(err)
    setup()
    fireEvent.click(await screen.findByRole('button', { name: /remove worktree/i }))
    expect(await screen.findByText(/access denied/i)).toBeInTheDocument()
    expect(screen.queryByRole('button', { name: /remove anyway/i })).not.toBeInTheDocument()
  })

  it("names THIS session in the tooltip when the session owning the tree is our own", async () => {
    // `active_session` covers both "another session" and "this one"; the row can
    // show the `this session` badge while the tooltip tells the user to switch
    // some other session's project — an instruction with no referent.
    ;(api.worktreeList as ReturnType<typeof vi.fn>).mockResolvedValue({
      worktrees: [
        wt({ path: '/repo', branch: 'trunk', is_main: true }),
        wt({ path: '/repo-wt-feature', branch: 'feat/x', is_main: false, active_session: 'chat-1' }),
      ],
    })
    setup()
    const btn = await screen.findByRole('button', { name: /remove worktree/i })
    expect(btn.getAttribute('title')).toMatch(/switch this session/i)
    expect(btn.getAttribute('title')).not.toMatch(/another session/i)
  })

  it('still names the OTHER session when the tree belongs to one', async () => {
    ;(api.worktreeList as ReturnType<typeof vi.fn>).mockResolvedValue({
      worktrees: [
        wt({ path: '/repo', branch: 'trunk', is_main: true }),
        wt({ path: '/repo-wt-feature', branch: 'feat/x', is_main: false, active_session: 'chat-9' }),
      ],
    })
    setup()
    const btn = await screen.findByRole('button', { name: /remove worktree/i })
    expect(btn.getAttribute('title')).toMatch(/another session/i)
  })

  it('Escape backs out of a pending force-confirm before it closes the picker', async () => {
    // The confirm row carries only the destructive action, so Escape is what
    // replaces a Cancel button — and it must not skip straight to closing.
    ;(api.worktreeRemove as ReturnType<typeof vi.fn>).mockResolvedValue({ dirty: true, error: 'uncommitted' })
    ...
    // First Escape clears the confirm and leaves the picker open.
    await waitFor(() =>
      expect(screen.queryByRole('button', { name: /remove anyway/i })).not.toBeInTheDocument())
    expect(onOpenChange).not.toHaveBeenCalled()

    // A second Escape, with nothing pending, closes it.
    fireEvent.keyDown(document, { key: 'Escape' })
