...
describe('goalDrafts', () => {
  ...
  it.each(['raw', 'atomic'] as const)(
    'retires a migrated timestamp-less %s legacy input without rewriting it',
    format => {
      const slot = `chat-timestampless-${format}`
      const now = Date.now()
      vi.spyOn(Date, 'now').mockReturnValue(now)
      const value = draft(`timestamp-less ${format} goal`)
      const legacyPayload = format === 'atomic'
        ? { __slotDraftStore: 1, drafts: { [slot]: value }, timestamps: {} }
        : { [slot]: value }
      localStorage.setItem(LEGACY_GOAL_DRAFTS_KEY, JSON.stringify(legacyPayload))
  ...
  it.each([
    {
      name: 'draft survives an older atomic-envelope clear',
      current: draft('new unsynced draft'),
      stale: { deleted: true },
    },
    {
      name: 'tombstone survives an older atomic-envelope save',
      current: null,
      stale: draft('old tab resurrection'),
    },
  ])('$name', ({ current, stale }) => {
    const slot = 'chat-version-skew'
    const currentStamp = Date.now()
    saveGoalDraft(slot, current, currentStamp)

    localStorage.setItem(LEGACY_GOAL_DRAFTS_KEY, JSON.stringify({
      __slotDraftStore: 1,
      drafts: { [slot]: stale },
      timestamps: { [slot]: currentStamp + 10_000 },
    }))
  ...
  it.each([
    { format: 'raw', legacy: draft('stale raw draft'), current: null },
    { format: 'raw', legacy: { deleted: true }, current: draft('newer v2 draft') },
    { format: 'atomic', legacy: draft('stale atomic draft'), current: null },
    { format: 'atomic', legacy: { deleted: true }, current: draft('newer v2 draft') },
  ] as const)(
    'does not resurrect $format legacy opposite-mode state after v2 expires',
    ({ format, legacy, current }) => {
      const slot = `chat-expired-opposite-${format}-${current ? 'draft' : 'clear'}`
  ...
  it('retires a future-skewed stale-tab draft while preserving equal-stamp identity', () => {
    ...
    localStorage.setItem(LEGACY_GOAL_DRAFTS_KEY, JSON.stringify({
      __slotDraftStore: 1,
      drafts: { [slot]: draft('future-skewed stale tab') },
      timestamps: { [slot]: currentStamp + 10_000 },
    }))
