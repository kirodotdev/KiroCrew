 ...
 * sibling rows do — aria-disabled and dimmed, with the flyout left openable so a
 * mid-browse disconnect cannot snatch it away — which is a claim about the PROPS
 * they pass, so the primitives are captured here rather than rendered.
 */
...
describe('the folder-move submenu trigger, offline', () => {
  it('passes NO native disabled, and leaves the flyout openable', () => {
    mount(false)
    ...
    // Closing it mid-browse yanked the flyout from under the pointer, so the
    // container stays uncontrolled and the refusal lives on its items.
    expect(seen.sub.open).toBeUndefined()
  })

  it('carries the reason inside the flyout, where the rows are', () => {
    const { container } = mount(false)
    const reason = container.querySelector('[data-testid="folder-move-offline-reason"]')
    expect(reason).not.toBeNull()
    expect(reason!.textContent).toContain('move folders')
    expect(reason!.textContent).not.toContain('session changes')
    // LAST, like the top-level menu: above the destinations a reconnect unmounts
    // it and shifts every row up under a mid-aim pointer.
    const content = reason!.parentElement!
    expect(content.children.length).toBeGreaterThan(1)
    expect(content.lastElementChild).toBe(reason)
  })

  it('names the caller\u2019s own verb when one is supplied', () => {
    const store = createTestStore()
    const { container } = render(
      <QueryClientProvider client={new QueryClient({ defaultOptions: { queries: { retry: false } } })}>
        <Provider store={store}>
          <FolderMoveSubmenu folders={[]} onPick={() => {}} variant="dropdown" offlineVerb="move sessions" />
        </Provider>
      </QueryClientProvider>,
    )
    const reason = container.querySelector('[data-testid="folder-move-offline-reason"]')
    expect(reason!.textContent).toContain('move sessions')
  })
