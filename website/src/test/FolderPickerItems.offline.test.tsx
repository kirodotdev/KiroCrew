...
describe('a folder destination row, gateway offline', () => {
  ...
  it('moves when connected, holding the menu open so a refusal has a host — the control', () => {
    const onPick = mount(true)
    const event = select(rows[1])
    // Defaulted either way now: a rejected move reports into the menu's ErrorNotice,
    // which cannot render if the pick dismissed its host first.
    expect(event.defaultPrevented).toBe(true)
