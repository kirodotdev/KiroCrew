 ...
 *
 * The method is a thin builder, so what can break is what is asserted here: the path
 * (slot percent-encoded), which optional body fields are omitted rather than sent as
 * `undefined`, that an explicit `visibleOnly: false` survives a truthiness-guarded
 * spread rather than looking like an omission, and that the response booleans arrive
 * separately — a caller sequences a destructive close on `appended === true`, so a
 * collapsed or `any`-typed response is a correctness bug, not a style one. `maxAge`
 * and `ephemeral` are deliberately NOT in this options type: no production caller
 * passed either, and the similarly-named options on `chatSlotContext` are a different
 * method's. Conventions follow `ApiClient.coverage.test.tsx`.
 */
