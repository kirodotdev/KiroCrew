...
export default function PromptsTab() {
  ...
  const select = (p: Prompt) => {
    ...
    //
    // The exception is a read that FAILED: the pane says so and offers nothing
    // else to click, so re-clicking the row is the only retry a user would
    // think to try, and a no-op there is a dead end. There is no editor to
    // protect in that state, because a failed read is never an edit base.
    if (key === selectedKey) {
      openDetail()
      if (readOnlyReason === 'failed') void load(p)
      return
    }
  ...
  return (<>
    ...
    {/* Create Prompt Modal. guardAccidentalDismiss: the body is typed-in work,
        so a backdrop click or Escape must not discard it silently. */}
    <Modal open={creating} onClose={() => { if (!createPrompt.isPending) { setCreating(false); setMutationError('') } }} title={i18nT('pages.overview.promptsTab.create_new_prompt')} maxWidth={560} guardAccidentalDismiss footer={<>
      <Btn disabled={createPrompt.isPending} onClick={() => { setCreating(false); setMutationError('') }}>{i18nT('pages.overview.promptsTab.cancel')}</Btn>
