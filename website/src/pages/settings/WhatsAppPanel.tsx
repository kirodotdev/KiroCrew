...
import { api, type WhatsAppConfigSave } from '../../api/client'
 ...
 * (POST /api/channels/whatsapp/qr/start then poll .../status) and never handles
...
export function WhatsAppPanel() {
  ...
  const startLogin = useMutation({
    ...
    onError: (e: unknown) => {
      setErrMsg(e instanceof Error ? e.message : i18nT('pages.settings.whatsAppPanel.could_not_start_the_login_flow'))
      setPhase('error')
    },
  })
  ...
  const save = (
    patch: Partial<WhatsAppConfigSave>,
    onRevert?: () => void,
    onSaved?: () => void,
  ) => {
  ...
  const credentialSet = !!data?.configured
    ...
    <div className="flex flex-col gap-5" data-testid="whatsapp-panel">
      ...
      <div className="flex items-start gap-3">
        ...
        <div className="min-w-0">
          <h3 className="text-[15px] font-semibold text-text-strong m-0">{i18nT('pages.settings.whatsAppPanel.wechat')}</h3>
          <p className="text-[12.5px] text-muted mt-1 mb-0">
            {i18nT('pages.settings.whatsAppPanel.talk_to_your_agent_from_personal_wechat_over_ten')}
          </p>
        </div>
      </div>
      ...
      >
        {isError ? (
          ...
          <>
            ...
            <span className="text-[12.5px] text-ok font-medium">{i18nT('pages.settings.whatsAppPanel.connected')}</span>
          </>
        ) : credentialSet ? (
          <>
            <span className="w-1.5 h-1.5 rounded-full bg-warn shrink-0" />
            <span className="text-[12.5px] text-warn font-medium">{i18nT('pages.settings.whatsAppPanel.signed_in_restart_to_connect')}</span>
          </>
        ) : (
          <span className="text-[12.5px] text-muted">{i18nT('pages.settings.whatsAppPanel.not_signed_in')}</span>
        )}
      </div>
      ...
      <div className="rounded-lg border border-border bg-card p-3.5">
        <div className="flex items-center justify-between gap-3">
          <div className="min-w-0">
            <div className="text-[13px] font-semibold text-text-strong">{i18nT('pages.settings.whatsAppPanel.sign_in_with_wechat')}</div>
            <div className="text-[11.5px] text-muted mt-0.5">
              {i18nT('pages.settings.whatsAppPanel.scan_the_code_with_the_wechat_mobile_app_then_co')}
            </div>
          </div>
          {!readOnly && (
            <button
            ...
            >
              ...
              {credentialSet ? i18nT('pages.settings.whatsAppPanel.sign_in_again') : i18nT('pages.settings.whatsAppPanel.connect_via_qr')}
            </button>
          )}
        </div>
        ...
        {(phase === 'waiting' || phase === 'scanned') && (
          <div className="mt-3 flex flex-col items-center gap-2" data-testid="whatsapp-qr">
            {qrImg ? (
              <img
                src={qrImg}
                alt={i18nT('pages.settings.whatsAppPanel.wechat_login_qr_code')}
                width={180}
                height={180}
            ...
            <div className="flex items-center gap-1.5 text-[12px] text-muted">
              ...
              {phase === 'scanned' ? i18nT('pages.settings.whatsAppPanel.scanned_confirm_in_wechat') : i18nT('pages.settings.whatsAppPanel.waiting_for_scan')}
            </div>
          </div>
        )}
      ...
      <div data-testid="whatsapp-enabled" className="max-w-[380px]">
        <SettingsToggle
          label={i18nT('pages.settings.whatsAppPanel.enable_the_wechat_channel')}
      ...
      <div>
        ...
        <div className="block" data-testid="whatsapp-dm-policy">
          ...
          {/* maxWidth: the native select was content-sized; the Radix trigger is
              ...
              content-scaled. */}
          <SimpleSelect
            options={['self', 'allowlist', 'open', 'disabled']}
            optionLabels={[
              i18nT('pages.settings.whatsAppPanel.only_me_the_default'),
              i18nT('pages.settings.whatsAppPanel.me_plus_allowed_numbers'),
              i18nT('pages.settings.whatsAppPanel.only_allowed_wa_ids'),
              i18nT('pages.settings.whatsAppPanel.nobody_ignore_all_messages'),
            ]}
      ...
      {data?.dm_policy === 'allowlist' && (
        <div data-testid="whatsapp-allowlist">
          <TagListEditor
            ...
            description={i18nT('pages.settings.whatsAppPanel.allowed_wechat_user_ids_empty_deny_all_fail_clos')}
            values={data?.allowed_wa_ids || []}
            placeholder={i18nT('pages.settings.whatsAppPanel.wxid')}
      ...
      <p className="text-[11.5px] text-muted m-0">
        {i18nT('pages.settings.whatsAppPanel.group_chats_are_not_supported_ilink_bot_identiti')}{' '}
        <a
          href={SETUP_GUIDE}
          target="_blank"
