...
describe('SecurityPanel — Docker registry access', () => {
  ...
  it('resets a cancelled permanent choice to the bounded duration', async () => {
    vi.mocked(api.getDockerRegistryAccess).mockResolvedValue({ enabled: false, supported: true })
    renderWithProviders(<SecurityPanel />, { route: '/?section=docker', store: linuxStore() })
    const toggle = await screen.findByRole('switch', { name: LABEL() })
    fireEvent.click(toggle)
    fireEvent.click(screen.getByRole('radio', {
    ...
    fireEvent.click(screen.getByRole('button', { name: i18nT('pages.settings.securityPanel.cancel') }))
    fireEvent.click(toggle)
    expect(screen.getByRole('radio', {
      name: i18nT('pages.settings.securityPanel.docker_access_six_hours'),
    })).toHaveAttribute('aria-checked', 'true')
    expect(screen.getByRole('checkbox')).not.toBeChecked()
  })

  it.each([true, false])('shows the saved grant duration (permanent=%s)', async permanent => {
    vi.mocked(api.getDockerRegistryAccess).mockResolvedValue({
      enabled: true, supported: true, permanent,
      expires_at: permanent ? null : 2_000_000_000,
    })
    renderWithProviders(<SecurityPanel />, { route: '/?section=docker', store: linuxStore() })
    await screen.findByRole('switch', { name: LABEL() })
    const duration = await screen.findByRole('status')
    expect(duration).toHaveTextContent(permanent
