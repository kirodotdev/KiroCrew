...
describe('SecurityPanel — agent identity', () => {
  ...
  it('states the consequence of Save beside the button for the pending posture', async () => {
    ...
    // Workload picked before a name: Save is disabled FOR THE NAME, and the
    // consequence line says exactly that rather than describing a Save that
    // would record the identity (a button the reader cannot press).
    await pickIdentityPosture('workload')
    expect(screen.getByTestId('agentcore-save-consequence')).toHaveTextContent(
      i18nT('pages.settings.securityPanel.agent_identity_save_consequence_name_required'),
    )
    expect(
      screen.getByRole('button', { name: i18nT('pages.settings.securityPanel.agent_identity_save') }),
    ).toBeDisabled()

    // Named, still no Gateway URL: grants nothing yet, and says so.
    fireEvent.change(
      screen.getByLabelText(i18nT('pages.settings.securityPanel.agent_identity_name')),
      { target: { value: 'kirocrew-e2e' } },
    )
  ...
  it('does not let a late preview for one Gateway unlock the commit for another', async () => {
    // URL A's preview is in flight when the draft moves to URL B. The response
    // is labelled with A (the request), so B stays "preview first" and locked.
    let resolveA: ((v: unknown) => void) | undefined
    ;(api.previewAgentcoreGateway as ReturnType<typeof vi.fn>).mockImplementation(
    ...
    const urlInput = screen.getByLabelText(i18nT('pages.settings.securityPanel.agent_identity_gateway_url'))
    fireEvent.change(urlInput, { target: { value: 'https://a.example.test/mcp' } })
    fireEvent.click(screen.getByRole('button', { name: i18nT('pages.settings.securityPanel.agent_identity_preview') }))
    await waitFor(() => expect(api.previewAgentcoreGateway).toHaveBeenCalledWith({
      gateway_url: 'https://a.example.test/mcp',
      posture: 'workload',
    }))
    // Draft moves on before A answers.
    fireEvent.change(urlInput, { target: { value: 'https://b.example.test/mcp' } })
    resolveA!({
      ...CATALOG_IDLE,
      code: 'ok',
      preview: true,
      posture: 'workload',
      gateway_url: 'https://a.example.test/mcp',
      gateway: { id: 'a', name: 'a', status: 'READY', authorizer_type: 'AWS_IAM', gateway_url: '', status_reasons: [] },
      targets: [
        {
          target_id: 'T1', name: 'ticketing-mcp', target_type: 'MCP_SERVER', status: 'READY',
          listing_mode: 'DEFAULT', last_synchronized_at: '', pending_auth: false,
          authorization_url: null, syncable: true, status_reasons: [],
        },
      ],
      targets_error: null,
      tools: { reachable: false, skip_reason: 'listed_after_save', items: [], via: null },
    })
    // Give the resolved mutation a tick to land, then assert B is still locked.
    ...
    expect(screen.queryByText('ticketing-mcp')).toBeNull()
  })

  it('requires a fresh preview when the draft returns to a previously previewed Gateway', async () => {
    // Preview A (unlocks), edit to B (locks), edit back to A: the earlier
    // snapshot is gone -- A's targets may have changed -- so Save stays locked
    // until a new preview lands.
    ;(api.previewAgentcoreGateway as ReturnType<typeof vi.fn>).mockResolvedValue({
      ...CATALOG_IDLE,
      code: 'ok',
      preview: true,
    ...
    fireEvent.change(urlInput, { target: { value: 'https://a.example.test/mcp' } })
    fireEvent.click(screen.getByRole('button', { name: i18nT('pages.settings.securityPanel.agent_identity_preview') }))
    const grantName = i18nT('pages.settings.securityPanel.agent_identity_grant_button_workload', {
      count: 1,
      targets: 'ticketing-mcp',
    })
    await screen.findByRole('button', { name: grantName })
    // Away and back.
    fireEvent.change(urlInput, { target: { value: 'https://b.example.test/mcp' } })
    fireEvent.change(urlInput, { target: { value: 'https://a.example.test/mcp' } })
    expect(screen.queryByRole('button', { name: grantName })).toBeNull()
    expect(screen.getByTestId('agentcore-save-consequence')).toHaveTextContent(
      i18nT('pages.settings.securityPanel.agent_identity_save_preview_first'),
    )
    expect(
