  ...
  const run = () => {
    ...
    void fetch('/api/channel-folders/backfill', {
      ...
      .then(async r => {
        ...
        // `['chat-slots']`, which is the key the sidebar's slot list is actually
        // registered under (`ChatSidebar.tsx`, `queryFn: () => api.chatSlots()`).
        // This said `['slots']`, which no `useQuery` in the app defines, so the
        // invalidation matched nothing and an open sidebar kept showing every
        // session this just filed as unfiled until something else refetched.
        void qc.invalidateQueries({ queryKey: ['chat-slots'] })
      })
