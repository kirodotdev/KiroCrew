...
export default function ArtifactDetailPage({ popout = false }: { popout?: boolean } = {}) {
  ...
  // The optimistic-concurrency base token, captured when editing STARTS and
  // held fixed for the whole edit session. Reading artifact.content_token at
  // save time instead would let any background refetch (a tag mutation's
  // invalidate, a WS artifact-change broadcast, staleness) silently refresh
  // the token underneath a buffer the user opened against OLDER content —
  // authenticating a stale save the guard exists to refuse. Only a 409's own
  // deliberate rebase path below updates it mid-edit, because that is the one
  // place the user has been SHOWN the content changed.
  const editBaseToken = useRef<string | undefined>(undefined)
  // Mirrors `saving` for the save path itself: a second save entering while
  // one is in flight (a repeated shortcut, a click during the request) would
  // run against whatever token the first one leaves behind — after a 409 that
  // is the LIVE token, and the second save becomes an overwrite the user never
  // chose. State is stale inside the callback closure; the ref is not.
  ...
  const handleSave = useCallback(async (snapshot = false) => {
    ...
    try {
      ...
      //
      // expected_token is the token captured when this edit session STARTED
      // (see editBaseToken): the save only applies if the live content still
      // matches what the editor was opened against. A 409 means someone
      // else — another window, an agent iteration, an external write to a
      // file-backed source — changed the content in between, and silently
      // overwriting them is the data-loss path this guard exists to close.
      const saved = await api.updateArtifact(artifact.slug, {
        content: editedContent,
        snapshot,
        expected_token: editBaseToken.current,
      }) as Artifact
      // Roll the base forward to what THIS save produced, so continued
      ...
      if (err instanceof ApiError && err.status === 409) {
        ...
        // buffer is intact. Rebase the held token from the 409 body itself —
        // the one mid-edit token refresh that is legitimate, because the
        // banner is showing the user the content changed — and refetch so
        // the viewer shows the live content. A subsequent Save is then an
        // informed overwrite instead of a silent one.
        try {
          const body = JSON.parse((err as ApiError).body || '{}') as { current_token?: string }
          if (body.current_token) editBaseToken.current = body.current_token
  ...
  useEffect(() => {
    ...
    const h = (e: KeyboardEvent) => {
      ...
      // rebase the held token to the live one (so the NEXT save is an informed
      // overwrite); a repeat firing on the same hold would then be that next
      // save, overwriting content the user has not seen. Repeats never save.
    ...
    <>
          ...
          <span className="mc-art-toolbar ml-auto flex flex-wrap items-center justify-end gap-2 text-[13px] text-muted">
            ...
            {editing ? (
              <>
                ...
                >
                  {/* After a 409 the held token has been rebased onto the newer
                      content, so this same button now overwrites it. The label
                      says so while the conflict banner is up: a reflex second
                      Cmd+S must read as the overwrite it is. */}
                  {saving
      ...
      <div className="px-4 md:px-6 pb-8 overflow-y-auto flex-1 min-h-0">
        ...
        {/* The conflict notice is its own state, not a flavour of saveError:
            ...
            clears saveError for its own message) because the held token was
            rebased onto the newer content when the 409 arrived, and the Save
            button performs that overwrite until a save attempt, Cancel,
            Revert or navigation ends the edit. Clearing the warning while
            keeping the rebased token would hide exactly what the label warns
            about. Rendered only WHILE editing: the warning is about the edit
            buffer, so whichever way an edit ends (Cancel, a version switch,
            a flush before a pull, navigation) there is no draft left for it
            to describe. */}
