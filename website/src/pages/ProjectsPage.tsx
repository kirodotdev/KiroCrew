          ...
          <>
            <div className="px-4 py-2 flex items-center gap-2 border-b border-border shrink-0">
              ...
              {!selectedRun.running && selectedRun.status !== 'planned' && selectedRun.status !== 'planning' && <>
                ...
                {selectedRun.status !== 'paused' && selectedRun.status !== 'completed' && selectedRun.status !== 'cancelled' && <Btn className="h-8 px-3 border-accent text-accent font-semibold hover:bg-accent hover:text-accent-fg" onClick={() => runAction(() => api.retryTaskRun(selectedRun.task_id, 1), load)}><RotateCcw className="lucide-inline" /> {i18nT('pages.projectsPage.restart')}</Btn>}
