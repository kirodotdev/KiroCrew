  ...
  const handleAdd = () => {
    if (addType === 'bedrock_kb') {
      ...
      if (profile !== addKbProfileCommitted) {
        // The submit gate and the consent card key on the blur-committed
        // profile, so a profile typed but not yet blurred (or edited after
        // the grant) is a target no grant covers: saving it would POST past a
        // button that read ready and the server would refuse with
        // bedrock_kb_grant_changed. Commit it instead, which re-targets the
        // card and the gate to the value the user actually typed, and point
        // the user at the card: the click otherwise adds nothing visible.
        ...
        <div className="border border-border rounded-lg p-4 bg-bg-elevated space-y-3">
          ...
          {addType === 'local_file' ? (
            ...
            <>
              ...
              <input id="bedrock-kb-name" aria-label={i18nT('pages.knowledge.sourcesList.source_name_optional')} value={addName} onChange={e => setAddName(e.target.value)} placeholder={i18nT('pages.knowledge.sourcesList.name_optional')}
              ...
              <div className="flex gap-2">
                ...
                <div className="flex-1 min-w-0">
                  <label htmlFor="bedrock-kb-profile" className="block text-[11px] text-muted">{i18nT('pages.knowledge.sourcesList.aws_profile')}</label>
                  <input id="bedrock-kb-profile" aria-label={i18nT('pages.knowledge.sourcesList.aws_profile')} value={addKbProfile} onChange={e => setAddKbProfile(e.target.value)}
