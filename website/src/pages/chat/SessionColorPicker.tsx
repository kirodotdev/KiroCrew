  ...
  return (
    <Popover open={open} onOpenChange={o => { setOpen(o); if (!o) setSaveError(null) }}>
      <PopoverTrigger asChild>
        <button className="group shrink-0 cursor-pointer transition-all pl-1 hover:brightness-125" title={i18nT('pages.chat.sessionColorPicker.session_color')} aria-label={i18nT('pages.chat.sessionColorPicker.session_color')}>
          <span className={`block w-3 h-3 rounded-full border-[1.5px] transition-colors ${color ? '' : 'border-muted group-hover:border-text'}`} style={color ? { background: color, borderColor: color, boxShadow: `0 0 4px ${color}` } : { background: 'transparent' }} />
        </button>
      </PopoverTrigger>
