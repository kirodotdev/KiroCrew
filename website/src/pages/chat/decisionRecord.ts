 ...
 * than no receipt.
 *
 * The field is read from the row's top level AND from `meta`, the same split
 * `CompactionCard.noticeKindOf` reads `kind` through: a row that arrived over
 * the websocket carries the gateway's own keys, and one reloaded from history
 * carries them under `meta`.
 *
