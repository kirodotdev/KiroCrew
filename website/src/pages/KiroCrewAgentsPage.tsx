...
import { wakesCrew, crewWakeQueryKey, crewWebhooksQueryKey } from '../components/crew/wakesCrew'
...
export default function KiroCrewAgentsPage({ embedded }: { embedded?: boolean } = {}) {
  ...
  const boundWebhookTokens = useMemo(
    () => (webhooksQuery.data?.tokens || []).filter((t: WebhookTokenEntry) => t.agent === editing),
    [webhooksQuery.data, editing],
  )
  const boundWebhooks = boundWebhookTokens.length
  // "Live" holds the pane's own standard: a token silenced by the global kill
  // switch cannot call in any more than one whose own switch is off.
  const activeWebhooks = webhooksQuery.data?.switch_on === false
    ? 0
    : boundWebhookTokens.filter((t: WebhookTokenEntry) => t.enabled !== false).length
