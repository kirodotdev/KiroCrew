...
export function SessionColorField({ value, onChange }: { value: string; onChange: (v: string) => void }) {
  ...
  return (
    <Field label="Session Color" hint="New sessions using this agent will automatically use this color. Leave empty for no default color.">
      <div className="flex items-center gap-2">
        <input
          type="color"
          value={displayValue || '#6366f1'}
          onChange={e => onChange(e.target.value.toLowerCase())}
          className="h-8 w-8 cursor-pointer rounded border border-border bg-transparent p-0.5"
          aria-label="Pick session color"
        />
        <Input
          placeholder="#rrggbb"
          value={displayValue}
          ...
          onBlur={e => {
            const v = e.target.value.trim().toLowerCase()
            if (v && !HEX_RE.test(v)) onChange('')
          }}
          className="flex-1 font-mono text-[13px]"
          aria-label="Session color hex value"
        />
        {value && (
          <button
            type="button"
            ...
            aria-label="Clear session color"
          >
            Clear
          </button>
        )}
