  ...
  return (
    <Card data-testid="overview-today-card">
      ...
      {today.sessionsError ? (
        ...
        <ErrorNotice title={i18nT('pages.overview.todayCard.sessions_read_failed')} message={today.sessionsError.message} messageClassName="font-mono" askAgent testId="overview-today-error" />
      ) : today.loading ? (
        <div className="skeleton h-14 rounded" />
      ) : (
        <div className="flex flex-col gap-2">
          ...
          {/* Names what this count is: sessions with activity today, including
              ones started earlier. The Usage card beside it says "sessions
              started", so the two "today" numbers read as two sets without a
              hover and without repeating the Usage card's basis here. */}
          <div className="text-[12px] text-muted" data-testid="overview-today-basis">{i18nT('pages.overview.todayCard.sessions_active_scope')}</div>
          {/* A failed day-file read must not look like an empty day: without
              this the "entries saved to memory" fragment simply vanishes. */}
          {today.historyError && (
            <ErrorNotice title={i18nT('pages.overview.todayCard.memory_read_failed')} message={today.historyError.message} messageClassName="font-mono" askAgent testId="overview-today-history-error" />
          )}
          {today.groups.length > 0 && (
            <div className="flex flex-wrap gap-1.5 text-[12px] text-muted">
              {today.groups.map(g => {
                const name = g.name ?? i18nT('pages.overview.todayCard.unfiled')
                return (
                  // The visible pill is "2 Website"; assistive tech reads "2
                  // sessions in Website" instead, so the number is not an id.
                  <span key={g.folder_id || 'unfiled'} className="inline-flex items-center gap-1 rounded-full border border-border px-2 py-0.5" data-testid="overview-today-folder-chip">
                    <span className="sr-only">{i18nT('pages.overview.todayCard.folder_sessions', { count: g.sessions.length, folder: name })}</span>
                    <span className="text-text font-semibold tabular-nums" aria-hidden="true">{fmtNumber(g.sessions.length)}</span>
