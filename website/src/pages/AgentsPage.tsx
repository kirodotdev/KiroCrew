...
function ContextUsageCard({ ctx, installed }: { ctx: CtxSession[]; installed: InstalledAgent[] }) {
  ...
  return (
    <Card>
      <CardTitle className="gap-1.5">{i18nT('pages.agentsPage.context_window_usage')} <InfoTip text={i18nT('pages.agentsPage.live_context_window_utilization_per_active_proces', { process: provider.labels.sessionProcess })} /></CardTitle>
  ...
  return (
    <Card>
      <div className="flex items-center justify-between mb-4">
        <CardTitle className="mb-0 gap-1.5">{provider.displayName} {i18nT('pages.agentsPage.usage')} <InfoTip text={i18nT('pages.agentsPage.provider_consumption_for_the_current_billing_perio', { provider: provider.displayName })} /></CardTitle>
      ...
      <table className="w-full border-collapse table-striped"><thead><tr>{[i18nT('pages.agentsPage.id'), i18nT('pages.agentsPage.task'), i18nT('pages.agentsPage.status'), ''].map(h => <th key={h} className="text-left text-muted text-[12px] uppercase tracking-[.04em] px-2.5 py-2 border-b border-border font-medium">{h}</th>)}</tr></thead>
        <tbody>{agents.length === 0 ? <tr><td colSpan={4}><EmptyState icon={<Bot className="lucide-inline" />} title={i18nT('pages.agentsPage.no_subagents')} subtitle={i18nT('pages.agentsPage.spawn_tasks_from_chat_or_cli')} /></td></tr> : agents.map(a => (
          <tr key={a.id} className="hover:bg-bg-hover transition-colors"><td className="px-2.5 py-2 border-b border-border text-sm"><code>{a.id}</code></td><td className="px-2.5 py-2 border-b border-border text-sm">{a.task}</td>
            ...
            <td className="px-2.5 py-2 border-b border-border text-sm text-right"><Btn danger type="button" className="px-1.5 py-0.5" aria-label={i18nT('pages.agentsPage.delete_subagent_named', { id: a.id })} onClick={() => onDelete(a.id)}><X className="lucide-inline" /></Btn></td></tr>
        ))}</tbody></table>
    </Card>
  )
}
...
export default function AgentsPage({ embedded }: { embedded?: boolean } = {}) {
  ...
  const filtered = useMemo(() => {
    ...
    return installed.filter(a => [a.name, a.description, a.package, a.model, ...(a.skills || []), ...(a.mcp_servers || [])]
  ...
  const modelPinned = !!selectedAgent?.model && selectedAgent.model !== 'auto'
    ...
    >
      <div className="flex-1 min-w-0">
        ...
        <div aria-hidden="true" className="flex gap-2 mt-0.5">
          <span className="text-[11px] text-muted font-mono truncate">{shortModel(a.model || 'auto')}</span>
            ...
            <div className="flex-1 min-w-0 flex flex-col">
              {selectedAgent ? (<>
                <div className="px-4 pt-3.5 border-b border-border">
                  ...
                  {selectedAgent.description && <p className="mt-1.5 mb-0 text-[13px] text-muted leading-relaxed max-w-[70ch]">{selectedAgent.description}</p>}
