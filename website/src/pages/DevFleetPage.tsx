...
interface ReleaseChannel {
  ...
  // backend so the prefix rule lives on ONE side of the boundary. Re-deriving it
  // here would let a change to WORKTREE_PREFIX desync this label from the real path.
  name: string
...
export default function DevFleetPage() {
  ...
  async function createReleaseChannel() {
    ...
    try {
      ...
      if (r?.ok) notify(i18nT('pages.devFleetPage.release_channel_created', { lane: r.lane || '', ref: r.version || r.ref || '?' }), { type: 'success' })
  ...
  async function advanceReleaseChannel() {
    // The channel name is read from the row rather than taken as an argument: it
    // is the same one value on every call, so a parameter would state a choice
    // the operation does not offer. It reaches the strings below only as the
    // label the operator reads -- the request body carries nothing.
    const lane = fleet?.release_channel?.lane || ''
    ...
    setRcBusy(true)
    try {
      const r = await api.post<{ ok?: boolean; moved?: boolean; lane?: string; version?: string | null; ref?: string | null; error?: string }>('/release-channel/advance', {})
      ...
      if (r?.ok && r.moved) notify(i18nT('pages.devFleetPage.release_channel_advanced', { lane: r.lane || lane, ref: r.version || r.ref || '?' }), { type: 'success' })
      ...
      else if (r?.ok) notify(i18nT('pages.devFleetPage.release_channel_already_at_tip', { lane: r.lane || lane }), { type: 'info' })
  ...
  function rowButtons(w: Worktree): ReactNode[] {
    ...
    if (channelFor(w) && rcBusy) out.push(<span key="rcbusy" style={{ display: 'inline-flex', alignItems: 'center', gap: 4, fontSize: 11, color: 'var(--muted)' } as CSSProperties}><LoaderCircle size={12} className="lucide-inline animate-spin" /> {i18nT('pages.devFleetPage.advance')}{"\u2026"}</span>)
    out.push(<MenuBtn key="menu" items={[
      ...
      channelFor(w)
        ...
        ? { label: i18nT('pages.devFleetPage.advance_to_channel_tip'), icon: <RefreshCw size={13} className="lucide-inline" />, onClick: () => advanceReleaseChannel(), disabled: rcBusy, title: channelFor(w)!.at_tip ? i18nT('pages.devFleetPage.at_the_channel_tip') : i18nT('pages.devFleetPage.moves_the_detached_worktree_to_the_channel_tip_refused_if_dirty') }
  ...
  function renderChannelPlaceholder(rc: ReleaseChannel) {
    ...
    // precedence order: the name is occupied, the channel will not resolve, or a
    // create is already running.
    // Only the unresolvable case reaches here now: a reserved name taken by a
    // branch is explained on that branch's own row instead (see
    // `channelNameTakenFor`), so it never produces a placeholder.
    const blocked = rc.error || null
    // One derivation, used by the cell, its tooltip and the Create button, so the
    // three cannot disagree about how a resolver failure reads.
    ...
    return (
      <div key={'rc-' + rc.lane} data-testid={'release-channel-placeholder-' + rc.lane} style={{ display: 'grid', gridTemplateColumns: '16px 84px minmax(0,1fr) 64px 48px 44px 212px', gap: 8, alignItems: 'center', padding: '5px 0', borderTop: '1px solid var(--border)', minHeight: 30, minWidth: 640, opacity: 0.62 } as CSSProperties}>
        ...
        <div style={{ minWidth: 0, display: 'flex', alignItems: 'baseline', gap: 6, whiteSpace: 'nowrap', overflow: 'hidden' } as CSSProperties}>
          ...
          {/* Framed, never the bare backend string. `rc.error` is git/resolver
              text, so rendering it alone puts mechanism where a first-time user
              needs "what happened" -- and this cell ellipsises, which can cut the
              sentence mid-clause. The same framing the adopted row's tooltip uses
              is reused here, and the full text goes on `title` because the framed
              form is longer than the cell. */}
          <span
            title={blockedText || undefined}
            style={{ fontSize: 11.5, color: blocked ? 'var(--warn)' : 'var(--muted)', overflow: 'hidden', textOverflow: 'ellipsis', minWidth: 0 }}
          >
            {blockedText || i18nT('pages.devFleetPage.no_worktree_yet')}
          </span>
        </div>
        ...
        <div style={{ display: 'flex', gap: 6, justifyContent: 'flex-end', alignItems: 'center', minWidth: 0 } as CSSProperties}>
          ...
          <Btn onClick={() => createReleaseChannel()} disabled={rcBusy || !!rc.error} title={blockedText || undefined}>
            {rcBusy ? i18nT('pages.devFleetPage.creating') : i18nT('pages.devFleetPage.create')}
          </Btn>
        </div>
      </div>
    )
