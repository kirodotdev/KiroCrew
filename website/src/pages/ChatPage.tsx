...
import { addPendingFile, prepareSendPayload, buildRelMap, isWindowsShapedPath, normalizeWindowsPath, parseDirTokens, serializeDirTokens, spliceDirTokens, tokenRegex } from '../utils/fileTokens'
 ...
 *  the unrelated, longer `@README.md` mention (fork GPT review). Shared
 *  between the reconciliation staleness check and the remove-chip strip
 *  so the two can never disagree about what counts as a boundary.
 *
...
const mentionBoundary = /(?:\s|$|[,.!?;:)\]}`"']+(?=\s|$)|:\d+(?=\s|$))/.source
...
const leadingMentionBoundary = /(?:^|\s)[($[{`"']?/.source
...
export default function ChatPage({ mode, embedded, embedMode, popout, noUrlSync }: { mode?: string; embedded?: boolean; embedMode?: 'chat' | 'sessions'; popout?: boolean; noUrlSync?: boolean } = {}) {
 ...
 // composer instead of auto-submitting. Auto-submitting would be a
  ...
  const boundaryFor = useCallback((fullAlias: string, otherAliases?: ReadonlySet<string>) => {
    const unsafe = otherAliases && [...otherAliases].some(o => o.length > fullAlias.length && o.startsWith(fullAlias))
    return unsafe ? strictMentionBoundary : mentionBoundary
  }, [])
  const mentionRegex = useCallback((token: string, otherAliases?: ReadonlySet<string>) => {
    const escaped = token.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')
    return new RegExp(`(${leadingMentionBoundary})@${escaped}(?=${boundaryFor(`@${token}`, otherAliases)})`)
  }, [boundaryFor])
  const relMentionedHere = useCallback((text: string, rel: string, otherAliases?: ReadonlySet<string>) => {
    if (mentionRegex(rel, otherAliases).test(text)) return true
    const project = currentProjectRef.current || ''
    if (!isWindowsShapedPath(project)) return false
    const altRel = rel.includes('\\') ? rel.replace(/\\/g, '/') : rel.replace(/\//g, '\\')
    return mentionRegex(altRel, otherAliases).test(text)
  }, [mentionRegex])
  ...
  const handleAddToContext = useCallback((absPath: string, kind: 'file' | 'dir') => {
    ...
    if (kind === 'dir') {
      ...
      if (!dup) {
        ...
        const spliced = spliceDirTokens(inputRef.current, voiceCaretRef.current?.start ?? null, [rel])
        if (spliced.changed) {
      ...
      // Checked against the live text (not inside the updater) because token
      // bookkeeping must follow the same branch and record the spelling that
      // is actually present.
      const alreadyMentioned = tokenRegex(rel).test(inputRef.current) ||
        (projectIsWindowsShaped && tokenRegex(rel.replace(/\//g, '\\')).test(inputRef.current))
      if (!alreadyMentioned) {
        ...
        const at = caret == null ? prev.length : Math.max(0, Math.min(caret, prev.length))
        ...
        const existing = tokenRegex(rel).test(inputRef.current) ? rel
          : projectIsWindowsShaped && tokenRegex(rel.replace(/\//g, '\\')).test(inputRef.current) ? rel.replace(/\//g, '\\')
          : null
  ...
  useEffect(() => {
    ...
    //
    // ONE uniqueness-scoped carve-out on top of that rule (fork GPT review):
    // a user who picked `@src/main.ts` and hand-shortens it to `@main.ts`
    // has NOT deleted the reference -- they rewrote it to a still-valid
    // spelling -- yet every EXACT recorded alias now fails, and the file
    // was silently unstaged and dropped from the send. So before declaring
    // an entry stale, each recorded alias's own segment-boundary suffixes
    // are probed against the text; a mention counts ONLY when the suffix
    // path-suffix-matches exactly ONE staged path (this one). The moment a
    // second staged path shares that trailing segment the probe refuses --
    // the ambiguous case keeps the original rule and unstages, so the
    // wrong-file hazard the paragraph above records cannot come back.
    const projectIsWindowsShaped = isWindowsShapedPath(currentProjectRef.current || '')
    const segSuffixes = (rel: string) => {
      const segs = projectIsWindowsShaped ? rel.split(/[/\\]/) : rel.split('/')
      return segs.slice(1).map((_, i) => segs.slice(i + 1).join('/'))
    }
    const pathEndsWith = (p: string, suffix: string) => {
      if (!projectIsWindowsShaped) return p === suffix || p.endsWith('/' + suffix)
      const norm = p.replace(/\\/g, '/')
      const normalizedSuffix = suffix.replace(/\\/g, '/')
      return norm === normalizedSuffix || norm.endsWith('/' + normalizedSuffix)
    }
    const uniquelyResolvedHere = (p: string, aliases: string[]) => {
      for (const token of aliases) {
        for (const suffix of segSuffixes(token.slice(1))) {
          const matches = [...staged].filter(q => pathEndsWith(q, suffix))
          if (matches.length !== 1 || matches[0] !== p) continue
          if (relMentionedHere(input, suffix, otherAliasesFor(p))) return true
        }
      }
      return false
    }
    ...
    // A staged file's mention hand-rewritten to its CURRENT project's own
    // rel form (stage under `/repo`, re-root the slot upward, edit the
    // token to the now-valid `@repo/src/main.ts`) matched neither the
    // recorded alias nor any shorter suffix -- the file entered `stale`,
    // and since revival only inspects UNSTAGED paths against the current
    // project, nothing could bring it back: the send silently omitted a
    // file the text plainly referenced (fork GPT review). This does not
    // reopen the trusted-stored-alias decision above -- stored aliases
    // still keep a file staged exactly as before; the freshly derived
    // current-project rel is accepted IN ADDITION, and recorded so the
    // remove control and revival see the live form the user actually
    // typed.
    const currentRelStillMentioned = (p: string) => {
      const rel = makeRelative(p, normalizeWindowsPath(currentProjectRef.current || ''))
      if (rel === p || !relMentionedHere(input, rel, otherAliasesFor(p))) return false
      const token = `@${rel}`
      if (!known[p].includes(token)) known[p].push(token)
      return true
    }
    const stale = pendingFilesRef.current.filter(p => {
      const aliases = known[p]
      if (aliases == null || aliases.length === 0) return false
      if (aliases.some(token => relMentionedHere(input, token.slice(1), otherAliasesFor(p)))) return false
      if (currentRelStillMentioned(p)) return false
      return !uniquelyResolvedHere(p, aliases)
    })
    const revived = Object.keys(known).filter(p => {
      ...
      const otherAliases = otherAliasesFor(p)
          ...
          <div className="relative flex flex-col flex-1 min-h-0" {...dropTargetProps}>
            ...
            <div className="relative">
              ...
              onRemoveFile={p => {
                ...
                // without any. Fall back to deriving ONE token from the path —
                ...
                const slotTokens = currentSlotTokens()
                const projectIsWindowsShaped = isWindowsShapedPath(currentProjectRef.current || '')
                const derivedRel = [...buildRelMap([p], inputRef.current).keys()][0]
                const suffixMatchesPath = (path: string, suffix: string) => {
                  if (!projectIsWindowsShaped) return path === suffix || path.endsWith('/' + suffix)
                  const normalizedPath = path.replace(/\\/g, '/')
                  const normalizedSuffix = suffix.replace(/\\/g, '/')
                  return normalizedPath === normalizedSuffix || normalizedPath.endsWith('/' + normalizedSuffix)
                }
                const liveToken = derivedRel && pendingFilesRef.current.filter(
                  path => suffixMatchesPath(path, derivedRel),
                ).length === 1 && suffixMatchesPath(p, derivedRel) ? `@${derivedRel}` : null
                ...
                setInput(prev => tokens.reduce((text, token) => {
                  ...
                  return candidates.reduce((t, candidate) => {
                    ...
                    if (guarded) return t
                    // Shares `mentionBoundary` (via `boundaryFor`, which also
                    ...
                    const boundarySrc = boundaryFor(candidate, otherAliases)
                    ...
                    for (const [open, close] of [['(', ')'], ['[', ']'], ['{', '}'], ['`', '`'], ['"', '"'], ["'", "'"]] as const) {
                      ...
                      stripped = stripped.replace(new RegExp(`(^|\\s)${openEsc}${esc}${closeEsc}(?: |(?=${boundarySrc})|$)`, 'g'), '$1')
                    }
                    // A `file:line` mention (`@src/main.ts:42` -- the
                    // `:\d+` form `mentionBoundary` admits as a zero-width
                    // trailing boundary) must have that suffix CONSUMED by
                    // the strip, not merely looked past (fork GPT review):
                    // matching the token via the lookahead alone removed
                    // `@src/main.ts` and left a stray `:42` behind as real
                    // composer text. Consumption is gated on the permissive
                    // boundary: under `strictMentionBoundary` a LONGER
                    // sibling alias literally starts with this one, so the
                    // trailing `:42` there can be the sibling's own name
                    // (`\`:\` is a legal POSIX filename character) and
                    // eating it would corrupt that other file's mention.
                    const lineSuffix = boundarySrc === mentionBoundary ? '(?::\\d+(?=\\s|$))?' : ''
