...
import { EVENTS, KAS_ONLY_EVENTS, AGENT_REQUESTED_EVENTS, PENDING_EVENTS, EVENT_GLOSS_KEYS } from './hookEventWireValues'
...
const eventGloss = (event: string): string | undefined => {
  const key = EVENT_GLOSS_KEYS[event]
  return key ? `(${i18nT(key)})` : undefined
}
                ...
                <tbody>
                  {filtered.length === 0 ? (
                    ...
                    <tr className="group/hookrow hover:bg-bg-hover transition-colors">
                      ...
                      <td className={rowCell(h.enabled, 'text-sm')}><span className={`px-1.5 py-[2px] rounded-full text-[11px] font-bold border font-mono ${EVENT_STYLE[h.event] || 'bg-bg-elevated text-muted border-border'}`}>{h.event}</span>{eventGloss(h.event) && <span className="ml-1.5 text-[11px] text-text/80 whitespace-nowrap" data-testid="lane-pill-gloss">{eventGloss(h.event)}</span>}</td>
