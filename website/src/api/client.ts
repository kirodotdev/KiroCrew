...
export const api = {
  ...
  editQueuedMessage: (slot: string, queueId: string, content: string, editId?: string) => patch('/api/chat/slots/' + encodeURIComponent(slot) + '/queue/' + encodeURIComponent(queueId), { content, ...(editId ? { editId } : {}) }).then(j),
