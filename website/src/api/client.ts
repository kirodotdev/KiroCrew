...
export interface DevcontainerStatus {
  ...
  trusted: boolean
  /**
   * Digest of the config's current bytes, or null when it cannot be computed
   * (a screen refused the tree — such a config can never be trusted).
   *
   * Published so the dashboard can bind a "don't ask again" dismissal to the
   * CONTENT the user was shown. Keyed on the path alone, a dismissal survived an
   * edit — and an edit is what drops trust, so that is exactly when the prompt
   * has to come back.
   */
