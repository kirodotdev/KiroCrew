...
export const api = {
  ...
  /** `onlyKeys` bounds the archive to a set the caller has guarded; the server
   *  intersects rather than substitutes, so it never archives more than it would. */
  cleanupSessions: (maxInactiveDays: number, activeSlot?: string, dryRun?: boolean, onlyKeys?: string[]) => post('/api/chat/slots/cleanup', { max_inactive_days: maxInactiveDays, active_slot: activeSlot || '', dry_run: !!dryRun, ...(onlyKeys ? { only_keys: onlyKeys } : {}) }).then(j) as Promise<{ ok: boolean; archived: number; keys: string[]; failed: string[]; dry_run?: boolean; count?: number; active_is_stale?: boolean }>,
