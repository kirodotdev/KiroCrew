 ...
 *
 *  Every boolean is kept separate because callers SEQUENCE on them: `appended === false`
 *  with `visibleDeferred === true` means the visible row is only HELD in memory until the
 *  running turn ends — not durable, so a caller about to destroy the slot must refuse to
...
export const api = {
   ...
   *  `appended === true`; a `visibleDeferred` note is held in memory only. A
   *  `visibleOnly` note whose durable write was refused REJECTS with 503
   *  `note_not_durable` rather than resolving, and its visible row is taken back
   *  out of the live window, so a re-post cannot leave the note there twice. */
