...
describe('switchSlot 404 — an announcing caller surfaces the recovery (#6372)', () => {
  ...
  it('announced 404: one notification naming the session, one synchronous eviction, payload unchanged', async () => {
    ...
    expect(n[0].payload).toEqual({ name: 'Ghost session', current: false })
    ...
    expect(store.getState().chat.switchSlotGone).toEqual({ name: 'Ghost session', current: false })
  })
  ...
  it('re-activating the session already open: the notice fires but the row is KEPT', async () => {
    ...
    // The pane is visibly open, so the stored notice is the CURRENT variant:
    // ChatPage renders the deleted-on-the-server copy instead of claiming the
    // open session "cannot be opened".
    expect(store.getState().chat.switchSlotGone).toEqual({ name: 'Ghost session', current: true })
  })
  ...
  it('a title the slot list no longer knows stores an empty name (ChatPage falls back to generic copy)', async () => {
    ...
    await unwrapRejection(store.dispatch(switchSlot({ key: 'gone', announceOnMissing: true })).unwrap())
    expect(store.getState().chat.switchSlotGone).toEqual({ name: '', current: false })
  })
