...
import { mergePreservedPastes, type PasteBlock } from '../utils/pasteTokens'
...
export type SendAttempt = { text: string; sendId: string; files?: string[]; pastes?: PasteBlock[] }
 ...
 *  duplicate REPLACES the older record wholesale, so the newer send's id and its
 *  sidecars both win. Merging field-by-field could not express omission:
 *  `recordSendAttempt` leaves `files`/`pastes` OFF an attempt that has none, so
 *  a resend of the same text with the attachment detached inherited the old
 *  send's `files` and ↑ re-staged — and then resent — a file the user removed. */
function pushSendAttempt(state: ChatState, slot: string, attempt: SendAttempt): void {
  if (!state.attemptedSends) state.attemptedSends = {}
  const list = (state.attemptedSends[safeKey(slot)] ??= [])
  const last = list[list.length - 1]
  if (last?.text === attempt.text) { list[list.length - 1] = attempt; return }
  list.push(attempt)
...
const chatSlice = createSlice({
  ...
  reducers: {
    ...
    recordSendAttempt(state, action: PayloadAction<{ slot: string; text: string; sendId: string; files?: string[]; pastes?: PasteBlock[] }>) {
      const { slot, text, sendId, files, pastes } = action.payload
      if (!slot || isUnsafeKey(slot) || !text) return
      pushSendAttempt(state, slot, {
        text,
        sendId,
        ...(files?.length ? { files } : {}),
    ...
    transferSendAttempt(state, action: PayloadAction<{ from: string; to: string; sendId: string }>) {
      ...
      pushSendAttempt(state, to, moved)
    },
    ...
    editQueuedMessage(state, action: PayloadAction<{ slot: string; queue_id: string; content: string; attachments?: QueueEntryAttachments }>) {
