...
export const createSlot = createAsyncThunk<
  ChatSlot,
  { agent?: string; model?: string; mode?: string; memory_mode?: string; clean_mode?: boolean; folder_id?: string | null; title?: string; color_index?: number | null; color_hex?: string | null; agent_color_hex?: string | null; project?: string | null; activate?: boolean } | string | undefined,
...
>(
  ...
  async (opts, { dispatch, getState, fulfillWithValue }) => {
    ...
    const agentColorHex = typeof opts === 'string' ? undefined : opts?.agent_color_hex
    ...
    // Resolve agent session color if neither an explicit color nor a dashboard
    // default is set. This lookup is cheap (the agents list is typically cached
    // by the browser) and ensures the agent color is applied from every call
    // site without requiring each caller to pass it explicitly.
    let resolvedAgentColorHex = agentColorHex
    if (!explicitHex && explicitColor == null && !resolvedAgentColorHex && agent) {
      try {
        const agentsResp = await api.kirocrewAgents()
        const matchedAgent = (agentsResp.agents || []).find((a: { name: string; session_color?: string }) => a.name === agent)
        if (matchedAgent?.session_color) resolvedAgentColorHex = matchedAgent.session_color
      } catch { /* non-fatal — agent color is cosmetic */ }
    }
    ...
    if (explicitHex != null) {
      ...
      } else if (resolvedAgentColorHex) {
        // No explicit color and no dashboard default-color policy — fall back
        // to the agent's configured session_color. This is a cosmetic default,
        // not a carried user choice, so it stays fire-and-forget like the
        // dashboard default-color policy branch above.
        slot.color_hex = resolvedAgentColorHex
        api.setSlotColorHex(slot.key, resolvedAgentColorHex).catch(() => {})
      }
    }
