...
interface ChatState {
  ...
  /** Text staged from the Side Chat into the main composer. Kept apart from
   *  `pendingInput` because it APPENDS to the live composer rather than replacing
   *  it: ChatPage merges it against `inputRef` (the only holder of unsent text)
   *  before seeding, so a hand-off never clobbers what the user is typing. */
  mainComposerAppend: string | null
...
const chatSlice = createSlice({
  ...
  reducers: {
    ...
    /** Stage Side Chat text for the main composer. ChatPage appends it to the
     *  live draft; passing null clears the field after it is consumed. */
    stageToMainComposer(state, action: PayloadAction<string | null>) { state.mainComposerAppend = action.payload },
