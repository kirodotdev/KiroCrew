...
const slotKeyedMaps = (state: ChatState) => [
  ...
  // `slotClearSeq` is deliberately NOT here: it is the generation an in-flight
  // fetch compares against, and an eviction is one of the events it must count
  // (see noteSlotCleared), so it has to outlive the slot.
].filter(Boolean)
...
const evictSlotState = (state: ChatState, slotKey: string): void => {
  ...
  // The transcript this key named is gone; a page fetched for it must not land
  // in a slot later recreated under the SAME key (deterministic app slot keys
  // make that an ordinary flow), so move the key's generation.
  noteSlotCleared(state, slotKey)
...
interface ChatState {
  ...
  /** Per-key GENERATION of the transcript a slot key names: moved by a `/clear`
   *  of the cache (`clearSlotCache`) or the live view (`clearMessages`) and by the
   *  slot's eviction (`evictSlotState`). A warm captures it before its fetch and
   *  declines its page when it moved: the page was built from a transcript that
   *  no longer exists under that key, and writing it back would resurrect it --
   *  into the same slot after a clear, or into a slot RECREATED under the same
   *  key after a delete. Survives eviction on purpose (not in `slotKeyedMaps`):
   *  a counter that died with the slot would read `undefined` both before and
   *  after a delete-and-recreate and see no move. */
...
/** Record that the transcript a slot key names was discarded -- by a `/clear`
 *  or by the slot's eviction -- so an in-flight fetch that predates it can tell
 *  (`warmSlotCache`). Monotonic per key and NOT evicted with the slot, so a
 *  delete-and-recreate under the same key reads as a move. */
 ...
 *    A partial unbounded read (`hasMore`) has no such derivation, and a RUNNING
 *    complete one is short by the in-flight row; both yield `undefined`. */
function settledTotalOf(page: {
  ...
  // A partial read has no derivation. A RUNNING complete read has one that is
  // short by the in-flight row (`isDurableRow` excludes the collapsed `streaming`
  // row the bounded count keeps): an under-count is safe against a false shrink
  // but hides a later one-row rewind (`serverShrank` needs `total < prior`), so
  // it is not offered either -- the bounded page's count carried as
  // `comparableTotal` is exact there, and a caller with none retains nothing.
  if (page.hasMore || page.running) return undefined
  let durable = 0
...
/** Has a FULL bounded window moved wholly past everything the view holds?
 *
 *  A pane holding N rows whose slot then produces more than min(N, ceiling) rows
 *  between two warms (a long-running loop across a reconnect; any cache past the
 *  handler ceiling with a turn that appends more than a page) gets a window that
 *  sits entirely past its cache: its oldest row anchors nowhere and the cache's
 *  oldest row is not on the page, so `pageStitchesOntoView` declines. That is
 *  not a hole to guess across and not a reason to read the whole transcript --
 *  it is the window having moved on. The page REPLACES the cache with the
 *  bounded marker set (`has_more` is the server's own word that rows sit behind
 *  it), and the rows behind stay reachable through the pane's load-earlier bar.
 *
 *  Three conditions, all of which the server's handler makes exact:
 *  - `boundedRead && hasMore`: the handler slices AFTER `_collapse_wire_rows`
 *    (chunk runs folded, `done` dropped), so `has_more` means exactly
 *    `requestedLimit` rows came back -- a SHORT page with `has_more` is not a
 *    window the server produced, and it keeps the append/keep behaviour.
 *  - `messages.length >= requestedLimit`: the page is that full window.
 *  - strict ts ORDER on parsed instants: the view's newest DURABLE row predates
 *    the page's oldest. Client-only rows (queued, streaming, thinking,
 *    permission) carry a clock of their own and are not asked. A row without a
 *    readable ts declines -- not guess. */
function windowMovedPastView(
  page: { messages: ChatMessage[]; hasMore: boolean; boundedRead: boolean },
  requestedLimit: number | undefined,
  viewNow: ChatMessage[],
): boolean {
  if (!page.boundedRead || !page.hasMore) return false
  if (typeof requestedLimit !== 'number' || page.messages.length < requestedLimit) return false
  let newest: ChatMessage | undefined
  for (let i = viewNow.length - 1; i >= 0; i--) {
    if (isDurableRow(viewNow[i])) { newest = viewNow[i]; break }
  }
  if (!newest) return false
  const viewNewestTs = tsEpoch(newest.ts)
  const pageOldestTs = tsEpoch(page.messages[0]?.ts)
  return viewNewestTs !== null && pageOldestTs !== null && viewNewestTs < pageOldestTs
}
...
export const warmSlotCache = createAsyncThunk(
  ...
  async (key: string, { getState }): Promise<
    | null
    | (Awaited<ReturnType<typeof fetchSlotDetail>> & { warmSeq: number; comparableTotal?: number; requestedLimit?: number })
  > => {
    const state = (getState() as { chat: ChatState }).chat
    if (state.activeSlot === key) return null
    ...
    // request left. A `/clear` (or a delete / prune, even one followed by a
    // recreate under the same key) landing during an await discards that
    // transcript, and a fulfilled reducer that guards only on `activeSlot` would
    // write the stale page back and resurrect it. So the key's generation is
    // captured here and re-read after EACH await; a cache that held rows and now
    // holds none, and a key missing from the loaded slot list, are read the same
    // way (belt and braces for state shapes that predate the generation).
    const clearSeq0 = state.slotClearSeq?.[safeKey(key)]
    type Root = { chat: ChatState; dashboard?: { slots?: ReadonlyArray<{ key: string; created?: string }>; slotsLoaded?: boolean } }
    ...
    const slotRecord = (root: Root) => root.dashboard?.slots?.find(s => s.key === key)
    const slotGone = (root: Root): boolean =>
      root.dashboard?.slotsLoaded === true && Array.isArray(root.dashboard.slots) && !slotRecord(root)
    // A slot deleted and RECREATED under the same key mid-flight is back in the
    // list, and a cold one left no residue for the eviction to move the
    // generation on -- but the recreated record carries a new `created`.
    const created0 = slotRecord(getState() as Root)?.created
    const slotRecreated = (root: Root): boolean => {
      const created = slotRecord(root)?.created
      return created0 !== undefined && created !== undefined && created !== created0
    }
    ...
    if (stitches) return { ...page, warmSeq, requestedLimit: limit }
    // A full window wholly past the cache is the window having MOVED, not a hole:
    // the reducer replaces the cache with the marked page and the pane's
    // load-earlier bar reaches what sits behind it. Without this every warm of a
    // busy pane whose slot outgrew the window fell through to the unbounded read
    // below -- the read this bound exists to remove, on the largest transcripts,
    // on every `done` until the pane was opened.
    if (windowMovedPastView(page, limit, cacheNow)) return { ...page, warmSeq, requestedLimit: limit }
  ...
  reducers: {
    ...
    hydrateSlotMessages(state, action: PayloadAction<{ slot: string; messages: ChatMessage[]; hasMore?: boolean; bounded?: boolean; total?: number; running?: boolean }>) {
      ...
      if (state.slotHydrated?.[slot]) {
        ...
        retainServerTotal(state, slot, settledTotalOf({ total, messages, hasMore, boundedRead: bounded, running }))
        return
      }
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(warmSlotCache.fulfilled, (state, action) => {
        if (!action.payload) return
        const { key, messages, queue, hasMore, running, warmSeq, comparableTotal, requestedLimit } = action.payload
        if (isUnsafeKey(key)) return
        ...
        // A FULL bounded window wholly past the cache (see windowMovedPastView) is
        // neither a disconnect to append across -- that splices an unmarked hole
        // between the cache and the page -- nor a longer prior to keep. The page
        // replaces the cache and, being the array written, carries its marker.
        const windowMovedPast = cutIdx < 0
          && windowMovedPastView({ messages: hydrated, hasMore, boundedRead: action.payload.boundedRead }, requestedLimit, prior)
        const priorEndsBeforePage = longerPrior && !windowMovedPast
        ...
        const keptPrior = longerPrior && !priorEndsBeforePage && !windowMovedPast
        ...
        // A warm that KEEPS an older head above its page (a pane widened past
        // the handler ceiling, then grown by live rows: the 500-row window lands
        // inside a 510-row cache) is not the prefix, so it says nothing about
        // `hasMore` -- but the array it writes is still bounded-page rows through
        // the page's last row: the head came from an earlier wider page (the old
        // marker is the proof), the page from this warm. Dropping the marker here
        // made `hydrateSlotMessages` decline EVERY later page for the pane
        // (`boundedLen === undefined` reads as "unbounded, nothing widens it"),
        // so load-earlier presses did nothing and the earliest rows were stranded
        // until a remount. Keep a boundary the next wider page can replace up to.
        const priorBoundedLen = state.slotPaneBounded?.[safeKey(key)]
        const keepsBoundedHead = !warmIsPrefix && priorBoundedLen !== undefined
        writeSlotPage(state, key, revived, warmIsPrefix ? hasMore : undefined,
          (warmIsPrefix && hasMore) || keepsBoundedHead ? boundedLen : undefined)
