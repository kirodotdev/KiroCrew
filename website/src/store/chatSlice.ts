...
interface ChatState {
  ...
  switchSlotGone: { name: string; current: boolean } | null
    ...
    try {
      ...
      if (typeof status === 'number') {
        ...
        if (announce && isMissingSlotError(payload)) {
          ...
          // `current`: the user re-activated the session they were already in
          // (the origin captured at `pending` names the gone key). That pane is
          // visibly open, so ChatPage renders copy that states the deletion
          // WITHOUT claiming the session "cannot be opened". Gated on the live
          // claim: a stale 404 that lost its claim to a newer gesture cannot
          // trust `slotSwitchOrigin` (the newer `pending` overwrote it), so it
          // falls back to the generic copy.
          const current = chat.slotSwitchRequestId === requestId && chat.slotSwitchOrigin !== null && chat.slotSwitchOrigin.key === key
          ...
          dispatch(chatSlice.actions.setSwitchSlotGone({ name: name ?? '', current }))
...
const chatSlice = createSlice({
  ...
  reducers: {
    ...
    setSwitchSlotGone(state, action: PayloadAction<{ name: string; current: boolean }>) { state.switchSlotGone = action.payload },
