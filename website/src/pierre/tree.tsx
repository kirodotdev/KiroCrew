...
export function PierreWorkspaceTree({ projectDir, onFileOpen, onAddToContext, onUploadRequest, searchQuery, mode, selectedPath }: {
  projectDir: string
  ...
  return (
    <Suspense fallback={<TreeSkeleton />}>
      <TreeImpl key={mode ?? 'all'} projectDir={projectDir} onFileOpen={onFileOpen} onAddToContext={onAddToContext} onUploadRequest={onUploadRequest} searchQuery={searchQuery} mode={mode} selectedPath={selectedPath} />
    </Suspense>
  )
}
