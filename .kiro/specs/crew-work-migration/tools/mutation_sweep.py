...
MUTATIONS: list[Mutation] = [
    ...
    Mutation(
        gap="secrecy",
        what="a finding's agent-authored key reaches the browser unredacted",
        path="src/kiro_crew/dashboard/handlers/migration.py",
        old='        "detail_key": _redact_plan_text(f.detail_key),',
        new='        "detail_key": f.detail_key,',
        test=(
            "test/test_api_migration_move.py::"
            "test_a_credential_in_a_ledger_artifact_key_never_reaches_the_dashboard"
        ),
    ),
    Mutation(
        gap="validation",
        what="a malformed task collection reaches the mid-run check and crashes",
        path="src/kiro_crew/migration/taskrun_adapter.py",
        old="    if not isinstance(tasks, (list, tuple)):",
        new="    if False:",
        test=(
            "test/test_migration_move_cli.py::"
            "test_taskrun_move_refuses_a_malformed_task_collection_at_the_mid_run_check"
        ),
    ),
    Mutation(
        gap="validation",
        what="a scalar commit_hashes reaches len() and crashes the CLI",
        path="src/kiro_crew/migration/taskrun_adapter.py",
        old="    if not isinstance(commits, (list, tuple)):",
        new="    if False:",
        test=(
            "test/test_migration_move_cli.py::"
            "test_taskrun_move_refuses_a_malformed_commit_hash_field"
        ),
    ),
    Mutation(
        gap="plan core",
        what="the session ledger stops reaching the plan, so its findings go dead",
        path="src/kiro_crew/dashboard/handlers/migration.py",
        old="    ledger = await asyncio.to_thread(session_ledger.read_state, slot.key)",
        new="    ledger = {}",
        test=("test/test_api_migration_move.py::" "test_a_real_sessions_ledger_reaches_the_plan"),
    ),
    Mutation(
        gap="plan core",
        what="a requirement is invented for MCP servers nothing can attribute to a session",
        path="src/kiro_crew/migration/session_adapter.py",
        old="        return reqs\n\n    async def serialize",
        new=(
            '        for s in raw.get("mcp_servers") or []:\n'
            '            reqs.append(P.HostRequirement(kind="mcp_server", identity=s,'
            ' severity="blocking"))\n'
            "        return reqs\n\n    async def serialize"
        ),
        test=(
            "test/test_migration_session_adapter.py::"
            "test_no_requirement_is_invented_for_a_sessions_mcp_servers"
        ),
    ),
    Mutation(
        gap="frontend",
        what="the derived crew-lookup notice becomes undismissable again",
        path="website/src/components/MoveToCrewDialog.tsx",
        old="            setSuggestionsHidden(true)",
        new="            void 0",
        test="src/components/MoveToCrewDialog.cov80.test.tsx",
    ),
    Mutation(
        gap="plan core",
        what="a dropped memory binding is retired silently instead of reported",
        path="src/kiro_crew/migration/cron_adapter.py",
        old="        if job.memory_store:",
        new="        if False:",
        test=(
            "test/test_migration_cron_adapter.py::"
            "test_the_memory_identity_is_reported_rather_than_silently_retired"
        ),
    ),
    Mutation(
        gap="plan core",
        what="the session plan stops seeing the slot's own non-portable references",
        path="src/kiro_crew/dashboard/handlers/migration.py",
        old='    for key in ("project", "model", "workspace"):',
        new="    for key in ():",
        test=(
            "test/test_api_migration_move.py::"
            "test_a_project_bound_session_plan_reports_every_nonportable_drop"
        ),
    ),
    Mutation(
        gap="validation",
        what="a malformed task entry reaches dataclasses.asdict and crashes",
        path="src/kiro_crew/migration/taskrun_adapter.py",
        old="        if bad:",
        new="        if False:",
        test=(
            "test/test_migration_move_cli.py::" "test_taskrun_move_refuses_a_malformed_task_entry"
        ),
    ),
    Mutation(
        gap="subtraction",
    ...
    Mutation(
        ...
        path="src/kiro_crew/migration/cron_adapter.py",
        old='    "source_template_prompt",\n)',
        new='    "source_template_prompt",\n    "secret_env",\n)',
        test=(
            "test/test_migration_cron_adapter.py::"
