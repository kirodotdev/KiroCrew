...
> **Scope note — this slice ships the PLAN only.**
>
> The transfer half (the five-step coordinator, the receiver, the in-flight
> journal, the tombstone registry and the redirect badge) was implemented and
> then REMOVED from the shipping change, because nothing in production called
> it: every `migrate()` / `reconcile()` / receiver call site was a test, no
> tombstone was ever written, and the badge that reads one could not render.
> Tasks marked DEFERRED below return with the change that wires transmit to
> the crew tunnel, where they get a production caller. Nothing about the
> design below is withdrawn — only the claim that it is shipped.
...
- [x] 1. Shared migration protocol (no unit kinds yet)
  - [ ] 1.1 (DEFERRED — see the note at the top) Add the data model: `MigrationBundle`, `HostRequirement`, `PreflightReport`, `Finding`, `Tombstone`, `CrewRef`, `QuiesceToken`
  ...
  - [ ] 1.3 (DEFERRED — see the note at the top) Implement `MigrationCoordinator` — the five-step handoff
  ...
  - [ ] 1.5 (DEFERRED — see the note at the top) Implement `MigrationReceiver` with two tunnel endpoints on the target
  ...
  - [ ] 1.6 (DEFERRED — see the note at the top) Implement startup reconciliation for the crash window between ack and tombstone
    - Query target by `handoff_id`; converge to exactly one owner; never resume unconditionally
    - _Requirements: 2.6_
  - [ ] 1.7 (DEFERRED — see the note at the top) Implement secret containment: credential-pattern scan over the serialized payload
  ...
  - [x] 1.9 Wire preflight as a standalone, migration-free check
    - _Requirements: 1.7_
  - [ ] 1.10 (DEFERRED — see the note at the top) Protocol tests with a fake receiver
...
- [x] 2. Cron slice (smallest self-contained unit)
  ...
  - [ ] 2.5 (DEFERRED — see the note at the top) Implement cron tombstone on source — retained, non-executing, names the target
    - _Requirements: 2.8_
...
- [x] 3. Session slice (extends the existing transfer path)
  ...
  - [x] 3.6 Warn when Layer B is unavailable and the move degrades to transcript-prefix fidelity
    - _Requirements: 5.3_
  - [ ] 3.7 (DEFERRED — see the note at the top) Implement session tombstone — transcript retained and readable, displays new home, refuses new turns
    - _Requirements: 5.9, 5.11_
  - [x] 3.8 Add the "Move to crew…" session action
    - POST /api/chat/slots/{slot}/move plans from the live slot, and SessionActionsMenu offers the action on every surface that renders it
    - The `kirocrew session move` CLI verb of requirement 5.10 is NOT in this slice: with no transfer to perform it could only refuse, so it would be command-group surface that exists to say "not here". It lands with the change that gives it a working action.
    - _Requirements: 5.10 (dashboard half)_
...
- [~] 5. Reversibility, discoverability, and integration _(5.2 partial: Req 7.3 done, Req 7.2 needs a transmit step)_
  - [x] 5.1 Make a migrated unit migratable again from its new owner, including back to the original crew
    - _Requirements: 7.4_
  - [ ] 5.2 (DEFERRED — see the note at the top) Surface the unit's target identity on success, and make the tombstone discoverable from the surface that listed the unit before the move
  ...
  - [x] 5.4 Documentation: add a migration section to the instances module spec and the CLI reference
    ...
    - `docs/system-specs/modules/cli.md` command table: the two move verbs that print a plan (`cron move`, `taskrun move`)

## Verification

Per `Makefile` and `conftest.py`:

- Full suite: `make test` (builds first), or `pytest -q` against the built venv
- Targeted: `pytest -q test/test_<name>.py`
- Tests live flat in `test/` under a root `conftest.py`;
